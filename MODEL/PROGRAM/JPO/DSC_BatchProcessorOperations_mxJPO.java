...
public class DSC_BatchProcessorOperations_mxJPO {
    ...
    public void doBatchprocessorRelatedOperations(Context context, String[] args) throws Exception {
        ...
        while (objItr.next()) {
            ...
            /*
             * if(!_util.isMajorObject(context, bus.getObjectId()) && !_generalUtil.isBusObjectFinalized(context, bus)) addToRevisionsList = true; else if(_util.isMajorObject(context,
             * bus.getObjectId()) && _generalUtil.isBusObjectFinalized(context, bus)) // _globalConfig.isMajorType(bus.getTypeName()) // [NDM] OP6 addToRevisionsList = true;
             */
    ...
    private String createMessageBody(Context context, String integrationName, String messageName, BusinessObject renamedMajorObject, Hashtable revisionInfoTable, Hashtable assemblyRelationshipTable,
            DSC_GetBatchProcessorDetails_mxJPO batchProcessorDetails, String personSiteName) throws Exception {
    ...
    private void addToAssemblyRelationshipTable(Context context, BusinessObject bus, Hashtable assemblyRelationshipTable) throws Exception {
        ...
        while (assemblyRelsList.hasMoreElements()) {
            ...
            while (itr.next()) {
                ...
                /*
                 * if(!_util.isMajorObject(context, busId) && !_generalUtil.isBusObjectFinalized(context, associatedBus)) addToTable = true; else if(_util.isMajorObject(context, busId) &&
                 * _generalUtil.isBusObjectFinalized(context, associatedBus))//_globalConfig.isMajorType(associatedBus.getTypeName()) // [NDM] Op6 addToTable = true;
                 */
