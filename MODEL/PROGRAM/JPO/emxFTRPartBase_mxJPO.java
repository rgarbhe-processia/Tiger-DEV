 ...
 * only and does not evidence any actual or intended publication of such program
 */
...
public class emxFTRPartBase_mxJPO extends emxPLCPart_mxJPO {
    ...
    public MapList getPartsForSelectedFeatures(Context context, String args[]) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        String strParentId = (String) programMap.get("objectId");
        MapList masterPartList = expandForParts(context, args, DomainConstants.EMPTY_STRING);
        ...
        for (int i = 0; i < lengthArr; i++) {
            ...
            MapList partList = expandForParts(context, arg, DomainConstants.EMPTY_STRING);
     ...
     * @throws Exception
     */
    public List getGroupNumber(Context context, String args[]) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
     ...
     * @throws Exception
     */
    public List getGBOMCheckBox(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        for (int i = 0; i < lstObjectIdsList.size(); i++) {
            ...
            strTypeChooser
                    .append("<input type=\"checkbox\" id=\"" + part + "\" value=\"" + relID + "|" + part + "\" onclick=\"doCheckboxClick(this); doSelectAllCheck(this)\" name=\"emxTableRowIds\"  >");
        ...
        try {
            while (expressionTokenZ.hasMoreTokens()) {
                ...
                if (strToken.trim().equals("(")) {
                    ...
                    if (stackOperator.get(stackOperator.size()).toString().equals("!") || stackOperator.get(stackOperator.size()).toString().equals("*")
                            || stackOperator.get(stackOperator.size()).toString().equals("+")) {
                        while (!stackOperator.isEmpty() && !(stackOperator.get(stackOperator.size()).toString().equals("("))) {
                            strSubTokenOp = (String) stackOperator.pop();
                            expPostFixExp.push(strSubTokenOp);
                        }
    ...
    private boolean isExpressionDuplicate(Map mapOutter, Map mapInner) {
        ...
        if (mapOutter != null && mapInner != null) {
            ...
            if (hasInclusionRuleForOuter && hasInclusionRuleForInner) {
                ...
                if ((isRuleSimpleOrComplex(mapOutter) && isRuleSimpleOrComplex(mapOutter)) || (!isRuleSimpleOrComplex(mapOutter) && !isRuleSimpleOrComplex(mapOutter))) {
     ...
     * @since R212
     */
    public MapList getDuplicatePartsForSelectedFeature(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains the feature object ID
     * @returns Boolean
     * @throws Exception
     *             if the operation fails
     * @since FTR R207
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains the given feature object ID
     * @returns StringList
     * @throws Exception
     *             if the operation fails
     * @since FTR R207
    ...
    public static StringList getSelfOwnedOpenReportedAgainstECs(Context context, String[] args) throws Exception {
        ...
        MapList relatedReportedAgainstECsML = featureObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_REPORTED_AGAINST_EC, DomainConstants.TYPE_ENGINEERING_CHANGE, objectSelects, null,
                getTo, getFrom, sRecurseLevel, "", "", 0);
        ...
        MapList allOpenReportedAgainstECsML = DomainObject.findObjects(context, DomainConstants.TYPE_ENGINEERING_CHANGE, DomainConstants.QUERY_WILDCARD, DomainConstants.QUERY_WILDCARD,
    ...
    /**
     * This method will be used to get the Inactive GBOM connected with the context Object inactive due to Part.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public MapList getInactiveGBOMStructure(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String strParentId = (String) programMap.get("objectId");
        ...
        String relWhere = "attribute[" + ConfigurationConstants.ATTRIBUTE_INACTIVE_FROM_VARIANT + "] == No";
        ...
        MapList objectList = compFtr.getInactiveGBOMStrucure(context, typePattern, relPattern, objSelects, relSelects, false, true, iLevel, 0, objWhere, relWhere, DomainObject.FILTER_STR_AND_ITEM,
                filterExpression);
        return objectList;
    }
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getVariantInactiveGBOMStrucure(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String strParentId = (String) programMap.get("objectId");
        LogicalFeature compFtr = new LogicalFeature(strParentId);
        // Where Condition to retrieve GBOMs with Inactive From Variant as Yes
        String relWhere = "attribute[" + ConfigurationConstants.ATTRIBUTE_INACTIVE_FROM_VARIANT + "] == Yes";
        String objWhere = DomainObject.EMPTY_STRING;
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getActiveGBOMStructure(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String strParentId = (String) programMap.get("objectId");
        String strProdId = (String) programMap.get("prodId");
        ...
        String sNameFilterValue = (String) programMap.get("FTRLogicalFeatureGBOMNameFilterCommand");
        String objWhere = DomainObject.EMPTY_STRING;
        ...
        // retrieve Active Inactive GBOM
        MapList objectList = logicalFTR.getActiveGBOMStructure(context, typePattern, relPattern, objSelects, relSelects, false, true, 1, 0, objWhere, relWhere, DomainObject.FILTER_STR_AND_ITEM,
                filterExpression);
     ...
     * @return
     * @since R211 TODO-- Need to revisit for performance
     */
    ...
    public List getGBOMStatus(Context context, String args[]) throws Exception {
        ...
        for (int i = 0; i < lstObjectIdsList.size(); i++) {
            ...
            if (strRelationship.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_GBOM))
                ...
                status = EnoviaResourceBundle.getProperty(context, SUITE_KEY, PART_ACTIVE_STATUS, strLocale);
            else
                status = EnoviaResourceBundle.getProperty(context, SUITE_KEY, PART_INACTIVE_STATUS, strLocale);
...

    {
        ...
        for (int i = 0; i < lstObjectIdsList.size(); i++) {
            ...
            String inactivedvselectable = "tomid[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY_GBOM + "].fromrel.to.name";
    ...
    public static Boolean isLogicalFeatureConnectedToVariants(Context context, String[] args) throws Exception {
        ...
        String strLeftFeatureType = domObject.getInfo(context, DomainObject.SELECT_TYPE);
        // in case if called from Product context- do not need to create IR if it has DV
        if (mxType.isOfParentType(context, strLeftFeatureType, ConfigurationConstants.TYPE_PRODUCTS)) {
            return true;
        }
    ...
    /**
     * To obtain the list of Object IDs to be excluded from the search for Add Existing Actions for Part Family
     * @param context
     *            - the eMatrix <code>Context</code> object
     * @param args
    ...
    public StringList excludeConnectedPartfamilies(Context context, String[] args) throws Exception {
        ...
        StringList objSelects = new StringList();
        MapList objectList = logicalFTR.getActiveGBOMStructure(context, typePattern, relPattern, objSelects, relSelects, false, true, 1, 0, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING,
    ...
    public static Boolean showGBOMReplaceCommand(Context context, String[] args) throws Exception {
        ...
        String stateRelease = PropertyUtil.getSchemaProperty(context, "policy", ConfigurationConstants.POLICY_LOGICAL_FEATURE, "state_Release");
        String stateObsolete = com.matrixone.apps.domain.util.PropertyUtil.getSchemaProperty(context, "policy", ConfigurationConstants.POLICY_LOGICAL_FEATURE, "state_Obsolete");
    ...
    public boolean isAuthorizingECRequired(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String stateRelease = com.matrixone.apps.domain.util.PropertyUtil.getSchemaProperty(context, "policy", ConfigurationConstants.POLICY_LOGICAL_FEATURE, "state_Release");
            String updateReleasedFeatureEnabled = EnoviaResourceBundle.getProperty(context, "emxConfiguration.GBOMPartUpdateAllowedForReleasedFeature");
    ...
    public String getGBOMReplaceRangeHref(Context context, String[] args) throws Exception {
        ...
        String strTypes = "";
        if (mxType.isOfParentType(context, sourceType, ConfigurationConstants.TYPE_PART_FAMILY)) {
        ...
        sbBuffer.append("onClick=\"javascript:showChooser('../common/emxFullSearch.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, objectId) + "&excludeOID="
                + XSSUtil.encodeForHTMLAttribute(context, objectId) + "&contextObjectId=" + XSSUtil.encodeForHTMLAttribute(context, contextObjectId) + "&suiteKey=Configuration");
    ...
    public int isPartUsedInBOMCheck(Context context, String[] args) throws Exception {
        ...
        String strSubjectKey = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Alert.UsedInBOMMakeInactive", strLanguage);
        ...
        MapList mapListPC1 = domObjFeature.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCT_CONFIGURATION, ProductLineConstants.TYPE_PRODUCT_CONFIGURATION, objSelects, null, true,
                false, sRecurse, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);
        ...
        MapList mapListLogicalFtrs = domObjFeature.getRelatedObjects(context, strRelPattern, strObjPattern, objSelects, relSelect, true, false, sRecurse, DomainConstants.EMPTY_STRING,
        ...
        for (int i = 0; i < lstPCs.size(); i++) {
            ...
            MapList mapListTopLevelParts = new DomainObject(strPC).getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_TOP_LEVEL_PART, ProductLineConstants.TYPE_PART, objSelects, null, false,
                    true, sRecurse, strObjWhere, DomainConstants.EMPTY_STRING, 0);
            if (!mapListTopLevelParts.isEmpty()) {
                ...
                for (int j = 0; j < slTopLevelParts.size(); j++) {
                    MapList mapListParts = new DomainObject((String) slTopLevelParts.get(j)).getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_EBOM, ProductLineConstants.TYPE_PART,
                            objSelects, null, false, true, (short) 0, strObjWhere, DomainConstants.EMPTY_STRING, 0);
     ...
     * @param context
     * @throws Exception
     * @sience R212 Not Used- Can remove this exclude prg
     */

    public StringList excludePartforReplace(Context context, String[] args) throws Exception {
        Map programMap = (Map) JPO.unpackArgs(args);
        // Logical Feature ID
        ...
        // retrieve Active Inactive GBOM
        MapList objectList = compFtr.getGBOMStructure(context, typePattern, relPattern, objSelects, relSelects, false, true, iLevel, 0, objWhere, relWhere, DomainObject.FILTER_STR_AND_ITEM,
                filterExpression);
    ...
    public String getGBOMAuthorizingECRangeHref(Context context, String[] args) throws Exception {
        ...
        sbBuffer.append(
                ":CURRENT!=policy_EngineeringChangeStandard.state_Complete,policy_EngineeringChangeStandard.state_Close,policy_EngineeringChangeStandard.state_Reject,policy_SoftwareEC.state_Complete,policy_SoftwareEC.state_Close,policy_SoftwareEC.state_Reject,policy_HardwareEC.state_Cancelled");
        sbBuffer.append("&table=PLCSearchPartsTable");
        sbBuffer.append("&Registered Suite=Configuration");
        sbBuffer.append("&selection=single");
     ...
     * @sience R212
     */
    public List getGroupNumberList(Context context, String args[]) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        StringList data = new StringList();
        ...
        try {
            ...
            for (int i = 0; i < lstObjectIdsList.size(); i++) {
                ...
                if (strObjId != null) {
                    ...
                    mlRelObj = domObj.getRelatedObjects(context, strRelPattern, strTypePattern, slTempId, slTempRel, true, false, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING,
                            0);
                    if (mlRelObj.size() > 0) {
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getDuplicatePartsForMergeReplace(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public MapList getPartsForSelectedLogicalFeatures(Context context, String args[]) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @throws Exception
     */
    public MapList getDuplicatePartsForMergeReplace(Context context, String[] args, MapList mapParts) throws Exception {
        try {
            ...
            for (int i = 0; i < mapParts.size(); i++) {
                ...
                if (mapPart.containsKey("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]")) {
                    strREList = (String) mapPart.get("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]");
                    mapPart.put("RightExpressionAttr", strREList.toString());

                } else
                    mapPart.put("RightExpressionAttr", DomainConstants.EMPTY_STRING);

                String strPrtName = (String) mapPart.get("name");
                if (mapPart.containsKey("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RULE_COMPLEXITY + "]")) {
                    strRuleComplexity = (String) mapPart
                            .get("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RULE_COMPLEXITY + "]");
                    mapPart.put("RuleComplexity", strRuleComplexity);

                } else
                    mapPart.put("RuleComplexity", DomainConstants.EMPTY_STRING);
                if (mapPart.containsKey("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS + "]")) {
                    strDVList = (String) mapPart.get("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS + "]");
                    mapPart.put("DesignVariantsAttr", strDVList.toString());

                } else
                    mapPart.put("DesignVariantsAttr", DomainConstants.EMPTY_STRING);

                mapPart.put(DomainConstants.SELECT_NAME, strPrtName);

                String strIncRuleId = (String) mapPart.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.id");
    ...
    public Object getGBOMInclusionRuleLink(Context context, String[] args) throws Exception {
        HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
        MapList objectList = (MapList) parametersMap.get("objectList");
        String strRuleType = "";
        Map objectMap;
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            MapList mapListDV = confUtil.getObjectStructure(context, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES, ConfigurationConstants.RELATIONSHIP_VARIES_BY, objSelects, relSelects, false,
                    true, 1, 0, null, null, (short) 0, null);
            String strLocale = context.getSession().getLanguage();
     ...
     * @throws Exception
     */
    public List getGroupNumberGBOMtable(Context context, String args[]) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        StringList data = new StringList();
    ...
    public MapList getUpdatedGBOMListPostMerge(Context context, String args[]) throws Exception {
        ...
        for (int j = 0; j < listUpdatedParts.size(); j++) {
            ...
            String strCommittedREAttrVal = (String) mpPart
            ...
            if (oldRightExpressionAttr != null) {
                ...
                while (tokenizeRTExp.hasMoreTokens()) {
                    ...
                    if (!rightExpressionOldPhyID.equalsIgnoreCase("AND")) {
                        ...
                        StringList selectableList = new StringList(DomainRelationship.SELECT_TYPE);
                        selectableList.add("from.id");
                        selectableList.add("torel.to.id");
                        MapList resultList = DomainRelationship.getInfo(context, relIdList, selectableList);
                        Map mpREToDetails = (Map) resultList.get(0);
                        String strRelName = mpREToDetails.get(DomainRelationship.SELECT_TYPE).toString();
                        if (strRelName.equals(ConfigurationConstants.RELATIONSHIP_CONFIGURATION_OPTIONS) || strRelName.equals(ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES)) {
                            String dvID = mpREToDetails.get("from.id").toString();
                            mlDVwithOldValue.put(dvID, rightExpressionOldPhyID);
                        } else {
                            // TODO CG Case
                            String dvID = mpREToDetails.get("torel.to.id").toString();
                            mlDVwithOldValue.put(dvID, rightExpressionOldPhyID);
                        }
                    }
                }
    ...
    public HashMap getPartFamilyChooser(Context context, String[] args) throws Exception {
        ...
        MapList partFamilyList = DomainObject.findObjects(context, ConfigurationConstants.TYPE_PART_FAMILY, ConfigurationConstants.QUERY_WILDCARD, ConfigurationConstants.EMPTY_STRING, strPFSelects);

        for (Iterator pfIttr = partFamilyList.iterator(); pfIttr.hasNext();) {
    ...
    public Vector getPartUsageForDisplay(Context context, String args[]) throws Exception {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (strRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_GBOM) || ProductLineCommon.isOfParentRel(context, strRelType, ConfigurationConstants.RELATIONSHIP_GBOM)
                    || strRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_INACTIVE_GBOM)
                    || ProductLineCommon.isOfParentRel(context, strRelType, ConfigurationConstants.RELATIONSHIP_INACTIVE_GBOM)) {
    ...
    /**
     * It is a check trigger method. Fires when a relationship GBOM is getting created It is used to check the cyclic condition
     * @param context
     *            The ematrix context object.
     * @param String
     *            [] The args .
    ...
    public Vector renderActionIconsOnGBOM(Context context, String[] args) throws FrameworkException {
        try {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                String strTypes = "";
                String sourceType = "";
                String policyType = "policy_ConfiguredPart";
                String partState = "policy_DevelopmentPart.state_Obsolete,policy_StandardPart.state_Obsolete,policy_Classification.state_Obsolete,policy_Product.state_Obsolete";
                ...
                if (!contextState.equalsIgnoreCase(strStateObsolete)) {
                    ...
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strCustomInactiveProcessJsp) + "?mode=CustomGBOMInactive");
                    strBufferIcons.append("%26objectId=");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strobjectId));
                    strBufferIcons.append("%26parentOID=");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, parentOID));
                    strBufferIcons.append("%26relId=");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strConnectionId));
                    strBufferIcons.append("%26timestamp=");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strTimeStamp));
                    strBufferIcons.append("%26targetLocation=");
                    strBufferIcons.append(
                            XSSUtil.encodeForHTMLAttribute(context, strCustomInactiveTargetLoc) + "' ," + "'InactiveGBOM'" + " ,'" + XSSUtil.encodeForHTMLAttribute(context, strConnectionId) + "'");
                    strBufferIcons.append(")\"><img border=\"0\" src=\"images/" + strCustomInactiveIconImage + "\"");
                    strBufferIcons.append(" TITLE=\"");
                    strBufferIcons.append(XSSUtil.encodeForXML(context,
                            EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.ToolTip.LogicalFeature_GBOM_Inactive", context.getSession().getLanguage())));
                    strBufferIcons.append("\"");
                    ...
                    strBufferIcons.append("<a href=\"javascript:removeActionIcon('../configuration/");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strCustomProcessJsp) + "?mode=CustomGBOMDisconnect");
                    strBufferIcons.append("%26objectId=");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strobjectId));
                    strBufferIcons.append("%26parentOID=");
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, parentOID));
                    ...
                    strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strTimeStamp));
                    strBufferIcons.append("%26targetLocation=");
                    strBufferIcons.append(strCustomTargetLoc + "' ," + "'RemoveGBOM'" + " ,'" + XSSUtil.encodeForHTMLAttribute(context, strConnectionId) + "'");
                ...
                if (repalce) {
                    ...
                    if (!contextState.equalsIgnoreCase(strStateRelease)) {
                        strBufferIcons.append("<a href=\"javascript:showModalDialog('../common/emxFullSearch.jsp?objectId=" + strobjectId + "%26excludeOID=" + strobjectId + "%26contextObjectId="
                                + parentOID + "%26suiteKey=Configuration");
                        ...
                        strBufferIcons.append(" TITLE=\"");
                        strBufferIcons.append(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.ToolTip.LogicalFeature_GBOM_Replace", context.getSession().getLanguage()));
                        strBufferIcons.append("\"");
                        strBufferIcons.append("/>");
                        strBufferIcons.append("</a>");
                    } else {
                        strBufferIcons.append(
                                "<a href=\"javascript:showModalDialog('../common/emxForm.jsp?form=FTRGBOMReplaceForm%26mode=Edit%26formHeader=emxProduct.Heading.ReplaceGBOM%26suiteKey=Configuration%26HelpMarker=emxhelpgbomreplace%26postProcessURL=../configuration/GBOMReplacePostProcess.jsp%26submitAction=doNothing%26mode1=fromReleaseContext%26relId=");
                        strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strConnectionId));
                        strBufferIcons.append("%26suiteKey=configuration%26SuiteDirectory=configuration%26parentOID=");
                        strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, parentOID));
                        strBufferIcons.append("%26timestamp=");
                        ...
                        strBufferIcons.append("%26objectId=");
                        strBufferIcons.append(XSSUtil.encodeForHTMLAttribute(context, strobjectId));
                        strBufferIcons.append("', '800', '700', 'true', 'popup', '')\"><img border=\"0\" src=\"images/" + strCustomIconImage + "\"");
                        strBufferIcons.append(" TITLE=\"");
                        strBufferIcons.append(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.ToolTip.LogicalFeature_GBOM_Replace", context.getSession().getLanguage()));
                        strBufferIcons.append("\"");
                        strBufferIcons.append("/></a>");
    ...
    public Vector renderActionIconsOnInactiveGBOM(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                strBufferIcons.append(" TITLE=\"");
                strBufferIcons.append(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.ToolTip.LogicalFeature_GBOM_Active", context.getSession().getLanguage()));
                strBufferIcons.append("\"");
                strBufferIcons.append("/></a>");
                actionIcons.add(strBufferIcons.toString());
            }
    ...
    public StringList includePCBasedOn(Context context, String[] args) throws Exception {
        ...
        mapProductObjsList = domObjFeature.getRelatedObjects(context,
                ConfigurationConstants.RELATIONSHIP_PRODUCT_CONFIGURATION + "," + ConfigurationConstants.RELATIONSHIP_FEATURE_PRODUCT_CONFIGURATION, ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION,
                objSelects, null, false, true, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);
