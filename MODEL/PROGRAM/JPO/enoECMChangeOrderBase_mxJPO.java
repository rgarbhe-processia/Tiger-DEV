...
/**
 * The <code>enoECMChangeOrderBase</code> class contains methods for executing JPO operations related to objects of the admin type Change. @author/R3D
 * @version Copyright (c) 1993-2015 Dassault Systemes. All Rights Reserved.
 */
public class enoECMChangeOrderBase_mxJPO extends emxDomainObject_mxJPO {

    /*********************************
     * MAP SELECTABLES/********************************* /** A string constant with the value "field_Choices".
     */
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (!context.isConnected()) {
            ...
            String strContentLabel = EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(), "EnterpriseChangeMgt.Error.UnsupportedClient");
    ...
    /**
     * @author Updates the Reported Against field in CO WebForm.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * @author Updates the Responsible Organisation in CO WebForm.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a MapList with the following as input arguments or entries: objectId holds the context CO object Id
     * @throws Exception
     *             if the operations fails
     * @since ECM-R211
     */
    ...
    /**
     * @author Generates dynamic query for Change Coordinator field
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    /**
     * @author Method for including Review Route templates owned by context user
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    public Object includeReviewRouteTemplates(Context context, String[] args) throws Exception {
        String objWhere = "attribute[" + ATTRIBUTE_ROUTE_BASE_PURPOSE + "]== Review && current == Active && revision==last";
    ...
    /**
     * @author Method for including Approval Route templates
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    public Object includeApprovalRouteTemplates(Context context, String[] args) throws Exception {
        String objWhere = "attribute[" + ATTRIBUTE_ROUTE_BASE_PURPOSE + "]== Approval && current == Active && revision==last";
        MapList routeTemplateList = (MapList) changeOrderUI.getRouteTemplates(context, objWhere);
        return changeUtil.getStringListFromMapList(routeTemplateList, SELECT_ID);
    }

    /**
     * @author Generates dynamic query for Tech/Sr Tech Assignee Chooser fields from CO affected items table.
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    /**
     * @author Generates dynamic query for Review Route Template field
     * @param context
     * @param args
     * @return String
     * @throws Exception
     */
    public String getTechnicalReviewerListDynamicSearchQuery(Context context, String[] args) throws Exception {
        // unpacking the Arguments from variable args
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        Map requestMap = (HashMap) programMap.get(ChangeConstants.REQUEST_MAP);
        String changeObjID = (String) requestMap.get(ChangeConstants.OBJECT_ID);
        Map typeAheadMap = (HashMap) programMap.get("typeAheadMap");
        String RTOId = "";
        String techRole = "";
        if (typeAheadMap != null & typeAheadMap.size() > 0) {
            StringList list = getTypeAndPolicy(context, typeAheadMap);
            String ID = (String) list.get(0);
            String TYPE = (String) list.get(1);
            String POLICY = (String) list.get(2);

            String rtoExpr = ECMAdmin.getRTOExpression(context, TYPE, POLICY);
            RTOId = DomainObject.newInstance(context, ID).getInfo(context, rtoExpr);
        }
        if (changeUtil.isNullOrEmpty(RTOId)) {
            RTOId = ChangeUtil.getRtoIdFromName(context, DomainObject.newInstance(context, changeObjID).getInfo(context, SELECT_ORGANIZATION));
        }
        return "TYPES=type_RouteTemplate:REL_ROUTETEMPLATE_OWNINGORGANIZATION_ID=" + RTOId + ":ROUTE_BASE_PURPOSE=Review:CURRENT=policy_RouteTemplate.state_Active:LATESTREVISION=TRUE";
    }

    /**
     * @author Generates dynamic query for Tech Role Column
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    /**
     * @author Generates dynamic query for Senior Tech Role Column
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    private StringList getTypeAndPolicy(Context context, Map fieldValuesMap) throws Exception {
        String relId = (String) fieldValuesMap.get("rowRelId");
        String result = MqlUtil.mqlCommand(context, "print connection $1 select $2 $3 $4 dump", relId, "to.id", "to.type", "to.policy");

        return FrameworkUtil.split(result, ChangeConstants.COMMA_SEPERATOR);
    }

    /**
     * @author Generates dynamic search types for Reported Against field
     * @param context
     * @param args
     * @return String
     * @throws Exception
     ...
     * @return HashMap containing the actual and display values of policies
     * @throws Exception
     *             if the operation fails
     */
    ...
    public HashMap getPolicies(Context context, String[] args) throws Exception {
        ...
        return changeOrderUI.getPolicies(context, type, ChangeConstants.CHANGE_POLICY_DEFAULT,
                (ChangeConstants.FOR_RELEASE.equalsIgnoreCase(functionality) || ChangeConstants.FOR_OBSOLETE.equalsIgnoreCase(functionality) || isconnectedtoCR));
    }

    /**
     * @author Generates dynamic query for new owner field in transfer ownership form
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    public String getSearchQueryForTransferOwnership(Context context, String[] args) throws Exception {
        ...
        return "TYPES=type_Person:MEMBER_ID=" + strResponsibleOrg + ":USERROLE=role_ChangeCoordinator,role_VPLMProjectLeader";
    }
    ...
    /**
     * @author This method is called from update program of CO, Create/Edit, Reviewer/Approval List fields.
     * @param context
     *            ematrix context.
     * @param args
     *            holds a Map with the following input arguments.
     * @throws Exception
    ...
    public void updateReviewRouteObject(Context context, String[] args) throws Exception {
        ...
        strNewToTypeObjId = (ChangeUtil.isNullOrEmpty(strNewToTypeObjId)) ? (String) paramMap.get(ChangeConstants.NEW_VALUE) : strNewToTypeObjId;

        strOldToTypeObjId = (ChangeUtil.isNullOrEmpty(strOldToTypeObjId)) ? (String) paramMap.get(ChangeConstants.OLD_VALUE) : strOldToTypeObjId;
    ...
    /**
     * @author Updates the Distribution List field in CO WebForm.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: object Id object Id of context CO. New Value object Id of updated Distribution List Object
     * @throws Exception
     *             if the operations fails
     * @since ECM-R211
     */
    ...
    public MapList getAffectedItems(Context context, String[] args) throws Exception {
        ...
        return changeOrder.getAffectedItems(context);
    }
    ...
    public MapList getAffectedChangeActions(Context context, String[] args) throws Exception {
        ...
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        // Map paramMap = (HashMap) programMap.get(ChangeConstants.PARAM_MAP);
        // PCM :TIGTK-5755 :Rutuja Ekatpure:start
        String strParentId = (String) programMap.get("objectId");
        // PCM :TIGTK-5755 :Rutuja Ekatpure:End
    ...
    /**
     * @author this method performs the cancel process of change - The Affected CAs,Affected Items, Routes,Reference Documents,Prerequisites Connected to this Particular CO are Disconnected and
     *         finally change promoted to cancel state.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
...

    {
        ...
        String objectId = changeUtil.isNullOrEmpty((String) paramMap.get(ChangeConstants.OBJECT_ID)) ? (String) requestMap.get(ChangeConstants.OBJECT_ID)
                : (String) paramMap.get(ChangeConstants.OBJECT_ID);
    ...
    /**
     * Disconnect Route, Route Templates, Distribution List, CA
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the Change Process
     ...
     * @since ECM R211.
     */
    public void disconnectLists(Context context, String objectId) throws Exception {
        changeOrder.setId(objectId);
        StringList sRelSelectList = new StringList();
        sRelSelectList.addElement(DomainRelationship.SELECT_ID);
        MapList sObjList = changeOrder.getRelatedObjects(context, RELATIONSHIP_OBJECT_ROUTE + "," + RELATIONSHIP_EC_DISTRIBUTION_LIST,
                TYPE_ROUTE + "," + TYPE_ROUTE_TEMPLATE + "," + ChangeConstants.TYPE_MEMBER_LIST, null, sRelSelectList, true, // getTo
                true, // getFrom
                (short) 1, null, null);
        StringList sRelIdList = new ChangeUtil().getStringListFromMapList(sObjList, DomainRelationship.SELECT_ID);
        DomainRelationship.disconnect(context, (String[]) sRelIdList.toArray(new String[0]));
    }

    /**
     * @author this method performs the hold process of change.Moves all associated CAs to hold state.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the Change Process
    ...
    /**
     * @author Resumes the Hold Changes and sends notification and updates the history
     * @param context
     * @throws Exception
     */
    ...
    /**
     * @author Method to return prerequisites for Change
     * @param context
     * @param args
     * @return
     * @throws Exception
    ...
    /**
     * Gets all the persons with which this Change object is connected with Assignee relationship
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing one String entry for key "objectId"
    ...
    public String getApprovalTasksOnChange(Context context, String[] args) throws Exception {
        ...
        taskMapList = this.getValidTaskList(context, taskmpList, objectId);
                ...
                if (UIUtil.isNotNullAndNotEmpty(strRouteTaskUser) && strRouteTaskUser.startsWith("role_")) {
                    ...
                    if (isToBeAccepted) {
                        ...
                        if (strType.equalsIgnoreCase(TigerConstants.TYPE_PSS_MANUFACTURINGCHANGEACTION)) {
                            ...
                            if (isPersonProgamMember && isPersonPlantMember)
     ...
     * @param args
     *            packed hashMap of request parameter
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList containing "objectId"
     * @return a <code>MapList</code> object having the list of Assignees,their relIds and rel names for this Change Object
     * @throws Exception
     *             if the operation fails
     * @since ECM R211
    ...
    public StringList getRouteTaskAssignedCOs(Context context, String personObjId) throws Exception {
        ...
        String objSelect = "to[" + RELATIONSHIP_PROJECT_TASK + "].from." + "from[" + RELATIONSHIP_ROUTE_TASK + "].to." + "to[" + RELATIONSHIP_OBJECT_ROUTE + "|from.type=='"
                + ChangeConstants.TYPE_CHANGE_ORDER + "'].from.id";
    ...
    public StringList getRouteTemplateAssignedCOs(Context context, String personObjId) throws Exception {
        ...
        String objSelect = "to[" + RELATIONSHIP_ROUTE_NODE + "|from.type=='" + TYPE_ROUTE_TEMPLATE + "']." + "from.to[" + RELATIONSHIP_INITIATING_ROUTE_TEMPLATE + "].from." + "to["
                + RELATIONSHIP_OBJECT_ROUTE + "|from.type=='" + ChangeConstants.TYPE_CHANGE_ORDER + "'].from.id";
    ...
    public Object getOwnedCO(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            MapList ownedCO = DomainObject.findObjects(context, ChangeConstants.TYPE_CHANGE_ORDER, // type filter
    ...
    /**
     * Program returns MapList of CA
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList ontaining objectID
     * @return a <code>MapList</code> object having the list of Assignees,their relIds and rel names for this Change Object
     * @throws Exception
     *             if the operation fails
     * @since ECM R211
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList expandProgramCO(Context context, String args[]) throws Exception {
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            ...
            sb.append(ChangeConstants.RELATIONSHIP_CHANGE_ACTION);
            sb.append("].to.id");
            ...
            if (!UIUtil.isNullOrEmpty(objectId)) {
                ...
                if (dmObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ORDER)) {
                    sConnectedItems = dmObj.getRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_ACTION, ChangeConstants.TYPE_CHANGE_ACTION, busSelects, null, false, true, (short) 1,
                            EMPTY_STRING, EMPTY_STRING);
                } else {
                    sConnectedItems = dmObj.getRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM, QUERY_WILDCARD, busSelects, null, false, true, (short) 1, EMPTY_STRING,
                            EMPTY_STRING);

                }
            }
            if (!sConnectedItems.isEmpty())
                return sConnectedItems;
    ...
    public StringList getRouteTaskAssignedCAs(Context context, String personObjId) throws Exception {
        ...
        String objSelect = "to[" + RELATIONSHIP_PROJECT_TASK + "].from." + "from[" + RELATIONSHIP_ROUTE_TASK + "].to." + "to[" + RELATIONSHIP_OBJECT_ROUTE + "|from.type=='"
                + ChangeConstants.TYPE_CHANGE_ACTION + "'].from.id";
    ...
    public StringList getRouteTemplateAssignedCAs(Context context, String personObjId) throws Exception {
        ...
        String objSelect = "to[" + RELATIONSHIP_ROUTE_NODE + "|from.type=='" + TYPE_ROUTE_TEMPLATE + "']." + "from.to[" + RELATIONSHIP_INITIATING_ROUTE_TEMPLATE + "].from." + "to["
                + RELATIONSHIP_OBJECT_ROUTE + "|from.type=='" + ChangeConstants.TYPE_CHANGE_ACTION + "'].from.id";
    ...
    public Object getOwnedCAs(Context context, String args[]) throws Exception {
        ...
        try {
            StringList objectSelects = new StringList(6);
            objectSelects.add(SELECT_ID);
            String objectWhere = "from[Technical Assignee].to.name == \"" + context.getUser() + "\" || owner == \"" + context.getUser() + "\" || from[Senior Technical Assignee].to.name==\""
                    + context.getUser() + "\"";

            MapList ownedCO = DomainObject.findObjects(context, ChangeConstants.TYPE_CHANGE_ACTION, // type filter
                    QUERY_WILDCARD, // vault filter
                    objectWhere, // where clause
                    objectSelects); // object selects
            return new ChangeUtil().getStringListFromMapList(ownedCO, "id");
     ...
     */
    /*
     * public StringList getAssigneeCAs(Context context,String personObjectId) throws Exception { MapList listCA = new MapList(); try{ StringList busSelects = new StringList();
     * busSelects.add(SELECT_ID); DomainObject dmObj = DomainObject.newInstance(context); dmObj.setId(personObjectId); listCA = dmObj.getRelatedObjects(context, RELATIONSHIP_TECHNICAL_ASSIGNEE+","
     * +RELATIONSHIP_SENIOR_TECHNICAL_ASSIGNEE, TYPE_CHANGE_ACTION, busSelects, null, true, false, (short)1, EMPTY_STRING, EMPTY_STRING);
     * 
     * return new ChangeUtil().getStringListFromMapList(listCA, "id"); }catch (Exception e) { throw e; }
     * 
     * }
     */

    /**
     * Method to list all the "Change Templates
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList ontaining objectID
     ...
     * @since ECM R211
     */
    public MapList getMyChangeTemplates(Context context, String args[]) throws Exception {
        MapList listCT = null;
        try {
            String typePattern = PropertyUtil.getSchemaProperty(context, "type_ChangeTemplate");
            StringList busSelects = new StringList();
            busSelects.add(SELECT_ID);

            String whereClause = DomainObject.SELECT_OWNER + "==" + "\'" + context.getUser() + "\'";

            listCT = DomainObject.findObjects(context, typePattern, // type filter
                    QUERY_WILDCARD, // vault filter
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getCAExpandProgram(Context context, String args[]) throws Exception {
        MapList sConnectedItems = null;
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            String objectId = (String) paramMap.get("objectId");
            ...
            if (!UIUtil.isNullOrEmpty(objectId)) {
                ...
                if (dmObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)) {
                    sConnectedItems = dmObj.getRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM, QUERY_WILDCARD, selectList, null, false, true, (short) 1, EMPTY_STRING,
                            EMPTY_STRING);
                } else {
                    sConnectedItems = dmObj.getRelatedObjects(context, RELATIONSHIP_PART_FAMILY_MEMBER, // check
                            QUERY_WILDCARD, selectList, null, false, true, (short) 1, EMPTY_STRING, EMPTY_STRING);
                }
            }
    ...
    /**
     * Program to get Approval Status For CO/CA ------- refer - emxLifecycle:getApprovalStatusForTaskSignatures
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - Object
     ...
     * @since ECM R211
     */
    public Vector getApprovalStatusForMyChanges(Context context, String[] args) throws Exception {
        try {

            String STRING_AWAITING_APPROVAL = "Awaiting Approval";
            String STRING_ACCEPT = "Pending ";
            String typeCO = ChangeConstants.TYPE_CHANGE_ORDER;
            ...
            strAcceptLink.append("<a target=\"listHidden\" href=\"../components/emxRouteAcceptTask.jsp?taskId=${OBJECT_ID}\">");
            ...
            // Form the Approve link template
            StringBuffer strTaskApproveLink = new StringBuffer(64);
            strTaskApproveLink.append("<a target=\"listHidden\" href=\"../common/emxLifecycleApproveRejectPreProcess.jsp?");
            strTaskApproveLink.append("\"><img border='0' src='../common/images/iconActionApprove.gif' title='Awaiting Approval' />");
            strTaskApproveLink.append(STRING_AWAITING_APPROVAL);
            strTaskApproveLink.append("</a>");

            // Do for each object --- Logic for showing status
            for (Iterator itrObjects = objectList.iterator(); itrObjects.hasNext();) {
                mapObjectInfo = (Map) itrObjects.next();
                String objectID = (String) mapObjectInfo.get(ChangeConstants.ID);
                ...
                StringList taskList = obj.getInfoList(context, "from[" + RELATIONSHIP_OBJECT_ROUTE + "].to.relationship[" + RELATIONSHIP_ROUTE_TASK + "].from.name");

                if (obj.isKindOf(context, typeCO) && taskList.size() == 0) {
                    // vecResult.add(strAcceptLink.toString());
                    vecResult.add(strTaskApproveLink.toString());
                } else {
                    vecResult.add("-");
                }
    ...
    public Vector showEditIconforStructureBrowser(Context context, String args[]) throws FrameworkException {
        ...
        try {
            ...
            String strEditCO = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "EnterpriseChangeMgt.Label.EditCO", context.getSession().getLanguage());
            String strEditCR = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "EnterpriseChangeMgt.Label.EditCR", context.getSession().getLanguage());
            String strEditCA = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "EnterpriseChangeMgt.Label.EditCA", context.getSession().getLanguage());
            ...
            if (!COInfoList.isEmpty()) {
                ...
                while (sItr.hasNext()) {
                    ...
                    if (type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ORDER)) {
                        if ((ChangeConstants.STATE_FORMALCHANGE_PROPOSE.equalsIgnoreCase(current) || ChangeConstants.STATE_FORMALCHANGE_PREPARE.equalsIgnoreCase(current)
                                || ChangeConstants.STATE_FORMALCHANGE_HOLD.equalsIgnoreCase(current)) && (context.getUser().equalsIgnoreCase(owner))) {
                    ...
                    if (type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_REQUEST)) {
                        if ((ChangeConstants.STATE_CHANGEREQUEST_CREATE.equalsIgnoreCase(current) || ChangeConstants.STATE_CHANGEREQUEST_EVALUATE.equalsIgnoreCase(current)
                                || ChangeConstants.STATE_CHANGEREQUEST_HOLD.equalsIgnoreCase(current)) && (context.getUser().equalsIgnoreCase(owner))) {
                            sbEditIcon.append("<a href=\"JavaScript:emxTableColumnLinkClick('");
                    ...
                    if (type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ACTION)) {
                        if ((ChangeConstants.STATE_CHANGE_ACTION_PENDING.equalsIgnoreCase(current) || ChangeConstants.STATE_CHANGE_ACTION_PREPARE.equalsIgnoreCase(current)
                                || ChangeConstants.STATE_CHANGE_ACTION_INWORK.equalsIgnoreCase(current) || ChangeConstants.STATE_CHANGE_ACTION_HOLD.equalsIgnoreCase(current))
                                && (context.getUser().equalsIgnoreCase(owner))) {
                            sbEditIcon.append("<a href=\"JavaScript:emxTableColumnLinkClick('");
                    ...
                    if (!type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ORDER) && !type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_REQUEST)
                            && !type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ACTION)) {
                        sbEditIcon.append("-");
                        columnVals.add(sbEditIcon.toString());
                    }
    ...
    /**
     * Program to Search Change Template while creating CO based on CT
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - Object
    ...
    public StringList getAllTemplateOIds(Context context, String[] args) throws Exception {
        StringList templeateOids = new StringList();
        String sWhereExp = "";

        MapList ChangeTemplateList = new MapList();
        ...
        ChangeTemplateList = DomainObject.findObjects(context, typePattern, // type filter
                QUERY_WILDCARD, // vault filter
                busWhere, // where clause
                busSelects);
        // System.out.println("Inside getAllPQPTemplateOIds method template size" +PQPTemplateList.size());
        if (ChangeTemplateList.size() > 0) {
            Iterator itr = ChangeTemplateList.iterator();
            Map map;
            while (itr.hasNext()) {
                map = (Map) itr.next();
                templeateOids.addElement((String) map.get(DomainObject.SELECT_ID));

            }
        }
        return templeateOids;
    ...
    public HashMap showPolicies(Context context, String[] args) throws Exception {
        ...
        if (fieldValues != null) {
            ...
            String selectType = (String) fieldValues.get("DefaultType");
            ...
            while (itr.hasNext()) {
                ...
                fieldDisplayRangeValues.add(policyName);

            }

            tempMap.put("RangeValues", fieldRangeValues);
            tempMap.put("RangeDisplayValues", fieldDisplayRangeValues);

            return tempMap;
        } else {
            fieldRangeValues.addElement(sFormalPolicy);
            fieldDisplayRangeValues.addElement(localsFormalProcess);
            fieldRangeValues.addElement(sFasttrackPolicy);
            fieldDisplayRangeValues.addElement(localsFasttrack);
    ...
    /**
     * Post Process Method for Change Order Creation
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param String
     *            [] of ObjectIds.
     ...
     * @since ECM R212
     */
    public void connectChangeOrderRelationships(Context context, String args[]) throws Exception {
        ...
        String sChangeTemplateOID = (String) reqMap.get("ChangeTemplateOID");

        String currentCTObjectID = (String) paramMap.get("objectId");

        DomainObject currentChangeOrderObject = new DomainObject(currentCTObjectID); // Change order object
        String sChangeRel = PropertyUtil.getSchemaProperty(context, "relationship_ChangeTemplate");

        if (sChangeTemplateOID != null && !"".equals(sChangeTemplateOID) && !"null".equals(sChangeTemplateOID)) {
            currentChangeOrderObject.addRelatedObject(context, new RelationshipType(sChangeRel), false, sChangeTemplateOID);
        }

    }

    /**
     * Program to get CO Task Names
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - Object
     ...
     * @since ECM R211
     */
    public MapList getCOTaskNames(Context context, String[] args) throws Exception {
        ...
        Object[] coType = (Object[]) requestValuesMap.get("cotype");
        String coId = (String) coType[0];
        StringList cotypesList = FrameworkUtil.split(coId, "|");

        for (int i = 0; i < cotypesList.size(); i++) {
            String objectId = (String) cotypesList.get(i);

            if (!UIUtil.isNullOrEmpty(objectId)) {
                DomainObject obj = new DomainObject(objectId);

                String strTask = obj.getInfo(context, "from[" + RELATIONSHIP_OBJECT_ROUTE + "].to.relationship[" + RELATIONSHIP_ROUTE_TASK + "].from.id");

                Map taskMap = new HashMap();
                if (strTask != null && !"".equals(strTask) && !"null".equals(strTask)) {
                    taskMap.put(ChangeConstants.ID, strTask);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - String containing Service object id.
    ...
    public int checkForAffectedItemConnected(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            String Message = EnoviaResourceBundle.getProperty(context, resourceFieldId, context.getLocale(), propertyKey);
            MapList affectedList = getRelatedObjects(context, relationshipName, DomainConstants.QUERY_WILDCARD, objectSelect, null, false, true, (short) 1, null, null, (short) 0);

            if (affectedList != null && affectedList.size() > 0) {
                if (affectedList.size() == 1) {
                    Map map = (Map) affectedList.get(0);
                    String type = (String) map.get(SELECT_TYPE);

                    if (type.equals(ChangeConstants.TYPE_CCA)) {
                        String ccaId = (String) map.get(SELECT_ID);
    ...
    public int checkIfAffectedItemConnected(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            String Message = EnoviaResourceBundle.getProperty(context, resourceFieldId, context.getLocale(), propertyKey);
            MapList affectedList = getRelatedObjects(context, relationshipName, "*", objectSelects, relSelects, false, true, (short) 1, null, null, (short) 0);

            if ((affectedList != null && affectedList.size() > 0)
                    || (!sOwner.equalsIgnoreCase(context.getUser()) && (ChangeConstants.TYPE_CHANGE_ORDER.equalsIgnoreCase(typeName) || ChangeConstants.TYPE_CHANGE_REQUEST.equalsIgnoreCase(typeName)))
                    || (ChangeConstants.TYPE_CHANGE_REQUEST.equalsIgnoreCase(typeName) && !ChangeConstants.STATE_CHANGEREQUEST_CREATE.equalsIgnoreCase(sCurrent))) {
                Iterator iter = affectedList.iterator();
                while (iter.hasNext()) {
                    Map map = (Map) iter.next();
                    strAffectedItemId = (String) map.get("from[" + ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM + "].to.id");
                    if (UIUtil.isNullOrEmpty(strAffectedItemId)) {
                        strAffectedItemId = (String) map.get("from[" + DomainConstants.RELATIONSHIP_AFFECTED_ITEM + "].to.id");
                    }
    ...
    public int RouteToChangeCoordinator(Context context, String args[]) throws Exception {
        ...
        StringList objSelects = new StringList(2);
        objSelects.addElement(coordinatorSelect);
    ...
    /**
     * This method is used check promote trigger on different states of the policies. checks if the RouteTemplate is attached for context change object or not
     * @author
     * @param context
     * @param args
    ...
    public int checkRouteTemplateForState(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String whrClause = "attribute[" + ATTRIBUTE_ROUTE_BASE_PURPOSE + "] match '" + routeBasePurpose + "' && current == Active";

            // get route template objects from change object
            mapRouteTemplate = getRelatedObjects(context, relationName, typeName, selectStmts, null, false, true, (short) 1, whrClause, null, (short) 0);
    ...
    /**
     * This method retrieves all the routes from Change Object, if no routes exists calls createRoute method which creates route from route template and attaches to Change object and if routes exists
     * means, in further process retrieves all Route Templates from Change Object and for each Route Template retrieves initiated routes, if any of these initiated routes already present in original
     * route list , makes booleanRoutePresent variable to True and skips method call of Create Route
     * @author
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            : 0 - OBJECTID change id 1 - Route Base Purpose The kind of Route (Route Base Purpose) can be Approval,Review,Standard 2 - Relationship Object Route 3 - Relationship Initiating Route
     *            Template 4 - type Route 5 - type Route Template 6 - state In-Review/In-Approve 7 - policy of the change object 8 - target State (In-Review/In-Approval) object state (from state ie
     *            that is the state on which the route will be started)
    ...
    public int createRouteFromRouteTemplate(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String routeTemplateSelect = "from[" + relObjectRoute + "|attribute[" + ATTRIBUTE_ROUTE_BASE_PURPOSE + "] " + "== " + routeBasePurpose + "].to[" + typeRouteTemplate + "].id";
    ...
    public int createRoute(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            templateList = changeObj.getRelatedObjects(context, RELATIONSHIP_OBJECT_ROUTE, TYPE_ROUTE_TEMPLATE, objectSelects, relSelects, false, true, (short) 1, whrClause, null, (short) 0);
            ...
            while (itr.hasNext()) {
                ...
                if (!ChangeUtil.isNullOrEmpty(templateId) && "Active".equals(templateState)) {
                    ...
                    routeObj.addToObject(context, new RelationshipType(DomainConstants.RELATIONSHIP_PROJECT_ROUTE),
                            DomainObject.newInstance(context, PersonUtil.getPersonObjectID(context, changeObjectOwner)).getId(context)); // Added For IR-337468
                    ...
                    MapList routeNodeList = routeObj.getRelatedObjects(context, RELATIONSHIP_ROUTE_NODE, nodeTypePattern, null, relProductSelects, false, true, (short) 1, null, null, (short) 0);
    ...
    /**
     * @author Set Scheduled Complete Date attribute for all RouteNodes constructed from the maplist The Completion Due Date is got by adding Offset days attribute to current System date-time
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param MapList
            ...
            if (ChangeConstants.POLICY_FORMAL_CHANGE.equalsIgnoreCase(sPolicy)) {
                ...
                for (int i = historyList.size() - 1; i >= 0; i--) {
                    ...
                    if (historyRecord.startsWith("promote")) {
                        ...
                        if (promotedBy.indexOf("User Agent") > -1)
                            continue;
                        break;
                    }
                }
    ...
    /**
     * this method checks the Related object state Returns Boolean determines whether the connected objects are in appropriate state.
     * @author
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     ...
     * @return Boolean determines whether the connected objects are in appropriate state.
     * @throws Exception
     *             if the operation fails.
     * @since ECM R211
     */
     ...
     * @author yoq
     * @param context
     *            matrix code context object
     * @param args
     *            trigger parameters
     * @param args
     *            [0]-------change object id
     * @param args
     *            [1]-------Change Action relationship helps to retrieve all CAs from CO
     * @param args
     *            [2]-------Assignee relationship helps to retrieve all Assignees from CA
     * @param args
     *            [3]-------Subject key to pass in the notification
     * @param args
     *            [4]-------message key to pass in the notification
     * @param args
     *            [5]-------Suite key to pass in the notification
     * @throws Exception
     *             if the operation fails
     */
    ...
    /**
     * This method performs a check for prerequisites state before moving Change to Complete state if any of prerequisite state is not complete , then raise a notice
     * @author
     * @param context
     * @param args
     *            0 - object id 1 - name of state 2 - Policy of the object 3 - typeName 4 - Property key for Notice 5 - Suite key
    ...
    public int checkPrerequisitesBeforeComplete(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            mListPrerequisites = getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, relName), // IR-248902V6R2014x
                    PropertyUtil.getSchemaProperty(context, typeName), sList, null, false, true, (short) 1, EMPTY_STRING, relWhereClause, (short) 0);
    ...
    /**
     * @author triggers the promotion process of Change to Complete
     * @param context
     * @param args
     * @return
     * @throws Exception
     ...
     * @param args
     *            The string array containing the following arguments 0 - The object id of the context Engineering Change Object 1 - The attribute person selectables 2 - The relationship person
     *            selectables
     * @throws Exception
     * @throws FrameworkException
     * @since ECM R211
    ...
    public int notifyOnComplete(Context context, String[] args) throws Exception, FrameworkException {
        ...
        try {
            ...
            emxNotificationUtilBase_mxJPO.sendNotification(context, strObjectId, toList, ccList, bccList, subjectKey, messageKey, lstAttachments, propertyKey, null, null, null);
        } catch (Exception ex) {
            ex.printStackTrace();
            return 1;
        }
    ...
    public StringList includeROs(Context context, String[] args) throws Exception {
        ...
        companyOID = !changeUtil.isNullOrEmpty(changeTemplateOID)
                ? ChangeUtil.getRtoIdFromName(context, (String) DomainObject.newInstance(context, changeTemplateOID).getInfo(context, SELECT_ORGANIZATION))
    ...
    public StringList excludeAffectedItems(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            String relPattern = new StringBuffer(ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM).append(",").append(StringUtil.join(ECMAdmin.getAllCustomChangeRels(context), ",")).toString();
    ...
    /**
     * excludeCandidateItems() method returns OIDs of Candidate Items which are already connected to context change object
     * @param context
     *            Context : User's Context.
     * @param args
     *            String array
    ...
    /**
     * @author R3D Updates the Change Template in CO WebForm.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a MapList with the following as input arguments or entries: objectId holds the context CO object Id
    ...
    public DomainRelationship connectChangeTemplate(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            // Logic to apply Interface of Template to CO
            /*
             * if(!UIUtil.isNullOrEmpty(ssChangeTemplateID)){ sInterfaceName = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3",ssChangeTemplateID,"interface","|");
             * if(!UIUtil.isNullOrEmpty(sInterfaceName)){ intrItr = FrameworkUtil.split(sInterfaceName, "|").iterator(); //Add all Change Template Interfaces to CO for(int i=0; intrItr.hasNext();i++){
    ...
    /**
     * @author R3D Updates the Change Template in CO Clone WebForm.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a MapList with the following as input arguments or entries: objectId holds the context CO object Id
    ...
    public DomainRelationship connectTemplateToCloneCO(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (!UIUtil.isNullOrEmpty(ssChangeTemplateID)) {
                ...
                /*
                 * sReferenceDoc = dmObj.getInfo(context, "from["+RELATIONSHIP_REFERENCE_DOCUMENT+"].to.id"); if(!UIUtil.isNullOrEmpty(sReferenceDoc)){ changeOrder.addRelatedObject(context, new
                 * RelationshipType(RELATIONSHIP_REFERENCE_DOCUMENT) , false, sReferenceDoc); }
                 */
    ...
    public Vector showQuickActions(Context context, String[] args) throws Exception {
        ...
        try {
            StringList changeActionIDList = changeUtil.getStringListFromMapList(objectList, ChangeConstants.ID);
            String transferOwnership = EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(),
                    "EnterpriseChangeMgt.Message.TransferOwnershipOfCA");
            ...
            vecReturn = new Vector(changeActionIDList.size());
            ...
            Iterator changeActionItr = changeActionIDList.iterator();

            while (changeActionItr.hasNext()) {
                strCAId = changeActionItr.next().toString();

                sb = new StringBuffer(500);
                strTransferOwnership = "<a href=\"javascript:getTopWindow().showSlideInDialog('../common/emxForm.jsp?form=type_TransferOwnership&amp;formHeader=TransferOwnership&amp;mode=edit&amp;submitAction=refreshCaller&amp;postProcessJPO=enoECMChangeUtil:transferOwnership&amp;objectId="
                        + XSSUtil.encodeForHTMLAttribute(context, strCAId) + "', 'true')\"><img border='0' src='../common/images/iconSmallPerson.gif' name='person' id='person' alt=\""
                        + transferOwnership + "\" title=\"" + transferOwnership + "\"/></a>";
                sb.append(strTransferOwnership);

                strApproveTasks = "<a href=\"javascript:showModalDialog('../common/emxTableEdit.jsp?program=enoECMChangeOrder:getTasks&amp;table=AEFMyTaskMassApprovalSummary&amp;selection=multiple&amp;header=emxComponents.Common.TaskMassApproval&amp;postProcessURL=../common/emxLifecycleTasksMassApprovalProcess.jsp&amp;HelpMarker=emxhelpmytaskmassapprove&amp;suiteKey=Components&amp;StringResourceFileId=emxComponentsStringResource&amp;SuiteDirectory=component&amp;objectId="
                        + XSSUtil.encodeForHTMLAttribute(context, strCAId) + "', '800', '575')\"><img border='0' src='../common/images/iconSmallSignature.gif' name='person' id='person' alt=\""
                        + approvalTasks + "\" title=\"" + approvalTasks + "\"/></a>";
                sb.append(strApproveTasks);

                strImpactAnalysis = "<a href=\"javascript:showModalDialog('../common/emxTree.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strCAId) + "&amp;DefaultCategory="
                        + XSSUtil.encodeForURL("Impact Analysis") + "', '800', '575')\"><img border='0' src='../common/images/iconSmallImpactAnalysis.gif' name='IA' id='IA' alt=\"" + ImpactAnalysis
            ...
            while (objectItr.hasNext()) {
                ...
                if (!ChangeUtil.isNullOrEmpty(strAffectedObjID)) {
                    ...
                    if (strReportFormat != null && strReportFormat.equals("null") == false && strReportFormat.equals("") == false) {
                        ...
                        sb.append("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strAffectedObjID)
                                + "', '800', '575')\"><img border='0' src='../common/images/" + XSSUtil.encodeForHTMLAttribute(context, objectIcon.toString()) + "'/>"
                                + XSSUtil.encodeForHTML(context, strAffectedObjName) + "</a>");
        ...
        try {
            ...
            while (objectItr.hasNext()) {
                ...
                if (!ChangeUtil.isNullOrEmpty(strAffectedObjID)) {
                    ...
                    objectIcon.append(UINavigatorUtil.getTypeIconProperty(context, strAffectedObjType));
                    strTreeLink = "<a class=\"object\" href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strAffectedObjID)
                            + "&amp;DefaultCategory=ECMCAAffectedItems', '800', '575','true','content')\"><img border='0' src='../common/images/"
                            + XSSUtil.encodeForHTMLAttribute(context, objectIcon.toString()) + "'/>" + XSSUtil.encodeForHTML(context, strAffectedObjName) + "</a>";
     ...
     * @since R211 ECM
     */
    public Vector showRelatedCONames(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objectList = (MapList) programMap.get(ChangeConstants.OBJECT_LIST);
        Vector vecReturn = null;
        StringBuffer sb = null;
        try {
            ...
            while (objectItr.hasNext()) {
                map = (Map) objectItr.next();
                sb = new StringBuffer(500);
                strAffectedObjID = (String) map.get(ChangeConstants.RELATED_CO_ID);
                if (!ChangeUtil.isNullOrEmpty(strAffectedObjID)) {
                    strAffectedObjType = (String) map.get(ChangeConstants.RELATED_CO_TYPE);
                    strAffectedObjName = (String) map.get(ChangeConstants.RELATED_CO_NAME);

                    objectIcon.append(UINavigatorUtil.getTypeIconProperty(context, strAffectedObjType));
                    strTreeLink = "<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strAffectedObjID)
                            + "', '800', '575')\"><img border='0' src='../common/images/" + XSSUtil.encodeForHTMLAttribute(context, objectIcon.toString()) + "'/>"
                            + XSSUtil.encodeForHTML(context, strAffectedObjName) + "</a>";
                    sb.append(strTreeLink);
                    objectIcon.setLength(0);
        ...
        try {
            ...
            if (!ChangeUtil.isNullOrEmpty(changeOrderId)) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (ChangeUtil.isNullOrEmpty(estimationDate))
                        continue;
    ...
    public Vector showQuickMassApproval(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (changeOrderItr.hasNext()) {
                ...
                if (changeOrderUI.isKindOf(context, ChangeConstants.TYPE_CHANGE_ORDER) || changeOrderUI.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST)
                        || changeOrderUI.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)) {
                    mlTableData = changeOrderUI.getCurrentAssignedTasksOnObject(context);

                    if (!mlTableData.isEmpty()) {
                        sb = new StringBuffer(500);

                        strApproveTasks = "<a href=\"javascript:showModalDialog('../enterprisechangemgt/ECMUtil.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strCOId)
                                + "&amp;functionality=MassTaskApproval', '400', '400')\"><img border='0' src='../common/images/iconActionApprove.gif' name='person' id='person' alt='+approvalTasks+' title='+approvalTasks+'/>"
                                + EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(), "EnterpriseChangeMgt.Command.AwaitingApproval")
                                + "</a>";
                        sb.append(strApproveTasks);
                        vecReturn.addElement(sb.toString());
                    } else {
            ...
            while (changeOrderItr.hasNext()) {
                ...
                if (changeOrderUI.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)) {
                    ...
                    if (!mlTableData.isEmpty()) {
                        ...
                        // strApproveTasks =
                        // "<a
                        // href=\"javascript:showModalDialog('../common/emxTableEdit.jsp?program=enoECMChangeOrder:getTasks&amp;table=AEFMyTaskMassApprovalSummary&amp;selection=multiple&amp;header=emxComponents.Common.TaskMassApproval&amp;postProcessURL=../common/emxLifecycleTasksMassApprovalProcess.jsp&amp;HelpMarker=emxhelpmytaskmassapprove&amp;suiteKey=Components&amp;StringResourceFileId=emxComponentsStringResource&amp;SuiteDirectory=component&amp;objectId="
                        // + strCOId +
                        // "', '800', '575')\"><img border='0' src='../common/images/iconActionApprove.gif' name='person' id='person' alt='"+approvalTasks+"'
                        // title='"+approvalTasks+"'/>"+i18nNow.getI18nString("EnterpriseChangeMgt.Command.AwaitingApproval",
                        // ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getSession().getLanguage())+"</a>";
                        strApproveTasks = "<a href=\"javascript:showModalDialog('../enterprisechangemgt/ECMUtil.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strCOId)
                                + "&amp;functionality=MassTaskApproval', '400', '400')\"><img border='0' src='../common/images/iconActionApprove.gif' name='person' id='person' alt='+approvalTasks+' title='+approvalTasks+'/>"
                                + EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(), "EnterpriseChangeMgt.Command.AwaitingApproval")
                                + "</a>";
     ...
     * @throws Exception
     */
    public String getConnectedTemplate(Context context, String[] args) throws Exception {
        ...
        if (!UIUtil.isNullOrEmpty(objectId)) {
            dmObj.setId(objectId);
            return dmObj.getInfo(context, "to[Change Instance].from.name");
        } else
            return "";
    }
    ...
    /**
     * @author R3D this method performs the Mass hold process of change.Moves all associated CAs to hold state.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the Change Process
    ...
    /**
     * @author R3D this method performs the Mass Cancel process of change.Moves all associated CAs to hold state.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the Change Process
    ...
    /**
     * @author R3D this method performs the Mass Transfer Ownership
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the Change Process
    ...
    public MapList getLegacyChanges(Context context, String args[]) throws Exception {
        ...
        HashMap sRangeProgramResultMap = (HashMap) JPO.invoke(context, sRangeProgram, null, sRangeFunction, JPO.packArgs(new HashMap()), HashMap.class);
        ...
        try {
            if (!UIUtil.isNullOrEmpty(sbSearchType.toString()))
                if (UIUtil.isNullOrEmpty(strObjectId))
                    ...
                    sResultList = partObject.getRelatedObjects(context, QUERY_WILDCARD, sbSearchType.toString(), strList, null, true, false, (short) 1, null, null, 0);
    ...
    public MapList getLegacyChangeForSearchType(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            if (UIUtil.isNullOrEmpty(strObjectId))
                ...
                DomainObject partObject = new DomainObject(strObjectId);
                sTableData = partObject.getRelatedObjects(context, QUERY_WILDCARD, sb.toString(), strList, null, true, false, (short) 0, null, null, 0);
            }
        }
            ...
            if (ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM.equalsIgnoreCase(strRelType)) {
                ...
                if (strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_NONE) || strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_UPDATE)) {
                    ...
                    return isError;
                        ...
                        || strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_REVISE)) {
                    ...
                    mlAffetcedItemList = domObj.getRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM + "," + ChangeConstants.RELATIONSHIP_IMPLEMENTED_ITEM,
                            ChangeConstants.TYPE_CHANGE_ACTION, slObjSelect, slRelSelect, true, false, (short) 1, ChangeConstants.EMPTY_STRING, ChangeConstants.EMPTY_STRING, 0);
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (!strChangeObjState.equalsIgnoreCase(strCompleteState) && !strChangeObjState.equalsIgnoreCase(strCancelledState)) {
                            ...
                            } else if (strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_REVISE) && strRCAttrValue.equalsIgnoreCase(ChangeConstants.FOR_REVISE)) {

                                emxContextUtil_mxJPO.mqlNotice(context, EnoviaResourceBundle.getProperty(context, args[3], context.getLocale(), args[8]));
                ...
                if (indexTargetState != -1 && indexObjectState != -1) {
                    ...
                    if ((strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_OBSOLESCENCE) || strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_REVISE)) && indexObjectState < indexTargetState) {
    ...
    /**
     * Method invoked from the Delete Trigger of rel "Change Affected Item". This method is Used to delete the CA Object, if the Affected Item is the Last one to be removed.
     * @param context
     * @param args
    ...
    /**
     * Method invoked from the Owner Notification Trigger. This method is Used to get transfer reason while transferring the owner of the context change
     * @param context
     * @param args
     *            Object ID
     * @return String
     * @throws Exception
     */
    public static String getTransferComments(Context context, StringList args, Locale locale, String string1, String string2, String string3) throws Exception {
        try {
     ...
     * @param Props
     *            Array of Properties.
     * @param stringResourceFileID
     *            String Resource File ID
     * @param languageStr
     *            Language String
    ...
    public HashMap FastTrackProcessOptionsRangeValues(Context context, String[] args) {
        String[] rangeOptionsStrings = { "EnterpriseChangeMgt.Range.FastTrackProcess.Immediate", "EnterpriseChangeMgt.Range.FastTrackProcess.Deferred" };
     ...
     * @param context
     * @param args
     *            to get Functionality (Mass Release or Mass Obsolete).
    ...
    /**
     * This method is used as access function for CA effectivity column in CO affected items page.
     * @param context
     * @param args
     * @return True or False
     * @throws Exception
     */
    public boolean showEffectivityInCOAffectedItems(Context context, String[] args) throws Exception {
        /*
         * boolean sReturn = false; HashMap paramMap = (HashMap) JPO.unpackArgs(args); HashMap requestMap = (HashMap) paramMap.get(REQUEST_MAP); String objectId = (String)paramMap.get(OBJECT_ID);
         * boolean isECCInstalled = FrameworkUtil.isSuiteRegistered(context, "appVersionEngineeringConfigurationCentral", false, null, null); if(isECCInstalled) { /*Class cl =
         * Class.forName("com.matrixone.apps.effectivity.EffectivityFramework"); Object classObj = cl.newInstance();
         * 
         * Method method = cl.getMethod("getEffectivityOnChange", new Class[] {Context.class, String.class}); MapList mlObjectExpression = (MapList)method.invoke(classObj, new Object[] {context,
         * objectId}); Map mapExpression = (Map)mlObjectExpression.get(0); String actualValue = (String)mapExpression.get("actualValue");
         */
        // String val = DomainObject.newInstance(context, objectId).getInfo(context, "from["+EffectivityFramework.RELATIONSHIP_NAMED_EFFECTIVITY+"]");
     ...
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            - Object Id and Policy of Change Order
     * @return int - Returns integer status code 0 - in case the trigger is successful
     * @throws Exception
     *             if the operation fails
     * @since ECM R211
        ...
        if (ChangeConstants.POLICY_FASTTRACK_CHANGE.equalsIgnoreCase(strPolicy)) {
            try {
                ...
                emxContextUtil_mxJPO.mqlNotice(context,
                        EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Notice.NotConnectedAsChangeCoordinator"));
                return 1;
            }
        }
        return 0;
    ...
    /**
     * Post Process of CO create. If nonempty effectivity is set on CO, then auto generate a PUE ECO and add same effectivity to it.
     * @author D2E
     * @param context
     *            - the eMatrix <code>Context</code> object
    ...
    public MapList getEffectivity(Context context, String[] args) throws Exception {
        ...
        /*
         * If user selects the type through type chooser in CO create form then type key of requestMap contains value _selectedType:Change Order,type_ChangeOrder which is causing issue in Effectivity
         * field of CO to fix the issue requestMap is modified
         */
    ...
    /**
     * Method to get dynamic search string for Change Template search in Mass Change Functionality (Mass Release or Mass Obsolete)
     * @author M24
     * @param context
     *            - the eMatrix <code>Context</code> object
     * @param <code>String[]</code>
    ...
    /**
     * Action Trigger on ( Prepare/In Review --> In work) to check whether the context Change Order related Change Action to be completed or not. If so then Promote the Change Order to "In Approval"
     * state and notify CO Owner.
     * @param context
     * @param args
     *            (Change Order Id and Notice)
     * @throws Exception
     */
    public void checkForCAsState(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList resultList = getRelatedObjects(context, relPattern, typePattern, slObjectSelect, slRelSelect, false, true, (short) 1, "", EMPTY_STRING, 0);
