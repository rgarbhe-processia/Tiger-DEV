 ...
 * publication of such program. static const char RCSID[] = $Id: ${CLASSNAME}.java.rca 1.12.1.4 Wed Oct 22 16:02:20 2008 przemek Experimental przemek $
 */
...
public class emxRetainedDocumentBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     */
    public emxRetainedDocumentBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    /**
     * This Method gets the Documents which are Connected to Retention Record or Retention Hold Type with Retained Record Relationship
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following list of arguments: 0 - objectId * @return MapList with the list of retained documents
     * @throws Exception
     *             if the operation fails
     */
    public MapList getDocuments(Context context, String[] args, String SelectedFilter) throws Exception {
        ...
        if (sFilterPurged.equalsIgnoreCase(SelectedFilter)) {
            objectList = dom.getRelatedObjects(context, RelPattern.getPattern(), PurgeDoctype, typeSelects, relSelects, false, true, (short) 2, null, null);
        } // If filter is Purged.
        else {
            ...
            if (SelectedFilter.equalsIgnoreCase(sFilterAll)) {
                String sWhere = "";
                objectList = dom.getRelatedObjects(context, RelPattern.getPattern(), typePattern.getPattern(), typeSelects, relSelects, false, true, (short) 0, sWhere, null);
                ...
                objectList = dom.getRelatedObjects(context, RelPattern.getPattern(), typePattern.getPattern(), typeSelects, relSelects, false, true, (short) 0, null, null);
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getScheduledDocuments(Context context, String[] args) throws Exception {
        String strLanguage = context.getSession().getLanguage();
        String strPropertyFile = "emxLibraryCentralStringResource";
        String sFilterScheduled = EnoviaResourceBundle.getProperty(context, strPropertyFile, new Locale(strLanguage), "emxLibraryCentral.Filter.Scheduled");
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getPurgedDocuments(Context context, String[] args) throws Exception {
        String strLanguage = context.getSession().getLanguage();
        String strPropertyFile = "emxLibraryCentralStringResource";
        String sFilterPurged = EnoviaResourceBundle.getProperty(context, strPropertyFile, new Locale(strLanguage), "emxLibraryCentral.Filter.Purged");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following list of arguments: 0 - paramMap contains relID the relationshipID and the newComment Value
     * @return 0
     * @throws Exception
     *             if the operation fails
     */
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following list of arguments: 0 - paramMap contains relID the relationshipID and the RetentionPeriodvalue Value
     * @return 0
     * @throws Exception
     *             if the operation fails
     */
    ...
    /**
     * This Method has been written to set the Attributes when a document is connected to Record Retenton with Retained Record relationship
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments
    ...
    public int triggerCreateActionSettingRelationshipAttributes(Context context, String[] args) throws Exception {
        int ret = 0;
    ...
    /**
     * This Method has been written for Access Program for Edit Retention Period and Add Existing Depending upon the conditions
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments
     ...
     * @since LC 10.7SP1
     */
    public boolean isInActiveState(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    /**
     * This Method has been written to dispaly the Retention Schedules From The Document Tree Node
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds The ObjectId
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getRetentionSchedules(Context context, String[] args) throws Exception {
        MapList objectList = new MapList();
        ...
        Pattern RelPattern = new Pattern(relname);
        objectList = dom.getRelatedObjects(context, RelPattern.getPattern(), typePattern.getPattern(), typeSelects, relSelects, true, false, (short) 1, null, null);
    ...
    /**
     * This Method has been written to dispaly the images Depending upon the conditions
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments
    ...
    public Vector getImageObject(Context context, String[] args) throws Exception {
        ...
        while (itr.hasNext()) {
            ...
            if (purged) {
                imageTag.append("<img border=\"0\" src=\"../documentcentral/images/compiconDelete.gif\" name=\"red\" id=\"red\" alt=\" Purged Record\" title=\""
                        + XSSUtil.encodeForHTML(context, sPurgedTitle) + "\"></img>");
            } else if (noOfRetentionRecords > 0 || retentionHold) {
                if (datePassedNotpurged && noOfRetentionRecords > 0) {
                    imageTag.append("<img border=\"0\" src=\"../common/images/iconStatusRetentionPassedNotPurged.gif\" name=\"red\" id=\"red\" alt='" + XSSUtil.encodeForHTML(context, sRetentionDate)
                            + "' title=\"" + XSSUtil.encodeForHTML(context, sDatePassed) + "\"></img>");
                } else if (noOfRetentionRecords == 1) {
                    imageTag.append("<img border=\"0\" src=\"../common/images/iconStatusRetentionSet.gif\" name=\"morethanone\" id=\"morethanone\" alt=\"more than one\" title=\""
                            + XSSUtil.encodeForHTML(context, sRetentionDate) + "\"></img>");
                } else if (noOfRetentionRecords > 1) {
                    imageTag.append("<img border=\"0\" src=\"../common/images/iconStatusRetentionScheduleDocument.gif\" name=\"morethanone\" id=\"morethanone\" alt=\"more than one\" title=\""
                            + XSSUtil.encodeForHTML(context, sConnectedMoreRecords) + "\"></img>");
                }

                if (retentionHold) {
                    imageTag.append("<img border=\"0\" src=\"../common/images/iconStatusObjectHold.gif\" name=\"morethanone\" id=\"morethanone\" alt=\"more than one\" title=\""
                            + XSSUtil.encodeForHTML(context, sConnectedToHolds) + "\"></img>");
                }
            }
    ...
    /**
     * This Method has been written to Purge The Selected Documents Those satisfying the condition for Purge
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds The ObjectId
    ...
    public static void doPurge(Context context, String[] args) throws Exception {
        ...
        if (retainRelationship.equalsIgnoreCase("FALSE")) {
            ...
            while (stkrelname.hasMoreTokens()) {
                ...
                while (itrRel.hasNext()) {
                    ...
                    MapList mapConnectedObjList = doObj.getRelatedObjects(context, sRelName, "*", null, relationshipSelects, true, true, (short) 1, null, null);
         ...
         * 
         * SelectList selectStmts = new SelectList(); selectStmts.addElement(DomainObject.SELECT_ID); selectStmts.addElement("from["+retainedRel+"].to[|type != '"+strType+"'].id");
         * 
         * retRecords = doObj.getRelatedObjects(context, retainedRel, strRecordType, selectStmts, null, true, false, (short)0, null, null, 0);
         * 
         * if(retRecords.size() > 0) { Iterator itr = retRecords.iterator(); while(itr.hasNext()){ Map retRecord = (Map)itr.next(); Set retRecordKeySet = retRecord.keySet(); boolean canBePurged =
         * true; Iterator keyItr = retRecordKeySet.iterator(); while(keyItr.hasNext()){ String retRecordKey = (String)keyItr.next(); if(retRecordKey.startsWith("from["+retainedRel+"].to[")){
         * canBePurged = false; break; } } if(canBePurged){ String retRecordId = (String)retRecord.get(DomainConstants.SELECT_ID); DomainObject retRecordIdObj=new DomainObject(retRecordId);
         * 
    ...
    /**
     * This Method has been written to serch the selected type Those satisfying the condition
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds The ObjectId
     ...
     * @since LC 10.7SP1
     */
    public MapList getExistingDocuments(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        objectList = DomainObject.findObjects(context, DisplayType, txtName, txtRev, txtOwner, "*", whereExpression, null, false, objectSelects, (short) intLimit,
                LibraryCentralConstants.QUERY_WILDCARD, null);
    ...
    /**
     * This Method has been written to display Retention Date or Purge Date Retention Date will be displayed for Documents other than Purged Record And Purge Date for Purged Records.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds The ObjectId
     ...
     * @since LC 10.7SP1
     */
    public Vector displayDate(Context context, String[] args) throws Exception {
    ...
    public Vector getFileName(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (!(strFileName == null || strFileName.equals(""))) {
                String sLink = strLink + "&amp;parentOID=" + XSSUtil.encodeForURL(context, (String) objectMap.get("masterId")) + "&amp;objectId="
                        + XSSUtil.encodeForURL(context, (String) objectMap.get(DomainConstants.SELECT_ID)) + "&amp;AppendParameters=true&amp;trackUsagePartId="
                        + XSSUtil.encodeForURL(context, strPartId) + "', '', '', 'false', 'popup', '')\">";
    ...
    /**
     * Method to get the Default Retention period. Used in Record Retention Summary.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed HashMap of the following entries: objectList- a MapList of objects information. paramList- a HashMap of parameter values including reportFormat, relId, and
     ...
     * @since R210
     */
    public Vector getDefaultRetionPeriod(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    /**
     * /** This Method has been written to display Retention Period Only for Objects of type Retention Record
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds The ObjectId
     ...
     * @since R210
     */
    public boolean isRetentionRecordType(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        boolean bAccess = false;
    ...
    /**
     * Method to get the Document Title For Documents and Purged Documents. Used in Reatined Documents.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed HashMap of the following entries: objectList- a MapList of objects information. paramList- a HashMap of parameter values including reportFormat, relId, and
     ...
     * @since R210
     */
    public Vector getDocumentTitle(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramList = (HashMap) programMap.get("paramList");
        MapList objectList = (MapList) programMap.get("objectList");
        String languageStr = (String) paramList.get("languageStr");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed HashMap of the following entries paramMap - HashMap containing objectid
    ...
    public static String getPurgedStateDisplay(Context context, String[] args) throws Exception {
        ...
        if (!(UIUtil.isNullOrEmpty(purgedPolicy) || UIUtil.isNullOrEmpty(purgedState))) {
            ...
            purgedStateDisplay = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", new Locale(context.getSession().getLanguage()),
                    "emxFramework.State." + purgedPolicy + "." + purgedState);
        }
        return purgedStateDisplay;
    }

    /**
     * this method checks which are of type 'Purged Record' in a list of Objects - used in Retained Document List to disable Edit of Retention Period for Purged Records
     * @param context
     * @param args
     *            - packed objectList
     * @return StringList of 'true' or 'false' values, false for 'Purged Record' type true for other types
     * @throws Exception
     *             during failure
     */
