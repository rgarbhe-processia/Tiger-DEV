...
public class emxPersonBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @grade 0
     */
    public emxPersonBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    /**
     * This is a trigger method used to keep Person admin object in sync. This is configured on Attribute 'Last Name', 'First Name' as modify override trigger changeName event of Person type and
     * promote/demote action
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: args[0] = even (modify/changeName) args[1] = old person name args[2] = new person name args[3] = attribute name args[4] = old attribute value
     ...
     * @grade 0
     */
    public int syncPersonAdminObject(Context context, String[] args) throws Exception {
        try {
            ...
            if (sEvent.equals("ChangeName")) {
                ...
                String sCmd = "modify person \"" + sOldPersonName + "\" name \"" + sNewPersonName + "\"";
                MqlUtil.mqlCommand(context, sCmd, true);
                ...
                MqlUtil.mqlCommand(context, "modify person \"" + personName + "\" type active \"" + sEmailFlag + "\" email disable iconmail", true);
            } else if (sEvent.equals("Demote")) {
                String personName = args[1];
                MqlUtil.mqlCommand(context, "modify person \"" + personName + "\" type inactive disable email disable iconmail", true);
     ...
     * @grade 0
     */
    public HashMap getLastQuery(Context context, String[] args) throws Exception {
    ...
    /**
     * A method used to return a value for the attribute from the matrix db which has been passed as a parameter in the args. The function returns a fullname in the [Firstname Lastname] format.
    ...
    public MapList getMemberListPeopleSearchResult(Context context, String[] args) throws Exception {
        ...
        if (ADD_EXISTING.equalsIgnoreCase(strMode) && objectId != null) {
            ...
            if (RELATIONSHIP_ASSIGNED_MEMBER.equals(srcDestRelName)) {
                busWhere += " && ";
                busWhere += "( ( to[" + RELATIONSHIP_MEMBER + "].attribute[" + ATTRIBUTE_PROJECT_ROLE + "] ~~ \"*role_SpecificationOfficeManager*\" ) && (to[" + RELATIONSHIP_MEMBER + "].from.type == "
                        + TYPE_COMPANY + ") )";
            }
        }
        ...
        mapList = (MapList) getGlobalSearchRelObjects(context, company.getId(), RELATIONSHIP_MEMBER, TYPE_PERSON, busWhere, sQueryLimit, "|", "@");
     ...
     * @since SpecificationCentral 10.0.0.0
     */
    protected MapList getGlobalSearchRelObjects(matrix.db.Context context, String objectId, String relName, String typeName, String busWhere, String sQueryLimit, String fieldSep, String recordSep)
            throws Exception {
        StringBuffer mqlCommand = new StringBuffer();

        mqlCommand.append("expand bus ");
        mqlCommand.append(objectId);
        mqlCommand.append(" from relationship \"");
        mqlCommand.append(relName);
        mqlCommand.append("\" type \"");
        mqlCommand.append(typeName);
        mqlCommand.append("\" select bus id where '");
        mqlCommand.append(busWhere);
        mqlCommand.append("' dump ");
        mqlCommand.append(fieldSep);
        mqlCommand.append(" recordsep ");
        mqlCommand.append(recordSep);
        mqlCommand.append(" terse limit ");
        mqlCommand.append(Short.parseShort(sQueryLimit));

        String strResult = MqlUtil.mqlCommand(context, mqlCommand.toString());
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getPersonSearchResult(Context context, String[] args) throws Exception {
        ...
        if (mapList != null) {
            ...
            if (mapList.size() > intQueryLimit) {
                String strMessage = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(languageStr), "emxComponents.Warning.ObjectFindLimit") + queryLimit
                        + EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(languageStr), "emxComponents.Warning.Reached");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds input arguments. first parameter contains personname and second vault name
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since Profile Mgmt 10.5
     */
    public void updateAdminPersonDefaultVault(matrix.db.Context context, String[] args) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        ...
        String strMql = "modify person \"" + strPersonName + "\" vault \"" + strVault + "\" ";
        try {
            ContextUtil.pushContext(context);
            MQLCommand prMQL = new MQLCommand();
            prMQL.open(context);
            prMQL.executeCommand(context, strMql);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds input arguments. 1st parameter - Object Id 2st parameter - Person Name 3rd parameter - Attribute Name 4th parameter - Attribute Value
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since Profile Mgmt 10.5
     */
    public void updateAdminPersonAttributes(matrix.db.Context context, String[] args) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        ...
        if (isAttributeModified) {
            ...
            strBufMQL.append(" \"").append(strPersonName).append("\" ");
            strBufMQL.append(strUpdateString);

            MqlUtil.mqlCommand(context, strBufMQL.toString(), true);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds input arguments. 1st parameter - Object Id 2st parameter - Person Name 3rd parameter - Attribute Name 4th parameter - Attribute Default Value 5th parameter - Attribute Old
     *            Value 6th parameter - Attribute New Value 7th parameter - Objects's Current State 8th parameter - Context User Name 9th parameter - Time Stamp
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since Profile Mgmt 10.5
     */
    public void overrideAdminPersonAttributes(matrix.db.Context context, String[] args) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        String strObjectId = args[0];
        ...
        if (personObject.ATTRIBUTE_EMAIL_ADDRESS.equals(strAttributeName)) {
            ...
            if (STATE_PERSON_ACTIVE.equals(strCurrentState)) {
                if (!strAttributeValue.equals(strAttributeDefault)) {
                    String[] arrMailArgs = { strPersonName, "emxFramework.ProgramObject.eServicecommonTrigaNotifyEmailChange_if.Subject", "0",
                            "emxFramework.ProgramObject.eServicecommonTrigaNotifyEmailChange_if.Message", "4", "oldEmail", strAttributeValue, "newEmail", strNewAttributeValue, "user", strContextUser,
                            "time", strTimeStamp };
                    emxMailUtil_mxJPO.sendNotificationToUser(context, arrMailArgs);
                }
            } else {
                String[] arrMailArgs = { strPersonName, "emxFramework.ProgramObject.eServicecommonTrigaNotifyEmailChange_if.RegSubject", "0",
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds input arguments. 1st parameter - Object Id 2st parameter - Person Name 3rd parameter - Attribute Name 4th parameter - Attribute New Value
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since 3DExperience 2015x.FD01
     */
    public void overrideUpdateEmailAddress(matrix.db.Context context, String[] args) throws Exception {
        if (args.length == 0) {
            throw new IllegalArgumentException();
        }
        String strObjectId = args[0];
    ...
    public String getStrRolesSymbolicAssigned(Context context, String args[]) throws Exception {
        ...
        if (mxType.isOfParentType(context, strType, DomainConstants.TYPE_PERSON)) {
            ...
            matrix.db.Person mxDbPerson = new matrix.db.Person((String) personMap.get(SELECT_NAME));

            mxDbPerson.open(context);

            matrix.db.UserItr userItr = new matrix.db.UserItr(mxDbPerson.getAssignments(context));
            ...
            while (userItr.next()) {
                ...
                if (userObj instanceof matrix.db.Role) {
                    ...
                    if (mxRoleName.endsWith("_PRJ")) {
                        continue;
                    }
                    if (geti18nRoleNames && !mxRoleName.startsWith("ctx::")) {
                        i18nNowRole = EnoviaResourceBundle.getRoleI18NString(context, mxRoleName, context.getSession().getLanguage());
                    } else {
    ...
    public MapList getAssignedRoleList(Context context, String[] args) throws FrameworkException {
        try {
            ...
            String personName = MqlUtil.mqlCommand(context, "print bus " + personId + " select name dump", true);
            List assignments = getPersonAssignments(context, personName, true);
            List hidden = FrameworkUtil.split(MqlUtil.mqlCommand(context, "list role * where hidden", false), System.getProperty("line.separator"));
    ...
    public MapList getAssignedGroupList(Context context, String[] args) throws FrameworkException {
        try {
            Map programMap = (Map) JPO.unpackArgs(args);
            String personId = (String) programMap.get("objectId");
            String personName = MqlUtil.mqlCommand(context, "print bus " + personId + " select name dump", false);

            List assignments = getPersonAssignments(context, personName, false);
            List hidden = FrameworkUtil.split(MqlUtil.mqlCommand(context, "list group * where hidden", false), System.getProperty("line.separator"));
            hidden.retainAll(assignments);
            ...
            if (cloud) {
                ...
                if ("VPLMProjectLeader".equals(strRole)) {
                    ...
                    if (!sExistingRoles.contains("role_ECRCoordinator")) {
                        ...
                        MqlUtil.mqlCommand(context, "modify person '" + context.getUser() + "' type system", true);
                        String str1 = MqlUtil.mqlCommand(context, "print person '" + context.getUser() + "' select system");
                        ...
                        MapList memberMapList = personID.getRelatedObjects(context, // context
                        ...
                        if (memberMapList != null || memberMapList.size() > 1) {
                            for (int i = 0; i < memberMapList.size(); i++) {
                                ...
                                if (memberRelId != null && memberRelId != "") {
                                    String iResult2 = Organization.checkLeadRoleAssign(context, personId, memberRelId, new String[] { "role_ECRChairman", "role_ECRCoordinator" });
    ...
    public void createPersonalWorkspace(Context context, String[] args) throws Exception {
        try {
            ...
            String cmd = "add bus $1 $2 $3 policy $4 vault $5 owner $6 project $7 organization $8";
            MqlUtil.mqlCommand(context, cmd, TYPE_PERSONAL_WORKSPACE, strPersonalWorkspaceName, personName, POLICY_PERSONAL_WORKSPACE, person.getDefaultVault(context, person), personName,
                    personName + "_PRJ", orgName);
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public void deletePersonalWorkspace(Context context, String[] args) throws Exception {
        try {
            ...
            objectSelects.addElement(SELECT_ID);
            MapList queryList = DomainObject.findObjects(context, TYPE_PERSONAL_WORKSPACE, "*", personName, personName, person.getDefaultVault(context, person), (String) null, (String) null, true,
                    objectSelects, (short) 0);
            if (queryList.size() > 0) {
