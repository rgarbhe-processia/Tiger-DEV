...
public class emxVPLMCompositeDocumentBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public MapList getVPLMRoots(Context context, String[] objIds) throws Exception {
    ...
    private MapList getVPLMRoots(Context context) throws Exception {
        ...
        try {
            ...
            for (int ii = 0; implementingRoots != null && ii < implementingRoots.length; ii++) {
                ...
                PLMxEntityDef[][] implementingRoots2 = (isFunction || isLogical
                        ? navModeler.getImplementing(IVPLMImplementLinkNav.implLinkNavPathMode.complete, new PLMxEntityPath(new String[] { rr1Id }))
                        : null);
                ...
                if (implementingRoots2 != null) {
                    for (int jj = 0; jj < implementingRoots2.length; jj++) {
                        ...
                        PLMxEntityDef[][] implementingRoots3 = (isLogical2 ? navModeler.getImplementing(IVPLMImplementLinkNav.implLinkNavPathMode.complete, new PLMxEntityPath(new String[] { rr2Id }))
                                : null);
    ...
    private String addVPLMRoot(Context context, HashMap rootMap, PLMxEntityDef rootDef) throws MatrixException {
        ...
        boolean isFunction = iFunctionalType.isKindOf(context, RFLPLMFUNCTIONAL);
        boolean isLogical = iFunctionalType.isKindOf(context, RFLVPMLOGICAL);
        String keyId = (isFunction ? VPLM_FUNCTION_ROOT : isLogical ? VPLM_LOGICAL_ROOT : VPLM_PHYSICAL_ROOT);
        String keyName = (isFunction ? VPLM_FUNCTION_NAME : isLogical ? VPLM_LOGICAL_NAME : VPLM_PHYSICAL_NAME);
    ...
    private MapList buildSpecStructureData(Vector reqIds) {
        ...
        try {
            ...
            MapList reqList = emxObject.getRelatedObjects(emxContext, strRelType, strObjTypes, objSelect, relSelect, false, true, (short) 0, where, DomainConstants.EMPTY_STRING, 0);
    ...
    private HashMap buildVPLMObjectMap(PLMxEntityDef entityDef) {
        ...
        for (Enumeration keys = entityAtts.keys(); keys.hasMoreElements();) {
            ...
            if (!(key.equals("reserved") || key.equals("reservedby") || key.equals("C_updatestamp") || key.equals("V_isTerminal") || key.equals("V_isLastVersion") || key.equals("V_ApplicabilityDate")
     ...
     */
    /*
     * private MapList buildVPLMPortList(String flObjId, int level) { MapList portList = new MapList(); try { FLPort[] flPorts = plmModeler.getFLPorts(flObjId); //System.out.println(
     * "==> flPorts.length = " + (flPorts == null? -1: flPorts.length));
     * 
     * for (int ii = 0; flPorts != null && ii < flPorts.length; ii++) { PLMxPortEntity portEnt = flPorts[ii].getPortEntity(); HashMap portMap = buildVPLMPortMap(portEnt); portMap.put("level", "" +
     * level); portMap.put("relationship", RELATIONSHIP_PORTS); portList.add(portMap);
     * 
     * // Append the related Type/Flow information... HashMap infoMap = new HashMap(); String entId = ""; String entType = ""; String entName = ""; String entRev = ""; String dirName = ""; String
     * relName = "";
     * 
     * if (flPorts[ii] instanceof FunctionalPort) { FunctionalPort fncPort = (FunctionalPort) flPorts[ii]; PLMxEntityDef fncFlow = fncPort.getFlowEntity(); Map flowMap = fncFlow.getAttributes();
     * //SpecificationStructure.printIndentedMap(flowMap);
     * 
     * // Hack: this name mapping should be provided by the metadata services. String catName = ""; if (flowMap.containsKey("V_Category")) { String flowCat = (String) flowMap.get("V_Category"); if
     * (flowCat.equals("1")) catName = "Data"; else if (flowCat.equals("2")) catName = "Control"; } infoMap.put("V_Category", catName);
     * 
     * entId = fncFlow.getPLMIdentifier(); entType = fncFlow.getPLMType(); entName = fncPort.getFlowName(); dirName = fncPort.getDirectionName(); relName = RELATIONSHIP_FLOWS; } else if (flPorts[ii]
     * instanceof LogicalPort) { LogicalPort logPort = (LogicalPort) flPorts[ii]; PLMxEntityDef logSyst = logPort.getSystemTypeEntity(); entId = logSyst.getPLMIdentifier(); entType =
     * logSyst.getPLMType(); entName = logPort.getSystemTypeName(); dirName = logPort.getDirectionName(); relName = RELATIONSHIP_TYPES; }
     * 
     * //IR Mx375143 portMap.put("V_Direction", dirName); // override the number with its name.
     * 
     * infoMap.put(DomainConstants.SELECT_ID, entId); infoMap.put(DomainConstants.SELECT_TYPE, entType); infoMap.put(DomainConstants.SELECT_NAME, entName); infoMap.put(DomainConstants.SELECT_REVISION,
     * entRev); // to validate CompositeDocument.dtd infoMap.put("level", "" + (level+1)); // one level below the current port level. infoMap.put("relationship", relName);
     * 
     * portList.add(infoMap); } } catch (Exception e) { System.err.println("WARNING: Cannot get FLPorts for: " + flObjId + "\n" + e.getMessage()); } return(portList); }
     */
    /**
     * @param entityDef
     * @return
    ...
    private HashMap buildVPLMPortMap(PLMxPortEntity portDef) {
        ...
        for (Enumeration keys = portAtts.keys(); keys.hasMoreElements();) {
            ...
            if (!(key.equals("reserved") || key.equals("reservedby") || key.equals("C_updatestamp") || key.equals("V_isUptodate") || key.equals("V_Direction") || key.equals("V_nature")
                    || key.equals("V_Owner") || key.equals("project"))) {
                if (key.equals("originated") || key.equals("modified")) {
                    long millis = Long.parseLong((String) val) * 1000;
                    Date dbdate = new Date(millis);
                    val = VPLM_DATE_FORMATTER.format(dbdate);
     ...
     */
    /*
     * private MapList buildVPLMRepList(String flObjId, int level) { MapList repList = new MapList(); try { PLMxEntityDef[] flReps = plmModeler.getRepresentations(flObjId); //System.out.println(
     * "==> flReps.length = " + (flReps == null? -1: flReps.length));
     * 
     * for (int ii = 0; flReps != null && ii < flReps.length; ii++) { HashMap repMap = buildVPLMObjectMap(flReps[ii]); repMap.put("level", "" + level); repMap.put("relationship",
     * RELATIONSHIP_REPRESENTATIONS);
     * 
     * // Add the data stream information... PLMxRepresentationEntity repEnt = (PLMxRepresentationEntity) flReps[ii]; HashMap strMap = buildVPLMStreamMap(repEnt); repMap.putAll(strMap);
     * repList.add(repMap); } } catch (Exception e) { System.err.println("WARNING: Cannot get FLReps for: " + flObjId + "\n" + e.toString()); }
     * 
     * return(repList); }
     */
    ...
    private HashMap buildVPLMStreamMap(PLMxRepresentationEntity repEntity) throws IOException {
        ...
        /*
         * try { PLMxStreamStorage stream = plmModeler.getStreamsFromRepReference(repEntity, 3); if (stream != null) { stream.open("r"); System.out.println("opened representation stream..."); byte[]
         * binData = stream.readStream(0); System.out.println("read " + binData.length + " bytes."); stream.close();
         * 
         * String cntData = Base64Utils.encode(binData); System.out.println("encoded size = " + cntData.length() + " bytes.");
         * 
         * // Don't know how we can find out what the actual data type is... strMap.put("Content Type", "rtf.b64"); strMap.put("Content Data", cntData); } } catch (Exception e) { e.printStackTrace();
         * }
         */
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (!context.isConnected()) {
            String mess = i18nNow.getI18nString("emxRequirements.Alert.FeaturesCheckFailed", "emxRequirementsStringResource", context.getSession().getLanguage());
    ...
    public Document buildCompositeDocument(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (requestMap != null && !requestMap.isEmpty()) {
                ...
                if (style != null) {
                    ...
                    opts.put("href", style);
                    rootDoc.addContent(new ProcessingInstruction("xml-stylesheet", opts));
                }
    ...
    private Map buildImplementMap(Context context, String specId, String funcRootId, String logicRootId, String phyRootId, Map objectPool) throws Exception {
        ...
        MapList reqObjects = emxObject.getRelatedObjects(context, strRelType, strObjTypes, objSelects, relSelects, false, true, (short) 0, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING,
                0);
        int indent = 0;
        ...
        for (int kk = 0; kk < attList.length; kk++) {
            ...
            if (attVal != null) {
                if (attKey.equals("Content Type")) {
                    ...
                    if (attVal instanceof PLMQLNULL || attKey.equals("V_ApplicabilityDate") && "NULL".equals(attVal)) {
                        attVal = "";
                    }
            ...
            if (entity instanceof PLMxReferenceEntity) { // FunctionRef, LogicalRef, CummunicationRef(Mux/Demux), FlowRef, TypeRef
                ...
                if (category == FUNCTION || category == LOGICAL || category == PHYSICAL || category == FLOW || category == TYPE) {
                    ...
                    for (int i = 0; instances != null && i < instances.length; i++) {
                        ...
                        if (!(childCategory == PORT || childCategory == CONNECTION || (category == FUNCTION || category == LOGICAL) && (childCategory == FLOW || childCategory == TYPE))) {
                ...
                if (category == FUNCTION || category == LOGICAL || category == COMMUNICATION) {
                    ...
                    if (category == FUNCTION || category == COMMUNICATION && isFunctionalCommunication(context, ref)) {
                ...
                if (recursive) {
                    ...
                    if (category == FUNCTION || category == LOGICAL || category == COMMUNICATION || category == PHYSICAL) {
                        if (category == FUNCTION || category == LOGICAL || category == COMMUNICATION) {
                            PLMxPortEntity[] ports = null;
                            // connections
                            if (category == FUNCTION || category == COMMUNICATION && isFunctionalCommunication(context, instance)) {
                            ...
                            for (int j = 0; ports != null && j < ports.length; j++) {
                                ...
                                if (category == FUNCTION || category == COMMUNICATION && isFunctionalPort(context, ports[j])) {
                                ...
                                Element sourcePort = buildElement(context, ports[j], getCategory(context, instance), false, objectPool, implMap);
                                ...
                                for (int n = 0; oConnectedInstances != null && n < oConnectedInstances.length; n++) {
                                    ...
                                    Element targetInstance = buildElement(context, oConnectedInstances[n], getCategory(context, oConnectedInstances[n]), false, objectPool, implMap);
                                    r.addContent(targetInstance);

                                    // pass in category of port's parent!!!
                                    r.addContent(buildElement(context, oConnectedPorts[n], getCategory(context, oConnectedInstances[n]), false, objectPool, implMap));
    ...
    int getCategory(Context context, PLMxEntityDef entity) throws MatrixException {
        IPLMDictionaryPublicItf dico = new IPLMDictionaryPublicFactory().getDictionary();
        IPLMDictionaryPublicClassItf iFunctionalType = dico.getClass(context, entity.getPLMType().substring(entity.getPLMType().indexOf("/") + 1));
        if (entity instanceof PLMxPortEntity) { // RFLPLMFunctionalDS/RFLPLMFunctionalConnectorDS
        ...
        { // RFLPLMFunctionalDS/RFLPLMFunctionalConnectionDS
          // RFLVPMLogicalDS/RFLVPMLogicalConnectionDS
            return CONNECTION;
        } else if (entity instanceof PLMxRepresentationEntity || entity instanceof PLMxRepInstanceEntity) { // RFLPLMFunctionalDS/RFLPLMFunctionalRepReferenceDS
        ...
        } else if (iFunctionalType.isKindOf(context, RFLPLMFUNCTIONAL)) { // RFLPLMFunctionalDS/RFLPLMFunctionalInstanceDS
                                                                          // RFLPLMFunctionalDS/RFLPLMFunctionalReferenceDS

            return FUNCTION;
        } else if (iFunctionalType.isKindOf(context, RFLVPMLOGICAL)) { // RFLVPMLogicalDS/RFLVPMLogicalReferenceDS
                                                                       // RFLVPMLogicalDS/RFLVPMLogicalInstanceDS
            return LOGICAL;
        } else if (iFunctionalType.isKindOf(context, RFLVPMPHYSICAL)) { // PLMProductDS/PLMProductDS
            return PHYSICAL;
        } else if (iFunctionalType.isKindOf(context, RFLPLMFUNCTIONALCOMMUNICATION) || iFunctionalType.isKindOf(context, RFLPLMLOGICALCOMMUNICATION)) { // RFLPLMFunctionalCommunicationDS/RFLPLMFunctionalCommunicationInstanceDS
                                                                                                                                                        // RFLVPMLogicalCommunicationDS/RFLVPMLogicalCommunicationInstanceDS
            return COMMUNICATION;
        } else if (iFunctionalType.isKindOf(context, RFLPLMFLOW)) { // RFLPLMFlowDS/RFLPLMFlowInstanceDS
                                                                    // RFLPLMFlowDS/RFLPLMFlowReferenceDS

            return FLOW;
        } else if (iFunctionalType.isKindOf(context, RFLPLMTYPE)) { // RFLVPMSystemTypeDS/RFLVPMSystemTypeReferenceDS
     ...
     * @param iListUnopenedPLMIdentifier
     *            - array of PLMIDs which are not opened
     */
    public PLMxEntityDef[] getOpenedObjectsFromServer(String[] iListUnopenedPLMIdentifier) throws Exception {
        ...
        for (int plmIDIndex = 0; plmIDIndex < iListUnopenedPLMIdentifier.length; plmIDIndex++) {
            ...
            if ((plmxresult != null) && (plmxresult.next())) {
                ...
                if (tempEnt != null) {
                    ...
                    if (debugTrace)
                        System.out.println(
                                "getOpenedObjectsFromServer=> ERROR: opened result for iListUnopenedPLMIdentitifier[" + plmIDIndex + "] (" + iListUnopenedPLMIdentifier[plmIDIndex] + ") is null!!!");
                }
            } else {
                if (debugTrace)
                    System.out.println(
                            "getOpenedObjectsFromServer=> ERROR: plmxresult set for iListUnopenedPLMIdentitifier [" + plmIDIndex + "] (" + iListUnopenedPLMIdentifier[plmIDIndex] + ") is null!!!");
            }
        }

        // ensure the same order of output as the input
        if (debugTrace)
            System.out.println("getOpenedObjectsFromServer=> Ensuring proper order now");
        if (debugTrace)
            System.out.println("getOpenedObjectsFromServer=> iListUnopenedPLMIdentifier.length = " + iListUnopenedPLMIdentifier.length);
        if (debugTrace)
            System.out.println("getOpenedObjectsFromServer=> tempoutputList.length = " + tempoutputList.length);

        for (int i = 0; i < iListUnopenedPLMIdentifier.length; i++) {
            for (int j = 0; j < tempoutputList.length; j++) {
                // find matching plm ids
                if (debugTrace)
                    System.out.println("getOpenedObjectsFromServer=> iListUnopenedPLMIdentifier[" + i + "]: " + iListUnopenedPLMIdentifier[i]);

                if ((tempoutputList[j] != null) && tempoutputList[j].getPLMIdentifier().equals(iListUnopenedPLMIdentifier[i])) {
                    if (debugTrace)
                        System.out.println("getOpenedObjectsFromServer=> tempoutputList[" + j + "] PLMID: " + tempoutputList[j].getPLMIdentifier());
                    // found correct position to put the output
                    outputList[i] = tempoutputList[j];
                } else {
                    if (debugTrace)
                        System.out.println("getOpenedObjectsFromServer=> tempoutputList[" + j + "]  is NULL!");
                }
            }
        }
