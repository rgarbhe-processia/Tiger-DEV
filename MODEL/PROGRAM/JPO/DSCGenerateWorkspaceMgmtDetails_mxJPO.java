...
public class DSCGenerateWorkspaceMgmtDetails_mxJPO {
     ...
     * @param args
     *            contains a Map with input args (see getColumn doc)
    ...
    public Vector getVer(Context context, String[] args) throws Exception {
    ...
    public Vector getLockedBy(Context context, String[] args) throws Exception {
        Vector vReadAcces = this.getColumn(context, "readAcces", args);
        Vector objectId = this.getColumn(context, "objectId", args);
    ...
    public Vector getInSession(Context context, String[] args) throws Exception {
        Vector vReadAcces = this.getColumn(context, "readAcces", args);
        Vector objectId = this.getColumn(context, "objectId", args);
    ...
    /**
     * This method used in DECPLMTreeDetails Table for getting current version getCurrentVersion() should pass true in last parameter to support above mention behavior.. if the busid is minor & in
     * release state then method should return major's current version.
     * @param context
     * @param args
     * @return
     * @throws Exception
    ...
    private Hashtable getLatestVersion(Context context, String[] objIds, String localeLanguage, HashMap integrationNameGCOTable) throws Exception {
        ...
        for (int i = 0; i < buslWithSelectionList.size(); i++) {
            ...
            if (integrationName != null && integrationNameGCOTable != null && integrationNameGCOTable.containsKey(integrationName)
                    && _util.getAssignedIntegrations(context).contains(integrationName)) {
                globalConfigObj = (MCADGlobalConfigObject) integrationNameGCOTable.get(integrationName);
            }
     ...
     * @param paramMap
     *            holds a JPO.packArgs packed TreeMap of file details (see emxDSCWorkspaceMgmtDetails2.jsp for details):
     ...
     * @param args
     *            contains a Map with the following entries: paramList contains a HashMap that contains the file details (see unpackFileDetails) objectList MapList
     * @return Vector contains list of column values
     * @throws Exception
     *             if the operation fails
     */
