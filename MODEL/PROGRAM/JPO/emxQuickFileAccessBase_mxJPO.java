...
public class emxQuickFileAccessBase_mxJPO {
    ...
    /**
     * This method returns the file list that are connected to the object with the specified relationships in the properties
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
    ...
    public Vector getFileName(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < objectListSize; i++) {
                ...
                downloadURL = "javascript:callCheckout('" + XSSUtil.encodeForJavaScript(context, masterId) + "','download', '" + XSSUtil.encodeForJavaScript(context, name) + "', '"
    ...
    public Vector getFileContext(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (objectListItr.hasNext()) {
                ...
                if (isCSVExport && isExporting) {
                    fileContextVector.add(fileContext);
                } else {
        ...
        try {
            ...
            while (objectListItr.hasNext()) {
                ...
                if (!reportMode) {
                    // Bug No 317834 - Start
                    /*
                     * Because column type is programHTMLOutput, the sortable prameter needs to come first in the href. earlier, object id coming first in the href so the sorting is failed on the
                     * column
                     */
                ...
                if (objectMap != null) {
                    ...
                    if (strRevision != null && strRevision.trim().length() > 0 && !"null".equalsIgnoreCase(strRevision)) {
                        if (!reportMode) {
                            ...
                            // Bug No 317834 - Start
                            /*
                             * Because column type is programHTMLOutput, the sortable prameter needs to come first in the href. earlier, object id coming first in the href so the sorting is failed on
                             * the column
                             */
