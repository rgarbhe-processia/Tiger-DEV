        ...
        if (mlObjects.size() > 0) {
            ...
            for (int i = 0; i < mlObjects.size(); i++) {
                ...
                if (sURLImage.equals("NoImageFound")) {
                    if (bAccess && !excluded) {
                        sbResult.append("<form class='dropAreaInColumn' id='").append(sFormId).append("' action=\"../common/emxExtendedPageHeaderFileUploadImage.jsp?objectId=").append(sOID)
                    ...
                    if (bAccess && !excluded) {
                        ...
                        sbResult.append("        ondrop=\"ImageDropOnImageColumn(event, '").append(sFormId).append("', '").append(sDivId).append("', '").append(sDivIdImage).append("', '")
                                .append(sLevel).append("')\" ");
                        ...
                        if (useInPlaceManager) {
                            ...
                            sbResult.append(
                                    "window.open('../components/emxImageManager.jsp?isPopup=false&amp;toolbar=APPImageManagerToolBar&amp;header=emxComponents.Image.ImageManagerHeading&amp;HelpMarker=emxhelpimagesview&amp;");
                            sbResult.append("objectId=").append(sOID)
                                    .append("', '', 'height=650,width=850,top=' + posTop + ',left=' + posLeft + ',toolbar=no,directories=no,status=no,menubar=no;return false;')\"");
                        }
                        sbResult.append(" />");
                        sbResult.append("   </div>");
                        sbResult.append("</form>");
