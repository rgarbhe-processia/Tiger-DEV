 ...
 * @exclude
 */
public class emxMultipleClassificationAttributeFormHtmlBase_mxJPO extends emxDomainObject_mxJPO implements ClassificationConstants {
     ...
     *             if the operation fails
     */
    public String getAttributeGroupAttributesHTML(String mode, String attributeGroupName) throws Exception {
     ...
     *             if the operation fails
     */
    public String getAttributeGroupHTML(String mode, String attributeGroupName) throws Exception {
        ...
        if (!access.equals("#DENIED!")) {
            StringBuffer buf = new StringBuffer(getAttributeGroupHeadingHTML(attributeGroupName));
            buf.append(access);
            return buf.toString();
        } else {
            return "";
     ...
     *             if the operation fails
     */
    public String getClassificationHTML(String mode, String ifName) throws Exception {
        StringBuffer buf = new StringBuffer();
        ...
        if (buf.length() == 0) {
            ...
            return getClassificationHeadingHTML(ifName) + buf.toString();
        }
    }
     ...
     * @exclude
     */
    protected String getFinalJS(Context context, String args[]) throws Exception {
        ...
        /*
         * _requestMap = requestMap; _lang = (String) requestMap.get("languageStr");
         */
        try {
            _lang = (String) requestMap.get("languageStr");
            _requestMap = requestMap;
        } catch (Exception e) {
            _lang = (String) _requestMap.get("languageStr");
        }
        ...
        if (_showRedundantMsg != null && !"null".equals(_showRedundantMsg)) {
            if (_showRedundantMsg.equalsIgnoreCase("true")) {
                ...
                buf.append(EnoviaResourceBundle.getProperty(_context, "emxLibraryCentralStringResource", new Locale(_lang), "emxMultipleClassification.Attribute.Duplicate.Message"));
                buf.append("\";\n");
            }
        }
     ...
     * @param classificationInterfaces
     *            the list of Intercases for which HTML has to be formed
     * @param mode
     *            edit or view
     * @return Attributes HTML based on the mode
     * @throws Exception
     *             if the operation fails
     */
    protected String getAllHTML(Context context, String args[], StringList classificationInterfaces, String mode) throws Exception {
        ...
        if (classificationInterfaces.size() != 0) {
            ...
            while (clsIfIter.hasNext()) {
                ...
                bodyBuf.append(getClassificationHTML(mode, clsIf));
            }
        }
     ...
     *            the eMatrix code>Context</code> object
     * @param args
     *            holds the following arguments 0 - requestMap contains mode and objectIds 1 - paramMap maximum number of columns and the fieldMap with fild details.
     * @return a StringList of Interfaces for the form fields.
     * @throws Exception
     *             if the operation fails
     */
    public StringList getClassificationAttributeGroups(Context context, String args[]) throws Exception {
        ...
        StringList classificationInterfaces = emxLibraryCentralUtil_mxJPO.getClassificationInterfaces(_context, objectId);
        ...
        if (classificationInterfaces.size() != 0) {
            ...
            if (classificationMode != null && "editAttributes".equals(classificationMode) && selectedClassIds != null && !"".equals(selectedClassIds)) { // called from classification page
                buf.append("temp query bus $1 $2 $3 where \"$4 and $5\" select $6 $7 name dump $8 recordsep $9");
                accessible = emxLibraryCentralUtil_mxJPO.commaPipeQueryToMapSkipTNR(_context, buf.toString(), false, _if2clsInfo, "Classification", "*", "*",
                        "attribute[" + ClassificationConstants.ATTRIBUTE_MXSYS_INTERFACE + "] matchlist '" + FrameworkUtil.join(classificationInterfaces, ",") + "' ','",
                        " id matchlist " + selectedClassIds + " ','' ','", "attribute[" + ClassificationConstants.ATTRIBUTE_MXSYS_INTERFACE + "]", "id", ",", "|");
                ...
                accessible = emxLibraryCentralUtil_mxJPO.commaPipeQueryToMapSkipTNR(_context, buf.toString(), false, _if2clsInfo, "Classification", "*", "*",
                        "attribute[" + ClassificationConstants.ATTRIBUTE_MXSYS_INTERFACE + "] matchlist '" + FrameworkUtil.join(classificationInterfaces, ",") + "' ','",
                        "attribute[" + ClassificationConstants.ATTRIBUTE_MXSYS_INTERFACE + "]", "id", ",", "|");
            }
            ...
            for (int i = 0; i < allAttributeGroups.size(); i++) {
                emxLibraryCentralUtil_mxJPO.commaPipeQueryToMap(_context, getAgAttrsCmd.toString(), true, _ag2attrs, (String) allAttributeGroups.get(i), ",", "|");
            }
        ...
        /*
         * String attributesHTML = getAllHTML(classificationInterfaces, mode); if (attributesHTML.length() == 0 && classificationMode != null && "editAttributes".equals(classificationMode)) {
     ...
     *             if the operation fails
     */
    public StringList getClassificationAttributes(Context context, String args[]) throws Exception {
     ...
     *             if the operation fails
     */
    public String getHTMLFields(Context context, String args[]) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        String classificationMode = (String) requestMap.get("classificationMode");
        String mode = (String) requestMap.get("mode");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - paramMap contains the objectID 1 - requestMap contains localeObj and timeZone
     * @throws Exception
     *             if the operation fails
     */
    public void updateFields(Context context, String[] args) throws Exception {
        ...
        while (it1.hasNext()) {
            ...
            if (key.endsWith(msSuffix)) {
                ...
                if (dispValue != null && !dispValue.trim().equals("") && timeZone != null) {
                    dateVal = eMatrixDateFormat.getFormattedInputDate(context, dispValue, iClientTimeOffset, locale);
                } else {
                    dateVal = dispValue;
                }
        ...
        while (it2.hasNext()) {
            ...
            if (key.startsWith("#") && key.indexOf(",units_") == -1 && key.indexOf("_tmp") == -1 && !key.endsWith("fieldValue") && !key.endsWith("AmPm") && !key.endsWith("fieldValueOID")) {
    ...
    private void createMultiValuesMap(String attributeName, String value, HashMap newAttrValuesMap) {
        ...
        if (attributeName.indexOf("mva") != -1) {
            ...
            // if the newAttrValuesMap already has a Map, meaning mva attribute value and attribute is already present in the Map
                ...
                // ((Map)existingValue).put(Integer.parseInt(index), value);//IR-505169-3DEXPERIENCER2015x
                if (existingValue instanceof String) {
                    HashMap map = new HashMap();
                    map.put(Integer.parseInt(index), value);
                } else
        ...
        // normal attribute
            ...
            if (existingValue == null)
                ...
                // ((Map)existingValue).put(1, value);//IR-505169-3DEXPERIENCER2015x
                if (existingValue instanceof String) {
                    HashMap map = new HashMap();
                    map.put(1, value);
                } else
     ...
     *             if the operation fails
     */
    public String getHTMLFieldsForClassify(Context context, String args[]) throws Exception {
        ...
        if (selectedClassIds.length() > 0) {
            ...
            classificationInterfaces = emxLibraryCentralUtil_mxJPO.commaPipeQueryToMapSkipTNR(_context, cmd, false, _if2clsInfo, "Classification", "*", "*",
                    "id matchlist '" + selectedClassIds + "' ',' ", "attribute[" + ClassificationConstants.ATTRIBUTE_MXSYS_INTERFACE + "]", "id", ",", "|");
            ...
            for (int i = 0; i < allAttributeGroups.size(); i++) {
                emxLibraryCentralUtil_mxJPO.commaPipeQueryToMap(_context, getAgAttrsCmd, true, _ag2attrs, (String) allAttributeGroups.get(i), ",", "|");
            }
            ...
            while (iterator.hasNext()) {
                ...
                for (int i = 0; i < slAttributes.size(); i++) {
                    ...
                    // changes for New IR - 514442 by KDR2
                    attributeSelect = "attribute[" + attribute + "]";
                    attributeValue = obj.getInfo(context, attributeSelect);
                    if (!(attributeValue.equals("")) || !(attributeValue.equals(null))) {
                        _attr2val.put((String) slAttributes.get(i), attributeValue);
                    } else {
    ...
    /**
     * This method returns initial Attributes HTML for Classification/reClassification
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
     ...
     *             if the operation fails
     */
    public String getAttributesHTML(Context context, String args[]) throws Exception {
    ...
    /**
     * This method reloads attributes on change of class during classificaion/reclassification
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
     ...
     *             if the operation fails
     */
    public Map reloadAttributesHTML(Context context, String args[]) throws Exception {
