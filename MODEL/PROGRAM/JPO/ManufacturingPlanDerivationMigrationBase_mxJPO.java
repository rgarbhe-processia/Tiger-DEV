...
/*
 * CFPMPDerivationMigrationBase.java program migrates the data into Common Document model i.e. to 10.5 schema.
 *
...
/**
 * The <code>CFPMPDerivationMigrationBase</code> class contains code to migrate Manufacturing Plan structure of the Models found in step1. This needs to be done for supporting Branching in 2012x
 */
public class ManufacturingPlanDerivationMigrationBase_mxJPO extends emxCommonMigration_mxJPO {
     ...
     */
    public ManufacturingPlanDerivationMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (isDMCInstall) {
                 ...
                 * 
                 * This code needs to be uncommented after phase 2 implementation is done and Pre and Post checks are implemented if(migrationStatus<2) { mqlLogRequiredInformationWriter(
                 * "Products Derivation PreChecks not executed.Please complete PreChecks before migration. \n"); bIsException=true; return -1; }
                 */
            }
        ...
        try {
            ...
            while (iterator.hasNext()) {
                try {
                    ...
                    mapModel = (MapList) domContextBus.getRelatedObjects(context, strRelPattern.toString(), strTypePattern.toString(), slObjSelects, slRelSelects, getTo, getFrom, iLevel, strObjWhere,
                            strRelWhere, limit);
                    ...
                    if (mapModel.size() != 0) {
                        ...
                        try {
                            ...
                            if (strType.compareTo(TYPE_MANUFACTURING_PLAN_MASTER) == 0) {
                                ...
                                DomainObject domMPMaster = DomainObject.newInstance(context, strID);
    ...
    private void verifyModelStructure(Context context, String modelId) throws Exception {
        ...
        try {
            ...
            MapList mapModel = (MapList) modelObject.getRelatedObjects(context, strRelPattern.toString(), strTypePattern.toString(), slObjSelects, slRelSelects, getTo, getFrom, iLevel, strObjWhere,
                    strRelWhere, limit);
            ...
            if (MPMap.size() == 0) {
                ...
                if (MPManagedRootBusIdList.size() == 0 && MPManagedSeriesBusIdList.size() != 0) {
                    ...
                    for (int j = 0; j < MPManagedSeriesBusIdList.size(); j++) {
                        ...
                        mqlLogRequiredInformationWriter("Id: " + currentId + " Type: " + MPManagedSeriesTypeInfoList.get(j) + " Name: " + MPManagedSeriesNameInfoList.get(j) + " Revision: "
                                + MPManagedSeriesRevisionInfoList.get(j) + " \n");
                    ...
                    if (!ProductLineCommon.isNotNull(rootNodeLevelValue)) {
                        mqlLogRequiredInformationWriter("Setting Node Index on Root Node Type: " + MPManagedRootTypeInfoList.get(0) + " Name: " + MPManagedRootNameInfoList.get(0) + " Revision: "
                                + MPManagedRootRevisionInfoList.get(0) + " Id: " + strRootNodeId + " \n");
                        attributeToSetup = DerivationUtil.createDerivedNode(context, null, "Level0", MPManagedRootTypeInfoList.get(0).toString());
                    }

                    mqlLogRequiredInformationWriter("Setting Rev Index on Root Node Type: " + MPManagedRootTypeInfoList.get(0) + " Name: " + MPManagedRootNameInfoList.get(0) + " Revision: "
                            + MPManagedRootRevisionInfoList.get(0) + " Id: " + strRootNodeId + " \n");
    ...
    private void setNodeIndexAttribute(Context context, String objectId) throws Exception {
        ...
        StringList childDerivationLevelList = ProductLineUtil.convertObjToStringList(context,
                selectableMap.get("from[" + RELATIONSHIP_MAIN_DERIVED + "].to.attribute[" + ATTRIBUTE_DERIVATION_LEVEL + "].value"));
        ...
        if (ProductLineUtil.convertObjToStringList(context, selectableMap.get("from[" + RELATIONSHIP_DERIVED + "].to.attribute[" + ATTRIBUTE_DERIVATION_LEVEL + "].value")).size() > 0) {
            childDerivationLevelList
                    .addAll(ProductLineUtil.convertObjToStringList(context, selectableMap.get("from[" + RELATIONSHIP_DERIVED + "].to.attribute[" + ATTRIBUTE_DERIVATION_LEVEL + "].value")));
        }
        ...
        if (firstLevelChildList.size() > 0) {
            for (int i = 0; i < firstLevelChildList.size(); i++) {
                ...
                if (!ProductLineCommon.isNotNull(level)) {
                    ...
                    mqlLogRequiredInformationWriter(
                            "Node Index attribute already present, skipping generation of Node Index Attribute for id: " + childId + " with parent id: " + objectId + " with level: " + level + "\n");
    ...
    /**
     * Sets the migration status as a property setting. Status could be :- MigrationPreCheckCompleted MigrationCompleted MigrationPostCheckCompleted
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strStatus
    ...
    public int getAdminMigrationStatus(Context context) throws Exception {
        ...
        String result = MqlUtil.mqlCommand(context, "print program $1 select $2 dump", "eServiceSystemInformation.tcl", "property[ManufacturingPlanDerivationMigration].value");
    ...
    /**
     * Outputs the help for this migration. Note: This method can be run only after running PLCDerivationFindObjects
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
