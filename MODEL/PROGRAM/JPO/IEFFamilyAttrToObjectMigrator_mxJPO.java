...
/*
 * Family table modifications 1. Create objects for instances (major and minor) 2. Connect instances with family like relation 3. Change type of family type objects 4. set attributes on instance
 * objects 5. set lock and owner 6. Clean attributes related to Attribute based configuration 7. Move CAD SubComponent/Part Specification relations to appropriate instance object 8. Promote Instances
 * to Family object's state
 *
 * ProE file name modification 1. remove .N extension for ProE files 2. Change Hashcode attribute to refer to this new file name
 *
 * Working set modifications 1. Change type of working set entries of family objects 2. Add corresponding instance object entries for each family entry 3. Change file name entry for ProE files to chop
 * off .N extension
 *
 */
...
public class IEFFamilyAttrToObjectMigrator_mxJPO {
    ...
    public IEFFamilyAttrToObjectMigrator_mxJPO(Context context, String[] args) throws Exception {
    ...
    private Hashtable createAndConnectInstances(Context context, String familyId, Hashtable instanceMajorNameIdTable, Hashtable instanceOldRevNameIdTable, MCADGlobalConfigObject globalConf,
            Hashtable idDetailsTable, boolean isMajor) throws Exception {
    ...
    private void changeFamilyType(Context context, MCADGlobalConfigObject globalConf, String familyId, String familyCadType, String familyType, boolean isMajor, String integrationName)
            throws Exception {
    ...
    private String createInstaceObject(Context context, MCADGlobalConfigObject globalConf, String instType, String instName, String instRev, String policy, String instOldRevId, Map familyMap,
            IEFXmlNodeImpl cadObjNode) throws Exception {
            ...
            if (tokenizer.countTokens() > 3) {
                ...
                if (familyTNRInstanceDetailsTable.containsKey(type + "|" + name + "|" + rev)) {
                    ...
                    for (int i = 0; i < instanceWSDetails.size(); i += 2) {
                        ...
                        modifiedWSEntry.append(preDirectoryStr).append(directory).append(postDirectoryStr).append("\n");
                    }
                }
                ...
                if (fileName.equals(origFileName) && directory.equals(origDirectory))
                    ...
                    modifiedWSEntry.append(type).append("|").append(name).append("|").append(rev).append("|").append(id).append("|").append(format).append("|").append(directory).append("|")
                            .append(fileName).append("|").append(lock).append("|").append(refCount).append("\n");
            } else {
                modifiedWSEntry.append(line).append("\n");
            }
        }
