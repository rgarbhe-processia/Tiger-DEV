...
/**
 * The <code>emxFileUtilBase</code> class contains utility methods for getting data using configurable table APPFileSummary
...
public class emxCommonFileUIBase_mxJPO {
     ...
     * @grade 0
     */
    public emxCommonFileUIBase_mxJPO(Context context, String[] args) throws Exception {
    }
     ...
     * @since EC 10-5.
     */
    public Boolean isInTreeContent(Context context, String[] args) throws Exception {
     ...
     * @since EC 10-5.
     */
    public Boolean isPopup(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String isPopup = (String) paramMap.get("popup");
        if ("true".equalsIgnoreCase(isPopup)) {
            return Boolean.valueOf(true);
    ...
    /**
     * This method is used to get the list of files in master (i.e. document holder) object
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * This method is used to show the Lock image. This method is called from the Column Lock Image.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - objectList MapList
        ...
        try {
            ...
            for (int i = 0; i < objectListSize; i++) {
                ...
                if ("TRUE".equalsIgnoreCase(fileLocked)) {
                    statusImageString = "<img border=\"0\" src=\"../common/images/iconStatusLocked.gif\" alt=\"" + XSSUtil.encodeForHTMLAttribute(context, fileLocker) + "\" title=\""
                            + XSSUtil.encodeForHTMLAttribute(context, fileLocker) + "\"/>";
                } else {
    ...
    /**
     * method to get file format info from master object Used in File Summary, Version Summary Page
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * method to get file size info from master object Used in File Summary, Version Summary Page
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            an array of String arguments for this method
     ...
     * @grade 0
     */
    public static Vector getNonVersionableFileActions(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (objectListItr.hasNext()) {
                ...
                canCheckout = "true".equalsIgnoreCase((String) objectMap.get(CommonDocument.SELECT_HAS_LOCK_ACCESS))
                        && "true".equalsIgnoreCase((String) objectMap.get(CommonDocument.SELECT_HAS_CHECKOUT_ACCESS)) && "false".equalsIgnoreCase(fileLocked);

                canCheckin = "true".equalsIgnoreCase((String) objectMap.get(CommonDocument.SELECT_HAS_CHECKIN_ACCESS)) && "true".equalsIgnoreCase(fileLocked) && context.getUser().equals(fileLockedBy);
                ...
                if (canViewAndDownload) {
                    if (!isprinterFriendly) {
                        downloadURL = "javascript:callCheckout('" + XSSUtil.encodeForJavaScript(context, objectId) + "','download', '" + XSSUtil.encodeForJavaScript(context, fileName) + "', '"
                ...
                if (canCheckout) {
                    if (!isprinterFriendly) {
                        checkoutURL = "javascript:findFrame(parent,'listFilter').callCheckout('" + XSSUtil.encodeForJavaScript(context, objectId) + "','checkout', '"
                                + XSSUtil.encodeForJavaScript(context, fileName) + "', '" + XSSUtil.encodeForJavaScript(context, fileFormat) + "');";
                    ...
                    if (!isprinterFriendly) {
                        checkinURL = "../components/emxCommonDocumentPreCheckin.jsp?override=false&amp;showComments=false&amp;refreshTable=true" + "&amp;objectId="
                                + XSSUtil.encodeForJavaScript(context, objectId) + "&amp;append=true&amp;objectAction=" + CommonDocument.OBJECT_ACTION_CHECKIN_WITHOUT_VERSION;
    ...
    public Object getFiles(Context context, String[] args) throws Exception {
        try {
            ...
            MapList versionList = masterObject.getRelatedObjects(context, CommonDocument.RELATIONSHIP_ACTIVE_VERSION, CommonDocument.TYPE_DOCUMENTS, versionSelectList, null, false, true, (short) 1,
            ...
            String currentState = (String) masterObjectMap.get(CommonDocument.SELECT_CURRENT);
            ...
            while (versionItr.hasNext()) {
                ...
                fileVersionMap.put(CommonDocument.SELECT_CURRENT, currentState);
        ...
        try {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                if ("TRUE".equalsIgnoreCase(fileLocked)) {
                    ...
                    statusImageString = "<img border=\"0\" src=\"../common/images/iconStatusLocked.gif\" alt=\"" + XSSUtil.encodeForHTMLAttribute(context, fileLocker) + "\" title=\"" + titleText
                            + " : " + XSSUtil.encodeForHTMLAttribute(context, fileLocker) + "\"/>";
                }
                showLock.add(statusImageString);
            }
    ...
    /**
     * Method to get the file names. Used in File Summary, Version Summary Page.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed HashMap of the following entries: objectList- a MapList of objects information. paramList- a HashMap of parameter values including reportFormat, relId, and
     *            trackUsagePartId
    ...
    public Vector getFileName(Context context, String[] args) throws Exception {
        ...
        String strLink = "<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?mode=insert&amp;emxSuiteDirectory=components&amp;relId=null&amp;jsTreeID="
                + (String) paramList.get("jsTreeID");
        ...
        for (int i = 0; i < ml.size(); i++) {
            ...
            strFileName = FrameworkUtil.findAndReplace(str, " ", "&nbsp;");
            // END BUG: 347008
            if (bExport) {
                ...
                fileRevisionVector
                        .add("<nobr>" + defaultTypeIcon + "&#160;" + XSSUtil.encodeForHTML(context, (String) objectMap.get("attribute[" + DomainConstants.ATTRIBUTE_TITLE + "]")) + "</nobr>");
            } else {// changed for 366568 : parameters for width n height are given 700 X 600
                    // String sLink = strLink +
                    // "&parentOID="+objectMap.get("masterId")+"&objectId="+objectMap.get(DomainConstants.SELECT_ID)+"&AppendParameters=true&trackUsagePartId="+strPartId+"', '700', '600', 'false',
                    // 'popup', '')\">";

                String sLink = strLink + "&amp;parentOID=" + XSSUtil.encodeForJavaScript(context, masterId) + "&amp;objectId=" + objectMap.get(DomainConstants.SELECT_ID)
                        + "&amp;AppendParameters=true&amp;trackUsagePartId=" + strPartId + "', '700', '600', 'false', 'content', '');\">";

                String strURL = sLink + defaultTypeIcon + "</a> ";
                // SR00472458
                // strURL += sLink + XSSUtil.encodeForXML(context,strFileName) + "</a>&#160;";
                // Rashmi: TIGTK-14249 : Commented DS SR fix above as it was failing for Space character in filename and customized below code - START
                strFileName = XSSUtil.encodeForXML(context, strFileName);
                strFileName = FrameworkUtil.findAndReplace(strFileName, "&#x26;nbsp&#x3b;", " ");
                strURL += sLink + strFileName + "</a>&#160;";
                // Rashmi: TIGTK-14249 END
    ...
    /**
     * Method to get the file revisions. Used in File Summary, Version Summary Page.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed HashMap of the following entries: objectList- a MapList of objects information. paramList- a HashMap of parameter values including reportFormat, relId, and
    ...
    public Vector getFileRevision(Context context, String[] args) throws Exception {
        ...
        String strLink = "<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTable.jsp?program=emxCommonFileUI%253AgetVersions&amp;table=APPFileVersions&amp;sortColumnName=Version&amp;sortDirection=descending&amp;header=emxComponents.Common.VersionsPageHeading&amp;FilterFramePage=..%252Fcomponents%252FemxCommonDocumentCheckoutUtil.jsp&amp;FilterFrameSize=1&amp;HelpMarker=emxhelpdocumentfileversions&amp;CancelButton=true&amp;CancelLabel=emxComponents.Common.Close&amp;emxSuiteDirectory=components&amp;relId=null&amp;jsTreeID="
                + paramList.get("jsTreeID") + "&amp;suiteKey=Components&amp;trackUsagePartId=" + strPartId;
        String[] strArrayIds = new String[objectList.size()];
        for (int i = 0; i < objectList.size(); i++) {
            Map objectMap = (Map) objectList.get(i);
            strArrayIds[i] = (String) objectMap.get(DomainConstants.SELECT_ID);
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (bExport || bPrintMode) {
                ...
                String sLink = strLink + "&amp;parentOID=" + objectMap.get("masterId") + "&amp;objectId=" + objectMap.get("id") + "', '700', '600', 'false', 'popup', '')\">"
                        + (String) objectMap.get(DomainConstants.SELECT_REVISION) + "</a>";
    ...
    /**
     * Method to format file size close to two decimal place 512 Bytes --> 512 B 748 Bytes --> 0.73 KB
     * @param fileSize
     ...
     * @grade 0
     */
    public static Vector getFileActions(Context context, String[] args) throws Exception {
        Vector fileActionsVector = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objectList = (MapList) programMap.get("objectList");
            ...
            while (objectListItr.hasNext()) {
                ...
                if (CommonDocument.canDownload(context, objectMap)) {
                    if (!isprinterFriendly) {
                        downloadURL = "javascript:getTopWindow().callCheckout('" + XSSUtil.encodeForJavaScript(context, masterId) + "','download', '" + XSSUtil.encodeForJavaScript(context, fileName)
                                + "', '" + XSSUtil.encodeForJavaScript(context, fileFormat) + "', null, null, null, null, '" + XSSUtil.encodeForJavaScript(context, strPartId) + "', '" + revision
                ...
                if (CommonDocument.canCheckout(context, objectMap) && isLatestRevision) {
                    if (!isprinterFriendly) {
                        checkoutURL = "javascript:getTopWindow().callCheckout('" + XSSUtil.encodeForJavaScript(context, masterId) + "','checkout', '" + XSSUtil.encodeForJavaScript(context, fileName)
                                + "', '" + XSSUtil.encodeForJavaScript(context, fileFormat) + "', null, null, null, null, '" + XSSUtil.encodeForJavaScript(context, strPartId) + "', '" + revision
                                + "',null,null, null,null, null,null ,null,null, null, '" + versionId + "');";
                        fileActionsStrBuff.append("<a href=\"" + checkoutURL + "\">");
                        fileActionsStrBuff.append("<img border=\"0\" src=\"../common/images/iconActionCheckOut.gif\" alt=\"" + sTipCheckout + "\" title=\"" + sTipCheckout + "\"></img></a>&#160;");
                    ...
                    if (!isprinterFriendly) {
                        ...
                        if (isAnyFileLockedByContext) {
                            checkinURL = "../components/emxCommonDocumentPreCheckin.jsp?showComments=required&amp;refreshTable=true&amp;deleteFromTree="
                                    + XSSUtil.encodeForJavaScript(context, versionId) + "&amp;objectId=" + XSSUtil.encodeForURL(context, masterId)
                                    + "&amp;showFormat=readonly&amp;append=true&amp;objectAction=" + CommonDocument.OBJECT_ACTION_UPDATE_MASTER + "&amp;format=" + encodedFormat + "&amp;oldFileName="
                                    + XSSUtil.encodeForJavaScript(context, encodedFileName);
    ...
    /**
     * returns Maplist containing Versions Info for the file Used for Version Summary Page
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            an array of String arguments for this method
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getVersions(Context context, String[] args) throws Exception {
        ...
        masterId = UIUtil.isNullOrEmpty(masterId) || objectId.equals(masterId) ? (UIComponent.hasReadAccess(context, objectId) ? versionObject.getInfo(context, CommonDocument.SELECT_MASTER_ID) : "")
                : masterId;
        if (!UIUtil.isNullOrEmpty(masterId)) {
            ...
            //
            MapList revisionsList = versionObject.getRevisionsInfo(context, busSelects, versionMultiSelectList);
    ...
    /**
     * method to get file format info from Version objects Used in File Summary, Version Summary Page
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            an array of String arguments for this method
    ...
    /**
     * method to get file size info from Version objects Used in File Summary, Version Summary Page
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            an array of String arguments for this method
    ...
    public static Vector getVersionFileActions(Context context, String[] args) throws Exception {
        Vector fileActionsVector = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objectList = (MapList) programMap.get("objectList");
            ...
            while (objectListItr.hasNext()) {
                ...
                if (canViewAndDownload && !UINavigatorUtil.isMobile(context)) {
                    if (is3DViaSupported(context, encodedFileName)) {
                        if (!isprinterFriendly) {
                            s3DViaURL = "../components/emxLaunch3DLiveExamine.jsp?objectId=" + XSSUtil.encodeForJavaScript(context, masterId) + "&amp;mode=fileBased&amp;fileName=" + encodedFileName
                                    + "&amp;fileFormat=" + encodedFormat;
                            fileActionsStrBuff.append("<a href='javascript:showModalDialog(\"" + s3DViaURL + "\",600,600)'>");
                            fileActionsStrBuff.append("<img border='0' src='../common/images/iconSmallShowHide3D.gif' alt=\"" + sTipDownload + "\" title=\"" + sTip3DVIAViewer + "\"></img></a>&#160;");
                        } else {
                            fileActionsStrBuff.append("<img border='0' src='../common/images/iconSmallShowHide3D.gif' alt=\"" + sTipDownload + "\" title=\"" + sTip3DVIAViewer + "\"></img></a>&#160;");
    ...
    public Object getFileVersions(Context context, String[] args) throws Exception {
        try {
            ...
            while (versionItr.hasNext()) {
                ...
                // for the Id passed, get revisions Info
                MapList revisionsList = versionObj.getRevisionsInfo(context, versionSelectList, versionMultiSelectList);
     ...
     *            the fileName to be checked out
     * @param bUIType
     *            to check the request is from SB or Table page
     * @returns String URL for all viewers
     * @throws Exception
     *             if the operation fails
     * @since Common 10.5
     ...
     *            the fileName to be checked out
     * @param partId
     *            the partId for which document belongs to
     * @returns String URL for all viewers
     * @throws Exception
     *             if the operation fails
     * @since Common 11.0
    ...
    public static String getViewerURL(Context context, String objectId, String format, String fileName, String partId, boolean bUIType) throws Exception {
        try {
            ...
            if (formatDetailsMap == null) {
                if (!bUIType)
                    viewerURL = "<a href=\"javascript:callCheckout('" + XSSUtil.encodeForJavaScript(context, objectId) + "','view', '" + XSSUtil.encodeForJavaScript(context, fileName) + "', '"
                            + XSSUtil.encodeForJavaScript(context, format) + "', null, null, null, null, '" + XSSUtil.encodeForJavaScript(context, partId)
                            + "');\"><img src=\"../common/images/iconActionView.png\" border=\"0\" alt=\"" + i18nViewerTip + "\" title=\"" + i18nViewerTip + "\"></img></a>";
                else
                    viewerURL = "<a href=\"javascript:openViewer('" + XSSUtil.encodeForJavaScript(context, objectId) + "','view', '" + XSSUtil.encodeForJavaScript(context, fileName) + "', '"
                            + XSSUtil.encodeForJavaScript(context, format) + "', null, null, null, null, '" + XSSUtil.encodeForJavaScript(context, partId)
                            + "');\"><img src=\"../common/images/iconActionView.png\" border=\"0\" alt=\"" + i18nViewerTip + "\" title=\"" + i18nViewerTip + "\"></img></a>";
                returnURL = viewerURL;
            } else {
                ...
                while (itr.hasNext()) {
                    ...
                    if (viewer == null || "".equals(viewer) || "null".equals(viewer) || viewerURL.equals(viewer)) {
                        ...
                        if (viewerTip.equalsIgnoreCase("Default")) {
                            if (!bUIType)
                                viewerURL = "<a href=\"javascript:getTopWindow().callCheckout('" + XSSUtil.encodeForJavaScript(context, objectId) + "','view', '"
                                        + XSSUtil.encodeForJavaScript(context, fileName) + "', '" + XSSUtil.encodeForJavaScript(context, format) + "', null, null, null, null, '"
                                        + XSSUtil.encodeForJavaScript(context, partId) + "');\"><img src=\"../common/images/iconActionView.png\" border=\"0\" alt=\"" + i18nViewerTip + "\" title=\""
                                        + i18nViewerTip + "\"></img></a>";
                            else
                                viewerURL = "<a href=\"javascript:openViewer('" + XSSUtil.encodeForJavaScript(context, objectId) + "','view', '" + XSSUtil.encodeForJavaScript(context, fileName)
                                        + "', '" + XSSUtil.encodeForJavaScript(context, format) + "', null, null, null, null, '" + XSSUtil.encodeForJavaScript(context, partId)
                            ...
                            viewerURL = "<a href=\"javascript:callViewer('" + XSSUtil.encodeForJavaScript(context, objectId) + "','view', '" + XSSUtil.encodeForJavaScript(context, fileName) + "', '"
                                    + XSSUtil.encodeForJavaScript(context, format) + "', '" + XSSUtil.encodeForJavaScript(context, viewerURL) + "', '" + XSSUtil.encodeForJavaScript(context, partId)
                                    + "');\"><img src=\"../common/images/iconActionView.png\" border=\"0\" alt=\"" + i18nViewerTip + "\" title=\"" + i18nViewerTip + "\"></img></a>";
                        }
                    }
                }
     ...
     * @throws Exception
     */
    private static boolean is3DViaSupported(Context context, String sFileName) throws Exception {
