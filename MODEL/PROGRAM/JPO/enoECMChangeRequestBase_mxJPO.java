...
/**
 * The <code>enoECMChangeOrderBase</code> class contains methods for executing JPO operations related to objects of the admin type Change.
 * @author YOQ/R3D
...
public class enoECMChangeRequestBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @since Ecm R211
     */
    public enoECMChangeRequestBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    public Map createChangeRequest(Context context, String[] args) throws Exception {
        ...
        try {
            changeRequestId = changeRequest.create(context, sType, sPolicy, sVault, sOwner);
        ...
        try {
            ...
            if (!ChangeUtil.isNullOrEmpty(strChangeRequestId)) {
                ...
                if (!bFlag) {
                    ...
                    emxNotificationUtilBase_mxJPO.sendNotification(context, strChangeRequestId, new StringList(strCROwner), ccList, bccList, SubjectKey, MessageKey, lstAttachments, suiteKey, null,
                            null, null);
                }
            }
        } catch (Exception Ex) {
    ...
    public Object getOwnedCR(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            MapList ownedCO = DomainObject.findObjects(context, ChangeConstants.TYPE_CHANGE_REQUEST, // type filter
    ...
    public StringList getRouteTaskAssignedCRs(Context context, String personObjId) throws Exception {
        ...
        String objSelect = "to[" + RELATIONSHIP_PROJECT_TASK + "].from." + "from[" + RELATIONSHIP_ROUTE_TASK + "].to." + "to[" + RELATIONSHIP_OBJECT_ROUTE + "|from.type=='"
                + ChangeConstants.TYPE_CHANGE_REQUEST + "'].from.id";
    ...
    public StringList getRouteTemplateAssignedCRs(Context context, String personObjId) throws Exception {
        ...
        String objSelect = "to[" + RELATIONSHIP_ROUTE_NODE + "|from.type=='" + TYPE_ROUTE_TEMPLATE + "']." + "from.to[" + RELATIONSHIP_INITIATING_ROUTE_TEMPLATE + "].from." + "to["
                + RELATIONSHIP_OBJECT_ROUTE + "|from.type=='" + ChangeConstants.TYPE_CHANGE_REQUEST + "'].from.id";
        ...
        try {
            ...
            if (!hasRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_ORDER, true)) {
                ...
                if (!ChangeUtil.isNullOrEmpty(sCRObjectId)) {
                    ...
                    StringList affectedObjList = changeUtil.getStringListFromMapList(changeActionList, SELECT_ID);
                    if (!ChangeUtil.isNullOrEmpty(affectedObjList.toString())) {
                        // Connects all the CA's to CO which are connect to CR.
                        DomainRelationship.connect(context, changeOrder, ChangeConstants.RELATIONSHIP_CHANGE_ACTION, true, (String[]) affectedObjList.toArray(new String[affectedObjList.size()]),
                                true);
                    }
                }
        ...
        try {
            ...
            while (objectListItr.hasNext()) {
                ...
                if (!UIUtil.isNullOrEmpty(strCAObjID)) {
                    ...
                    sConnectedItems = dmObj.getRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_ACTION, ChangeConstants.TYPE_CHANGE_ORDER, busSelects, null, true, false, (short) 1,
    ...
    public StringList includeCOOIDs(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            String whereclause = "(policy == \"" + ChangeConstants.FASTTRACK_CHANGE + "\") && (current == \"" + ChangeConstants.STATE_FORMALCHANGE_PREPARE + "\") && ((to["
                    + ChangeConstants.RELATIONSHIP_CHANGE_ORDER + "].from.id == \"" + crId + "\") || (to[" + ChangeConstants.RELATIONSHIP_CHANGE_ORDER + "]== \"" + isConnected + "\"))";
            StringList objectSelects = new StringList(SELECT_ID);
            MapList tempList = findObjects(context, ChangeConstants.TYPE_CHANGE_ORDER, "*", whereclause, objectSelects);

            returnList = changeUtil.getStringListFromMapList(tempList, SELECT_ID);
        } catch (Exception e) {
            throw e;
        }
        return returnList;
    ...
    public MapList getChangeOrders(Context context, String strChangeRequestId) throws Exception {
        StringList slObjectSelect = new StringList(SELECT_ID);
        ...
        setId(strChangeRequestId);
        return getRelatedObjects(context, ChangeConstants.RELATIONSHIP_CHANGE_ORDER, ChangeConstants.TYPE_CHANGE_ORDER, slObjectSelect, slRelSelect, false, true, (short) 1, "", EMPTY_STRING, 0);
    }
    ...
    public void connectToCR(Context context, String[] args) throws Exception {
        ...
        if (UIUtil.isNotNullAndNotEmpty(crId)) {
            DomainRelationship.connect(context, new DomainObject(crId), DomainConstants.RELATIONSHIP_ECR_SUPPORTING_DOCUMENT, sketchDomObj);
        }
    ...
    public boolean displayGovernedCO(Context context, String[] args) throws Exception {
        ...
        return DomainObject.newInstance(context, objectId).isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST) ? new Boolean(true) : new Boolean(false);
    ...
    public StringList excludeConnectSupportingDocOIDs(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            StringList objectSelects = new StringList(SELECT_ID);
            MapList tempList = findObjects(context, TYPE_SKETCH + "," + TYPE_MARKUP, "*", whereclause, objectSelects);
            returnList = changeUtil.getStringListFromMapList(tempList, SELECT_ID);
        } catch (Exception e) {
            throw e;
        }
    ...
    /**
     * @author YOQ this method performs the cancel process of change - The Affected CAs,Affected Items, Routes,Reference Documents,Prerequisites Connected to this Particular CO are Disconnected and
    ...
    /**
     * @author YOQ this method performs the hold process of change.Moves all associated CAs to Cancel state.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the Change Process
    ...
    /**
     * @author YOQ Resumes the Hold Changes and sends notification and updates the history
     * @param context
     * @throws Exception
     */
