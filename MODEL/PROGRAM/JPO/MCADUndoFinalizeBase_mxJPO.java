...
public class MCADUndoFinalizeBase_mxJPO extends IEFCommonUIActions_mxJPO {
    ...
    public void executeCustom(Context _context, Hashtable resultAndStatusTable) throws MCADException {
        ...
        try {
            ...
            String busObjId = _busObject.getObjectId();
            IEFBaselineHelper baselineHeper = new IEFBaselineHelper(_context, _globalConfig, _serverResourceBundle, new IEFGlobalCache());

            boolean isObjectConnectedToBaseline = baselineHeper.isBaselineRelationshipExistsForId(_context, busObjId);

            if (isObjectConnectedToBaseline) {
                Hashtable tokensTable = new Hashtable(2);
                tokensTable.put("NAME", _busObject.getName());
                String msg = _serverResourceBundle.getString("mcadIntegration.Server.Message.CantDemoteObjectWithBaselineAttached", tokensTable);
    ...
    private void demoteFinalizedInstance(Context context, BusinessObject famObj, BusinessObject majorInstObject, StringBuffer majorMinorObjIdString, StringList familyIds) throws Exception {
        ...
        /*
         * for(int k=0; k <familyIds.size(); k++) { BusinessObject famObjToConnWithInst = new BusinessObject((String)familyIds.get(k));
    ...
    private void demoteConnectedInstances(Context context, Collection busIDs, String targetState, String demotedFamId, String demotedInstId, StringBuffer majorMinorObjIdString) throws MCADException {
        String name = "";
        String id = "";
        String type = "";
            ...
            for (int i = 0; i < busWithSelectList.size(); i++) {
                ...
                for (int j = 0; j < familyIds.size(); j++) {
                    ...
                    for (int k = 0; k < familyStates.size(); k++) {
                        ...
                        if (demotedInstId.equals(majorInstObject.getObjectId(context)) && !connFamId.equals(demotedFamId)
                                && stateList.indexOf(stateOfConnectedFam) >= stateList.indexOf(instCurrentState)) {
                        ...
                        if ((stateList.indexOf(stateOfConnectedFam) >= stateList.indexOf(instCurrentState) && stateList.indexOf(targetStateTodemoteInst) < stateList.indexOf(instCurrentState)
    ...
    /*
     * Undoes the finalization process. Steps: 1. Files will be moved back to the versioned object. 2. "Finalized" relationship will be disconnected. 3. All "whereused" cases of major object will be
     * rplaced by minor object.
     */
    protected String undoFinalize(Context _context, BusinessObject majorObject, String familyMinorObjId) throws MCADException, MatrixException {
        ...
        try {
            // start [NDM] :- NE4
            /*
             * String sFinalizedRelName = MCADMxUtil.getActualNameForAEFData(_context, "relationship_Finalized"); //Get the minor type business object BusinessObject minorObject = null; Relationship
             * finalizedRel = null;
             * 
             * String minorObjId = ""; String majorObjId = "";
             */
            majorObject.open(_context);
            dodemotionOperation(_context, majorObject);
            /*
             * 
             * BusinessObjectList list = _util.getRelatedBusinessObjects(_context, majorObject, sFinalizedRelName, "to"); if(list != null && list.size() > 0) { BusinessObjectItr itr = new
             * BusinessObjectItr(list); while(itr.next()) { minorObject = itr.obj(); } }
             * 
             * if(minorObject == null) { //check the "IsFinalized" attribute on the "VersionOf" relationship String sFinalizedAttribName = MCADMxUtil.getActualNameForAEFData(_context,
             * "attribute_IsFinalized");
             * 
             * RelationshipItr relItr = new RelationshipItr(_util.getToRelationship(_context, majorObject, (short)0, false)); while (relItr.next()) { Relationship rel = relItr.obj();
             * if(sVersionOfRelName.equals(rel.getTypeName())) { //get the attributes AttributeItr attribItr = new AttributeItr(rel.getAttributes(_context)); while (attribItr.next()) { Attribute
             * thisAttrib = attribItr.obj(); if( (thisAttrib.getName().equals(sFinalizedAttribName))&& (thisAttrib.getValue().equalsIgnoreCase("true"))) { minorObject = rel.getFrom(); finalizedRel =
             * rel; break; } } } if(minorObject!=null) { break; } } }
             */

            /**
             * If the Major object is directly finalized at the time of checkin, then Minor object doesn't exists. But for UNdo-finalize, our Assumption is that atleast one minor is always existing.
             * If it doesn't then Undo-finalize will fail. To make the undo finalize work, we need to forcefully create one minor with version 0 and connect it with Finalized major by "VersionOf"
             * Relationship.
             */

            /*
             * boolean bDirectlyFinalized = false; boolean isVersionOfRelExists = false; if(minorObject == null) { // Check if there is any minor object in this stream // If yes, //
             * createNextMinorInStream // else // createFirstMinorInStream // get bogus object of CDM minorObject = _util.getActiveMinor(_context, majorObject); if(minorObject == null) { minorObject =
             * createFirstMinorInStream(_context, majorObject); } else if(_util.isCDMInstalled(_context)) { minorObjId = minorObject.getObjectId(); String Args[] = new String[2]; Args[0] = minorObjId;
             * Args[1] = "from[" + sVersionOfRelName + "].to.id";
             * 
             * String result = _util.executeMQL(_context, "print bus $1 select $2 dump", Args); if( result.startsWith("false") || result.equals("true|") ) { //do nothing. minor object found is bogus
             * minor. } else { String versionedType = _util.getCorrespondingType(_context, majorObject.getTypeName()); String majObjName = majorObject.getName(); String majRev =
             * majorObject.getRevision(); String latestminorversion = _generalUtil.getLatestRevisionStringForStream(_context, versionedType,majObjName,majRev, majRev); minorObject =
             * createNextMinorInStream(_context, versionedType, majObjName, latestminorversion); isVersionOfRelExists = true; } } else { String versionedType = _util.getCorrespondingType(_context,
             * majorObject.getTypeName()); String majObjName = majorObject.getName(); String majRev = majorObject.getRevision(); String latestminorversion =
             * _generalUtil.getLatestRevisionStringForStream(_context, versionedType,majObjName,majRev, majRev);
             * 
             * minorObject = createNextMinorInStream(_context, versionedType, majObjName, latestminorversion); isVersionOfRelExists = true; }
             * 
             * minorObjId = minorObject.getObjectId();
             * 
             * if(!isVersionOfRelExists) { //connect the newly created bus obj to the Target Rev BO _util.connectBusObjects(_context, minorObjId, majorObject.getObjectId(), sVersionOfRelName,
             * true,null); } bDirectlyFinalized = true; }
             * 
             * majorObjId = majorObject.getObjectId(); minorObjId = minorObject.getObjectId();
             * 
             * if(bDirectlyFinalized) { String cadType = _util.getCADTypeForBO(_context, majorObject);
             * 
             * if(_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE) && !familyMinorObjId.equals("")) { BusinessObject familyObject = new
             * BusinessObject(familyMinorObjId); familyObject.open(_context);
             * 
             * BusinessObject majorFamilyObject = _util.getMajorObject(_context, familyObject);
             * 
             * if(majorFamilyObject == null) { Hashtable exceptionDetails = new Hashtable(3); exceptionDetails.put("TYPE", familyObject.getTypeName()); exceptionDetails.put("NAME",
             * familyObject.getName()); exceptionDetails.put("REVISION", familyObject.getRevision());
             * 
             ...
             * 
             * _generalUtil.connectFamilyClassObjects(_context, familyObject, minorObject); familyObject.close(_context); } else if(_globalConfig.isTypeOfClass(cadType,
             * MCADAppletServletProtocol.TYPE_FAMILY_LIKE)) { String instanceOfRelName = MCADMxUtil.getActualNameForAEFData(_context,"relationship_InstanceOf"); _util.moveRelationShips(_context,
             * majorObject, minorObject, instanceOfRelName, "from"); } } retStr.append(majorObjId); retStr.append("|"); retStr.append(minorObjId);
             */
            ...
            /*
             * String breakPartSpec = (String)_argumentsTable.get("BreakPartSpec"); String partSpecRelName = MCADMxUtil.getActualNameForAEFData(_context,"relationship_PartSpecification");
             * if("true".equalsIgnoreCase(breakPartSpec)) { _sourceName = _generalUtil.getCSENameForBusObject(_context, majorObject); // Break Part Specification from the major Object
             * removePartRelations(_context, majorObject); //If EBOM Synch is done with the minor object //then get a list of all the minor Objects and //break part specification BusinessObjectList
             * MinorObjList = _util.getMinorObjects(_context, majorObject); BusinessObjectItr MinorObjListItr = new BusinessObjectItr(MinorObjList); while(MinorObjListItr.next()) { BusinessObject
             * MinorObjectNew = MinorObjListItr.obj(); removePartRelations(_context, MinorObjectNew); }
             * 
             * } //end [NDM] NE4
             * 
             * // start [NDM] :- NE4 /*else if(partSpecRelName != null && partSpecRelName.length() > 0) { if(!isAssignPartToMajor(_context)) { _util.moveRelationShips(_context, majorObject,
             * minorObject, partSpecRelName, "to");
             * 
             * StringList selectBus = new StringList(); StringList selectRel = new StringList();
             * 
             ...
             * RelationshipWithSelectList relSelList = expansionWithSelect.getRelationships(); if(relSelList.size() > 0) { String cadObjectAttrName =
             * MCADMxUtil.getActualNameForAEFData(_context,"attribute_CADObjectName");
             * 
             * Enumeration relationsList = relSelList.elements(); while(relationsList.hasMoreElements()) { RelationshipWithSelect select = (RelationshipWithSelect)relationsList.nextElement();
             * select.open(_context); boolean isPushed = false; try { com.matrixone.apps.domain.util.ContextUtil.pushContext(_context); isPushed= true;
             * 
             * _util.setRelationshipAttributeValue(_context, select, cadObjectAttrName, minorObject.getName()); } finally { if(isPushed)
             * com.matrixone.apps.domain.util.ContextUtil.popContext(_context); } select.close(_context); } } } }
             */
            ...
            // start [NDM] :- NE4
            /*
             * if(_util.isCDMInstalled(_context)) { String moveFilesToVersionAttrName = MCADMxUtil.getActualNameForAEFData(_context,"attribute_MoveFilesToVersion");
             * _util.setAttributeOnBusObject(_context, majorObject,moveFilesToVersionAttrName,"True"); }
             * 
             * if(_globalConfig.isCreateVersionObjectsEnabled()) _util.copyAttributesForFinalization(_context, _globalConfig, majorObject, minorObject,COPY_BLANK_ATTR_DURING_UNDO_FINALIZE);
             * 
             * // transfer description Also. String desc = majorObject.getDescription(_context); if(!desc.equals("") || COPY_BLANK_ATTR_DURING_UNDO_FINALIZE) minorObject.setDescription(_context,desc);
             * minorObject.update(_context);
             * 
             * //Reset Attributes on Minor //resetAttributes(majorObject);
             * 
             * RelationshipItr relItr = new RelationshipItr(_util.getToRelationship(_context, majorObject,(short)0, false)); while (relItr.next()) { Relationship rel = relItr.obj();
             * if(sFinalizedRelName.equals(rel.getTypeName())) { majorObject.disconnect(_context, rel); } }
             */

            // start [NDM] NE4:-

            /*
             * String sFinalizedAttribName = MCADMxUtil.getActualNameForAEFData(_context,"attribute_IsFinalized"); if(finalizedRel!= null) _util.setRelationshipAttributeValue(_context, finalizedRel,
             * sFinalizedAttribName, "false");
             * 
             * /* Step 3: All "whereused" cases of major object will be replaced by minor object.
             */
    ...
    /*
     * [NDM] start NE4:- protected BusinessObject createFirstMinorInStream(Context _context, BusinessObject majorObject) throws MCADException, MatrixException { //Assumption: majorObject
     * BusinessObject is Opened and closed by caller. BusinessObject minorObject = null;
     * 
     * String versionedType = _util.getCorrespondingType(_context, majorObject.getTypeName()); String version = _util.getFirstVersionStringForStream(majorObject.getRevision()); String policy =
     * _globalConfig.getDefaultPolicyForType(versionedType); String name = majorObject.getName();
     * 
     * minorObject = new BusinessObject(versionedType, name, version, ""); if(!minorObject.exists(_context)) { minorObject.create(_context, policy); //Set the CAD Type Attribute on newly created minor
     * String cadType = _util.getCADTypeForBO(_context, majorObject); String cadTypeAttName = MCADMxUtil.getActualNameForAEFData(_context,"attribute_CADType"); _util.setAttributeValue(_context,
     * minorObject, cadTypeAttName, cadType); minorObject.open(_context); String majorObjId = majorObject.getObjectId(); String minorObjId = minorObject.getObjectId(); minorObject.close(_context); //
     * connect CDM Relationships for newly created minor objects String relLatestVersion = MCADMxUtil.getActualNameForAEFData(_context,"relationship_LatestVersion"); String relActiveVersion =
     * MCADMxUtil.getActualNameForAEFData(_context, "relationship_ActiveVersion"); _util.connectBusObjects(_context, minorObjId, majorObjId, relLatestVersion, false, null);
     ...
     * return minorObject; }
     * 
     * protected BusinessObject createNextMinorInStream(Context _context, String versionedType, String boName, String version) throws MCADException, MatrixException { BusinessObject minorObject =
     * null;
     * 
    ...
    /*
     * protected void removePartRelations(Context _context, BusinessObject busObject) throws Exception { // For a drawing object (or any type of object which is part of // Associated Spec setting in
     * EBOM config object), // only break Part Spec. The associated Model should not be touched. boolean isDrawing = isObjectTypeAssociatedSpec(_context, busObject); String policyPartSpecification =
     * MCADMxUtil.getActualNameForAEFData(_context, "policy_PartSpecification"); if (!isDrawing) { BusinessObjectList mainPartList = _util.getRelatedBusinessObjects(_context, busObject,
     * policyPartSpecification, "to"); BusinessObjectItr mainPartsItr = new BusinessObjectItr(mainPartList); while(mainPartsItr.next()) { BusinessObject mainPart = mainPartsItr.obj();
     * disconnectRelationships(_context, mainPart, MCADMxUtil.getActualNameForAEFData(_context, "relationship_EBOM"), "from"); } } _util.disconnectRelationShips(_context, busObject,
     * policyPartSpecification, "to");
     * 
     * //get all the children of busObject and recursively remove //part relations Hashtable childObjectList = _generalUtil.getAllWheareUsedObjectsForFinalization(busObject, true,
     * MCADServerSettings.ASSEMBLY_LIKE); Enumeration childObjects = childObjectList.keys(); while(childObjects.hasMoreElements()) { BusinessObject childObject =
     * (BusinessObject)childObjects.nextElement(); removePartRelations(childObject); } }
     */

    // [NDM] NE4 start:-
    /*
     * protected boolean isAssignPartToMajor(Context _context) throws Exception { boolean bAssignedToMajor = false; String confObjTNR = _globalConfig.getEBOMRegistryTNR(); StringTokenizer token = new
     * StringTokenizer(confObjTNR, "|"); if(token.countTokens() < 3) MCADServerException.createException(_serverResourceBundle.getString("mcadIntegration.Server.Message.EBOMRegistryNotDefined"),
     * null);
     * 
     * String confObjType = (String) token.nextElement(); String confObjName = (String) token.nextElement(); String confObjRev = (String) token.nextElement(); IEFEBOMConfigObject ebomConfigObj = new
     * IEFEBOMConfigObject(_context, confObjType, confObjName, confObjRev);
     * 
     * String assignPartToMajor = ebomConfigObj.getConfigAttributeValue(IEFEBOMConfigObject.ATTR_ASSIGN_PART_TO_MAJOR);
     * 
     * if("true".equalsIgnoreCase(assignPartToMajor)) { bAssignedToMajor = true; }
     * 
     * return bAssignedToMajor; } // [NDM] NE4 End:-
     */
    ...
    /**
     * Disconnect all the relationships of given name in which the input bus participates at the input "end".
     */
    protected boolean disconnectRelationships(Context _context, BusinessObject fromBus, String relName, String relEnd) throws Exception {
        boolean bRet = true;
        try {
            short level = 1;
    ...
    /**
     * Checks if all the first level parents of the input bus object are finalized (Only CAD Subcomponent relationship is considered for now..) thorws Exception incase anything unfinalized found
     */
    ...
    /**
     * Do proper rev replace after successful finalization. can implement business rules, default impl. is Disconnect all the children (dependees) of this object undoFinalizedFromObj, Move the 'Parent
     * (from)' relationships from undoFinalizedFromBO to the undoFinalizedToBO
     */
    // [NDM] NE4 start:-
    /*
     * protected boolean manageRelationShipsAfterUndoFinalization(Context _context, BusinessObject undoFinalizedFromObj, BusinessObject undoFinalizedToObj, boolean bDirectlyFinalized ) throws
     * Exception { boolean bRet = true; boolean isPushed = false;
     * 
     * try {
     * 
     * com.matrixone.apps.domain.util.ContextUtil.pushContext(_context); isPushed = true;
     * 
     * // Disconnect all the children (dependees) of this object, undoFinalizedFromBO // Skip the External Ref relations
     * 
     * if(bDirectlyFinalized) { _generalUtil.shuffleAssemblyLikeRelatinships(_context, undoFinalizedFromObj, undoFinalizedToObj, true, false, MCADServerSettings.ASSEMBLY_LIKE); } else {
     ...
     * 
     * // Handle external ref relations - Move all from Ext. Ref. relationships _generalUtil.shuffleRelatinships(_context, undoFinalizedFromObj, undoFinalizedToObj, true, false,
     * MCADServerSettings.EXTERNAL_REFERENCE_LIKE);
     * 
     * // Move all DERIVEDOUTPUT_LIKE relationship from Major to Minor _generalUtil.shuffleRelatinships(_context, undoFinalizedFromObj,undoFinalizedToObj ,true, false,
     * MCADServerSettings.DERIVEDOUTPUT_LIKE);
     * 
     * // Move the 'Parent (from)' relationships (wheare the object in questin is a dependee) // of the undoFinalizedFromBO to the undoFinalizedToBO // Skip the External Ref relations
     * //_generalUtil.shuffleRelatinships(undoFinalizedFromObj, undoFinalizedToObj, false, false, MCADServerSettings.ASSEMBLY_LIKE); _generalUtil.shuffleAssemblyLikeRelatinships(_context,
     * undoFinalizedFromObj, undoFinalizedToObj, false, false, MCADServerSettings.ASSEMBLY_LIKE);
     * 
     * // Handle external ref relations - Move all from Ext. Ref. relationships _generalUtil.shuffleRelatinships(_context, undoFinalizedFromObj, undoFinalizedToObj, false, false,
     * MCADServerSettings.EXTERNAL_REFERENCE_LIKE);
     * 
     * String cadType = this._util.getCADTypeForBO(_context, undoFinalizedFromObj);
     * 
     * if(this._globalConfig.isTypeOfClass(cadType,MCADAppletServletProtocol.TYPE_FAMILY_LIKE)) { String strAciveInstance = MCADMxUtil.getActualNameForAEFData(_context, "relationship_ActiveInstance");
     * _util.disconnectRelationShips(_context, undoFinalizedFromObj,strAciveInstance,"from"); } else if(_globalConfig.isTypeOfClass(cadType,MCADAppletServletProtocol.TYPE_INSTANCE_LIKE))
     * _generalUtil.shuffleActiveInstanceRelatinship(_context, undoFinalizedFromObj, undoFinalizedToObj); } catch(Exception me) { MCADServerException.createException(me.getMessage(), me); }
     * 
     * finally { if(isPushed) { try { com.matrixone.apps.domain.util.ContextUtil.popContext(_context); } catch(Exception ex) { MCADServerException.createException(ex.getMessage(), ex); } } }
     * 
     * return bRet; }
     */
