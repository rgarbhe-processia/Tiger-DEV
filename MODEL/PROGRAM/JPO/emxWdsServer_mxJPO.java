...
/**
 * @author vadikv The <code>emxWdsServer_mxJPO</code> class contains implementation of web service used by ENOVIA provider for Microsoft Desktop Search
...
public class emxWdsServer_mxJPO implements MatrixService {
        ...
        if (mqlCommand.executeCommand(context, mqlQeryBuilder.toString(), dumpSeparator, recordsSeparator)) {
            ...
            if (mqlResult.length() > 0) {
                ...
                for (int i = 0; i < recordsArray.length; i++) {
                    ...
                    /*
                     * Fixed attribute mappings: 1. Title 2. Author 3. Date 4. Type (optional) 5. Folder (optional) 6. ID - ObjectID
                     */
                    resultBuilder.append(itemPrefix);
    ...
    private String getStyle() {
        final String style = "<style>" + "body { background-color: white; }" + "td.label { background-color: #dadada; color: black; font-weight: bold; height: 24px; }"
                + "td.labelRequired { background-color: #dadada; color: #990000; font-weight: bold; height: 24px; ; font-style: italic }"
                + "td.inputField { background-color: #eeeeee; font-size: 12px; }" + "td.requiredNotice {  font-family: Arial, Helvetica, sans-serif; color: #990000}"
                + "td.heading1 { font-size: 10pt; font-weight: bold; border-top: 1px solid #003366;  height: 24px;}"
                + "td.heading2 { font-size: 8pt; font-weight: bold; background-color: #dddddd;  height: 24px;}" + "</style>";
