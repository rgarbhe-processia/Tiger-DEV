...
public class emxTeamECOBase_mxJPO extends emxECO_mxJPO {
     ...
     * @throws Exception
     */
    public emxTeamECOBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    /**
     * Check trigger method on VPLM_SMB_Definition policy to check for the higher rev existence, Release of product revision will be blocked, if there is higher rev exists of the same
     * @param context
     * @param args
     * @return int
     * @throws Exception
        ...
        if (prodObj.isKindOf(context, type)) {
            ...
            if (EngineeringConstants.STATE_VPLM_SMB_RELEASE.equals(prodNextState)) {
                ...
                String connectedDevPartId = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", prodId,
                        "to[" + EngineeringConstants.RELATIONSHIP_PART_SPECIFICATION + "|from.policy==\"" + EngineeringConstants.POLICY_DEVELOPMENT_PART + "\"].from.id");

                if (connectedDevPartId != null && !"".equals(connectedDevPartId)) {
                    if (!prodObj.isLastRevision(context)) {
                        throw new FrameworkException(EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.VPLMProduct.NotLatestProductRevision", context.getSession().getLanguage()));
                    }
                }
            }
        }
    ...
    public String bomGotoProduction(Context context, String[] args) throws Exception {
        ...
        String strECOId = programMap.containsKey("changeId") ? (String) programMap.get("changeId") : (String) paramMap.get("objectId");

        // Selected Affected Item Ids
        String[] strObjIdAr = programMap.containsKey("selectedPartId") ? (String[]) programMap.get("selectedPartId") : (String[]) requestMap.get("selectedPartId");
    ...
    /**
     * This method is called from check trigger when promoting Development Part to Complete state. This method checks wheather the connected Team Change object is in Release state. If Team Change
     * object is in Relese state it returns 0 else it returns 1.
     * @param context
     *            ematrix context.
     * @param args
     *            packed arguments.
    ...
    public int checkConnectedTeamChangeIsInReleaseState(Context context, String[] args) throws Exception {
        ...
        if (!"TRUE".equals(strCalledFromTCpromotion)) {
            ...
            MapList affectedItemList = domPartObj.getRelatedObjects(context, relPattern, typePattern, objectSelect, null, true, false, (short) 1, null, null, null, null, null);
    ...
    /**
     * This method is called from check trigger when promoting Team change to Release state. This method checks wheather all the affected items connected to it is in Peer Review state. If all the
     * Affected Items are in Peer Review state it returns 0 else it return 1.
     * @param context
     *            ematrix context.
     * @param args
     *            packed arguments.
    ...
    public int checkAllTheAffectedItemsAreInPeerReviewState(Context context, String[] args) throws Exception {
        ...
        MapList affectedItemList = domTeamObj.getRelatedObjects(context, EngineeringConstants.RELATIONSHIP_AFFECTED_ITEM, DomainConstants.TYPE_PART, null, null, false, true, (short) 1, objectWhere,
                null, null, null, null);
    ...
    /**
     * This method will be called as an action trigger of Team Change Policy when promoting to Release state. This method releases the affected item connected to it.
     * @param context
     *            ematrix context.
     * @param args
     *            packed arguments.
