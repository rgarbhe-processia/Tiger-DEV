...
public class MCADIntegGetDrawingBase_mxJPO {
     ...
     * @return a ParsedXML which contains nodes strcuture to be shown in checkout page. XML structure should be as shown below. <viewdetails> <node busid="" relid="" isfloated="">
     *         <node busid="" relid="" isfloated=""/> </node> <node busid="" relid="" isfloated=""/> </viewdetails>
     */
    ...
    private IEFXmlNode getVerticalViewBOIDs(Context context, String busId, HashSet expandedObjectIds, String expandDrawing, HashMap relIdParentDetails, Hashtable busIdFloatedRelIdsMap)
            throws Exception {
    ...
    // [NDM] QWJ Start
    /*
     * private Hashtable getIdPreviousVersionsIdtable(Context context, ArrayList objectIds) throws Exception { String REL_ACTIVE_VERSION = MCADMxUtil.getActualNameForAEFData(context,
     ...
     * 
     * String [] oidList = new String [objectIds.size()]; objectIds.toArray(oidList);
     * 
     * StringList busSelectList = new StringList();
     * 
     * busSelectList.add("id"); busSelectList.add("type"); busSelectList.add("revision"); busSelectList.add("revisions"); busSelectList.add("revisions.id");
     * 
     * busSelectList.add(SELECT_ON_ACTIVE_MINOR + "revision"); busSelectList.add(SELECT_ON_ACTIVE_MINOR + "revisions"); busSelectList.add(SELECT_ON_ACTIVE_MINOR + "revisions.id");
     * 
     * BusinessObjectWithSelectList busWithSelectList = BusinessObject.getSelectBusinessObjectData(context, oidList, busSelectList);
     * 
     * for(int i = 0 ; i < busWithSelectList.size(); i++) { ArrayList versionsList = new ArrayList();
     * 
     * BusinessObjectWithSelect busWithSelect = busWithSelectList.getElement(i);
     * 
     * // [NDM] : QWJ String busId = busWithSelect.getSelectData("id"); returnTable.put(busId, versionsList);
     * 
     * //String typeName = busWithSelect.getSelectData("type"); String revision = busWithSelect.getSelectData("revision"); StringList versions = busWithSelect.getSelectDataList("revisions");
     * 
     * // [NDM] : QWJ if(_util.isMajorObject(context, busId)) { //If Input is major the add all versions older than the active minor (major means it is finalized) revision =
     * busWithSelect.getSelectData(SELECT_ON_ACTIVE_MINOR + "revision"); versions = busWithSelect.getSelectDataList(SELECT_ON_ACTIVE_MINOR + "revisions");
     * 
     * if(versions != null) { for(int j = 0; j < versions.size();j++) { String currentVersion = (String)versions.elementAt(j); if(currentVersion.equals(revision)) break; else {
     * versionsList.add(busWithSelect.getSelectData(SELECT_ON_ACTIVE_MINOR + "revisions[" + currentVersion + "].id")); } } } } else { if(versions != null) { for(int j = 0; j < versions.size();j++) {
     * String currentVersion = (String)versions.elementAt(j); if(currentVersion.equals(revision)) break; else { versionsList.add(busWithSelect.getSelectData("revisions[" + currentVersion + "].id")); }
     * } } } }
     * 
     * return returnTable; } [NDM] QWJ end
     */
    ...
    private void getModifiedRelidsTableBasedOnChildTypeAndName(Context context, ArrayList childIdsTypeAndName, ArrayList previousVersionsList, Hashtable busIdDependentRelIdTable,
            HashMap relIdParentDetails, Hashtable childidTypeAndNameTable, HashSet expandedObjectIds, ArrayList floatedRelList) throws Exception {
        for (int i = previousVersionsList.size() - 1; i >= 0; --i) {
            ...
            if (ChildRelIdParentDetails != null && !ChildRelIdParentDetails.isEmpty()) {
                ...
                while (itr.hasNext()) {
                    ...
                    String correspondingTypeAndName = _globalConfig.getCorrespondingType(childTypeAndName.substring(0, childTypeAndName.indexOf("|")))
                            + childTypeAndName.substring(childTypeAndName.indexOf("|"));
    ...
    private Hashtable floatDrawingOnDesignVersion(Context context, Hashtable requestTable, Hashtable relsAndEnds, Hashtable busIdFloatedRelIdsMap) throws Exception {
        ...
        /*
         * Hashtable busIdDependentRelIdTable = _objectExpander.getRelationshipAndChildObjectInfoForParent(context, objectIdExpLevelMap, relsAndEnds, new HashMap(), new Hashtable(),
         * false,MCADAppletServletProtocol.VIEW_AS_STORED,null,null);
         */
        ...
        while (inputIds.hasMoreElements()) {
            ...
            getModifiedRelidsTableBasedOnChildTypeAndName(context, childIdsTypeAndName, previousVersionsList, busIdDependentRelIdTable, relIdParentDetails, busidTypeAndNameTable, expandedObjectIds,
                    floatedRelList);
            ...
            if (!isVersion) {
                ...
                for (int j = (majorRevisions.size() - 1); j > -1; j--) {
                    ...
                    if (revIndex <= currentRevIndex) {
                        ...
                        for (int k = (minorVersions.size() - 1); k > -1; k--) {
                            String minorId = busObjectWithSelect.getSelectData("revisions[" + majorRevisions.get(j) + "]." + SELECT_ON_ACTIVE_MINOR + "revisions[" + minorVersions.get(k) + "].id");
                ...
                for (int j = (majorRevisions.size() - 1); j > -1; j--) {
                    ...
                    if (revIndex <= currentRevIndex) {
                        ...
                        for (int k = (minorVersions.size() - 1); k > -1; k--) {
                            String minorId = busObjectWithSelect
                                    .getSelectData(SELECT_ON_MAJOR + "revisions[" + majorRevisions.get(j) + "]." + SELECT_ON_ACTIVE_MINOR + "revisions[" + minorVersions.get(k) + "].id");
