...
public class DECCaptureModifiedAttributes_mxJPO {
    ...
    public void captureModifiedAttributes(matrix.db.Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (!attrNm.equals("") && !attrNm.equals(modifiedAttribNm) && !isCheckin && !isAttributeSynch && !isCheckinEx) {
    ...
    private String getModifiedAttributeList(matrix.db.Context context, String attrNm, BusinessObject bo, MCADMxUtil mxUtil, String attrVal) throws Exception {
        ...
        try {
            ...
            /*
             * System.out.println("--inside getModifiedAttributeList()-->"+integName+"--->"+context.getUser()); IEFSimpleConfigObject simpleGCO = IEFSimpleConfigObject.getSimpleGCO(context,
             * integName);
             * 
             * 
             * 
             * if (null == simpleGCO) { BusinessObject registryObj = new BusinessObject(globalRegistryType,globalRegistryName,globalRegistryRevision , ""); String registryXmlStr =
             * (registryObj.getAttributeValues(context, registryDataAtrNm)).getValue(); IEFXmlNode registryXml = MCADXMLUtils.parse(registryXmlStr, charSet);
             * 
             * Enumeration registoryList = registryXml.elements(); while(registoryList.hasMoreElements()) { IEFXmlNode integNode = (IEFXmlNode)registoryList.nextElement(); IEFXmlNode defaultGCONode =
             * integNode.getChildByName("defaultGCOName");; String defaultGCONm = defaultGCONode.getFirstChild().getContent(); IEFXmlNode nameNode = integNode.getChildByName("name"); String integNmXML
             * = nameNode.getFirstChild().getContent();
             * 
             * if(integName.equals(integNmXML)) { gcoName = defaultGCONm; break; } } simpleGCO = IEFSimpleConfigObject.getSimpleGCO(context, gcoType, gcoName); }
             * 
            ...
            // [NDM] OP6
            /*
             * if (isVersionObject) { objType = mxUtil.getCorrespondingType(context, objType); }
             */
