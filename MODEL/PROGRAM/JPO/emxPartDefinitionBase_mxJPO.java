...
/**
 * This JPO contains the code formerly contained in the following files: com/matrixone/framework/beans/PartDefinition.java com/matrixone/framework/beans/CADDrawing.java
 * com/matrixone/framework/beans/CADModel.java com/matrixone/framework/beans/DrawingPrint.java for triggers.
 */
...
/**
 * The <code>emxPartDefinitionBase</code> class contains promote triggers for the following types: CAD Drawing, CAD Model, and Drawing Print.
...
public class emxPartDefinitionBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @since EC Rossini.
     */
    public emxPartDefinitionBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    /**
     * Floats the unfulfilled ECRs attached to the previous revison of the specification to this revision. Uses the Request Specification Revision relationship and calls floatUnfulfilledECR method
     * which does the following: - Checks the previous rev for connected ECRs that are not fulfilled or rejected. - Floats them to this revision if they do not have an ECO attached.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no input arguments.
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since EC 10.0.0.0.
    ...
    /**
     * Floats the unfulfilled ECRs attached to the previous revison of the specification to this revision. The following steps are performed: - Checks the previous rev for connected ECRs that are not
     * fulfilled or rejected. - Floats them to this revision if they do not have an ECO attached.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param relTypes
     *            list of relationship types.
     * @param policyStates
     *            states for the policy.
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since EC 10.0.0.0.
    ...
    protected void floatUnfulfilledECR(Context context, StringList relTypes, String policyStates) throws Exception {
        ...
        try {
            ...
            MapList mapList = prevRevPartDefinition.getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
     ...
     * @since EC 10.0.0.0.
     */
    protected MapList getECO(Context context, StringList selectStmts, StringList selectRelStmts) throws Exception {
        DebugUtil.debug("emxPartDefinition:getECO");
        Pattern relPattern = new Pattern(RELATIONSHIP_AFFECTED_ITEM);
        return (getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
    ...
    /**
     * Floats the latest Released or higher Reference Documents attached to the previous revision of this part to this revision. The following steps are performed: - Checks the previous rev for
     * connected objects that are connected with Reference Document Relationship. - Floats the latest released and higher revisions to this revision.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds following input arguments: 0 - policy - a String holding the Policy name. 1 - floatOnState - a String holding the State name.
     * @throws Exception
     *             if the operation fails.
     * @return void.
     * @since EC 10.0.0.0.
     ...
     */
    public void floatReferenceDocumentsOnRelease(Context context, String[] args) throws Exception {
        String state = "";
        String oid = "";
        ...
        try {
            ...
            String sImplementedItemCO = null;
            String sAffectedItemCO = null;
            ...
            if ("True".equalsIgnoreCase(sFloatToOlderRevision)) {
                ...
                sImplementedItemCO = "to[" + RELATIONSHIP_IMPLEMENTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "|from.current != \"" + completeState + "\"].from.id";
                sAffectedItemCO = "to[" + RELATIONSHIP_CHANGE_AFFECTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "|from.current != \"" + completeState + "\"].from.id";
                sNextRevConnectedImplementedCO = "next.to[" + RELATIONSHIP_IMPLEMENTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "|from.current != \"" + completeState
                        + "\"].from.id";
                sNextRevConnectedAffectedItemCO = "next.to[" + RELATIONSHIP_CHANGE_AFFECTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "|from.current != \"" + completeState
                        + "\"].from.id";
                ...
                sImplCO = "to[" + RELATIONSHIP_IMPLEMENTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "].from.id";
                sAICO = "to[" + RELATIONSHIP_CHANGE_AFFECTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "].from.id";
                sNextRevConnectedImplCO = "next.to[" + RELATIONSHIP_IMPLEMENTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "].from.id";
                sNextRevConnectedAICO = "next.to[" + RELATIONSHIP_CHANGE_AFFECTED_ITEM + "].from.to[" + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "].from.id";

                sChangeOrder = UIUtil.isNotNullAndNotEmpty((String) mCO.get(sImplCO)) ? (String) mCO.get(sImplCO) : (String) mCO.get(sAICO);
            }
            ...
            MapList mapList = prevRevObject.getRelatedObjects(context, RELATIONSHIP_REFERENCE_DOCUMENT, // relationship pattern
            ...
            while (partItr.hasNext()) {
                ...
                if ("True".equalsIgnoreCase(sFloatToOlderRevision)) {
                    ...
                    sAssociatedCO = UIUtil.isNotNullAndNotEmpty((String) partInfo.get(sImplCO)) ? (String) partInfo.get(sImplCO) : (String) partInfo.get(sAICO);
                    sNextRevAssociatedCO = UIUtil.isNotNullAndNotEmpty((String) partInfo.get(sNextRevConnectedImplCO)) ? (String) partInfo.get(sNextRevConnectedImplCO)
                            : (String) partInfo.get(sNextRevConnectedAICO);
                    // connect all Unreleased rev with latest reference document
                    ...
                    if (PolicyUtil.checkState(context, partId, releaseState, PolicyUtil.LT)
                            || (releaseState.equalsIgnoreCase(sCurrentState) && (!(sChangeOrder.equalsIgnoreCase(sNextRevAssociatedCO)) && !releaseState.equalsIgnoreCase(sNextRevState))))
    ...
    /**
     * Checks that an ECO is connected when the object is promoted. Allows the promotion of the object when the object is connected as only a Reference Document (i.e., there are no 'New
     * Specification/Specification Revision relationships' ) and the flag that change management is not required if reference only is set to true.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds following input arguments: 0 - noChangeManagementIfReferenceOnly - a String holding true or false to specify if change management for Reference Documents is allowed.
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, "objectId", "languageStr".
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: objectId - a String holding the Specification object id.
     * @return Object - Boolean true if object contains attribute "Model Type".
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Object checkModelTypeDisplay(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, "objectId", "languageStr", "New Value".
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, "objectId", "Old value", "New OID".
     * @return Object - Boolean true if operation successful otherwise false.
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
    ...
    public Object connectECR(Context context, String args[]) throws Exception {
        ...
        if (!"".equals(strOldECRName)) {
            ...
            ECRList = getRelatedObjects(context, strRelationship, strProductLineType + "," + ChangeConstants.TYPE_CHANGE_REQUEST, ObjectSelectsList, RelSelectsList, true, false, (short) 1,
    ...
    /**
     * Checks the previous rev for connected objects that are connected with Part Specification Relationship. Floats the relationship objects connected with 'Part Specification' to this revision if
     * the connected object's higher revisions are not in 'Released' state.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - Revised object id 1 - Newly created revision 2 - Type of the object 3 - Comma sparated type symbolic names to excuded the exection, Example:
     *            type_CADModel,type_ABC,type_XYZ
     * @return Returns 0
     * @throws Exception
     *             if the operation fails.
     ...
     */
    public int connectSpecToRelatedParts(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String sObjectWhere = "(next.from[" + sRelPartSpec + "].businessobject.id == " + sOid + " || previous.from[" + sRelPartSpec + "].businessobject.id == " + sOid
                    + ") && !(current == Obsolete)";
            ...
            for (int i = 0; i < mlPartsConnected.size(); i++) {
                ...
                if (!bLaterRevFound) {
                    if (!("Release".equals((String) mapCurrent.get(DomainConstants.SELECT_CURRENT))) && !("Complete".equals((String) mapCurrent.get(DomainConstants.SELECT_CURRENT)))) {
        ...
        try {
            ...
            for (int i = 0; i < mlFinalResult.size(); i++) {
                ...
                sNewRevId = MqlUtil.mqlCommand(context, "print bus $1 $2 $3 select $4 dump", doDocObject.getInfo(context, DomainConstants.SELECT_TYPE),
                        doDocObject.getInfo(context, DomainConstants.SELECT_NAME), sNewRevision, "id");
    ...
    /**
     * Check that at other revisions of this object do not exist. If one does, fail with a message telling the user to use "OBJECT | NEW | REVISION" instead of "OBJECT | NEW | ORIGINAL.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - Type of the object trying to create 1 - Name of the object trying to create 2 - Revision of the object trying to create 2 - Comma sparated
     *            type symbolic names to excuded the exection, Example: type_CADModel,type_ABC,type_XYZ
     ...
     * @trigger TypeCADModelReviseAction.
     */
    public int specEnsureNoRevisionsExist(Context context, String[] args) throws Exception {
            ...
            if (sExpand.equalsIgnoreCase("EXPAND_FROM_SPECIFIC_LEVEL")) {
                ...
                if (!"".equals(sPolicy)) {
                    ...
                    if ("Equivalent".equals(sPolicyClass)) {
                        ...
                        MapList mlLst = (MapList) findObjects(context, sType, sName, sQueryRev, "*", "*", null, false, objectSelects);
                        if (mlLst.size() > 0) {
    ...
    public MapList getSpecRelatedECRs(Context context, String[] args) throws Exception {
        ...
        if (ctxPerson.hasRole(context, sDgnEngr) || ctxPerson.hasRole(context, sSrDgnEngr) || ctxPerson.hasRole(context, sMfgEngr) || ctxPerson.hasRole(context, sSrMfgEngr)
                || ctxPerson.hasRole(context, sECREvtr) || ctxPerson.hasRole(context, sECRCdtr) || ctxPerson.hasRole(context, sECRChrm)) {
            String initargs[] = {};
