...
public class emxGenericFieldsBase_mxJPO {
    ...
    public static String fieldLifecycle(Context context, String[] args) throws Exception {
        ...
        sbResult.append("data='../common/emxLifecycleDialog.jsp?export=false&toolbar=AEFLifecycleMenuToolBar&objectId=").append(sOID)
                .append("&header=emxFramework.Lifecycle.LifeCyclePageHeading&mode=basic'");
    ...
    public String fieldImages(Context context, String[] args) throws Exception {
        ...
        if ((null == sGalleryZoom) || (sGalleryZoom.equalsIgnoreCase("TRUE"))) {
            sGalleryZoom = "onComplete:function(){ $('.cboxPhoto').wheelzoom(); },";
        } else {
            sGalleryZoom = "";
        }
        if (null == sZooming) {
        ...
        if (null == sZoomWidth) {
            sZoomWidth = "var widthZoom   = jQuery( window ).width()  * 0.5;";
        } else {
            sZoomWidth = "var widthZoom  = " + sZoomWidth + ";";
        }
        if (null == sZoomHeight) {
            sZoomHeight = "var heightZoom  = jQuery( window ).height() * 0.6;";
        } else {
            sZoomHeight = "var heightZoom = " + sZoomHeight + ";";
        }
        if (null == sFormat) {
            sFormat = "mxSmall Image";
        }
        if (null == sHeight) {
            if (sFormat.equals("mxThumbnail Image")) {
                sHeight = "42";
            } else if (sFormat.equals("mxSmall Image")) {
                sHeight = "64";
            } else if (sFormat.equals("mxMedium Image")) {
                sHeight = "102";
            } else if (sFormat.equals("mxLarge Image")) {
                sHeight = "480";
            } else {
                sHeight = "64";
            }
        ...
        if (null == sStyle) {
            sStyle = "margin:5px;border:1px solid #bababa;box-shadow:1px 1px 2px #ccc;height:" + sHeight + "px;";
        }
        if (null == sMaxItems) {
            iCounterMax = 10;
        } else {
            iCounterMax = Integer.parseInt(sMaxItems);
        }
            ...
            if (fileList.size() > 0) {
                for (int k = 0; k < fileList.size(); k++) {
                    ...
                    if (iCounter <= iCounterMax) {
                        sbResult.append("<img id='").append(fieldName + k).append("' title='").append(sTitle).append("' style='").append(sStyle).append("' src='").append(sURLImage)
                                .append("' data-zoom-image='").append(sURLImageGeneric).append("' />");
                    }
                    if (sGallery.equalsIgnoreCase("TRUE")) {
                        sbResult.append("</a>");
                    }
            ...
            if (fileList.size() > 0) {
                ...
                for (int k = 0; k < fileList.size(); k++) {
                    ...
                    if (sPrimary.equalsIgnoreCase("FALSE") && (sFileName.equals(sPrimaryImage))) {
                        ...
                        if (iCounter <= iCounterMax) {
                            sbResult.append("<img id='").append(fieldName + k).append("' title='Click to open gallery view' style='").append(sStyle).append("' src='").append(sURLImage)
                                    .append("' data-zoom-image='").append(sURLImageGeneric).append("' />");
                        }
                        if (sGallery.equalsIgnoreCase("TRUE")) {
                            sbResult.append("</a>");
        ...
        if (sGallery.equalsIgnoreCase("TRUE")) {
            sbResult.append("<script>jQuery(\"a[rel='gallery" + fieldName + "']\").colorbox({").append(sGalleryZoom)
                    .append("photo:'true', maxWidth:'100%', maxHeight:'100%', opacity: 1.0});</script>");
        }
    ...
    private String retrievePluginURLs(Context context, DomainObject dObject, String OIDImage, String fieldName, int k, String sFileName, String sGallery, String sZooming, StringBuilder sbResult,
            StringBuilder sbZoom, String sMCSURL) throws MatrixException, Exception {
        ...
        if (sZooming.equalsIgnoreCase("TRUE")) {
            ...
            sbZoom.append("     }");
            sbZoom.append("} ");
            sbZoom.append("jQuery('#").append(idItem).append(
                    "').elevateZoom({zoomWindowOffety: 8, borderSize : 1, borderColour: '#bababa', scrollZoom : true, zoomWindowWidth:widthZoom, zoomWindowHeight:heightZoom, zoomWindowPosition: pos")
                    .append(idItem).append("});");
        }
    ...
    public static Boolean setAccess(Context context, String[] args) throws Exception {
        ...
        if (sShowAll != null) {
            if (sShowAll.equalsIgnoreCase("true")) {
                sShowView = "true";
                sShowPrint = "true";
                sShowExport = "true";
            }
        ...
        if (null == sMode) {
            if (sShowExport.equalsIgnoreCase("TRUE")) {
                bResult = true;
            } else if (sShowView.equalsIgnoreCase("TRUE")) {
                bResult = true;
            }
        } else if (sMode.equalsIgnoreCase("edit")) {
            if (sShowEdit.equalsIgnoreCase("TRUE")) {
                bResult = true;
            }
        } else if (sMode.equalsIgnoreCase("view")) {
            if (null == sPFMode) {
                if (sShowView.equalsIgnoreCase("TRUE")) {
                    bResult = true;
                }
            } else {
                if (sShowPrint.equalsIgnoreCase("TRUE")) {
                    bResult = true;
                }
            }
        }
    ...
    public String fieldFormat(Context context, String[] args) throws Exception {
        ...
        if (null != sItalic) {
            bApplyStyle = true;
            if (sItalic.equalsIgnoreCase("TRUE")) {
        ...
        if (null != sBGColor) {
            bApplyStyle = true;
            sbStyle.append("background-color:").append(sBGColor).append(";");
        }
        if (null != sImage) {
            sbImage.append(sImagePrefix).append(sImage).append(sImageSuffix);
        }
        if (null == sFormat) {
            sFormat = "";
        }
        if (null == sDisplayFormat) {
            sDisplayFormat = "2";
        }
        if (null == sTarget) {
            sTarget = "";
        ...
        if (null != sStyles) {
            aStyles = sStyles.split(",");
        }
        if (null != sImages) {
            aImages = sImages.split(",");
        }
        if (null != sValuesAfter) {
            aValuesAfter = sValuesAfter.split(",");
        }
        if (null != sValuesBefore) {
            aValuesBefore = sValuesBefore.split(",");
        }
        if (null == sAdminType) {
            sAdminType = "";
        }
        ...
        if ((aValues.length == aStyles.length) || (aValues.length == aImages.length)) {
            for (int i = 0; i < aValues.length; i++) {
                if (sValue.equals(aValues[i])) {
                    if (aStyles.length == aValues.length) {
                        sbStyle.append(aStyles[i]);
                    }
                ...
                if ((aValuesFrom.length == aStyles.length) || (aValuesFrom.length == aImages.length)) {
                    ...
                    for (int i = aValuesFrom.length - 1; i >= 0; i--) {
                        ...
                        if (bdValue.compareTo(bdValueFrom) != -1) {
                            if (aStyles.length == aValuesFrom.length) {
                                sbStyle.append(aStyles[i]);
                            }
            ...
            if (null != sValuesTo) {
                ...
                if ((aValuesTo.length == aStyles.length) || (aValuesTo.length == aImages.length)) {
                    ...
                    for (int i = 0; i < aValuesTo.length; i++) {
                        ...
                        if (bdValue.compareTo(bdValueFrom) != 1) {
                            if (aStyles.length == aValuesTo.length) {
                                sbStyle.append(aStyles[i]);
                            }
            ...
            if ((aValuesAfter.length == aStyles.length) || (aValuesAfter.length == aImages.length)) {
                for (int i = aValuesAfter.length - 1; i >= 0; i--) {
                    ...
                    if (cActual.after(cAfter)) {
                        if (aStyles.length == aValuesAfter.length) {
                            sbStyle.append(aStyles[i]);
                        }
            ...
            if ((aValuesBefore.length == aStyles.length) || (aValuesBefore.length == aImages.length)) {
                for (int i = 0; i < aValuesBefore.length; i++) {
                    ...
                    if (cActual.before(cAfter)) {
                        if (aStyles.length == aValuesBefore.length) {
                            sbStyle.append(aStyles[i]);
                        }
        ...
        if (!sValue.equals("")) {
            ...
            if (bApplyStyle) {
                if (null != sFilterFrom) {
                    ...
                    BigDecimal bdValue = new BigDecimal(sValue);
                    if (bdValue.compareTo(bdFilterFrom) != -1) {
                        bApplyStyle = true;
                    } else {
                        bApplyStyle = false;
                    }
            ...
            if (bApplyStyle) {
                if (null != sFilterTo) {
                    ...
                    BigDecimal bdValue = new BigDecimal(sValue);
                    if (bdValue.compareTo(bdFilterTo) != 1) {
                        bApplyStyle = true;
                    } else {
                        bApplyStyle = false;
                    }
        ...
        if (!sAdminType.equals("")) {
            String sResult = MqlUtil.mqlCommand(context, "print program eServiceSchemaVariableMapping.tcl select property[" + sAdminType + "].to dump;", true, false);
            if (sResult.startsWith(("att "))) {
    ...
    public String fieldRouteTasks(Context context, String[] args) throws Exception {
        ...
        if (sRoutes.equalsIgnoreCase("blocking")) {
            sWhere = "attribute[Route Base State] == 'state_" + sCurrent + "'";
        }

        MapList mlRoutes = dObject.getRelatedObjects(context, DomainConstants.RELATIONSHIP_OBJECT_ROUTE, DomainConstants.TYPE_ROUTE, busSelects, null, false, true, (short) 1, "(current != 'Archive')",
                sWhere, 0);
    ...
    public static String fieldKeywords(Context context, String[] args) throws Exception {
        ...
        if (sTable == null) {
            sTable = "AEFGeneralSearchResults";
        }
        DomainObject dObject = new DomainObject(sOID);
        ...
        if (null != sKeywords) {
            if (!"".equals(sKeywords)) {
                ...
                for (int i = 0; i < aKeywords.length; i++) {
                    ...
                    sbResult.append("   window.open('../common/emxFullSearch.jsp?field=TYPES=").append(sRootType).append(":KEYWORDS=").append(sKeyword).append("&amp;table=").append(sTable)
                            .append("', ");
