...
public class emxWorkspaceFolderBase_mxJPO extends emxWorkspaceConstants_mxJPO {
     ...
     * @grade 0
     */
    public emxWorkspaceFolderBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    public void grantWorkspaceFolderAccess(matrix.db.Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mapObjectList = getObjects(context, relPattern.getPattern(), typePattern.getPattern(), false, true, (short) 1, listObjectSelect, null, null, null, null, null, null, null, false);
            ...
            if (mapObjectList.size() != 0) {
                ...
                if (isWLG) {
                    ...
                    if (access.hasNoAccess()) {
                        ...
                        matrix.db.BusinessObject.grantAccessRights(context, busRouteList, (Access) ((accessUtil.getAccessList()).elementAt(0)));
                    }
                } else if (isWAG) {
                    contentIds = new StringList(mapObjectList.size());
                    while (itrObjects.hasNext()) {
                ...
                if (isWAG || isWLG) {
                    if (contentIds.size() > 0) {
                        ...
                        new VaultedObjectsAccessUtil().updateVaultedObjectsAccess(context, (String[]) contentIds.toArray(new String[] {}), isWAG ? grantees : new String[] {},
                                isWLG ? grantees : new String[] {}, VaultedObjectsAccessUtil.UPDATE_ACCESS);
                    }
                } else {
    ...
    public MapList getWorkspaceFolders(Context context, String args[]) throws Exception {
        ...
        totalresultList = workspace.getRelatedObjects(context, DomainObject.RELATIONSHIP_WORKSPACE_VAULTS + "," + RELATIONSHIP_LINKED_FOLDERS, DomainObject.TYPE_WORKSPACE_VAULT, objectSelects,
    ...
    public MapList getSubFolders(Context context, String args[]) throws Exception {
        ...
        totalresultList = workspaceVault.getRelatedObjects(context, DomainObject.RELATIONSHIP_SUBVAULTS + "," + RELATIONSHIP_LINKED_FOLDERS, DomainObject.TYPE_PROJECT_VAULT, objectSelects, relSelects,
                false, true, (short) 1, sWhere, null);
        return totalresultList;
    }
    ...
    private boolean checkAccess(Context context, String objectId) throws Exception {
        try {
            ...
            if (sOwner.equals(person.getName(context)) || "Project Lead".equals(sProjectAccess)
    ...
    /**
     * The main method which all filter access method calls. Returns Workspace Folder Access List Based on the parameters.
     * @param context
     *            the eMatrix Context object
     * @param args
     *            holds the grantee and grantor name
    ...
    /**
     * Method to show the PersonName in Folder Access List Returns a Vector of person names along with image icons
     * @param context
     *            the eMatrix Context object
     * @param args
     *            holds the grantee and grantor name
    ...
    public Vector getAvailableAccesses(Context context, String[] args) throws Exception {
        ...
        for (Iterator objectListItr = objectList.iterator(); objectListItr.hasNext();) {
            ...
            if (stCheckNull != null && !stCheckNull.equals("null") && !stCheckNull.equals("")) {
                availableAccesses
                        .add(EnoviaResourceBundle.getProperty(context, stringResFileId, new Locale(languageStr), "emxTeamCentral.Access." + (String) i18nMap.get(mapAccess.get("SpecificAccess"))));
            } else {
    ...
    private void getAccess(Context context, AccessList baseAccessList, AccessList parentAccessList, String sGrantee, String sTypeName, HashMap hashMap, AccessUtil accessUtil) {
        String sAccessString = "";
        try {
    ...
    /**
     * This method is used to show Push subscription link or not Args Array takes objectid, event, boolean if object is a version
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public MapList getFolders(Context context, String[] args) throws Exception {
        /*
         * DomainObject object = DomainObject.newInstance(context, objId); StringList selects = new StringList(3); selects.add(SELECT_TYPE); selects.add("to[Vaulted Objects].from.id"); Map objMap =
         * object.getInfo(context, selects); String folderId = (String)objMap
         */
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) programMap.get("objectId");
    ...
    static public String getWorkspaceName(matrix.db.Context context, String folderId) throws MatrixException {
        ...
        MapList mapList = domainObject.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(), objSelects, null, true, false, (short) -1, "", "", null, null, null);
        Iterator mapItr = mapList.iterator();
        if (mapItr.hasNext()) {
            Map map = (Map) mapItr.next();
    ...
    public void folderCreateProcess(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String strMQLCommand = "print bus " + objectId + " select physicalid dump |";

            String strProjectRevision = MqlUtil.mqlCommand(context, strMQLCommand);
            ...
            if (strFolderName.length() > (Integer.parseInt(MAX_LENGTH))) {
                ...
                MqlUtil.mqlCommand(context, "notice \"" + strLengthMessage + " " + MAX_LENGTH + " " + strChars + "\"");
                return;
            }

            if (workspace.isFolderExists(context, strFolderName)) {
                bExists = true;
                String i18NCategory = EnoviaResourceBundle.getProperty(context, "emxTeamCentralStringResource", new Locale(context.getSession().getLanguage()),
                        "emxTeamCentral.AddCategories.Category");
                String i18NNotUnique = EnoviaResourceBundle.getProperty(context, "emxTeamCentralStringResource", new Locale(context.getSession().getLanguage()),
                        "emxTeamCentral.AddCategories.NotUnique");
            ...
            /*
             * Hashtable hashRouteWizFirst = (Hashtable)session.getValue("hashRouteWizFirst"); if (hashRouteWizFirst != null) { hashRouteWizFirst.put("newFolderIds" , folderId);
    ...
    public void subfolderCreateProcess(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            // <--!Modied for the Bug No:339243 08/06/2007 10:00AM End-->
                ...
                // treeUrl = UINavigatorUtil.getCommonDirectory(application) + "/emxTree.jsp?objectId=" + objID +"&emxSuiteDirectory="+appDirectory+"&mode=insert&AppendParameters=true&folderId=" +
                // strObjectId + "&workspaceId=" + workspaceId;
            }
    ...
    static public void CloneFolders(matrix.db.Context context, String sfolderId, String sCloneId, String strFolderName) throws MatrixException {
        ...
        ExpansionWithSelect expandWSelectCategory = ObjFolder.expandSelect(context, relSubVaultPattern.getPattern(), typeCategoryPattern.getPattern(), typeCategorySelectList, selListRelationship,
                false, true, (short) 1);
    ...
    public MapList getPredefinedFolderName(Context context, String[] args) throws Exception {
        ...
        boProject.open(context);
        ExpansionWithSelect expandWSelectProject = boProject.expandSelect(context, relPattern.getPattern(), typePattern.getPattern(), typeselectList, new StringList(), false, true, (short) 1);
        boProject.close(context);
    ...
    public Map removeFolderContentPreProcess(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            StringBuffer sbSelRtState = new StringBuffer("from[");
            sbSelRtState.append(DomainObject.RELATIONSHIP_OBJECT_ROUTE);
            sbSelRtState.append("].to.current");
            ...
            selDocumentList.add(sbSelRtState.toString());
            ...
            DomainConstants.MULTI_VALUE_LIST.add(sbSelRtState.toString());
            ...
            DomainConstants.MULTI_VALUE_LIST.remove(sbSelRtState.toString());
            ...
            while (accessMapItr.hasNext()) {
                ...
                StringList docRouteState = (StringList) accessMap.get(sbSelRtState.toString());
                ...
                String id = (String) accessMap.get("id");
                boolean hasRoutes = false;

                if (docRouteId != null) {
                    for (int k = 0; k < docRouteId.size(); k++) {
                        String state = (String) docRouteState.get(k);
                        if (DomainConstants.STATE_ROUTE_DEFINE.equals(state) || DomainConstants.STATE_ROUTE_IN_PROCESS.equals(state)) {
                            hasRoutes = true;
                        }
                    }
                }

                if (!UIUtil.isNullOrEmpty(docLocked) && docLocked.indexOf("TRUE") != -1 || !UIUtil.isNullOrEmpty(vcDocLocked) && vcDocLocked.indexOf("TRUE") != -1) {
                    errorMsgKey = "emxTeamCentral.ContentSummary.AlertCannotRemoveLockedContent";
                } else if (hasRoutes || isContentWithRouteScope(context, docRouteId)) {
    ...
    public Map createWorkspaceFolderProcess(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            workspace.setId(strProjectId);
            String strProjectVault = boProject.getVault();
            String strProjectRevision = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3", strProjectId, "physicalid", "|");
        ...
        try {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                if (idList.size() > 2) {
                    ...
                    if (strComments.length() != 0 && !ownerCheck) {
    ...
    public void publishOnConnect(Context context, String[] args) throws Exception {
        try {
            ...
            if ("True".equals(publishOnConnectValue)) {
                ...
                StringList selects = new StringList(3);
                selects.addElement("policy");
                selects.addElement(DomainObject.SELECT_CURRENT);
                selects.addElement("state");
     ...
     * @throws Exception
     */
    public Vector getFolderDisplayName(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramList");
            MapList objectList = (MapList) programMap.get("objectList");
            Vector vecFolderDisplayName = new Vector();
            String languageStr = (String) paramMap.get("languageStr");
            String stringResFileId = "emxTeamCentralStringResource";
            String strLinkedSuffix = EnoviaResourceBundle.getProperty(context, stringResFileId, new Locale(languageStr), "emxTeamCentral.Common.Linked");
            String sFolderName = "";
            ...
            while (objectListItr.hasNext()) {
                ...
                sFolderName = (String) objectMap.get(DomainConstants.SELECT_NAME);
                sRelationship = (String) objectMap.get("relationship");
                if (sRelationship.equals(RELATIONSHIP_LINKED_FOLDERS)) {
                    sFolderName = sFolderName + "(" + strLinkedSuffix + ")";
                }
                vecFolderDisplayName.add(sFolderName);
            }
            return vecFolderDisplayName;
        } catch (Exception ex) {
            System.out.println("Error in showWorkspaceTemplateFolderName = " + ex.getMessage());
            throw ex;
        }
    }
