...
/*
 * emxVariantConfigurationIntermediateObjectMigrationBase.java program migrates the data into Common Document model i.e. to 10.5 schema.
 *
...
/**
 * The <code>emxVariantConfigurationIntermediateObjectMigrationBase</code> class contains the utilities necessary to migrate the Variant Configuration schema to V6R2012x. It must be run on all FTR
 * data created prior to V6R2012x.
 */
public class emxVariantConfigurationIntermediateObjectMigrationBase_mxJPO extends emxCommonMigration_mxJPO {
     ...
     * @param args
     *            String array containing program arguments
     * @throws Exception
     *             if the operation fails
     */
    public emxVariantConfigurationIntermediateObjectMigrationBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    public void migrateObjects(Context context, StringList objectIdList) throws Exception {
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_FEATURES) || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_PRODUCTS))// To handle use case
                                                                                                                                                                                   // if "Product" is
                                                                                                                                                                                   // added as Technical
                                                                                                                                                                                   // Feature
                {
    ...
    /**
     * Migrates Feature objects and all connections to those features. The intermediate Feature List objects will be deleted and replaced with appropriate relationship connections. Also calls external
     * migration methods to allow external applicatons to migrate custom schema.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectIdList
     *            StringList holds list objectids to migrate
    ...
    private void migrateFeatures(Context context, StringList objectIdList) throws Exception {
        ...
        try {
            ...
            featureObjSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].attribute[" + ConfigurationConstants.ATTRIBUTE_EFFECTIVITY_STATUS + "]");// Varies By rel id if
                                                                                                                                                                             // present
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strParentFeatureType
     *            String specifying the type of the parent feature
     * @param newChildFeatureChangeType
     *            String specifying the type of the child feature
     * @param strParentFeatureId
     *            String specifying parent feature object id
     * @param strChildFeatureId
     *            String specifying child feature object id
     * @param strNewRelToConnect
     *            String specifying relationship name to connect the parent and child
     * @return String specifying the new relationship id
     * @throws Exception
     *             if the operation fails
     */

    // private String connectFeaturesWithNewRel(Context context,Map strParentFeatureId,String strChildFeatureId,String strChildFeatureType,String strNewRelToConnect) throws FrameworkException,
    // Exception{
    private String connectFeaturesWithNewRel(Context context, Map htParentObjData, String strChildFeatureId, String strChildFeatureType, String strNewRelToConnect)
        ...
        try {
            ...
            // String strMqlCmd =
            // "expand bus \""+strParentFeatureId+"\" from relationship \""+ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES+"\" preventduplicates select rel id where \"to.id
            // =='"+strChildFeatureId
            // +"'\" dump | " ;
            String strMqlCmd = "expand bus $1 from relationship $2 preventduplicates select $3 $4 where $5 dump $6 ";

            String strRelID = MqlUtil.mqlCommand(context, strMqlCmd, true, strParentFeatureId, ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES, "rel", "id",
                    "to.id ==" + strChildFeatureId + "", "|");
            ...
            if (strRelId != null && strRelId.trim().equalsIgnoreCase("")) {
                ...
                mqlLogRequiredInformationWriter(
                        "Connection between   " + strParentFeatureId + "  and " + strChildFeatureId + "  rel " + strNewRelToConnect + " executed." + strParentFeatureType + "\n\n");
                ...
                if (mxType.isOfParentType(context, newParentFeatureType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                    ...
                    Map mRelIds = DomainRelationship.connect(context, new DomainObject(strParentFeatureId), RelToConnect, true, sARelId);
                ...
                mqlLogRequiredInformationWriter(
                        "Duplicate connection between   " + strParentFeatureId + "  and" + strChildFeatureId + "  rel " + strNewRelToConnect + " skipped as connection already exists\n");
            }
    ...
    /**
     * Migrates GBOM objects and all connections to those objects. The intermediate GBOM objects will be deleted and replaced with GBOM relationship connections. All connections to GBOM objects will
     * be floated to the new GBOM relationships.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectIdList
     *            StringList holds list objectids to migrate
            ...
            while (itr.hasNext()) {
                try {
                    ...
                    if (newReltoConnect.equals(ConfigurationConstants.RELATIONSHIP_INACTIVE_GBOM)) {
                        String strInactiveFromPart = (String) gbomObjMap
                                .get("to[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_GBOM_FROM + "].attribute[" + ConfigurationConstants.ATTRIBUTE_INACTIVE_FROM_PART + "]");
                        String strInactiveFromVariant = (String) gbomObjMap
                                .get("to[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_GBOM_FROM + "].attribute[" + ConfigurationConstants.ATTRIBUTE_INACTIVE_FROM_VARIANT + "]");
    ...
    /**
     * Migrates Rule related data. Updates the cached attributes to reflect the migrated Feature and GBOM schema.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectIdList
     *            StringList holds list objectids to migrate
     ...
     *             if the operation fails
     */
    private void migrateRule(Context context, StringList objectIdList) throws Exception {
        int migrationStatus = getAdminMigrationStatus(context);
    ...
    private boolean updateRuleCache(Context context, Map ruleInfoMap) throws Exception {
        ...
        try {
            /*
             * Depending upon the Rule Type Left and Right expression attributes will be computed
             */
            StringList sLOfExpAttr = new StringList();

            if (strRuleType.equalsIgnoreCase(ConfigurationConstants.TYPE_BOOLEAN_COMPATIBILITY_RULE) || strRuleType.equalsIgnoreCase(ConfigurationConstants.TYPE_MARKETING_PREFERENCE)
                    || strRuleType.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_COMPATIBILITY_RULE) || strRuleType.equalsIgnoreCase(ConfigurationConstants.TYPE_INCLUSION_RULE)) {
            ...
            for (int iCntExpAttr = 0; iCntExpAttr < sLOfExpAttr.size(); iCntExpAttr++) {
                ...
                for (int iCnt = 0; iCnt < sLOfExp.size(); iCnt++) {
                    ...
                    /*
                     * "Sequence Order" is stored in StringList format... converted to String as this is used further in "addSortKey" method
                     */
                    if (!((StringList) mRelData.get(SELECT_ATTRIBUTE_SEQUENCE_ORDER)).isEmpty()) {
                        mRelData.put("attribute[" + ATTRIBUTE_SEQUENCE_ORDER + "]", ((StringList) mRelData.get(SELECT_ATTRIBUTE_SEQUENCE_ORDER)).get(0));
                    }
                ...
                strBuffer = strBuffer.append(STR_ATTRIBUTE).append(OPEN_BRACE).append(ConfigurationConstants.ATTRIBUTE_SEQUENCE_ORDER).append(CLOSE_BRACE);
                ...
                for (int m = 0; m < mLOfExpToSideRelInfo.size(); m++) {
                    ...
                    if (strToken != null && strToken.length() != 0) {
                        ...
                        if (strToRelPhyId != null && strToRelPhyId.length() != 0) {
                            strExpBuffer = strExpBuffer.append("R").append(strToRelPhyId).append(SYMB_SPACE);
                        } else if (strToPhyId != null && strToPhyId.length() != 0) {
                            strExpBuffer = strExpBuffer.append("B").append(strToPhyId).append(SYMB_SPACE);
                        }
                    }
                }
                ...
                if (strRuleType.equalsIgnoreCase(ConfigurationConstants.TYPE_MARKETING_PREFERENCE) && strExpAttr.equalsIgnoreCase(ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION)) {
                ...
                if (strRuleType.equalsIgnoreCase(ConfigurationConstants.TYPE_INCLUSION_RULE) && strDVAttrVal != null && !strDVAttrVal.equals("") && !strDVAttrVal.isEmpty()) {
                    ...
                    if (strRuleComplexity != null && strRuleComplexity.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_SIMPLE)) {
                        ...
                        while (newValueTZ.hasMoreElements()) {
                            ...
                            if (domDVId.exists(context)) {
                                ...
                                mqlLogRequiredInformationWriter("\n\n\n" + "This Design Variant doesn't exist now" + strDVVal + "\n" + "\n");
                            }
                        }
     ...
     *            the eMatrix <code>Context</code> object
     * @param typeName
     *            String containing the type to query
    ...
    /**
     * Stamps the Feature objects to the new appropriate Object Type Also computes if the Feature is in "Mixed Composition" or "Mixed Usage" If yes then Feature is stamped as "Conflict".
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            String array containing program arguments
    ...
    public void preMigration(Context context, String[] args) throws Exception {
        try {
            ...
            for (m = minRange; m <= maxRange; m++) {
                ...
                try {
                    ...
                    for (int iCntObjList = 0; iCntObjList < objectList.size(); iCntObjList++) {
                        ...
                        emxVariantConfigurationIntermediateObjectMigration_mxJPO VariantConfigInterObjMigrationInst = new emxVariantConfigurationIntermediateObjectMigration_mxJPO(context,
                                new String[0]);
                        ...
                        MapList featureStructureList = domTopLevelObject.getRelatedObjects(context, sbRelPattern.toString(), // relationshipPattern
                        ...
                        if (featureStructureList.size() == 0) {
                            ...
                            mqlLogRequiredInformationWriter(
                                    "Query to check for 'Design Variants' or 'GBOM' or 'Managed Series' or 'Quantity Rules' connections started ------------>" + " --- " + now() + "\n");
                            MapList LF_RelatedList = domTopLevelObject.getRelatedObjects(context, sbRelPattern1.toString(), // relationshipPattern
                                    ...
                                    (int) 0); // limit
                            mqlLogRequiredInformationWriter(
                                    "Query to check for 'Design Variants' or 'GBOM' or 'Managed Series' or 'Quantity Rules' connections ended   ------------>" + " --- " + now() + "\n");
                            ...
                            if (!LF_RelatedList.isEmpty()) {
                                ...
                                if (topLevelObjectType != null && !topLevelObjectType.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_LINE)
                            ...
                            if (mxType.isOfParentType(context, topLevelObjectType, ConfigurationConstants.TYPE_FEATURES) || isOfDerivationChangedType(context, topLevelObjectType)) {
                                ...
                                outer: for (int j = 0; j < featureStructureList.size(); j++) {
                                    ...
                                    if (mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_FEATURES)
                                            || mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_PRODUCTS) || isOfDerivationChangedType(context, strObjectType)) {
                                    ...
                                    if (!isMixedUsage && !isMixedComposition && !mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_FEATURES)
                                            && !mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_FEATURE_LIST)
                                    ...
                                    if (level == 1) {
                                        ...
                                        outer2: for (int k = j; k < featureStructureList.size(); k++) {
                                            ...
                                            if (mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_FEATURES)
                                                ...
                                                String featureTypeSelect = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from.attribute["
                                        ...
                                        for (int k = j + 1; k < featureStructureList.size(); k++) {
                                            ...
                                            // !isOfDerivationChangedType(context, strObjectType)) {
                                            {
                                                ...
                                                boolean isLeafLevel = false;
                                                if (mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_FEATURES) && currentLevel != 2) {
            ...
            unStampedObjWhere.append(")");
            /*
             * unStampedObjWhere.append("||"); unStampedObjWhere.append("("); unStampedObjWhere.append("type.kindof == \"" + ConfigurationConstants.TYPE_PRODUCTS + "\"");
             * unStampedObjWhere.append("&&"); unStampedObjWhere.append("to["+ ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO+ "]==True"); unStampedObjWhere.append("&&");
             * unStampedObjWhere.append("interface["+ INTERFACE_FTRIntermediateObjectMigration +"]==FALSE"); unStampedObjWhere.append(")");
             */
            ...
            List unStampedObjectsList = DomainObject.findObjects(context, unStampedTypePattern.toString(), DomainConstants.QUERY_WILDCARD, unStampedObjWhere.toString(), sLobjSelects);
            ...
            try {
                ...
                while (FLMapKeySetItr.hasNext()) {
                    ...
                    while (relIdsItr.hasNext()) {
                        ...
                        /*
                         * In case of "Manufacturing Features,Marketing Features" we are not going to add PFL in 2012x which we used to have in 2012.
                         */
                        if (key != null && key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].id")) {

                            if (strFeatType != null && !strFeatType.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_MANUFACTURING)
                                ...
                                if (ProductLineCommon.isNotNull(strFATVal) && strFATVal.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_MANDATORY)) {

                                    attributeMap.put(ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE, ConfigurationConstants.RANGE_VALUE_REQUIRED);
                                ...
                                } else if (!ProductLineCommon.isNotNull(strFATVal) || " ".equalsIgnoreCase(strFATVal)) {

                                    mqlLogRequiredInformationWriter("Value of 'Feature Allocation Type' attribute for Rel id : " + relId + "\n" + "is blank." + "\n\n");
                                    attributeMap.put(ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE, ConfigurationConstants.RANGE_VALUE_OPTIONAL);
                            ...
                            if (ifFLFromRelExits != null && ifFLFromRelExits.length() != 0) {
                                ...
                                if (strFeatType != null && strFeatType.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_TECHNICAL)) {
                                    ...
                                    /*
                                     * In R211, Mandatory CF where getting added as Committed Features. In R212 they should be added as "Mandatory Configuration Features".
                                     */
                                    ...
                                    /*
                                     * if(strMandAtt!=null && !strMandAtt.equalsIgnoreCase("Yes")){ strNewRelType = ConfigurationConstants.RELATIONSHIP_COMMITTED_CONFIGURATION_FEATURES; }
                                     */
                                ...
                                if (strFeatType != null && strFeatType.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_MARKETING)
                                   ...
                                   * else if(strFeatType!=null && strFeatType.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_NONE)){ strNewRelType =
                            ...
                            if (ProductLineCommon.isNotNull(strNewRelType)) {
                                ...
                                if (strNewRelType != null && strNewRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES)) {
                                    ...
                                    mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_SEQUENCE_ORDER, (String) mFLConnections
                                            .get("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].attribute[" + ConfigurationConstants.ATTRIBUTE_SEQUENCE_ORDER + "]"));
                                    ...
                                    mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_LIST_PRICE, (String) mFLConnections.get("attribute[" + ConfigurationConstants.ATTRIBUTE_LIST_PRICE + "]"));
                                ...
                                if (ProductLineCommon.isNotNull(strContxtProdId)) {
                                    ...
                                    PL.connectObject(context, strRelType, strContxtProdId, false);

                                    mqlLogRequiredInformationWriter("Connection between ::" + relId + strRelType + strContxtProdId);
                                }

                            }
                        } else if (key.startsWith("to") && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].type"))
                                ...
                                && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
                                        + "].from.to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id"))
                                ...
                                && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
                                        + "].from.to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id"))
                                && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES
                                        + "].tomid[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id"))
                                && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES
                                        + "].tomid[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id"))
                                && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to["
                                        + ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES + "].tomid[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id"))
                                && !(key.equalsIgnoreCase("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to["
                                        + ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES + "].tomid[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id"))) {
                            DomainRelationship domRel = new DomainRelationship(relId);
                            // Get the attributes on Relationship
                            Map attributeMap = new HashMap();
                            attributeMap = domRel.getAttributeMap(context, true);
                ...
                while (gbomMapKeySetItr.hasNext()) {
                    ...
                    if (key.startsWith("to") || key.startsWith("from")) {
                        ...
                        if (key.indexOf("tomid") == -1 && key.indexOf("frommid") == -1) {
                            ...
                            if (!key.equals("to[" + ConfigurationConstants.RELATIONSHIP_GBOM_FROM + "].id") && !key.equals("to[" + ConfigurationConstants.RELATIONSHIP_GBOM_FROM + "].from.id")
                                    && !key.equals("to[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_GBOM_FROM + "].id")
                                    && !key.equals("to[" + ConfigurationConstants.RELATIONSHIP_CUSTOM_GBOM + "].id") && !key.equals("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].id")
                        ...
                        if (key.indexOf("tomid") != -1 || key.indexOf("frommid") != -1) {
                            ...
                            while (midRelIdsItr.hasNext()) {
                                ...
                                /*
                                 * String strMqlCommand = "modify connection " + relId + " " + toOrFromrel + " " + gbomRelId; MqlUtil.mqlCommand(context, strMqlCommand, true);
                                 */
                                DomainRelationship domRel = new DomainRelationship(relId);
                                // Get the attributes on Relationship
                                Map attributeMap = new HashMap();
        ...
        try {
            ...
            if (strConflictValue == null || "null".equals(strConflictValue) || "".equals(strConflictValue)) {
                ...
                if (strNewFeatureType.equals("MixedUsage")
                    ...
                    IS_CONFLICT = true;
                    writeOIDToTextFile(strFeatureId + "-->Conflict Due to: " + strNewFeatureType, "Conflict_IncorrectMapping");
                    ...
                    if (ProductLineCommon.isNotNull(RESOLVE_CONFLICTS) && RESOLVE_CONFLICTS.equalsIgnoreCase("ResolveConflicts")) {
                        ...
                        writeOIDToTextFile(strFeatureId + "-->Conflict resolved from : " + strNewFeatureType + " " + "to   " + strResolvedNewFeatureType, "Resolved_Conflict");
                        ...
                        strConflict = "Yes";
                        IS_CONFLICT = true;
                        writeOIDToTextFile(strFeatureId + "-->Conflict Due to: " + strNewFeatureType + " " + "Recommended to change this value to " + strResolvedNewFeatureType,
                                "Recommendations_For_Conflicts");
                    }
                }

                if (strNewFeatureType != null && (strNewFeatureType.equalsIgnoreCase("type_ConfigurationFeature") || strNewFeatureType.equalsIgnoreCase("type_ConfigurationOption"))) {
                    ...
                    if (!sLGBOMIds.isEmpty()) {
                        ...
                        if (ProductLineCommon.isNotNull(RESOLVE_CONFLICTS) && RESOLVE_CONFLICTS.equalsIgnoreCase("ResolveConflicts")) {
                            ...
                            writeOIDToTextFile("Feature Id---->" + strFeatureId + "---->Conflict resolved   : " + "GBOM connected to Marketing Feature deleted--->" + sLGBOMIds,
                                    "Resolved_MarketingFeatureWithGBOM");

                            if (ProductLineCommon.isNotNull(strResolvedNewFeatureType)) {

                                writeOIDToTextFile("---->Also this Feature had conflict : " + "was resolved from --->" + strOriginalStampValue + "  to --->" + strResolvedNewFeatureType,
                                        "Resolved_MarketingFeatureWithGBOM");
                            }
                            ...
                            // Set the value to "true" so that user will get notification about "Conflict.txt" at end of Pre-Migration
                            IS_CONFLICT = true;
                            writeOIDToTextFile(strFeatureId + "-->Conflict Due to: " + strNewFeatureType, "Conflict_MarketingFeatureWithGBOM");
     ...
     *            the eMatrix <code>Context</code> object
     * @param strStatus
     *            String containing the status setting
     * @throws Exception
     */
    ...
    /**
     * Stamps the Rule object with the migration interface to indicate that the expression attributes for this Rule have been converted.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strRuleId
     *            the Rule object id
    ...
    private void stampRuleObject(Context context, String strRuleId) throws Exception {
        ...
        /*
         * This means the Rule Object is successfully stamped.. hence we added the interface and attribute "Conflict" value as "No"
         */
    ...
    private String replaceIntermediateObjectWithRel(Context context, String strOldRelId, String strNewRelId, String strNewRelIdSide, Map attributeMap) throws Exception {
        ...
        try {
            ...
            // Get the attributes on Relationship
            /*
             * Map attributeMap = new HashMap(); attributeMap = domRel.getAttributeMap(context,true);
             */
    ...
    /**
     * Need to call from DomainRelationship Connects the specified relationship with the given relationship.
     * @param context
     ...
     */
    public String connectRelationship(Context context, RelationshipType relationshipType, java.lang.String relId, String strNewRelId, boolean isFrom) throws Exception {
    ...
    /**
     * Need to call from DomainRelationship Connects the specified relationship with the given Object.
     * @param context
     * @param relationshipType
     * @param objectId
     * @param strNewRelId
     ...
     * @throws FrameworkException
     */
    public String connectObject(Context context, RelationshipType relationshipType, java.lang.String objectId, String strNewRelId, boolean isFrom) throws Exception {
        String fromId = null;
        String toId = null;
        String connId = null;
        StringBuffer sbCmd = new StringBuffer();
    ...
    /**
     * Replaces the Feature List object with the specified relationship. Computes new Relationship and internally calls "floatConnections" method.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param featObjMap
     *            Feature related info which is to be processed "FL" objects related to Feature retrieved then "FloatFLConnection" method called.
     * @throws Exception
     */

    private void migrateFeaturesStep_1(Context context, Map featObjMap) throws Exception {
        ...
        if ((strType != null && (mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_FEATURE) || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_STRUCTURES)))) {
            isConflictFeature = "No";
        }

        if (isConflictFeature.equalsIgnoreCase("No") || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_FEATURE) || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_STRUCTURES)) {
            ...
            flObjSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].to.attribute["
                    + ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE + "]");
            flObjSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].to.attribute["
                    + ConfigurationConstants.ATTRIBUTE_FEATURE_SELECTION_TYPE + "]");
            ...
            flObjSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from.to["
                    + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id");
            flObjSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from.to["
                    + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id");

            flObjSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES + "].tomid["
                    + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id");
            flObjSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES + "].tomid["
                    + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id");
            ...
            MapList FL_List = domFeatureObj.getRelatedObjects(context,
                    ...
                    sbTypePattern2.toString(), flObjSelects, // objectSelects
            ...
            if (FL_List.size() == 0) {
                ...
                if (strType != null && (!mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                        ...
                        && !mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_STRUCTURES)) || isOfDerivationChangedType(context, strType)) {
                    if (strType != null && !isOfDerivationChangedType(context, strType)) {
                        ...
                        boFeatureObj.change(context, newFeatureChangeType, (String) featObjMap.get(DomainConstants.SELECT_NAME), (String) featObjMap.get(DomainConstants.SELECT_REVISION),
                                (String) featObjMap.get(DomainConstants.SELECT_VAULT), newFeaturePolicy);

                        mqlLogRequiredInformationWriter("Object id 				:: " + featureObjId + "\n");
                        mqlLogRequiredInformationWriter("Object changed from type :: " + strType + " to new type " + newFeatureChangeType + " new policy " + newFeaturePolicy + "\n");

                        mapAttribute.put(ConfigurationConstants.ATTRIBUTE_VARIANT_DISPLAY_TEXT, (String) featObjMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_MARKETING_TEXT + "]"));
                        mapAttribute.put(ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME, (String) featObjMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_MARKETING_NAME + "]"));
                        mapAttribute.put(ConfigurationConstants.ATTRIBUTE_SUBFEATURECOUNT, (String) featObjMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_SUBFEATURECOUNT + "]"));
                        mapAttribute.put(ConfigurationConstants.ATTRIBUTE_ORIGINATOR, (String) featObjMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_ORIGINATOR + "]"));
                        if (newFeatureChangeType != null && (newFeatureChangeType.equalsIgnoreCase(ConfigurationConstants.TYPE_LOGICAL_FEATURE)
                                || newFeatureChangeType.equalsIgnoreCase(ConfigurationConstants.TYPE_MANUFACTURING_FEATURE))) {
                            mapAttribute.put(ConfigurationConstants.ATTRIBUTE_DUPLICATE_PART_XML, (String) featObjMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_DUPLICATE_PART_XML + "]"));
                        }
                    ...
                    try {
                        ...
                        if (strRelType != null && strRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_CANDIDATE_ITEM)) {
                            ...
                            if (strType != null && !mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                                ...
                                if (newFeatureChangeType != null && (newFeatureChangeType.equalsIgnoreCase(TYPE_CONFIGURATION_FEATURE)) && (strManAttr != null && strManAttr.equalsIgnoreCase("Yes"))
                                ...
                                if (newFeatureChangeType != null && (newFeatureChangeType.equalsIgnoreCase(TYPE_CONFIGURATION_FEATURE)) && (strManAttr != null && strManAttr.equalsIgnoreCase("No"))) {
                                ...
                                // Set the new Feat Type attribute values when Feat is standalone
                                if (ProductLineCommon.isNotNull(strNewRelType) && !strNewRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES)) {
                            ...
                            /*
                             * if(strNewRelType!=null && strNewRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES)){ HashMap mapRelAttributes = new
                             * HashMap();
                             * 
                             * 
                             * mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_CONFIGURATION_TYPE, ConfigurationConstants.RANGE_VALUE_SYSTEM);
                            ...
                            if (strParentFeatureId != null && strParentFeatureId.length() != 0) {
                                ...
                                if (isConflictFeature.equalsIgnoreCase("No") && isConflictParentFeature.equalsIgnoreCase("No")) {
                                    ...
                                    /*
                                     * First ...Check if Parent is already converted to New Type or not If Not then convert it to the new Type then continue Attributes will be set later ..whenever the
                                     * Feature comes as child in the list.
                                     */
                                    if (strParentFeatureType != null && !mxType.isOfParentType(context, strParentFeatureType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                                        ...
                                        boParFeatureObj.change(context, newParentFeatureChangeType, (String) htParentObjData.get(DomainConstants.SELECT_NAME),
                                                (String) htParentObjData.get(DomainConstants.SELECT_REVISION), (String) htParentObjData.get(DomainConstants.SELECT_VAULT), strNewParentFeatPolicy);

                                        mqlLogRequiredInformationWriter("Object id :: " + strParentFeatureId + "\n");
                                        mqlLogRequiredInformationWriter(
                                                "Object changed from type :: " + strType + " to new type " + newParentFeatureChangeType + " new policy " + strNewParentFeatPolicy + "\n");
                                    ...
                                    if (strType != null && !mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
                                        ...
                                        } else if (newChildFeatureChangeType != null && (newChildFeatureChangeType.equalsIgnoreCase(ConfigurationConstants.TYPE_LOGICAL_FEATURE)
                                        ...
                                        } else if (strType != null && (mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_FEATURE)
                                                || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_SOFTWARE_FEATURE))) {
                                            newReltoConnect = ConfigurationConstants.RELATIONSHIP_LOGICAL_FEATURES;
                                    ...
                                    if (strKITAttrValue.equals(RANGE_VALUE_INPUT) && strFSTAttrValue.equals(ConfigurationConstants.RANGE_VALUE_KEY_IN)
                                        ...
                                        Object objSOAttrKIVs = flMap
                                                .get("to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].attribute[" + ConfigurationConstants.ATTRIBUTE_KEY_IN_VALUE + "]");
                                        if (objSOAttrKIVs instanceof StringList) {
                                            slSOAttrKIVs = (StringList) flMap
                                                    .get("to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].attribute[" + ConfigurationConstants.ATTRIBUTE_KEY_IN_VALUE + "]");

                                        } else if (objSOAttrKIVs instanceof String) {
                                            slSOAttrKIVs.addElement((String) flMap
                                                    .get("to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].attribute[" + ConfigurationConstants.ATTRIBUTE_KEY_IN_VALUE + "]"));
                                        }

                                        String strParentPCIdSel = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
                                                + "].from.to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id";
                                        Object objParentPCIds = flMap.get(strParentPCIdSel);

                                        if (objParentPCIds == null || "".equals(objParentPCIds)) {
                                            strParentPCIdSel = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES
                                                    + "].tomid[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id";
                                        ...
                                        String strParentSORelIdSel = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
                                                + "].from.to[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id";
                                        Object objParentSORelIds = flMap.get(strParentSORelIdSel);
                                        if (objParentSORelIds == null || "".equals(objParentSORelIds)) {
                                            strParentSORelIdSel = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.to["
                                                    + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES + "].tomid[" + ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS + "].id";
                                    ...
                                    mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_RULE_TYPE, (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + "]"));

                                    if (newReltoConnect != null && (newReltoConnect.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_LOGICAL_FEATURES)
                                        ...
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_FIND_NUMBER, (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_FIND_NUMBER + "]"));
                                        ...
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_QUANTITY, (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_QUANTITY + "]"));
                                        ...
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_USAGE, (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_USAGE + "]"));
                                    }
                                    ...
                                    if (newReltoConnect != null && newReltoConnect.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_CONFIGURATION_OPTIONS)) {

                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_SEQUENCE_ORDER, (String) flMap
                                                .get("to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].attribute[" + ConfigurationConstants.ATTRIBUTE_SEQUENCE_ORDER + "]"));
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_DEFAULT_SELECTION,
                                                (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_DEFAULT_SELECTION + "]"));
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_LIST_PRICE, (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_LIST_PRICE + "]"));
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_MAXIMUM_QUANTITY,
                                    ...
                                    if (newReltoConnect != null && newReltoConnect.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES)) {
                                        ...
                                        mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_INHERITED, (String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_INHERITED + "]"));

                                        // mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_ROLLED_UP,ConfigurationConstants.RANGE_VALUE_FALSE);
                                    }

                                    // Mandatory Configuration Feature relationship
                                    if (newReltoConnect != null && newReltoConnect.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES)) {
                                    ...
                                    if (newReltoConnect != null && (newReltoConnect.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_VARIES_BY)
                                        ...
                                        if ((String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_MANDATORY_FEATURE + "]") != null
                                                && ((String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_MANDATORY_FEATURE + "]"))
                                                        .equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_YES)) {

                                            mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_CONFIGURATION_TYPE, ConfigurationConstants.RANGE_VALUE_MANDATORY);
                                            ...
                                            mapRelAttributes.put(ConfigurationConstants.ATTRIBUTE_ROLLED_UP, ConfigurationConstants.RANGE_VALUE_TRUE);
                                                ...
                                                && ((String) flMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_MANDATORY_FEATURE + "]"))
                                                        .equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_NO)) {
            ...
            if ((StringList) featObjMap.get("to[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].id") != null) {
                ...
                /*
                 * Not using the above statements as there is issue in sequence of ids in String list coming from above getInfo Doing the getInfo again on the given Feature Id
                 */
                ...
                sLEffectivityStatusValue = (StringList) MFeatObj
                        .get("to[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].attribute[" + ConfigurationConstants.ATTRIBUTE_EFFECTIVITY_STATUS + "]");
     ...
     *            the eMatrix <code>Context</code> object
     * @param flMap
     *            "Feature List" related information
     * @param htChildObjData
     *            -- Child Feat Obj related information
     * @param iCntFLObjs
     *            -- Count of FL objects
     * @throws Exception
     */

    private String setAttributeValuesOfChildFeatureObj(Context context, Map flMap, Map htChildObjData, boolean bConflictSelType) throws Exception {
        ...
        if (strChildFeatureType != null && !mxType.isOfParentType(context, strChildFeatureType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
            ...
            boChildFeatureObj.change(context, newChildFeatureChangeType, (String) htChildObjData.get(DomainConstants.SELECT_NAME), (String) htChildObjData.get(DomainConstants.SELECT_REVISION),
                    (String) htChildObjData.get(DomainConstants.SELECT_VAULT), strNewChildFeatPolicy);

            mqlLogRequiredInformationWriter("Child Object changed from type :: " + strChildFeatureType + " to new type " + newChildFeatureChangeType + " new policy " + strNewChildFeatPolicy + "\n");
        ...
        mapChildFeatAttribute.put(ConfigurationConstants.ATTRIBUTE_SUBFEATURECOUNT, (String) htChildObjData.get("attribute[" + ConfigurationConstants.ATTRIBUTE_SUBFEATURECOUNT + "]"));
        mapChildFeatAttribute.put(ConfigurationConstants.ATTRIBUTE_ORIGINATOR, (String) htChildObjData.get("attribute[" + ConfigurationConstants.ATTRIBUTE_ORIGINATOR + "]"));
        ...
        if (strChildNewFeatureType != null && strChildNewFeatureType.equalsIgnoreCase(TYPE_CONFIGURATION_FEATURE)) {
            ...
            Object objKITOnChildFLs = flMap.get("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].to.attribute["
                    + ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE + "]");
            if (objKITOnChildFLs instanceof StringList) {
                slKITOnChildFLs = (StringList) flMap.get("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM
                        + "].to.attribute[" + ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE + "]");

            } else if (objKITOnChildFLs instanceof String) {
                slKITOnChildFLs.addElement((String) flMap.get("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM
                        + "].to.attribute[" + ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE + "]"));
            }
        ...
        if (strChildNewFeatureType != null && (strChildNewFeatureType.equalsIgnoreCase(ConfigurationConstants.TYPE_LOGICAL_FEATURE)
    ...
    /**
     * This method gets the Relationship Type with which the "Configuration Feature" Object is to be connected with the Parent Object.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param flMap
     *            --"Feature List" related information
     * @param htChildObjData
     *            -- Child Feat Obj(Configurature Feature) related information
     * @param strParentFeatureId
     *            -- Parent Object Id
     * @param strParentFeatureType
     *            -- Parent Object Type
    ...
    private String getNewRelForConfigFeatType(Context context, Map flMap, Map htChildObjData, String strParentFeatureId, String strParentFeatureType) throws Exception {
        ...
        try {
            ...
            /*
             * if(strInherited!=null && strInherited.equalsIgnoreCase("True") && strMandatory!=null && strMandatory.equalsIgnoreCase("No")){
             */
            /*
             * This means CF is added to the Product due to add DV.. it is rolled up Need to decide it should be "Varies By" or "Inactive Varies By" rel Also simultaneously check for new
             * "Valid Context" & "Invalid Context" relationship
             */
            // Get the Product Id.. compute whether DV is Active /Inactive under this Product context
            if (strParentFeatureType != null && strParentFeatureType.equals(ConfigurationConstants.TYPE_PRODUCT_VARIANT)) {
                ...
                sLRelSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from["
                        + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.type");
                ...
                DomainConstants.MULTI_VALUE_LIST
                        .add("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].torel.to.from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.type");
                DomainConstants.MULTI_VALUE_LIST.add("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
                        + "].to.from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.type");
                DomainConstants.MULTI_VALUE_LIST.add("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].id");
                DomainConstants.MULTI_VALUE_LIST
                        .add("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].frommid[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY + "].id");
                DomainConstants.MULTI_VALUE_LIST
                        .add("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.id");
                DomainConstants.MULTI_VALUE_LIST
                        .add("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.name");

                Map featureListMapList = domPVId.getInfo(context, sLRelSelects);

                DomainConstants.MULTI_VALUE_LIST
                        .remove("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].torel.to.from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.type");
                DomainConstants.MULTI_VALUE_LIST.remove("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
                        + "].to.from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.type");
                DomainConstants.MULTI_VALUE_LIST.remove("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].id");
                DomainConstants.MULTI_VALUE_LIST
                        .remove("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].frommid[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY + "].id");
                DomainConstants.MULTI_VALUE_LIST
                        .remove("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.id");
                DomainConstants.MULTI_VALUE_LIST
                        .remove("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.name");

                StringList sLInactiveVBy = new StringList();
                sLInactiveVBy = (StringList) featureListMapList
                        .get("from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].frommid[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY + "].id");
     ...
     * @param context
     * @param relID
     *            -- relationship Id for modification
     * @param targetrelationshipId
    ...
    public String setToRelationship(Context context, java.lang.String relID, String targetrelationshipId, boolean isFrom) throws Exception {
     ...
     * @param context
     * @param sLFeatAndRuleIds
     *            --Stringlist of Feature and Rule Ids on which the interface was added.
     * @return Returns boolean true : If it is executed for all given objects false : If it is not executed for all given objects (In case interface doesnt exist on the given object)
     * @throws Exception
     */
    private void removalOfTemporaryAttSet(Context context, StringList sLFeatAndRuleIds) throws Exception {
     ...
     * @throws Exception
     */
    public static String stringDecode(Context context, String strFeatureName) throws Exception {
     ...
     * @param context
     * @param strId
     *            - Object Id or connection Id on which Interface to be added
     * @param strAdmin
     *            - Interface to be added on bus or relationship Possible values : bus or rel
     * @param strInterfaceName
     *            - Name of the Interface to be added
     * @param mAttrDetails
     *            - Map containing the key-value pair of interface attributes.
     * @return - bResult - true if attribute is set successfully - false in case of failure.
     * @throws Exception
     */
    public boolean addInterfaceAndSetAttributes(Context context, String strId, String strAdmin, String strInterfaceName, Map mAttrDetails) throws Exception {
        ...
        try {
            ...
            if (mAttrDetails != null && mAttrDetails.size() > 0) {
                ...
                /*
                 * String strMqlCommandOff = "trigger off"; MqlUtil.mqlCommand(context,strMqlCommandOff,true); mqlLogRequiredInformationWriter("trigger off .." + "\n");
                 */
                ...
                /*
                 * String strMqlCommandOn = "trigger on"; MqlUtil.mqlCommand(context,strMqlCommandOn,true); mqlLogRequiredInformationWriter("trigger on .." + "\n");
                 */
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param sLRERelIdsToBeDeleted
     *            StringList of Rel ids to be deleted
     * @throws Exception
     */
