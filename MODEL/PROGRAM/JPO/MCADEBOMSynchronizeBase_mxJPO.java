...
public class MCADEBOMSynchronizeBase_mxJPO extends IEFCommonUIActions_mxJPO {
    ...
    // [NDM] Start
    /*
     * public String canshowEBOMSynchronize(Context context, String []args) throws MCADException { //_context = context; if(_util == null) { _util = new MCADMxUtil(context, new
     * MCADServerResourceBundle(args[3]), new IEFGlobalCache()); }
     * 
     * if(isAEFInstalled(context)) return "true"; else return "false"; }
     * 
     * // Business Logic for implementing protected void canPerformOperationCustom(Context context, Hashtable resultDataTable) throws MCADException { //System.out.println(
     * "[MCADEBOMSynchronize.canPerformOperationCustom] :..."); /*try { // } catch(Exception e) { throw new MCADException("EBOMSynch Operation Cannot be performed because of an error condition :" +
     * e.getMessage()); }
     */
    // }
    // [NDM] end
    ...
    public void executeCustom(Context _context, Hashtable resultAndStatusTable) throws MCADException {
        try {
            ...
            /*
             * if(_globalConfig.isMajorType(busTypeName) && !_generalUtil.isBusObjectFinalized(_context,_busObject)) { _busObject = _util.getActiveMinor(_context,_busObject); if(_busObject == null)
             ...
             * 
             * _busObject.open(_context); _busObjectID = _busObject.getObjectId(_context); busTypeName = _busObject.getTypeName(); _busObject.close(_context); } else
             * if(_globalConfig.isMinorType(busTypeName) && _generalUtil.isBusObjectFinalized(_context,_busObject))
             */
            // {
            // _busObject = _util.getMajorObject(_context,_busObject);
    ...
    private String getIfRelatedDocumentExists(Context _context, String typeName, String childBusAndRelIds, StringBuffer childBusAndRelIdsBuffer, ArrayList busObjIDsList, ArrayList typeList)
            throws Exception {
    ...
    protected void validateDrawingForEBOM(Context context, BusinessObject model) throws MatrixException, MCADException, MCADServerException {
        ...
        for (int j = majorRevisions.size() - 1; j > -1; j--) {
            ...
            if (partIdOfPreviousRevision != null && !"".equals(partIdOfPreviousRevision)) {
                ...
                if (!_generalUtil.isBusObjectFinalized(context, _busObject) && isModelReleased && isPrevisiousRevisionReleased && partIdOfModel != null && partIdOfPreviousRevision != null
                        && partIdOfModel.equalsIgnoreCase(partIdOfPreviousRevision) && _generalUtil.isPartReleased(context, partIdOfModel, PART_RELEASE_STATE)) {
                    String busType = _busObject.getTypeName();
    ...
    protected String getRelatedEBOMPart(Context _context, String busId, String busName, String instanceName) {
        ...
        try { // [NDM]
             ...
             * 
             * if("true".equalsIgnoreCase(assignPartToMajor)) { BusinessObject busObj = new BusinessObject(busId); busObj.open(_context); BusinessObject majorObj = _util.getMajorObject(_context,
             * busObj); if(majorObj!= null) { majorObj.open(_context); busId = majorObj.getObjectId(); majorObj.close(_context); } busObj.close(_context); }
             */
            // [NDM] end
    ...
    protected String RecursiveBom(Context _context, String parentPartObjId, Vector cadRelIds, String cadObjId, String cadBusName, String instanceName, Hashtable relsAndEnds, String busTypeName,
            StringBuffer ebomDoneObjectBuffer, boolean isParentFantom, String parentCadObjID, String childObjId) throws Exception {
                ...
                for (Enumeration childIds = childAndRelIds.keys(); childIds.hasMoreElements();) {
                    ...
                    if (!_util.isMajorObject(_context, childId))// _globalConfig.isMinorType(childBusObj.getTypeName())) //[NDM] OP6
                    {
                        ...
                        if (majorBusObject == null) {
                            ...
                            MCADServerException.createException(_serverResourceBundle.getString("mcadIntegration.Server.Message.CannotPerformOperationAsMajorAbsentOrNotAccessible", messageDetails),
                                    null);
                        }
                    }
                    ...
                    if (relFilterList != null && relFilterList.size() > 0) {
                        String correspondingPartID = RecursiveBom(_context, partObjId, relIdsVector, childId, busName, null, relsAndEnds, childTypeName, ebomDoneObjectBuffer, isFantomNode, cadObjId,
                                childId);
    ...
    /**
     * This method is provided for to customize the behavior of creating EC Part on EBOMSync This validation can be based on the relationship attribute. This method should return a List which will
     * contain relationship ids for which the validation is suceessful.
     * @param context
     *            Context ENOVIA Context object reference
     * @param relIdsVector
     *            Vector This vector contains Relationship Id of the component with its parent
     */
    ...
    /*
     * private String getPartAutoName(Context context,String busName,String instanceName) throws Exception { String partName = null;
     * 
     * if(busNameAutoNameTable.get(busName) != null) partName = (String)busNameAutoNameTable.get(busName); else { Hashtable<String, Object> paramJPO = new Hashtable<String, Object>();
     * paramJPO.put("type_CADModel_Series", confAttrPartSeries); paramJPO.put("type_CADModel_Count", new Integer(1)); //not needed paramJPO.put("type_CADDrawing_Series", confAttrPartSeries);
     * paramJPO.put("type_CADDrawing_Count", new Integer(0));
     * 
     * Hashtable typeAutoNameListTable = (Hashtable)JPO.invoke(context, "DECNameGenerator", new String[0], "getNames", JPO.packArgs(paramJPO), Hashtable.class); Vector cadModelAutonamesList = (Vector)
     * typeAutoNameListTable.get("type_CADModel_Names"); partName = (String)cadModelAutonamesList.get(0); } return partName; }
     */
    ...
    protected String createNewEBOM(Context _context, String parentPartId, String childPartId, Vector uniqueRelIdsVector, String parentChildFindNumber, boolean isParentFantom, String parentCadObjID,
            String childObjId) throws Exception {
        String ceratedEBOM = "false";
        try {

            if (parentPartId != null && parentPartId.length() > 0 && childPartId != null && childPartId.length() > 0 && uniqueRelIdsVector != null && uniqueRelIdsVector.size() > 0
                    && !parentPartId.equalsIgnoreCase(childPartId)) {
        ...
        if (!partsTable.containsKey(busObjectID)) {
            ...
            if (partsToBeCreated.size() > 0) {
                ...
                if (confAttrFailAtMissingPart) {
                    errMessage = _serverResourceBundle.getString("mcadIntegration.Server.Message.EBOMCreationFailedPartsMissing") + errMessage;
                    resultAndStatusTable.put(MCADServerSettings.JPO_EXECUTION_STATUS, "false");
                    resultAndStatusTable.put(MCADServerSettings.JPO_STATUS_MESSAGE, errMessage);
                } else {
                    errMessage = _serverResourceBundle.getString("mcadIntegration.Server.Message.EBOMCreationCompletedPartsMissing") + errMessage;
    ...
    private BusinessObjectList getRelatedPartList(Context _context, BusinessObject cadObject) throws Exception {
        ...
        /*
         * if("true".equalsIgnoreCase(confAttrAssignPartToMajor)) //{NDM] { BusinessObject majorBusObject = _util.getMajorObject(_context, cadObject); if(majorBusObject == null) { majorBusObject =
         * cadObject; } relatedPartsList = _util.getRelatedBusinessObjects(_context,majorBusObject,relPartSpecification,"to"); } else
         */
        {
