...
public class emxSearchIndexExpansionBase_mxJPO {
    ...
    /*
     * public void initRelNames(Context context, String symRelsCsv) throws Exception { StringList symRels = FrameworkUtil.split(symRelsCsv, ","); StringList rels = new StringList(symRels.size());
     * 
     * for (int i = 0; i < symRels.size(); i++) { String sym = (String) symRels.get(i); String relName = PropertyUtil.getSchemaProperty(context, sym); if (relName == null || relName.equals("")) {
     * relName = sym; } rels.add(relName); } _relNamesCSL = FrameworkUtil.join(rels, ","); }
     */
    ...
    public ArrayList getExpansionPaths(Context context, String objectId) throws Exception {
        ...
        MapList mList = null;
        mList = domObj.getRelatedObjects(context, _relNamesCSL, "*", objectSelects, relselects, true, // boolean getTo,
