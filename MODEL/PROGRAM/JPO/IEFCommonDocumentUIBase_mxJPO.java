...
public class IEFCommonDocumentUIBase_mxJPO {
     ...
     * @grade 0
     */
    public IEFCommonDocumentUIBase_mxJPO(Context context, String[] args) throws Exception {
        if (args.length > 1) {
        ...
        try {
            ...
            for (int i = 0; i < objectListSize; i++) {
                ...
                try {
                    ...
                    /*
                     * DomainObject domainObject = DomainObject.newInstance(context, objId); StringList selects = new StringList(7); selects.add(DomainConstants.SELECT_ID);
                     * selects.add(DomainConstants.SELECT_TYPE); selects.add(DomainConstants.SELECT_FILE_NAME); objIEFCDMSupport.IEFAddElement(selects, objIEFCDMSupport.CDM_SELECT_ACTIVE_FILE_LOCKED);
                     * objIEFCDMSupport.IEFAddElement(selects, objIEFCDMSupport.CDM_SELECT_MOVE_FILES_TO_VERSION); objIEFCDMSupport.IEFAddElement(selects,
                     * objIEFCDMSupport.CDM_SELECT_FILE_NAMES_OF_ACTIVE_VERSION);
                     * 
                     * if(!isVersionable) // If the object is not versionable then get the lock status from the major BO. selects.add(DomainConstants.SELECT_LOCKED);
                     * 
                     * //objectMap = (Hashtable) objectList.get(i); objectMap = domainObject.getInfo(context,selects);
                     */
                ...
                if (parentType.equals(CommonDocument.TYPE_DOCUMENTS)) {
                    ...
                    try {
                        if (isVersionable)
                            ...
                            // If the object is not versionable then get the lock status from the major BO.
                    ...
                    if (locked != null) {
                        ...
                        while (itr.hasNext()) {
                            ...
                            if (lock.equalsIgnoreCase("true")) {
                                lockCount++;
                                /*
                                 * if(isVersionable) lockCount ++; else { lockCount = fileCount; break; }
                                 */
                            }
                        }
                    }
                    ...
                    /*
                     * if(isVersionable) urlBuf.append(lockCount + "/" + fileCount); else // If the object is not versionable then lockCount = fileCount urlBuf.append(fileCount + "/" + fileCount);
                     */
        ...
        try {
            ...
            for (int i = 0; i < objectListSize; i++) {
                ...
                if (parentType.equals(CommonDocument.TYPE_DOCUMENTS)) {
                    ...
                    /*
                     * if(!isprinterFriendly && !"&nbsp;".equals(version) ) { urlBuf.append("<a href =\"javascript:showNonModalDialog('"); urlBuf.append(baseURLBuf.toString());
                     * urlBuf.append("&objectId="); urlBuf.append(objectId); urlBuf.append("',730,450)\">"); }
                     */
                    urlBuf.append(version);
                    /*
                     * if(!isprinterFriendly && !"&nbsp;".equals(version)) { urlBuf.append("</a>"); }
                     */
    ...
    public static void setParentTypeMapping(Context context, String type) throws Exception {
        ...
        /*
         * if ( isAbstract && parentType.equals("DOCUMENTS") ) { typeMapping.put(type, currentType); } else { typeMapping.put(type, parentType); }
         */
    }
                ...
                if ((CommonDocument.TYPE_DOCUMENTS).equals(parentType)) {
                    ...
                    if ("false".equalsIgnoreCase(isVersionable)) {
                        ...
                        if (hasCheckinAccess) {
                            // checkin
                            if (!isprinterFriendly) {
                                strBuf.append("<a href=\"javascript:showNonModalDialog('../components/emxCommonDocumentPreCheckin.jsp?objectId=" + objectId
                                        + "&showComments=true&showFormat=true&objectAction=checkin','780','570');\">");
                            ...
                            if (lockCount > 0) {
                                if (!isprinterFriendly) {
                                    strBuf.append("<a href=\"javascript:showNonModalDialog('../components/emxCommonDocumentPreCheckin.jsp?objectId=" + objectId
                                            + "&showFormat=readonly&showComments=required&objectAction=update&allowFileNameChange=true','730','450');\">");
