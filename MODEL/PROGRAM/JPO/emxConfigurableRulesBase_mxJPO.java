 ...
 **
 ** static const char RCSID[] = $Id: /ENOVariantConfigurationBase/CNext/Modules/ENOVariantConfigurationBase/JPOsrc/base/${CLASSNAME}.java 1.32.2.7.1.1 Wed Oct 29 22:27:16 2008 GMT przemek
 * Experimental${CLASSNAME}.java 1.8 Fri Sep 14 11:29:22 2007 GMT ds-pborgave Experimental$
 */
...
public class emxConfigurableRulesBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     */
    protected static final String TYPE_PRODUCT = ProductLineConstants.TYPE_PRODUCTS;
     ...
     */
    protected static final String KEY_FIRST_PART = "emxProduct.ProductRuleCreation";
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (!context.isConnected()) {
            ...
            String strContentLabel = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Error.UnsupportedClient", strLanguage);
     ...
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6SP1 this could not be deprecated as referenced from migration JPO
     **/
    ...
    public static List getExpression(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < lstobjectList.size(); i++) {
            ...
            try {
                ...
                mlIncRule = dom.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION, ProductLineConstants.TYPE_INCLUSION_RULE, slObjectSel, null, true, true, (short) 1, null,
                        null, 0);
    ...
    /**
     * This will retrieve the Rule Type, either Inclusion on Simple Inclusion, and will be a href, for configuring the Inclusion rule
     * @param context
     * @param args
     * @return
     * @throws Exception
    ...
    public Object showGBOMInclusionRuleLink(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            for (int j = 0; j < objectList.size(); j++) {
                ...
                String strIRuleSel = new StringBuffer("tomid[").append(ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append("].from.id").toString();

                String strRuleComplSel = new StringBuffer("tomid[").append(ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append("].from.attribute[")
                        .append(ConfigurationConstants.ATTRIBUTE_RULE_COMPLEXITY).append("]").toString();
                ...
                // TODO DV Related
                String dvselectable = new StringBuffer("from.from[").append(ConfigurationConstants.RELATIONSHIP_VARIES_BY).append("].to.id").toString();
                ...
                if (ruleComplexity == null || "".equals(ruleComplexity)) {
                    isRuleExists = false;
                    if (designVariants.size() > 0 && !mxType.isOfParentType(context, relFromType, ConfigurationConstants.TYPE_PRODUCTS)
                ...
                if (designVariants != null && !("".equals(designVariants)) && !("null".equals(designVariants)) && designVariants.size() > 0
                        && mxType.isOfParentType(context, objectType, ConfigurationConstants.TYPE_PART_FAMILY) && !mxType.isOfParentType(context, relFromType, ConfigurationConstants.TYPE_PRODUCTS)) {

                    flag2 = true;
                    sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/GBOMRuleExpressionPreProcess.jsp?modetype=create");

                } else if (ruleComplexity.equals("Simple")) {

                    flag2 = true;
                    sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/GBOMSimpleInclusionPreProcess.jsp?modetype=create&amp;jsTreeID=");
                    strRuleTypeDisplay = strRuleType;
                } else {
                    if (isRuleExists) {
                        sIncLink.append(
                                "<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?modetype=edit%26commandName=FTRInclusionRuleSettings%26ruleType=InclusionRule%26SuiteDirectory=configuration%26hyperLinkCheck=yes");
                        ...
                        sIncLink.append(
                                "<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?modetype=create%26commandName=FTRInclusionRuleSettings%26ruleType=InclusionRule%26SuiteDirectory=configuration%26hyperLinkCheck=yes");
                        sIncLink.append("%26submitURL=");
                ...
                sb = sb.append("&amp;objectId=").append((String) objectMap.get("id"));
                sb = sb.append("&amp;relId=").append((String) objectMap.get("id[connection]"));
                sb = sb.append("&amp;iRuleId=").append(strIncRuleID);
                sb = sb.append("&amp;partName=").append(XSSUtil.encodeForXML(context, (String) objectMap.get(DomainObject.SELECT_NAME)));
                sb = sb.append("&amp;lfeatureId=").append((String) objectMap.get(DomainRelationship.SELECT_FROM_ID));
                ...
                sb = sb.append(strEndPart1).append(strRuleTypeDisplay).append(strEndPart2);

                if (strReportFormat != null && strReportFormat.equals("null") == false && strReportFormat.equals("") == false && strReportFormat.length() > 0)
     ...
     * @since R211
     */
    public static List getGBOMExpression(Context context, String[] args) throws Exception {
        List lstExpressionList = new StringList();
        ...
        try {
            ...
            String strLeftExpr = new StringBuffer("tomid[").append(ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append("].from.attribute[")
                    .append(ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION).append("]").toString();
            String strRightExpr = new StringBuffer("tomid[").append(ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append("].from.attribute[")
                    .append(ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION).append("]").toString();
            for (int i = 0; i < lstobjectList.size(); i++) {
                Map objectMap = (Map) lstobjectList.get(i);
                ...
                objectMap.put("attribute[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "]", leftExpression);
                objectMap.put("attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]", rightExpression);
            }

            lstExpressionList = (StringList) (JPO.invoke(context, "emxBooleanCompatibility", null, "showInclusionRuleExpression", JPO.packArgs(parametersMap), StringList.class));
    ...
    // }
    /**
     * This will be used to render the Rule Type column in Duplicate Part Table
     * @param context
     * @param args
     * @return
     * @throws Exception
     * @since R212
     */
    public Object showGBOMInclusionRuleType(Context context, String[] args) throws Exception {
        Vector vGBOMInclusionRuleLink = new Vector();
        ...
        try {
            ...
            Map paramMap = (Map) parametersMap.get("paramList");
            String strParentOID = (String) paramMap.get("objectId");
            // getting the active design variants connected to the parent
            // feature
            argMap.put("objectId", strParentOID);
            argMap.put("parentOID", strParentOID);
            String[] arrJPOArgs = JPO.packArgs(argMap);
            LogicalFeature_mxJPO lfeatureJPO = new LogicalFeature_mxJPO(context, null);
            MapList mapListDV = lfeatureJPO.getActiveDesignVariants(context, arrJPOArgs);
            String strSeleDVAttr = new StringBuffer().append("tomid[").append(ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append("].from.attribute[")
                    .append(ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS).append("]").toString();
            String strSeleIRuleID = new StringBuffer().append("tomid[").append(ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append("].from.id").toString();

            for (int i = 0; i < objectList.size(); i++) {
                objectMap = (Map) objectList.get(i);
                // get inclusion rule id, if exist
                String iRuleID = "";
                String strDVListOnIR = "";
                if (!(objectMap.get(strSeleIRuleID).toString().isEmpty()))
                    iRuleID = objectMap.get(strSeleIRuleID).toString();
                if (!(objectMap.get(strSeleDVAttr).toString()).isEmpty())
                    strDVListOnIR = objectMap.get(strSeleDVAttr).toString();
                argMap.put("FeatureObjId", strParentOID);
                argMap.put("IRuleId", iRuleID);
                argMap.put("DVariantListOnIR", strDVListOnIR);
                argMap.put("ActiveDVList", mapListDV);

                boolean bIsSimpleInclRule = Part.isSimpleInclusionRule(context, argMap).booleanValue();

                String strLanguage = context.getSession().getLanguage();

                if (bIsSimpleInclRule) {
                    strRuleType = EnoviaResourceBundle.getProperty(context, SUITE_KEY, strLanguage, SIMPLE_INCLUSION_RULE_KEY);
                } else {
                    strRuleType = EnoviaResourceBundle.getProperty(context, SUITE_KEY, strLanguage, "emxProduct.Rule_Type.ComplexInclusionRule");
                }
    ...
    /**
     * This method is used to return inclusion/exclusion rule link for Selection view In Table column. @param context the eMatrix <code>Context</code> object @param args holds arguments @return
     * Vector @throws Exception if the operation fails @since R212 @author IXH @category
     */

    public Vector showInclusionRuleLinkInRuleTypeColumn(Context context, String[] args) throws Exception {
        HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
        List lstobjectList = (MapList) parametersMap.get("objectList");
    ...
    /**
     * This method is used to return inclusion/exclusion rule link. @param context the eMatrix <code>Context</code> object @param args holds arguments @return Vector @throws Exception if the operation
     * fails @since R212 @author IXH @category
     */

    public Vector showInclusionRuleLink(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < lstobjectList.size(); i++) {
            ...
            Object leftExpFromType = (Object) objectMap.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.type");
            ...
            if (iRuleIndex != -1) {
                Object iRuleObjects = (Object) objectMap.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.id");
            ...
            if (hasIR) {
                ...
                sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?rowLevel=" + strLevel
                        + "%26modetype=edit%26commandName=FTRInclusionRuleSettings%26ruleType=InclusionRule%26SuiteDirectory=configuration%26hyperLinkCheck=yes");
                sIncLink.append("%26submitURL=");
                sIncLink.append("../configuration/InclusionRuleEditPostProcess.jsp?mode=edit%26ruleType=InclusionRule");
                ...
                sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?rowLevel=" + strLevel
                        + "%26modetype=create%26commandName=FTRInclusionRuleSettings%26ruleType=InclusionRule%26SuiteDirectory=configuration%26hyperLinkCheck=yes");
                sIncLink.append("%26submitURL=");
                sIncLink.append("../configuration/InclusionRuleEditPostProcess.jsp?mode=create%26ruleType=InclusionRule");
            ...
            sIncLink = sIncLink.append("&amp;parentOID=").append(XSSUtil.encodeForHTMLAttribute(context, strParentOID)).append("&amp;objectId=");

            strObjectId = (String) objectMap.get("id");
            DomainObject doObject = DomainObject.newInstance(context);
            doObject.setId(strObjectId);
            String typeFeature = doObject.getInfo(context, DomainConstants.SELECT_TYPE);

            if (mxType.isOfParentType(context, typeFeature, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES) || mxType.isOfParentType(context, typeFeature, ConfigurationConstants.TYPE_PRODUCTS)
                    || mxType.isOfParentType(context, typeFeature, ConfigurationConstants.TYPE_LOGICAL_STRUCTURES)) {
                checkType = true;
            }
            strRelId = (String) objectMap.get("id[connection]");
            ...
            objSelect.add("attribute[" + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + "]");
            ...
            if (strRelId != null && !("".equals(strRelId)) && !("null".equals(strRelId))) {
                ...
                strRuleType = (String) ((StringList) ht.get(ConfigurationConstants.SELECT_RULE_TYPE)).get(0);
            } else {
            ...
            sb = new StringBuffer();
            sb = sb.append(sIncLink.toString());
            sb = sb.append(XSSUtil.encodeForHTMLAttribute(context, strObjectId)).append("&amp;relId=").append(XSSUtil.encodeForHTMLAttribute(context, strRelId)).append("&amp;ruleId=")
                    .append(XSSUtil.encodeForHTMLAttribute(context, strIncRuleID));
            if (ProductLineCommon.isNotNull(strRuleType)) {
                strRuleTypeDisplayKey = "emxFramework.Range.Rule_Type." + strRuleType.toString();
                strRuleTypeDisplay = EnoviaResourceBundle.getProperty(context, "Framework", strRuleTypeDisplayKey, strLanguage);
            }
            String strRuleTypeToolTip = strRuleTypeDisplay.toString();
            sb = sb.append(strEndPart1).append(strMidPart1).append("\"" + XSSUtil.encodeForHTMLAttribute(context, strRuleTypeToolTip) + "\"").append(strExpression.trim()).append(strMidPart2)
                    .append(XSSUtil.encodeForXML(context, strRuleTypeDisplay)).append(strEndPart2);

            if ((strReportFormat != null && strReportFormat.equals("null") == false && strReportFormat.equals("") == false && strReportFormat.length() > 0) || !isFTRUser) {
                vInclusionRuleLink.add(strRuleTypeToolTip);
            } else {

                if (checkType && !typeFeature.equals(ConfigurationConstants.TYPE_BOOLEAN_COMPATIBILITY_RULE) && !typeFeature.equals(ConfigurationConstants.TYPE_FIXED_RESOURCE)
                        && !typeFeature.equals(ConfigurationConstants.TYPE_RULE_EXTENSION)) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a Map with the following entries: objectId - a String giving object id reportFormat - a String indicating wheather method is called from table or report.
     * @return List - A Vector containing the HTML code to generate a HREF for rule type.
     * @throws Exception
     *             if the operation fails
     * @author np4
     */
    public List showRuleExpressionForInactiveGBOM(Context context, String[] args) throws Exception {
        Vector vInclusionRuleLink = new Vector();
        try {
            StringBuffer sIncLink = new StringBuffer(260);
            sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/GBOMRuleExpressionPreProcess.jsp?modetype=create");
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                strRuleType = (String) objectMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + "]");
                sb = sb.append("&amp;ruleExcludeOrInclude=").append(strRuleType);
                iRuleID = (String) objectMap.get("tomid[" + ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.id");
                sb = sb.append("&amp;iRuleId=").append(iRuleID);
                logicalFTRID = (String) objectMap.get(DomainRelationship.SELECT_FROM_ID);
    ...
    public Object createNewLinkDisplayInTable(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (strProductObjectId != null) {
                ...
                BusinessType busType = new BusinessType(ConfigurationConstants.TYPE_PRODUCTS, context.getVault());
                ...
                busType = new BusinessType(ConfigurationConstants.TYPE_LOGICAL_STRUCTURES, context.getVault());
                ...
                busType = new BusinessType(ConfigurationConstants.TYPE_CONFIGURATION_FEATURES, context.getVault());
                busTypeList = busType.getChildren(context);
                for (int iCount = 0; iCount < busTypeList.size(); iCount++) {
                    if (strType.equals((busTypeList.get(iCount)).toString())) {
                ...
                sKey = sKey.append(KEY_FIRST_PART).append(DOT).append(KEY_LAST_PART);
                ...
                if ((strNotAllowedStates.indexOf(strCurrentState)) != -1) {
                    ...
                    if (strFinalType.equals(TYPE_PRODUCT) || strFinalType.equals(TYPE_MODEL) || strFinalType.equals(TYPE_PRODUCTLINE)) {
                        ...
                        if ((person.isAssigned(context, ProductLineConstants.ROLE_SYSTEM_ENGINEER)) || (person.isAssigned(context, ProductLineConstants.ROLE_PRODUCT_MANAGER))) {
                            bDisplay = Boolean.valueOf(true);
                        }
                    }

                    if (bDisplay.booleanValue() == false
                            && (strFinalType.equals(ConfigurationConstants.TYPE_CONFIGURATION_FEATURES) || strFinalType.equals(ConfigurationConstants.TYPE_LOGICAL_STRUCTURES))) {
                        bDisplay = Boolean.valueOf(true);
                    } else if (strType.equals(ProductLineConstants.TYPE_FIXED_RESOURCE)) {
                        bDisplay = Boolean.valueOf(true);
    ...
    /**
     * This method is called by an action trigger on Revise of 'Product' to connect the new revision of the product to the Product Compatibility Rule if the compatibility option is "upward" or "all".
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     */
    public void connectProductOnRevise(Context context, String args[]) throws Exception {
        ...
        String newPrdPhyID = new DomainObject(boNewRevision).getInfo(context, ConfigurationConstants.SELECT_PHYSICAL_ID);
        ...
        sbBuffer = sbBuffer.append(STR_ATTRIBUTE).append(OPEN_BRACE).append(ProductLineConstants.ATTRIBUTE_LEFT_EXPRESSION).append(CLOSE_BRACE);
        String strLeftExpkey = sbBuffer.toString();
        lstObjectSelects.add(strLeftExpkey);

        sbBuffer.delete(0, sbBuffer.length());
        sbBuffer = sbBuffer.append(STR_ATTRIBUTE).append(OPEN_BRACE).append(ProductLineConstants.ATTRIBUTE_RIGHT_EXPRESSION).append(CLOSE_BRACE);
        String strRightExpkey = sbBuffer.toString();
        lstObjectSelects.add(strRightExpkey);

        sbBuffer.delete(0, sbBuffer.length());
        sbBuffer = sbBuffer.append(STR_ATTRIBUTE).append(OPEN_BRACE).append(ProductLineConstants.ATTRIBUTE_COMPATIBILITY_OPTION).append(CLOSE_BRACE);
        String strCompOptionkey = sbBuffer.toString();

        sbBuffer.delete(0, sbBuffer.length());
        sbBuffer = sbBuffer.append(STR_ATTRIBUTE).append(OPEN_BRACE).append(ProductLineConstants.ATTRIBUTE_COMPARISION_OPERATOR).append(CLOSE_BRACE);
        ...
        MapList lstPCRuleList = contextObj.getObjectStructure(context, strTypePatten, strRelPattern, lstObjectSelects, lstRelationshipSelects, true, false, 1, 0, DomainObject.EMPTY_STRING,
        ...
        for (int i = 0; i < lstPCRuleList.size(); i++) {
            ...
            if ((strCompOption.equals(ProductLineConstants.RANGE_VALUE_UPWARD)) || (strCompOption.equals(ProductLineConstants.RANGE_VALUE_ALL))) {
                String strRuleID = (String) mapRuleMap.get(DomainConstants.SELECT_ID);
                Product boBean = new Product(strProductId);
                Map relAttributeMap = new HashMap();
                Map objAttributeMap = new HashMap();
                objAttributeMap.put("Comparison Operator", strCompOperatorValue);
                objAttributeMap.put("Include Other Feature Revisions", ConfigurationConstants.RANGE_VALUE_YES);
                // Left expression should be unchanged
                String strLeftExpression = (String) mapRuleMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "]");
                String strRightExpression = (String) mapRuleMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]");
                ...
                boBean.editProductCompatibilityRule(context, (String) mapRuleMap.get(DomainConstants.SELECT_ID), (String) mapRuleMap.get(DomainConstants.SELECT_TYPE),
                        (String) mapRuleMap.get(DomainConstants.SELECT_NAME), (String) mapRuleMap.get(DomainConstants.SELECT_REVISION), (String) mapRuleMap.get(DomainConstants.SELECT_POLICY),
                        (String) mapRuleMap.get(DomainConstants.SELECT_VAULT), (String) mapRuleMap.get(DomainConstants.SELECT_OWNER), (String) mapRuleMap.get(DomainConstants.SELECT_DESCRIPTION),
    ...
    public void copyRuleStructure(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            String relationshipPattern = sb.append(strProductBCRReln).append(strComma).append(strProductRuleExtnReln).append(strComma).append(strProductResourceReln).append(strComma)
                    .append(strRuleMarketingPreferenceReln).toString();

            sb = new StringBuffer(250);
            String typePattern = sb.append(strBooleanCompatibilityRuleType).append(strComma).append(strRuleExtensionType).append(strComma).append(strFixedResourceType).append(strComma)
            ...
            MapList objFeatureList = dom.getRelatedObjects(context, relationshipPattern, typePattern, false, true, (short) 1, objectSelects, relationshipSelectsForRule, DomainConstants.EMPTY_STRING,
                    DomainConstants.EMPTY_STRING, (short) 0, null, null, null);

            MapList objFeatureListDestination = destinationDO.getRelatedObjects(context, relationshipPattern, typePattern, false, true, (short) 1, objectSelects, relationshipSelectsForRule,
            ...
            for (int i = 0; i < objFeatureListDestination.size(); i++) {
                hmObjectId.put((String) ((Map) objFeatureListDestination.get(i)).get(ConfigurationConstants.SELECT_ID),
                        (String) ((Map) objFeatureListDestination.get(i)).get(DomainConstants.SELECT_RELATIONSHIP_TYPE));
            }
            ...
            while (itr.hasNext()) {
                ...
                StringList slAttrRE = (StringList) relDataTable
                ...
                for (int j = 0; j < ruleType.size(); j++) {
                    if (ruleType.get(j).equals(ConfigurationConstants.TYPE_INCLUSION_RULE) && slAttrRE != null && slAttrRE.size() != 0) {
                        ...
                        boCompRule.createAndConnectComplexInclusionRule(context, ruleType.get(0).toString(), strAutoName, strRevision, strPolicy, strVault, strOwner, strDescription, expMap,
                                objAttributeMap, relAttributeMap);
                    }
                }
            }
    ...
    public Vector showInclusionRuleLinkInRuleTypeColumnForProductContext(Context context, String[] args) throws Exception

    {
        HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
        List lstobjectList = (MapList) parametersMap.get("objectList");
        Vector vInclusionRuleLink = showInclusionRuleLinkForProductContext(context, args);
        ...
        for (int j = 0; j < lstobjectList.size(); j++) {
            ...
            if (strRelType != null && !strRelType.trim().isEmpty()) {
                vtIncRuleLink.add(vInclusionRuleLink.get(j));
            } else {
                vtIncRuleLink.add("");
    ...
    public Vector showInclusionRuleLinkForProductContext(Context context, String[] args) throws Exception {
        String strLevel = DomainConstants.EMPTY_STRING;
        String strObjectId;
        String strRelId;
        Vector vInclusionRuleLink = new Vector();
        ...
        String strRuleTypeDisplayInclusion = i18nnow.GetString("emxFrameworkStringResource", strLanguage, "emxFramework.Range.Rule_Type.Inclusion");
        String strRuleTypeDisplayExclusion = i18nnow.GetString("emxFrameworkStringResource", strLanguage, "emxFramework.Range.Rule_Type.Exclusion");
        for (int i = 0; i < lstobjectList.size(); i++) {
            objectMap = (Map) lstobjectList.get(i);
            String strIncRuleID = "";
            ...
            if (strIncRuleID == null || (strIncRuleID != null && strIncRuleID.trim().isEmpty())) {
                sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?rowLevel=" + strLevel
                        + "%26modetype=create%26commandName=FTRInclusionRuleSettings%26ruleType=InclusionRule%26SuiteDirectory=configuration");
                sIncLink.append("%26submitURL=");
                sIncLink.append("../configuration/InclusionRuleEditPostProcess.jsp?mode=create%26ruleType=InclusionRule");
                ...
                sIncLink = sIncLink.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?rowLevel=" + strLevel
                        + "%26modetype=edit%26commandName=FTRInclusionRuleSettings%26ruleType=InclusionRule%26SuiteDirectory=configuration");
                sIncLink.append("%26submitURL=");
                sIncLink.append("../configuration/InclusionRuleEditPostProcess.jsp?mode=edit%26ruleType=InclusionRule");
            ...
            strRelId = (String) objectMap.get("id[connection]");
            if (strRelId != null && !("".equals(strRelId)) && !("null".equals(strRelId))) {
                if (objectMap.containsKey(ConfigurationConstants.SELECT_ATTRIBUTE_RULE_TYPE)
                        && ProductLineCommon.isNotNull((String) objectMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_RULE_TYPE))) {
                    strRuleType = (String) objectMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_RULE_TYPE);
                } else if (objectMap.containsKey(DomainRelationship.SELECT_ID) && ProductLineCommon.isNotNull((String) objectMap.get(DomainRelationship.SELECT_ID))) {
                    DomainRelationship domRel = new DomainRelationship((String) objectMap.get(DomainRelationship.SELECT_ID));
            ...
            sb = sb.append(XSSUtil.encodeForHTMLAttribute(context, strObjectId)).append("&amp;relId=").append(XSSUtil.encodeForHTMLAttribute(context, strRelId)).append("&amp;ruleId=")
                    .append(XSSUtil.encodeForHTMLAttribute(context, strIncRuleID));
            if (strRuleType.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_INCLUSION)) {
            ...
            String strRuleTypeToolTip = strRuleTypeDisplay.toString();
            sb = sb.append(strEndPart1).append(strMidPart1).append("\"" + XSSUtil.encodeForHTMLAttribute(context, strRuleTypeToolTip) + "\"").append(strExpression.trim()).append(strMidPart2)
                    .append(XSSUtil.encodeForHTML(context, strRuleTypeDisplay)).append(strEndPart2);
