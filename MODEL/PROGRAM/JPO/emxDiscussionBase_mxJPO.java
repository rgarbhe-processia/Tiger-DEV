...
public class emxDiscussionBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @grade 0
     */
    public emxDiscussionBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public Object getDiscussionMembersList(Context context, String[] args) throws Exception {
        try {
    ...
    public Object getDiscussionList(Context context, String[] args) throws Exception {
        ...
        if (assignments.contains(roleEmployee)) {
            ...
            // for OCDX user to check for if the user have ctx::
            Iterator itr = assignments.iterator();
            while (itr.hasNext()) {
                String assignment = (String) itr.next();
                if (assignment.contains("ctx::")) {
                    hasEmpRole = true;
                    break;
                }
            }
        ...
        if (FrameworkUtil.isObjectId(context, objectId)) {
            ...
            // added for the Bug 339013
            String objectWhere = "((policy ==" + "\"" + DomainConstants.POLICY_MESSAGE + "\")||((policy ==" + "\"" + strPolicy + "\") && (" + Boolean.valueOf(hasEmpRole) + " || (owner ==" + "\""
                    + strOwner + "\"))))";
    ...
    protected void revokeAccessGrantorGrantee(matrix.db.Context context, String sGrantor, String sGrantee) throws Exception {
        String sCommand = "mod bus " + getId() + " revoke grantor '" + sGrantor + "' grantee '" + sGrantee + "'";
        MqlUtil.mqlCommand(context, sCommand);
    }
    ...
    /**
     * getDiscussionAttachmentsList - gets the list of Attachments for the Discussion.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - objectList MapList
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getDiscussionAttachmentsList(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            attachmentList = discussionObj.getRelatedObjects(context, relPattern.getPattern(), "*", typeSelects, relSelects, false, true, (short) 2, null, null, null, includeRelPattern, null);
            attachmentList.sort("level", "ascending", "integer");
    ...
    protected Object getFilteredDiscussionList(Context context, String whereExpression) throws Exception {
        ...
        try {
            ...
            mqlString.append("temp query bus '");
            mqlString.append(TYPE_MESSAGE);
            mqlString.append("' * *  vault '");
            mqlString.append(company.getAllVaults(context, true));
            mqlString.append("' where \"");
            mqlString.append(whereExpression);
            mqlString.append("\" select id  policy dump |");

            BufferedReader in = new BufferedReader(new StringReader(MqlUtil.mqlCommand(context, mqlString.toString())));
            String line;
    ...
    public Vector getSubscriptionIcon(Context context, String[] args) throws Exception {
        ...
        String selSubPersonName = new StringBuffer().append("from[").append(relPublishSubscribe).append("].to.from[").append(relPublish).append("].to.from[").append(relSubscribedPerson)
                .append("].to.name").toString();

        String selRootObjId = new StringBuffer().append("to[").append(PropertyUtil.getSchemaProperty(context, "relationship_Message")).append("].from.to[")
                .append(PropertyUtil.getSchemaProperty(context, "relationship_Thread")).append("].from.id").toString();
        ...
        try {
            ...
            for (int j = 0; j < bosWithSelect.size(); j++) {
                ...
                if (subscriptionStatus != null && subscriptionStatus.contains(contextUser)) {
                    ...
                    BusinessObjectWithSelectList eventInfo = BusinessObject.getSelectBusinessObjectData(context, (String[]) eventIds.toArray(new String[] {}), eventSelList);
    ...
    public Object getSubscribedDiscussionList(Context context, String[] args) throws Exception {
        ...
        StringBuffer mqlString = new StringBuffer();
        mqlString.append("expand bus Person \"");
        mqlString.append(context.getUser());
        mqlString.append("\" - to rel \"");
        mqlString.append(RELATIONSHIP_SUBSCRIBED_PERSON);
        mqlString.append("\" type \"");
        mqlString.append(TYPE_EVENT);
        mqlString.append("\" select bus id where \"attribute[");
        mqlString.append(ATTRIBUTE_EVENT_TYPE);
        mqlString.append("] == 'New Reply' || attribute[");
        mqlString.append(ATTRIBUTE_EVENT_TYPE);
        mqlString.append("] == 'New Discussion'\" dump |");
        String strOutput = MqlUtil.mqlCommand(context, mqlString.toString());
        ...
        for (int j = 0; j < bosWithSelect.size(); j++) {
            ...
            if (TYPE_MESSAGE.equals(type)) {
                ...
                MapList discussions = bs.getRelatedObjects(context, RELATIONSHIP_THREAD + "," + RELATIONSHIP_MESSAGE, TYPE_MESSAGE, selects, null, false, true, (short) -1, null, null, 0, null, null,
                        null);
    ...
    public Object getLast30DaysDiscussions(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String selRootObjId = new StringBuffer().append("to[").append(PropertyUtil.getSchemaProperty(context, "relationship_Message")).append("].from.to[")
                    .append(PropertyUtil.getSchemaProperty(context, "relationship_Thread")).append("]").toString();
    ...
    public static Vector getMessageType(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            for (Iterator iter = objectList.iterator(); iter.hasNext();) {
                ...
                policy = policy == null ? MqlUtil.mqlCommand(context, "print bus " + discusion.get(SELECT_ID) + " select policy dump") : policy;
                vMessageTypeVec.addElement(EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(lang),
                        POLICY_MESSAGE.equals(policy) ? "emxComponents.Discussion.Public" : "emxComponents.Discussion.Private"));
            }
        } catch (Exception ex) {
            throw ex;
        }
    ...
    public static Vector getLatestMessageOriginatedDate(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (objItr.hasNext()) {
                ...
                leafNodeList = messageObject.getRelatedObjects(context, DomainConstants.RELATIONSHIP_REPLY, DomainConstants.TYPE_MESSAGE, objectSelects, null, false, true, (short) 0, null, null,
                        (short) 0);
    ...
    public Vector getReplyCountForDiscussion(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            while (objItr.hasNext()) {
                objMap = (Map) objItr.next();
                objId = (String) objMap.get(SELECT_ID);
                mqlString = "eval expr 'count TRUE' on expand bus " + objId + " from rel " + PropertyUtil.getSchemaProperty(context, SYMBOLIC_relationship_Reply) + " recurse to all";
                count = MqlUtil.mqlCommand(context, mqlString);
    ...
    // ///////////////// My Discussions /////////////////////////////////////////////////////////////////
    /**
     * isPrivateUser - returns true if user has the Autherity to access Private Message else false.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - objectList MapList/ Object Id of parent
    ...
    private boolean hasAccessToPrivateDiscussion(Context context, String objectId) throws Exception, FrameworkException {
        ...
        person = (com.matrixone.apps.common.Person) newInstance(context, person.getPerson(context).getId(context));
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - ObjectId of the Created Reply
     * @return int
     * @throws Exception
     *             if the operation fails
     * @since AEF 11-0
    ...
    public static int updateRootMessageObject(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            String mqlString = "eval expr 'count TRUE' on expand bus " + rootMessageObjectId + " from rel " + PropertyUtil.getSchemaProperty(context, SYMBOLIC_relationship_Reply) + " recurse to all";
            String strCount = MqlUtil.mqlCommand(context, mqlString);
    ...
    public String getDiscussionVisibility(Context context, String[] args) throws Exception {
        ...
        discType = discType.contains("Public") ? EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(languageStr), "emxComponents.Discussion.Public")
                : discType.contains("Private") ? EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(languageStr), "emxComponents.Discussion.Private") : discType;
    ...
    public void setDiscussionSubject(Context context, String[] args) throws Exception {
        ...
        message.setId(objectId);
        /*
         * Temporarily fixed as part of IR-185089V6R2013x Whole function is to be removed but since we are using edit form update function is a must. Public and Private discussions creation to be
         * migrated to create component to avoid the usage of update function and program. As this existing Update function is also a dummy function
         */
    }
    ...
    public Map createDiscussionPostProcess(Context context, String[] args) throws Exception {
        ...
        String strDiscussionObjectId = message.getId(context);
        String event = "New Reply";
        boolean globalSubscription = false;
        boolean isRecursive = true;
        SubscriptionUtil.createSubscription(context, globalSubscription, SubscriptionUtil.ADMIN_BUSINESS_TYPE, strDiscussionObjectId, event, isRecursive, SubscriptionUtil.BOTH);
        retMap.put("id", message.getId());
        return retMap;

    }

    public boolean isProtectedCollabspace(Context context, String[] args) throws Exception {
        String objectId = this.getId();
        DomainObject object = DomainObject.newInstance(context, objectId);
        StringList selectList = new StringList(4);
        selectList.add("to[Message].from.to[Thread].from.project.property[vplm_readAbility].value");
        selectList.add("to[Message].from.to[Thread].from.current");
        selectList.add("to[Message].from.to[Thread].from.project");
        selectList.add("to[Message].from.to[Thread].from.type");
        Map objectInfo = object.getInfo(context, selectList);
        String projectReadAbility = (String) objectInfo.get("to[Message].from.to[Thread].from.project.property[vplm_readAbility].value");
        String project = (String) objectInfo.get("to[Message].from.to[Thread].from.project");
        String objectState = (String) objectInfo.get("to[Message].from.to[Thread].from.current");
        String type = (String) objectInfo.get("to[Message].from.to[Thread].from.type");
        String strdefPrj = PersonUtil.getDefaultProject(context, context.getUser());
        if ("Public".equals(projectReadAbility)) {
            return true;
        } else if ("Private".equals(projectReadAbility) && project.equals(strdefPrj)) {
            return true;
        } else if ("Protected".equals(projectReadAbility) && project.equals(strdefPrj) && Message.isObjectNotReleased(type, objectState)) {
            return true;
        } else if ("Protected".equals(projectReadAbility) && Message.isObjectReleased(type, objectState)) {
            return true;
        } else {
            return false;
        }
