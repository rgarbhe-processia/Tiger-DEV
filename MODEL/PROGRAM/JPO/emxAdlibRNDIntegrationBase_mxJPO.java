 ...
 *
 * FileName : "$RCSfile: ${CLASSNAME}.java.rca $" Author : "$Author: przemek $" Version : "$Revision: 1.12 $" Date : "$Date: Wed Oct 22 15:53:03 2008 $"
 *
 */
import java.io.File;
...
public class emxAdlibRNDIntegrationBase_mxJPO extends emxAbsPDFIntegration_mxJPO {
     ...
     * @param queue
     *            Queue of documents to be rendered
     * @since AEF 9.5.4.0
     */
    public emxAdlibRNDIntegrationBase_mxJPO(Context context, emxPDFQueue_mxJPO queue) {
        /*
         * Author : DJ Date : 02/04/2003 Notes : History :
         */
        super(context, queue);
    }

    /**
     * Main processing method for the emxAdlibRNDIntegrationBase object This method checks out all the files to the input folder, writes out the adlib control .dpi file, and then polls the output and
     * error folders until something appears or poll runs out of time. As the files are appear, it will check them in to the PDF format, and if the config file states for it to happen, they will be
     * promoted.
     * @since AEF 9.5.4.0
     */
    public void run() {
         ...
         */
        // Get config
        String inputFolder = _util.getString(_context, "eServiceBatchPrintPDF.PDF.InputFolderPath");
        String outputFolder = _util.getString(_context, "eServiceBatchPrintPDF.PDF.OutputFolderPath");
        String renderingState = _util.getString(_context, "eServiceBatchPrintPDF.PDF.RenderingState");
        String promote = _util.getString(_context, "eServiceBatchPrintPDF.PDF.PromoteOnRendering");
        try {
            ...
            if (!hasPDF) {
                String errorMessage = _util.getString(_context, "emxBatchPrintPDF.PDF.NeedsPDFFormat", "emxDocumentCentralStringResource");
            ...
            while (!_queue.isEmpty()) {
                ...
                FileWriter fileID = new FileWriter(inputJobFolder + java.io.File.separator + dpiFileName + ".dpi", true);
                ...
                String printHeader = _util.getString(_context, "eServiceBatchPrintPDF.Batch.PrintHeader");
            ...
            if (promote.equalsIgnoreCase("True") && currentState != null && currentState.equalsIgnoreCase(renderingState)) {
                try {
                    bo.promote(_context);
                } catch (Exception e) {
            ...
            String errorMessage = _util.getString(_context, "emxBatchPrintPDF.FileWriteFailure", "emxDocumentCentralStringResource");
     ...
     * @param bo
     *            Business Object that has been rendered
     * @param jobFolder
     *            name of the job folder
     * @param dpiFileName
     *            name of the dpi file
     * @exception Exception
     *                Adlib Failed
     * @since AEF 9.5.4.0
     */
    protected void pollJob(BusinessObject bo, String jobFolder, String dpiFileName) throws Exception {
        /*
         * Author : DJ Date : 02/04/2003 Notes : History :
         */
        // Get config info
        String inputFolder = _util.getString(_context, "eServiceBatchPrintPDF.PDF.InputFolderPath");
        String outputFolder = _util.getString(_context, "eServiceBatchPrintPDF.PDF.OutputFolderPath");
        String errorFolder = _util.getString(_context, "eServiceBatchPrintPDF.PDF.ErrorFolderPath");
        String sleepInterval = _util.getString(_context, "eServiceBatchPrintPDF.PDF.SleepInterval");
        String timeoutInterval = _util.getString(_context, "eServiceBatchPrintPDF.PDF.TimeoutIntervals");
        try {
            // various things needed for the polling
            int sleepInt = parseInt(sleepInterval);
            int timeoutInt = parseInt(timeoutInterval);
            String jobFile = jobFolder + java.io.File.separator + dpiFileName + ".pdf";
            ...
            String fileDescription = _util.getString(_context, "emxBatchPrintPDF.PDF.RenderedFileDescription", "emxDocumentCentralStringResource");
            ...
            if (outputFile.exists()) {
                ...
                // Send an Icon Mail
                String errorMessage = _util.getString(_context, "emxBatchPrintPDF.PDF.RenderingOutputErrorMessage", "emxDocumentCentralStringResource") + "\n" + errorFolder + jobFolder
                        + java.io.File.separator + dpiFileName + ".dpi ";
                sendErrorMessage(_queue.getObjectId(), errorMessage, outputFolder + jobFolder);
                throw new Exception(errorMessage);
            } else {
                // Write out to the log file & send a mail
                String errorMessage = _util.getString(_context, "emxBatchPrintPDF.PDF.RenderingTimedOutMessage", "emxDocumentCentralStringResource") + dpiFileName;
                sendErrorMessage(_queue.getObjectId(), errorMessage, outputFolder + jobFolder);
            }
        } catch (Exception e) {
            String errorMessage = _util.getString(_context, "emxBatchPrintPDF.PollingFailure", "emxDocumentCentralStringResource");
     ...
     *            Description for the file Object to be checked in
     * @param vault
     *            Name of the vault
     * @param folder
     *            folder name
    ...
    protected void checkinRenderedFile(Context context, String objectId, String fileName, String fileDescription, String vault, String folder) throws Exception {
        try {
            String masterObjectId = "";
            ...
            MapList mlist = masterObject.getRelatedObjects(context, CommonDocument.RELATIONSHIP_ACTIVE_VERSION, CommonDocument.TYPE_DOCUMENTS, selectList, null, false, true, (short) 1, objectWhere,
     ...
     *            name of the file
     * @exception Exception
     *                version updation failed
     * @since AEF 9.5.4.0
     */

    protected void updateVersionDocument(Context context, String objectId, String fileName) throws Exception {
        try {
            String checkInReason = _util.getString(_context, "emxBatchPrintPDF.PDF.RenderedFileDescription", "emxDocumentCentralStringResource");
    ...
    /**
     * Mails an error message to the person who rendered, and to the person who is set to recieve Error Notifications
     * @param objectId
     *            Object's Id
     * @param errorMessage
     *            The message to send
     * @param outputFolder
     *            Folder where the files are/were
     * @since AEF 9.5.4.0
     * @grade 0
     */
    protected void sendErrorMessage(String objectId, String errorMessage, String outputFolder) {
        /*
         * Author : DJ Date : 02/04/2003 Notes : History :
         */
        try {
            // Get the user to send notifications to
            String errorNotify = _util.getString(_context, "eServiceBatchPrintPDF.PDF.ErrorNotify");
            ...
            // Build Subject Line
            sbSubject.append(_util.getString(_context, "emxBatchPrintPDF.PDF.RenderingFailed1", "emxDocumentCentralStringResource"));
            sbSubject.append(bo.getName() + ", ");
            sbSubject.append(_util.getString(_context, "emxBatchPrintPDF.Rev", "emxDocumentCentralStringResource"));
            sbSubject.append(": " + bo.getRevision());

            // Build Message Body
            sbMessage.append(_util.getString(_context, "emxBatchPrintPDF.PDF.RenderingFailed1", "emxDocumentCentralStringResource"));
            sbMessage.append("\n\n");
            sbMessage.append(_util.getString(_context, "emxBatchPrintPDF.Type", "emxDocumentCentralStringResource"));
            sbMessage.append(": " + bo.getTypeName());
            sbMessage.append("\n");
            sbMessage.append(_util.getString(_context, "emxBatchPrintPDF.Name", "emxDocumentCentralStringResource"));
            sbMessage.append(": " + bo.getName());
            sbMessage.append("\n");
            sbMessage.append(_util.getString(_context, "emxBatchPrintPDF.Rev", "emxDocumentCentralStringResource"));
            ...
            if (!outputFolder.equals("")) {
                sbMessage.append(_util.getString(_context, "emxBatchPrintPDF.PDF.RenderingFailed3", "emxDocumentCentralStringResource"));
                sbMessage.append("\n");
                int idx = outputFolder.lastIndexOf("/");
                sbMessage.append(outputFolder.substring(idx + 1, outputFolder.length()));
            }

            // Send the mail
            _util.sendMail(_context, userList, null, null, sbSubject.toString(), sbMessage.toString());
