...
/*
 * emxWorkspaceConstantsBase.java
 *
...
public class emxWorkspaceConstantsBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @grade 0
     */
    public emxWorkspaceConstantsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    /**
     * Revokes access on all childern of a object when disconnected/removed from the Workspace data structure
     * @param context
     *            the eMatrix Context object
     * @param empty
     *            String array - Mandatory parameter for JPO.
     * @return void
     * @throws Exception
     *             if the operation fails
     * @since TC V3
    ...
    public void revokeAccess(matrix.db.Context context, String[] args) throws Exception {
        ...
        if (!hasMultipleReferences(context)) {
            ...
            try {
                ContextUtil.pushContext(context);
                mapList = dObj.getRelatedObjects(context, RELATIONSHIP_SUB_VAULTS + "," + RELATIONSHIP_VAULTED_OBJECTS + "," + relVaultedObjectsRev2, QUERY_WILDCARD, selects, null, false, true,
                        (short) 0, null, null);
            } catch (Exception ex) {
                ex.printStackTrace();
            } finally {
    ...
    /**
     * Inherit access from Parent when ever object added to Workspace data structure.
     * @param context
     *            the eMatrix Context object
     * @param Id
     *            of object added to Workspace data structure
     * @return void
     * @throws Exception
     *             if the operation fails
     * @since TC V3
    ...
    public void inheritAccess(matrix.db.Context context, String[] args) throws Exception {
        Access access = null;
        // Construct BO list with the new connect object to grant acccess.
        BusinessObjectList busList = new BusinessObjectList();
        DomainObject contentObject = new DomainObject(args[0]);
        busList.addElement(contentObject);
        String contentType = contentObject.getInfo(context, SELECT_TYPE);
        ...
        if (!"All".equalsIgnoreCase(inheritAccessForTypes) && !"".equals(inheritAccessForTypes)) {
            ...
            if (_documentTypes.indexOf(contentType) == -1 && !typeProjectVault.equals(contentType)) {
                return;
            }
        }

        String accessType = contentObject.getInfo(context, WorkspaceVault.SELECT_ACCESS_TYPE);
        ...
        try {
            ...
            selects.add(SELECT_TYPE);
            MapList mapList = contentObject.getRelatedObjects(context, RELATIONSHIP_SUB_VAULTS + "," + RELATIONSHIP_VAULTED_OBJECTS + "," + relVaultedObjectsRev2, QUERY_WILDCARD, selects, null, false,
            ...
            if (mapList != null) {
                ...
                while (itr.hasNext()) {
                    ...
                    String type = FrameworkUtil.getAliasForAdmin(context, "type", (String) m.get(SELECT_TYPE), true);
                    DomainObject otherObject = new DomainObject(id);
                    if (inheritTypes.contains(type) || otherObject.isKindOf(context, CommonDocument.TYPE_DOCUMENTS)) {
                        busList.addElement(otherObject);
                    }
                }
            }
        ...
        // INCIDENT 352727 - ONLY GRANT ACCESS ON THE CURRENT REVISION
        // busList = contentObject.getRevisions(context);
        // busList.addElement(contentObject);
        // Get the Grantee access list granted by 'Workspace Access Grantor' to the parent object
        // and grant the each access to the added child.
        AccessList accessGrantorList = getAccessForGrantor(context, AEF_WORKSPACE_ACCESS_GRANTOR_USERNAME);
        if (accessGrantorList.size() > 0) {
            pushContextForGrantor(context, AEF_WORKSPACE_ACCESS_GRANTOR_USERNAME);
            try {
                grantAccessRights(context, busList, accessGrantorList);
            } catch (Exception exp) {
                throw exp;
            } finally {
                ContextUtil.popContext(context);
        ...
        // Get the Grantee access list granted by Workspace Lead Grantor to the Parent objects
        // and grant the each access to the new added child
        AccessList accessLeadGrantorList = getAccessForGrantor(context, AEF_WORKSPACE_LEAD_GRANTOR_USERNAME);
        if (accessLeadGrantorList.size() > 0) {
            pushContextForGrantor(context, AEF_WORKSPACE_LEAD_GRANTOR_USERNAME);
            try {
                grantAccessRights(context, busList, accessLeadGrantorList);
            } catch (Exception exp) {
                throw exp;
            } finally {
                ContextUtil.popContext(context);
    ...
    public void updateCount(matrix.db.Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList parentFolders = this.getRelatedObjects(context, RELATIONSHIP_SUB_VAULTS, TYPE_WORKSPACE_VAULT, new StringList(new String[] { SELECT_ID, SELECT_ATTRIBUTE_COUNT }), null, true,
    ...
    public void inheritAccessToVaultedObjects(Context context, String[] args) throws Exception {
        new VaultedObjectsAccessUtil().updateVaultedObjectsAccess(context, new String[] { args[0] }, null, null, VaultedObjectsAccessUtil.ADD_RELATION);
    }
