...
public class emxUnresolvedPartBase_mxJPO extends emxPart_mxJPO {
    ...
    private void removePartFromUEBOM(Context context, String relId, String ecoId, String selectedpart, boolean sameECO, StringList cutRelIDList) throws Exception {
        try {
            ...
            if (sameECO) {
                ...
                /*
                 * Check for whether selected CHILD part and CORRESPONDING PARENT was added in multiple assemblies with same context change, if exists do not remove Affected Item relationship of those
                 * if not exists means if existed in only bom assembly then remove affected Item relationship of corresponding
                 */

                // expanding selected CHILD object in TO and FROM side and getting parent ,child list
                StringList parentObjListOfChild = getFromOrToIdsBasedOnIntermediateRel(context, ecoId, selectedpart, false, UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING,
                        UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE);
                StringList childObjListOfChild = getFromOrToIdsBasedOnIntermediateRel(context, ecoId, selectedpart, true, UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING,
                        UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE);

                parentObjListOfChild.addAll(childObjListOfChild);

                // expanding PARENT object in TO and FROM side and getting parent ,child list
                StringList parentObjListOfParent = getFromOrToIdsBasedOnIntermediateRel(context, ecoId, parentId, false, UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING,
                        UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE);
                StringList childObjListOfParent = getFromOrToIdsBasedOnIntermediateRel(context, ecoId, parentId, true, UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING,
                        UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE);
    ...
    /**
     * Returns StringList of expanded Objects FROM/TO side based on Intermediate Relationship A Add | |___B ECO-1 Here in above example, if we pass B as object id and ECO-1 to be checked on
     * intermediate rel and returns A object finally.
     * @param context
     *            the eMatrix code context object
     * @param ecoId
     *            the eco Id to be checked on other side of intermediate relationship.
     * @param objID
     *            the selected part object id
     * @param isFromExpand
     *            a boolean value which decides whether fromside expand Or tosideexpand.
     * @param expandRel
     *            expand relationship
     * @param intermediateRel
     *            Intemediate relationship from Rel to Object, here its EBOM Pending--EBOM Change--->ECO ID
     ...
     * @throws Exception
     */
    private void addAffectedItem(Context context, String selectedObjectId, String childPartId, String contextECOId) throws Exception {
        ...
        String aliasStateName = PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_CONFIGURED_PART, "state_Preliminary");
        ...
        if (aliasStateName.equals(childState) && "Unresolved".equalsIgnoreCase(sParentPolicyClass) && !affectedItems.contains(childPartId)) {
    ...
    public HashMap updateUnresolvedEBOM(Context context, String[] args) throws Exception {
        ...
        StringBuffer updateoXMLBuffer = new StringBuffer();
        String rowFormat = "";
        ...
        try {
            ...
            String[] objSizeArr = { "emxEngineeringCentral.Common.ASize", "emxEngineeringCentral.Common.BSize", "emxEngineeringCentral.Common.CSize", "emxEngineeringCentral.Common.DSize",
            ...
            while (objItr.hasNext()) {
                ...
                if (attrOperation != null) {
                    ...
                    if (isENGSMBInstalled) {
                        ...
                        String vpmControlState = MqlUtil.mqlCommand(context, mqlQuery, parentPartId, "from[" + RELATIONSHIP_PART_SPECIFICATION + "|to.type.kindof["
                    ...
                    // 2012x--Added condition check for to proceed with normal change process
                        ...
                        if (!sameECO) {
                            ...
                            returnMsgBuffer.append(" FreezePaneregister(\"").append(rowInfo).append("\"); rebuildView();")
                ...
                // 2012x-Starts-- Code to start for addExisting and remove operations for WIP BOM
                    ...
                    while (objectItr.hasNext()) {
                        ...
                        strExpression = (strExpression != null && !"null".equals(strExpression))
                                ? strExpression.indexOf(EffectivityFramework.KEYWORD_PHYSID_PREFIX) != -1 ? strExpression : existingRelIdActExpr
                                : "";
            ...
            if (!"".equals(updateoXMLBuffer.toString())) {
                ...
                returnMsgBuffer.append("var objDOM = emxUICore.createXMLDOM();")
                        ...
                        .append("emxUICore.checkDOMError(objDOM);updateoXML(objDOM);refreshStructureWithOutSort();").append("arrUndoRows = new Object();")
                        .append("postDataXML.loadXML(\"<mxRoot/>\");");
            } else {
            ...
            if (e.toString().indexOf("license") > -1) { // If any License Issue throw the exception to user.
                ...
                String recursionMesssage = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", Local, "emxEngineeringCentral.RecursionError.Message");
                retMap.put("Message", rowFormat + recursionMesssage);
            } else if ((e.toString().indexOf("recursion")) == -1 && ((e.toString().indexOf("Check trigger")) != -1)) {
                String tnrMesssage = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", Local, "emxEngineeringCentral.TNRError.Message");
    ...
    /**
     * Returns a MapList of the ECO ids when Modstack filter is selected for a given context.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
    ...
    /**
     * Returns a StringList of Prerequisite ECOs for a given context.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap containing the eco ids which are connected to a product
     * @return StringList.
     * @since EngineeringCentral X3
     * @throws Exception
     *             if the operation fails.
    ...
    public StringList getPrerequisiteECO(Context context, String args[]) throws Exception {
        ...
        if (objList != null && objList.size() > 0) {
            ...
            while (i.hasNext()) {
                ...
                MapList mapList = domainObjectECO.getRelatedObjects(context, relToExpand, // relationship pattern
    ...
    /**
     * The method that builds the return map for Table and Indented Table pre processing methods.
     * @param actionValue
     *            the calling method's Action string (Continue or Stop).
     * @param messageValue
     *            the calling method's Message string (normally built by a previous call to the tableBuildMessageString method).
     * @param objectList
     *            the MapList of objects (normally built by a previous call to the tableBuildObjectList method).
     * @return returns to the calling processing method a HashMap that contains the incoming actionValue, messageValue, and objectList.
     * @since XBOM
     */
    ...
    /**
     * This method will check whether the context product has been selected or not. If user tries to edit any information with out selecting context product then it will alert the user to select
     * context product
     * @param args
     *            - a packed HashMap containing toolbar related information.
     * @return returns to the calling processing method a MapList that contains the 'modified' object list.
     * @since XBOM
     */
     ...
     *            the eMatrix <code> Context </code> object
     * @param args
     *            holds objectId as first argument
        ...
        try {
            ...
            if (UIUtil.isNotNullAndNotEmpty(sPCFilterId)) {
                ...
                if (UIUtil.isNotNullAndNotEmpty(sPCFilterExpr)) {
                    filterActualExpression = UIUtil.isNullOrEmpty(filterActualExpression) ? sPCFilterExpr : "(" + sPCFilterExpr + ") OR (" + filterActualExpression + ")";
                }
            }

            // pass " " to CFF for empty expressions
            if (UIUtil.isNullOrEmpty(filterActualExpression)) {
                filterActualExpression = " ";
            }
            ...
            tempList = partObj.getRelatedObjects(context, sRelName, sTypeName, selectStmts, selectRelStmts, false, true, (short) nExpandLevel, null, sWhere, (short) 0, DomainObject.CHECK_HIDDEN,
                    DomainObject.PREVENT_DUPLICATES, (short) DomainObject.PAGE_SIZE, null, null, null, (String) null, filterExpression, DomainObject.FILTER_STRUCTURE);
     ...
     *            ematrix context
     * @param ecoIdList
     *            contains ecoIds selected by user to filter.
    ...
    private StringList getPreRequisiteECOIds(Context context, StringList ecoIdList) throws Exception {
        ...
        for (int i = 0, size = ecoIdList.size(); i < size; i++) {
            ...
            prerequisitesList = doECO.getRelatedObjects(context, UnresolvedEBOMConstants.RELATIONSHIP_PREREQUISITE, UnresolvedEBOMConstants.TYPE_PUE_ECO, new StringList(SELECT_ID), null, false, true,
                    (short) 0, null, null);
    ...
    public Map reformatMap(Map map, String applicability, String effectivity, String add, String remove, String operation) {
        ...
        /*
         * // Do not allow edit for removed columns or resolved bom if (!"".equals(remove) || ("".equals(add)) && "".equals(remove)) { map.put("RowEditable", "readonly"); }else{ map.put("RowEditable",
         * "show"); }
         */
        return map;
    }
     ...
     * @return boolean
     */
    public boolean checkForTopLevelPart(Context context, String objectID) throws Exception {
        boolean chkTopLevelPart = false;
        String parentObjectId = objectID;
        DomainObject domObj = new DomainObject(parentObjectId);
        MapList mapList = domObj.getRelatedObjects(context, UnresolvedEBOMConstants.RELATIONSHIP_ASSIGNED_PART, UnresolvedEBOMConstants.TYPE_PRODUCTS, null, null, true, false, (short) 1, null, null);
    ...
    /**
     * Returns a StringList of the parent object ids and select Object Id which are connected using UEBOM Relationship for a given context.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap containing objectId of object
     ...
     *            the eMatrix <code>Context</code> object
     * @param strObjId
     *            : objectId to check for Reserved
    ...
    public boolean sendMailForBGProcess(Context context, String objectId, boolean success) throws Exception {
        ...
        // Send notification
        MailUtil.sendMessage(context, toList, // toList
                null, // ccList
                null, // bccList
                strSubject, // subject
                strBody, // message
    ...
    /**
     * Returns a MapList of the ECO ids when ECO filter is selected for a given context.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap containing objectId of object
    ...
    public boolean checkForBOMMode(Context context, String args[]) throws Exception {
        return true;
    }
    ...
    public StringList includeUnresolvedParts(Context context, String[] args) throws Exception {
        ...
        String STATE_CONPART_SUPERSEDED = PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_CONFIGURED_PART, "state_Superseded");
        String sVault = context.getVault().toString();
        StringBuffer sbWhere = new StringBuffer(128);
        sbWhere.append('(').append("policy == \"").append(UnresolvedEBOMConstants.POLICY_CONFIGURED_PART).append("\" && ").append(DomainConstants.SELECT_REVISION).append(" == \"").append("last\"")
                .append(" && relationship[").append(UnresolvedEBOMConstants.RELATIONSHIP_ASSIGNED_PART).append("] == \"False\" && ").append("to[")
                .append(UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING).append("] == \"False\" && ").append("to[").append(UnresolvedEBOMConstants.RELATIONSHIP_EBOM).append("] == \"False\" && ")
                .append(UnresolvedEBOMConstants.SELECT_CURRENT).append(" != \"").append(STATE_CONPART_SUPERSEDED).append("\")");
        MapList mlUnresolvedParts = DomainObject.findObjects(context, UnresolvedEBOMConstants.TYPE_PART, sVault, sbWhere.toString(), slBusSelectList);
     ...
     * @param val
     *            Object where values gets retrieved from
     * @param colBuffer
     *            prepared StringBuffer for a particular column
     */
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed hashmap with the following arguments paramMap HashMap of parameter values - fieldMap requestMap HashMap of request values - objectId, mode, form
     * @throws Exception
     *             if the operation fails
     * @since CFF R210 for ECC Reports
     */
    public String getEffectivityExpressionDisplayBOM1(Context context, String[] args) throws Exception

    {
        ...
        String editEffectivityURL = "../effectivity/EffectivityDefinitionDialog.jsp?modetype=filter&invockedFrom=fromForm&formName=" + formName + "&parentOID=&"
                + "fieldNameEffExprDisplay=EffectivityExpression&" + "fieldNameEffExprActual=EffectivityExpressionActual&" + "fieldNameEffExprActualList=EffectivityExpressionOIDList&"
                + "fieldNameEffExprActualListAc=EffectivityExpressionOIDListAc&" + "fieldNameEffExprOID=EffectivityExpressionOID";
        ...
        if (mode != null && mode.equalsIgnoreCase("edit")) {
            ...
            sb.append(
                    "&nbsp<a href=\"javascript:basicClear('EffectivityExpression');basicClear('EffectivityExpressionActual');basicClear('EffectivityExpressionOIDList');basicClear('EffectivityExpressionOIDListAc');basicClear('EffectivityExpressionOID') \">")
                    .append(EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.Common.Clear", context.getSession().getLanguage())).append("</a>");
            sb.append("</div>");
     ...
     * @since CFF R210 for ECC Reports
     */
    public String getEffectivityExpressionDisplayBOM2(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        ...
        String editEffectivityURL2 = "../effectivity/EffectivityDefinitionDialog.jsp?modetype=filter&invockedFrom=fromForm&formName=" + formName + "&parentOID=&"
                + "fieldNameEffExprDisplay=EffectivityExpression1&" + "fieldNameEffExprActual=EffectivityExpressionActual1&" + "fieldNameEffExprActualList=EffectivityExpressionOIDList1&"
                + "fieldNameEffExprActualListAc=EffectivityExpressionOIDListAc1&" + "fieldNameEffExprOID=EffectivityExpressionOID1";
        ...
        if (mode != null && mode.equalsIgnoreCase("edit")) {
            ...
            sb.append("<img src=\"../common/images/iconActionEdit.gif\"  border=\"0\"/></a>");
            sb.append(
                    "&nbsp<a href=\"javascript:basicClear('EffectivityExpression1');basicClear('EffectivityExpressionActual1');basicClear('EffectivityExpressionOIDList1');basicClear('EffectivityExpressionOIDListAc1');basicClear('EffectivityExpressionOID1') \">")
                    .append(EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.Common.Clear", context.getSession().getLanguage())).append("</a>");
            sb.append("</div>");
            ...
            sb.append("\" />");
            /*
             * if(UIUtil.isNullOrEmpty(objectId)) { Map effType = null; MapList mlEffectivityTypes = EFF.getEffectivityTypeData(context, effTypes2); if (!mlEffectivityTypes.isEmpty()) { for (int i=0;
             * i < mlEffectivityTypes.size(); i++) { effType = (Map)mlEffectivityTypes.get(i); actualValue = (String)effType.get(EffectivityFramework.ACTUAL_VALUE); displayValue =
             * (String)effType.get(EffectivityFramework.DISPLAY_VALUE); sb.append("<input type=\"checkbox\" name=\"effTypes2\" style=\"display:none;\" checked=\"checked\" value=\"" );
             * sb.append(XSSUtil.encodeForHTMLAttribute(context, actualValue)); sb.append("@displayactual@"); //XSS OK sb.append(displayValue); sb.append("\"/>"); } } }
             */
        } else // view mode only display expression
        {
            sb.append(XSSUtil.encodeForHTML(context, displayValue));
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a packed hashmap with the following arguments paramMap String of parameter values - args
     * @throws Exception
     *             if the operation fails
     * @since CFF R210 for ECC Reports
     */

    public Vector getProposedEffectivity(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix code context object
     * @param parentPartId
     *            the parent part object id
     * @param childPartId
     *            the child part object id.
     * @param relAttributes
     *            hashMap of ebom relationship attributes
     * @param strExpression
     *            the effectivity expression to be applied on the EBOM relid.
     * @param operation
     *            the operation to be called with CFF api.
     * @throws Exception
     *             if the operation fails
    ...
    public boolean isWipBomAllowed(Context context, String args[]) throws Exception {
    ...
    private StringBuffer constructUpdateOXMLStr(StringBuffer updateOXML, String childPartId, String rowId, String relId, String markup) throws Exception {
        ...
        return updateOXML.append("<item oId=\"").append(childPartId).append("\" rowId=\"").append(rowId).append("\" pId=\"null\" relId=\"").append(relId).append("\" markup=\"").append(markup)
                .append("\"></item>");
    }
    ...
    public int checkVPMProductInReleasedState(Context context, String[] args) throws Exception {
        ...
        String targetState = PropertyUtil.getSchemaProperty(context, "policy", PropertyUtil.getSchemaProperty(context, "policy_VPLM_SMB"), args[1]);
        ...
        if ("true".equalsIgnoreCase(vplmVisible)) {
            ...
            if (productId != null && !"".equals(productId)) {
                if (!PolicyUtil.checkState(context, productId, targetState, PolicyUtil.GE)) {
                    String strMessage = EngineeringUtil.i18nStringNow("emxEngineeringCentral.alert.releaseVPMProduct", context.getSession().getLanguage());
    ...
    public Boolean displayProductConfigurationFilter(Context context, String[] args) throws Exception {
    ...
    public String getPCFilterExpressionDisplayBOM(Context context, String divName, String textField, String hiddenField, String objectId, String javascriptFun) throws Exception {
        ...
        if (null != objectId && !"null".equals(objectId)) {
            ...
            policyClassification = EngineeringConstants.TYPE_PART_MASTER.equals(type) ? (String) dataMap.get(SELECT_PART_POLICYCLASS) : (String) dataMap.get(policyClassification);
        }
    ...
    public String getPCFilterExpressionDisplayBOM(Context context, String divName, String textField, String hiddenField, String policy, String objectId, String javascriptFun) throws Exception {
        ...
        if (divName.equalsIgnoreCase("editPCFilter"))
            sb.append("&nbsp<a href=\"javascript:basicClear('").append(textField).append("');basicClear('PCFilterId1') \">")
                    .append(EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.Common.Clear", context.getSession().getLanguage())).append("</a>");
        else
            sb.append("&nbsp<a href=\"javascript:basicClear('").append(textField).append("');basicClear('PCFilterId2') \">")
                    .append(EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.Common.Clear", context.getSession().getLanguage())).append("</a>");
        sb.append("</div>");
        return sb.toString();
    }
    ...
    public StringList getConfigurationContextPCIds(Context context, String[] args) throws Exception {
        ...
        String SELECT_PC_ID = "from[" + PropertyUtil.getSchemaProperty(context, "relationship_ConfigurationContext") + "].to.from["
                + PropertyUtil.getSchemaProperty(context, "relationship_MainProduct") + "].to.from[" + PropertyUtil.getSchemaProperty(context, "relationship_ProductConfiguration") + "].to.id";
