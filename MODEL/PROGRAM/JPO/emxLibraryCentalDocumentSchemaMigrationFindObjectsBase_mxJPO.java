...
/*
 * ${CLASSNAME}.java Base JPO for finding the Objects to be migrated for Document Library Schema Unification
 *
...
public class emxLibraryCentalDocumentSchemaMigrationFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO {
    ...
    public emxLibraryCentalDocumentSchemaMigrationFindObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
     ...
     * @param args
     *            holds two arguments args[0] - chunk size (i.e No of Object Ids to be written to each flat file) args[1] - Directory to where files containing ObjectIds are to be written
     * @returns 0 always
     * @throws Exception
    ...
    /**
     * Evaluates temp query to get all - Objects of type 'Document Library' - Objects of type 'Bookshelf' which are not connected to any 'document Library' - Objects of type 'Book' which are not
     * connected to any 'Bookshelf'
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param chunksize
    ...
    public void getIds(Context context, int chunkSize) throws Exception {
        ...
        try {
            ...
            MqlUtil.mqlCommand(context, command, bookshelfType, name, revision, vaultList, "1",
                    "if (to[Has Bookshelves] == False) then (program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true) else false");

            MqlUtil.mqlCommand(context, command, bookType, name, revision, vaultList, "1",
                    "if (to[Has Books] == False) then (program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true) else false");
