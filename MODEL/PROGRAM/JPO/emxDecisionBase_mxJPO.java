...
public class emxDecisionBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public MapList getRelatedDecisions(Context context, String[] args) throws Exception {
        ...
        /*
         * additional Decision information is required to display it in Meeting Summary table. i.e. name,type,policy,description,owner and current state
         */
        ...
        relBusObjPageList = doObj.getRelatedObjects(context, strRelName, strType, objectSelects, relSelects, true, false, sRecursionLevel, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: paramMap - a Map containing the paramMap which contains object ID
    ...
    public MapList getWhereUsedRel(Context context, String[] args) throws Exception {
        ...
        StringBuffer buffQuery = new StringBuffer(75);
        // append the mql query to get the all 'from' Requirement Object and rel ids
        buffQuery.append("print bus '");
        buffQuery.append(strObjectId);
        buffQuery.append("' select from.torel.from.id from.torel.id dump |");

        // execute the mql command with super user & assign the result to a variable.
        String result = MqlUtil.mqlCommand(context, buffQuery.toString());
        ...
        if (fromObjectRelIdList != null) {
            ...
            for (int itr = 0; itr < fromObjectCount; itr++) {
                ...
                /*
                 * Above mql query returns the connection id of req-req. We need to get the connection id b/w decision and Derived Req or Sub Req. We need to pass that id over id[connection] so that
                 * when the user performs remove operations from where used channel it will remove only that relationship. Not b/w the req-req.
                 */
                String connectionId = (String) fromObjectRelIdList.elementAt(fromObjectCount + itr);
                if (connectionId != null && !"".equals(connectionId)) {
                    StringBuffer query = new StringBuffer(75);
                    query.append("print connection '");
                    query.append(connectionId);
                    query.append("' select tomid.from.id tomid.id dump |");
                    result = MqlUtil.mqlCommand(context, query.toString());
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getAllDecisions(Context context, String[] args) throws Exception {
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getOwnedDecisions(Context context, String[] args) throws Exception {
     ...
     * @grade 0
     */
    protected MapList getDecisions(Context context, String strWhereCondition, String strOwnerCondition) throws Exception {
    ...
    public List getRelToObject(Context context, String[] args) throws Exception {
        ...
        while (objectListItr.hasNext()) {
            ...
            if (strRelId != null && strRelId.length() > 0)
                strRelId = MqlUtil.mqlCommand(context, "print connection '" + strRelId + "' select torel.id dump |");
            ...
            if (strRelId != null && strRelId.length() > 0) {
                ...
                if (toObjIdList != null && toObjIdList.size() > 0) {
                    ...
                    String aHref = "<a href=\"javascript:emxTableColumnLinkClick('../common/emxTree.jsp?emxSuiteDirectory=common&parentOID=null&jsTreeID=null&suiteKey=Framework&objectId=" + toObjId
                            + "', '700', '600', 'false', 'popup', '')\" class=\"object\">";
                    String attHtml = "<table border=\"0\">";
                    attHtml = attHtml + "<tr>";
                    attHtml = attHtml + "<td rmb=\"\" valign=\"top\" rmbID=" + XSSUtil.encodeForHTMLAttribute(context, toObjId) + ">" + aHref + "<img src='../common/images/" + attIcon
                            + "' border=0></a></td>";
    ...
    public MapList getWhereUsed(Context context, String[] args) throws Exception {
        ...
        relBusObjPageList = getRelatedObjects(context, strRelName, "*", objectSelects, relSelects, false, true, sRecursionLevel, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
    ...
    public MapList getAppliesTo(Context context, String[] args) throws Exception {
        ...
        relBusObjPageList = getRelatedObjects(context, strRelName, strType, objectSelects, relSelects, false, true, sRecursionLevel, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
    ...
    public StringList excludeDecisions(Context context, String[] args) throws Exception {
        ...
        MapList ancestors = domObject.getRelatedObjects(context, DomainConstants.RELATIONSHIP_DECISION, // Expand this relationship
     ...
     */
    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
    public StringList excludeDecisionWhereUsed(Context context, String[] args) throws Exception {
        Map programMap = (Map) JPO.unpackArgs(args);
        String strObjectId = (String) programMap.get("objectId");
     ...
     */
    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
    public StringList excludeDecisionAppliesTo(Context context, String[] args) throws Exception {
        Map programMap = (Map) JPO.unpackArgs(args);
        String strObjectId = (String) programMap.get("objectId");
        ...
        MapList ancestors = domObject.getRelatedObjects(context, DomainConstants.RELATIONSHIP_DECISION_APPLIES_TO, // Expand this relationship
                DomainConstants.QUERY_WILDCARD, // retrieving all object types
                false, true, 1, // in the to direction, 1 level
    ...
    public Vector getReleaseDate(Context context, String[] args) throws Exception {
        ...
        while (objListItr.hasNext()) {
            ...
            for (int i = historyData.size() - 1; i >= 0; i--) {
                ...
                if (historyRecord.startsWith("promote") && historyRecord.endsWith(STATE_PART_RELEASE)) {
    ...
    public List getRelationshipType(Context context, String[] args) throws Exception {
        ...
        while (objectListItr.hasNext()) {
            ...
            strRelId = (String) objectMap.get(DomainConstants.SELECT_RELATIONSHIP_ID);
            if (strRelId != null && strRelId.length() > 0)
                strRelId = MqlUtil.mqlCommand(context, "print connection '" + strRelId + "' select torel.name dump |");
