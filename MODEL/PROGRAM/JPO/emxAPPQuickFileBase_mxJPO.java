...
/*
 ** emxAPPQuickFileBase Created on Jun 28, 2007 Dassault Systemes, 1993 2007. All rights reserved. All Rights Reserved This program contains proprietary and trade secret information of Dassault
 * Systemes. Copyright notice is precautionary only and does not evidence any actual or intended publication of such program.
 */
import matrix.db.*;
import matrix.util.*;
import java.util.*;
...
public class emxAPPQuickFileBase_mxJPO {
    ...
    public HashMap listReferenceDocuments(Context context, String[] args) throws Exception {
        ...
        if (strRelName != null && !"".equals(strRelName)) {
            ...
            for (int k = 0; k < selects.size(); k++) {
                ...
                idsList.addAll(FrameworkUtil.split(MqlUtil.mqlCommand(context, "print bus " + sId + " select " + sExpr + " dump", true), ","));
     ...
     * @since AEF 10.7 SP3
     */
    public HashMap listQuickFiles(Context context, String[] args) throws Exception {
        HashMap hmpInput = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) hmpInput.get("paramMap");
        HashMap commandMap = (HashMap) hmpInput.get("commandMap");
     ...
     * @since AEF 10.7 SP3
     */
    public HashMap listRelatedFiles(Context context, String[] args) throws Exception {
        ...
        hmpDummy.put("settings", null);
        MapList mapContent = new MapList();
        String strRelName = EnoviaResourceBundle.getProperty(context, "emxFramework.QuickFileAccess.RelationShips.Default");
        StringList relList = null;
        if (strRelName != null && !"".equals(strRelName)) {
            // Split the relationship which is been passed as comma separated.
            relList = FrameworkUtil.split(strRelName, ",");
            StringList sbRel = new StringList();
            // Get the Actual name of the relationships
            if (relList != null) {
                for (int i = 0; i < relList.size(); i++) {
                    String sRel = (String) relList.get(i);

                    MxPattern pattern = MxPattern.compile("(\\[)(\\w*)(\\])", MxPattern.CASE_INSENSITIVE | MxPattern.MULTILINE);
                    MxMatcher matcher = pattern.matcher(sRel);

                    while (matcher.find()) {
                        String strMatch = matcher.group(2);
                        String sActualRel = PropertyUtil.getSchemaProperty(context, strMatch);
                        // String sActualRel = "Test";

                        sRel = FrameworkUtil.findAndReplace(sRel, strMatch, sActualRel);
                    }
                    sbRel.add(sRel);
                }
                strRelName = FrameworkUtil.join(sbRel, ",");
            }
        }
        String strObjId = "";
        String rmbTableRowId = (String) paramMap.get("rmbTableRowId");
        ...
        if (strRelName != null) {
            StringTokenizer relString = new StringTokenizer(strRelName, ",");
            while (relString.hasMoreTokens()) {
                ...
                if (relStr != null && !"null".equals(relStr) && !"".equals(relStr)) {
                    ...
                    while (matcher.find()) {
                        ...
                        if ("relationship_".indexOf(strMatch) != -1) {
        ...
        DomainObject masterObject = DomainObject.newInstance(context, strObjId);
        ...
        MapList documents = masterObject.getRelatedObjects(context, relPattern.getPattern(), "*", typeSelects, relSelects, true, true, (short) 0, objectWhere, null, null, null, null);
        ...
        for (int i = 0; i < documents.size(); i++) {
            tempMap = (Map) documents.get(i);
            objectId = (String) tempMap.get(DomainConstants.SELECT_ID);
            if (!tempHashSet.contains(objectId)) {
                tempHashSet.add(objectId);
                tempMapList.add(tempMap);
            }
        }

        documents = new MapList();
        documents.addAll(tempMapList);
        ...
        HashMap settings = new HashMap();
        settings.put("Registered Suite", "Components");
        settings.put("Image", "../common/images/iconSmallDocument.gif");
        settings.put("Registered Suite", "Framework");
        settings.put("Pull Right", "true");
        StringList selectList = new StringList(2);
        selectList.add(DomainConstants.SELECT_TYPE);
        selectList.add(DomainConstants.SELECT_NAME);
        if (documents != null && documents.size() > 0) {
     ...
     * @since AEF 10.7 SP3
     */
    public HashMap getProperties(Context context, String[] args) throws Exception {
     ...
     * @since AEF 10.7 SP3
     */
    public HashMap listFileVersion(Context context, String[] args) throws Exception {
        HashMap hmpInput = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) hmpInput.get("paramMap");
        HashMap commandMap = (HashMap) hmpInput.get("commandMap");
        HashMap requestMap = (HashMap) hmpInput.get("requestMap");
     ...
     * @since
     */
    public HashMap getQuickFileTable(Context context, String[] args) throws Exception {
        HashMap hmpInput = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) hmpInput.get("paramMap");
        HashMap commandMap = (HashMap) hmpInput.get("commandMap");
        String partId = "";
        ...
        String sRelation = EnoviaResourceBundle.getProperty(context, "emxFramework.QuickFileAccess.RelationShips.Default");
        StringList relList = null;
        if (sRelation != null && !"".equals(sRelation)) {
            // Split the relationship which is been passed as comma separated.
            relList = FrameworkUtil.split(sRelation, ",");
            // Get the Actual name of the relationships
            if (relList != null) {
                StringList relSymbolicList = new StringList();
                for (int i = 0; i < relList.size(); i++) {
                ...
                relList = relSymbolicList;
                sRelation = FrameworkUtil.join(relList, ",");
            }
        }
        String strImg = "";
    ...
    protected static String replaceSymbolicTokens(Context context, String expression) throws FrameworkException {
