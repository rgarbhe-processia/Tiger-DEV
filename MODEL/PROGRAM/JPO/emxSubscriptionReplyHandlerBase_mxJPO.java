...
public class emxSubscriptionReplyHandlerBase_mxJPO {
    ...
    /**
     * This method captures Email reply and links it to the respective Discussion object if already exiting else creates a new discussion with the information that is in the reply mail
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            [] contains all the Email information like Subject, message body, To, CC, BCC etc
     * @throws Exception
     *             if the operation fails
     * @since V6R2009-1
     */
    ...
    public void captureMailInfo(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String cmd = "list person email " + fromId;
            ...
            String discussionId = MqlUtil.mqlCommand(context, "print bus " + typeNameRev + " select Id dump;");
            ...
            if (strTypeMsg.equalsIgnoreCase(typename)) {
                ...
                MapList objlist = oldMessageBusObject.getRelatedObjects(context, // matrix context
                ...
                try {
                    oldMessageBusObject.connectTo(context, strRelReply, newMessageDomObject);
                ...
                if (threadObjId == null || "null".equals(threadObjId)) {
                    ...
                    try {
                        oldMessageBusObject.connectTo(context, strRelThread, newThreadDomObject);
                    } catch (Exception ex) {
                        System.out.println("ERROR::" + ex.getMessage());
                        ex.printStackTrace();
                    }
                ...
                newMessageDomObject.setAttributeValue(context, "Subject", subject);
                try {
                    newThreadDomObject.connectTo(context, strRelMsg, newMessageDomObject);
                } catch (Exception ex) {
                    System.out.println("ERROR::" + ex.getMessage());
                    ex.printStackTrace();
                }
     ...
     *            the eMatrix <code>Context</code> object
     * @param discussionId
     *            is the discussion object Id on which the subscription is to be done.
     * @param mailList
     *            is the list of persons that are to be subscribed on the discussion object for the new Discussion event.
     * @throws Exception
     *             if the operation fails
     * @since V6R2009-1
     */
    ...
    private boolean autoCcSubscription(Context context, String discussionId, com.matrixone.util.MxInternetAddress[] mailList) throws Exception {
        ...
        for (int len = 0; len < mailList.length; len++) {
            ...
            if (cmd != null && !"".equals(cmd.trim()) && !cmd.equalsIgnoreCase(subscriptionAgent)) {
                String fromName = MqlUtil.mqlCommand(context, cmd);
                ContextUtil.pushContext(context, fromName, null, null);
    ...
    /**
     * This method check for any unsupported character in the message and replaces that character with a supported one and also eliminate trim to get the exact reply message
     * @param message
     *            message that is to parsed for checking the unsupported characters
     * @throws Exception
     *             if the operation fails
     * @since V6R2009-1
     */
    ...
    private String checkForUnsupportedCharacter(String message) {
        ...
        com.matrixone.jsystem.util.MxPattern _originalMessagePattern = com.matrixone.jsystem.util.MxPattern.compile("(Original Message:)" + "|(-----\\s?Original Message\\s?-----)" + "|(wrote:$)" +
