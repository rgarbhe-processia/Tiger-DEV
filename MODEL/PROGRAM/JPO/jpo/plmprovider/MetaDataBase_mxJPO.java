...
/**
 * @author bucknam The <code>MetaDataBase</code> class provides web services associated with meta data.
...
public class MetaDataBase_mxJPO extends jpo.plmprovider.Mat3DLive_mxJPO {
    ...
    private static String strExpandRelationship = "";
    ...
    /**
     * Get the meta info modeled on this server. This is the web service entry point.
     * @param username
     *            the matrix user name
     * @param password
     *            password of the user
     * @param language
     *            the language for translations
    ...
    /**
     * Get the meta info modeled on this server. This method is for testing purposes only.
     * @param context
     *            the matrix context
     * @param args
     *            the only member of the array should contain the language for translations
     * @return MetaInfo
     * @throws Exception
     *             if there are errors retrieving the meta info
     * @since AEF 10.7.1.0
     */
    public MetaInfo getMetaData(Context context, String[] args) throws Exception {
        String language = "en";
    ...
    private MetaInfo getMetaData(Context context, String language) throws Exception {
        ...
        try {
            ...
            // TIGTK-7004 - 2017-05-17 - VP - START
            nodeMap = Collections.synchronizedMap(new MxLinkedHashMap(128));
            ...
            if (nodeMap == null || nodeMap.size() == 0) {
                ...
                for (int envItr = 0; envItr < listEnvironments.size(); envItr++) {
                    ...
                    NodeType environmentNode = addNode(nodeMap, environmentName, EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), environmentName));
     ...
     * @param context
     *            the matrix context
     * @param appDomainPrefix
     *            the appDomainPrefix eg. 3DLive.Domains. 3DLive.Domains.TEAM
     * @param environmentNameforProperty
     *            the environment Name eg.Engineering_Central
     * @param language
     *            the language
     * @return List of nodetypes
     * @throws Exception
     */
    private List getDomains(Context context, String appDomainPrefix, String environmentNameforProperty, String language) throws Exception {
        String strDomains = "";
        try {
            ...
            try {
                ...
                if (typeName != null && !"".equals(typeName)) {
                    ...
                    if (nodeMap.containsKey(symbolicTypeName)) {
                        ...
                        if (typeName.equals("Part") && UIUtil.isNotNullAndNotEmpty(strExpandRelationship)) {
                            typeNode.setExpandRelationshipPattern(strExpandRelationship);
                        } else {
                            typeNode.setExpandRelationshipPattern(getPropertyValue(context, PROPERTY_3DLIVE_EXPAND_REL_PATTERN_PREFIX, environmentNameforProperty, typeName));
                        }
                        // TIGTK-7004 - 2017-05-17 - VP - END
                        // Following statements gets the Expand Type Pattern value from emxSystem.properties file
                        typeNode.setExpandTypePattern(getPropertyValue(context, PROPERTY_3DLIVE_EXPAND_TYPE_PATTERN_PREFIX, environmentNameforProperty, typeName));

                        // Following statements gets the Expand Relationship Pattern value from emxSystem.properties file
                        typeNode.setGetParentsRelationshipPattern(getPropertyValue(context, PROPERTY_3DLIVE_GET_PARENTS_REL_PATTERN_PREFIX, environmentNameforProperty, typeName));

                        // Following statements gets the Expand Type Pattern value from emxSystem.properties file
                        typeNode.setGetParentsTypePattern(getPropertyValue(context, PROPERTY_3DLIVE_GET_PARENTS_TYPE_PATTERN_PREFIX, environmentNameforProperty, typeName));

                        // Following statements gets the Skip Intermediate Nodes During Expand value from emxSystem.properties file
                        typeNode.setSkipIntermediateNodeDuringExpand(getPropertyValue(context, PROPERTY_3DLIVE_SKIP_INTERMEDIATE_NODES_DURING_EXPAND, environmentNameforProperty, typeName));

                        // Following statement gets the View Table value from emxSystem.properties file
                        String symbolicViewTable = getPropertyValue(context, PROPERTY_3DLIVE_VIEW_TABLE_PREFIX, environmentNameforProperty, typeName);
                        ...
                        typeNode.setImagePath(getPropertyValue(context, PROPERTY_3DLIVE_IMAGE_PATH_PREFIX, environmentNameforProperty, typeName));

                        // Following statement gets the Image Format values (comma separated list) from emxSystem.properties file
                        typeNode.setImageFormat(getPropertyValue(context, PROPERTY_3DLIVE_IMAGE_FORMAT_PREFIX, environmentNameforProperty, typeName));

                        // Following statement gets the Thumbnail Path values (comma separated list) from emxSystem.properties file
                        typeNode.setThumbnailPath(getPropertyValue(context, PROPERTY_3DLIVE_THUMBNAIL_PATH_PREFIX, environmentNameforProperty, typeName));

                        // Following statement gets the Thumbnail Format values (comma separated list) from emxSystem.properties file
                        typeNode.setThumbnailFormat(getPropertyValue(context, PROPERTY_3DLIVE_THUMBNAIL_FORMAT_PREFIX, environmentNameforProperty, typeName));

                        // Following statements gets the Document Relationship Pattern value from emxSystem.properties file
                        typeNode.putDocumentRelationshipPattern(getPropertyValue(context, PROPERTY_3DLIVE_DOCUMENT_REL_PATTERN_PREFIX, environmentNameforProperty, typeName));
                        ...
                        typeNode.putExcludeVersionedDocuments(PlmProviderUtil.isKindof(context, typeName, type_DOCUMENTS) && !PlmProviderUtil.isKindof(context, typeName, "MCAD*"));
    ...
    /**
     * Add a node to the node map. If a node of that name already exists, the do not add a new node but return the existing one.
     * @param nodeMap
     *            the map of nodes
     * @param name
     *            the name of the new node
     * @param displayName
     *            the display name of the new node
     * @return the node from the map
     * @throws NoSuchAlgorithmException
     *             if the name hash class can not be found
     * @since AEF 10.7.1.0
     */
    private NodeType addNode(Map nodeMap, String name, String displayName) throws NoSuchAlgorithmException {
    ...
    /**
     * Print the nodes contained in the array. This method is meant for debugging purposes only.
     * @param nodes
    ...
    /**
     * Send a message to the log. Made this separate method just to catch the exception.
     * @param message
     ...
     * @param attributeMap
     *            the map of attribute definitions.
     * @param attributeName
     *            the name of the attribute
     * @param hashName
     *            the hash name of the attribute
     * @param type
     *            the type of attribute (string, integer, double, date)
     * @since AEF 10.7.1.0
     */
     ...
     *            Name of the Environment which is exposed to 3D Live
     * @param typeName
     *            Name of the Domain which is exposed to 3D Live under the Environment environmentName
     * @param mlColumns
     *            the MapList of Table Columns
     * @return an array of attribute definitions
     * @throws Exception
     *             if the table can not be found
     * @since AEF 10.7.1.0
     */
    private AttributeDefinition[] processTableColumns(Context context, String environmentName, String typeName, MapList mlColumns) throws Exception {
        ...
        try {
            ...
            for (int i = 0; columnItr.hasNext(); i++) {
                ...
                if (isEmptyColumn(column) == false) {
                    ...
                    log("column label <<" + displayName + ">> column select <<" + select + ">> hash <<" + hashName + ">>");

                    attribute = addAttribute(attributes, displayName, hashName, select, UITable.getSetting(column, SETTING_SORT_TYPE));
    ...
    /**
     * Return the property value of the given key and type, or any of its parent types. This method is for testing purpose only.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            the array should contain prefix, environment and type.
     ...
     * @param environment
     *            the environment to append to the property key
     * @param type
     *            the type to append to the property key
     * @return the property value
     * @throws Exception
     *             if operation fails
     * @since AEF 10.7.1.0
    ...
    public String getPropertyValue(Context context, String prefix, String environment, String type) throws Exception {
        String result = "";
        StringBuffer key = new StringBuffer();
        key.append(prefix).append(environment).append(".");
        try {
            String parentType = null;
            String symbolicTypeName = FrameworkUtil.getAliasForAdmin(context, "type", type, true);

            try {
                result = EnoviaResourceBundle.getProperty(context, key.toString() + symbolicTypeName);
            } catch (Exception e) {
                // no property found
                ...
                while (parentTypes.size() > 0) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        symbolicTypeName = FrameworkUtil.getAliasForAdmin(context, "type", parentType, true);
                        try {
                            result = EnoviaResourceBundle.getProperty(context, key.toString() + symbolicTypeName);
                        } catch (Exception e1) {
                            // keep trying
    ...
    /**
     * This method returns all type names (symbolic names) that are configured in emxSystem.properties file for 3DLive/3DVIA.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            the array should contain launguage.
     * @return String Containing Configured types
     * @throws Exception
     * @since R210
     */
    public String getConfiguredTypes(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String language = (String) programMap.get("launguage");
        // TIGTK-7004 - 2017-05-17 - VP - START
        strExpandRelationship = (String) programMap.get("PSS_ExpandRelationship");
        // TIGTK-7004 - 2017-05-17 - VP - END
