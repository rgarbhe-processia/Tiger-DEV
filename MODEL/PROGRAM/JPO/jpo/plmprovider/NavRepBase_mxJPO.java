...
//
// Copyright (c) 2006-2017 by Dassault Systemes.
// All Rights Reserved
...
/**
 * @author bucknam The <code>NavRepBase</code> class provides web services associated with providing graphical representations of a tree structure.
...
public class NavRepBase_mxJPO extends jpo.plmprovider.Mat3DLive_mxJPO {
    ...
    /**
     * This function will get FCS urls for all the objects in the request object given as an array of objIds.
     * @param username
    ...
    /**
     * This function will get a NavRepResult for the given object and given image type. it is called from an JSP page used for launching 3DLiveEmbed player arguments are packed in the string array
     * args
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param mcsURL
     *            the inital portion of the URL
     * @param objectId
     *            the object to process
     * @param imageType
     *            the type of image to return (cgr or jpg)
    ...
    /**
     * This function will get the file name for the given object and given image type. it is called from an JSP page used for launching 3DVIA viewer arguments are packed in the string array args
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param mcsURL
     *            the inital portion of the URL
    ...
    protected NavRepResult getResult(Context context, String mcsURL, String objectId, String imageType) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param mcsURL
     *            the initial portion of the URL
     * @param objectId
     *            the object to process
     * @param imageType
     *            the type of image to return (cgr or jpg)
     * @param typeNode
     *            the Node type for the corresponding object ID
     * @return NavRepResult the FCS url and timestamp of the resulting image file
     * @throws MatrixException
     * @since R209
     */

    private NavRepResult getResult(Context context, String mcsURL, String objectId, String imageType, NodeType typeNode)
        ...
        try {
            ...
            if (symbolicPathList != null && symbolicPathList.size() > 0 && symbolicFormatList != null && symbolicFormatList.size() > 0
                    && ((symbolicPathList.size() == symbolicFormatList.size() || imageType.equalsIgnoreCase("CGM") || imageType.equalsIgnoreCase("png")))) {
                ...
                for (int pathItr = 0; pathItr < symbolicPathList.size(); pathItr++) {
                    ...
                    if (imageType.equalsIgnoreCase("CGM") || imageType.equalsIgnoreCase("png")) {// CGM and PNG formats have only one format in the list
                ...
                if (majorAndMinorIds.length == 2) {
                    ...
                    if (UIUtil.isNotNullAndNotEmpty(majorAndMinorIds[0]) && UIUtil.isNotNullAndNotEmpty(majorAndMinorIds[1]) && !majorAndMinorIds[1].trim().equals("0.0.0.0")) {
    ...
    /**
     * This function will get a NavRepResult for the given object (if it is VPLM Part) and given image type. If a select contains any relationship,the getInfo method will query for all derived
     * relationships and if it finds any object at any one of these relationships it will return result with corresponding key. (i.e the key in the return result may differs from the key in selects).
     * We can't get the actual value from map if the key in the result and selects is different. The method getResult will not work for VPLM data, since we have to query for abstract relationship but
     * not the implemented one. This method can be removed if core provides some way to identify the modified key in the result MapList
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param mcsURL
     *            the initial portion of the URL
     ...
     * @since R211
     */
    private NavRepResult getResultForVPLMParts(Context context, String mcsURL, String objectId, String imageType, NodeType typeNode) throws MatrixException {
    ...
    private NavRepResult getResult(Context context, String objectId, String mcsURL, Map resultsmap, Map selectsMap) throws MatrixException {
        NavRepResult result = new NavRepResult();// NavRep result

        if (resultsmap != null && resultsmap.size() > 0 && selectsMap != null && selectsMap.size() > 0) {
            try {
                String imageId = "";
                String fileFormat = "";
                String fcsUrl = "";
                ...
                OUTERMOST: for (int pathItr = 0; pathItr < pathSelects.size(); pathItr++) {
                    ...
                    if (imageIdList != null && imageIdList.size() > 0 && fileNamesList != null && fileNamesList.size() > 0 && fileTimeStampList != null && fileTimeStampList.size() > 0) {
                        for (int imageIdItr = 0; imageIdItr < imageIdList.size(); imageIdItr++) {
                            ...
                            for (int fileItr = 0; fileItr < fileNamesList.size(); fileItr++) {
                                ...
                                if (imageId != null && !"".equals(imageId) && fileName != null && !"".equals(fileName) && fileTimestamp != null && !"".equals(fileTimestamp)) {
                ...
                log("NavRep.getResult Image Id <<" + imageId + ">> file <<" + fileName + ">> of format <<" + fileFormat + ">> with timestamp <<" + fileTimestamp + ">>");
    ...
    protected NavRepResult getMarkupURL(Context context, String objectId, String mcsURL) throws Exception {
    ...
    protected String getUrl(Context context, String mcsURL, String oid, String fileName, String format) throws MatrixException {
        /*
         * Depending on flag flagForgetURLs avoided checkout code by generating one static url Generating URL is required, because client is doing some logic with parsing of URL In new mechanism
         * client need - ObjectId, File Format and FileName
         */
        ...
        if (doNotCheckout) {
            ...
            try {
                ...
                BusinessObjectProxy bproxy = new BusinessObjectProxy(oid, format, fileName, false, false);
                bops.add(bproxy);
    ...
    /**
     * This method checks whether cross highlight should be enabled or not returns true if all all the following conditions are satisfied 1)VPM/TBE or DEC installed 2)User preference is On
     * @param context
     *            the matrix context
     * @param args
