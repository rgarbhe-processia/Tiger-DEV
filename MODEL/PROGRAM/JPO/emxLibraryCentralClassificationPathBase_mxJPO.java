...
/**
 * The <code>emxLibraryCentralClassificationPathBase</code> class. This Class manages the Interface paths and the Classification Paths and also provides API for Dispaly of the Classification paths.
 * @exclude
 */

public class emxLibraryCentralClassificationPathBase_mxJPO implements LibraryCentralConstants {
     ...
     */
    public emxLibraryCentralClassificationPathBase_mxJPO(Context context, String[] args) throws Exception {

    }

    /*
     * Some of the following nested classes may appear to be superfluous, but they make the main class code easier to understand as compared to if it operated on more generic classes like List.
     */

    /**
     * Each String in a List of this type is the name of a classification interface. The root is first in the least, the leaf last. The path is NOT meant to include the CLASSIFICATION_TAXONOMIES name,
     * it starts below that.
     */
    public class InterfacePath extends StringList {
    }

    /**
     * Similar to InterfacePath, but each entry in the List is a ClassificationInfo
     */
    ...
    /**
     * Returns the InterfacePath representing all nodes from CLASSIFICATION_TAXONOMIES down to the given interfaceName
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param interfaceName
     ...
     *             if the operation fails
     */
    public InterfacePath getInterfacePath(Context context, String interfaceName) throws FrameworkException {
        ...
        String strParentsResult = MqlUtil
                .mqlCommand(context, strParentsCmd, true, interfaceName, "allparents.name", "allparents.kindof[" + DomainConstants.INTERFACE_CLASSIFICATION_TAXONOMIES + "]", ",").trim();
     ...
     *             if the operation fails
     */
    public ClassificationPath convertInterfacePathToClassPath(Context context, InterfacePath interfacePath) throws FrameworkException {
        ClassificationPath result = new ClassificationPath();

        String tailIf = (String) interfacePath.get(interfacePath.size() - 1);
        // First off, check for cached result
        ClassificationPath cachedResult = (ClassificationPath) _ifName2clsPath.get(tailIf);
        if (cachedResult != null) {
            _clsPathCacheHits++;
            return cachedResult;
        }
        ...
        emxLibraryCentralUtil_mxJPO.commaPipeQueryToMapSkipTNR(context, getClsCmd, true, ifName2clsInfo, sbtypes.toString(), "*", "*",
                "attribute[" + ATTRIBUTE_MXSYS_INTERFACE + "] matchlist '" + FrameworkUtil.join(interfacePath, ",") + "' ','", "attribute[" + ATTRIBUTE_MXSYS_INTERFACE + "]", "id", ",", "|");
    ...
    /**
     * Returns the depth of a classificatoin object within its library; e.g. for MechanicalParts=>Fasterners, the depth would be 2
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strClsId
     ...
     *             if the operation fails
     */
    public int getDepth(Context context, String strClsId) throws Exception {
    ...
    /**
     * Returns Vector of ClassificationPath's. Each ClassificationPath in the Vector represents one classification path for the given endItemId. Each ClassificationInfo in each ClassificatinoPath in
     * the Vector represents an element in the path (a Classification or Libaries) and stores name and id of the classification or lib object. The entire Vector is sorted by the Comparator above.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param endItemId
     *            the objectId
     ...
     * @throws Exception
     */
    public Vector getEndItemClassificationPaths(Context context, String endItemId) throws Exception {

        // get obj's classification interfaces
        StringList lstInterfaces = emxLibraryCentralUtil_mxJPO.getClassificationInterfaces(context, endItemId);
    ...
    /**
     * This method differs from getEndItemClassificationPaths in that the former applies to end items, and this one applies to classifications. Classification objects may have only one path, whereas
     * end items may have any number of them.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param clsObjId
     ...
     * @return ClassificationPath
     */
    public ClassificationPath getClassificationPath(Context context, String strClsObjId) throws FrameworkException {
    ...
    /**
     * This method fills in two Vectors of MapLists similar to the one from getClassificationPaths(); the difference is that this one splits the information into two Vectors, the first one is of paths
     * that lead to relevantParentId or below, the second is all other paths. Each Vector is sorted.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param endItemId
     *            the end Item OjectId
     * @param relevantParentId
     *            parrent OjectId
     * @param relevantPathsOut
     *            the path list
     * @param otherPathsOut
     *            other paths list
     * @throws Exception
     ...
     */
    public void getPrioritizedClassificationPaths(Context context, String endItemId, String relevantParentId, Vector relevantPathsOut, Vector otherPathsOut) throws Exception {
    ...
    public String renderPathElem(Context context, String strName, String strObjectId, String reportFormat) {
        if ("HTML".equals(reportFormat) || "CSV".equals(reportFormat)) {
            ...
            return "<a href=\"javascript:void(0)\" " + "onClick=\"javascript:showModalDialog(" + "'../common/emxTree.jsp?objectId=" + XSSUtil.encodeForURL(context, strObjectId) + "',"
                    + "'860','520');\"" + ">" + XSSUtil.encodeForXML(context, strName) + "</a>";
        }
    }

    /**
     * Renders an entire path by rendering its constituent elements separated by the separator HTML
     * @param pathElems
     ...
     *            the paths
     * @param forUseInForm
     *            not used; was previously needed to allow different rendering in UIForm vs. UITable, and could still be used for that purpose in a customization
     * @param reportFormat
     *            the report format
    ...
    /**
     * This method operates on a single object, whose oid is given in args. If that object is a Classification, this method calls getClassificationPath for that object, and then renders the resulting
     * path as HTML. If the object is not a Classification, then it is treated as an end-item; in that case, this method calls getEndItemClassificationPaths and renders the resulting paths as HTML.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments 0 - objectId the objectId 1 - pfMode printer friendly mode\ 2 -
     * @return String the Classification path HTML
     * @throws FrameworkException
     */
    public String getClassificationPathsHTML(Context context, String[] args) throws FrameworkException {
        try {
    ...
    /**
     * This method operates on a list of end-items, with reference to a single classification. It returns classification path info for each end item, but returns it as rendered HTML, one HTML string
     * per end-item, where each HTML lists the "relevant" paths first, then other paths below. In args, objectId is the Classification object, in reference to which path relevance is judged.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments 0 - objectList the list of Object details 1 - reportFormat the report format
     * @return Vector the with list of paths for each End item with respect to the Classification
     * @throws FrameworkException
     */
    public Vector getPrioritizedClassificationPathsHTMLs(Context context, String[] args) throws FrameworkException {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            ...
            while (mapItr.hasNext()) {
                ...
                getPrioritizedClassificationPaths(context, strEndItemId, strClsId, vecRelevantPaths, vecOtherPaths);
