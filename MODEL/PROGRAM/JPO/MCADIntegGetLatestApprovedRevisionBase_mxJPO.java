...
/*
 ** MCADIntegGetLatestApprovedRevisionBase
 **
...
public class MCADIntegGetLatestApprovedRevisionBase_mxJPO {
    ...
    public MCADIntegGetLatestApprovedRevisionBase_mxJPO() {
    }

    public MCADIntegGetLatestApprovedRevisionBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    /**
     * This function contains the implementation for returning the latest revision of a business object The function is passed the business object ID as argument. The implementation should return the
     * bus ID of the latest revision, i.e. the latest finalized major object. - Additionally a check has been made in case a finalized instance comes into a root node. If it is caught then the
     * connected family is called.
     * @param context
     *            The user context
     * @param args
    ...
    public Hashtable getLatestForObjectIds(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < buslWithSelectionList.size(); i++) {
            ...
            if (_util.isMajorObject(context, busID)) // [NDM] : QWJ
            {
                // /handle minor inbus
                StringList revisions = busObjectWithSelect.getSelectDataList("revisions");
                for (int j = (revisions.size() - 1); j > -1; j--) {
                    String revision = (String) revisions.elementAt(j);
                    String policy = busObjectWithSelect.getSelectData("revisions[" + revision + "].policy");
                    String finalizationState = _globalConfig.getFinalizationState(policy);
                    String majorState = busObjectWithSelect.getSelectData("revisions[" + revision + "].current");
                    StringList stateList = busObjectWithSelect.getSelectDataList("revisions[" + revision + "].state");

                    if (stateList.lastIndexOf(majorState) >= stateList.lastIndexOf(finalizationState)) {
                        outputId = busObjectWithSelect.getSelectData("revisions[" + revision + "].id");
                        break;
                    }
                }
            } else {
                // [NDM] : QWJ : Latest Released Revision should not be used on minors, it should return As-Built structure for minors
                outputId = busID;
            }
