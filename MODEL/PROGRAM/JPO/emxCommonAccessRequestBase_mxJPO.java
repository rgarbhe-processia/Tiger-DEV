...
/*
 * emxCommonAccessRequestBase Copyright (c) 2006-2015 Dassault Systemes. All Rights Reserved. This program contains proprietary and trade secret information of MatrixOne, Inc. Copyright notice is
 * precautionary only and does not evidence any actual or intended publication of such program This JPO contains the implementation of emxAccessRequestBase
 */
...
public class emxCommonAccessRequestBase_mxJPO extends emxDomainObject_mxJPO {
                ...
                if (objId instanceof StringList) {
                    ...
                    for (int j = 0; j < slId.size(); j++) {
                        if (isprinterFriendly || !hasReadShowAccess(objectAccessMap, (String) slId.get(j))) {
                            ...
                            String href = "../common/emxTree.jsp?emxSuiteDirectory=components&amp;treeMenu=" + (String) regUtil.getRegisteredList(context, (String) slType.get(j)) + "&amp;objectId="
                                    + (String) slId.get(j);
                    ...
                    if (isprinterFriendly || !hasReadShowAccess(objectAccessMap, (String) objId)) {
                        ...
                        // changes for LBC FT to SB
                        String href = "../common/emxTree.jsp?emxSuiteDirectory=components&amp;treeMenu=" + (String) regUtil.getRegisteredList(context, (String) objType) + "&amp;objectId="
                                + (String) objId;
                        String link = "<b><a href=\"javascript:emxTableColumnLinkClick('" + href + "',600,400,'false','popup')\" >";
                        sbNames.append(link);
                        sbNames.append(XSSUtil.encodeForHTML(context, (String) objName));
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList objectList
    ...
    public MapList getAssignees(Context context, String objectId) throws Exception {
        ...
        return domObj.getRelatedObjects(context, RELATIONSHIP_REQUESTED_ASSIGNEE, TYPE_PERSON, objSelects, null, true, false, (short) 1, null, null);
    }
    ...
    public MapList getDocuments(Context context, String objectId) throws Exception {
        ...
        try {
            DomainObject domObj = new DomainObject(objectId);
            return domObj.getRelatedObjects(context, RELATIONSHIP_REQUESTED_DOCUMENT, DomainConstants.QUERY_WILDCARD, objSelects, null, false, true, (short) 1, null, null);
        } finally {
            ContextUtil.popContext(context);
        }
    }
    ...
    /**
     * This Method returns the MapList of Access Requests based on Filter options.This method will be called in the emxTable.jsp. This value will be passed as Link Href of menu's
     * LCAccessRequestsMyDesk and LCLibrariesMyDesk.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds all arguments that are being passed in the to emxTable.jsp in the above specified Command Href.
    ...
    public MapList getAllAccessRequests(Context context, String[] args) throws Exception {
        ...
        return DomainObject.findObjects(context, TYPE_DOWNLOAD_ACCESS_REQUEST, DomainConstants.QUERY_WILDCARD, strWhere.toString(), objectSelects);
    }
    ...
    public void setAccessRequestGrant(Context context, String objectId, String strOriginator) throws Exception {
        ...
        try {
            while (docMapListItr.hasNext()) {
                ...
                // Access Grant for Originatior
                if ("Yes".equalsIgnoreCase(strIncludeSelf)) {
                    MqlUtil.mqlCommand(context, "modify bus " + strDocumentId + " grant \"" + strOriginator + "\" access CheckOut,read,show key " + strDesKey);
                }
                ...
                while (assigneeMapListItr.hasNext()) {
                    ...
                    if (!strOriginator.equals(strUserName)) {
                        MqlUtil.mqlCommand(context, "modify bus " + strDocumentId + " grant \"" + strUserName + "\" access CheckOut,read,show key " + strDesKey);
                        ContextUtil.pushContext(context);
                        try {
                            MqlUtil.mqlCommand(context, "modify bus " + objectId + " grant \"" + strUserName + "\" access read,show");
                        } finally {
                            ContextUtil.popContext(context);
                        }
                    }
    ...
    public void setAccessRequestRevoke(Context context, String objectId, String strOriginator) throws Exception {
        ...
        try {
            while (docMapListItr.hasNext()) {
                ...
                // Access Revoke for Originatior
                if ("Yes".equalsIgnoreCase(strIncludeSelf)) {
                    MqlUtil.mqlCommand(context, "modify bus " + strDocumentId + " revoke grantee \"" + strOriginator + "\" key " + strDesKey);
                }
                ...
                while (assigneeMapListItr.hasNext()) {
                    ...
                    String strUserName = (String) assigneeMap.get(DomainConstants.SELECT_NAME);
                    if (!strOriginator.equals(strUserName)) {
                        MqlUtil.mqlCommand(context, "modify bus " + strDocumentId + " revoke grantee \"" + strUserName + "\" key " + strDesKey);
                    }
                }
            }
        } finally {
    ...
    public void autoExpireRequests(Context context, String[] args) throws Exception {
        ...
        MapList requestMapList = DomainObject.findObjects(context, TYPE_DOWNLOAD_ACCESS_REQUEST, DomainConstants.QUERY_WILDCARD, "( current == \"" + strApproved + "\" )", objectSelects);
        ...
        /**
         * While checking for the expirydate we will not considering time. So setting current time and expiry time also to 12:00:00 AM
         */
    ...
    /**
     * This method notifies owner and all the assignees connected to Request Access by "Requested Assignees" relationship when the Request Access is going to expire
     * @param context
     *            The ematrix context of the request.
     * @param objectId
     *            The string containing object id of Request Access Object:
     * @throws Exception
    ...
    public void notifyOfPendingExpiration(Context context, String objectId) throws Exception, FrameworkException {
        ...
        String strSubjectKey = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR, strLocale, "emxComponents.AccessRequest.MessageSubject.PendingExpiration");
        String strMessageKey = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR, strLocale, "emxComponents.AccessRequest.MessageDescription.PendingExpiration");
        ...
        emxMailUtil_mxJPO.sendNotification(context, (StringList) slAssignees, null, null, strSubjectKey, subjectKeys, subjectValues, strMessageKey, messageKeys, messageValues,
        ...
        for (int i = 0; i < reqMapList.size(); i++) {
            ...
            if (null != slGrantExpirationDate) {
                ...
                if (strGrantExpirationDate != null && !"null".equals(strGrantExpirationDate) && !"".equals(strGrantExpirationDate)) {
                    ...
                    if (strCurrentState.equals(strApproved)) {
                        if (slipDays < strRedDays)
                            strStatus = "Red";
                        else if (slipDays > strGreenDays)
                            strStatus = "Green";
                        else
                            strStatus = "Yellow";
            ...
            if (strStatus == null) {
                ...
                if ("Yellow".equals(strStatus)) {
                    strToolTip = slipDays + " "
                            + EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR, context.getLocale(), "emxComponents.AccessRequest.ToolTip" + strStatus + "Days");
                } else {
                ...
                // IR-020143V6R2011x - End
                enableCheckbox.add("<img border='0' src='../common/images/iconStatus" + strStatus + ".gif' name='" + strStatus + "' id='" + strStatus + "' alt=\"" + strToolTip + "\" title=\""
                        + strToolTip + "\" />");
            }
        }
        synchronized (this) {
     ...
     * 
     * @param args - args contains a Map with the following entries objectId - The object Id of Context object
     * 
    ...
    public HashMap getFilterStatuses(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < slStatusValues.size(); i++) {
            String strStatusFilterValue = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR, context.getLocale(),
                    "emxComponents.AccessRequest.StatusFilterValue" + FrameworkUtil.findAndReplace((String) slStatusValues.get(i), " ", "_"));
    ...
    /**
     * This method notifies owner and all the assignees connected to Request Access by "Requested Assignees" relationship when the Request Access is promoted to the Review , Rejected Or Approved and
     * Expired.
     * @param context
     *            The ematrix context of the request.
     * @param objectId
     *            The string containing object id of Request Access Object:
     * @param strNextState
     *            The string containing promoted state of Request Access Object:
     * @throws Exception
     * @throws FrameworkException
     * @since Common 11.0
     */
    public void setUserAccessAndNotification(Context context, String[] args) throws Exception, FrameworkException {
        ...
        if (strNextState.equals(strReview)) {
            ContextUtil.pushContext(context);
            try {
                MqlUtil.mqlCommand(context, "modify bus " + objectId + " grant \"" + strOriginator + "\" access " + strAccessOriginator);
            } finally {
                ContextUtil.popContext(context);
            }
    ...
    public void removeSignatureOnDemote(Context context, String[] args) throws Exception, FrameworkException {
        MqlUtil.mqlCommand(context, "unsign bus " + args[0] + " signature ToApproved");
     ...
     *            The ematrix context of the request.
     * @param args
     *            The String array containing Packed arguments, This also contains list of object ids
     ...
     * @param args
     *            The String array containing Packed arguments, This also contains list of object ids
     * @return a Vector object containing Details Popup links (HTML output) for all objects in the object list that is passed as arguments
     * @throws Exception
     * @since Common 11.0
     */
     ...
     *            The String array containing Packed arguments, This also contains list of object ids
     * @param column
     *            a String to which HTML output is to be generated
        ...
        if (reqMapList != null) {
            ...
            if (LAUNCH.equals(column) || DomainConstants.SELECT_NAME.equals(column)) {
                ...
                for (int i = 0; i < reqMapListLength; i++) {
                    ...
                    if (isprinterFriendly || !parentType.equals(CommonDocument.TYPE_DOCUMENTS) || !hasReadShowAccess(objectAccessMap, objId)) {
                        ...
                        sbColumValues.append(
                                "emxTable.jsp?program=emxCommonDocumentUI:getRevisions&popup=true&table=APPDocumentRevisions&header=emxComponents.Common.RevisionsPageHeading&subHeader=emxComponents.Menu.SubHeaderDocuments&suiteKey=Components");
                        sbColumValues.append("&objectId=");
                        sbColumValues.append(XSSUtil.encodeForJavaScript(context, objId));
    ...
    /**
     * This method is used to get HTML output for the Document Action Icons of each object in the object list that is passed as arguments.
     * @param context
     *            The ematrix context of the request.
     * @param args
     *            The String array containing Packed arguments, This also contains list of object ids
     * @return a Vector object containing HTML output for the Document Action Icons of each object in the object list that is passed as arguments
     * @throws Exception
     * @since Common 11.0
     */
     ...
     * @return HashMap object containing map of object id and its access mask for the context user list that is passed as arguments
     * @throws Exception
     * @since Common 11.0
     */
     ...
     * @param objAccessMap
     *            a HashMap of Object Id to Access Mask.
     * @param objId
     *            a String containing Object Id
    ...
    private boolean hasReadShowAccess(HashMap objAccessMap, String objId) {
        if (objAccessMap != null) {
            ...
            if (objAccessMask == null || "null".equalsIgnoreCase(objAccessMask)
                    || (objAccessMask.indexOf("all") == -1 && (objAccessMask.indexOf("read") == -1 || objAccessMask.indexOf("show") == -1))) {
                return false;
            }
        }
        return true;
