...
/**
 * VPLMIntegMatrixVPLMSynchronize JPO for publishing and synchronizing VPLM data with Matrix and Matrix data with VPLM.
 */
...
public class VPLMIntegMatrixVPLMSynchronize_mxJPO {
    ...
    /**
     * Wrapper JPO to automatically synchronize Enovia MatrixOne data with VPLM . While packing the input string array provide the following data using the mentioned keys: ROOTID=<Business Object id
     * of the root part of the structure.> SYNC_DEPTH=<Depth up to which to synchronize. It should be a string specifying the depth in numerals. Specify "0" if only the root is to be synchronized;
     * "-1" if full depth is to be sychronized; > SYNC_AND_TRANSFER=<"give" to give control to other side. Default is "no". Specifying any other value means "no". It is not possible to take control.
     * @param Context
     *            object for the Enovia MatrixOne session.
     * @param argument
     *            string array.
     * @throws VPLMIntegException
     *             in case of severe error.
    ...
    /**
     * Wrapper JPO to automatically synchronize VPLM data with Enovia MatrixOne. While packing the input string array provide the following data using the mentioned keys: IDLIST=< Vector containing
     * Business Object ids of all the roots of the product structures to be synchronized.> SYNC_DEPTH=<Depth up to which to synchronize. It should be a string specifying the depth in numerals. Specify
     * "0" if only the root is to be synchronized; "-1" if full depth is to be sychronized; > SYNC_AND_TRANSFER=<"give" to give control to other side. Default is "no". Specifying any other value means
     * "no". It is not possible to take control.> vplmContext=<VPLM context string to be used.>
     * @param Context
     *            object for the Enovia MatrixOne session.
     * @param argument
     *            string array.
