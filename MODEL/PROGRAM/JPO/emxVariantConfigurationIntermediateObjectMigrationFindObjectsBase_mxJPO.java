...
public class emxVariantConfigurationIntermediateObjectMigrationFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO {
     ...
     * @since FTR V6R2012x
     */
    public emxVariantConfigurationIntermediateObjectMigrationFindObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
        ...
        while (stTypes.hasMoreTokens()) {
            ...
            if (typeName != null) {
                if (typeName.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCTS)) {
                    commandList.addElement("temp query bus '" + typeName + "' '" + name + "' '" + revision + "'  vault '" + vaultList + "' limit 1 where 'program[" + migrationProgramName
                            + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true &&" + "attribute[" + ATTRIBUTE_FTRMigrationConflict + "]!=\"\"'");
                } else {
                    commandList.addElement("temp query bus '" + typeName + "' '" + name + "' '" + revision + "'  vault '" + vaultList + "' limit 1 where 'program[" + migrationProgramName
    ...
    public void preMigration(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            List lsMalFormedFLObjects = DomainObject.findObjects(context, sBFeatListTypePattern.toString(), DomainConstants.QUERY_WILDCARD, sBFeatListObjWhere.toString(), sLFeatListobjSelects);
            ...
            List lsMalFormedGBOMObjects = DomainObject.findObjects(context, sBGBOMTypePattern.toString(), DomainConstants.QUERY_WILDCARD, sBGBOMObjWhere.toString(), sLGBOMobjSel);
