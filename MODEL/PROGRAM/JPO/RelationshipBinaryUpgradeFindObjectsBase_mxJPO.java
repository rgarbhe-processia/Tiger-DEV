...
/**
 * The <code>RelationshipBinaryUpgradeFindObjectsBase</code> class contains implementation code for RelationshipBinaryUpgradeFindObjectsBase.
...
public class RelationshipBinaryUpgradeFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO {
     ...
     * @since CFF V6R2013x
     */
    public RelationshipBinaryUpgradeFindObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public void getIds(Context context, int chunkSize) throws Exception {
        ...
        String command = "";
        if (!isType) {
            command = "query connection relationship '" + type + "' limit 1 where 'interface[" + EffectivityFramework.INTERFACE_EFFECTIVITY_FRAMEWORK + "]==TRUE && program[" + migrationProgramName
                    + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true '";
        } else {
            command = "temp query bus '" + type + "' '" + name + "' '" + revision + "'  vault '" + vaultList + "' limit 1 where 'program[" + migrationProgramName
                    + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true'";
        }
