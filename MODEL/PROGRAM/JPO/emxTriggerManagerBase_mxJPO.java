...
public class emxTriggerManagerBase_mxJPO extends emxUtil_mxJPO {
    ...
    private static Logger logger = LoggerFactory.getLogger("emxTriggerManagerBase");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: i - a String contains time value
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        iCurrentLevel++;
        sIndentation = "";
        sIndentation += generateIndent();
        logger.info("mxMain() - args.length = <" + args.length + ">");

        try {
            for (int i = 0; i < args.length; i++) {
                if (logger.isDebugEnabled()) {
                    String sEvent = MqlUtil.mqlCommand(context, "get env EVENT");
                    String sAction = MqlUtil.mqlCommand(context, "get env INVOCATION");
                    displayLogInfo(context, sEvent, sAction);
                    logger.debug("[Level:" + iCurrentLevel + "]" + sIndentation + " #" + sEvent + ":" + sAction + " Input:" + args[i]);
                }
                ...
                while (it.hasNext()) {
                    Float fSeq = (Float) it.next();
                    BusinessObjectWithSelect bows = (BusinessObjectWithSelect) tm.get(fSeq);
                    ...
                    if (result.compareTo("TRUE") == 0) {
                        ...
                        if (logger.isDebugEnabled()) {
                            String sUser = MqlUtil.mqlCommand(context, "get env USER");
                            logger.debug("[Level:" + iCurrentLevel + "]" + sIndentation + " Sequence:" + fSeq + " User:" + sUser + " Program:" + prog + ":" + method);
                        }

                        logger.info("mxMain() - invoke program <" + prog + "> method <" + method + ">");
                        int retValue = JPO.invoke(context, prog, constructor, method, arguments);
                        if (retValue == 1) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("[Level:" + iCurrentLevel + "]" + sIndentation + " Trigger block " + args[i] + " Program:" + prog + ":" + method);
                            }
                        ...
                        if (logger.isDebugEnabled()) {
                            String sUser = MqlUtil.mqlCommand(context, "get env USER");
                            logger.debug("[Level:" + iCurrentLevel + "]" + sIndentation + " Sequence:" + fSeq + " User:" + sUser + " Tcl:" + prog);
                        }
                        ...
                        if (result.startsWith("1")) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("[Level:" + iCurrentLevel + "]" + sIndentation + " Trigger block " + args[i] + " Tcl:" + prog);
                            }
                            if (invocation.compareTo("action") == 0) {
                                String keys[] = new String[2];
                                keys[0] = "emxFrameworkStringResource";
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: i - a String Contains environment variable value
     * @throws Exception
     *             if the operation fails
     */
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - a String contains boolean value
     * @return an int 0 for success and non-zero for failure
     * @throws Exception
     *             if the operation fails
     */
    ...
    /**
     * Construct a message to be pushed on log during trigger process
     * @param context
     *            : Matrix user context
     * @param sEvent
     *            : String, Event of the trigger (Connect, Create...)
     * @param sAction
     *            : String, Invocation of the trigger (Action, Check...)
     * @throws Exception
     * @author Faurecia
     */
    private void displayLogInfo(Context context, String sEvent, String sAction) throws Exception {
        String sLoggerMessage = "[Level:" + iCurrentLevel + "]" + sIndentation;
        String sFromObject = MqlUtil.mqlCommand(context, "get env FROMOBJECT");
        String sRelType = MqlUtil.mqlCommand(context, "get env RELTYPE");
        String sToObject = MqlUtil.mqlCommand(context, "get env TOOBJECT");
        String sObject = MqlUtil.mqlCommand(context, "get env OBJECT");
        String sPolicy = MqlUtil.mqlCommand(context, "get env POLICY");
        String sNewAttValue = MqlUtil.mqlCommand(context, "get env NEWATTRVALUE");
        String sAttName = MqlUtil.mqlCommand(context, "get env ATTRNAME");
        String sCurrentState = MqlUtil.mqlCommand(context, "get env CURRENTSTATE");
        String sNextState = MqlUtil.mqlCommand(context, "get env NEXTSTATE");
        String sDescription = MqlUtil.mqlCommand(context, "get env NEWDESCRIPTION");
        if ("ModifyAttribute".equals(sEvent) || "Modify".equals(sEvent))
            sLoggerMessage += " Put " + sNewAttValue + " on attribute " + sAttName;
        else if ("ModifyDescription".equals(sEvent))
            sLoggerMessage += " Put " + sDescription + " on Description";
        else if ("Connect".equals(sEvent) && "check".equalsIgnoreCase(sAction))
            sLoggerMessage += " Check for relationship " + sRelType;
        else if (!"".equals(sFromObject))
            sLoggerMessage += " Connect " + sToObject + " from " + sFromObject + " with relationship " + sRelType;
        else if ("Create".equals(sEvent))
            sLoggerMessage += " Create " + sObject + " with policy " + sPolicy;
        else if ("Promote".equals(sEvent) && "check".equalsIgnoreCase(sAction))
            sLoggerMessage += " Promote " + sObject + " from " + sCurrentState + " to " + sNextState;
        else if ("Promote".equals(sEvent))
            sLoggerMessage += " Promote " + sObject + " to " + sNextState;
        else if ("Demote".equals(sEvent) && "check".equalsIgnoreCase(sAction))
            sLoggerMessage += " Demote " + sObject + " from " + sCurrentState + " to " + sNextState;
        else if ("Demote".equals(sEvent))
            sLoggerMessage += " Demote " + sObject + " to " + sNextState;
        else
            sLoggerMessage += " MESSAGE TO BE DEFINED ON emxTriggerManagerBase:displayLogInfo";
        logger.info(sLoggerMessage);
    }

    private String generateIndent() {
        try {
            return new String(new char[iCurrentLevel - 1]).replace("\0", " ");
        } catch (Exception e) {
            return "";
        }
    }
