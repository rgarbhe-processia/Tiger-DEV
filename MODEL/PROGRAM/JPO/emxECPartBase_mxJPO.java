...
public class emxECPartBase_mxJPO extends emxBOMPartManagement_mxJPO {
    private static String EBOM_FROZEN_STATES = null;
    ...
    static protected final RelationshipType _ebomHistory = new RelationshipType(DomainConstants.RELATIONSHIP_EBOM_HISTORY);
    ...
    /** relationship "Manufacturer Equivalent". */
    public static final String RELATIONSHIP_MANUFACTURER_EQUIVALENT_HISTORY = PropertyUtil.getSchemaProperty("relationship_ManufacturerEquivalentHistory");

    /** The Manufacturer Equivalent History Relationship Object. */
    static protected final RelationshipType _mepHistory = new RelationshipType(RELATIONSHIP_MANUFACTURER_EQUIVALENT_HISTORY);

    /** type "Substitute". */
    public static final String TYPE_SUBSTITUTE = PropertyUtil.getSchemaProperty("type_Substitute");

    /* Added for Substitue Part */
    static final String SELECT_STANDARD_COMPONENT_EBOM_STRING = "from[" + RELATIONSHIP_STANDARD_COMPONENT + "].to.to[" + RELATIONSHIP_EBOM + "].";

    /** selects the standard components parent objectId. */
    public static final String SELECT_STANDARD_COMPONENT_EBOM_FROM_ID = SELECT_STANDARD_COMPONENT_EBOM_STRING + "from.id";

    /** selects the standard components quantity in the ebom. */
    public static final String SELECT_STANDARD_COMPONENT_EBOM_QUANTITY = SELECT_STANDARD_COMPONENT_EBOM_STRING + "attribute[" + ATTRIBUTE_QUANTITY + "]";
    ...
    public static final String RELATIONSHIP_PART_FAMILY_MEMBER = PropertyUtil.getSchemaProperty("relationship_PartFamilyMember");
    ...
    public static final String RELATIONSHIP_ASSIGNED_AFFECTED_ITEM = PropertyUtil.getSchemaProperty("relationship_AssignedAffectedItem");
    ...
    public static final String SELECT_REL_EBOM_FROM_LAST_REVISION_EXISTS = "last.from[" + RELATIONSHIP_EBOM + "]";
     ...
     * @since EC 9.5.JCI.0.
     */
    public emxECPartBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    /**
     * Clears the Start and End Effectivity dates on the EBOM or Manufacturer Equivalent connection between this assembly and any child components.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
    ...
    /**
     * Synchronizes the changes made on an EBOM relationship with any Substitutes the EBOM has. When the Find Number or Reference Designator attributes are updated on an EBOM, we need to update the
     * same attributes on any of its Substitue objects.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - the EBOM relationship id that is being updated 1 - the attribute name being updated 2 - the current value of the attribute 3 - the new value
     *            of the attribute
     * @throws Exception
     *             if the operation fails.
     * @since 10.0.0.0.
     ...
     */
    public void syncSubstitutes(Context context, String[] args) throws Exception {
    ...
    /**
     * Ensure Part Specification or Reference Document Exists for the Part and it has at least 1 file checked in.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments
     ...
     */
    public int ensureSpecsConnected(Context context, String[] args) throws Exception {
     ...
     * @trigger PolicyECPartStatePreliminaryPromoteCheck.
     */
    public int ensureDesignResponsibilityExists(Context context, String[] args) throws Exception {
    ...
    /**
     * Checks the specified relationships to see if the connected object has at least one file checked in.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param relTypes
     *            StringList of relationships to check.
     * @return true if connected object has file checked in else returns false.
     * @throws Exception
     *             if the operation fails.
     * @since 10.0.0.0.
     */
    protected boolean relsExistWithFileCheckedIn(Context context, StringList relTypes, String noCheckRequiredTypes) throws Exception {
        ...
        MapList mapList = getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
                "*", // object pattern
     ...
     * @author ZGQ
     */
    protected boolean isFileCheckedIntoSpec(Context context, MapList mapList, String noCheckRequiredTypes) throws Exception {
    ...
    /**
     * Floats all unfulfilled ECRs to the Released Part. The following steps are performed: - Checks the previous rev for connected ECRs that are not fulfilled or rejected. - Floats them to this
     * revision if they do not have an ECO attached.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
     ...
     */
    public void floatUnfulfilledECRs(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object.
     * @param relTypes
     *            StringList of relationship types.
     * @param policyStates
     *            String of policies and states delimited by "|".
     * @throws Exception
     *             If the operation fails.
     * @since 10.0.0.0.
     */
    protected void floatUnfulfilledECR(Context context, StringList relTypes, String policyStates) throws Exception {
        ...
        try {
            ...
            MapList mapList = prevRevPart.getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
            ...
            if (mapList.size() > 0) {
                ...
                if (ecoMapList.size() > 0) {
                    ...
                    MapList ecrMapList = eco.getItems(context, RELATIONSHIP_ECO_CHANGEREQUESTINPUT, selectStmts, selectRelStmts, "", "");
            ...
            while (e.hasMoreElements()) {
                ...
                if (strRelName != null && strRelName.length() > 0 && strRelName.equals(RELATIONSHIP_AFFECTED_ITEM)) {
                    ...
                    String strAssignedECRelId = MqlUtil.mqlCommand(context, "print connection $1 select $2 dump", relId,
                            "tomid[" + RELATIONSHIP_ASSIGNED_AFFECTED_ITEM + "].fromrel[" + DomainConstants.RELATIONSHIP_ASSIGNED_EC + "].id");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectId - a String containing the Part id. reportType - a String either BOM or AVL. location - a String containing the Location
     *            id, only needed for AVL report.
    ...
    public MapList getEBOMs(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectList - a MapList of object information.
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: languageStr - a String continaing the language objectId - a String containing the Part id. slevels - a String containing the
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getConsolidatedEBOMs(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains packed HashMap with the following entries: objectList - a MapList of object information.
     * @return Vector of "level" values for each row.
     * @throws Exception
     *             if the operation fails.
     * @since AEF 10.0.0.0.
    ...
    public Vector getQuantity(Context context, String[] args) throws Exception {
        ...
        if (strReportFormat != null
                && (strReportFormat.indexOf("getExpandedEBOM") != -1 || strReportFormat.indexOf("getDelimitedRollupEBOM") != -1 || strReportFormat.indexOf("getRangeRollupEBOM") != -1)) {
            while (i.hasNext()) {
                Map m = (Map) i.next();
     ...
     * @since EC 10.6.
     */
    public Vector getSubstituteEBOMQuantity(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @since AEF 10.0.0.0.
     */
    public Vector getHistoryImage(Context context, String[] args) throws Exception {
    ...
    /**
     * Returns a Vector of "ECROActive" icons based on whether the part has an active ECR or ECO connected to it.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: paramList - a HashMap of parameter information. languageStr needs to be set for Tool Tip display. objectList - a MapList of
     *            object information.
     ...
     * @since AEF 10.0.0.0.
     */
    public Vector getECRECOImage(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        try {
            ...
            while (stringtokenizer.hasMoreTokens()) {
                stateName = FrameworkUtil.lookupStateName(context, POLICY_ECR, stringtokenizer.nextToken().trim());
            ...
            while (stringtokenizer.hasMoreTokens()) {
                stateName = FrameworkUtil.lookupStateName(context, POLICY_ECO, stringtokenizer.nextToken().trim());
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap with the following entries: objectId - a String containing the Part id. vault - a String containing the Vault name.
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectId - the Part id.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getManufacturerEquivalents(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectId - String of the context Part object id. selType - String of the Type to search. txtName - Strng of the Name to search.
     *            txtRev - String of the Revision to search. txtDesc - String of the Description to search. txtOwner - String of the Owner to search. txtOriginator - String of the Originator to
     *            search. revPattern - String of either ALL_REVISIONS, HIGHEST_REVISION or HIGHEST_AND_PRESTATE_REVS. queryLimit - String of the Query Limit. vaultOption - String of the Vault option
     *            to use.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getAllManufacturerEquivalents(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        ...
        try {
            ...
            mepList = findObjects(context, txtType, txtName, txtRev, txtOwner, txtVault, whereExpression, null, true, selectStmts, Short.parseShort(queryLimit));
        } catch (FrameworkException Ex) {
            throw Ex;
        }
        return mepList;
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectList - a MapList of object information. paramList - a HashMap of parameters.
     * @return a Vector of part origin values.
     * @throws Exception
     *             if the operation fails.
     * @since AEF 10.0.0.0.
     */
    public Vector getPartOrigin(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap with the following entries: objectId - the object id of the Manufacturer Equivalent Part.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getEnterpriseEquivalents(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        ...
        try {
            ...
            listMEPs = partObj.getRelatedObjects(context, sbRelPattern.toString(), // relationship pattern
            ...
            corpMEPs = partObj.getRelatedObjects(context, RELATIONSHIP_MANUFACTURER_EQUIVALENT, // relationship pattern
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap with the following entries: objectId - the object id of the context Part.
    ...
    public MapList getReferenceDocuments(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            refDocList = FrameworkUtil.toMapList(partObj.getExpansionIterator(context, RELATIONSHIP_REFERENCE_DOCUMENT, "*", selectStmts, selectRelStmts, true, true, (short) 1, null, null, (short) 0,
                    false, false, (short) 0, false), (short) 0, null, null, null, null);
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap of the following entries: selType - a String containing the Type to search for. txtName - a String containing the Name to search for. txtRev - a String
     *            containing the Revision to search for. txtOwner - a String containing the Owner to search for. txtWhere - a String containing the where clause to use in the search. txtOriginator - a
     *            String containing the Originator to search for. txtDesc - a String containing the Description to search for. txtSearch - a String containing the Search text for the query. txtFormat
     *            - a String containing the Format text for the query. languageStr - a String containing the language setting. setRadio - a String containing the Set name. revPattern - a String of
     *            either ALL_REVISIONS, HIGHEST_REVISION or HIGHEST_AND_PRESTATE_REVS. Vault - a String containing the Vault name to search in. queryLimit - a String containing the Query Limit value.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public Object getPartSearchResult(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        // Fix for bug --022268
        int limit = UISearchUtil.getQueryLimit(paramMap);
        ...
        if ("".equals(sSetName)) {
            if ("".equals(savedQueryName)) {
                try {
                    ...
                    if (index != -1) {
                        ...
                        // sWhereExp = sWhereExp.substring(0, index) + sQuote + subString + sQuote + sWhereExp.substring(lastIndex+1);
                    ...
                    try {
                        // IR-040943V6R2011
                        totalresultList = DomainObject.findObjects(context, selType, txtName, txtRev, txtOwner, txtVault, sWhereExp, null, true, resultSelects, Short.parseShort(queryLimit), txtFormat,
                                txtSearch);
                    } finally {
                        if (escape) {
                            ContextUtil.pushContext(context);
            ...
            totalresultList = SetUtil.getMembers(context, sSetName, resultSelects);
        }
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap of the following entries: paramList - a Map of parameter values. objectList - a MapList of object information.
     ...
     * @since 10.0.0.0.
     */
    public Vector getManufacturerLink(Context context, String[] args) throws Exception {
    ...
    /**
     * Checks that the Part being connected to the EBOM is in the State passed as an argument. An error message is displayed if the Part cannot be connected.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input parameters: 0 - a String containing the object id of the Part to connect. 1 - a String containing the symbolic name of the State.
     ...
     * @since 10.0.0.0.
     */
    public int CheckECPartToConnectAsEBOM(Context context, String[] args) throws Exception {
        String toId = args[0];
        String stateSymbolic = args[1];
        String stateRealName = PropertyUtil.getSchemaProperty(context, "policy", POLICY_PART, stateSymbolic);
        ...
        if (sPolicy != null && strPolicyClass.equalsIgnoreCase("production") && currentState.equals(stateRealName) && !selObj.canAttachECR(context)) {
            // end of fix
            String strError = selObj.getInfo(context, SELECT_TYPE) + " " + selObj.getInfo(context, SELECT_NAME) + " " + selObj.getInfo(context, SELECT_REVISION)
                    + EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.CheckIfECPartLatestReleasedRevision.Message1", context.getSession().getLanguage()) + " " + stateRealName + " "
                    + EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.CheckIfECPartLatestReleasedRevision.Message2", context.getSession().getLanguage());
    ...
    /**
     * Returns true if this Part is a Manufacturer Equivalent Part otherwise return false.
     * @param context
     *            the eMatrix <code>Context</code> object.
     ...
     * @since EC 10.0.0.0.
     */
    public boolean isMEP(Context context) throws Exception {
    ...
    /**
     * Notify all users with the Component Engineer Role that a Manufacturer Equivalent Part was created by someone other than a Component Engineer.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - a String containing the subject of the message. 1 - a String containing the message. 2 - a String containing the resouce bundle name.
     * @throws Exception
     *             if the operation fails.
     * @since EC 10.0.0.0.
     * @trigger TypePartCreateMEAction.
     */
    public void notifyComponentEngineerOfMEPCreate(Context context, String[] args) throws Exception {
        try {
            // args[] parameters
            ...
            if (isMEP(context)) {
                ...
                if (!origPerson.hasRole(context, sCompEngRoleName)) {
                    ...
                    emxMailUtil_mxJPO.sendNotification(context, toList, ccList, bccList, sSubject, null, null, sMessage, null, null, objectIdList, null, sResourceBundle);
                }
            }
        } catch (Exception ex) {
            throw ex;
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap with the following entries: objectList - a MapList of object information. paramList - a Map of parameter values, SuiteDirectory and suiteKey must be set..
     * @return Vector containing Equivalents Links.
     * @exception Exception
     *                if the operation fails.
     * @since AEF 10.5.
     */
    public Vector getEquivalentLink(Context context, String[] args) throws Exception {
            ...
            for (i = 0; i < listEquiv.size(); i++) {
                ...
                if (entIdList != null && entIdList.size() > 0) {
                    for (int j = 0; j < entIdList.size(); j++) {
                        ...
                        output.append("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?emxSuiteDirectory=" + suiteDir + "&suiteKey=" + suiteKey + "&objectId=" + entId
                                + "', '', '', 'false', 'popup', '')\">" + entName + " " + entRev + "</a> <br>");
                    }
                }
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap of the following entries: objectList - a MapList of object information. paramList - a Map of parameter values, reportFormat and exportFormat can be set.
     * @return Vector of Manufacturer Equivalent Parts.
     * @throws Exception
     *             if the operation fails.
     * @since 10.5.
     */
    public Vector getMEPart(Context context, String[] args) throws Exception {
     ...
     * @since 10.5.
     */
    public Vector getManufacturer(Context context, String[] args) throws Exception {
     ...
     * @since 10.5.
     */
    public Vector getLocation(Context context, String[] args) throws Exception {
     ...
     * @since 10.5.
     */
    public Vector getLocationStatus(Context context, String[] args) throws Exception {
     ...
     * @since 10.5.
     */
    public Vector getLocationPreference(Context context, String[] args) throws Exception {
     ...
     * @since 10-5.
     */
    public Vector getObjectFromAVLList(Context context, String[] args, String id) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
                        ...
                        if (partId != null && !("").equals(partId)) {
                            ...
                            if (TYPE_MPN.equals(type)) {
                                ...
                                if ("false".equalsIgnoreCase(showBlankName)) {
                                    partName = EnoviaResourceBundle.getProperty(context, "emxManufacturerEquivalentPartStringResource", context.getLocale(),
     ...
     * @since 10-5.
     */
    public Vector getAttributeFromAVLList(Context context, String[] args, String attName) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a HashMap of the following entries: objectId - a String containing the Enterprise Part id.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getEnterpriseManufacturerEquivalents(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) paramMap.get("objectId");
        ...
        try {
            ...
            listLocEquivMEPs = partObj.getRelatedObjects(context, sbRelPattern.toString(), // relationship pattern
                    sbTypePattern.toString(), // object pattern
                    selectStmts, // object selects
                    selectRelStmts, // relationship selects
                    false, // to direction
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a HashMap of the following entries: objectList - a MapList of object information. paramList - a Map of parameter values, SuiteDirectory, suiteKey, reportFormat, publicPortal.
     * @return a StringList HTML output of Names of Location(s).
     * @throws Exception
     *             if the operation fails.
     * @since 10.5.
     */
    public StringList getMEPLocationNamesHTMLOutput(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (objectList != null && objectList.size() > 0) {
                ...
                String manEquAloResNameSel = "to[" + RELATIONSHIP_MANUFACTURER_EQUIVALENT + "].from.to[" + RELATIONSHIP_ALLOCATION_RESPONSIBILITY + "].from.name";
                String manEquAloResIdSel = "to[" + RELATIONSHIP_MANUFACTURER_EQUIVALENT + "].from.to[" + RELATIONSHIP_ALLOCATION_RESPONSIBILITY + "].from.id";
    ...
    /**
     * Gets the Location Status attribute value on Allocation Responsibility relationship between Location(s) and an MEP.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a HashMap of the following entries: objectList - a MapList of object information. paramList - a Map of parameter values, SuiteDirectory, suiteKey.
     * @return a StringList HTML output of Status attribute value.
     * @throws Exception
     *             if the operation fails.
     * @since 10.5.
     */
    public StringList getMEPLocationStatusHTMLOutput(Context context, String[] args) throws Exception {
        StringList result = new StringList();
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            ...
            if (objectList != null && objectList.size() > 0) {
                ...
                // Part --> Manufacture Equivalent --> Allocation Responsibility
                String manEquAloResStaSel = "to[" + RELATIONSHIP_MANUFACTURER_EQUIVALENT + "].from.to[" + RELATIONSHIP_ALLOCATION_RESPONSIBILITY + "].attribute[" + ATTRIBUTE_LOCATION_STATUS
                        + "].value";

                // Location Status select via the relationship route
                // Part --> Allocation Responsibility
                String aloResStaSel = "to[" + RELATIONSHIP_ALLOCATION_RESPONSIBILITY + "].attribute[" + ATTRIBUTE_LOCATION_STATUS + "].value";
    ...
    /**
     * Gets the Location Preference attribute value on Allocation Responsibility relationship between Location(s) and MEP.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a HashMap of the following entries: objectList - a MapList of object information. paramList - a Map of parameter values, SuiteDirectory, suiteKey.
     ...
     * @since 10.5.
     */
    public StringList getMEPLocationPreferenceHTMLOutput(Context context, String[] args) throws Exception {
        StringList result = new StringList();
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            ...
            if (objectList != null && objectList.size() > 0) {
                ...
                // Part --> Manufacture Equivalent --> Allocation Responsibility
                String manEquAloResPreSel = "to[" + RELATIONSHIP_MANUFACTURER_EQUIVALENT + "].from.to[" + RELATIONSHIP_ALLOCATION_RESPONSIBILITY + "].attribute[" + ATTRIBUTE_LOCATION_PREFERENCE
                        + "].value";

                // Location Preference select via the relationship route
                // Part --> Allocation Responsibility
                String aloResPreSel = "to[" + RELATIONSHIP_ALLOCATION_RESPONSIBILITY + "].attribute[" + ATTRIBUTE_LOCATION_PREFERENCE + "].value";
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap with the following entries: objectId - a String containing the Manufacturer Equivalent object id.
     * @return a MapList of Location information.
     * @throws Exception
     *             if the operation fails.
     * @since 10.5.
     */
    public MapList getMEPLocations(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) paramMap.get("objectId");
        ...
        try {
            ...
            // (Location Equiv MEP)
            locEquivMEPLocList = getRelatedObjects(context, sbRelPattern.toString(), // relationship pattern
                    sbTypePattern.toString(), // object pattern
                    selectStmts, // object selects
                    selectRelStmts, // relationship selects
                    true, // to direction
            ...
            // (Corporate MEP)
            corpMEPLocList = getRelatedObjects(context, RELATIONSHIP_ALLOCATION_RESPONSIBILITY, // relationship pattern
     ...
     * @since 10.5.
     */
    public StringList getMEPEnterpriseParts(Context context, String[] args) throws Exception {
        StringList result = new StringList();
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        MapList objectList = (MapList) paramMap.get("objectList");
                ...
                if (entIdList != null && entIdList.size() > 0) {
                    for (int j = 0; j < entIdList.size(); j++) {
                        ...
                        // length will be >0 when format is HTML, ExcelHTML, CSV or TXT
                            ...
                            output.append("<a href=\"JavaScript:emxTableColumnLinkClick('../common/" + linkFile + "?emxSuiteDirectory=" + suiteDir + "&suiteKey=" + suiteKey + "&objectId=" + entId
                                    + "', '', '', 'false', 'popup', '')\">" + entName + " " + entRev + "</a> <br>");
                        }
                    }
                }
    ...
    /**
     * Copy over all substitutes from a part. All connections found will be copied to the other Part. It is assumed the next revision is meant to be the target.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
     ...
     * @trigger TypePartReviseAction
     */
    public void copySubstitutes(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            _substitutePartList = boPrev.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, TYPE_PART, strObjList, relsel, false, true, (short) 1, "", "");
            ...
            while (finalsubstitutePartListitr.hasNext()) {
                ...
                String sEBOMSubRelID = (String) relmap1.get(DomainRelationship.SELECT_ID);
                ...
                _nextEBOMList = boNext.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, TYPE_PART, strObjList, relsel, false, true, (short) 1, "", sbRelWhereCond.toString());
                ...
                String strnextMessage = MqlUtil.mqlCommand(context, "add connection $1 fromrel $2 to $3 select $4 dump", RELATIONSHIP_EBOM_SUBSTITUTE, snextEBOMID, SubId, SELECT_ID);
     ...
     * @param inMapList
     *            holds the MapList for the corresponding MEP.
     * @param key
     *            holds the String to be checked for.
     * @return true if the MEP has 2 or more rows, otherwise return false.
     * @throws Exception
     *             if the operation fails.
     * @since 10.5.
     */
    public boolean isMultipleRows(MapList inMapList, String key) throws Exception {
        boolean flag = false;
        String oId = "";
        int ct = 0;
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - String containing the parent object id.
     * @return void
     * @throws Exception
     *             if the operation fails
     * @since EC 10.5
    ...
    public void resetOwner(Context context, String[] args) throws Exception {
        try {
            ...
            for (int i = historyData.size() - 1; i >= 0; i--) {
                ...
                if (historyRecord.startsWith("promote") && (historyRecord.endsWith(STATE_PART_APPROVED) || historyRecord.endsWith(STATE_DRAWINGPRINT_APPROVED)
                        || historyRecord.endsWith(STATE_CADDRAWING_APPROVED) || historyRecord.endsWith(STATE_CADMODEL_APPROVED) || historyRecord.endsWith(STATE_PART_SPECIFICATION_APPROVED))) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: requestMap - a HashMap of the request. paramMap - a HashMap of containing String values for, "objectId", "Old value", "New OID".
     * @return Object - boolean true if the operation is successful
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Object setECO(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        if (!"".equals(oldECOName) && (newEcoId != null && !"".equals(newEcoId))) {
            ...
            while (ecos.hasMoreTokens()) {
                ...
                ecoList = getRelatedObjects(context, relPattern.getPattern(), strECOType, ObjectSelectsList, RelSelectsList, true, true, (short) 1, strWhereCondition, DomainConstants.EMPTY_STRING);

                // 364525 - Starts
                if (ecoList != null && !ecoList.isEmpty()) {
                    int iSize = ecoList.size();
                    for (int index = 0; index < iSize; index++) {
                        String ecoId = (String) ((Hashtable) ecoList.get(index)).get(DomainConstants.SELECT_ID);
                ...
                BusinessObject.grantAccessRights(context, ecoObjects, accessMask);
                ContextUtil.popContext(context);
                blnFlag = true;
                // 364525 - Ends
                if (ecoList != null) {
                    String strRelId = (String) ((Hashtable) ecoList.get(0)).get(DomainConstants.SELECT_RELATIONSHIP_ID);
        ...
        if (!"".equals(newEcoId)) {
            ...
            if (revId != null) {
                ...
                String revRelease = PropertyUtil.getSchemaProperty(context, "policy", revPolicy, "state_Release");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap of the following entries: paramMap - a HashMap containing String values for "objectId", "languageStr". This Map contains the arguments passed to the jsp which called
     *            this method.
     ...
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Object getMepPartOrigin(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap of the following entries: paramMap - a HashMap containing String values for "objectId", "Old value" and "New OID".
     * @return Object - boolean true if the operation is successful
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10-6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Object updateManufacturer(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        String strPartId = (String) paramMap.get("objectId");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap with the following entries: objectId - a String containing the Part id.
     ...
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Boolean isEnterprisePart(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String strPartId = (String) programMap.get("objectId");
     ...
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Boolean checkPartType(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String strPartId = (String) programMap.get("objectId");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap with the following entries: objectId - a String containing the Part id. languageStr - s String containing the language setting.
     * @return Object - boolean true we are to allow change management for the Part.
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Boolean checkTypeChangeMgmt(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap with the following entries: mode - a String containing the mode.
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap with the following entries: paramMap - a HashMap containing the following Strings, objectId, languageStr.
     * @return Object - String object "Yes" if connected otherwise String object "No"
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Object displayActiveECRorECO(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a HashMap containing the following entries: objectList - a MapList of ECO information. paramList - a Map of parameter values containing SuiteDirectory, languageStr, objectId.
     * @return StringList of Edit ECO urls.
     * @throws Exception
     *             if the operation fails.
     * @since AEF 10.0.0.0.
     */
    public StringList getEcoDetails(Context context, String[] args) throws Exception {
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (strReptFormat == null || "null".equals(strReptFormat)) {
                    String modifyURL = "../engineeringcentral/emxpartECOAttributesDialogFS.jsp?objectId=" + partId + "&selectedECOId=" + sECOId + "&Mode=Modify" + "&RelationshipType=" + sRelName
                            + "&sRelId=" + sRelId;
                    modifyURL = "<a href=\"JavaScript:emxTableColumnLinkClick('" + modifyURL
                            + "', '700', '600', 'true', 'popup', '')\"><img src=\"../common/images/iconActionEdit.gif\" border=\"0\"/></a>";
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments as it is getting the params list,objects list from the Config UI table
     * @returns Vector of "true/false" values for each row
     * @throws Exception
     *             if the operation fails
     * @since Common 10.6
     */
    public Vector showCheckBoxInEco(Context context, String[] args) throws Exception {
        try {
            ...
            if (objectList != null && (listSize = objectList.size()) > 0) {
                while (itr.hasNext()) {
                    ...
                    if (FrameworkUtil.hasAccess(context, part, "toDisconnect") && FrameworkUtil.hasAccess(context, ECOObj, "fromDisconnect")) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: objectList - a MapList of object information. paramList - a Map of parameter values containing SuiteDirectory, suiteKey, objectId,
     *            reportFormat.
     ...
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public StringList getDispCodesURL(Context context, String[] args) throws FrameworkException {
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            MapList objectList = (MapList) paramMap.get("objectList");
            ...
            while (itr.hasNext()) {
                ...
                String modifyURL = "../engineeringcentral/emxengchgECRRelAttributesFS.jsp?busId=" + sECRId + "&objectId=" + sECRId + "&changeType=" + sRelName + "&Type=ECR" + "&Name=" + sECRName
                        + "&Rev=" + sECRRev + "&sChildToken=" + sRelId + "&selectedType=" + objectId + "&rSelectComponent=" + tempStr + "&hChildToken=" + sRelId + "&rndNum=" + rndNum;
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectId - a String of the Part id.
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectList - a MapList of object information. paramList - a Map of parameter values objectId - a String of the Part id.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getSubstitutePart(Context context, String[] args) throws Exception {
        try {
            MapList _substitutePartList = null;
            MapList _finalsubstitutePartList = new MapList();
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            ...
            _substitutePartList = domObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, TYPE_PART, strObjList, relsel, false, true, (short) 1, "", "");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains: actual name of Attribute objectList - a MapList of object information.
     * @return Vector of "attribute values for each row.
     * @throws Exception
     *             if the operation fails.
     * @since EC X3.
     */
    public Vector getAttributeOnEBOMRel(Context context, String sAttribute, MapList objectList) throws Exception {
     ...
     * @since EC X3.
     */
    public Vector getPrimaryFindNumber(Context context, String[] args) throws Exception {
        Vector columnVals = new Vector();
     ...
     * @since EC X3.
     */
    public Vector getPrimaryReferenceDesignator(Context context, String[] args) throws Exception {
        Vector columnVals = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objList = (MapList) programMap.get("objectList");
     ...
     * @since EC X3.
     */
    public Vector getPrimaryEBOMQty(Context context, String[] args) throws Exception {
        Vector columnVals = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objList = (MapList) programMap.get("objectList");
            String srealAttrQtyName = PropertyUtil.getSchemaProperty(context, "attribute_Quantity");
            columnVals = getAttributeOnEBOMRel(context, srealAttrQtyName, objList);
        } catch (Exception ex) {
            throw ex;
        }
        return columnVals;
     ...
     * @since EC X3.
     */
    public Vector getPrimaryForSubstitute(Context context, String[] args) throws Exception {
        Vector columnVals = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objList = (MapList) programMap.get("objectList");
            ...
            while (i.hasNext()) {
                ...
                if (reportFormat != null && !"null".equals(reportFormat) && (reportFormat.length() > 0) || isexport) {
                    ...
                    output.append("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?objectId=" + XSSUtil.encodeForURL(context, sPartid) + "', '700', '600', 'false', 'popup', '')\">"
                            + XSSUtil.encodeForHTML(context, sPartName) + "</a> <br> </br>");
                }
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains following entries: What to get for the primary part objectList - a MapList of object information.
     * @return Vector of required iformation for each row.
     * @throws Exception
     *             if the operation fails.
     * @since EC X3.
     */
    public Vector getPrimartPartInfo(Context context, String getWhat, MapList objectList) throws Exception {
        Vector columnVals = new Vector();
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                if ("type".equals(strgetWhat)) {
                    ...
                    strtype = i18nNow.getAdminI18NString("Policy", (String) map.get("to.policy"), context.getSession().getLanguage());
                }
     ...
     * @since EC X3.
     */
    public Vector getPrimaryComponentType(Context context, String[] args) throws Exception {
        Vector columnVals = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objList = (MapList) programMap.get("objectList");
     ...
     * @since EC X3.
     */
    public Vector getPrimaryRev(Context context, String[] args) throws Exception {
        Vector columnVals = new Vector();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objList = (MapList) programMap.get("objectList");
     ...
     *             if the operation fails.
     */
    public Vector getPrimaryComponentPolicy(Context context, String[] args) throws Exception {
    ...
    /**
     * Checks that the Specification which is going to be connected to Part is having states "Review", "Approved" and "Released". If any spec object not having these states, it cannot be added as
     * "Part Specification". Also, "Review" should not be the first state.
     * @param context
     *            the eMatrix <code>Context</code> object.
    ...
    /*
     * This method returns a Vector contains the modified Find Numbers . It appends zeros according to the property key configuration and displays. The change is display only no updation of db is
     * done.
     * 
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getExpandedEBOM(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String langStr = (String) paramMap.get("languageStr");
        ...
        if ((reportFormat != null) && (!"expandedFormat".equalsIgnoreCase(reportFormat)) && (!"getExpandedEBOM".equalsIgnoreCase(reportFormat))) {
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getDelimitedRollupEBOM(Context context, String[] args) throws Exception {
        Compare c = new Compare();
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String langStr = (String) paramMap.get("languageStr");
        String reportFormat = (String) paramMap.get("repFormat");// IR-044467
        ...
        // 375650: reverting changes of 369074 for reports
        // below code Added for--044467
        if ((reportFormat != null) && (!"delimitedRollUpFormat".equalsIgnoreCase(reportFormat)) && (!"getDelimitedRollupEBOM".equalsIgnoreCase(reportFormat))) {
            ebomList.addSortKey("id", "ascending", "String");
            ebomList.sort();
        }
        String delimiter = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentral.DelimitedReferenceDesignatorSeparator");
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getRangeRollupEBOM(Context context, String[] args) throws Exception {
        Compare c = new Compare();
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String multilevelReport = (String) paramMap.get("multilevelReport");
        String reportFormat = (String) paramMap.get("repFormat");// IR-044467
        ...
        // 375650: reverting changes of 369074 for reports
        // below code Added for--044467
        if ((reportFormat != null) && (!"rangeRollUpFormat".equalsIgnoreCase(reportFormat)) && (!"getRangeRollupEBOM".equalsIgnoreCase(reportFormat)))
     ...
     * @since EC 10.6
     */
    public StringList getReferenceDesignator(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        ...
        while (i.hasNext()) {
            ...
            // Added for Bug 354293
            if (format.equalsIgnoreCase("expandedFormat") || format.equalsIgnoreCase("delimitedRollUpFormat") || format.equalsIgnoreCase("rangeRollUpFormat")
                    || format.equalsIgnoreCase("getExpandedEBOM") || format.equalsIgnoreCase("getDelimitedRollupEBOM") || format.equalsIgnoreCase("getRangeRollupEBOM"))
     ...
     * @since EC 10.6
     */
    public StringList getComponentLocation(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) paramMap.get("objectList");
        StringList columnVals = new StringList();
        Iterator i = objList.iterator();
     ...
     * @since EC 10.6
     */
    public StringList getUsage(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) paramMap.get("objectList");
        StringList columnVals = new StringList();
        Iterator i = objList.iterator();
     ...
     * @since EC 10.6
     */
    public StringList getEBOMQuantity(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) paramMap.get("objectList");
        StringList columnVals = new StringList();
        Iterator i = objList.iterator();
     ...
     * @param String
     *            the Reference Designator token
     * @param String
     *            expected prefix
     ...
     * @param StringList
     *            List of RDs in different formats
     ...
     * @param String
     *            Reference Designator values in different formats
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds param arguments.
     * @throws Exception
     *             if the operation fails.
     * @since EC 10.6
     */
    public Vector getEditColumnDisplay(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
        HashMap paramList = (HashMap) programMap.get("paramList");
            ...
            if (selectedFilter != null) {
                if (selectedFilter != null && selectedFilter.equals("emxPart:getEBOMsWithRelSelectables") && selectedProgram == null) {
                    if (!isPrinterFriendly) {
                        columnVals.addElement("<a href=\"javascript:emxTableColumnLinkClick('../engineeringcentral/emxpartEditEBOMDialogFS.jsp?emxSuiteDirectory=engineeringcentral&amp;relId=" + relId
                                + "&amp;suiteKey=EngineeringCentral&amp;parentOID=" + parentId + "&amp;objectId=" + id
                                + "', '700', '600', 'true', 'popup', '')\"><img border=\"0\" src=\"images/iconActionEdit.gif\" alt=\"\"/></a>");
                    } else {
    ...
    /**
     * This class extends the comparator class and implements the compare method. If prefixes are equal it performs the integer comparision on suffixes. If prefixes are not equal then it compares the
     * length of the prefixes. returns -1 if first object and second objects in ascending order else 1. This method is called when both objects prefixes starts with the same letter.
     * @since EC 10.6
     */
    ...
    /**
     * This method enables or disables the checkbox in the EBOM Summary Page.If Selected Filter is not Stored, checkbox will be disabled.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments as it is getting the params list,objects list from the Config UI table
    ...
    public MapList getEBOMDataForChangePosition(Context context, String[] args) throws Exception {
        ...
        if (isValidData(parentId) && isValidData(selectedId)) {
            ...
            ebomList = domObj.getRelatedObjects(context, RELATIONSHIP_EBOM, TYPE_PART, new StringList(SELECT_ID), new StringList(SELECT_RELATIONSHIP_ID), false, true, (short) 1, objectWhere, null,
                    null, null, null);
        }
        ...
        try {
            ...
            while (strItr.next()) {
                ...
                if (!"Latest Complete".equals(sActualValue)) {
                    ...
                    if (sPolicy.equals(DomainConstants.POLICY_DEVELOPMENT_PART)) {
                        slActualValue.add(sActualValue);
                        sDisplayValue = sActualValue.replace(' ', '_');
                        // Multitenant
                        sDisplayValue = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),
    ...
    /**
     * getBOMViewFilters,this method is executed to show the BOM Views like Engineering, Common and Plant Specific Views.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap
     ...
     *             if the operation fails.
     */
    public HashMap getBOMViewFilters(Context context, String args[]) throws Exception {
        HashMap ViewMap = new HashMap();
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        ...
        for (int i = 0; i < slBOMFilterProp.size(); i++) {
            ...
            // Multitenant
            String strBom1Filter = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),
                    "emxEngineeringCentral.Filter.BOMFilterOptions." + strBomFilter);
     ...
     * @param args
     *            0 - relId - relationship ID
     * @param args
     *            1 - objectId - part object OID
     * @param args
     *            2 - parentOID - parent object ID
    ...
    public MapList resequenceBOM(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            DomainObject domObj = DomainObject.newInstance(context, objectId);
            ebomList = domObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, // relationship pattern
                    DomainConstants.TYPE_PART, // object pattern
                    selectStmts, // object selects
                    selectRelStmts, // relationship selects
                    false, // to direction
     ...
     *            2 - parentOID - parent object ID
     * @param args
     *            3 - radioPartObjectId - selected Part in Dialog Page
     * @returns MapList EBOM list after resequencing Find numbers.
     * @throws Exception
     *             if the operation fails
     * @since version X+2
    ...
    public MapList changeBOMPartPosition(Context context, String[] args) throws Exception {
     ...
     *            holds the following input arguments:
     * @param args
     *            0 - fromObject - 'From' Part Object ID
     * @param args
     *            1 - toObject - 'To' Part Object ID
     * @param args
     *            2 - relType - Relationship Type
     * @returns MapList EBOM list after resequencing Find numbers.
     * @throws Exception
     *             if the operation fails
     * @since version X+2
    ...
    public void connectPartToBOM(Context context, String[] args) throws Exception {
     ...
     * @param args
     *            selPartObjectId - Selected part in BOM
     * @param args
     *            createdPartObjId - New part created
     * @param args
     *            parentOID - Parent ObjectId
     * @param args
     *            partFamilyContextId - part Family Context Id
     * @param args
     *            radioOption - radio button Option selected
     * @returns MapList EBOM list after resequencing Find numbers.
     * @throws Exception
     *             if the operation fails
     * @since version X+2
     */
    public void replacePartinBOM(Context context, String[] args) throws Exception {
     ...
     *            holds the following input arguments:
     * @param args
     *            selPartRelId - Relationship ID
     * @param args
     *            selObjectId - Selected part in BOM
     * @param args
     *            selPartObjId - New part created
     * @param args
     *            selPartParentOId - Parent ObjectId
     * @param args
     *            objectId - context part Object Id
     * @param args
     *            vSelIds - selected parts in search page
     * @returns void
     * @throws Exception
     *             if the operation fails
     * @since version X+2
     */
    public void addNextinBOM(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (ebomList != null && !"null".equals(ebomList)) {
                ...
                while (itr.hasNext()) {
                    ...
                    if ((ctxPolicy.equals(DomainConstants.POLICY_DEVELOPMENT_PART) && !"Complete".equals(state))
                            || (ctxPolicy.equals(DomainConstants.POLICY_EC_PART) && state.equals(DomainConstants.STATE_PART_PRELIMINARY))) {
     ...
     * @since version X+2
     */
    public void copyPartFromBOM(Context context, String[] args) throws Exception {
    ...
    /**
     * Gets the Multi-Level EBOM Summary information with EBOM relation selectables. This method can also be used to get the AVL report by specifying reportType as AVL
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectId - a String containing the Part id. relationship - a String containing the name of the relationship to expand on. This
     *            can be "EBOM" or "EBOM,EBOM History". reportType - a String specifying either BOM or AVL. location - a String containing the id of the Location, only needed for AVL Report. level - a
     *            String containing the number of levels to expand.
     * @return MapList of EBOM Objects containing EBOM object id, ECR state, ECO state, level, relationship id, relationship type.
     * @throws Exception
     *             if the operation fails.
     * @since AEF 10.6.
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getMultiLevelEBOMsWithRelSelectables(Context context, String[] args) throws Exception {
     ...
     * @since 10-6.
     */
    public StringList getMEPNames(Context context, String[] args) throws Exception {
        StringList result = new StringList();
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @since 10-6.
     */
    public StringList getMEPTypes(Context context, String[] args) throws Exception {
        StringList result = new StringList();
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @since 10-6.
     */
    public StringList getMEPRevisions(Context context, String[] args) throws Exception {
        StringList result = new StringList();
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @since 10-5.
     */
    public MapList getEnterpriseValidManufacturerEquivalents(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @since 10-6.
     */
    public Boolean isNewWindowViewable(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if ((launched != null && launched.equalsIgnoreCase("true")) && (portalMode != null && portalMode.equalsIgnoreCase("false"))) {
    ...
    public int updatePartRevision(Context context, String[] args) throws Exception {
        ...
        if (isMep && !"".equals(strRevValue) && !strRev.equals(strRevValue)) {
            String strType = (String) map.get(SELECT_TYPE);
            String strName = (String) map.get(SELECT_NAME);
            MapList mapList = DomainObject.findObjects(context, strType, strName, strRevValue, QUERY_WILDCARD, QUERY_WILDCARD, "", false, null);
            if (mapList == null || mapList.size() == 0) {
                (new com.matrixone.apps.common.EngineeringChange()).setRevision(context, strPartId, strRevValue, strName);
            } else {
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectId of Part. mode of webform
     * @return a String : connected Part Family -->Part as HTMLOutput.
     * @throws Exception
     *             if the operation fails.
     * @since 10.6.
     */
    public String getPartFamilyListHTMLOutput(Context context, String[] args) throws Exception {
        StringBuffer output = new StringBuffer();
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap requestMap = (HashMap) programMap.get("requestMap");
            ...
            MapList mapList = partObj.getRelatedObjects(context, RELATIONSHIP_CLASSIFIED_ITEM, // relationship pattern
            ...
            if (strMode != null && "edit".equalsIgnoreCase(strMode)) {
                ...
                output.append("<input type='text'  maxlength=\"\" readonly='readonly' size='20' name=\"partFamilyDisplay\" id='' value=\"" + strpartFamilyName + "\">");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing String values for "objectId", "Old value" and "New OID".
     * @return Object - boolean true if the operation is successful.
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2004, MatrixOne, Inc.
     */
    public Object connectPartFamily(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        String strPartId = (String) paramMap.get("objectId");
        ...
        MapList partFamilyList = partObj.getRelatedObjects(context, RELATIONSHIP_CLASSIFIED_ITEM, // relationship pattern
                TYPE_PART_FAMILY, // object pattern
                selectStmts, // object selects
    ...
    public MapList getStoredEBOM(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            /*
             * if(UIUtil.isNotNullAndNotEmpty(sMatchBasedOn)){ if("Find Number".equals(sMatchBasedOn)){ tList.sort(EngineeringConstants.SELECT_FIND_NUMBER, "ascending", "integer"); } if(
             * "Reference Designator".equals(sMatchBasedOn)){ tList.sort("attribute["+DomainConstants.ATTRIBUTE_REFERENCE_DESIGNATOR+"]", "ascending", "real"); } }
             */
            ebomList.clear();
            ebomList.addAll(tList);
            // 369074
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing String values for "relId" and "New Value".
    ...
    public Boolean BOMCompareupdateFindNumber(Context context, String[] args) throws Exception {
        ...
        if (UIUtil.isNullOrEmpty(sErrorMsg) || null != relType && !"null".equalsIgnoreCase(relType) && !"".equals(relType) && !relType.equalsIgnoreCase(EngineeringConstants.RELATIONSHIP_ALTERNATE)
                && CurrentState.equals(DomainConstants.STATE_PART_PRELIMINARY)) {
            domRel.setAttributeValue(context, DomainConstants.ATTRIBUTE_FIND_NUMBER, newFindNumberValue);
            return Boolean.TRUE;
        } else {
    ...
    public String getAppFields(Context context, String[] args) throws Exception {
        ...
        if (mode.equalsIgnoreCase("edit")) {
            ...
            returnString.append(
                    "<input type=\"button\" value=\"...\" name=\"\" onclick=\"Javascript:showChooser('../engineeringcentral/emxEngrPersonSearchDialogFS.jsp?form=editDataForm&field=OwnerDisplay', 550, 550);\"> </td>");
            ...
            returnString.append(
                    "<input type=\"button\" value=\"...\" name=\"\" onclick=\"Javascript:showChooser('../engineeringcentral/emxpartRDOSearchDialogFS.jsp?form=editDataForm&field=RDODisplay&fieldId=RDO&searchLinkProp=SearchRDOLinks', 550,500,false);\">");
    ...
    public int checkIfPartRevisionExists(Context context, String args[]) throws Exception {
        ...
        if (busList == null || busList.size() == 0) {
            ...
            String strMessage = MqlUtil.mqlCommand(context, "execute program $1 -method $2 $3 $4 $5 $6 $7 $8 $9 $10", "emxMailUtil", "getMessage",
                    "emxFramework.ProgramObject.eServiceValidRevisionChange_if.NoCreate", "3", "Type", type, "Name", name, "Rev", rev);
    ...
    /**
     * This method is used while connecting or disconnecting the RDOs in Part Revise functionality .and Go To Production functionality in case of Development Part.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, "objectId", "old RDO Ids", "New OID".
     * @return Object - boolean true if the operation is successful
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral 10.6 - Copyright (c) 2005, MatrixOne, Inc.
     */
    public Object connectRDOsForRevise(Context context, String[] args) throws Exception {
    ...
    public String getPolicyClassificationPolicies(Context context, String[] args) throws Exception {
                ...
                if (hasChangePolicyAccess) {
                    ...
                    while (partPolicyItr.next()) {
                        ...
                        if (policyClassification.equalsIgnoreCase(currentPolicyClassification)) {
                            ...
                            returnString.append("<option value=\"" + partPolicyName + "\" " + (currentPartPolicyName.equals(partPolicyName) ? "selected=\"true\"" : "") + ">"
                                    + i18nNow.getAdminI18NString("Policy", partPolicyName, languageStr) + "</option>");
    ...
    /**
     * This method is used to display the Yield command for below types and its sub types Probed Wafer, Performance Graded Wafer, Unit Test, UT Performance Grade, UT Burn-In
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, "objectId", "old RDO Ids", "New OID".
     ...
     * @since EC 10.6.SP2
     */
    public Object showYieldData(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object
     * @param childType
     *            The child business type that needs to be checked
     * @param parentType
     *            The parent business type under which the child needs to be checked for being derived
     * @return booelan true if the child type is a derived type of the parent. booelan false if the child type is not
     * @throws Exception
     *             if operation fails
     * @since EC 10.6.SP2
     */
    public static boolean isOfParentType(Context context, String childType, String parentType) throws FrameworkException {
        try {
     ...
     * @mx.whereUsed This method will be called when the user clicks on the "Manufacturer Location" field from Part Edit Details property page.Used in "edit" or "view" mode of "Part" Webform. This
     *               field will be displayed for MEP Part only.
     * @mx.summary This method will display the location name that are existing on the "To" side of the relationship "Manufacturing Location" associated with selected Manufacturer in property page. In
     *             Edit Details Page, it will show the chooser button that will allow the user to choose a location associated with selected Manufacturer
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectId of Part.
     ...
     * @mx.whereUsed This method will be called for the "Manufacturer Location" field when the user clicks on the "Done" from "Edit Details" page. Used in "Edit" mode of "Part" WebForm.This field will
     *               be displayed for MEP Part only
     * @mx.summary This method will connect selected location with newly created MEP with relationship "Manufacturing Location".
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectId of Part. mode of form
                ...
                if (sExistManufacturerLocationId != null && !"null".equalsIgnoreCase(sExistManufacturerLocationId) && !"".equalsIgnoreCase(sExistManufacturerLocationId)) {
                    ...
                    if (sNewManufacturerLocationId != null && !"null".equalsIgnoreCase(sNewManufacturerLocationId) && !"".equalsIgnoreCase(sNewManufacturerLocationId)) {
                        if (!sExistManufacturerLocationId.equals(sNewManufacturerLocationId)) {
                            ...
                            DomainRelationship.connect(context, partObj, // the Part object to connect 'From'
     ...
     * @mx.whereUsed This method will be called when the user clicks on the "Manufacturer Equivalent Parts" link from My Desk -->Engineering menu.Used "ENCManufacturerEquivalentParts" table.
     * @mx.summary This method will return the list of location name that are existing on the "To" side of the relationship "Manufacturing Location" associated with selected Manufacturer in the
     *             summary page.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     ...
     * @mx.whereUsed This method will be called from part list pages. Used as access program to show/hide the compliance definition columns added to EC tables in EC install. It used in following
     *               tables mentioned below :ENCPartSearchResult, ENCGeneralSearchResult, ENCEBOMIndentedSummary, ENCEBOMSummary, ENCManufacturerEquivalentParts.
     * @mx.summary This method is for showing the Compliance columns only in the EC table if the respective Compliance Definitions exists in the system.The logic in the access program for each column
     *             would be : First Check whether MCC is installed using the FrameworkUtil.isSuiteRegistered(context,"appVersionMaterialsComplianceCentral",false,null,null)method, if MCC is installed
     *             then check whether the context Compliance Definition does exists in the system by doing a find object with type as 'Compliance Definition' and name as Compliance Definition name
     *             passed in the Applies To setting. If all above conditions are met then returns "true" else "false". Example : For "RoHS" column,following is the setting: Applies To = RoHS Then used
     *             DomainObject.findObjects() which takes Type: "Compliance Definition" Name: "RoHS" Revision:"*" to search for compliance definition exists in the system or not.
     * @param context
     *            the eMatrix <code>Context</code> object.
    ...
    public boolean hasAccessToComplianceDefinition(Context context, String[] args) throws Exception {
        if (isMCCInstalled(context, args)) {
            ...
            MapList finalList = DomainObject.findObjects(context, sComplianceDefinitionType, sComplianceDefinitionName, "*", "*", "*", "", true, objSelects);
     ...
     * @mx.summary This method check whether MCC is installed or not, this method can be used as access program to show/hide the compliance definition columns added to EC tables in EC install. Given
     *             below is a code: FrameworkUtil.isSuiteRegistered(context,"appVersionMaterialsComplianceCentral",false,null,null)
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @return boolean true or false based condition.
     * @throws Exception
    ...
    // EC-MCC Interoperability
    /**
     * Method to associate External interface to the equivalent part and set the "Enable Compliance" attribute as "Enabled".
     * @mx.whereUsed Invoked by the trigger object on creation of Part- TypePartCreateAction,
     * @mx.summary This method associates External Part Data interface to a Equivalent part. Uses <code>MqlUtil.mqlCommand</code> to associate the interface to the part. Uses
     *             <code>DomainObject.setAttribute() </code>to set an attribute value
     * @param context
     *            the eMatrix <code>Context</code> object
     ...
     * @throws Exception
     *             if the operation fails. Since EC X3
     */
    public Object getCompareType(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @throws Exception
     *             if the operation fails. Since EC X3 R210 - Modified for ECC Reports
     */
    public Object getRevisionOptions(Context context, String[] args) throws Exception {
        ...
        if ("RevisionOption1".equals(sFieldName)) {
            if (isConfigPart1) {
                ...
                for (int i = 0; i < iCountTokens; i++) {
                    ...
                    // Multitenant
                    sTokenName = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", new Locale("en"),
                            "emxEngineeringCentral.BOMCompareForm.RevisionOptions." + sTokenTemp);
                    // Getting i18n string for tokens
                    // Multitenant
                    sTokenVal = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),
                            "emxEngineeringCentral.BOMCompareForm.RevisionOptions." + sTokenTemp);
     ...
     * @throws Exception
     *             if the operation fails. Since EC X3 R201 - Modified for ECC Reports.
     */
     ...
     *             if the operation fails. Since EC X3
     */
    public Object getReportDifferences(Context context, String[] args) throws Exception {
        MapList columns = null;
        Map TableMap = null;
        Map TableSettingMap = null;
        ...
        UITableCommon uiTable = new UITableCommon();
        String strEBOMMBOMVisualReportTable = PropertyUtil.getSchemaProperty(context, "table_ENCBOMCompareVisualTableReport");
        columns = uiTable.getColumns(context, strEBOMMBOMVisualReportTable, null);
        int MapListSize = columns.size();
        int cnt = 0;
        ...
        if (MapListSize > 0) {
            ...
            for (int i = 0; i < MapListSize; i++) {
                ...
                if (strFieldLabel != null && !"null".equals(strFieldLabel)) {
                    ...
                    if ("true".equalsIgnoreCase(strComparable) && (strFieldValue.equals(strName) || strFieldValue.equals(strRevision) || strFieldValue.equals(strQty))) {
                        cnt++;
                        isField = true;
                        strBuf.append("<td>&nbsp;<input type=\"checkbox\" id=\"repDiffChk_default\" checked name=\"" + sVal + "\" value=\"true\"/>" + "   " + strFieldValue + "&nbsp;</td>");
                        ...
                        if ("Substitute_For".equals(sVal) && !MBOMInstalldFlag) {
                            ...
                            strBuf.append("<td>&nbsp;<input type=\"checkbox\" id=\"repDiffChk\" name=\"" + sVal + "\" value=\"false\" disabled/>" + "   " + strFieldValue + "&nbsp;</td>");
                            strBuf.append("</td></tr><tr><td>");
                        } else {
                            strBuf.append("<td>&nbsp;<input type=\"checkbox\" id=\"repDiffChk\" name=\"" + sVal + "\" value=\"true\"/>" + "   " + strFieldValue + "&nbsp;</td>");
                        }
                    }
                } else {
                    continue;
    ...
    public Object getExpandLevel(Context context, String[] args) throws Exception {
        ...
        strBuf.append("<SELECT name=\"ExpandLevel\" > <OPTION value=\"1\" SELECTED >1 <OPTION value=\"2\">2 <OPTION value=\"3\">3 <OPTION value=\"4\">4 <OPTION value=\"5\">5 <OPTION value=\"0\">"
                + EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.Common.All") + " </SELECT>");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectId of the context Part.
     * @returns an Object contanining the String Name for Display.
     * @throws Exception
     *             if the operation fails. Since EC X3 Modified for ECC Reports
     */
    public Object getPartNameDisplay(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap fieldMap = (HashMap) programMap.get("fieldMap");
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        String sName = (String) fieldMap.get("name");
     ...
     * @throws Exception
     *             if the operation fails. Since EC 2012x
     */

    public Object getBOM2HiddenFields(Context context, String[] args) throws Exception {
    ...
    public Object getBOM1HiddenFields(Context context, String[] args) throws Exception {
        // to obtain the part objectId
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds HashMap containing the following entries: objectId of the context Part. name of the webform field invoking the method
     * @returns an Object contanining HTML content for revision field and the revision of the context part to be displayed.
     * @throws Exception
     *             if the operation fails. Since EC X3 R210 Modified for ECC Reports
     */
    public Object getPartRevisionDisplay(Context context, String[] args) throws Exception {
        // to obtain the part objectId
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        // HashMap paramMap = (HashMap) programMap.get("paramMap");
        HashMap fieldMap = (HashMap) programMap.get("fieldMap");
        ...
        if (sName != null && sName.equals("BOM1Revision")) {
            ...
            if (null != sobjectId) {
                ...
                /*
                 * if(isConfigPart) { String sLastid = getLatestRelRevision(context,sobjectId); DomainObject dPart1 = DomainObject.newInstance(context, sLastid); sRevison = dPart1.getInfo(context,
                 * DomainObject.SELECT_REVISION); } else { sRevison = dPart.getInfo(context,DomainObject.SELECT_REVISION); }
                 */
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap of the following entries: objectId of the context Part. paramList - a Map of parameter values.
    ...
    public Vector getPartVersion(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            getPartVersion = obj.getRelatedObjects(context, RELATIONSHIP_PART_VERSION, // relationship pattern
                    DomainConstants.TYPE_PART, // object pattern
                    selectStmts, // object selects
                    null, // relationship selects
                    false, // to direction
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getPartEcosSummary(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            ecoList = partObj.getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getAffetedItemforPart(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            totalresultList = dObj1.getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
    ...
    /**
     * displayDeviationOptions,this method is executed for 'to populate 'For Radio buttons with and Without for BOM1 Assembly' in BOM Compare Dialog
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            []
    ...
    /**
     * displayDeviationOptions,this method is executed for 'to populate 'For Radio buttons with and Without for BOM2 Assembly' in BOM Compare Dialog
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            []
    ...
    /**
     * This method is used as Access Function in the command determines if the Part Series Functionality is turned on or off by ANUPAMA
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            []
    ...
    public void floatEBOMrelationship(Context context, String[] args) throws Exception {
        ...
        // String relId = (String) programMap.get(strRel);
        ...
        RelationshipType relType = new RelationshipType(PropertyUtil.getSchemaProperty(context, "relationship_EBOM"));
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds arguments from bean containing the following entries: objectId of the context. Expand Level of objects parent option for object search
     * @throws Exception
     *             if the operation fails. Since EC X3
     */
    public MapList getBOMComparison(Context context, String[] args) throws Exception {
        ...
        objectsMapListOne = domObjectOne.getRelatedObjects(context, relationshipPattern.getPattern(), typePattern.getPattern(), objectSelects, relationshipSelects, false, true, recusrsionLevel,
    ...
    public MapList getEndItemMapList(Context context, MapList objectsMapListOne, DomainObject domObjectOne, StringList objectSelects, StringList relationshipSelects)
        ...
        sbPRSelect3.append(EngineeringConstants.SELECT_RIGHTBRACE);
        objectSelects.addElement(sbPRSelect1.toString() + sbPRSelect2.toString() + sbPRSelect3.toString());
        String sAccessSelectable = sbPRSelect1.toString() + sbPRSelect3.toString();
        String slObjectSelect = sbPRSelect1.toString() + sbPRSelect2.toString() + sbPRSelect3.toString();
        /* Added for RDO\RMO Access check ENDS */
        MapList mlEndItem = getLeafEndItems(context, objectsMapListOne, objectSelects, relationshipSelects, sAccessSelectable, slObjectSelect);
        ...
        MapList mlEIBOMList = domObjectOne.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, objectSelects, relationshipSelects, false, true, (short) 1,
                "attribute[" + EngineeringConstants.ATTRIBUTE_END_ITEM + "] == 'Yes'", "", 0);
    ...
    public MapList getPartVersions(Context context, String args[]) throws Exception {
        ...
        mapList = domObj1.getRelatedObjects(context, RELATIONSHIP_PART_VERSION, DomainConstants.TYPE_PART, objectSelects, null, false, true, (short) 0, null, null);
        return mapList;
    ...
    public int createPartVersionObject(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            if (checkPartVersion.equalsIgnoreCase("TRUE")) {
                ...
                String whereclause = "(attribute[" + attrRequestedChange + "]==\"" + "" + attrRequestedChangeValue + "\")";
                MapList revisedPartsList = ecoDomainObject.getRelatedObjects(context, RELATIONSHIP_AFFECTED_ITEM, "*", selectStmts, null, false, true, (short) 1, null, whereclause);
                ...
                if (revisedPartsList.size() > 0) {
                    ...
                    while (itemIterator.hasNext()) {
                        ...
                        String strNewPartId = (String) JPO.invoke(context, "emxChange", null, "getIndirectAffectedItems", inputArgs, String.class);
                        ...
                        if (strSelectedPolicy.equals(policyECPart)) {
                            ...
                            if (attrIsVersionValue.equalsIgnoreCase("FALSE")) {
                                ...
                                partVerObj.cloneWithStructure(context, sTypePart, verName, objectRev, strSelectedPolicy, sVaultName, sDesc, new HashMap(), sOwner, null, sRDOId, partFamilyId, null,
                                        false, revObjectId, relStructure);
                                ContextUtil.popContext(context);
                                ...
                                if (isMFGInstalled) {
                                    ...
                                    versionBusObj.setAttributeValue(context, EngineeringConstants.ATTRIBUTE_END_ITEM_OVERRIDE_ENABLED,
                                            domObj.getAttributeValue(context, EngineeringConstants.ATTRIBUTE_END_ITEM_OVERRIDE_ENABLED));
                                ...
                                MapList ebomMarkupList = revBusObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM_MARKUP, "*", selectStmts, selectRelStmts, true, true, (short) 1,
                                        "current == " + DomainConstants.STATE_PART_APPROVED + " && to[" + strRelAppliedMarkup + "] == False", null);
                                ...
                                while (markupMapItr.hasNext()) {
                                    ...
                                    // String strMarkupRelId = (String) marukupMap.get(DomainConstants.SELECT_RELATIONSHIP_ID);
                                ...
                                ContextUtil.pushContext(context);
                                MapList partSpecList = revBusObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_PART_SPECIFICATION, "*", selectStmts, selectRelStmts, true, true, (short) 1,
                                        null, null);
            ...
            if (flagRevokeAccess.booleanValue()) {
                // StringBuffer cmdBuf = new StringBuffer(250);
                ContextUtil.pushContext(context, PERSON_REQUEST_ACCESS_GRANTOR, null, null);
    ...
    public boolean setRevision(Context context, String strObjectId, String strRevision, String strName) throws FrameworkException {
        try {
    ...
    // EC-Part Series
    /**
     * Method to associate an interface to the part and set the "Part Family Reference" attribute as "Master or Reference or Unassigned".
     * @mx.whereUsed Invoked by the trigger object on creation of Part- TypePartCreateAction,
     * @mx.summary This method associates External Part Data interface to a Equivalent part. Uses <code>MqlUtil.mqlCommand</code> to associate the interface to the part. Uses
     *             <code>DomainObject.setAttribute() </code>to set an attribute value
     * @param context
    ...
    public int addInterfaceToPart(Context context, String args[]) throws Exception {
        ...
        // String sToObjectId = args[2]; // ${TOOBJECTID}
        try {
            ...
            // String sTOObjectType = doToObject.getInfo(context, DomainConstants.SELECT_TYPE);
            // Get Created Part ObjectId from Environment Variables
            ...
            boolean isMEP = false;
            // boolean PartseriesActive = checkPartSeriesEnabled ( context, args);
            ...
            if (isMEP) {
                ...
                // if(sFromObjectType.equals(DomainConstants.TYPE_PART_FAMILY) && sTOObjectType.equals(DomainConstants.TYPE_PART)) {
     ...
     *            containing the Revision to search for. txtOwner - a String containing the Owner to search for. txtWhere - a String containing the where clause to use in the search. txtOriginator - a
     *            String containing the Originator to search for. txtDesc - a String containing the Description to search for. txtSearch - a String containing the Search text for the query. txtFormat
     *            - a String containing the Format text for the query. languageStr - a String containing the language setting. revPattern - a String of either ALL_REVISIONS, HIGHEST_REVISION or
     *            HIGHEST_AND_PRESTATE_REVS. Vault - a String containing the Vault name to search in. queryLimit - a String containing the Query Limit value.
     * @return Object a MapList containing search result.
     * @throws Exception
     *             if the operation fails.
     * @since X3
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public Object getPartsInEBOM(Context context, String[] args) throws Exception {
        ...
        partList = domobj.getRelatedObjects(context, RELATIONSHIP_EBOM, // relationship pattern
    ...
    public Vector getConflictColumnDisplay(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        while (itr.hasNext()) {
            ...
            if (hasConflict != null && "true".equals(hasConflict)) {
                ...
                if (!isPrinterFriendly) {
                    ...
                    columnVals.addElement("<a href=\"javascript:emxTableColumnLinkClick('../engineeringcentral/emxEngrConflictMarkupsFS.jsp?emxSuiteDirectory=engineeringcentral&amp;relId="
                            + XSSUtil.encodeForURL(context, strRelId) + "&amp;suiteKey=EngineeringCentral&amp;parentOID=" + XSSUtil.encodeForURL(context, strParentId) + "sChangeOID="
                            + XSSUtil.encodeForURL(context, strChangeId) + "&amp;objectId=" + XSSUtil.encodeForURL(context, strId) + "&amp;key=" + XSSUtil.encodeForURL(context, strKey) + "&amp;level="
                            + XSSUtil.encodeForURL(context, strLevel) + "', '700', '600', 'true')\"><img border=\"0\" src=\"images/iconStatusMulti.gif\" alt=\"\"/></a>");
                } else {
     ...
     * @mx.whereUsed Invoked as a Access Function in the Default Clone Based On field in common create web form.
     * @mx.summary this methods checks the Typ ethat is present in the arguments. and returns true if Part is a Compliance Part and false if part is a EC part.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     *             if the operation fails
     */
    public boolean turnOffDefCloneBasedOnforECPart(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            ...
            if (typeName != null && !typeName.equals("")) {
                if (typeName.startsWith("_selectedType:")) {
                    ...
                    if (typeName.startsWith("_selectedType:")) {
                        // String ary[]=typeName.split("_selectedType:");
     ...
     * @mx.whereUsed Invoked as a Access Function in the EC Speciifc Clone Based On field in common Clone Part webform
     * @mx.summary this methods checks the Type that is present in the arguments. and returns true if Part is a EC Part and false if part is a MCC part.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     * @return the <code>boolean</code> value
     ...
     *             if the operation fails
     */
    public boolean showCloneBasedOnFieldforECPart(Context context, String[] args) throws Exception {
    ...
    public String getPartFamily(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            output.append("<input type=\"button\" name=\"btnpartFamily\" value=\"...\" onClick=\"");
            output.append(
                    "javascript:showChooser('../engineeringcentral/emxEngrPartFamilySearchDialogFS.jsp?form=emxCreateForm&field=partFamilyDisplay&partFamilyId=partFamilyOID&partFamilyAutoGenName=partFamilyAutoGenerate&suiteKey=EngineeringCentral',");
            ...
            output.append("</a>");
            output.append(
                    "<script>function clearPartFamily(){document.emxCreateForm.partFamilyDisplay.value=\"\";document.emxCreateForm.partFamily.value=\"\";document.emxCreateForm.partFamilyOID.value=\"\";document.emxCreateForm.partFamilyAutoGenerate.value=\"TRUE\";document.emxCreateForm.Name.value=\"\";document.emxCreateForm.autoNameCheck.disabled=false;document.emxCreateForm.Name.disabled=false;}</script>");
    ...
    // Engineering Central
    /**
     * This method is used to Post Process Actions after creating PartClone This method Connect with Part Family using Part Family Member Relationship Connect to ECO object Connect with Company using
    ...
    public int performPostProcessActions(Context context, String[] args) throws Exception {
        HashMap ProgramMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) ProgramMap.get("paramMap");
        HashMap requestMap = (HashMap) ProgramMap.get("requestMap");
        String ObjectId = (String) paramMap.get("objectId");
        ...
        if (!bmpart) {
            ...
            if (partFamilyID != null && !("null").equalsIgnoreCase(partFamilyID) && partFamilyID.length() > 0) {
                try {
                    ContextUtil.pushContext(context);
                    ...
                    if (partFamily.exists(context)) {
                        ...
                        usePartFamilyForName = partFamily.getAttributeValue(context, attrPartFamilyNameGeneratorOn);
    ...
    public Object showDesignResponsibilityItem(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            sbReturnString.append("</input>");
            sbReturnString.append(
                    "<input type=\"button\" name=\"btnCompany\" value=\"...\" onclick=\"javascript:showChooser('../engineeringcentral/emxpartRDOSearchDialogFS.jsp?form=emxCreateForm&amp;field=DesignResponsibilityDisplay&amp;fieldId=DesignResponsibilityOID&amp;searchLinkProp=SearchRDOLinks&amp;objectId=");
     ...
     */
    public boolean checkTypeChangeMgmtForMEP(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails.
     */
    public String showUsageLocationStatus(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        String languageStr = (String) paramMap.get("languageStr");
    ...
    /**
     * Connects the created MPN for the given Enterprise Part and location objects.
     * @param context
     *            The Matrix Context.
     * @param args
     *            holds a packed HashMap which contains usagelcoation,mepid ep id,manufacturer,mfg location,loc status
     * @throws FrameworkException
     *             If the operation fails.
     */
    ...
    public void createMPN(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (manufacturerId != null && !manufacturerId.equals("null") && manufacturerId.length() > 0) {
            ...
            Company contextComp = com.matrixone.apps.common.Person.getPerson(context).getCompany(context);
            ...
            if (doEntPart != null) {
                ...
                if (sLocId != null && !"null".equals(sLocId) && sLocId.length() > 0) {
                    ...
                    sLocEquivObjId = FrameworkUtil.autoName(context, LOCATION_EQUIVALENT_OBJECT, "", LOCATION_EQUIVALENT_POLICY, null, null, false, false);
                ...
                if (context.isAssigned(sCompEngrRole) || context.isAssigned(sDesignEngrRole) || context.isAssigned(sMfgEngrRole)) {
                    if (sEpId != null && !"null".equalsIgnoreCase(sEpId) && !"".equalsIgnoreCase(sEpId)) {
                ...
                // context is not component engineer role
                    ...
                    if (doLocEquiv != null) {
                        ...
                        // false);
                        doEntPart.addRelatedObject(context, new RelationshipType(
                                // RELATIONSHIP_MANUFACTURER_EQUIVALENT),-IR-026840V6R2011-need to refer rel_LocationEquivalent when the location is chosen
                                RELATIONSHIP_LOCATION_EQUIVALENT), false, doLocEquiv.getInfo(context, SELECT_ID));
                    }
     ...
     *             if the operation fails.
     */
    public String showUsageLocationPreference(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        // HashMap requestMap = (HashMap) programMap.get("requestMap");
        String languageStr = (String) paramMap.get("languageStr");
    ...
    /**
     * Show the Manufacturer field. * @param context the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap.
    ...
    public String showManufacturerFieldForMPN(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            returnString.append("<a href=\"JavaScript:ResetField('forms[0]','Manufacturer','ManufacturerDisplay','" + XSSUtil.encodeForJavaScript(context, defaultManufacturer) + "','"
            ...
            returnString.append("<input type=\"hidden\" name=\"Manufacturer\" value=\"" + XSSUtil.encodeForHTMLAttribute(context, defaultManufacturerId) + "\"></input>");
        ...
        try {
            ...
            while (itrObjects.hasNext()) {
                ...
                if ("add".equals(markup)) {
                    ...
                    DomainRelationship dr = DomainRelationship.connect(context, parentObj, sRelType, childObj);
                    ...
                    if (strParam2 != null && strParam2.length() > 0) {
                        ...
                        if (!slParamObjs.contains(strParam2)) {
                            ...
                            if (connectAsDerived.equalsIgnoreCase("true")) {
                                /*
                                 * DomainRelationship doRelDerived = DomainRelationship.connect(context, childObj, RELATIONSHIP_DERIVED, new DomainObject(strParam2));
                                 */
    ...
    /**
     * handleNotificationForSubstitutes. This method used to Send Subscription notification to the user.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     ...
     * @since Engineering Central - X3
     */
    public void handleNotificationForSubstitutes(Context context, String[] args) throws Exception {
    ...
    /**
     * Gets all the Part and it's Subtype's objects that are in released state. Used with the excludeOIDprogram setting of Autonomy Search in case of CopyTo functionality * @param context the eMatrix
    ...
    public StringList getReleasedParts(Context context, String args[]) throws Exception {
        ...
        SelectList resultSelects = new SelectList(1);
        resultSelects.add(DomainObject.SELECT_ID);
        MapList totalresultList = DomainObject.findObjects(context, DomainConstants.TYPE_PART, "*", "*", "*", "*", sWhereExp, null, true, resultSelects, (short) 0);
    ...
    /**
     * Gets all the ECR states on or after Plan ECOand ECO after Release Used with the excludeOIDprogram setting of Autonomy Search in case Save SaveAS functionality * @param context the eMatrix
     * <code>Context</code> object.
     * @param args
     *            holds a packed HashMap.
     * @return a StringList that contains.
    ...
    public StringList getExcludedMarkupStatesECRECO(Context context, String args[]) throws Exception {
        ...
        // String typePattern = DomainConstants.TYPE_ECO+","+DomainConstants.TYPE_ECR;
        ...
        try {
            ...
            if (strPartId != null && strPartId.length() > 0) {
                ...
                MapList mapListMarkups = doPart.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM_MARKUP, PropertyUtil.getSchemaProperty(context, "type_ItemMarkup"), strlObjectSelects,
    ...
    @com.matrixone.apps.framework.ui.ConnectionProgramCallable
    public HashMap validateStateForApplyForBOMCompare(Context context, String[] args) throws Exception {
    ...
    /**
     * To allow Apply edits in EBOM emxTable/Indented Table based on the state of the Parent assembly Part
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds objectId.
    ...
    public HashMap validateStateForApply(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            // HashMap hmpElements = new HashMap();
    ...
    /**
     * Returns a StringList of the object ids which are connected using Part Specification Relationship and objects revisions ids for a given context.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
    ...
    /**
     * This utility modifies the maplist mplOut recursively to add parts of all levels. Level to 1 for all items and add customLevel instead of Level
     * @param mplIn
     *            - Input MapList
     * @param mplOut
     *            - Map to populate result
     * @return void
     * @since EngineeringCentral X3
     */
    ...
    /**
     * Added to fix IR-018596V6R2011, IR-034237V6R2011 This utility modifies the maplist mplOut recursively to add parts of all levels. Level to 1 for all items and add customLevel instead of Level
     * @param mplIn
     *            - Input MapList
     * @param mplOut
     *            - Map to populate result
    ...
    public void deleteRelatedPartMaster(matrix.db.Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (strPlanningReqd.equalsIgnoreCase("Yes") && strsendItem.equalsIgnoreCase("Yes")) {
                ...
                MapList mpList = partObj.getRelatedObjects(context, relationshipPlannedFor, typeManufacturingPlan, busSelects, null, false, true, (short) 1, null, null);
    ...
    /**
     * This function is called while revising the Part. Newly revised part will also get connect with the Part Master.
     * @param context
     * @param args
     * @throws Exception
     */
    ...
    public String showPartModeField(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            if (strPartMode != null && !"null".equalsIgnoreCase(strPartMode) && !"".equalsIgnoreCase(strPartMode)) {
                // Multitenant
                /*
                 * strPartMode = i18nNow.getI18nString("emxFramework.Range.Part_Mode."+strPartMode, "emxFrameworkStringResource", context.getSession().getLanguage());
                 */
     ...
     * @author ZGQ
     */
    public int checkForFileInSpec(Context context, String args[]) throws Exception {
    ...
    /**
     * createAndConnectPartMaster method checks for the Part Master object for the current Part. If not available, Part Master will be created. This method is invoked on creation of Part
     * @param context
     *            Context : User's Context.
     * @param args
     *            String array having the object Id of the part.
     * @throws FrameworkException
     *             if creation of the Part Master object fails. 364886 :added a trigger method to create a part master when part is created IR-061415V6R2011x - modified
     */

    public void createAndConnectPartMaster(Context context, String[] args) throws Exception {
            ...
            if ("Production".equalsIgnoreCase(policyClass) || "Development".equalsIgnoreCase(policyClass) || "Unresolved".equalsIgnoreCase(policyClass)) {
                ...
                if (checkPartVersion.equalsIgnoreCase("TRUE")) {
                    ...
                    if (strPartName.indexOf(sSplChar) != -1) {
                        ...
                        BusinessObject boPartTemp = new BusinessObject(EngineeringConstants.TYPE_PART, sPartNametemp, sPartRevTemp, doPart.getVault());
    ...
    /**
     * lookupEntries method checks the object entered manually is exists or not in the database Method to Inline create & connect new Part objects in EBOM powerview IndentedTable This method is
     * invoked on clicking on Apply button in EBOM
     * @param args
     *            String array having the object Id(s) of the part.
     * @throws FrameworkException
     *             if creation of the Part Master object fails.
    ...
    public HashMap inlineCreateAndConnectPart(Context context, String[] args) throws Exception {
        ...
        if (isENGSMBInstalled) { // Commented for IR-213006
            ...
            vpmControlState = MqlUtil.mqlCommand(context, mqlQuery, parentObjectId, "from[" + RELATIONSHIP_PART_SPECIFICATION + "|to.type.kindof[" + EngineeringConstants.TYPE_VPLM_CORE_REF
                    + "]].to.attribute[" + EngineeringConstants.ATTRIBUTE_VPM_CONTROLLED + "]");
        }
    ...
    public DomainObject createchildObj(Context context, String objectType, String objectName, String objectRev, String objectPolicy, String objectVault, DomainObject childObj, boolean useAutoName)
    ...
    public HashMap getAutoNameSeries(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < size; i++) {
            columnVals.add(EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),
                    "emxEngineeringCentral.Common." + ((String) sResultList.get(i)).replace(" ", "")));
            columnVals_Choices.add(EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", new Locale("en"),
                    "emxEngineeringCentral.Common." + ((String) sResultList.get(i)).replace(" ", "")));
        }
     ...
     * @throws Exception
     */
    public String showCustomRevisionField(Context context, String[] args) throws Exception {
    ...
    /**
     * To displays the VPMProductName field value(V_Name). Displays the V_Name on part if the part is not at synchronized If already Synchronized, displays the V_Name of the corresponding product and
    ...
    public String displayVName(Context context, String[] args) throws Exception {
        ...
        MapList mapList = partObj.getRelatedObjects(context, RELATIONSHIP_PART_SPECIFICATION, "PLMEntity", busSelects, null, false, true, (short) 1, null, null, 0);
        ...
        if (itr.hasNext()) {
            ...
            if (reportFormat == null || "".equals(reportFormat)) {
                ...
                strBuf.append("','popup', '', '', '')\">");
                strBuf.append("<img src=\"images/I_VPMNavProduct.png\" alt=\"VPM Product : " + XSSUtil.encodeForHTML(context, productName) + "\" title=\"VPM Product : "
                        + XSSUtil.encodeForHTML(context, productName) + "\" border=\"0\" align=\"absmiddle\"></img>");
                strBuf.append("</a>&nbsp;");
            }
        }
        return strBuf.toString();
    ...
    /**
     * This method is to disply the VPM Product Name filed in the table To displays the VPMProductName field value(V_Name). Displays the V_Name on part if the part is not at synchronized If already
     * Synchronized, displays the V_Name of the corresponding product and links to the Product.
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    public Vector displayVNameInTable(Context context, String[] args) throws Exception {
        ...
        String selectProdctIdSel = "from[" + DomainConstants.RELATIONSHIP_PART_SPECIFICATION + "].to[PLMEntity]." + DomainConstants.SELECT_ID;
        String selectProdctVNameSel = "from[" + DomainConstants.RELATIONSHIP_PART_SPECIFICATION + "].to[PLMEntity]." + attrVPLMVName;
        ...
        busSelects.add(selectProdctVNameSel);
        ...
        while (objItr.hasNext()) {
            ...
            objIdArr[count++] = ((String) objMap.get("id"));
        }

        MapList completeMapList = DomainObject.getInfo(context, objIdArr, busSelects);
        Iterator mapListItr = completeMapList.iterator();
        String href = "";

        while (mapListItr.hasNext()) {
            partDtlMap = (Map) mapListItr.next();

            if (partDtlMap.size() > 1) {
                Iterator keySetIterator = partDtlMap.keySet().iterator();
                String key;
                String prodIdKey = "";
                String prodAttrKey = "";
                ...
                /* if(partDtlMap.containsKey(selectProdctId)){ */
                productName = (String) partDtlMap.get(prodAttrKey);

                if (partDtlMap.get(prodIdKey) instanceof String) {
                    productId = StringUtils.split((String) partDtlMap.get(prodIdKey), "\\a")[0];
                } else if (partDtlMap.get(prodIdKey) instanceof StringList) {
                    productId = (String) ((StringList) partDtlMap.get("from[" + DomainConstants.RELATIONSHIP_PART_SPECIFICATION + "].to." + DomainConstants.SELECT_ID)).get(0);
                }

                productName = StringUtils.replace(productName, "&", "&amp;");
                if (reportFormat == null || "".equals(reportFormat)) {

                    String modifyURL = "../common/emxTree.jsp?objectId=" + productId;
                    href = productName + "<a href=\"JavaScript:emxTableColumnLinkClick('" + modifyURL
                            + "', '700', '600', 'false', 'popup', '')\"><img src=\"../common/images/I_VPMNavProduct.png\" border=\"0\"/></a>";

                    vNameVector.add(href);
                } else {
     ...
     * @throws Exception
     */
    public Object checkEditMode(Context context, String[] args) throws Exception {
    ...
    /**
     * Added for IR-044514V6R2011 This method is to disply all child objects for substitute functionality
     * @param context
     * @param args
     * @return MapList
     * @throws Exception
     ...
     * @throws Exception
     */
    public String showRevisionField(Context context, String[] args) throws Exception {
     ...
     * @throws Exception
     *             if the operation fails Since R210 Added for PUE Reports
     **/
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getStoredEffectivityEBOM(Context context, String[] args) throws Exception {
        // String sChildren = null;
        MapList ebomList = null;
        try {
    ...
    public MapList getEBOMsWithEffectivityRelSelectables(Context context, String[] args) throws Exception {
        ...
        /*
         * Check to see which side of report is this processing done. If its left side, the first effectivity info is considerd and if its right, the second effectivity info is considered.
         */
        ...
        try {
            ...
            /*
             * if(strConsolidatedReport != null && strConsolidatedReport.equalsIgnoreCase("Yes")) { level=0; }
             */
            ...
            ebomList = partObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, // relationship pattern
                    DomainConstants.TYPE_PART, // object pattern
                    selectStmts, selectRelStmts, false, true, (short) level, sObjWhereCond.toString(), null, (short) 0, false, false, (short) 0, null, null, null, null, strEffBinary);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - objectId - OID of the object to inspect
     * @returns boolean true or false based on the part policy
     * @throws Exception
     *             if the operation fails Since R210 Added for PUE Reports
     **/
    public boolean isConfigurablePart(Context context, String strObjectId) throws Exception {
     ...
     *             if the operation fails Since R210 Added for PUE Reports
     **/
    public String getLatestRelRevision(Context context, String strObjId) throws Exception {
    ...
    /**
     * Added for excluding the reference documents connected to the object. This can be used for generic purpose.
     * @param context
     * @param args
    ...
    public StringList excludeConnectedObjects(Context context, String[] args) throws Exception {
        ...
        if (sMode != null && sMode.equals("ECRAddExisting")) {
            ...
            childObjects = DomainObject.findObjects(context, strField, DomainConstants.QUERY_WILDCARD, sWhere, sSelectables);
        }
    ...
    class SpecDocuments {
        ...
        public void connectPart(Context context) throws Exception {
            if (isValidData(this.partId)) {
                DomainRelationship.connect(context, new DomainObject(this.partId), // from side object Part
        ...
        public void connectECO(Context context) throws Exception {
            if (isValidData(this.ecoId)) {
                DomainRelationship.connect(context, new DomainObject(this.ecoId), // from side object ECO
        ...
        public void connectRDO(Context context) throws Exception {
            if (isValidData(this.rdoId)) {
                ...
                if (hasFromConnectAccess) {
                    DomainRelationship.connect(context, new DomainObject(this.rdoId), // from side object Design Responsibilty
                            DomainConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY, // Relationship
                            this.domobj);// toSide object Document
        ...
        public String toString() {
            return new StringBuffer("[PartId =").append(this.partId).append(", ecoId=").append(this.ecoId).append(", RDOID=").append(this.rdoId).append("]").toString();
        }
    }
    ...
    public void connectToECR(Context context, String[] args) throws Exception {
        ...
        if (isValidData(ecrId)) {
            DomainRelationship.connect(context, new DomainObject(ecrId), DomainConstants.RELATIONSHIP_ECR_SUPPORTING_DOCUMENT, sketchDomObj);
        }
    ...
    public MapList getConnectedECOFromPart(Context context, String[] args) throws Exception {
        ...
        // String objectWhere = getObjectWhere (context, POLICY_ECO, states);
    ...
    public Map createSpecification(Context context, String[] args) throws Exception {
        ...
        PartDefinition partDefinition = new PartDefinition();
        partDefinition.create(context, sType, sName, sAutoNameSeries, null, autoNameSelected, sPolicy, sVault, null, null, null, null, sCustomRevisionLevel, null);
    ...
    public void connectRelECO(Context context, String[] args) throws Exception {
        ...
        if ((fromObject != null) && !"".equals(fromObject)) {
            try {
                ...
                DomainRelationship.connect(context, fromBusObj, RELATIONSHIP_AFFECTED_ITEM, toBusObj);
            } catch (Exception e) {
                e.printStackTrace();
                throw e;
            }
    ...
    public void connectRDO(Context context, String[] args) throws Exception {
        ...
        DomainObject toBusObj = new DomainObject(toObject);
        /*
         * if(UIUtil.isNotNullAndNotEmpty(strNewRDOName)) { toBusObj.setPrimaryOwnership(context, EngineeringUtil.getDefaultProject(context), strNewRDOName); } else {
        ...
        if ((!"".equals(strNewRDOName) && "".equals(fromObject)) || ("False".equals(strHasRDO) || "".equals(strHasRDO))) {
        ...
        if ((!"".equals(strNewRDOName) && "".equals(fromObject)) || ("".equals(strNewRDOName) && "".equals(fromObject))) {
            connectRDO = false;
        }
        ...
        if (connectRDO) {
            try {
                DomainObject fromBusObj = new DomainObject(fromObject);
                DomainRelationship.connect(context, fromBusObj, DomainConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY, toBusObj);
            } catch (Exception e) {
                e.printStackTrace();
                throw e;
            }
     ...
     * @Since R211
     */
    public Object connectPartToPartFamily(Context context, String[] args) throws Exception {
            ...
            try {
                ...
                if (newPartFamilyList.size() > 0) {
                    ...
                    while (partFamilyItr.hasNext()) {
                        String newPartFamily = (String) partFamilyItr.next();
                        setId(strPartId);
                        DomainObject domainObjectFromType = newInstance(context, newPartFamily);
                        DomainRelationship.connect(context, domainObjectFromType, strPartFamilyRelationship, doPartObj);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public Map createPartJPO(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            strPartId = part.createPart(context, sType, sName, sCustomRevisionLevel, sPolicy, sVault, sOwner, sAutoNameChecked, sPartFamilyId, sAutoNameSeries);
    ...
    /**
     * if objectId is of Part Family then policy will be returned from partfamily attribute "Deafult Part Policy" else it will return "Development Part" as default policy.
     * @param context
     *            ematrix context
     * @param map
    ...
    /**
     * To display the policy list in part creation page Also considers the app specific policies
     * @param context
     * @param args
     * @return HashMap
     * @throws Exception
    ...
    /**
     * To display the Part Family based when invoked from the part family page Also considers the app specific policies
     * @param context
     * @param args
     * @return String
     * @throws Exception
     * @Since R211
     */
    public String getPartFamilyDisplay(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (null != strPartFamilyId && !"".equals(strPartFamilyId)) {
                ...
                if (domObj.isKindOf(context, PropertyUtil.getSchemaProperty(context, "type_PartFamily")))
    ...
    /**
     * To display the Part Family Field only when invoked from the Part Family page Also considers the app specific policies
     * @param context
     * @param args
     * @return boolean
     * @throws Exception
    ...
    /**
     * To display the General Class Field only when invoked from the General Class page Also considers the app specific policies
     * @param context
     * @param args
     * @return String
     * @throws Exception
     * @Since R211
     */
    public boolean isGeneralClassPart(Context context, String[] args) throws Exception {
        DomainObject domObj = null;
     ...
     * @Since R211
     */
    public String showDefaultVault(Context context, String[] args) throws Exception {
     ...
     * @Since R211
     */
    public String partFamilyAutoGenaration(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        ...
        try {
            ...
            if (null != sObjectId && !"".equals(sObjectId) && null != sCreateMode && sCreateMode.equals("LIB")) {
                DomainObject DOBJ = new DomainObject(sObjectId);

                String TYPE_PARTFAMILY = PropertyUtil.getSchemaProperty(context, "type_PartFamily");
                String attrPFNameGen = PropertyUtil.getSchemaProperty(context, "attribute_PartFamilyNameGeneratorOn");

                if (DOBJ.isKindOf(context, TYPE_PARTFAMILY)) {
                    String attrPFNameGenVal = DOBJ.getAttributeValue(context, attrPFNameGen);
                    field.append("<input type=\"hidden\" value=\"" + XSSUtil.encodeForHTMLAttribute(context, attrPFNameGenVal) + "\" name=\"PartFamilyAutoName\"/>");
                    field.append(" <script language=\"javascript\"> ");
                    field.append("  document.forms[\'emxCreateForm\'].elements[\'PartFamilyOID\'].value =\"" + XSSUtil.encodeForJavaScript(context, sObjectId) + "\"; ");
    ...
    public String showDefaultDesignRes(Context context, String[] args) throws Exception { // IR:073691
        String strName = "";
    ...
    /**
     * This method will be called whenever ECO and Part getting connected with "Affected Item" relationship. method is called in check trigger of Affected Item relationship.
     * @param context
     *            ematrix context
     * @param args
     *            contains paramater passed from trigger RelationshipAffectedItemCreateCheck
     * @return 0 or 1.
     * @throws Exception
     *             if any error occurs.
     */
    public int checkForChangeAlreadyConnected(Context context, String[] args) throws Exception {
        ...
        if (chgObj.isKindOf(context, DomainConstants.TYPE_ECO) && isValidData(partId)) {
            ...
            if (!DomainConstants.STATE_PART_RELEASE.equals(partCurrent)) {
                ...
                MapList changeList = domObj.getRelatedObjects(context, RELATIONSHIP_AFFECTED_ITEM, DomainConstants.TYPE_ECO, null, null, true, false, (short) 1, objectWhere, null, null, null, null);
    ...
    public MapList getPartWhereUsed(Context context, String[] args) throws Exception {
        ...
        if (isValidData(objectId)) {
            ...
            if (isWhereUsedPLBOM) {
                ...
                objectSelect = createStringList(new String[] { DomainConstants.SELECT_ID, SELECT_PART_MODE, SELECT_RAISED_AGAINST_ECR, SELECT_PART_TO_ECR_CURRENT, SELECT_PART_TO_ECO_CURRENT,
                        REL_TO_EBOM_EXISTS, SELECT_RAISED_AGAINST_ECR_CURRENT, POLICY_CLASSIFICATION });
    ...
    /*
     * This method is called from part where used expand program when user selects related field other than none option. From URL objectId and selected related option is passed. This method returns
     * MapList which contains data related to selected option.
     * 
     ...
     *            ematrix context
     * @param domObj
     *            current part domainobject instance
     * @param objectSelect
     *            StringList object select statements
     * @param relSelect
     *            StringList relationship select statements
     * @param objectWhere
     *            where condition to apply on objects
     * @return MapList
     * @throws Exception
     *             if any exception occurs.
     */
     ...
     * @param context
     *            ematrix context
     * @param effectivityOID
     *            contains effectivity OID
     * @return String
     * @throws Exception
     *             if any exception occurs.
     */
     ...
     * @param objectId
     * @param objectSelect
     *            selectable to select from object
     * @param relSelect
     *            selectable to select from object
     * @return MapList
     * @throws Exception
        ...
        if (isValidData(strSubPartIds)) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (hSetUniqueId.add(parentObjectId)) {
                    ...
                    listTemp = domObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, objectSelect, relSelect, true, false, (short) 1, objectWhere, whereExpr);
    ...
    public StringList getPartModeForPartWhereUsed(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < size; i++) {
            /*
             * Modified to get the PartMode attributes as StringList, as the displayed Part may be connected more than once to the Part Master Object (When the Part displayed in the whereused is
             * connected to a Plant with Manufacturing Responsibility Relationship and is in the Release state, the Part is connected more than once with the Part Master and the Part Mode attributes
             * returned will be StringList.)
             */
            listPartMode = getListValue((Map) list.get(i), SELECT_PART_MODE);
            if (listPartMode.size() > 0) {
                // Multitenant
                strPartMode = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), "emxFramework.Range.Part_Mode." + listPartMode.get(0)); // Modified for
                                                                                                                                                                                   // IR-204622
            } else {
                strPartMode = "";
            }
    ...
    /*
     * Checks wheather active ECR OR ECO Exists. If exists returns true else returns false.
     */
    ...
    public HashMap getLevelFilterForPartWhereUsed(Context context, String[] args) throws Exception {
        String[] levelOptions = { "emxEngineeringCentral.Part.WhereUsedLevelUpTo", "emxEngineeringCentral.Part.WhereUsedLevelAll", "emxEngineeringCentral.Part.WhereUsedLevelHighest",
                "emxEngineeringCentral.Part.WhereUsedLevelUpToAndHighest" };
    ...
    public HashMap getRevisionsFilterForPartWhereUsed(Context context, String[] args) throws Exception {
        String[] revisionOptions = { "emxEngineeringCentral.Part.WhereUsedRevisionLatest", "emxEngineeringCentral.Part.WhereUsedRevisionLatestReleased",
    ...
    public MapList getSpecRelatedParts(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            objMapList = FrameworkUtil.toMapList(
                    specObj.getExpansionIterator(context, relPattern, typePattern, selectStmts, selectRelStmts, true, true, (short) 1, null, null, (short) 0, false, false, (short) 1, false),
    ...
    public MapList getSparePartsForExpand(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            sparePartMapList = FrameworkUtil.toMapList(partObj.getExpansionIterator(context, DomainConstants.RELATIONSHIP_SPARE_PART, "*", selectStmts, selectRelStmts, false, true, (short) 1, null,
                    null, (short) 0, false, false, (short) 0, false), (short) 0, null, null, null, null);
    ...
    public MapList findMyViewObjects(Context context, String[] args) throws Exception {
        ...
        if (!("MFG".equals(sXBOMApp))) {
            if ("Part".equals(selType)) {
                whereExpression = whereExpression + "&& policy!=\"" + EngineeringConstants.TYPE_MANUFACTURING_PART + "\"";
            } else if ("Change".equals(selType)) {
                whereExpression = whereExpression + " && type!=" + EngineeringConstants.TYPE_MECO + " && type!=" + EngineeringConstants.TYPE_MCO + " && type!=" + EngineeringConstants.TYPE_DCR;
            }
        }
        ...
        MapList maplist = DomainObject.findObjects(context, selType, QUERY_WILDCARD, // namepattern
    ...
    public static StringList removeNotNeededStates(StringList stateList) {
        ...
        if (stateList != null) {
            ...
            while (itr.hasNext()) {
                ...
                if (state.indexOf("Obsolete") > -1 || state.indexOf("Cancelled") > -1 || state.indexOf("Superseded") > -1 || state.indexOf("solete") > -1 || state.indexOf("ancel") > -1
                        || state.indexOf("uperseded") > -1)
                    continue;
    ...
    /**
     * Gets the change completion percentage based on state where current change objectList e.g if MCO has 4 states, and currently at 2nd state mean returns 50% completion percentage
     * @param context
     *            - the eMatrix <code>Context</code> object
    ...
    public MapList getBOMChartData(Context context, String[] args) throws Exception {
        ...
        rootNodeMap.put(SELECT_ATTRIBUTE_QUANTITY, "1.0");
    ...
    public boolean disableRDOField(Context context, String[] args) throws Exception {
        /*
         * if(!EngineeringUtil.isENGInstalled(context, args)) { return false; }
         */
    ...
    public boolean hideCreatePartENGField(Context context, String[] args) throws Exception {
    ...
    public boolean displayCreatePartTypeField(Context context, String[] args) throws Exception {
        boolean displayField = Boolean.FALSE;
        displayField = hideCreatePartENGField(context, args);
        return displayField;
    }

    public boolean hideEditPartENGField(Context context, String[] args) throws Exception {
        boolean displayField = Boolean.FALSE;
        displayField = hideCreatePartENGField(context, args);
        return displayField;
    }

    public boolean hideCreatePartMFGField(Context context, String[] args) throws Exception {
        boolean displayField = Boolean.FALSE;
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            String strPartCreateMode = (String) programMap.get("createMode");
            String strType = (String) programMap.get("type");
            if ("MFG".equals(strPartCreateMode) && !"Synthetic Part".equals(strType) && !"Kit Part".equals(strType) && !"type_SyntheticPart".equals(strType) && !"type_KitPart".equals(strType)) {
                displayField = Boolean.TRUE;
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        ...
        if (fnKeyExists) {
            ...
            while (iterator.hasNext()) {
                ...
                if (UIUtil.isNullOrEmpty(findNumber)) {
                    ...
                    fnKeyExists = false;
                    break;
                } else if ("true".equalsIgnoreCase(fnDisplayLeadingZeros)) {
    ...
    public MapList expandEBOM(Context context, HashMap paramMap) throws Exception { // name modified from getEBOMsWithRelSelectablesSB
        ...
        StringList objectSelect = createStringList(new String[] { SELECT_ID, SELECT_TYPE, SELECT_REVISION, SELECT_LAST_ID, SELECT_LAST_REVISION, SELECT_REL_FROM_EBOM_EXISTS,
                SELECT_REL_EBOM_FROM_LAST_REVISION_EXISTS, SELECT_ATTR_ENABLE_COMPLIANCE });
        ...
        MapList ebomList = partObj.getRelatedObjects(context, RELATIONSHIP_EBOM, TYPE_PART, objectSelect, relSelect, false, true, (short) nExpandLevel, null, null, 0);
        ...
        if ("Latest".equals(selectedFilterValue) || ("Latest Complete".equals(selectedFilterValue))) {
            ...
            while (itr.hasNext()) {
                ...
                if ("Latest".equals(selectedFilterValue)) {
                    ...
                    // newMap.put("hasChildren", getStringValue(newMap, SELECT_REL_EBOM_FROM_LAST_REVISION_EXISTS));
                    newMap.put("hasChildren", EngineeringUtil.getHasChildren(newMap, ebomDerivativeList));
                }
    ...
    /*
     * This method verifies all the part is connected to same RDO. if the given array of part is connected to same RDO it returns RDO Id, else it returns null.
     */
    ...
    /**
     * This method will be called when common view expand filter selected as "End Item" The method get all the leaf end items for a given EBOM maplist
     * @param context
     * @param mlCommonBOMList
     *            - EBOM List
     ...
     * @throws FrameworkException
     */
    private MapList getLeafEndItems(Context context, MapList mlCommonBOMList, StringList slObjectSelectStmts, StringList slRelSelectStmts, String sAccessSelectable, String slObjectSelect)
            throws NumberFormatException, Exception, FrameworkException {
    ...
    public HashMap displayLeadTimeAttributeRangeValues(Context context, String[] args) throws Exception {
        String[] rangeOptions = { "emxFramework.Range.Lead_Time.1_Week", "emxFramework.Range.Lead_Time.2_Weeks", "emxFramework.Range.Lead_Time.3_Weeks", "emxFramework.Range.Lead_Time.4_Weeks",
                "emxFramework.Range.Lead_Time.6_Weeks", "emxFramework.Range.Lead_Time.8_Weeks", "emxFramework.Range.Lead_Time.12_Weeks", "emxFramework.Range.Lead_Time.20_Weeks" };
    ...
    /**
     * PreProcessJPO for opening markup With XSS settings also
     * @param context
     * @param args
     * @return
     * @throws Exception
    ...
    public Map getPartInfo(Context context, String args[]) throws Exception {
        ...
        StringList selectList = createStringList(new String[] { DomainConstants.SELECT_VAULT, DomainConstants.SELECT_TYPE, DomainConstants.SELECT_NAME, SELECT_RDO_REL_EXISTS, SELECT_RDO_REL_ID });
    ...
    public HashMap checkLicense(Context context, String[] args) throws Exception {
        ...
        try {
            String[] app = { "ENO_TBE_TP", "ENO_LBC_TP" };
            ComponentsUtil.checkLicenseReserved(context, app);
        } catch (Exception tbe) {
        ...
        try {
            String[] app = { "ENO_ENG_TP", "ENO_LBC_TP" };
            ComponentsUtil.checkLicenseReserved(context, app);
        } catch (Exception eng) {
    ...
    /**
     * Method is called from postProcessJPO when cloning the part from global actions and part properties page. This method disconnectes the EBOM OR Reference Document Connections if it is unselected
     * in UI. All options selected in Include Related Data from UI will be done in this method. Part Family connections. Derived Relation connection.
     */
    ...
    public void postProcessForClonePart(Context context, String[] args) throws Exception {
        ...
        while (st.hasMoreTokens()) {
            ...
            if (isValidData(selectedIncludeRelatedData)) {
                ...
                boolRefDocRelSelected = listRelatedData.remove(RELATIONSHIP_REFERENCE_DOCUMENT);// As Reference Document is replicate, It will be connected already.If user unchecks the option need to
                                                                                                // disconnect.
            ...
            if (!boolRefDocRelSelected) {
                relToDisconnect = (relToDisconnect == null) ? RELATIONSHIP_REFERENCE_DOCUMENT : (relToDisconnect + "," + RELATIONSHIP_REFERENCE_DOCUMENT); // If user has deselected Reference Doc
                                                                                                                                                           // option collect the rel to disconnect
            }
            ...
            try {
                if (relToDisconnect != null) {
                    MapList listToDisconnect = domNewObj.getRelatedObjects(context, relToDisconnect, QUERY_WILDCARD, null, new StringList(SELECT_RELATIONSHIP_ID), false, true, (short) 1, null, null,
                            null, null, null);
                    ...
                    if (getListSize(listToDisconnect) > 0) {
                        ...
                        MqlUtil.mqlCommand(context, "history off");
                        try {
                            DomainRelationship.disconnect(context, (String[]) relIdListToDisconnect.toArray(new String[0])); // To disconnect EBOM OR Reference Document if user unselects which is
                                                                                                                             // defaulty selected.
                        } catch (Exception e) {
                            throw e;
                        } finally {
    ...
    public int checkIfPartSynchronized(Context context, String[] args) throws Exception {
        ...
        if ("true".equalsIgnoreCase(vplmVisible)) {
            String productId = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", partId,
                    "from[" + RELATIONSHIP_PART_SPECIFICATION + "|to.type.kindof[" + EngineeringConstants.TYPE_VPLM_CORE_REF + "]]" + ".to.id");

            if (productId == null || "".equals(productId)) {
                String strMessage = EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.alert.PublishToVPM", context.getSession().getLanguage());
                emxContextUtil_mxJPO.mqlNotice(context, strMessage);
                return 1;
            }
        }
    ...
    /**
     * Trigger method to check if the VPLM product is in Released state. Part promotion is possible if product is in Released or Obsolete state, if VPM Visible = true.
     * @param context
     * @param args
     * @return
     * @throws Exception
        ...
        if ("true".equalsIgnoreCase(vplmVisible)) {
            ...
            if (UIUtil.isNotNullAndNotEmpty(productId)) {
                ...
                if (strPolicy.equalsIgnoreCase(EngineeringConstants.POLICY_VPLM_SMB_DEFINITION)) {
                    String targetState = PropertyUtil.getSchemaProperty(context, "policy", strPolicy, args[1]);
                    // Modified for IR-219370 end
                    if (!PolicyUtil.checkState(context, productId, targetState, PolicyUtil.GE)) {
                        String strMessage = EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.alert.releaseVPMProduct", context.getSession().getLanguage());
                        emxContextUtil_mxJPO.mqlNotice(context, strMessage);
                        return 1;
                    }
                }
    ...
    public String getErrorMessageForBOMCompareSync(Context context, DomainObject obj) throws Exception {
        ...
        if ("Unresolved".equalsIgnoreCase(policyClassification)) {
            ...
            sErrorMsg += "\n" + sType + " " + sName + " " + sRev + " " + EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.EBOM.ParentInReleaseError1", langStr) + CurrentState + ". "
    ...
    /***
     * This method is used for displaying Classification Attributes, when only LBC is installed. If not it returns an empty MapList. Invoked LBC Specific JPO methods to retrive all the Classification
     * Attributes.
     * @param context
     * @param args
     * @return MapList
    ...
    /*
     * This method verifies all the part is connected to same Organization(AltOwner1). if the given array of part is connected to same Organizationa it returns Altowner1 name, else it returns null.
     */
    ...
    public int ensureECOConnected(Context context, String[] args) throws Exception {
        ...
        MapList mapListECOs = getRelatedObjects(context, strRelPattern, strTypePattern, // object pattern
                selectStmts, // object selects
                null, // relationship selects
                true, // to direction
                false, // from direction
        ...
        if (mapListECOs.size() > 0) {
            ...
            String langStr = context.getSession().getLanguage();
            String strMessage = EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.CheckIfECOConnected.Message1", langStr) + " "
                    + EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.ChangeOrderChangeAction", langStr) + " "
                    + EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.CheckIfECOConnected.Message2", langStr) + " "
                    + EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), "emxFramework.Relationship.Change_Affected_Item") + ", "
                    + EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), "emxFramework.Relationship.Implemented_Item");
            emxContextUtil_mxJPO.mqlNotice(context, strMessage);
            return 1;
        }
    }
     ...
     * @Since R216
     */
    public Object checkForClone(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
    ...
    public String getCONameForDisplay(Context context, String args[]) throws Exception {
        ...
        String SELECT_AffectedItem_CO = "to[" + ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM + "|attribute[" + attrRequestedChange + "]=='For Release'].from.to["
                + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "].from.name";
        String SELECT_ImplementedItem_Exist = "to[" + ChangeConstants.RELATIONSHIP_IMPLEMENTED_ITEM + "]";
        String SELECT_ImplementedItem_CO = "to[" + ChangeConstants.RELATIONSHIP_IMPLEMENTED_ITEM + "|attribute[" + attrRequestedChange + "]=='For Release'].from.to["
                + ChangeConstants.RELATIONSHIP_CHANGE_ACTION + "].from.name";
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a MapList with the following as input arguments or entries: objectId holds the context ECR/ECO object Id New Value holds the newly selected Related ECR Object Id
     * @throws Exception
     *             if the operations fails
     * @since EC - X3
     */
    ...
    public int addAffectedItemstoCR(Context context, String[] args) throws Exception {
        ...
        String strCRId = programMap.containsKey("newObjectId") ? (String) programMap.get("newObjectId") : (String) paramMap.get("newObjectId");
        String strSourceECRId = programMap.containsKey("sourceECRId") ? (String) programMap.get("sourceECRId") : (String) requestMap.get("sourceECRId");
        String affectedItemsLists = programMap.containsKey("strSelectedAffectedItem") ? (String) programMap.get("strSelectedAffectedItem") : (String) requestMap.get("strSelectedAffectedItem");
    ...
    public Object getClassCode(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - Part object id that is being revised
     * @throws Exception
     *             if the operation fails.
     * @since X3.
     */
    public void connectMasterPartsAndReferenceParts(Context context, String[] args) throws Exception {
        ...
        if (UIUtil.isNotNullAndNotEmpty(PartSeriesON) && "TRUE".equalsIgnoreCase(PartSeriesON)) {
            ...
            if (previousRevObj != null && (!(previousRevObj.toString().trim().equals("..")) || !(previousRevObj.getTypeName().equals("")))) {
                ...
                if ((SYMB_R.equals(sRefType) && (strState.equalsIgnoreCase(STATE_PART_PRELIMINARY) || strState.equalsIgnoreCase(STATE_DEVELOPMENT_PART_CREATE)))
                        || (SYMB_M.equals(sRefType) && (strState.equalsIgnoreCase(STATE_PART_RELEASE) || strState.equalsIgnoreCase(STATE_DEVELOPMENT_PART_COMPLETE)))) {
                    sContextPartFamilyId = (String) doPrevRevObjId.getInfo(context,
                            "relationship[" + RELATIONSHIP_CLASSIFIED_ITEM + "].tomid[" + RELATIONSHIP_PART_FAMILY_REFERENCE + "].torel.from.id");
                    sNewRevClassifiedItemRelIdList = (StringList) doLatestRev.getInfoList(context, "relationship[" + RELATIONSHIP_CLASSIFIED_ITEM + "].id");
                    ...
                    if (SYMB_R.equals(sRefType)) {
                        ...
                        StringList sMasterClassifiedItemRelId = (StringList) doPrevRevObjId.getInfoList(context,
                                "relationship[" + RELATIONSHIP_CLASSIFIED_ITEM + "].frommid[" + RELATIONSHIP_PART_FAMILY_REFERENCE + "].torel.id");
                    ...
                    if (SYMB_M.equals(sRefType)) {
                        ...
                        StringList PART_FAMILY_REFERENCE_REL_ID = (StringList) doPrevRevObjId.getInfoList(context,
                                "relationship[" + RELATIONSHIP_CLASSIFIED_ITEM + "].tomid[" + RELATIONSHIP_PART_FAMILY_REFERENCE + "].id");
    ...
    /**
     * Program HTML Output for name column in alternate tab. This program is used to display different icons for Part and MEP along with part name.
     * @param context
     *            Context : User's Context.
     * @param String
     *            [] args : Arguments
     * @return <code>StringList</code> of HTML elements for displaying name column with icon image.
     * @throws Exception
     */
    public StringList getAlternatePartName(Context context, String[] args) throws Exception {
        ...
        // String nameLinkSrcBeg = new StringBuilder("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxNavigator.jsp?emxSuiteDirectory=")
        String nameLinkSrcBeg = new StringBuilder("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?emxSuiteDirectory=").append(XSSUtil.encodeForJavaScript(context, suiteDir))
                .append("&amp;suiteKey=").append(XSSUtil.encodeForJavaScript(context, suiteKey)).append("&amp;jsTreeID=").append(XSSUtil.encodeForJavaScript(context, jsTreeID))
                .append("&amp;parentOID=").append(XSSUtil.encodeForJavaScript(context, parentOID)).append("&amp;objectId=").toString();
        String nameLinkSrcEnd = "','700','600','false','content')\" class=\"object\">";
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            boolean hasreadAccess = UIComponent.hasReadAccess(context, part.get(SELECT_ID));
            ...
            if (!hasreadAccess)
                nameLinkSrc = imgSrc + part.get(SELECT_NAME);
    ...
    public String getPartConnectedProduct(Context context, String[] args) throws Exception {
        ...
        String sProduct = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3", strObjectId,
                "to[" + PropertyUtil.getSchemaProperty(context, "relationship_GBOM") + "|from.type=='" + PropertyUtil.getSchemaProperty(context, "type_HardwareProduct") + "'].from.name", "|");
     ...
     * @param context
     * @param args
     *            contains the Policy of the Business Object
     * @return 0 for success.
     * @throws Exception
     */
    public int deleteLicenseCheck(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - OBJECTID 1 - KINDOFOWNER 2 - OWNER
     * @throws Exception
     *             if the operation fails.
