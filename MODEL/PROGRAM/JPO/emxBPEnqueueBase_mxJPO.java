 ...
 *
 * FileName : "$RCSfile: ${CLASSNAME}.java.rca $" Author : "$Author: przemek $" Version : "$Revision: 1.9 $" Date : "$Date: Wed Oct 22 15:53:03 2008 $"
 *
 */
...
public class emxBPEnqueueBase_mxJPO extends emxAbsPDFEnqueue_mxJPO {
     ...
     * @param args
     *            Contains a map that has been packed by JPO.packArgs That map Contains: "objectId" - String, Object Id "printerName" - String, the printer's name to print "numberOfCopies" - Integer,
     *            number of copies to print "fileList" - LinkedList, the list of files to print
     * @exception Exception
    ...
    /**
     * runs the Adlib Batch Printing integration (on a seperate Thread) the integration will check out the files, and then poll the output and error direcotries until it's time runs out
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param queue
     *            The PDF Queue with the documents to be Printed
     * @since AEF 9.5.4.0
     */
    protected void runIntegration(Context context, emxPDFQueue_mxJPO queue) {
         ...
         * Author : DJ Date : 02/04/2003 Notes : History :
         */
        String integrationName = _util.getString(context, "eServiceBatchPrintPDF.Batch.Integration");
        ...
        try {
            if (integrationName.equals("emxAdlibRNDIntegration")) {
                ...
                for (int i = 0; i < _numberOfCopies; i++) {
                    new emxAdlibBPIntegration_mxJPO(context, queue, _printerName, _numberOfCopies).start();
