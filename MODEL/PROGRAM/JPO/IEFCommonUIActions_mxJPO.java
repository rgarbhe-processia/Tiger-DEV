...
abstract public class IEFCommonUIActions_mxJPO extends IEFBaseJPO_mxJPO {
    ...
    /**
     * Common Entry Point to JPO This method decided if a perticular link can be displayed in the dashboard UI, based on whether the opration can be done on the selected businessobject After
     * initialization, the controll is given to the derived class for checkin if operation can be performed or not Default implementation is to allow the operation
     */
    ...
    /**
     * Common Entry Point to JPO After initialization, the control is given to the derived class for executing custom logic A Pre-execution-check is also made, whether the operation can be performed
     * The logic for this pre-execution check can be implemented in the Derived class. Default implementation is to allow the operation
     */
    ...
    /**
     * All derived Classes to impelment this method All return parameters like files list for file operations etc. to be returned using Hashtable "resultDataTable". For this hastable, use following
     * String as key MCADServerSettings.JPO_EXECUTION_RESULT for any error condition encountered, the method must throw an MCADException
     */
    ...
    /**
     * Checks if the operation can be performed on the selected BusinessObject. All derived Classes to impelment this method, if required. for any error condition encountered, or for a valid reason
     * for which the operation cannot be performed (any custom business logic) the method must throw an MCADException Default implementation - do nothing, i.e. operation is allowed
     */
