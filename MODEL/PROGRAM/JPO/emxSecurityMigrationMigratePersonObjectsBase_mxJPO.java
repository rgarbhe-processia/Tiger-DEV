 ...
 * @author WGI
 */
public class emxSecurityMigrationMigratePersonObjectsBase_mxJPO extends emxCommonMigration_mxJPO {
     ...
     * @throws Exception
     */
    public emxSecurityMigrationMigratePersonObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
            ...
            while (itr.hasNext()) {
                ...
                if (memberCompanyNames != null) {
                    ...
                    comment += "Person " + personName
                            + " is not connected to any organization with Member relationship or Member relationship doesn't contain any value for Project Role attribute. \n";
                }
                if (unconverted) {
    ...
    public void assignPersonalSecurityContext(Context context, String personName, String companyName) throws Exception {
        try {
            ...
            String prjCommand = "list role '" + projectName + "'";
            String prjResult = MqlUtil.mqlCommand(context, prjCommand, true);
            if (prjResult.indexOf(projectName) == -1) {
                String addProjectCmd = "add role '" + projectName + "' asaproject parent 'User Projects' hidden";
                MqlUtil.mqlCommand(context, addProjectCmd, true);
                mqlLogRequiredInformationWriter("Personal Project Successfully created for " + personName);
            } else {

                prjCommand = "print role $1 select hidden dump $2";
                prjResult = MqlUtil.mqlCommand(context, prjCommand, true, projectName, "|");
                if ("false".equalsIgnoreCase(prjResult)) {
                    String addProjectCmd = "mod role '" + projectName + "' hidden";
                    MqlUtil.mqlCommand(context, addProjectCmd, true);
                    mqlLogRequiredInformationWriter("Personal Project Successfully hidden for " + personName);
                } else {
                    mqlLogRequiredInformationWriter("Personal Project is already hidden for " + personName);
                    mqlLogWriter("Personal Project is already hidden for " + personName);
                }
            ...
            String scCommand = "list role '" + SCName + "'";
            String result = MqlUtil.mqlCommand(context, scCommand, true);
            if (result.indexOf(SCName) == -1) {
                String addSecurityContextCmd = "add role '" + SCName + "' parent Grant,'" + companyName + "','" + personName + "_PRJ' assign person '" + personName + "' hidden";
                MqlUtil.mqlCommand(context, addSecurityContextCmd, true);
    ...
    public void assignSecurityContext(Context context, String companyId, String personId, String personName, String attrValue, String companyName, String companyType, String companyRev,
            StringList securityContextNames, String memberRelId) throws Exception {
        try {
            ...
            if ("TRUE".equalsIgnoreCase(sResult)) {
                ...
                while (addItr.hasNext()) {
                    ...
                    // create a security context with 'Basic User' role
                    if (roleName.equals("role_ExchangeUser") || ComponentsUtil.isChildRole(context, "Exchange User", PropertyUtil.getSchemaProperty(context, roleName))) {
                        addBasicUserRole = true;
                    }
    ...
    private DomainObject createAndAssignSecurityContext(Context context, String personId, String personName, String roleName, String companyType, String companyName, String companyRev)
            throws Exception {
    ...
    public StringList mapProgramCentralLegacyRoles(Context context, StringList addedRoles, String personName, String companyId, String memberRelId) throws Exception {
        ...
        while (roleItr.hasNext()) {
            ...
            if (programLegacyRoleList.contains(legacyRole)) {
                ...
                if (!addedRoles.contains(nwSymbolicRoleName)) { // if nw mapped role is not assigned to user on role attribute
        ...
        if (hasLegacyRole) {
            ...
            AttributeUtil.setAttributeList(context, memberRel, DomainConstants.ATTRIBUTE_PROJECT_ROLE, addedRoles, false, "~");
