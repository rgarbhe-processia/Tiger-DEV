...
public class emxDashboardIssuesBase_mxJPO {
    ...
    public String[] getUserDashboardData(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < mlIssues.size(); i++) {
            ...
            if (!"".equals(sStart)) {
                if (!"".equals(sEnd)) {
                    ...
                    sbDataDate.append("{ low:Date.UTC(").append(sStartDate.get(Calendar.YEAR)).append(",").append(sStartDate.get(Calendar.MONTH)).append(",")
                            .append(sStartDate.get(Calendar.DAY_OF_MONTH)).append("),");
                    sbDataDate.append("  high:Date.UTC(").append(sEndDate.get(Calendar.YEAR)).append(",").append(sEndDate.get(Calendar.MONTH)).append(",").append(sEndDate.get(Calendar.DAY_OF_MONTH))
                            .append("), ");
        ...
        if (sbAxisDate.length() > 0) {
            sbAxisDate.setLength(sbAxisDate.length() - 1);
        }
        if (sbDataDate.length() > 0) {
            sbDataDate.setLength(sbDataDate.length() - 1);
        }

        StringBuilder sbCounter = new StringBuilder();
        sbCounter.append(
                "<td onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=IssueListDetails&program=emxDashboardIssues:getIssuesAssignedPending&header=emxFramework.String.AssignedPendingIssues&freezePane=Name,Edit&suiteKey=Framework\")'");
        sbCounter.append(" class='counterCell ");
        if (mlIssues.size() == 0) {
            sbCounter.append("grayBright");
        } else {
            sbCounter.append("yellow");
        }
        ...
        if (iCountMRU > 0) {
            sbUpdates.append(
                    " onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=IssueListDetails&program=emxDashboardIssues:getIssuesAssignedPending&mode=MRU&header=emxFramework.String.MRUIssues&freezePane=Name,Edit&suiteKey=Framework\")' ");
