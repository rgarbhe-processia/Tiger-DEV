...
public class emxProcessBase_mxJPO {
     ...
     * @grade 0
     */
    public emxProcessBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    public Vector getRadioButtons(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            ...
            // Bug No 318351 - Start
            /*
             * Read the PrinterFriendly parameter from paramMap. In fowwing while loop, checked whether it is PrinterFriendly page. If it is PrinterFriendly an empty space is shown, else radio button
             * is shown as usual.
             */
            ...
            while (index < size) {
                if (isPrinterFriendly != null && "true".equalsIgnoreCase(isPrinterFriendly)) {
                    ...
                    if (index == 0)
                        StatusIcons.add("<input type=\"radio\" name=\"process\" value=\"" + XSSUtil.encodeForHTMLAttribute(context, objectId) + "\" onclick=\"javascript:parent.self.changePage('"
                                + XSSUtil.encodeForURL(context, objectId) + "')\"/>");
                    else
                        StatusIcons.add("<input type=\"radio\" name=\"process\" value=\"" + XSSUtil.encodeForHTMLAttribute(context, objectId) + "\" onclick=\"javascript:parent.self.changePage('"
                                + XSSUtil.encodeForURL(context, objectId) + "')\"/>");
                }
