...
public class emxTeamSearchBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @since Team 10-5
     */
    public emxTeamSearchBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    /**
     * This method will be called when ever we invoke the JPO without calling any method.explicitly
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no value
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds input arguments. ownedFiles - a String true/false to show only files owned by the User owner - a String containing the owner to search for. name - a String containg the name
     *            pattern to search for. keywords - a String containg the text search patterns. WorkspaceFolder - a String containg the Workspace Folder pattern to searchfor. WorkspaceFolderId - a
     *            String containing the Object ID of the Workspace Folder createdAfter - a String containing a created after date. createdBefore - a String containing a created before date. matchCase
     *            - a String true/false to match case. timeZone - a String containing the client timezone.
     * @return MapList containing search result.
     * @exception Exception
     *                if the operation fails.
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getFilesSearchResult(Context context, String[] args) throws Exception {
        ...
        String sRelVaultedDocumentsRev2 = PropertyUtil.getSchemaProperty(context, DomainObject.SYMBOLIC_relationship_VaultedDocumentsRev2);
        ...
        if (sWorkspaceFolder.equals("*")) {
            ...
            while (st.hasMoreTokens()) {
                relPattern.addPattern(sRelVaultedDocumentsRev2);
                folderId = st.nextToken();
                Pattern includeRelPattern = new Pattern(sRelVaultedDocuments);
                includeRelPattern.addPattern(sRelVaultedDocumentsRev2);
                StringList objSelects = new StringList(3);
                objSelects.addElement(DomainObject.SELECT_ID);
                objSelects.addElement(DomainObject.SELECT_NAME);
                objSelects.addElement(DomainObject.SELECT_TYPE);
                templateObj.setId(folderId);
                MapList docMapList = templateObj.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(), objSelects, new StringList(), false, true, expandLevel,
                        busWhere.toString(), null, Short.parseShort(queryLimit), null, includeRelPattern, null);
     ...
     * @param args
     *            holds input arguments. ownedFiles - a String true/false to show only files owned by the User owner - a String containing the owner to search for. name - a String containg the name
     *            pattern to search for. keywords - a String containg the text search patterns. createdAfter - a String containing a created after date. createdBefore - a String containing a created
     *            before date. matchCase - a String true/false to match case. timeZone - a String containing the client timezone.
     * @return MapList containing search result.
     * @exception Exception
     *                if the operation fails.
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public Object getUnmanagedFilesSearchResult(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        ...
        // Formatting Date to Ematrix Date Format
        if (sCreatedAfter != null && !"".equals(sCreatedAfter)) {
            sCreatedAfter = eMatrixDateFormat.getFormattedInputDate(sCreatedAfter, clientTZOffset, localeObj);
        }
        if (sCreatedBefore != null && !"".equals(sCreatedBefore)) {

            sCreatedBefore = eMatrixDateFormat.getFormattedInputDate(sCreatedBefore, clientTZOffset, localeObj);
        ...
        if (null == sFileName || ("null").equals(sFileName.trim()) || ("").equals(sFileName.trim())) {
            sFileName = "*";
        }
        ...
        while (vaultNamesItr.hasNext()) {
            String vault = (String) vaultNamesItr.next();
            MapList docList = DomainObject.findObjects(context, typePattern.getPattern(), // typePattern
                    "*", // namePattern
                    "*", // revPattern
                    owner, // owner pattern
                    vault, // vaultPattern
    ...
    /**
     * getWorkspaceName - This method is used to show the Workspace Name. This method is called from the Column Workspace.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - objectList MapList
    ...
    public MapList getFileSearchOwnerChooser(Context context, String[] args) throws Exception {
        ...
        if (boListGeneric.size() != 0) {
            ...
            MapList vaultMapList = domainCompany.getRelatedObjects(context, vaultRelPattern.getPattern(), // relationshipPattern
            ...
            MapList personList = templateObj.querySelect(context, templateObj.TYPE_PERSON, // type pattern
