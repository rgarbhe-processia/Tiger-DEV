 ...
 **
 ** Program for perfroming SaveAs of MCAD objects
 */
...
import com.matrixone.MCADIntegration.server.managers.IEFSaveAsManager;
...
public class PSS_MCADSaveAs_mxJPO extends MCADSaveAsBase_mxJPO {
    protected static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(PSS_MCADSaveAs_mxJPO.class);
     ...
     * @since Sourcing V6R2008-2
     */
    public PSS_MCADSaveAs_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public String cloneCAD(Context context, String[] argsMap) throws Exception {
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(argsMap);
            String objectId = (String) paramMap.get("objectId");
            String result = "";

            String mlSelectedPartStructure = (String) paramMap.get("SelectedPartStructure");
            // TIGTK-9732 -- 14/9/17 -- START
            PropertyUtil.setGlobalRPEValue(context, "SelectedPartStructure", "'" + mlSelectedPartStructure + "'");
            // TIGTK-9732 -- 14/9/17 -- END
            MCADIntegrationSessionData integSessionData = (MCADIntegrationSessionData) paramMap.get("integSessionData");

            String language = "en-us";
            MCADMxUtil mxUtil = new MCADMxUtil(context, new MCADServerResourceBundle(language), new IEFGlobalCache());
            String integrationName = mxUtil.getIntegrationName(context, objectId.split(";")[0]);
            ...
            inputDetailsMap.put("StdPart", "");
            inputDetailsMap.put("folderId", "");
            inputDetailsMap.put("regEx", "");
            inputDetailsMap.put("replaceString", "");
            inputDetailsMap.put("cadDrwSeries", "FAURECIA");
            inputDetailsMap.put("cadModelSeries", "FAURECIA");
            inputDetailsMap.put("IntegrationName", integrationName);
            inputDetailsMap.put("silentSaveAs", "true");
            inputDetailsMap.put("objectIds", objectId);
            inputDetailsMap.put("Selected", "false");

            IEFSaveAsManager saveAsManager = new IEFSaveAsManager(new Hashtable());
            result = saveAsManager.executeSaveAs(context, objectId, inputDetailsMap, globalConfigObject, integSessionData);

            // TIGTK- 8812 - Modified on 6/7/2017 by SIE :Start
            String strSelectedCADObjectIds = PropertyUtil.getGlobalRPEValue(context, "strSelectedCADObjectIds");
            String strAlreadyProcessedCADs = PropertyUtil.getGlobalRPEValue(context, "strAlreadyProcessedCADs");

            createAsBuildCADStructure(context, strAlreadyProcessedCADs, strSelectedCADObjectIds);
            // TIGTK- 8812 - Modified on 6/7/2017 by SIE :End
            PropertyUtil.setGlobalRPEValue(context, "SelectedPartStructure", "");
            PropertyUtil.setGlobalRPEValue(context, "ProcessedCADObjects", "");

            return result;
        } catch (Exception ex) {
            // TIGTK-5405 - 18-04-2017 - PTE - START
            logger.error("Error in cloneCAD: ", ex);
            throw ex;
    ...
    public void executeCustom(Context _context, Hashtable resultAndStatusTable) throws MCADException {
        ...
        // SteepGraph Modifications: DF_ALM_1491: CLone selected Parts of the EBOM along with related CAD Structure: START
        // TIGTK-4291 - START
        // TIGTK-5259 - 20-03-2017 - VP - START
        try {
            // OriginalPartId|NewPartId|NewPartName|RelatedCADObjects<comma separated>@
            String strSelectedPartStructure = PropertyUtil.getGlobalRPEValue(_context, "SelectedPartStructure");
            String processedCADObjectIds = PropertyUtil.getGlobalRPEValue(_context, "ProcessedCADObjects");
            // TIGTK- 8812 - Modified on 6/7/2017 by SIE :Start
            String strAlreadyProcessedCADs = PropertyUtil.getGlobalRPEValue(_context, "strAlreadyProcessedCADs");
            String strSelectedCADObjectIds = PropertyUtil.getGlobalRPEValue(_context, "strSelectedCADObjectIds");

            StringBuffer strBfrProcessedCADObjectIds = new StringBuffer();
            StringBuffer sbSelectedCADObjectIds = new StringBuffer();
            StringBuffer strBfrAlreadyProcessedCADs = new StringBuffer();
            if (UIUtil.isNotNullAndNotEmpty(strSelectedPartStructure)) {
                String[] sAllCADObjects = busObjectsString.split("@");

                busObjectsString = "";
                StringBuffer strBfrBusObjectsString = new StringBuffer();
                String[] strSelectedParts = strSelectedPartStructure.split(",@");
                String[] strSelectedPart;
                String strNewPartName = "";
                String strSelectedCADs = "";
                String[] strSelectedCADArray;

                for (String selectedPart : strSelectedParts) {
                    strSelectedPart = selectedPart.split("\\|");
                    strSelectedCADs = strSelectedPart[2];
                    strSelectedCADArray = strSelectedCADs.split(",");

                    for (String strSelectedCAD : strSelectedCADArray) {

                        String[] sCADDetails = strSelectedCAD.split("&");
                        strSelectedCAD = sCADDetails[0];
                        strNewPartName = sCADDetails[1];

                        for (String sCADObject : sAllCADObjects) {
                            String[] sObjectInfo = sCADObject.split("\\|");
                            if (strSelectedCAD.trim().equals(sObjectInfo[0].trim())) {

                                sbSelectedCADObjectIds.append(strNewPartName);
                                sbSelectedCADObjectIds.append("@");
                                sbSelectedCADObjectIds.append(sObjectInfo[0].trim());
                                sbSelectedCADObjectIds.append("@");

                                if (!processedCADObjectIds.contains(sObjectInfo[0].trim())) {
                                    strBfrProcessedCADObjectIds.append(sObjectInfo[0].trim());
                                    sCADObject = sCADObject.replace(sObjectInfo[2].trim(), strNewPartName);
                                    strBfrBusObjectsString.append(sCADObject + "@");
                                } else {
                                    // Already Processed CAD
                                    strBfrAlreadyProcessedCADs.append(strNewPartName);
                                    strBfrAlreadyProcessedCADs.append("|");
                                    strBfrAlreadyProcessedCADs.append(sObjectInfo[0].trim());
                                    strBfrAlreadyProcessedCADs.append("@");
                                }
                            }
                        }
                ...
                busObjectsString = strBfrBusObjectsString.toString();
                strBfrProcessedCADObjectIds.append(processedCADObjectIds);
            }
            strBfrAlreadyProcessedCADs.append("&");
            strBfrAlreadyProcessedCADs.append(strAlreadyProcessedCADs);

            sbSelectedCADObjectIds.append("&");
            sbSelectedCADObjectIds.append(strSelectedCADObjectIds);
            // TIGTK-9732 -- 14/9/17 -- START
            PropertyUtil.setGlobalRPEValue(_context, "strAlreadyProcessedCADs", "'" + strBfrAlreadyProcessedCADs.toString() + "'");
            PropertyUtil.setGlobalRPEValue(_context, "strSelectedCADObjectIds", "'" + sbSelectedCADObjectIds.toString() + "'");
            // TIGTK-9732 -- 14/9/17 -- END
            // TIGTK- 8812 - Modified on 6/7/2017 by SIE :End
            processedCADObjectIds = strBfrProcessedCADObjectIds.toString();
            // TIGTK-9732 -- 14/9/17 -- START
            PropertyUtil.setGlobalRPEValue(_context, "ProcessedCADObjects", "'" + processedCADObjectIds + "'");
            // TIGTK-9732 -- 14/9/17 -- END
        ...
        // TIGTK-4291 - END
        // SteepGraph Modifications: DF_ALM_1491: CLone selected Parts of the EBOM along with related CAD Structure: END
        ...
        try {
            ...
            logger.error("Error in executeCustom: ", exception);
    ...
    /**
     * This method is for adding the newly creates instance through saveas to the instance structure for the family.This new instance is to be added at the same level in the structure as the instance
     * from which it is cloned
     * @param instStructureNode
     *            - instance structure
     * @param originalInstanceCadId
     *            - cadid of the instance node from which the new instance was cloned
     * @param newInstNodeForInstStructure
     *            - new instance node to be added to the structure
     * @throws Exception
     */
    protected void addNewInstanceToInstanceStructure(IEFXmlNode instStructureNode, String originalInstanceCadId, IEFXmlNode newInstNodeForInstStructure) throws Exception {
    ...
    private BusinessObject buildInstanceStructure(Context _context, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable, Hashtable busObjExtendedTable, String rootBusID,
            BusinessObject busObject, BusinessObject clonedBusObject, String cadTypeName, boolean isCloned) throws Exception {
    ...
    private BusinessObject buildFamilyStructure(Context _context, String rootBusID, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable, Hashtable busObjExtendedTable,
            BusinessObject clonedBusObject) throws Exception {
    ...
    private BusinessObject buildChildNode(Context _context, String busID, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable, Hashtable busObjExtendedTable, boolean isFirstLevel)
            throws Exception {
        BusinessObject clonedBusObject = null;
        ...
        if (isSavedAs && !MCADSaveAsUtil.isSavedAs(busID, savedAsObjList)) {
            if (sharedPartsIDClonedBusObjectsTable.containsKey(busID)) {
                ...
                if (_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) {
                    ...
                    clonedBusObject = buildInstanceStructure(_context, busIDSaveAsNameTable, busIDTargetRevTable, busObjExtendedTable, busID, busObject, null, cadType, true);

                } else if (_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_FAMILY_LIKE)) {
                    clonedBusObject = buildFamilyStructure(_context, busID, busIDSaveAsNameTable, busIDTargetRevTable, busObjExtendedTable, busObject);
                } else {
            ...
            while (childObjIds.hasMoreElements()) {
                ...
                if (!alreadyExpandedNodesInPath.contains(busID) && shouldAddBus) {
                    ...
                    if (_globalConfig.isTypeOfClass(cadTypeForChild, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) {
                        clonedChildBusObject = buildInstanceStructure(_context, busIDSaveAsNameTable, busIDTargetRevTable, busObjExtendedTable, childBusID, childBusObj, clonedChildBusObject,
                                cadTypeName.toString(), isCloned);
                    } else {
...

                    {
                        ...
                        MCADSaveAsUtil.addConnectedObjList(clonedChildBusObject.getObjectId(_context), clonedBusObject.getObjectId(_context), relId, relationship.getName(), savedAsObjList,
                                processedRelIds);
        ...
        if (isCloned) {
            if (_util.doesObjectHaveInstances(_context, busObject)) {
                ...
                boolean bInstanceUpdated = getUpdatedInstanceInfoTable(_context, busID, busObject, instanceInfo, null, changedNameTable, clonedObjName);
     ...
     */
    private boolean applyVerticalViewOnObject(Context _context, String busID, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable, BusinessObject clonedBusObject, String cadTypeAndName)
            throws MCADException {
    ...
    protected void connectBusinessObject(Context _context, String relID, String originalBusID, BusinessObject clonedBusObj, BusinessObject clonedRelatedDocumentObj, String newInstanceName)
            throws MCADException, MatrixException {
    ...
    protected void updateInstanceFilesAndRelsForSaveAs(Context _context, Hashtable instanceInfo, Hashtable instanceAttrInfo, IEFXmlNodeImpl instanceStructure, BusinessObject clonedBusObject)
            throws Exception {
    ...
    /**
     * This can be invoked from outside and is meant for renaming of files and making rename related changes (for Instances only)
     */
    ...
    private BusinessObject buildChildNodeForFamily(Context _context, String busID, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable, Hashtable busObjExtendedTable, boolean isFirstLevel)
            throws Exception {
        BusinessObject clonedBusObject = null;
        boolean isCloned = false;
    ...
    private BusinessObject buildInstance(Context _context, BusinessObject clonedBusObject, BusinessObject busObject, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable,
            ...
            while (allRels.hasMoreElements()) {
                ...
                if (!MCADSaveAsUtil.isConnectedWith(rel.getName(), "", processedRelIds)) {
                    ...
                    if (canConnect) {
                        ...
                        _util.copyAttributesonRelatinship(_context, rel, relationship);
                        MCADSaveAsUtil.addConnectedObjList(clonedChildBusObject.getObjectId(_context), clonedBusObject.getObjectId(_context), rel.getName(), relationship.getName(), savedAsObjList,
                                processedRelIds);
    ...
    protected BusinessObject saveAsBusinessObject(Context _context, String boID, Hashtable busIDSaveAsNameTable, Hashtable busIDTargetRevTable) throws Exception {
        ...
        try {
            ...
            // TIGTK-5405 - 18-04-2017 - PTE - START
            logger.error("Error in saveAsBusinessObject: ", ex);
            throw ex;
            // TIGTK-5405 - 18-04-2017 - PTE - End
    ...
    public void removeAllInterfaces(Context context, String objectId) {
        try {
            ...
            // TIGTK-5405 - 18-04-2017 - PTE - START
            logger.error("Error in removeAllInterfaces: ", ex);
            // TIGTK-5405 - 18-04-2017 - PTE - End
    ...
    /**
     * input: boID - id of selected instance obnject busSaveAsName - new name busTargetRev - new target rev familyName - name of family of the instance Output: cloned bus object
     * @param busIDSaveAsNameTable
     */
    ...
    private String getAutoName(Context _context, BusinessObject busObject) throws Exception {
        ...
        /*
         * boolean isTMCSilentSaveAS = false; if (_argumentsTable.get("TMCSilentSaveAS") != null) isTMCSilentSaveAS = (boolean) _argumentsTable.get("TMCSilentSaveAS");
         */
        /*
         * if(isTMCSilentSaveAS) { sType = "type_MCADModel"; }
         */
    ...
    protected void createTowerForSavesAsInstance(Context _context, BusinessObject instObject, BusinessObject clonedInstObj, String targetRevision, Hashtable busIDSaveAsNameTable) throws Exception {
        try {
            Relationship rel = null;
            ...
            if (null != famObj) {
                ...
                for (int i = 0; i < relNames.size(); i++) {
                    ...
                    _util.copyAttributesonRelatinship(_context, relationship, relBetMajors);
                    MCADSaveAsUtil.addConnectedObjList(clonedInstObj.getObjectId(_context), famObj.getObjectId(_context), relationship.getName(), relBetMajors.getName(), savedAsObjList,
                            processedRelIds);
    ...
    public void renameIndividual(Context _context, BusinessObject renamedObj, String originalObjectId, boolean isShared, boolean setRenamedFromOnFamilyLike) throws Exception {
        ...
        StringBuffer strBufferSELECT_TITLE_ATT = new StringBuffer();
        strBufferSELECT_TITLE_ATT.append("attribute[");
        strBufferSELECT_TITLE_ATT.append(MCADMxUtil.getActualNameForAEFData(_context, "attribute_Title"));
        strBufferSELECT_TITLE_ATT.append("]");
        String SELECT_TITLE_ATT = strBufferSELECT_TITLE_ATT.toString();
        ...
        if (renamedObj != null) {
            ...
            if (_globalConfig.isTypeOfClass(cad_type, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) {
                ...
                if (newMajorObj != null && newMajorObj.exists(_context)) {
            ...
            if (!isDerivedOutputLike) {
                ...
                try {
                    ...
                    if (hasAccess)
                        _generalUtil.doActualRename(_context, renamedObj, newName, oldName, newNameForFileOperation, valueForRenamedFrom, setRenamedFromOnFamilyLike, false);
                } // Fix for FindBugs issue RuntimeException capture: Suchit Gangurde: 28 Feb 2017: START
                catch (RuntimeException ex) {
                    // TIGTK-5405 - 18-04-2017 - PTE - START
                    logger.error("Error in renameIndividual: ", ex);
                    throw ex;
                    // TIGTK-5405 - 18-04-2017 - PTE - End
                ...
                try {
                    // Find Bug Issue Fix : PS
                    if (businessObj != null) {
                        boolean hasAccess = businessObj.checkAccess(_context, (short) Access.cModify);
                        if (hasAccess)
                            _generalUtil.doActualRename(_context, businessObj, newName, oldName, newNameForFileOperation, valueForRenamedFrom, setRenamedFromOnFamilyLike, false);
                    }
                } // Fix for FindBugs issue RuntimeException capture: Suchit Gangurde: 28 Feb 2017: START
                catch (RuntimeException ex) {
                    // TIGTK-5405 - 18-04-2017 - PTE - START
    ...
    /**
     * Copies Objects connected with busID object with "Dependent Document" relationship, and connect them with oldNewDepDocNames. If arg3(oldNewDepDocNames) is null, name of the dependent doc object
     * will be same as oldNewDepDocNames, otherwise name in the oldNewDepDocNames will be used. Note that the oldNewDepDocNames contains a string of the form: newInstName + "|" + newDepDocName
     */
        ...
        try {
            ...
            while (allRels.hasMoreElements()) {
                ...
                if (!MCADSaveAsUtil.isConnectedWith(rel.getName(), "", processedRelIds)) {
                    ...
                    MCADSaveAsUtil.addConnectedObjList(clonedDepDocBOId, clonedBusObjId, rel.getName(), "", savedAsObjList, processedRelIds);
                    ...
                    MCADSaveAsUtil.addConnectedObjList(clonedDepDocBOId, clonedBusMinorObjId, rel.getName(), "", savedAsObjList, processedRelIds);
            ...
            MCADServerException.createException(_serverResourceBundle.getString("mcadIntegration.Server.Message.DependentDocsCopyingFailed") + e.getMessage(), e);
        }
    }

    /**
     * Returns a hashtable of new instances to be set on newly cloned family object contains the instances of the original object - if the instance also got saved as, the new name is sent back,
     * otherwise the original name is retained.
     */
    protected boolean getUpdatedInstanceInfoTable(Context _context, String familyID, BusinessObject familyBusObject, Hashtable retInstanceInfoTable, Hashtable retInstanceAttrInfoTable,
    ...
    /**
     * Input : ParsedXML inNode - Node to be copied from Returns : cloned ParsedXML
     */
    ...
    /**
     * This method is used to connect the newly created Object with the selected folder durign saveas operation
     */
    ...
    /**
     * This method will Create the information for EBOM Synchronization for selected root nodes This method will return clonedObjectID or objectID1|objectID2|...
     */
    ...
    /**
     * This Method is Used to Rebuild the Structure
     * @param context
     * @param strAlreadyProcessedCADs
     * @param strSelectedCADObjectIds
     * @throws Exception
     *             TIGTK- 8812 Added on 6/7/2017 by SIE START
     */
    public void createAsBuildCADStructure(Context context, String strAlreadyProcessedCADs, String strSelectedCADObjectIds) throws Exception {
        try {
            String[] sAllselectedOldCAD;
            String[] sAllAlreadyProcessedCAD;
            String[] sAllselectedNewCAD;
            String[] sAllSelectedCADObjectId;
            String[] sAllAlreadyProcessedCADs = strAlreadyProcessedCADs.split("&");
            String[] sAllSelectedCADObjectIds = strSelectedCADObjectIds.split("&");

            for (String strAlreadyProcessedCAD : sAllAlreadyProcessedCADs) {
                if (UIUtil.isNotNullAndNotEmpty(strAlreadyProcessedCAD)) {
                    sAllAlreadyProcessedCAD = strAlreadyProcessedCAD.split("@");
                    for (String strSelectedCADObjectId : sAllSelectedCADObjectIds) {
                        sAllSelectedCADObjectId = strSelectedCADObjectId.split("@");
                        for (String selectedOldCAD : sAllAlreadyProcessedCAD) {
                            sAllselectedOldCAD = selectedOldCAD.split("\\|");
                            String strCADName = sAllselectedOldCAD[0];
                            String strCADId = sAllselectedOldCAD[1];

                            DomainObject domOldCADObj = DomainObject.newInstance(context, strCADId);
                            String strNewCADType = domOldCADObj.getInfo(context, DomainConstants.SELECT_TYPE);
                            String strNewCADRevision = domOldCADObj.getInfo(context, DomainConstants.SELECT_REVISION);

                            for (String selectedNewCAD : sAllSelectedCADObjectId) {
                                sAllselectedNewCAD = selectedNewCAD.split("\\|");
                                String strNewCADName = sAllselectedNewCAD[0];
                                ...
                                // Object Where clause
                                // TIGTK-9732 - 13/09/17 -- START
                                String strObjectWhere = "name == '" + strNewCADName + "'";
                                // TIGTK-9732 - 13/09/17 -- END
                                Pattern relationshipPattern = new Pattern(TigerConstants.RELATIONSHIP_ASSOCIATEDDRAWING);
                                relationshipPattern.addPattern(TigerConstants.RELATIONSHIP_CADSUBCOMPONENT);
                                // Type Pattern
                                Pattern typePattern = new Pattern(DomainConstants.TYPE_CAD_DRAWING);
                                typePattern.addPattern(DomainConstants.TYPE_CAD_MODEL);
                                // Get New CADs which connected to Old CADs
                                MapList mlConnectedCADList = domOldCADObj.getRelatedObjects(context, // context
                                ...
                                if (!mlConnectedCADList.isEmpty()) {
                                    for (int i = 0; i < mlConnectedCADList.size(); i++) {
                                        Map mapObject = (Map) mlConnectedCADList.get(i);
                                        String strRelId = (String) mapObject.get(DomainConstants.SELECT_RELATIONSHIP_ID);
                                        BusinessObject busCADObject = new BusinessObject(strNewCADType, strCADName, strNewCADRevision, TigerConstants.VAULT_ESERVICEPRODUCTION);
                                        // Replaces Old Connected CAD with new CAD
                                        if (busCADObject.exists(context)) {
                                            String strNewCADId = busCADObject.getObjectId(context);
                                            DomainObject domNewCADObjId = DomainObject.newInstance(context, strNewCADId);
                                            DomainRelationship.setToObject(context, strRelId, domNewCADObjId);
                                        }
                                    }
                                }
                            }
                        }
                        sAllSelectedCADObjectIds = (String[]) ArrayUtils.remove(sAllSelectedCADObjectIds, 0);
                        break;
                    }
                }
            }
        } catch (RuntimeException ex) {
            logger.error("Error in createAsBuildCADStructure: ", ex);
            throw ex;
        } catch (Exception ex) {
            logger.error("Error in createAsBuildCADStructure: ", ex);
            throw ex;
        } finally {
            PropertyUtil.setGlobalRPEValue(context, "strBfrNotProcessedCADObjectIds", "");
            PropertyUtil.setGlobalRPEValue(context, "strSelectedCAD", "");
