...
/**
 * @author DAR Publish Subscribe objects migration in order to support unicorn new relationship model 02/2010
 */
...
public class emxPubSubNewRelMigration_mxJPO {
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (true) {
            if (0 == getGlobalRc())
                setGlobalRc(1);
            throw new Exception("must specify method on emxPubSubNewRelMigration invocation");
        }
        return getGlobalRc();
    }
    ...
    private void applyMQLCommandList(Context context, boolean simulate) throws Exception {
        ...
        // commit transaction ;
        if (null == _modBusCommandList || _modBusCommandList.size() <= 0)
            return;
        String tmpCmd = null;
        MQLCommand mql = new MQLCommand();
        try {
    ...
    private int run(Context context, boolean simulate) throws Exception {
        ...
        try {
            ...
            if (0 == getGlobalRc())
                setGlobalRc(8);
            throw new Exception(DCexc.getMessage());
        } finally {
    ...
    public int dataMigration(Context context, String[] args) throws Exception {
        try {
            ...
            System.out.println("emxPubSubNewRelMigration rc=" + getGlobalRc());
        }
        return getGlobalRc();
    }
