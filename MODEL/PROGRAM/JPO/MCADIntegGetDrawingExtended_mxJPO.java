...
public class MCADIntegGetDrawingExtended_mxJPO {
    ...
    /***
     *** The value needs to be set in this way <relName1>,<relDirection1>,<type1@type2@type3>|<relName2>,<relDirection2>,<type2> The direction specified is the direction on which the type specified is
     * connected. for eg Cat Design Table is connected at "to" side with Cat Drawing in CatDesignTable relationship. Only major types need to be specified here the code will look for the minor tpye
     * also.
     **/
     ...
     * show it's dependent objects on the checkout page. The implementation should be designed to use this business object ID and expand it, and create a hashtable which contains relationship IDs as
     * "keys" and busIDs as "values". The keys in the hashtable should be the relationship IDs of those objects which are to be shown in the vertical view , and the values should be the busIDs of the
     * objects. This hashtable should be returned by the function. It is then used by IEF to show the related objects in the vertical view on the checkout page.
     * @param context
     *            The user context
     * @param args
     ...
     * @return a hashtable which contains relationship IDs of the objects which need to be shown as dependents in the vertical view as "keys" and the objects corresponding busIDs as "values".
     */
