 ...
 *
 * Last Updated On: 20th October 2005. Modified for optionCompatibility rule validation to check for FL id and not feature id.
 */
 ...
 * @version ProductCentral 10.0.0.0 - Copyright (c) 2004-2015 Dassault Systemes.
...
public class emxBatchRuleProcessingBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strPositionString
     *            a string containing the positing string of the feature.
     * @param errorHolder
     *            a ProductConfigurationHolder object
     * @param strFeatureType
     *            a String giving the type of the feature
     * @param strFeatureName
     *            a String giving the name of the feature.
     * @param strFeatureRevision
     *            a String giving the revision of the feature.
     * @param strOptionType
     *            a String giving the type of the option.
     * @param strOptionName
     *            a String giving the name of the option.
     * @param strOptionRevision
     *            a String goving the revision of the option.
     * @return String "TRUE" or "FALSE"
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.0.0.0 this could not be deprecated as referenced from migration JPO
     */
    protected static String evaluateExpressionForSubFeatures(Context context, String strPositionString, ProductConfigurationHolder errorHolder, String strFeatureType, String strFeatureName,
            String strFeatureRevision, String strOptionType, String strOptionName, String strOptionRevision) throws Exception {

        String strResult = STR_FALSE;
        String strLanguage = context.getSession().getLanguage();

        String strGeneralErrorMessage = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Error.UnknownException", strLanguage);
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strFeatureOptionID
     *            string containg the id of feture/option
     * @param errorHolder
     *            the ProductConfiguration holder object.
     * @return String Position String of the feature option.
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.0.0.0 this could not be deprecated as referenced from migration JPO
     */
    protected static String getFeatureOptionPositionString(Context context, String strFeatureOptionID, ProductConfigurationHolder errorHolder) throws Exception {
    ...
    /**
     * isOptionSelected() method returns the boolean true/false depending on whether the feature/option which is passed to this function as strpassString is a selected option for the
     ...
     * @return boolean
     * @since ProductCentral 10.0.5.0 this could not be deprecated as referenced from migration JPO
     */
    public static boolean isOptionSelected(Context context, String strpassString, String strProductConfigurationID) throws Exception {
        ...
        try {
            ...
            String strFeatType = "relationship[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].businessobject." + DomainConstants.SELECT_TYPE;

            String strFeatRev = "relationship[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].businessobject." + DomainConstants.SELECT_REVISION;

            String strFeatName = "relationship[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].businessobject." + DomainConstants.SELECT_NAME;

            String strParentFeatType = "to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from." + DomainConstants.SELECT_TYPE;

            String strParentFeatRev = "to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from." + DomainConstants.SELECT_REVISION;

            String strParentFeatName = "to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from." + DomainConstants.SELECT_NAME;
            ...
            relationshipAttributeList.add("attribute[" + ProductLineConstants.ATTRIBUTE_QUANTITY + "]");
            ...
            // "",
            // "");
            String contextFeature = productConfiguration.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_PRODUCT_CONFIGURATION + "].from." + ProductLineConstants.SELECT_ID);
            DomainObject domProductConfigurationObject = new DomainObject(contextFeature);

            MapList lstSelectedOptionsFeature = productConfiguration.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_SELECTED_OPTIONS, "*", objSelects, null, false, true, (short) 1, "",
                    "", 0);
            ...
            if (strFeatureType.equalsIgnoreCase("CommonGroup")) {
                ...
                // "print connection \""+strFeatureName+"\" select
                // frommid["+ConfigurationConstants.RELATIONSHIP_COMMON_VALUES+"].to."+"from["+ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO+"].to."+DomainConstants.SELECT_ID+" dump |";
                String strCommonValues = MqlUtil.mqlCommand(context, "print connection $1 select $2 dump $3", strFeatureName,
                        "frommid[" + ConfigurationConstants.RELATIONSHIP_COMMON_VALUES + "].to." + "from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + DomainConstants.SELECT_ID,
    ...
    /**
     * calculateExpression() method evaluates the input expression containing feature/option pairs to True or false This method does the function of separating each feature option pairs from the
     * expression obtained from the attribute of a Feature List in the format fType::fName::fRevision~oType::oName::oRevision. The expression is then send to findFeatureOptions() method which returns
     * a boolean depending on wheather user has selected this feature option or not.The boolean obtained is used to form a mql command by replacing it back in the expression with its feature/options
     * which is then evaluated.The result obtained is returned back as a String depending upon if the entire expression was true then "TRUE" else "FALSE".
     * @param context
     *            context for this request
     * @param strObjectID
     *            Object ID on which the expression is to be evaluated
     * @param strExpression
     *            Expression containing feature/option pairs which are to be evaluated
     * @param errorHolder
     *            The bean instance for the product
     * @return String "TRUE" or "FALSE"
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.0.0.0 this could not be deprecated as referenced from migration JPO
    ...
    public static String calculateExpression(Context context, String strObjectID, String strExpression, String strProductConfigurationID, ProductConfigurationHolder errorHolder) throws Exception {
        ...
        try {
            ...
            if (strTempExpression != null && !strTempExpression.equals(DomainConstants.EMPTY_STRING)) {
                ...
                for (itemp = 0; itemp < iTempExpressionLength; itemp++) {
                    ...
                    if ((strTempExpression.charAt(itemp) == STR_QUOTES.charAt(0)) || (strTempExpression.charAt(itemp) == OPEN_BRACE) || (strTempExpression.charAt(itemp) == CLOSE_BRACE)
                        ...
                        String strpassString = strTempExpression.substring(icount, itemp).trim();
                        ...
                        if (!strpassString.equals(DomainConstants.EMPTY_STRING) && strpassString != null) {
                            ...
                            if (bfCheckPoint == true && boCheckPoint == true) {
                                ...
                                if (itemp < iTempExpressionLength - 1) {
                                    if (bcheckResult == true) {
                                        strFinalExpressionString = strFinalExpressionString.concat(STR_TRUE) + strTempExpression.substring(itemp + 1, iTempExpressionLength);
                                    } else if (bcheckResult == false) {
                                        strFinalExpressionString = strFinalExpressionString.concat(STR_FALSE) + strTempExpression.substring(itemp + 1, iTempExpressionLength);
                                    }
                                } else {
                                    if (bcheckResult == true) {
                ...
                if (ienterPoint == 0) {
                    boolean bcheckResult = findFeatureOptions(context, strTempExpression, errorHolder);
                    if (bcheckResult == true) {
                        strFinalExpressionString = strFinalExpressionString.concat(STR_TRUE);
                    } else if (bcheckResult == false) {
                        strFinalExpressionString = strFinalExpressionString.concat(STR_FALSE);
                ...
                if (countOpeningBraces < countClosingBraces) {
                    ...
                    String strsymb = "(";
                    sbFinalExpression.append(strsymb).append(strFinalExpressionString);
                ...
                /*
                 * String strcommand = "evaluate expression \"" + strFinalExpressionString + "\" on bus " + strObjectID;
                 */
                ...
                if (bMQLResult == true) {
                    ...
                    String strEvaluateExpressionFailure = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Error.EvaluateExpressionFailure", strLanguage);
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strFeatureOption
     *            String containing the TNR for the Feature Option to be found out
     * @param errorHolder
     *            the bean instance
     ...
     * @since ProductCentral 10.0.0.0 this could not be deprecated as referenced from migration JPO
     */
    public static boolean findFeatureOptions(Context context, String strFeatureOption, ProductConfigurationHolder errorHolder) throws Exception {
        ...
        try {
            ...
            for (iCount = 0; iCount < iSelectedOptionsize; iCount++) {
                ArrayList arrVelement = (ArrayList) arrSelectedOptionsArrayList.get(iCount);
                String strSelectedFeatureType = arrVelement.get(0).toString().trim();
                String strSelectedFeatureName = arrVelement.get(1).toString().trim();
                String strSelectedFeatureRevision = arrVelement.get(2).toString().trim();
                String strSelectedOptionType = arrVelement.get(14).toString().trim();
                String strSelectedOptionName = arrVelement.get(15).toString().trim();
                String strSelectedOptionRevision = arrVelement.get(16).toString().trim();
                String strSelectedOptionId = arrVelement.get(17).toString().trim();
                ...
                if (bOnlyFeature) {
                    ...
                    if (strSelectedFeatureType.equals(strFeatureType) && strSelectedFeatureName.equals(strFeatureName) && strSelectedFeatureRevision.equals(strFeatureRevision)) {
                        bResult = true;
                        break;
                    }
                } else {
                    if (strSelectedFeatureType.equals(strFeatureType) && strSelectedFeatureName.equals(strFeatureName) && strSelectedFeatureRevision.equals(strFeatureRevision)
                            && strSelectedOptionType.equals(strOptionType) && strSelectedOptionName.equals(strOptionName) && strSelectedOptionRevision.equals(strOptionRevision)) {
                        bResult = true;
                        break;
                    }
                }
                ...
                String strSelectedOptionPositionString = getFeatureOptionPositionString(context, strSelectedOptionId, errorHolder);
                ...
                if (strSelectedOptionPositionString != null && !strSelectedOptionPositionString.equals("") && !strSelectedOptionPositionString.equals("null")) {
                    String strExpressionEvaluation = evaluateExpressionForSubFeatures(context, strSelectedOptionPositionString, errorHolder, strFeatureType, strFeatureName, strFeatureRevision,
     ...
     */
    public static MapList getSelectedOptions(Context context, String objectId) throws Exception {
        String strComma = ",";
        String relationshipName = ProductLineConstants.RELATIONSHIP_SELECTED_OPTIONS;
        String strType = ProductLineConstants.TYPE_FEATURE_LIST + strComma + ProductLineConstants.TYPE_FEATURES;
        ...
        MapList relBusObjPageList = domainObject.getRelatedObjects(context, relationshipName, strType, objectSelects, relSelects, false, true, shRecursionLevel, DomainConstants.EMPTY_STRING,
     ...
     */
    public static String calculateExpression(Context context, String strObjectID, String strProductConfigurationID, ProductConfigurationHolder errorHolder) throws Exception {
        ...
        try {
            ...
            String strContextId = domPC.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_PRODUCT_CONFIGURATION + "].from.id");
            ...
            if (!(strRightExpression == null || ("".equals(strRightExpression)) || ("null".equals(strRightExpression)))) {
                ...
                strBuffer = strBuffer.append("evaluate expression \"").append(boolleanExpression).append("\" on bus ").append(strObjectID);
    ...
    /**
     * This method is called from gbomParseRulesIdBased(). It does the actual evaluation of part Inclusion Rules against the selected options & returns True or False.
     * @param context
     * @param strObjectID
     ...
     *             this could not be deprecated as referenced from migration JPO
     */
    public static String calculateExpressionIdBased(Context context, String strObjectID, String strRightExpression, HashMap selectedOptions) throws Exception {
                ...
                for (int k = 0; k < tempList.size(); k++) {
                    ...
                    if (b_rIDS.length() > 0 && !(b_rIDS.equals(AND) || b_rIDS.equals(OR) || b_rIDS.equals(NOT) || b_rIDS.charAt(0) == OPEN_BRACE || b_rIDS.charAt(0) == CLOSE_BRACE)) {
                        ...
                        if (b_rIDS.startsWith("R")) {
                            ...
                            if (selectedOptions.keySet().contains(b_rIDS.substring(1)) || isFeature) {
                                boolleanExpression = boolleanExpression.replace(b_rIDS, STR_TRUE);
                            } else {
                                boolleanExpression = boolleanExpression.replace(b_rIDS, STR_FALSE);
                            }
