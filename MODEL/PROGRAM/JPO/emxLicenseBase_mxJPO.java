...
/**
 * @author ZWE The <code>emxLicenseBase</code> class/interface contains ...
...
public class emxLicenseBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public MapList getListOfLicenseServerProducts(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            boolean isITAdministrationUser = isITAdministrationUser(context);
            ArrayList<String> alAssignableProducts = fpdm.pno.Product_mxJPO.getAssignableProductsManually(context, null);
            ...
            if (UIUtil.isNotNullAndNotEmpty(objectId)) {
                ...
                if (objectId.indexOf(".") == -1 && !dObj.exists(context) && "".equals(parentId)) {
                    ...
                    if (!"".equals(title)) {
                        ...
                        for (int i = 0; i < strlist.size(); i++) {
                            ...
                            if (!isITAdministrationUser && !alAssignableProducts.contains(productName)) {
                ...
                for (int k = 0, len = info.size(); k < len; k++) {
                    ...
                    if (!isITAdministrationUser && !alAssignableProducts.contains(licTrigram)) {
                        prodtMap.put("disableSelection", "true");
                    }
    ...
    public MapList getListOfDatabaseProducts(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (objectId != null && !"".equalsIgnoreCase(objectId)) {
                ...
                mqlCommand = MqlUtil.mqlCommand(context, "list product * where !technical");
                ...
                for (int m = 0; m < strlProducts.size(); m++) {
                    ...
                    if (!isValidProduct) {
                        prodtMap = new HashMap();
                        StringList slAssignees = null;
                        String strCommand = "print product " + strProductName.trim() + " select person dump";
                        String strlistOfAssignees = MqlUtil.mqlCommand(context, strCommand);
                        if (strlistOfAssignees != null && !"".equals(strlistOfAssignees)) {
                            slAssignees = FrameworkUtil.split(strlistOfAssignees, ",");
                            if (slAssignees.size() != 0) {
                                strTotalLicensesAssigned = String.valueOf(slAssignees.size());
        ...
        try {
            ...
            for (int i = 0; i < objPageList.size(); i++) {
                ...
                if (UIUtil.isNotNullAndNotEmpty(sCasaulHours)) {
                    ...
                    if (iCasualHours > 0) {
                        columnValues
                                .add(EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(context.getSession().getLanguage()), "emxComponents.Common.Licensing.Casual"));
                    } else {
                        columnValues
                                .add(EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(context.getSession().getLanguage()), "emxComponents.Common.Licensing.Full"));
                    }
                } else {
                    columnValues.add("");
                }
    ...
    public void updateLicense(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String strUserCommand = "print bus " + strObjectId + " select name dump";
            strPersonName = MqlUtil.mqlCommand(context, strUserCommand);
            ...
            if ("Yes".equals(strNewValue)) {
                ...
                if (strlConsumedPersonList.contains(strPersonName)) {
                    ...
                    try {
                        ...
                        ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, null);
                        bPushedContext = true;
                        LicenseUtil.releaseLicenses(context, strPersonName, new String[] { strLicenseTrigram }, null);
                    } finally {
                        if (bPushedContext) {
                            ContextUtil.popContext(context);// Pop context from User Agent
                            bPushedContext = false;
                        }
                    }
                }
                String strPersonCommand = "print product " + strLicenseTrigram + " select person dump |";
                String strPersonResult = MqlUtil.mqlCommand(context, strUserCommand, true);
                StringList strResultList = FrameworkUtil.split(strPersonResult, "|");
                if (strResultList.contains(strPersonName)) {
                    strUserCommand = "modify product " + strLicenseTrigram + " remove person \"" + strPersonName + "\"";
                    strResult = MqlUtil.mqlCommand(context, strUserCommand, true);
                }
     ...
     *            the eMatrix <code>Context</code> object.
     * @param name
     *            contains the Trigram of the product
     * @param language
     *            contains the current session language
     ...
     *            the eMatrix <code>Context</code> object.
     * @param prodId
     *            holds product trigram.
     * @param licenseServername
     *            holds name of the server where LUM Server is installed.
    ...
    /**
     * This method will be used to reload the 'User Product License' structure browser data. When a license is assigned or deassigned for a person, we need to display the updated information against
     * Licenses - Assigned and Consumed columns. In this method we are updating the 'ObjectList' with the new information.
     * @param context
     * @param args
    ...
    public Map reloadUserProductLicense(Context context, String[] args) throws Exception {
        ...
        String strUserName = MqlUtil.mqlCommand(context, "print bus " + personObjectId + " select name dump"); // Get the Person Name from object Id
    ...
    public StringList isPersonInactive(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            sProductName = (String) map.get("id");
            if (!isITAdministrationUser && !alAssignableProducts.contains(sProductName)) {
    ...
    private boolean isITAdministrationUser(Context context) {
        boolean bRet = false;

        try {
            bRet = PersonUtil.hasAssignment(context, PropertyUtil.getSchemaProperty(context, "role_FPDM_ITAdministrationUser"));
