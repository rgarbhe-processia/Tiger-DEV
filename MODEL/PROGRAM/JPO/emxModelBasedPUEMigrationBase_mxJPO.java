...
public class emxModelBasedPUEMigrationBase_mxJPO extends emxCommonMigration_mxJPO {
    ...
    private static final String RELATIONSHIP_NAMED_EFFECTIVITY = PropertyUtil.getSchemaProperty("relationship_NamedEffectivity");

    public static final String RELATIONSHIP_NAMED_EFFECTIVITY_USAGE = PropertyUtil.getSchemaProperty("relationship_NamedEffectivityUsage");

    private static final String SELECT_NAMED_EFFECTIVITY_ID = "from[" + RELATIONSHIP_NAMED_EFFECTIVITY + "].to.id";

    public static final String SELECT_NAMED_EFFECTIVITY_USAGE_RELIDS = "to[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].fromrel.id";

    public static final String SELECT_CHANGE_ID_FOR_NAMED_EFFECTIVITY = "to[" + RELATIONSHIP_NAMED_EFFECTIVITY + "].from.id";

    public static final String ATTRIBUTE_EFFECTIVITY_EXPRESSION = PropertyUtil.getSchemaProperty("attribute_EffectivityExpression");

    private static final String SELECT_ATTRIBUTE_EFFECTIVITY_EXPRESSION_ON_NE = "from[" + RELATIONSHIP_NAMED_EFFECTIVITY + "].to.attribute[" + ATTRIBUTE_EFFECTIVITY_EXPRESSION + "]";

    private static final String SELECT_EBOMRELIDS_TO_NE_FOR_CHANGEID = "from[" + RELATIONSHIP_NAMED_EFFECTIVITY + "].to.to[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].fromrel.id";
     ...
     * @grade 0
     */
    public emxModelBasedPUEMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }

    /**
     * This method does the migration work. All the assigned part relationships between Product and the Top Level part gets replaced by corresponding Model and the Top Level part.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectIdList
    ...
    public void migrateObjects(Context context, StringList objectIdList) throws Exception {
        ...
        // just write the problematic ids to log else proceed with migration
        if (scan) {
            return;
        }
        ...
        MapList autoNEExprList = new MapList();
        MapList changeObjList = new MapList();
        MapList mlEffectivity = new MapList();
        Map mlAutoEffectivity = new HashMap();

        // HashMap changeAppMap = new HashMap();
        HashMap changeExprMap = new HashMap();
        HashMap autoNEExprMap = new HashMap();
        Map mapEffectivity;
        Map prodAppMap;
        ...
        try {
            ...
            // Code for auto NE migration started.

            mlAutoEffectivity = effectivity.getAutoEffectivity(context, changeObjList);

            Iterator itr1 = mlAutoEffectivity.keySet().iterator();
            while (itr1.hasNext()) {
                String autoNEid = (String) itr1.next();
                mapEffectivity = (Map) mlAutoEffectivity.get(autoNEid);
                actualExpr = (String) mapEffectivity.get(EffectivityFramework.ACTUAL_VALUE);
                if ("".equals(actualExpr))
                    continue;
                ...
                Iterator itr2 = prodAppMap.keySet().iterator();
                while (itr2.hasNext()) {
                    sProdObjId = (String) itr2.next();
                ...
                autoNEExprMap.put(SELECT_ATTRIBUTE_EFFECTIVITY_EXPRESSION, modelExpr);
                autoNEExprMap.put(DomainObject.SELECT_ID, autoNEid);
                autoNEExprList.add(autoNEExprMap);
                autoNEExprMap = new HashMap();
            }
            effectivity.setAutoEffectivity(context, autoNEExprList);

            // Code for auto NE migration over.
            ...
            mqlLogRequiredInformationWriter("******************Effectivity Maplist with for the given StringList of PUE ECO object ids :--\n" + mlEffectivity + "\n");
            int EffectivitySize = mlEffectivity.size();
            for (int i = 0; i < EffectivitySize; i++) {
                ...
                prodAppMap = effectivity.getExpressionSequence(context, actualExpr);
                mqlLogRequiredInformationWriter("\n****************** product Applicable Map ************\n" + prodAppMap);
                // This Loop prepares Models and Effectivities as key/value pairs for a given expression
                itr = prodAppMap.keySet().iterator();
                while (itr.hasNext()) {
                    sProdObjId = (String) itr.next();
                    String prodnamemodelPhyId = (String) DomainObject.newInstance(context, sProdObjId).getInfo(context, SELECT_NAME);
                    String prodnamemodelType = (String) DomainObject.newInstance(context, sProdObjId).getInfo(context, SELECT_TYPE);
                    modelPhyId = (String) DomainObject.newInstance(context, sProdObjId).getInfo(context, SELECT_MODEL_ID);
                    if (prodnamemodelType.equals("Model")) {
                        mqlLogRequiredInformationWriter("Migration was already done for this id so modelPhyId is " + modelPhyId + "\n");
                    }
                    if (modelPhyId == null) {
                        mqlLogRequiredInformationWriter("Either change is already migrated or Product in expression does not have connected Model \n");
                        mqlLogRequiredInformationWriter("Error *****Failed******");
                    }
                    applValue = (String) prodAppMap.get(sProdObjId);
                    // if expression involves two product revisions from same model id then make union of those applicabilities
                    mqlLogRequiredInformationWriter("ModelPhyID and appValue passed :" + modelPhyId + " & " + applValue + "\n");
                    if (modelAppMap.containsKey(modelPhyId)) {
                        applValue = getUnionOfApplicabilities(context, new String[] { modelAppMap.get(modelPhyId), applValue });
                        mqlLogRequiredInformationWriter("ModelPhyID and appValue after union :" + modelPhyId + " & " + applValue + "\n");
                    }
                    modelAppMap.put(modelPhyId, applValue);

                }
                mqlLogRequiredInformationWriter("\n****************** model Applicable Map ************\n" + modelAppMap);

                changeId = (String) objectIdList.get(i);
                mqlLogRequiredInformationWriter("\n****************** changeid  ************\n" + changeId);
                StringList selectStmts = new StringList();
                /*********************** Effectivity on PUE ECO***** retrieved through Named Effectivity ***********************/
                selectStmts.add(DomainConstants.SELECT_NAME);
                selectStmts.addElement("from[Named Effectivity].to.attribute.value");
                selectStmts.addElement("from[Named Effectivity].to.id");
                selectStmts.addElement("from[Named Effectivity].to.name");
                selectStmts.addElement("from[Named Effectivity].to.type");
                selectStmts.addElement("from[Named Effectivity].to.from[Effectivity Usage].to.type");
                selectStmts.addElement("from[Named Effectivity].to.from[Effectivity Usage].to.id");
                selectStmts.addElement("from[Named Effectivity].to.from[Effectivity Usage].to.name");
                selectStmts.addElement("from[Named Effectivity].to.from[Effectivity Usage].attribute.value");

                DomainObject doChangeID = new DomainObject(changeId);
                Map mInfo = doChangeID.getInfo(context, selectStmts);
                mqlLogRequiredInformationWriter("\n******************Selectable on PUE ECO ************\n" + mInfo);
                ...
                MapList affectedPartIds = doChangeID.getRelatedObjects(context, EngineeringConstants.RELATIONSHIP_AFFECTED_ITEM, DomainConstants.TYPE_PART, slBusSelect, slRelSelect, false, true,
                        (short) 1, "", "");
                mqlLogRequiredInformationWriter("\n****************** Affected Items of PUE ECO  ************\n" + affectedPartIds);
                ...
                // changeExprListAll.add(changeExprMap);
                mqlLogRequiredInformationWriter("\n*******************Processing Map with below ChangeId and Expression:" + changeExprList + "************ \n");

                try {
                    effectivity.setEffectivityOnChange(context, changeExprList);
                    changeExprList.clear();
                    changeExprMap = new HashMap();
                } catch (Exception ex) {
                    // ContextUtil.abortTransaction(context);
                    mqlLogRequiredInformationWriter("Error Ocurred ***FAILED***");
                    ex.printStackTrace();
                }
            }

            mqlLogRequiredInformationWriter("\n\n ################ Named Effectivity of the file is migrated, check last file for affected Id's migration logs################## \n\n");
            // performMODStackCalculations(context, changeAppMap);
     ...
     * @param modelAppMap
     *            contains Model physical id and effectivity as key value pairs
    ...
    private String getModelBasedExpression(HashMap modelAppMap) throws Exception {
        ...
        mqlLogRequiredInformationWriter("the expression to get Model Based Expre : " + modelAppMap + "\n");
        ...
        while (itr.hasNext()) {
            ...
            // change by e2w to take care of 0 in range values which is not supported will be replaced to 1
            int zeroIndex = applValue.indexOf("0");
            if (zeroIndex == 0) {
                applValue = applValue.replaceFirst("0", "1");
            }

            exprBuffer = exprBuffer.length() > 0 ? exprBuffer.append(" ").append("OR").append(" ") : exprBuffer;
            exprBuffer.append(_KEY_PREFIX).append(modelPhyId).append("[").append(applValue).append(_KEY_SUFFIX);
        }
        mqlLogWriter(exprBuffer.toString() + "\n");
        mqlLogRequiredInformationWriter("the model expression passsed out effectivity is : " + exprBuffer.toString() + "\n");
        return exprBuffer.toString();

    }

    /**
     * This method gives the merged/union of two applicabilites eg: 1-50 ,51-100 ==>1-100
     * @param context
     *            the Ematrix code context object
     * @param args
     *            Strings of applicabilities
    ...
    public String getUnionOfApplicabilities(Context context, String[] args) throws Exception {
        ...
        mqlLogRequiredInformationWriter("Sorted List Received \n");
    ...
    /**
     * This method gives the final union applicability from _delimiterList and contiguous list eg:[10,30] [11-20,50-100]==>10-20,30,50-100
     * @param _delimiterList
     *            StringList of delimiterValues
     * @param resultList
     *            StringList of contiguous range values
    ...
    /**
     * This method gives the Union of range values list from contiguous list of range values eg: [1-5,1-8,1-20]===>[1-20]
     * @param _contiguousList
     *            StringList of range values
    ...
    /**
     * Removes delimiter values if those are in contiguous range values eg: [5,30] [1-20,50-100]===>[30] removes 5 from list as 5 is in range of 1-20
     * @param _delimiterList
     *            StringList of delimiter values
     * @param _contiguousList
     *            StringList of range Values
    ...
    /**
     * This method sorts the given range values eg:[1-10,6-8]===>[6-8,1-10]
     * @param _contiguousList
     *            StringList of unsorted rangeValues
    ...
    private StringList getSortedList(StringList _contiguousList) {
        ...
        for (int index = 0; index < _contiguousList.size(); index++) {
            ...
            try {
                mqlLogRequiredInformationWriter("String passed to get lower limit is" + val + nextVal + "\n");
            } catch (Exception e) {
     ...
     * @param totalAppString
     *            contains appended values of total applicability string
    ...
    private long getHighestBuildValue(Context context, String totalAppString) throws Exception {
        ...
        long maxNum = 0, numVal = 0;
        while (itr.hasNext()) {
            String numStr = (String) itr.next();
    ...
    /**
     * check the given digit is in range or not in 4 possible conditions.eg:(1,5,10) isInRange(1,5-10)||isInRange(1+1,5-10)||isInRange(5,5-10)||isInRange(5+1,5-10)
     * @param lowerLimit
     *            the lower limit long value
     * @param higherLimit
     *            the higher limit long value
     * @param currVal
     *            range value to be checked
    ...
    private static Boolean isNumberIsInRange(long lowerLimit, long higherLimit, String currVal) throws Exception {
        ...
        return (checkNumberIsInRange(lowerLimit, currVal) || checkNumberIsInRange(lowerLimit + 1, currVal) || checkNumberIsInRange(higherLimit, currVal)
                || checkNumberIsInRange(higherLimit + 1, currVal));
    }

    /**
     * Method used to return lower limit of particular string
     * @param currStr
     *            contains any value like 1 or 1,1-5
     ...
     * @param currStr
     *            contains any value like 1,1-5 or 1-5
