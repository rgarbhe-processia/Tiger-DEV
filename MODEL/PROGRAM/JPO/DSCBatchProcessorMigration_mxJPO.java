 ...
 * 
 * This program contains proprietary and trade secret information of Dassault Systemes and its subsidiaries. Copyright notice is precautionary only and does not evidence any actual or intended
...
public class DSCBatchProcessorMigration_mxJPO {
    ...
    /**
     * Get the list of messages connected to the queue with the relationship name passed as an argument to the method.
     */
    private Vector getMessages(Context context, DSCQueue queue, String relationName) throws Exception {
        ...
        ExpansionWithSelect expansionWithSelect = queue.expandSelect(context, relationName, DSCBackgroundProcessorUtil.getActualNameForAEFData(context, "type_DSCMessage"), new StringList(), relSelect,
                true, true, (short) 1, "", "", true);
    ...
    /**
     * Promote either 1) Failed messages to the completed state. OR 2) Created messages to the submitted state depending on the State passed as an argument.
     */
    ...
    /**
     * Validate the input arguments. Two arguments have to be passed First argument - the queue names as a comma-seperated list. e.g. "Queue1,Queue2". Second argument - the path of the directory where
     * the logs are to be genereted is needed.
     */
