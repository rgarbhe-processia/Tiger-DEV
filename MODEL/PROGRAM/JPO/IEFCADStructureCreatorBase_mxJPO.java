...
public class IEFCADStructureCreatorBase_mxJPO {
    ...
    /**
     * Entry Point This method is to be called for creating the structure for Create CAD Structure functionality.It takes as an input the object ID of the template object which is selected in the
     * UI.Other inputs are the type, name, revision and policy for the design object to be created and a map of the attribute name and values which are to be set on the design object.A flag indicating
     * whether AutoName is selected for naming of object, is also passed as one of the input parameters. Based on the template object ID and other parameters, this object creates the design object. If
     * the template object is of assembly type, structure below this template object is traversed and design objects are created for each of the template objects in the structure. These design objects
     * are then connected to build the structure.
     * @param context
     * @param args
     *            - a packed HashMap containing the following keys and values "GCO" = Global Config Object "LCO" = Local Config Object "language" = locale language name "partID" = Part object's ID
     *            "integrationName" = Integration name "assemblyTemplateID" = The assembly template's object id "componentTemplateID" = The component template's object id
     * @return
     * @throws Exception
     */
