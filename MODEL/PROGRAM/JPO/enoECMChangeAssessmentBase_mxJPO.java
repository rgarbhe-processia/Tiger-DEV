...
public class enoECMChangeAssessmentBase_mxJPO extends DomainObject {
        ...
        try {
            ...
            for (int i = 0; i < slRelatedList.size(); i++) {
                ...
                if (slExclusionList != null && !slExclusionList.isEmpty() && !slExclusionList.contains(strRelatedItem)) {
                    String strParentType = MqlUtil.mqlCommand(context, "print type $1 select $2 dump", strRelatedType, "derived");
    ...
    public MapList getCandidateItems(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            mlOutput = getRelatedObjects(context, RELATIONSHIP_CANDIDATE_AFFECTED_ITEM, "*", objectSelects, relSelects, false, true, recureLevel, "", DomainConstants.EMPTY_STRING, 0);
        } catch (Exception Ex) {
            Ex.printStackTrace();
            throw Ex;
        }
