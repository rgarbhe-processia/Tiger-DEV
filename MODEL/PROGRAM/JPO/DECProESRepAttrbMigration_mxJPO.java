 ...
 *
 ** Copyright Dassault Systemes, 1992-2009. All Rights Reserved. This program contains proprietary and trade secret information of Dassault Systemes and its subsidiaries, Copyright notice is
        ...
        while ((line = iefIDsReader.readLine()) != null) {
            try {
                if (transactionMode) {
                    ...
                    /*
                     * if transaction is false then read lines between ### and then add it to a list. // And limit the transaction to that list. Lines in the list are interdependendent hence some line
                     * is failed then // we have roll back transation for that list only. If it fails then list would be written into failedIDs file // so that user can execute that file later.
                     */
    ...
    private void writeCommandToFile(Context context, String[] args) throws Exception {
        ...
        try {
            while (queryIterator.hasNext()) {
                ...
                if (null != attrProESimpRepValue && !"".equalsIgnoreCase(attrProESimpRepValue) && (null == attrProESRepExcludedRelListValue || "".equalsIgnoreCase(attrProESRepExcludedRelListValue))) {
