 ...
 * publication of such program. static const char RCSID[] = $Id: ${CLASSNAME}.java.rca 1.19 Wed Oct 22 16:54:23 2008 przemek Experimental przemek $
 */
...
/**
 * The <code>emxMultipleClassificationSearchBase</code> class contains utility methods for getting data using configurable tables in Library Central.
 * @exclude
 */
public class emxMultipleClassificationSearchBase_mxJPO implements ClassificationConstants {
     ...
     */
    public emxMultipleClassificationSearchBase_mxJPO(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following list of arguments: 0 - objectId 1 - matchlistLimit 2 - txtTypeActual 3 - txtName 4 - txtRev 5 - txtOwner 6 - levelSelction 7 - state 8 - languageStr 9 -
     *            vaultSelction
    ...
    public Object searchClassifiedItems(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            for (Iterator matchingInterfaceChunksItr = matchingInterfaceChunks.iterator(); matchingInterfaceChunksItr.hasNext();) {
                ...
                MapList resultList = DomainObject.findObjects(context, selType, txtName, txtRev, txtOwner, txtVault, strWhereExpr, null, true, selectStmts, Short.parseShort(queryLimit), txtFormat,
                        txtSearch);
    ...
    private static String getInterfaceList(Context context, String oid) throws Exception {
        ...
        String expandData = MqlUtil.mqlCommand(context, expandCmd, oid, LibraryCentralConstants.RELATIONSHIP_SUBCLASS, "attribute[" + LibraryCentralConstants.ATTRIBUTE_MXSYSINTERFACE + "]", ",", "|");
     ...
     * @param paramMap
     *            HashMap object containing all the parameters
        ...
        try {
            ...
            for (int i = 0; i < attrList.size(); i++) {
                ...
                try {
                    ...
                    throw (new FrameworkException("Exception caught while getting the attribute type  of '" + sAttrib + "':" + exp.getMessage()));
                }
                ...
                if (sValue != null && (!sValue.equals("")) && (!sValue.equals(QUERY_WILDCARD))) {
                    ...
                    if (patternAttribute.match(sMatrixType) || patternAttribute.match(sMatrixName) || patternAttribute.match(sMatrixRevision) || patternAttribute.match(sMatrixOwner)
                            || patternAttribute.match(sMatrixVault) || patternAttribute.match(sMatrixDescription) || patternAttribute.match(sMatrixCurrent) || patternAttribute.match(sMatrixModified)
                            || patternAttribute.match(sMatrixOriginated) || patternAttribute.match(sMatrixGrantor) || patternAttribute.match(sMatrixGrantee) || patternAttribute.match(sMatrixPolicy)
                    ...
                    if (patternOperator.match(sMatrixIncludes)) {
                        ...
                        if (patternAttribute.match(sMatrixDescription) || patternAttribute.match(sMatrixCurrent) || patternAttribute.match(sMatrixModified) || patternAttribute.match(sMatrixOriginated)
                                || patternAttribute.match(sMatrixGrantor) || patternAttribute.match(sMatrixGrantee) || patternAttribute.match(sMatrixPolicy)) {
                            sbWhereExp.append(sAttrib);
                        } else {
                            sbWhereExp.append(sAttribute);
                            sbWhereExp.append(sOpenBracket);
                        ...
                        if (patternAttribute.match(sMatrixType) || patternAttribute.match(sMatrixName) || patternAttribute.match(sMatrixRevision) || patternAttribute.match(sMatrixOwner)
                                || patternAttribute.match(sMatrixVault) || patternAttribute.match(sMatrixDescription) || patternAttribute.match(sMatrixCurrent)
                                || patternAttribute.match(sMatrixModified) || patternAttribute.match(sMatrixOriginated) || patternAttribute.match(sMatrixGrantor)
                                || patternAttribute.match(sMatrixGrantee) || patternAttribute.match(sMatrixPolicy) || sAttrib.equals("State")) {
                            sbWhereExp.append(sAttrib);
                        } else {
                            sbWhereExp.append(sAttribute);
                            sbWhereExp.append(sOpenBracket);
