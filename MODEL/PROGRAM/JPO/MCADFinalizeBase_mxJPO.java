...
public class MCADFinalizeBase_mxJPO extends IEFCommonUIActions_mxJPO {
    ...
    protected void finalizeIndivisual(Context _context, BusinessObject bus, Vector handledBusIds, DSCServerErrorMessageTable errorMessageTable) throws Exception {
        try {
             ...
             * String cadType = _util.getCADTypeForBO(_context, bus);
             * 
             * if(_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) { //NDM:- NE4 boolean isMajorIns = _util.isMajorObject(_context, bus.getObjectId());
             * BusinessObject instMajorObject = null;
             * 
             * if(isMajorIns) instMajorObject = bus; else instMajorObject = _util.getMajorObject(_context, bus);
             * 
             * //BusinessObject instMajorObject = _util.getMajorObject(_context, bus);
             * 
             * //End NDM:- NE4 instMajorObject.open(_context); boolean isPushed = false;
             * 
             * try { com.matrixone.apps.domain.util.ContextUtil.pushContext(_context); isPushed = true;
             * 
             * //Start NDM:- NE4
             * 
             * //_generalUtil.shuffleRelatinships(_context, bus, instMajorObject, true, false, MCADServerSettings.FAMILY_LIKE); //_generalUtil.shuffleRelatinships(_context, bus, instMajorObject,
             * false, false, MCADServerSettings.FAMILY_LIKE);
             * 
             * //End NDM:- NE4
             * 
             * }catch(Exception me) { throw me; } finally { if(isPushed) { try { com.matrixone.apps.domain.util.ContextUtil.popContext(_context); } catch(Exception ex){ String errorMessage =
             * ex.getMessage(); errorMessageTable.addErrorMessage(_context, instMajorObject.getObjectId(),errorMessage ); errorMessageList.add(errorMessage); } } } instMajorObject.close(_context); }
             * else if(_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_FAMILY_LIKE)) { //Start [NDM] :- NE4 boolean isMajorFam = _util.isMajorObject(_context, bus.getObjectId());
             * BusinessObject familyMajorObject = null;
             * 
             * if(isMajorFam) familyMajorObject = bus; else familyMajorObject = _util.getMajorObject(_context, bus); //END [NDM] :- NE4
             * 
             * 
             * // BusinessObject familyMajorObject = _util.getMajorObject(_context, bus); familyMajorObject.open(_context);
             * 
             * boolean isPushed = false; try { com.matrixone.apps.domain.util.ContextUtil.pushContext(_context); isPushed = true;
             * 
             * //Start [NDM]:- NE4 //_generalUtil.shuffleRelatinships(_context, bus, familyMajorObject, true, true, MCADServerSettings.FAMILY_LIKE); //End [NDM]:- NE4
             * 
             * } catch (Exception me) { throw me; } finally { if(isPushed) { try { com.matrixone.apps.domain.util.ContextUtil.popContext(_context); } catch(Exception ex){ String errorMessage =
             * ex.getMessage(); errorMessageTable.addErrorMessage(_context, familyMajorObject.getObjectId(), errorMessage); errorMessageList.add(errorMessage); } } } familyMajorObject.close(_context);
             * }
             */
        } catch (Exception e) {
    ...
    protected void finalizeBO(Context _context, BusinessObject bus, Vector handledBusIds, DSCServerErrorMessageTable errorMessageTable) throws MCADException {
        ...
        try {
            ...
            /*
             * if(_globalConfig.getPromoteUnRequiredRef()) areAllChildrenFinalized(_context, bus, errorMessageTable);
             */
            // [NE4] NDM end

            /*
             * if(_globalConfig.isCreateVersionObjectsEnabled()) { // Attribute transfer _util.copyAttributesForFinalization(_context, _globalConfig, bus,
             * majorObject,COPY_BLANK_ATTR_DURING_FINALIZATION); }
             */
            // bus.open(_context);
            majorObject.open(_context);

            /*
             * AttributeList attrList = new AttributeList(); String fileMsgDigestAttrName = MCADMxUtil.getActualNameForAEFData(_context, "attribute_IEF-FileMessageDigest"); String fileMsgDigestAttrVal
             * = _util.getAttributeForBO(_context, bus,fileMsgDigestAttrName);
             * 
             * Attribute fileMsgDigest = new Attribute(new AttributeType(fileMsgDigestAttrName),fileMsgDigestAttrVal); attrList.addElement(fileMsgDigest);
             * 
             * if(_util.isCDMInstalled(_context)) { String moveFilesToVersionAttrName = MCADMxUtil.getActualNameForAEFData(_context, "attribute_MoveFilesToVersion"); Attribute moveFilesToVersion = new
             * Attribute(new AttributeType(moveFilesToVersionAttrName),"False"); attrList.addElement(moveFilesToVersion); }
             * 
             * if(attrList.size()>0) { majorObject.setAttributes(_context,attrList); } // transfer description Also. String desc = bus.getDescription(_context); if(!desc.equals("") ||
             * COPY_BLANK_ATTR_DURING_FINALIZATION) majorObject.setDescription(_context,desc); majorObject.update(_context);
             */
            // revision replace
    ...
    /*
     * protected void createVirtualStructure(Context _context, IEFXmlNode finalizePacket, DSCServerErrorMessageTable errorMessageTable) throws Exception { IEFXmlNode structureNode =
     * MCADXMLUtils.getChildNodeWithName(finalizePacket, "structure"); Hashtable objIDNodeListTable = new Hashtable(); Hashtable objIDChangedIDTable = new Hashtable();
     * 
     * Enumeration rootCadObjectElements = structureNode.elements(); Hashtable virtualStructureCreatedforNodes = new Hashtable();
     * 
     * Hashtable processedIdcadobjNodeMap = new Hashtable(); Hashtable objectIdRevisedBusObjMap = new Hashtable();
     * 
     * 
     * while(rootCadObjectElements.hasMoreElements()) { // Get top node IEFXmlNode rootCadObjectNode = (IEFXmlNode)rootCadObjectElements.nextElement();
     * errorMessageTable.setRootId((String)rootCadObjectNode.getAttribute("id"));
     * 
     * createVirtaulStructureforNode(_context, rootCadObjectNode, objIDNodeListTable, objIDChangedIDTable, virtualStructureCreatedforNodes, errorMessageTable,processedIdcadobjNodeMap,
     * objectIdRevisedBusObjMap); } }
     * 
     * protected void createVirtaulStructureforNode(Context _context, IEFXmlNode cadObjectNode, Hashtable objIDNodeListTable, Hashtable objIDChangedIDTable, Hashtable virtualStructureCreatedforNodes,
     * DSCServerErrorMessageTable errorMessageTable,Hashtable processedIdcadobjNodeMap, Hashtable objectIdRevisedBusObjMap) throws Exception { Enumeration childCadObjectElements =
     * cadObjectNode.elements();
     * 
     * while(childCadObjectElements.hasMoreElements()) { String parentid = cadObjectNode.getAttribute("id");
     * 
     ...
     * 
     * // call the function recursively createVirtaulStructureforNode(_context, childCadObjectNode, objIDNodeListTable, objIDChangedIDTable, virtualStructureCreatedforNodes,
     * errorMessageTable,processedIdcadobjNodeMap, objectIdRevisedBusObjMap);
     * 
     * processedIdcadobjNodeMap.remove(parentid); }
     * 
     * boolean isFloatedVerticalViewNode = isFloatedVerticalViewNode(cadObjectNode);
     * 
     * if(!virtualStructureCreatedforNodes.containsKey((String)cadObjectNode.getAttribute("id"))) { // check if the revsion is required boolean isStructureDisturbed = isRevisionRequired(_context,
     * cadObjectNode);
     * 
     * String currObjID = (String)cadObjectNode.getAttribute("id"); String isPreFinalizationState = (String)cadObjectNode.getAttribute("isprefinalizationstate"); String changedID = currObjID;
     * 
     * Vector sharedNodeList = (Vector)objIDNodeListTable.get(currObjID);
     * 
     * if(sharedNodeList == null) { sharedNodeList = new Vector(); objIDNodeListTable.put(currObjID, sharedNodeList); }
     * 
     * boolean isOtherNodeInPath = processedIdcadobjNodeMap.containsKey(currObjID);
     * 
     * if(!isStructureDisturbed && isOtherNodeInPath) { IEFXmlNode otherCADObjectNode = (IEFXmlNode) processedIdcadobjNodeMap.get(currObjID); isStructureDisturbed = isRevisionRequired(_context,
     * otherCADObjectNode);
     * 
     * if(!isStructureDisturbed && cadObjectNode.getParentNode() != null && isRevisionRequired(_context, cadObjectNode.getParentNode())) isStructureDisturbed = true; }
     * 
     * if(isStructureDisturbed && isPreFinalizationState.equalsIgnoreCase(MCADAppletServletProtocol.TRUE)) { if(_globalConfig.isCreateVersionObjectsEnabled()) createRevisonAndRelationships(_context,
     * cadObjectNode, virtualStructureCreatedforNodes, errorMessageTable, processedIdcadobjNodeMap, objectIdRevisedBusObjMap, objIDNodeListTable); else connectStructureWithoutRevision(_context,
     * cadObjectNode);
     * 
     * changedID = (String)cadObjectNode.getAttribute("id"); objIDChangedIDTable.put(currObjID, changedID);
     * 
     * for(int i=0; i<sharedNodeList.size(); i++) { IEFXmlNode cadNode = (IEFXmlNode)sharedNodeList.elementAt(i); cadNode.setAttributeValue("id", changedID); }
     * 
     * }
     * 
     * if(isFloatedVerticalViewNode && isPreFinalizationState.equalsIgnoreCase(MCADAppletServletProtocol.TRUE)) { //isFloatedVerticalViewNode - with create version of drawing cannot float
     * if(!isStructureDisturbed && _globalConfig.isCreateVersionObjectsEnabled()) { BusinessObject revisedMinorObject = reviseWithLatestMinor(_context, currObjID);
     * 
     ...
     * }
     * 
     * if(!isStructureDisturbed && !isFloatedVerticalViewNode && objIDChangedIDTable.containsKey(currObjID)) { changedID = (String)objIDChangedIDTable.get(currObjID);
     * cadObjectNode.setAttributeValue("id", changedID); }
     * 
     * sharedNodeList.addElement(cadObjectNode); } else { cadObjectNode.setAttributeValue("id" , (String)virtualStructureCreatedforNodes.get((String)cadObjectNode.getAttribute("id"))); } }
     * 
     * protected boolean isFloatedVerticalViewNode(IEFXmlNode cadObjectNode) { boolean isFloatedVerticalViewNode = false;
     * 
     * String isSelected = (String)cadObjectNode.getAttribute("selected"); String isVerticalViewNode = (String)cadObjectNode.getAttribute("isverticalviewnode"); String isFloatedNode =
     * (String)cadObjectNode.getAttribute("isfloated");
     * 
     * if(isVerticalViewNode.equalsIgnoreCase("true") && isFloatedNode.equalsIgnoreCase("true") && (MCADAppletServletProtocol.TRUE).equalsIgnoreCase(isSelected)) isFloatedVerticalViewNode = true;
     * 
     * return isFloatedVerticalViewNode; }
     * 
     * protected boolean isRevisionRequired(Context _context, IEFXmlNode cadObjectNode) throws Exception { String objectID = (String)cadObjectNode.getAttribute("id"); String isSelected =
     * (String)cadObjectNode.getAttribute("selected"); String cadType = (String)cadObjectNode.getAttribute("cadtype");
     * 
     * if(!(MCADAppletServletProtocol.TRUE).equalsIgnoreCase(isSelected)) return false;
     * 
     * Vector childObjectIDs = new Vector(); String childBusInfoAndRelIds = "";
     * 
     ...
     * if(_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_FAMILY_LIKE)) childBusInfoAndRelIds = childBusInfoAndRelIds + "\n" +
     * _generalUtil.getChildRelsAndIdsForCheckout(_context,objectID, true, MCADAppletServletProtocol.FAMILY_LIKE, "", "", "", null); else childBusInfoAndRelIds = childBusInfoAndRelIds + "\n" +
     * _generalUtil.getChildRelsAndIdsForCheckout(_context,objectID, false, MCADAppletServletProtocol.FAMILY_LIKE, "", "", "", null);
     * 
     * childBusInfoAndRelIds = childBusInfoAndRelIds + "\n" + _generalUtil.getChildRelsAndIdsForCheckout(_context,objectID, true, MCADAppletServletProtocol.ASSEMBLY_LIKE, "", "", "", null); } else
     ...
     * 
     * while (childBusInfoTokens.hasMoreTokens()) { String childBusInfo = childBusInfoTokens.nextToken();
     * 
     * Enumeration childBusInfoElements = MCADUtil.getTokensFromString(childBusInfo, "|"); String childLevel = (String) childBusInfoElements.nextElement(); String relationshipName = (String)
     * childBusInfoElements.nextElement(); String relationshipDirection = (String) childBusInfoElements.nextElement(); String childObjectID = (String) childBusInfoElements.nextElement(); String
     * relationshipID = (String) childBusInfoElements.nextElement();
     * 
     * childObjectIDs.addElement(childObjectID); }
     * 
     * Enumeration cadObjectElements = cadObjectNode.elements(); while(cadObjectElements.hasMoreElements()) { IEFXmlNode childObjectNode = (IEFXmlNode)cadObjectElements.nextElement();
     * 
     * String childObjectID = (String)childObjectNode.getAttribute("id"); String isChildSelected = (String)childObjectNode.getAttribute("selected"); String isVerticalViewNode =
     * (String)childObjectNode.getAttribute("isverticalviewnode");
     * 
     * if(isVerticalViewNode.equalsIgnoreCase("true")) { continue; }
     * 
     * BusinessObject childBusObject = new BusinessObject(childObjectID); childBusObject.open(_context);
     * 
     * The "isChildSelectable" flag is for handling the case when promoting a structure which after lateral navigation contains a child which is already finalized, and which does not participate in
     * the as-stored structure. In this case, even if the child is not selected, the promotion of the structure to finalized state should be allowed
     * 
     * String childBusType = childBusObject.getTypeName(); BusinessObject childMajorObject = null; boolean isChildSelectable = false;
     * 
     * if(_globalConfig.isMinorType(childBusType)) childMajorObject = _util.getMajorObject(_context, childBusObject); else childMajorObject = childBusObject;
     * 
     * if((MCADAppletServletProtocol.TRUE).equalsIgnoreCase(isChildSelected) || _generalUtil.isBusObjectFinalized(_context, childMajorObject)) isChildSelectable = true;
     * 
     * childBusObject.close(_context);
     * 
     * if(!childObjectIDs.contains(childObjectID) && isChildSelectable) return true; }
     * 
     * return false; }
     * 
     * protected void createRevisonAndRelationships(Context _context, IEFXmlNode cadObjectNode, Hashtable virtualStructureCreatedforNodes, DSCServerErrorMessageTable errorMessageTable, Hashtable
     * processedIdcadobjNodeMap, Hashtable objectIdRevisedBusObjMap, Hashtable objIDNodeListTable) throws Exception { String currentObjectId = null; //Place holder for storing object id for giving
     * object details in exception block. boolean bGrantCreatorFromconnectAccess = false; StringList slMajorIdWithAccess = new StringList(); String key = "DECVirtualStructureFromConnect"; try { String
     * minorObjectID = cadObjectNode.getAttribute("id");
     * 
     ...
     * 
     * if(objectIdRevisedBusObjMap.containsKey(currentObjectId)) revisedMinorObject = (BusinessObject)objectIdRevisedBusObjMap.get(currentObjectId); else revisedMinorObject =
     * reviseVersionedTypeObject(_context, minorObjectID, revReplaceTable,virtualStructureCreatedforNodes,objIDNodeListTable);
     * 
     ...
     * 
     * 
     * if(familyId != null && (String)virtualStructureCreatedforNodes.get(familyId) != null) { revisedFamilyId = (String)virtualStructureCreatedforNodes.get(familyId); revisedNodeList =
     * (Vector)objIDNodeListTable.get(familyId);
     * 
     * }
     * 
     * if(revisedNodeList != null) {
     * 
     * for(int i=0; i<revisedNodeList.size(); i++) { IEFXmlNode cadNode = (IEFXmlNode)revisedNodeList.elementAt(i);
     * 
     ...
     * 
     * StringList roleList = new StringList(); roleList.add(vplmCreatorRoleName);
     * 
     * 
     * 
     * //if other node with same id present in path just create the new revision, cached it and connect from that node. IR-152872V6R2013 if(!isOtherNodeInPath) {
     ...
     * 
     * Enumeration childCadObjectElements = childIdCadNodeTable.keys(); while(childCadObjectElements.hasMoreElements()) { String childObjectID = (String)childCadObjectElements.nextElement();
     * 
     * 
     * 
     * if(MCADMxUtil.isSolutionBasedEnvironment(_context)) { String currentRole = _context.getRole(); if(currentRole.contains(vplmCreatorRoleName)){ BusinessObject childTempObject = new
     * BusinessObject(childObjectID); if(_generalUtil.isBusObjectFinalized(_context, childTempObject)){ bGrantCreatorFromconnectAccess = true;
     * _generalUtil.grantAccessForRole(_context,childObjectID,roleList,"fromconnect",key); slMajorIdWithAccess.addElement(childObjectID); } } }
     * 
     * currentObjectId = childObjectID; Vector cadObjectNodeList = (Vector)childIdCadNodeTable.get(childObjectID);
     * 
     * for(int i=0; i<cadObjectNodeList.size(); i++) { IEFXmlNode childObjectNode = (IEFXmlNode)cadObjectNodeList.elementAt(i); String isChildVerticalViewNode =
     * childObjectNode.getAttribute("isverticalviewnode"); String childObjectRelationshipID = (String)childObjectNode.getAttribute("rid");
     * 
     * if(childObjectRelationshipID != null && !childObjectRelationshipID.equals("")) { Relationship childParentRelationship = new Relationship(childObjectRelationshipID);
     * childParentRelationship.open(_context);
     * 
     * String childParentRelationshipType = childParentRelationship.getTypeName(); Hashtable attrNameVals = _util.getRelationshipAttrNameValMap(_context, childParentRelationship);
     * 
     * childParentRelationship.close(_context); String cadRelTypeName = _globalConfig.getCADRelFromMxRel(childParentRelationshipType); String relDirection =
     * _globalConfig.getRelatinshipEndFromMapping(cadRelTypeName, childParentRelationshipType);
     * 
     * BusinessObject childObject = new BusinessObject(childObjectID);
     * 
     * childObject.open(_context);
     * 
     * StringList selectList = new StringList(); selectList.add(MCADAppletServletProtocol.STR_LOGICALID);
     * 
     * Hashtable basicToSet = _generalUtil.getSelectDataForRelation(_context, childObjectRelationshipID, selectList);
     * 
     * if(isChildVerticalViewNode == null || !isChildVerticalViewNode.equalsIgnoreCase("true")) { if(relDirection.equalsIgnoreCase("from")) _util.connectBusObjects(_context, childObjectID,
     * revisedMinorObjectID, childParentRelationshipType, true, attrNameVals,basicToSet); else _util.connectBusObjects(_context, childObjectID, revisedMinorObjectID, childParentRelationshipType,
     * false, attrNameVals,basicToSet); } else { boolean isNodeOnFromSide = false; boolean isNodeParent = false; BusinessObject fromSideObject = childParentRelationship.getFrom();
     * fromSideObject.open(_context);
     * 
     * if(fromSideObject.getObjectId().equals(childObjectID)) isNodeOnFromSide = true;
     * 
     * fromSideObject.close(_context);
     * 
     * if(relDirection.equals("from") && !isNodeOnFromSide) isNodeParent = true; else if(relDirection.equals("to") && isNodeOnFromSide) isNodeParent = true;
     * 
     * if(isNodeParent) { removedRelIds.add(childObjectRelationshipID); childParentRelationship.remove(_context); }
     * 
     ...
     * childObject.close(_context); } } }
     * 
     * } else { objectIdRevisedBusObjMap.put(currentObjectId, revisedMinorObject); } revisedMinorObject.close(_context);
     * 
     * cadObjectNode.setAttributeValue("id" , revisedMinorObjectID); } catch(Exception e) { String errorMessage = e.getMessage(); errorMessageTable.addErrorMessage(_context, currentObjectId,
     * errorMessage); errorMessageList.add(errorMessage); } finally{ if(bGrantCreatorFromconnectAccess) { for(int k=0;k<slMajorIdWithAccess.size();k++){ String tempOid = (String)
     * slMajorIdWithAccess.get(k); _generalUtil.revokeAccessForRole(_context, tempOid,key); } } } }
     * 
     * protected void connectStructureWithoutRevision(Context _context, IEFXmlNode cadObjectNode) throws Exception { try { String minorObjectID = cadObjectNode.getAttribute("id");
     * 
     * Hashtable revReplaceTable = new Hashtable(); Hashtable childIdCadNodeTable = getChildIdCadNodeTable(_context, cadObjectNode, revReplaceTable);
     * 
     * Hashtable relsAndEnds = new Hashtable(); Hashtable assemblyRelsAndEnds = _globalConfig.getRelationshipsOfClass(MCADAppletServletProtocol.ASSEMBLY_LIKE); Hashtable extRefRelsAndEnds =
     * _globalConfig.getRelationshipsOfClass(MCADServerSettings.EXTERNAL_REFERENCE_LIKE);
     * 
     * relsAndEnds.putAll(assemblyRelsAndEnds);
     * 
     * Enumeration assemblyLikeRels = relsAndEnds.keys();
     * 
     * while(assemblyLikeRels.hasMoreElements()) { String assemblyLikeRel = (String)assemblyLikeRels.nextElement(); if(extRefRelsAndEnds != null && extRefRelsAndEnds.containsKey(assemblyLikeRel))
     ...
     * 
     * Enumeration childCadObjectElements = childIdCadNodeTable.keys(); while(childCadObjectElements.hasMoreElements()) { String childObjectID = (String)childCadObjectElements.nextElement(); Vector
     * cadObjectNodeList = (Vector)childIdCadNodeTable.get(childObjectID);
     * 
     * IEFXmlNode childObjectNode = (IEFXmlNode)cadObjectNodeList.elementAt(0); String isChildSelected = (String)childObjectNode.getAttribute("selected"); String childObjectRelationshipID =
     * (String)childObjectNode.getAttribute("rid"); String isVerticalViewNode = (String)childObjectNode.getAttribute("isverticalviewnode"); String childMajorBusObjectId =
     * (String)childObjectNode.getAttribute("majorobjectid");
     * 
     * BusinessObject childMajorObject = new BusinessObject(childMajorBusObjectId); childMajorObject.open(_context);
     * 
     * boolean isChildSelectable = false; if((MCADAppletServletProtocol.TRUE).equalsIgnoreCase(isChildSelected) || _generalUtil.isBusObjectFinalized(_context, childMajorObject)) isChildSelectable =
     * true; //Operation should continue even if child is not selected but is finalized.
     * 
     * childMajorObject.close(_context);
     * 
     * if(isVerticalViewNode.equalsIgnoreCase("true") || !isChildSelectable) continue;
     * 
     * if(childObjectRelationshipID != null && !childObjectRelationshipID.equals("")) { Relationship childParentRelationship = new Relationship(childObjectRelationshipID);
     * childParentRelationship.open(_context);
     * 
     ...
     * _globalConfig.getRelatinshipEndFromMapping(cadRelTypeName, childParentRelationshipType);
     * 
     * if(!_util.doesRelationExist(_context, childObjectID, minorObjectID, childParentRelationshipType)) { StringList relSelectList = new StringList();
     ...
     * 
     * for(int i=0;i<cadObjectNodeList.size();++i) { if(relDirection.equalsIgnoreCase("from")) _util.connectBusObjects(_context, childObjectID, minorObjectID, childParentRelationshipType, true,
     * attrNameVals,basicsToset); else _util.connectBusObjects(_context, childObjectID, minorObjectID, childParentRelationshipType, false, attrNameVals,basicsToset); } } } } } catch(Exception
     * exception) { exception.printStackTrace(); MCADServerException.createException(exception.getMessage(), exception); } }
     * 
     * protected void disconnectExistingVersionRelationship(Context _context, String parentMinorObjId, String nodeChildObjId, Hashtable childIdFirstLevelRelIdsTable) throws Exception { Enumeration
     * connectedChildObjIds = childIdFirstLevelRelIdsTable.keys(); BusinessObject nodeChildObject = new BusinessObject(nodeChildObjId);
     * 
     * BusinessObject nodeChildMajorObject = _util.getMajorObject(_context, nodeChildObject); if(nodeChildMajorObject == null) nodeChildMajorObject = nodeChildObject;
     * 
     * nodeChildMajorObject.open(_context);
     * 
     * String nodeChildMajorObjType = nodeChildMajorObject.getTypeName(); String nodeChildMajorObjName = nodeChildMajorObject.getName();
     * 
     * while(connectedChildObjIds.hasMoreElements()) { String connectedChildId = (String)connectedChildObjIds.nextElement(); Vector childRelationships =
     * (Vector)childIdFirstLevelRelIdsTable.get(connectedChildId);
     * 
     * for(int i=0; i<childRelationships.size();++i) { String childRelationshipId = (String)childRelationships.elementAt(i);
     * 
     * Relationship childRelationship = new Relationship(childRelationshipId); childRelationship.open(_context);
     * 
     * BusinessObject connectedChildObj = childRelationship.getTo(); connectedChildObj.open(_context);
     * 
     * BusinessObject connectedChildMajorObj = _util.getMajorObject(_context, connectedChildObj); if(connectedChildMajorObj == null) connectedChildMajorObj = connectedChildObj; else
     * connectedChildMajorObj.open(_context);
     * 
     * String connectedChildMajorObjType = connectedChildMajorObj.getTypeName(); String connectedChildMajorObjName = connectedChildMajorObj.getName();
     * 
     * if(connectedChildMajorObjType.equals(nodeChildMajorObjType) && connectedChildMajorObjName.equals(nodeChildMajorObjName)) { childRelationship.remove(_context); }
     * 
     * connectedChildMajorObj.close(_context); } } }
     * 
     * protected Hashtable getChildIdCadNodeTable(Context _context, IEFXmlNode cadObjectNode, Hashtable revReplaceTable) throws Exception { Hashtable childIdCadNodeTable = new Hashtable();
     * 
     * Enumeration childCadObjectElements = cadObjectNode.elements(); while(childCadObjectElements.hasMoreElements()) { IEFXmlNode childObjectNode = (IEFXmlNode)childCadObjectElements.nextElement();
     * String childObjectID = (String)childObjectNode.getAttribute("id"); String isSelected = (String)childObjectNode.getAttribute("selected"); String childMajorObjectId =
     * (String)childObjectNode.getAttribute("majorobjectid"); boolean isMajor = childMajorObjectId.equals(childObjectID);
     * 
     * if(!(MCADAppletServletProtocol.TRUE).equalsIgnoreCase(isSelected)) { BusinessObject childMajorObj = new BusinessObject(childMajorObjectId); childMajorObj.open(_context);
     * 
     * if(_generalUtil.isBusObjectFinalized(_context, childMajorObj)) { childObjectID = childMajorObjectId; isMajor = true; }
     * 
     * childMajorObj.close(_context); }
     * 
     * // If shared part is found, append the childIdCadNodesList with the cad node again. // This will be done for each shared part. if(childIdCadNodeTable.containsKey(childObjectID)) { Vector
     * childIdCadNodesList = (Vector)childIdCadNodeTable.get(childObjectID); childIdCadNodesList.addElement(childObjectNode); childIdCadNodeTable.put(childObjectID, childIdCadNodesList); } else {
     * Vector childIdCadNodesList = new Vector(); childIdCadNodesList.addElement(childObjectNode); childIdCadNodeTable.put(childObjectID, childIdCadNodesList); }
     * 
     * Vector boIdList = _util.getRevisionBoIdsOfAllStreams(_context, childObjectID, !isMajor); for(int i=0; i<boIdList.size(); i++) { revReplaceTable.put(boIdList.elementAt(i), childObjectID); } }
     * return childIdCadNodeTable; }
     * 
     * protected BusinessObject reviseVersionedTypeObject(Context _context, String minorObjectID, Hashtable revReplaceTable,Hashtable virtualStructureCreatedforNodes, Hashtable objIDNodeListTable)
     * throws Exception { BusinessObject revisedMinor = null; String cadType = _util.getCADTypeForBO(_context, new BusinessObject(minorObjectID));
     * 
     * if(_globalConfig.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) { Hashtable oldIdRevisedObjTable = new Hashtable(); String instanceId = minorObjectID; String familyId =
     * _generalUtil.getTopLevelFamilyObjectForInstance(_context, instanceId); BusinessObject familyObj = new BusinessObject(familyId);
     * 
     * familyObj.open(_context);
     * 
     * BusinessObject revisedFamily = null; if(virtualStructureCreatedforNodes.containsKey(familyId)) { revisedFamily = new BusinessObject(virtualStructureCreatedforNodes.get(familyId).toString()); }
     * else if(finalizationCandidates.contains(familyId)) { revisedFamily = familyObj; } else { revisedFamily = reviseWithLatestMinor(_context, familyId); virtualStructureCreatedforNodes.put(familyId,
     * revisedFamily.getObjectId()); finalizationCandidates.add(revisedFamily.getObjectId()); }
     * 
     * 
     ...
     * 
     * revisedMinor = (BusinessObject) oldIdRevisedObjTable.get(instanceId); revisedInstanceFamilyIdTable.put(revisedMinor.getObjectId(),familyId);
     * 
     * } else { revisedMinor = reviseWithLatestMinor(_context, minorObjectID); }
     * 
     * return revisedMinor; }
     * 
     * private BusinessObject reviseWithLatestMinor(Context _context, String inputMinorId) throws Exception { String latestMinorObjectID = _util.getLatestRevisionID(_context, inputMinorId);
     * 
     ...
     * 
     * String fileMsgDigestAttrName = MCADMxUtil.getActualNameForAEFData(_context, "attribute_IEF-FileMessageDigest"); String fileMsgDigestAttrVal = _util.getAttributeForBO(_context,
     * inputMinorObject,fileMsgDigestAttrName);
     * 
     ...
     * return revisedMinor; }
     * 
     * protected void reviseFamilyTower(Context _context, BusinessObject familyObj, BusinessObject revisedFamily, String activeInstanceId, Hashtable oldIdRevisedObjTable, Hashtable revReplaceTable)
     * throws Exception { Vector instanceList = _generalUtil.getInstanceListForFamilyObject(_context, familyObj.getObjectId(_context));
     * 
     * //in case of second instance of family gets revised both input family-object will be same IR-152981V6R2013 boolean isBothFamilySame =
     * familyObj.getObjectId().equals(revisedFamily.getObjectId());
     * 
     * for(int i=0; i<instanceList.size(); i++) { BusinessObject instObj = (BusinessObject) instanceList.elementAt(i); BusinessObject revisedInst = instObj;
     * 
     * instObj.open(_context);
     * 
     * String instObjId = instObj.getObjectId();
     * 
     * if(activeInstanceId.equals(instObjId)) { revisedInst = reviseWithLatestMinor(_context, instObjId);
     * 
     * _generalUtil.connectFamilyClassObjects(_context, revisedFamily, revisedInst);
     * 
     * if(isBothFamilySame) { String instanceOfRelName = MCADMxUtil.getActualNameForAEFData(_context, "relationship_InstanceOf"); _util.disconnectBusObjects(_context,
     * revisedFamily.getObjectId(),instObjId, instanceOfRelName, true); }
     * 
     * oldIdRevisedObjTable.put(instObjId, revisedInst); } else if(!isBothFamilySame) { _generalUtil.connectFamilyClassObjects(_context, revisedFamily, instObj); connectInstances(_context, null,
     * instObj, instObjId, revReplaceTable); }
     * 
     * instObj.close(_context); } }
     * 
     * protected void connectInstances(Context _context, BusinessObject revisedInst, BusinessObject instObj, String instObjId, Hashtable revReplaceTable) throws Exception { Hashtable relationsList =
     * _generalUtil.getAllWheareUsedRelationships(_context, instObj, true, MCADServerSettings.ASSEMBLY_LIKE); Enumeration allRels = relationsList.keys();
     * 
     * Set disconnectedRelsInfo = new HashSet();
     * 
     * while(allRels.hasMoreElements()) { boolean isDisconnected = false;
     * 
     * Relationship rel = (Relationship)allRels.nextElement(); String end = (String)relationsList.get(rel);
     * 
     * rel.open(_context);
     * 
     * String relName = rel.getTypeName();
     * 
     * if(_globalConfig.isRelationshipOfClass(relName, MCADServerSettings.EXTERNAL_REFERENCE_LIKE)) continue;
     * 
     * BusinessObject childBus = null; boolean bIsFrom = end.equals("from");
     * 
     * if (bIsFrom) childBus = rel.getTo(); else childBus = rel.getFrom();
     * 
     * String childBusId = childBus.getObjectId(_context);
     * 
     * StringBuffer relValidationInfo = new StringBuffer(); relValidationInfo.append(instObjId); relValidationInfo.append("|"); relValidationInfo.append(childBusId); relValidationInfo.append("|");
     * relValidationInfo.append(relName);
     * 
     * if(disconnectedRelsInfo.contains(relValidationInfo.toString())) continue;
     * 
     * Hashtable attrNameVals = new Hashtable(); AttributeList attrList = rel.getAttributes(_context); AttributeItr attrItr = new AttributeItr(attrList);
     * 
     * while (attrItr.next()) { Attribute attr = attrItr.obj(); attrNameVals.put(attr.getName(), attr.getValue()); }
     * 
     * rel.close(_context);
     * 
     * if(revReplaceTable.containsKey(childBusId)) { String childID = (String)revReplaceTable.get(childBusId);
     * 
     * if (revisedInst == null && !childID.equals(instObjId)) { _util.disconnectBusObjects(_context, instObjId, childBusId, relName, bIsFrom);
     * 
     * isDisconnected = true;
     * 
     * StringBuffer disconnectedRelInfo = new StringBuffer(); disconnectedRelInfo.append(instObjId); disconnectedRelInfo.append("|"); disconnectedRelInfo.append(childBusId);
     * disconnectedRelInfo.append("|"); disconnectedRelInfo.append(relName);
     * 
     * disconnectedRelsInfo.add(disconnectedRelInfo.toString()); }
     * 
     * childBus = new BusinessObject(childID); childBus.open(_context); }
     * 
     * if(revisedInst == null) { if(!isDisconnected) continue;
     * 
     * // required for inactive instance virtual structure promotion. -> inactive instance connected to non-finalized minor BusinessObject majorObject = _util.getMajorObject(_context, childBus);
     * 
     * if(majorObject != null) { majorObject.open(_context);
     * 
     * if(_generalUtil.isBusObjectFinalized(_context, majorObject)) { childBus.close(_context); childBus = majorObject; } else { majorObject.close(_context); } }
     * 
     * _util.connectBusObjects(_context, instObj, childBus, relName, bIsFrom, attrNameVals); } else _util.connectBusObjects(_context, revisedInst, childBus, relName, bIsFrom, attrNameVals);
     * 
