...
public class msoiMSFCommon_mxJPO extends emxDomainObject_mxJPO {
    ...
    private MapList getVersionObjectList(Context context, DomainObject masterObject) throws Exception {
        ...
        return masterObject.getRelatedObjects(context, CommonDocument.RELATIONSHIP_ACTIVE_VERSION, CommonDocument.TYPE_DOCUMENTS, versionSelectList, null, false, true, (short) 1, null, null, null,
                null, null);
    }
    ...
    public Vector getVersions(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            for (Iterator itrSelectables = mapList.iterator(); itrSelectables.hasNext();) {
                ...
                boolean vcDocumentType = (vcFileType != null && vcFileType.equalsIgnoreCase("true")) || (vcFolderType != null && vcFolderType.equalsIgnoreCase("true"))
    ...
    /**
     * Where : In Search Object result details, to hide 'Attach to existing' and revise menu item for older revision documents How : Check the current object revision and its last revision. If its not
     * the latest then remove revise, checkin
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - String containing the "programMap" programMap holds the following input arguments: 0 - String containing "objectList"
     * @returns MapList
     * @throws Exception
     *             if operation fails
     * @since MSF V6R2016x
