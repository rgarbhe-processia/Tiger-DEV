 ...
 *
 * FileName : "$RCSfile: ${CLASSNAME}.java.rca $" Author : "$Author: przemek $" Version : "$Revision: 1.10 $" Date : "$Date: Wed Oct 22 15:53:02 2008 $"
 *
 */
...
public class emxRNDEnqueueBase_mxJPO extends emxAbsPDFEnqueue_mxJPO {
     ...
     * @param args
     *            holds the following input arguments: "objectId" - String, Object Id "fileList" - LinkedList, the list of files to print
     * @exception Exception
     *                IllegalArgumentException, if wrong args are passed in
     * @since AEF 9.5.4.0
     */
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: Object Id
     * @return int, success
     * @exception Exception
     *                throws exceptions on any mql errors
     * @since AEF 9.5.4.0
     */
    ...
    /**
     * Goes through all the formats on an object, and adds all their files to the list of waht is to be rendered, then executes the rendering method.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectID
     *            Object Id
     * @exception Exception
     *                Throws an exception on any mql errors
     * @since AEF 9.5.4.0
     */
    public void doFiles(Context context, String objectId) throws Exception {
        ...
        StringTokenizer formatTokenizer = new StringTokenizer(result.substring(0, result.length() - 1), "|");
        if (testTokenizer.hasMoreTokens() && !result.trim().equals("\n")) {
            ...
            while (formatTokenizer.hasMoreTokens()) {
                ...
                if (!format.equals("PDF")) {
                    ...
                    StringTokenizer testTokenizer2 = new StringTokenizer(fileResult, "\n");
                    StringTokenizer fileTokenizer = new StringTokenizer(fileResult.substring(0, fileResult.length() - 1), "|");
    ...
    /**
     * runs the Adlib PDF Rendering integration (on a seperate Thread) the integration will check out the files, and then poll the output and error directories until it's time runs out
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param queue
     *            The PDF Queue with
    ...
    protected void runIntegration(Context context, emxPDFQueue_mxJPO queue) {
         ...
         * Author : DJ Date : 02/04/2003 Notes : History :
         */
        String integrationName = _util.getString(context, "eServiceBatchPrintPDF.PDF.Integration");
