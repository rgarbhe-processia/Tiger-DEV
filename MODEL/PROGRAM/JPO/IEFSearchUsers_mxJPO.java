 ...
 */
/**
 * IEFSearchUsers.java This JPO is used for searching users in the database based on input criteria
 * @since IEF 10.5
 */
...
public class IEFSearchUsers_mxJPO {
    ...
    /**
     * List returned by this method is used to render a table displaying users corresponding to the query entered in the user-search dialog. It returns a CustomMapList containing hashmap objects. Each
     * hashmap descrbes a row in the table.
     * @param Context
     *            context for user logged in
     * @param String
     *            array This method expects following parameters to be packed in string array sTopLevel=<toplevel_value> sPerson=<person_value> sGroup=<group_value> sRole=<role_value>
    ...
    public Object getTableData(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (((sListPerson != null) && (sListPerson.size() != 0)) || ((sListGroup != null) && (sListGroup.size() != 0)) || ((sListRole != null) && (sListRole.size() != 0))) {
                int count = 0;
                count = filterList(sListPerson, iFilterCount, usersList, "images/iconPerson.gif", MCADMxUtil.getActualNameForAEFData(context, "type_Person"));

                count += filterList(sListGroup, iFilterCount, usersList, "images/iconGroup.gif", "Group");

                count += filterList(sListRole, iFilterCount, usersList, "images/iconRole.gif", "Role");
    ...
    private int filterList(StringList currentList, int iLimit, CustomMapList usersList, String sImageSrc, String userType) {
        int count = 0;
