...
/*
 * ${CLASSNAME}.java Base JPO for migrating Objects for Document Library Schema Unification
 *
...
public class emxLibraryCentalDocumentSchemaMigrationBase_mxJPO extends emxCommonMigration_mxJPO {
    ...
    public emxLibraryCentalDocumentSchemaMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
     ...
     * @param args
     *            holds three arguments args[0] - Directory where files containing ObjectIds are to be read args[1] - minimum Range (ie. Flat file to start migration from) args[2] - maximum Range (ie.
     *            Flat file till which migration is to be run)
     * @returns 0 always
     * @throws Exception
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        ...
        String noOfUnConvertedDocLibraries = MqlUtil.mqlCommand(context, "eval expr $1 on temp query bus $2 $3 $4 where $5", "count TRUE", TYPE_DOCUMENT_LIBRARY, "*", "*",
                "interface[" + docLibInterface + "] == FALSE");
        try {
    ...
    /**
     * This method is executed for each Flat file this method migrates whole structure under each 'Document Library' object in objectList
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectList
     *            - contains the list of ObjectIds in a specific Flat file
     * @returns nothing
     * @throws Exception
     *             if operation fails
     */
    public void migrateObjects(Context context, StringList objectList) throws Exception {
        ...
        for (int docLibItr = 0; docLibItr < lengthObjectList; docLibItr++) {
            ...
            try {
                ...
                MapList childBookshelfs = DLUnderProcess.getRelatedObjects(context, RELATIONSHIP_HAS_BOOKSHELVES, "*", new StringList("id"), null, false, true, new Short("1"), "", "", 0);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - String array containing args[0] - objectId_BookshelfUnderProcess - OID of the BookshelfUnderProcess of migration args[1] - objectId_DLUnderProcess - OID of the DLUnderProcess of
     *            migration args[2] - physicalId_DLUnderProcess - physical Id of the DLUnderProcess of migration args[3] - hasParent - "true" if BookshelfUnderProcess of migration is connected to
     *            atleast one 'Document Library'
     * @returns 0
     * @throws Exception
     *             if operation fails
     */
    public int migrateBookshelf(Context context, String[] args) throws Exception {
        ...
        if (hasParent) {
            ...
            MapList parentOIDs_BookshelfUnderProcess = BookshelfUnderProcess.getRelatedObjects(context, RELATIONSHIP_HAS_BOOKSHELVES, "*", new StringList("id"), null, true, false, new Short("1"), "",
                    "", 0);
            ...
            if (noOfParentDLs > 1) {
                ...
                MqlUtil.mqlCommand(context, "copy bus $1 to $2 $3 history", objectId_BookshelfUnderProcess, name_BookshelfUnderProcess + "." + revision_BookshelfUnderProcess,
                        physicalId_DLUnderProcess);
                String clonedObjectID = MqlUtil.mqlCommand(context, "print bus $1 $2 $3 select $4 dump", TYPE_BOOKSHELF, name_BookshelfUnderProcess + "." + revision_BookshelfUnderProcess,
                        physicalId_DLUnderProcess, "id");
            ...
            if (!(noOfParentDLs > 1)) {
                ...
                MqlUtil.mqlCommand(context, "modify bus $1 name $2 revision $3", objectId_BookshelfUnderProcess, name_BookshelfUnderProcess + "." + revision_BookshelfUnderProcess,
                        physicalId_DLUnderProcess);
            }
        ...
        MapList childBooks = BookshelfUnderProcess.getRelatedObjects(context, RELATIONSHIP_HAS_BOOKS, "*", new StringList("id"), null, false, true, new Short("1"), "", "", 0);
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectId_BookUnderProcess
     *            - OID of the BookUnderProcess of migration
     * @param objectId_BookshelfUnderProcess
     *            - OID of the BookshelfUnderProcess of migration
     * @param physicalId_BookshelfUnderProcess
     *            - Physical Id of the BookshelfUnderProcess of migration
     * @param hasParent
     *            - true if BookUnderProcess of migration is connected to atleast one 'Bookshelf'
     * @returns 0
     * @throws Exception
     *             if operation fails
     */
    public int migrateBook(Context context, String[] args) throws Exception {
        ...
        if (hasParent) {
            ...
            MapList parentOIDs_BookUnderProcess = BookUnderProcess.getRelatedObjects(context, RELATIONSHIP_HAS_BOOKS, "*", new StringList("id"), null, true, false, new Short("1"), "", "", 0);
            ...
            if (noOfParentBookShelfs > 1) {
                ...
                String clonedObjectID = MqlUtil.mqlCommand(context, "print bus $1 $2 $3 select $4 dump", TYPE_BOOK, name_BookUnderProcess + "." + revision_BookUnderProcess,
                        physicalId_BookshelfUnderProcess, "id");
        ...
        MapList mapListChildGenDocs = BookUnderProcess.getRelatedObjects(context, RELATIONSHIP_HAS_DOCUMENTS, "*", null, selectables, false, true, new Short("1"), "", "", 0);
    ...
    /**
     * This method is executed for pre-migration changes required before starting actual Migration pre migration is executed only once
     * @param context
     * @param args
     *            holds nothing
     * @return 0 always
    ...
    public int preMigration(Context context, String[] args) throws Exception {
        if (getAdminMigrationStatus(context, migrationStatusProperty) < 2) {
            try {
                ...
                MqlUtil.mqlCommand(context, "add interface $1 attribute $2 attribute $3 attribute $4 attribute $5 type $6", docLibInterface, "Approver", "Created On", "Count", "mxsysInterface",
                        "All");
    ...
    /**
     * this method modifies parent type of 'Document Library' to Libraries and adds attributes 'Approver' and 'Created On' to type 'Document Library'
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds nothing
    ...
    /**
     * This method creates Classification Interface and updates attribute[mxsysInterface] with the name of the created Interface
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectId_objectUnderProcess
     *            objectId of the Object to which interface should be created
     * @param interfaceName_objectUnderProcess
     *            interface name of the interface to be created
     * @param name_parentInterface
     *            name of the parent interface , if present
     * @param hasParent
     *            true if parent interface is present, false otherwise
     * @returns nothing
     * @throws Exception
     *             if the operation fails
     */
    public void createClassificationInterface(Context context, String objectId_objectUnderProcess, String interfaceName_objectUnderProcess, String name_parentInterface, boolean hasParent)
            throws Exception {
        if (hasParent) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param objectId
     *            object Id of the object that failed
     * @param reason
    ...
    /**
     * this is a help method which displays help about migration on console command to be executed to get help on console is execute program emxLibraryCentalDocumentSchemaMigration -method help;
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds nothing
    ...
    /**
     * This method sets flatFileNoUnderProcess and Calls super method which Reads the contents of the file and puts in Arraylist.
     * @param args
