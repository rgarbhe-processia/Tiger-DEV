...
public class emxInboxTaskNotificationBase_mxJPO extends emxSpool_mxJPO {
    protected static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(emxInboxTaskNotificationBase_mxJPO.class);
     ...
     * @grade 0
     */
    public emxInboxTaskNotificationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public StringList getRouteTaskOwner(Context context, String[] args) throws MatrixException {
        ...
        try {
            ...
            if ("group".equals(type) || "role".equals(type)) {
                // TIGTK-5450 : START
                if (TigerConstants.TYPE_CHANGEACTION.equalsIgnoreCase(strContentObjType)) {
                    String strProgProjId = routeTask.getInfo(context, "from[" + TigerConstants.RELATIONSHIP_ROUTE_TASK + "].to.to[" + TigerConstants.RELATIONSHIP_OBJECT_ROUTE + "].from.to["
                            + TigerConstants.RELATIONSHIP_CHANGEACTION + "].from.to[" + TigerConstants.RELATIONSHIP_PSS_CONNECTEDPCMDATA + "].from.id");
                    routeTaskOwnerList = getProgramProjectMembers(context, type, routeTaskOwner, strProgProjId);
                } else if (TigerConstants.TYPE_PSS_MANUFACTURINGCHANGEACTION.equalsIgnoreCase(strContentObjType)) {
                    String strProgProjId = routeTask.getInfo(context, "from[" + TigerConstants.RELATIONSHIP_ROUTE_TASK + "].to.to[" + TigerConstants.RELATIONSHIP_OBJECT_ROUTE + "].from.to["
     ...
     * @throws Exception
     */
    private StringList getProgramProjectMembers(Context context, String groupOrRole, String groupOrRoleName, String strProgProjId) throws Exception {
    ...
    public MapList getMembersFromProgram(Context context, DomainObject domProgram, StringList objectSelect, StringList relSelect, String objWhere, String relWhere) throws Exception {
        ...
        try {
            ...
            System.out.println("Error is method getMembersFromProgram in JPO pss.ecm.enoECMChange");
        }
        return mlMemberList;
    }
    // TIGTK-5450 : END
    ...
    private StringList getGroupOrRoleMembers(Context context, String groupOrRole, String groupOrRoleName) throws Exception {
        ...
        if ("role".equalsIgnoreCase(groupOrRole)) {
            // PCM TIGTK-3891: 10/01/2017 : KWagh : START
            Role role = new Role(groupOrRoleName);
            String strAssignee = MqlUtil.mqlCommand(context, "print role '" + role + "' select person dump |");

            members = FrameworkUtil.split(strAssignee, "|");
            // role.open(context);
            ...
            // PCM TIGTK-3891: 10/01/2017 : KWagh : End
