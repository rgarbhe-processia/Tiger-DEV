 ...
 * publication of such program. static const RCSID [] = "$Id: ${CLASSNAME}.java.rca 1.21 Wed Oct 22 16:02:31 2008 przemek Experimental przemek $";
 */
...
public class emxLibraryCentralCreateInterfaceBase_mxJPO {
     ...
     */
    public emxLibraryCentralCreateInterfaceBase_mxJPO(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails
     */
    public int createInterfaceObject(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails
     */
    public int createInterfaceObject(Context context, String objectId) throws Exception {
    ...
    private void createLibraryFeatureReference(Context context, String libraryObjectId, String libraryFeatureReferenceName) throws Exception {
        ...
        try {
            ...
            commonBase.createAndConnect(context, LibraryCentralConstants.TYPE_LIBRARY_FEATURE_REFERENCE, libraryFeatureReferenceName,
                    new DomainObject().getDefaultRevision(context, LibraryCentralConstants.POLICY_LIBRARY_FEATURE_REFERENCE), LibraryCentralConstants.POLICY_LIBRARY_FEATURE_REFERENCE,
                    context.getVault().getName(), LibraryCentralConstants.RELATIONSHIP_LIBRARY_REFERENCE_FEATURE, new DomainObject(libraryObjectId), true);
            ...
            if (strParentInterface == null || "".equals(strParentInterface) || "null".equals(strParentInterface)) {
                try {
                    if (createInterfaceObject(context, strFromObjectId) == 0) {
                        ...
                        result = (MapList) parentObj.getRelatedObjects(context, LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM, LibraryCentralConstants.QUERY_WILDCARD, selectStmts,
            ...
            if (strChildInterface == null || "".equals(strChildInterface) || "null".equals(strChildInterface)) {
                try {
                    if (createInterfaceObject(context, strToObjectId) == 0) {
                        ...
                        selectStmts.addElement(DomainObject.SELECT_ID);
                        MapList result = new MapList();
                        result = (MapList) childObj.getRelatedObjects(context, LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM, LibraryCentralConstants.QUERY_WILDCARD, selectStmts,
                                new StringList(), false, true, (short) 1, null, null);
                        int iSize = result.size();
                        for (int k = 0; k < iSize; k++) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments 0 - FROMOBJECTID 1 - TOOBJECTID
     * @return int 0 or 1 for Success or failure
     * @throws Exception
     *             if the operation fails
     */
     ...
     *             if the operation fails
     */
    public static void implementInterfaceOnClassifiedItem(Context context, String[] args) throws Exception, FrameworkException {
        String strResult = "";
        if (args.length > 0) {
            String strToObjectId = args[0];
            String strFromObjectId = args[1];
     ...
     *             if the operation fails
     */
    public static String implementInterfaceOnClassifiedItem(Context context, String parentId, String objectId) throws Exception, FrameworkException {
        String strResult = "";
        ...
        try {
            ...
            if (strResult != null && strResult.indexOf(strInterface) == -1) {
                ...
                if (CommonDocument.TYPE_DOCUMENTS.equals(CommonDocument.getParentType(context, toObject.getInfo(context, DomainObject.SELECT_TYPE)))
                    ...
                    MapList result = (MapList) toObject.getRelatedObjects(context, CommonDocument.RELATIONSHIP_ACTIVE_VERSION, "*", selectStmts, new StringList(), false, true, (short) 1, null, null);
                    ...
                    if (result != null && result.size() > 0) {
                        for (int lCnt = 0; lCnt < result.size(); lCnt++) {
                            ...
                            if (!moreThanOneInterfaces && interfaceStr != null && interfaceStr.indexOf(strInterface) == -1) {
                                addInterface = true;
                            } else if (moreThanOneInterfaces && interfacesList != null && !interfacesList.contains(strInterface)) {
                                addInterface = true;
                            }
     ...
     *             if the operation fails
     */
    public static String modifyInterfaceOnClassifiedItem(Context context, String[] args) throws Exception, FrameworkException {
        String strResult = "";
        if (args.length > 0) {
            String strToObjectId = args[0];
            String strFromObjectId = args[1];
            ...
            try {
                ...
                if (strResult != null && strInterface != null && strResult.length() > 0 && strInterface.length() > 0 && strResult.indexOf(strInterface) != -1) {
                    ...
                    if (CommonDocument.TYPE_DOCUMENTS.equals(CommonDocument.getParentType(context, toObj.getInfo(context, DomainObject.SELECT_TYPE)))
                        ...
                        selectStmts.addElement(SELECT_ACTIVE_VERSION_ID);
                        selectStmts.addElement("interface");
                        MapList result = (MapList) toObj.getRelatedObjects(context, CommonDocument.RELATIONSHIP_ACTIVE_VERSION, "*", selectStmts, new StringList(), false, true, (short) 1, null, null);
                        String activeVersionId = "";
                        String implementedInterfaces = "";
                        ...
                        if (result != null && result.size() > 0) {
                            for (int lCnt = 0; lCnt < result.size(); lCnt++) {
                                ...
                                if (!moreThanOneInterfaces && interfaceStr != null && interfaceStr.length() > 0 && interfaceStr.indexOf(strInterface) != -1) {
                                    removeInterface = true;
                                } else if (moreThanOneInterfaces && interfacesList != null && interfacesList.size() > 0 && interfacesList.contains(strInterface)) {
                                    removeInterface = true;
                                }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments: 0 - objectId 1 - minorObjId
     * @return int 0 or 1 for success or failure
     * @throws Exception
     *             if the operation fails
     */
    public int update(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            String implementIntOnMinorObjs = EnoviaResourceBundle.getProperty(context, "emxLibraryCentral.MinorObjects.Classify");
            if (CommonDocument.TYPE_DOCUMENTS.equals(CommonDocument.getParentType(context, type)) && (implementIntOnMinorObjs != null && implementIntOnMinorObjs.equalsIgnoreCase("true"))) {
    ...
    /**
     * Update Minor object and its revisions to add interface(s) if the property MinorObjects.Classify is set to true and parent's object type is DOCUMENTS
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectId
     *            the id of the master object
     * @param minorObjectId
     *            the id of the minor object
     * @return int 0 or 1 for success or failure
     * @throws Exception
     *             if the operation fails
     */
    public static int updateMinorObjects(Context context, String objectId, String minorObjectId) throws FrameworkException {

        if (objectId == null || "null".equals(objectId) || objectId.length() <= 0 || minorObjectId == null || "null".equals(minorObjectId) || minorObjectId.length() <= 0) {
        ...
        try {
            ...
            if (CommonDocument.TYPE_DOCUMENTS.equals(CommonDocument.getParentType(context, type)) && (implementIntOnMinorObjs != null && implementIntOnMinorObjs.equalsIgnoreCase("true"))) {
                ...
                DomainObject minorObj = (DomainObject) DomainObject.newInstance(context, minorObjectId);
                ...
                if (minObjInterfaces instanceof String && minObjInterfaces != null && ((String) minObjInterfaces).length() > 0) {
                ...
                for (int i = 0; i < revObjectList.size(); i++) {
                    ...
                    for (int j = 0; j < interfacesListTmp.size(); j++) {
                        ...
                        if (newInterfaceStr != null && newInterfaceStr.length() > 0 && !minObjInterfacesList.contains(newInterfaceStr)) {
