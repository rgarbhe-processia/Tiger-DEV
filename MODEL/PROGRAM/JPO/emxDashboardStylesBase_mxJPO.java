...
public class emxDashboardStylesBase_mxJPO {
    ...
    public StringList highlightContent(Context context, String[] args) throws Exception {
        ...
        if (mlObjects.size() > 0) {
            ...
            if (sContents != null) {
                ...
                System.out.println(
                        "Error in execution of GNVStyles.highlightContent : Contents to be highlighted cannot be retrieved from column settings. Please apply the setting 'Contents' properly!");
            }
            ...
            for (int i = 0; i < mlObjects.size(); i++) {
                ...
                if (sExpression.endsWith(".value")) {
                    sExpression = sExpression.substring(0, sExpression.length() - 6);
                }
    ...
    public StringList highlightOwnership(Context context, String[] args) throws Exception {
        ...
        if (mlObjects.size() > 0) {
            ...
            if (sStyleOwner == null) {
                sStyleOwner = "custOwner";
            }
            if (sStyleOthers == null) {
                sStyleOthers = "custOtherOwners";
            }
    ...
    public StringList compareToEstimatedStart(Context context, String[] args) throws Exception {
        return compareTo(context, args, "attribute[" + DomainConstants.ATTRIBUTE_TASK_ESTIMATED_START_DATE + "]");
    }

    public StringList compareToEstimatedFinish(Context context, String[] args) throws Exception {
        return compareTo(context, args, "attribute[" + DomainConstants.ATTRIBUTE_TASK_ESTIMATED_FINISH_DATE + "]");
    }
