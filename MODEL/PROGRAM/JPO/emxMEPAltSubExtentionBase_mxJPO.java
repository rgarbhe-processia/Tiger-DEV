...
public class emxMEPAltSubExtentionBase_mxJPO extends emxCommonPart_mxJPO implements EngineeringConstants {
     ...
     * @since EC 9.5.JCI.0.
     */
    public emxMEPAltSubExtentionBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    /**
     * Access Function to show MEP Usage (Alt/Sub) columns. Checks for Alt/Sub feature is enabled from ENG property file.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     ...
     *             if the operation fails.
     */
    public boolean showAltSub(Context context, String[] args) throws Exception {
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * This method gives MEP usage (Alternate, Substitute or none/blank) for a context part connected as MFG Equivalent relationship.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the input arguments.
     ...
     */
    public StringList getMEPUsage(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                if (EngineeringUtil.isPartAuthorizedToHostCompany(context, strMEPId)) {
                    ...
                    mepAltSubInfo = FrameworkUtil
                            .split(MqlUtil.mqlCommand(context, "print bus $1 select $2 $3 $4 $5 dump", strMEPId, "to[" + RELATIONSHIP_ALTERNATE + "|from.id=='" + contextPartId + "']",
                                    "to[" + RELATIONSHIP_EBOM_SUBSTITUTE + "|fromrel.to.id=='" + contextPartId + "']", SELECT_TYPE, SELECT_CURRENT), ",");
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Column program for MEP Usage column. Displays if MEP is added as alternate or Subtitute or none of them (blank) to context part.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the input arguments.
     ...
     * @return StringList of Part Usage(Alternate/Substitute/blank).
     */
    public StringList getPartUsageForMEP(Context context, String[] args) throws Exception {
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Column program for MEP Usage Context column. Displays context parent part for which the MEP is added as Subtitute and blank for alternate.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the input arguments.
     ...
     */
    public StringList getContextPartsForMEP(Context context, String[] args) throws Exception {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                if (EngineeringUtil.isPartAuthorizedToHostCompany(context, strMEPId) && !UIUtil.isNullOrEmpty(ebomRelId)) {
                    ...
                    for (int j = 0; j < subContextPartList.size(); j++) {
                        ...
                        sbCellData.append("<b><a href=\"javascript:showNonModalDialog('../common/emxTree.jsp?emxSuiteDirectory=engineeringcentral&amp;objectId=");
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Column program for MEP Usage Actions column. Displays edit Icon for alternate MEP and edit/add/remove icons for substitute MEP and blank for others.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the input arguments.
     ...
     * @return StringList of Context Parent Parts.
     */
    public StringList getActionsForMEP(Context context, String[] args) throws Exception {
                ...
                if (EngineeringUtil.isPartAuthorizedToHostCompany(context, strMEPId)) {
                    ...
                    if (ALTERNATE.equals(mepAltSubList.get(i))) {
                        ...
                        if (bHasModifyAccess) {
                            ...
                            cellData.append(
                                    "','../common/emxIndentedTable.jsp?field=TYPES=type_Part&amp;table=ENCAlternateSubstitutePartSummary&amp;selection=multiple&amp;MEPActions=DefineSubstitute&amp;submitAction=refreshCaller&amp;submitURL=../engineeringcentral/emxEngrMEPUsage.jsp&amp;SubmitLabel=Done&amp;CancelButton=true&amp;objectId=");
                        ...
                        if (bHasModifyAccess) {
                            ...
                            cellData.append(
                                    "<a href=\"javascript:showChooser('../common/emxIndentedTable.jsp?field=TYPES=type_Part&amp;table=ENCAlternateSubstitutePartSummary&amp;selection=multiple&amp;submitAction=refreshCaller&amp;MEPActions=DefineSubstitute&amp;submitURL=../engineeringcentral/emxEngrMEPUsage.jsp&amp;SubmitLabel=Done&amp;CancelButton=true&amp;objectId=");
                            cellData.append(XSSUtil.encodeForURL(context, strMEPId));
                            cellData.append("&amp;parentOID=");
                            cellData.append(XSSUtil.encodeForURL(context, contextPartId));
                            cellData.append(
                                    "&amp;program=emxMEPAltSubExtention:getContextParents', '700', '600', 'false', 'popup', '')\"><img border=\"0\" src=\"images/iconActionAddExistingPart.gif\" alt=\"");
                            ...
                            cellData.append(
                                    "<a href=\"javascript:showChooser('../common/emxIndentedTable.jsp?field=TYPES=type_Part&amp;table=ENCAlternateSubstitutePartSummary&amp;selection=multiple&amp;submitAction=refreshCaller&amp;MEPActions=RemoveSubstitute&amp;submitURL=../engineeringcentral/emxEngrMEPUsage.jsp&amp;SubmitLabel=Done&amp;CancelButton=true&amp;objectId=");
                            cellData.append(XSSUtil.encodeForURL(context, strMEPId));
                            cellData.append("&amp;parentOID=");
                            cellData.append(XSSUtil.encodeForURL(context, contextPartId));
                            cellData.append(
                                    "&amp;program=emxMEPAltSubExtention:getContextParents', '700', '600', 'false', 'popup', '')\"><img border=\"0\" src=\"images/iconActionRemove.gif\" alt=\"");
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Program to select context parents for MEP. Only context parents of MEP connected as Substitute are returned for remove case. Add case returns parents of MEP not connected as substitute.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the input arguments.
     ...
     * @return StringList of Context Parent Parts.
     */
    public MapList getContextParents(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList contextParentList = DomainObject.newInstance(context, strContextPartId).getRelatedObjects(context, RELATIONSHIP_EBOM, TYPE_PART, new StringList(SELECT_ID), relSelect, true, false,
                    (short) 1, null, null, 0);
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Method is used to switch the MEP Usage from Alt to Sub and vice versa. It also add/remove new parents as Sub for a MEP. It performs the operations of Actions column.
     * @param context
     *            the eMatrix <code>Context</code> object.
     ...
     * @throws Exception
     */
    public void modifyUsageForMEP(Context context, String[] args) throws Exception {
            ...
            if ("DefineAlternate".equalsIgnoreCase(sAction)) {
                ...
                if (slParentId == null || "null".equals(slParentId) || slParentId.length == 0) {
                    throw new FrameworkException(
                            EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.Common.PleaseMakeSelection"));
                }
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Method connects the MEP as Alternate to the part if it is not already connected.
     * @param context
     *            the eMatrix <code>Context</code> object.
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Trigger Program on create Manufacturing Equivalent Rel and on promote approved of Manufacturing Equivalent policy. If the part policy is MEP and state is released, it is added as alternate
     * while adding or releasing MEP. This will be done if Alt/Sub feature is enabled and MEP is authorized to Host Company.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the input arguments.
    ...
    public void defineAsAlternate(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if ("false".equalsIgnoreCase(equivalentPartExtensionEnabled) || !EngineeringUtil.isPartAuthorizedToHostCompany(context, strMEPId)
                    || !POLICY_MANUFACTURER_EQUIVALENT.equals(mepInfo.get(SELECT_POLICY)) || TYPE_MPN.equals(mepInfo.get(SELECT_TYPE))) {
                return;
            } else if (STATE_MANUFACTURER_EQUIVALENT_RELEASE.equals(mepInfo.get(SELECT_CURRENT))) {

                String parentsForMEP = MqlUtil.mqlCommand(context, "print bus $1 select $2 $3 dump", strMEPId,
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    /**
     * Trigger Program on delete Manufacturing Equivalent Rel. If the part policy is MEP and it is deleted as Manufacturing Equivalent, then all Alt/Sub definition related to that MEP will also be
     * deleted.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
