...
/**
 * @author QZV The &lt;code&gt;${CLASSNAME}&lt;/code&gt; class contains ... Script to migrate "Design Part", "Engineering Part" policy parts to "Development Part" policy
...
public class emxTBEPartMigrationBase_mxJPO extends emxCommonMigration_mxJPO {
     ...
     * @grade 0
     */
    public emxTBEPartMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }

    /**
     * This method does the migration work. All exsting "Engineering Part", "Design Part" policy objects will be migrated to "Development Part" policy
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectIdList
    ...
    public void migrateObjects(Context context, StringList objectIdList) throws Exception {
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                altOwnerList = FrameworkUtil.split(MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3", objectId, "from[" + EngineeringConstants.RELATIONSHIP_PART_SPECIFICATION
                        + "|to.type.kindof==\"PLMEntity\"].to.altowner1 from[" + EngineeringConstants.RELATIONSHIP_PART_SPECIFICATION + "|to.type.kindof==\"PLMEntity\"].to.altowner2", "|"), "|");
                ...
                if (altOwnerList.size() == 2) {
                    ...
                    MqlUtil.mqlCommand(context, "mod bus $1 policy $2 $3 $4 $5 $6 altowner1 $7 altowner2 $8", objectId, EngineeringConstants.POLICY_DEVELOPMENT_PART, ATTRIBUTE_VPM_VISIBLE,
                            Boolean.toString(!"Engineering Part".equalsIgnoreCase(strPartPolicy)), ATTRIBUTE_V_NAME, vpmProductName, (String) altOwnerList.get(0), (String) altOwnerList.get(1));
                } else {
                    mqlLogRequiredInformationWriter(objectId + " doesnt have altowner information \n");
                    MqlUtil.mqlCommand(context, "mod bus $1 policy $2 $3 $4 $5 $6", objectId, EngineeringConstants.POLICY_DEVELOPMENT_PART, ATTRIBUTE_VPM_VISIBLE,
                            Boolean.toString(!"Engineering Part".equalsIgnoreCase(strPartPolicy)), ATTRIBUTE_V_NAME, vpmProductName);
                }
