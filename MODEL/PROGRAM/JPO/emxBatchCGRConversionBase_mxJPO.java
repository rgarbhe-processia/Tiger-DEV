...
public class emxBatchCGRConversionBase_mxJPO {
     ...
     */
    public emxBatchCGRConversionBase_mxJPO(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds command line arguments for vault if passed
    ...
    /**
     * This method is primary method which gets cgr objects and prcesses them and converts cgr images to 3dxml images
     * @param context
     *            the eMatrix <code>Context</code> object
     * @returns nothing
     * @throws Exception
    ...
    public void ConvertCGRto3Dxml(Context context) throws Exception {
        DomainObject cgrObject, xmlObject;
        ...
        StringList files;
        String cgrObjectId = "";
        String xmlCgrObjectId = "";
        String workspacepath, jpegfile;
        String mqlCmd = "";
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (tempList.size() == 5) {
                    ...
                    cgrFiles = FrameworkUtil.split(MqlUtil.mqlCommand(context, "print bus " + cgrId + " select format[" + CGR_FORMAT + "].file.name dump |"), "|");
                    Iterator cgritr = cgrFiles.iterator();
                    if (cgrFiles.size() > 0) {
                        MqlUtil.mqlCommand(context, "add businessobject " + TYPE_3DXMLCGR + " " + xmlcgrname + " A policy 'Viewable Policy' vault '" + VAULT_PRODUCTION + "'");
                        xmlcgrId = (String) (FrameworkUtil.split(MqlUtil.mqlCommand(context, "temp query bus " + TYPE_3DXMLCGR + " " + xmlcgrname + " A select id dump |"), "|")).get(3);
                        MqlUtil.mqlCommand(context, "connect businessobject " + parentId + " relationship Viewable to " + xmlcgrId);
                        while (cgritr.hasNext()) {
                            String cgrfile = (String) cgritr.next();
                            tempString = getFileBaseName(cgrfile) + ".3dxml";
                            mqlCmd = "checkout businessobject " + cgrId + " format " + CGR_FORMAT + " file " + cgrfile + " '" + workspacepath + "'";
                            MqlUtil.mqlCommand(context, mqlCmd);
                            prepare3dxmlFromCGR(context, workspacepath, cgrfile);
                            mqlCmd = "checkin businessobject " + xmlcgrId + " format " + XMLCGR_FORMAT + " append '" + workspacepath + java.io.File.separatorChar + tempString + "'";
                            MqlUtil.mqlCommand(context, mqlCmd);
                        }
                    }
                ...
                // If cgr and 3dxml or 3dxmlcgr objects are connected to CAD object with Viewable relationship
                    ...
                    if (flag.equals(TYPE_3DXML)) {
                        ...
                        xmlFiles = FrameworkUtil.split(MqlUtil.mqlCommand(context, "print bus " + xmlId + " select format[" + XML_FORMAT + "].file.name dump |"), "|");
                        if (cgrFiles != null && cgrFiles.size() > 0) {
                            MqlUtil.mqlCommand(context, "add businessobject " + TYPE_3DXMLCGR + " " + xmlcgrname + " A policy 'Viewable Policy' vault '" + VAULT_PRODUCTION + "'");
                            xmlcgrId = (String) (FrameworkUtil.split(MqlUtil.mqlCommand(context, "temp query bus " + TYPE_3DXMLCGR + " " + xmlcgrname + " A select id dump |"), "|")).get(3);
                            MqlUtil.mqlCommand(context, "connect businessobject " + parentId + " relationship " + REL_VIEWABLE + " to " + xmlcgrId);
                            Iterator cgritr = cgrFiles.iterator();
                            while (cgritr.hasNext()) {
                                String cgrfile = (String) cgritr.next();
                                tempString = getFileBaseName(cgrfile) + ".3dxml";
                        ...
                        xmlcgrFiles = FrameworkUtil.split(MqlUtil.mqlCommand(context, "print bus " + xmlcgrId + " select format[" + XMLCGR_FORMAT + "].file.name dump |;"), "|");
                        Iterator cgritr = cgrFiles.iterator();
                        while (cgritr.hasNext()) {
                            String cgrfile = (String) cgritr.next();
                            tempString = getFileBaseName(cgrfile) + ".3dxml";
    ...
    public ArrayList getViewableObjects(Context context) {
        ...
        String mqlcmd = "temp query bus '" + TYPE_CGR + "' * * !expandtype where 'to[" + REL_VIEWABLE + "]==True' select to[" + REL_VIEWABLE + "].from.id id to[" + REL_VIEWABLE + "].from.from["
                + REL_VIEWABLE + "].to[" + TYPE_3DXML + "].type to[" + REL_VIEWABLE + "].from.from[" + REL_VIEWABLE + "].to[" + TYPE_3DXML + "].id to[" + REL_VIEWABLE + "].from.from[" + REL_VIEWABLE
                + "].to[" + TYPE_3DXMLCGR + "].type to[" + REL_VIEWABLE + "].from.from[" + REL_VIEWABLE + "].to[" + TYPE_3DXMLCGR + "].id dump |";
        try {
            String result = MqlUtil.mqlCommand(context, mqlcmd);
    ...
    /**
     * This method is used to prepare the 3dxml file from the given cgr file
     * @param matrix
     *            .db.Context,String workspacepath and String filename of the cgr
     * @return nothing @ places the created 3dxml from cgr in given workspace path
     * @since AppsCommon 10.7.SP1
     */
