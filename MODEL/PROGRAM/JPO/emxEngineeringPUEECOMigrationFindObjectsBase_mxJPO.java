...
public class emxEngineeringPUEECOMigrationFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO {
     ...
     * @grade 0
     */
    public emxEngineeringPUEECOMigrationFindObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public void getIds(Context context, int chunkSize) throws Exception {
        ...
        try {
            ...
            String command = "temp query bus '" + type + "' '" + name + "' '" + revision + "'  vault '" + vaultList + "' limit 1 where '" + whereClause + " && program[" + migrationProgramName
                    + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true'";

            writer.write("command = " + command + "\n");
            MqlUtil.mqlCommand(context, "temp query bus $1 $2 $3 vault $4 limit 1 where (policy==$5) && program[$6 -method writeOID ${OBJECTID} ${TYPE}] == true", type, name, revision, vaultList,
                    UnresolvedEBOMConstants.POLICY_PUE_ECO, migrationProgramName);
        } catch (Exception me) {
            throw me;
        }
