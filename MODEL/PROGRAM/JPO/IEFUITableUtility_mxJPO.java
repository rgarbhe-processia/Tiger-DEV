...
/*
 ** IEFUITableUtility This is an extract of the emxTable.jsp from AEF The extract of the JSP is made into a JPO since the Office Integration does not go through the jsp layer Minor modifcations are
 * done to support special use cases
 **
 ** Copyright (c) 1992-2003 MatrixOne, Inc. All Rights Reserved. This program contains proprietary and trade secret information of MatrixOne, Inc. Copyright notice is precautionary only and does not
...
public class IEFUITableUtility_mxJPO {
    ...
    /**
     * Method for rendering a table This is an extract of the emxTable.jsp from AEF The extract of the JSP is made into a JPO since the Office Integration does not go through the jsp layer
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param tableName
     *            the name of the table to be used for rendering
     * @param totalResult
     *            the list of ids that needs to be rendered this list is obtained by running the JPO or inquiry if id is absent "nodeType" key must be present with value "file" - denoting file like
     *            node "folder" - denoting folder like node
        ...
        if (tableMap == null) {
            ...
            if (columns == null || columns.size() == 0) {
                ...
                for (int i = 0; i < noOfColumns; i++) {
                    ...
                    if (selectTypes[i].compareTo(typeImage) == 0 || selectTypes[i].compareTo("icon") == 0 || selectTypes[i].compareTo("programHTMLOutput") == 0
                            || columnName[i].compareTo("PopupName") == 0 || columnName[i].compareTo("File") == 0) {
                    ...
                    if (!isFileNameColumnAdded && columnHeaderOrderForFileName.indexOf(",File,") == -1 && !columnHeaderOrderForFileName.startsWith("File,")
                            && !columnHeaderOrderForFileName.endsWith(",File")) {
                ...
                while (resultMapListItr.hasNext()) {
                    ...
                    for (int i = 0; i < noOfColumns; i++) {
                        ...
                        if (selectTypes[i].compareTo(typeImage) == 0 || selectTypes[i].compareTo("icon") == 0 || selectTypes[i].compareTo("programHTMLOutput") == 0
                                || columnName[i].compareTo("PopupName") == 0) {
                    ...
                    if (tableName.equals("AEFCollection")) {
                        ...
                        if (busId != null && !busId.equals("")) {
                            ...
                            if (gco != null) {
                                ...
                                if (gco.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_CADMODEL_LIKE) || gco.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) {
                                    ...
                                    else if (gco.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_COMPONENT_LIKE)
                                            || gco.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_COMPONENT_FAMILY_LIKE))
                        ...
                        if (isCommonDocumentLike)// [SUPPORT]
                        {
                            ...
                            if (showFileNameColumn && !tableName.equals("APPFileVersions") && !tableName.equals("APPFileSummary")) {
                                String fileNameString = UINavigatorUtil.getI18nString("emxIEFDesignCenter.Common.MSDIFileName", UINavigatorUtil.getStringResourceFileId("IntegrationFramework"),
                                        language);
                            ...
                            // If PMC change the lookup for project then we need to change the following if condition. Look the file emxProgramCentralWBSSummaryFS.jsp to get how the "isProject"
        ...
        try {
                ...
                try {
                    ...
                    if ("$".equals(token)) {
                        ...
                        if (delimiter == null) {
                            ...
                            tokens.add(token);
                            types.add(type);
                            if (!(sOriginalToken.startsWith("type_") || sOriginalToken.startsWith("attribute_") || sOriginalToken.startsWith("relationship_") || sOriginalToken.startsWith("policy_")
            ...
            while (itr.hasNext()) {
                ...
                if (name.startsWith(SCHEMA)) {
                    value = (String) PropertyUtil.getSchemaProperty(context, name.substring(SCHEMA.length()));
                }
