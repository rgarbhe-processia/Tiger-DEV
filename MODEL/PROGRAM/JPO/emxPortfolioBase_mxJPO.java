...
public class emxPortfolioBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @exception Exception
     *                when unable to find object in the ProductCentral
     ...
     * @exception Exception
     *                when problems occurred in the ProductCentral
    ...
    public MapList getContents(Context context, String[] args) throws Exception {
        ...
        // Getting the related
        relBusObjPageList = this.getRelatedObjects(context, strRelName, WILD_CHAR, objectSelects, relSelects, false, true, sRecursionLevel, "", "");
    ...
    /**
     * This method is used to display the roadmap for selected Portfolios in the Portfolio List Page. It fecthes objectids of all the Products present under selected portfolios. If the selected
     * portfolio contains a model connected to it then ids of all the products present under that model are also returned.
     * @param context
     *            The ematrix context of the request.
     * @param args
     *            The string array containing following packed arguments: programMap - contains RequestValuesMap RequestValuesMap - cotains emxTableRowId of the selected portfolios.
     * @return MapList containing the object ids of the Products under selected portfolios.
     * @throws Exception
     * @since ProductCentral10.6
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public List getPortfolioProducts(Context context, String args[]) throws Exception {
        ...
        return getPortfolioProductIds(context, strPortfolioIds);
    }

    /**
     * This method returns all the Products connected to the Portfolios by "Portfolio" relationship. If portfolio contains any Model connected to it then all the Products connected to that Model by
     * "Products" relationship are also returned. Any duplicate Product is removed from the final MapList.
     * @param context
     *            The ematrix context of the request.
     * @param strPortfolioIds
     *            The string array containing the object ids of portfolios.
     * @return MapList containing object ids of the Products.
     * @since ProductCentral10.6
     */
    public List getPortfolioProductIds(Context context, String[] strPortfolioIds) throws Exception {

        DomainObject domPortfolio = new DomainObject();

        // Form the Relationship pattern.
        StringBuffer sbRelPattern = new StringBuffer();
        sbRelPattern.append(ProductLineConstants.RELATIONSHIP_PORTFOLIO);
        sbRelPattern.append(",");
        sbRelPattern.append(ProductLineConstants.RELATIONSHIP_PRODUCTS);
        ...
        List lstProductChildTypes = (StringList) ProductLineUtil.getChildrenTypes(context, ProductLineConstants.TYPE_PRODUCTS);
        ...
        for (int i = 0; i < strPortfolioIds.length; i++) {
            ...
            lstProducts = domPortfolio.getRelatedObjects(context, sbRelPattern.toString(), DomainConstants.QUERY_WILDCARD, false, true, (short) 0, (StringList) lstObjSelects, null, null, null, null,
                    sbPostPattern.toString(), null);

            // merge the Products list to remove duplicates
            mergeProductsLists((MapList) lstProductIds, (MapList) lstProducts);
        }
        return lstProductIds;
    }

    /**
     * This method is used to merge the two MapLists by removeing the duplicate Maps having same value for the key DomainConstants.SELECT_ID.
     * @param lstDestList
     *            The maplist to be merged.
     * @param lstSourceList
     *            The final merged maplist which will be returned.
     * @return merged MapList.
     * @since ProductCentral10.6
     */
    public void mergeProductsLists(MapList lstDestList, MapList lstSourceList) {
        ...
        for (int i = 0; i < lstSourceList.size(); i++) {
            ...
            for (int j = 0; j < lstDestList.size(); j++) {
                strDestProdId = (String) ((Map) lstDestList.get(j)).get(DomainConstants.SELECT_ID);
            ...
            if (!bProdExists) {
                ...
                mapProduct.put(DomainConstants.SELECT_ID, (String) mapSourceProd.get(DomainConstants.SELECT_ID));
    ...
    /**
     * This method is used to get all the ids of all the Products from the selected content amoung particular portfolio. If the selected content is model then all the Products present under that Model
     * are retriebed.
     * @param context
     *            The ematrix context object.
     * @param args
     *            String array containing the packed arguments.
     * @return List of product ids.
     * @throws Exception
     * @since ProductCentral10.6
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public List getPortfolioContentProducts(Context context, String[] args) throws Exception {
        // The packed argument send from the JPO invoke method is unpacked to
        // retrive the HashMap.
        Map programMap = (HashMap) JPO.unpackArgs(args);
        ...
        return getPortfolioContentProductIds(context, strContentIds);
    }
     ...
     *            The ematrix context object.
     * @param strContentIds
     *            String array containing selected content ids.
     * @return List of product ids.
     * @throws Exception
     * @since ProductCentral10.6
     */
    public List getPortfolioContentProductIds(Context context, String[] strContentIds) throws Exception {
        ...
        for (int i = 0; i < strContentIds.length; i++) {
            ...
            strContentType = domContent.getInfo(context, DomainConstants.SELECT_TYPE);

            if (!strContentType.equals(ProductLineConstants.TYPE_MODEL)) {
                mapProduct = new HashMap();
                ...
                lstProducts = domContent.getRelatedObjects(context, sbRelPattern.toString(), strTypePattern, (StringList) lstObjSelects, null, false, true, (short) 0, null, null, 0);
            }
