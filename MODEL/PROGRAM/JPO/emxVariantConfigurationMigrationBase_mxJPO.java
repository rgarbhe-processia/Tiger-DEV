...
/*
 * emxVariantConfigurationHF6MigrationBase.java program migrates the data into Common Document model i.e. to 10.5 schema.
 *
 ...
 * "$Id: /ENOVariantConfigurationBase/CNext/Modules/ENOVariantConfigurationBase/JPOsrc/base/emxVariantConfigurationHF6MigrationBase.java 1.1.1.1 Wed Oct 29 22:27:16 2008 GMT przemek Experimental$"
 */
 ...
 * @author pmogusala
 */
public class emxVariantConfigurationMigrationBase_mxJPO extends emxCommonMigration_mxJPO {

    protected static final String parentModelSelect = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "]." + "from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "]."
    ...
    protected static final String productPF_ID = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "]." + "from.to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.id";
    ...
    protected static final String MF_FeatureOrProduct_IDs = "from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "]" + ".to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
            + "].to.id";

    protected static final String MF_Feature_PFL_IDs = "from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "]" + ".to.from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO
            + "].to.from[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].id";
    ...
    protected static final String managedProdz = "from[" + ConfigurationConstants.RELATIONSHIP_MANAGED_MODEL + "].to.from[" + ConfigurationConstants.RELATIONSHIP_PRODUCTS + "].to.id";
     ...
     * @throws Exception
     */
    public emxVariantConfigurationMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    public void migrateObjects(Context context, StringList objectList) throws Exception {
        try {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                String hasManModelRel = (String) JPO.invoke(context, "emxDomainObject", null, "hasRelationship", arrJPOArgs, String.class);
    ...
    private String checkModelTemplate(Context context, String objID) throws Exception {
        ...
        return (String) JPO.invoke(context, "emxDomainObject", null, "hasRelationship", argsJPO, String.class);
    ...
    public boolean migrateProductRev(Context context, StringList objList) throws Exception {
        ...
        for (Iterator iterator = mapList.iterator(); iterator.hasNext();) {
            ...
            if (!(checkisconenctedtomodeltempalte.equals("true"))) {
                ...
                boolean isConencted = connectWithModelTemplate(context, (String) ((StringList) objectMAp.get(parentModelSelect)).get(0), ConfigurationConstants.RELATIONSHIP_MODEL_MASTER_FEATURES,
                        (String) objectMAp.get(managedModelSelect));

                if (isConencted) {
                    // check for rule before deleteing them
                    checkForRules(context, new StringList(mAsterFeature));
                    // mqlLogRequiredInformationWriter("ERROR: MAsterfeature ("+mAsterFeature+")has rules connected to it, as teh master feature is been deleted please updat ethe IR related to it
                    // "+newline);
                    ...
                    deleteLog.write(
                            (String) objectMAp.get(ConfigurationConstants.SELECT_ID) + newline + (String) objectMAp.get(productPF_ID) + newline + (String) objectMAp.get(MF_productPF_FLID) + newline);
                    deleteReport.write(
                            (String) objectMAp.get(ConfigurationConstants.SELECT_ID) + "," + (String) objectMAp.get(productPF_ID) + "," + (String) objectMAp.get(MF_productPF_FLID) + ", " + newline);
                    deleteLog.flush();
                    deleteReport.flush();

                    mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.success", language) + "( " + mAsterFeature + ") " + newline);

                } else {
                    mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.Failure", language) + "( " + mAsterFeature + ") " + newline);
                ...
                mqlLogRequiredInformationWriter(
                        MessageUtil.getMessage(context, null, "MasteFeature.migration.Migrated", mastefeaturenameArgs, null, context.getLocale(), "emxConfigurationStringResource") + newline);
                continue;
            }
            ...
            if (!updateFeatureAllocationTypeOnProducts(context, objectMAp)) {
                ...
                mqlLogRequiredInformationWriter(
                        EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.featureAllocatuopntype.error", language) + (String) objectMAp.get(managedModelSelect) + newline);
            }
        ...
        for (Iterator iterator = mapList.iterator(); iterator.hasNext();) {
            ...
            if (!(checkisconenctedtomodeltempalte.equals("true"))) {
                ...
                for (int k = 0; k < parentModelList.size(); k++) {
                    ...
                    isConnected = connectWithModelTemplate(context, strPArentModel, ConfigurationConstants.RELATIONSHIP_MODEL_MASTER_FEATURES, masterFeature);
            ...
            if (objectMAp.get(MF_FeatureOrProduct_IDs) != null) {
                ...
                if (fetaureRevIDs.size() > 0) {
                    String MarketingName = new DomainObject((String) fetaureRevIDs.get(fetaureRevIDs.size() - 1)).getInfo(context,
                            "attribute[" + ConfigurationConstants.ATTRIBUTE_MARKETING_NAME + "]");
                ...
                try {
                    ...
                    feature_relIDMAp = DomainRelationship.connect(context, new DomainObject(masterFeature), ConfigurationConstants.RELATIONSHIP_MANAGED_REVISION, true, fateureidsArray);
                } catch (Exception e) {
                    // TODO: handle exception
                ...
                if (isConnected && isConnectFeatRev) {
                    ...
                    mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.Failure", language) + "( " + masterFeature + ") " + newline);
                    writeUnconvertedOID(masterFeature);
                    continue;
                }
                ...
                if (!(updateFeatureAllocationTypeOnManagedSeries(context, feature_relIDMAp, (StringList) objectMAp.get(MF_FeatureOrProduct_FLIDs)))) {
                    mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.featureAllocatuopntype.ManagedSeries.error", language) + "( "
                            + masterFeature + ") " + newline);
                }
                // checking for rules
                checkForRules(context, (StringList) objectMAp.get(MF_FeatureOrProduct_FLIDs));
                // check and then migrating design effectivity
    ...
    private boolean checkDesignEffecivelyMatrix(Context context, StringList featureListIDs) throws Exception {
        ...
        try {
            ...
            for (Iterator iterator = featureListIDs.iterator(); iterator.hasNext();) {
                ...
                argsJPO[0] = objectID;
                String hasPFLRel = (String) JPO.invoke(context, "emxDomainObject", null, "hasRelationship", argsJPO, String.class);
                if ("true".equals(hasPFLRel)) {
                    ...
                    // write to Delete Log
                    mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.FeatureList.delete", language) + newline);
            ...
            successful = false;
            mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.checkDesignEffecivelyMatrix.ERROR", language) + e.toString() + newline);
        }
    ...
    private boolean checkForRules(Context context, StringList featureListIDs) throws Exception {
        ...
        for (Iterator iterator = featureListIDs.iterator(); iterator.hasNext();) {
            ...
            try {
                ...
                String hasRERel = (String) JPO.invoke(context, "emxDomainObject", null, "hasRelationship", argsJPO, String.class);

                // for Left Expression
                argsJPO[2] = ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION;
                String hasLERel = (String) JPO.invoke(context, "emxDomainObject", null, "hasRelationship", argsJPO, String.class);
                ...
                mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.checkForRules.ERROR", language) + e.toString() + newline);
            }
    ...
    private boolean migrateDesignEffecivelyMatrix(Context context, String strObjId) throws Exception {
        ...
        try {
            ...
            flObjArg[0] = strObjId;
            mqlLogWriter(MessageUtil.getMessage(context, null, "MasteFeature.migration.migrateDesignEffecivelyMatrix.start", flObjArg, null, context.getLocale(), "emxConfigurationStringResource")
                    + newline);
            ...
            if (productsMap.size() == 1) {
                ...
                mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.migrateDesignEffecivelyMatrix.debug1", language) + newline);

                productMigrating = (String) productsMap.keySet().toArray()[0];
                DomainRelationship.setFromObject(context, strFLF_RelId, new DomainObject((String) productsMap.keySet().toArray()[0]));
                ...
                for (int i = 0; i < arrayPrdKey.length; i++) {
                    ...
                    if (i == 0) {
                        ...
                        DomainRelationship.setFromObject(context, strFLF_RelId, prdObject);
                        ...
                        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.migrateDesignEffecivelyMatrix.debug2", language) + newline);
                        DomainObject domFLObj = new DomainObject(strObjId);

                        String strObjectGeneratorName = FrameworkUtil.getAliasForAdmin(context, DomainConstants.SELECT_TYPE, ConfigurationConstants.TYPE_FEATURE_LIST, true);
                        ...
                        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.migrateDesignEffecivelyMatrix.debug3", language) + newline);
                        newClonedFL.connect(context, new RelationshipType(ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM), false, prdObject);
                        // connecting with ProductFeatureListRelatiobship
                        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.migrateDesignEffecivelyMatrix.debug4", language) + newline);
                        newClonedFL.connect(context, new RelationshipType(ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST), false, prdObject);
                        ...
                        /*
                         * newClonedFL.connect(context, new RelationshipType(ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO), true, new DomainObject(strF_Id));
                         */
                        ...
                        if (productVariantMap.size() > 0) {
                            ...
                            for (int j = 0; j < prodcutVariants.size(); j++) {
                                if (productVariantMap.keySet().contains((String) prodcutVariants.get(j))) {
                                    newClonedFL.connect(context, new RelationshipType(ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST), false,
            ...
            successful = false;
            mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.migrateDesignEffecivelyMatrix.error", language) + "(" + productMigrating + ")"
                    + e.toString() + newline);
        }
    ...
    private boolean connectMigrateProductConfiguration(Context context, DomainObject domFLObj, String productid, DomainObject newClonedFL) throws Exception {
        ...
        try {
            ...
            MapList relSO_ID_Maplist = domFLObj.getRelatedObjects(context, relTypePatterns, objTypePatterns, true, false, 1, objectSelect, relSelect, objectWhereCond,
                    ConfigurationConstants.EMPTY_STRING, 0, null, null, null);

            mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.ProductConfiguration", language) + newline);

            for (int i = 0; i < relSO_ID_Maplist.size(); i++) {
                Map relSO_IDMap = (Hashtable) relSO_ID_Maplist.get(0);
                DomainRelationship.setToObject(context, (String) relSO_IDMap.get(SELECT_RELATIONSHIP_ID), newClonedFL);
            }
            ...
            mqlLogRequiredInformationWriter(
                    MessageUtil.getMessage(context, null, "MasteFeature.migration.ProductConfiguration.error", pcArgs, null, context.getLocale(), "emxConfigurationStringResource") + e.toString()
                            + newline);
        }
    ...
    /**
     * This method updates 4 attributes on Feature List when ever the new featrure List from relationship is created The attribiurtes are: parent object name,parent marketing name,child object name
     * ,child marketing name
     */
    private void updateFeatureListObjects(Context context, String strObjectId, String strFeatureListId, String strDirection) throws Exception {
        ...
        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.featuelist.updateAttribute", language) + strFeatureListId + newline);

        // selects
        StringList lstSelect = new StringList("attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]");
        ...
        if (strDirection.equals("FROM")) {
            attvaluesMap.put(ConfigurationConstants.ATTRIBUTE_PARENT_OBJECT_NAME, (String) mapTemp.get(DomainConstants.SELECT_NAME));
            attvaluesMap.put(ConfigurationConstants.ATTRIBUTE_PARENT_MARKETING_NAME, (String) mapTemp.get("attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]"));

        } else {
            attvaluesMap.put(ConfigurationConstants.ATTRIBUTE_CHILD_OBJECT_NAME, (String) mapTemp.get(DomainConstants.SELECT_NAME));
            attvaluesMap.put(ConfigurationConstants.ATTRIBUTE_CHILD_MARKETING_NAME, (String) mapTemp.get("attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]"));
        ...
        try {
            ...
            e.printStackTrace();
            mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.featuelist.updateAttribute.error", language) + strFeatureListId + "   " + e.toString() + newline);
        }
    ...
    private boolean updateFeatureAllocationTypeOnManagedSeries(Context context, Map feature_relIDMAp, StringList FLIDs) throws Exception {
        ...
        try {
            ...
            for (Iterator iterator = relData.iterator(); iterator.hasNext();) {
                ...
                if (strDefaultSelection.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_NO) && strMandatory.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_NO)) {
                    strFeatureAllocationTypeAttrValue = ConfigurationConstants.RANGE_VALUE_OPTIONAL;
                }

                new DomainRelationship((String) feature_relIDMAp.get(strFeatureID)).setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE,
                ...
                mqlLogWriter(
                        MessageUtil.getMessage(context, null, "MasteFeature.migration.FeatureAllocationTypeOnManagedSeries.info", flObjArg, null, context.getLocale(), "emxConfigurationStringResource")
                                + newline);
    ...
    private boolean updateSeriesCount(Context context, StringList contextObjIds) {
        ...
        try {
            ...
            for (Iterator ctxtObjIdIttr = contextObjIds.iterator(); ctxtObjIdIttr.hasNext();) {
                ...
                MapList relatedObjectData = ctxObj.getRelatedObjects(context, strRelPattern, strTypePattern, strObjecSelect, strRelSelect, false, true, (short) 1, null, null, 0);
                ...
                if (relatedObjectData.size() != seriesCntVal) {
                    ...
                    mqlLogWriter(MessageUtil.getMessage(context, null, "SeriesCount.migration.update.log", flObjArg, null, context.getLocale(), "emxConfigurationStringResource") + newline);
                }
            }
        ...
        try {
            ...
            for (Iterator relDataIttr = relData.iterator(); relDataIttr.hasNext();) {
                ...
                if (relName.equals(ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST)) {
                    String strFeaAllocTypeVal = (String) relDataMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE + "]");
                    // Added condition IR-040613V6R2011
                    if (strFeaAllocTypeVal.equals(ConfigurationConstants.RANGE_VALUE_MANDATORY) || strFeaAllocTypeVal.equals("")) {
                        domRelObj.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE, ConfigurationConstants.RANGE_VALUE_STANDARD);
                        // logging
                        String[] flObjArg = new String[1];
                        flObjArg[0] = relId;
                        mqlLogWriter(
                                MessageUtil.getMessage(context, null, "FeatureAllocationType.migration.update.log", flObjArg, null, context.getLocale(), "emxConfigurationStringResource") + newline);
                    }
    ...
    private boolean updateFeatureAllocationTypeOnProducts(Context context, Map MasterFeaMap) throws Exception {
        ...
        try {
            ...
            for (Iterator ittrPrd = strLstPrdId.iterator(); ittrPrd.hasNext();) {
                ...
                MapList relFLObj = domFromObj.getRelatedObjects(context, relTypePatterns, objTypePatterns, false, true, 1, objectSelect, ConfigurationConstants.EMPTY_STRINGLIST, objectWhereCond,
                        ConfigurationConstants.EMPTY_STRING, 0, null, null, null);
                if (relFLObj.size() > 0) {
                    ...
                    DomainRelationship domRelObj = new DomainRelationship(
                            (String) feaLisDataMap.get("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.to[" + ConfigurationConstants.RELATIONSHIP_PRODUCTS + "].id"));

                    domRelObj.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE, strFeatureAllocationTypeAttrValue);
                    // logging
                    String[] flObjArg = new String[2];
                    flObjArg[0] = strFeatureAllocationTypeAttrValue;
                    flObjArg[1] = domRelObj.getName();
                    mqlLogWriter(
                            MessageUtil.getMessage(context, null, "MAsterfeature.FeatureAllocationType.migration.update.log", flObjArg, null, context.getLocale(), "emxConfigurationStringResource")
                                    + newline);
                }
    ...
    private boolean processMFForRuleMigration(Context context) throws Exception {
        ...
        try {
            mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MAsterFeature.processMFForRuleMigration.start", language) + newline);
            ...
            for (Iterator FLIdIttr = objectIds.iterator(); FLIdIttr.hasNext();) {
                ...
                if (frmType.equals(ConfigurationConstants.TYPE_PRODUCT_PLATFORM) && toType.equals(ConfigurationConstants.TYPE_MASTER_FEATURE)) {
                    ...
                    if (toType.equals(ConfigurationConstants.TYPE_PRODUCTS)) {
                        ...
                        mqlLogRequiredInformationWriter(
                                MessageUtil.getMessage(context, null, "MasteFeature.migration.ruleMigration.error", flObjArg, null, context.getLocale(), "emxConfigurationStringResource") + newline);
                    }
                    ...
                    for (Iterator strExpRelIttr = strREnLERelIds.iterator(); strExpRelIttr.hasNext();) {
                        String strExpRelId = (String) strExpRelIttr.next();
                        // changing the toside of R.E/L.E
                        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MAsterFeature.processMFForRuleMigration.debug1", language) + newline);
                        ...
                        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MAsterFeature.processMFForRuleMigration.debug.update", language) + strRuleId + newline);
                        ...
                        if (strLEObjType.contains(ConfigurationConstants.TYPE_GBOM) || strREObjType.contains(ConfigurationConstants.TYPE_GBOM)) {

                            objectSelect.clear();
                            objectSelect.add("to[" + ConfigurationConstants.RELATIONSHIP_GBOM_FROM + "].from.id");

                            MapList objectData = domRuleObj.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "," + ConfigurationConstants.RELATIONSHIP_RIGHT_EXPRESSION,
                                    ConfigurationConstants.TYPE_GBOM, false, true, 1, objectSelect, ConfigurationConstants.EMPTY_STRINGLIST, ConfigurationConstants.EMPTY_STRING,
                                    ConfigurationConstants.EMPTY_STRING, 0, null, null, null);
                            for (Iterator ittrGBOM = objectData.iterator(); ittrGBOM.hasNext();) {
                                ...
                                mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MAsterFeature.processMFForRuleMigration.updateDuplicatexml", language) + strFeatureId + newline);
                                ...
                                mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MAsterFeature.processMFForRuleMigration.EquipmentListReportXML", language) + strFeatureId + newline);
                            }
                        }
                    }
                }
            }
            mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MAsterFeature.processMFForRuleMigration.end", language) + newline);
        } catch (Exception e) {
            successful = false;
    ...
    private boolean customMasterFeatureMigration(Context context) throws Exception {
        boolean successful = true;
        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.CustomMigration.start", language) + newline);
        ...
        if (cmdMap == null || "null".equals(cmdMap) || cmdMap.size() <= 0) {
            mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.CustomMigration.command.error", language) + newline);
            return false;
        }
            ...
            for (int i = 0; i < keyValues.length; i++) {
                ...
                if (strkey.startsWith("MigrateJPO")) {
                    ...
                    if (tokens.hasMoreTokens()) {
                        ...
                        mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.CustomMigration.pattern.error", language) + newline);
                        return false;
                    }
                    ...
                    try {
                        ...
                        mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.CustomMigration.inovkingJPO.error", language) + newline);
        ...
        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.CustomMigration.end", language) + newline);
        return successful;
    ...
    private boolean DeleteObsoleteandMigratedObject(Context context) throws Exception {
        boolean successful = true;
        String objectId = "";
        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.Deleteobjects.start", language) + newline);
        ...
        try {
            ...
            while ((objectId = fileReader.readLine()) != null) {
                ...
                if (objectId != null && !objectId.equals("") && !"null".equals(objectId)) {
                    ...
                    if (dmObj.exists(context)) {
                        ...
                        mqlLogWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.Deleteobjects", language) + " " + objectId + newline);
                    }
                }
            ...
            successful = false;
            mqlLogRequiredInformationWriter(EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.Deleteobjects.error", language) + e.toString() + newline);
        }
        mqlLogWriter(newline + EnoviaResourceBundle.getProperty(context, SUITE_KEY, "MasteFeature.migration.Deleteobjects.end", language) + newline);
        return successful;
    }
