 ...
 *
 * Copyright (c) 2004-2015 Dassault Systemes. All Rights Reserved. This program contains proprietary and trade secret information of MatrixOne, Inc. Copyright notice is precautionary only and does not
 * evidence any actual or intended publication of such program.
 *
 * static const char RCSID[] = $Id: /java/JPOsrc/base/${CLASSNAME}.java 1.53.2.4.1.1.1.3.1.1 Wed Jan 07 13:05:24 2009 GMT ds-shbehera Experimental${CLASSNAME}.java 1.19 Thu Nov 01 14:40:05 2007 GMT
 * ds-dpathak Experimental${CLASSNAME}.java 1.11 Fri Oct 12 11:29:34 2007 GMT ds-dpathak Experimental${CLASSNAME}.java 1.4 Thu Aug 23 07:55:30 2007 GMT ds-pvoggu Experimental$
 *
 */
 ...
 */
public class emxProductConfigurationEBOMBase_mxJPO extends emxDomainObject_mxJPO implements BackgroundJob {
    ...
    public static String attrDefaultPartType = "attribute[" + PropertyUtil.getSchemaProperty("attribute_DefaultPartType") + "]";

    public static String attrDefaultPartPolicy = "attribute[" + PropertyUtil.getSchemaProperty("attribute_DefaultPartPolicy") + "]";
    ...
    public emxProductConfigurationEBOMBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    /**
     * This method is called to avoid the rule evaluation when there is any inactive design variant in the rule expression. This situation arises only in case of Product Variant.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strGBOMID
     *            the GBOM id <code>String</code> object
     * @param errorHolder
     *            the ProductConfigurationHolder <code>ProductConfigurationHolder</code> object
     * @param strGBOMID
     *            the rule expression <code>String</code> object
     * @return Returns a <code>String</code> object. The return object is true or false string based on the evaluation.
     * @throws Exception
     *             if the operation fails modified access specifier for 377168
     * @since PRC 10.5.1.2 this could not be deprecated as referenced from migration JPO
     */
    public String checkDesignVariantsValidity(Context context, String strGBOMID, String strContextId) throws Exception {
        DomainObject domGbom = new DomainObject(strGBOMID);
        // added for the bug 377168 - start
        StringList objSelects = new StringList();
        objSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_GBOM_FROM + "].from.id");
        objSelects.addElement("to[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS + "].value");

        Map objSelectsMap = (Map) domGbom.getInfo(context, objSelects);
        /*
         * String strFeatureId = domGbom.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_GBOM_FROM + "].from.id"); String strAtrDesignVariants = domGbom.getInfo(context, "to[" +
         * ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS + "].value");
         */
        String strFeatureId = (String) objSelectsMap.get("to[" + ConfigurationConstants.RELATIONSHIP_GBOM_FROM + "].from.id");
        String strAtrDesignVariants = (String) objSelectsMap
                .get("to[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS + "].value");
        ...
        slRelSelects.add("attribute[" + ConfigurationConstants.ATTRIBUTE_INACTIVE_CONTEXTS + "]");
        slRelSelects.add("attribute[" + ConfigurationConstants.ATTRIBUTE_EFFECTIVITY_STATUS + "]");
        ...
        /*
         * StringList lstFeaturesChildren = ProductLineUtil.getChildrenTypes( context, ProductLineConstants.TYPE_FEATURES);
         * 
         * StringBuffer sbChildren = new StringBuffer(); for (int ctr = 0; ctr < lstFeaturesChildren.size(); ctr++) { sbChildren.append(lstFeaturesChildren.get(ctr)); sbChildren.append(","); } String
         * strTypePattern = sbChildren.toString();
         */

        MapList dvMapList = domFeature.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_VARIES_BY, ConfigurationConstants.TYPE_FEATURES, objSelects, slRelSelects, false, true, (short) 1,
                null, sbDVRel.toString(), 0);

        /*
         * MapList dvMapList = domFeature.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_VARIES_BY, "*", false, true, (short) 1, null, slRelSelects, null, sbDVRel.toString(), null,
         * strTypePattern, null);
         */
        ...
        for (int i = 0; i < invalidDVList.size(); i++) {
            ...
            selectable.append("tomid[").append(ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY).append("].id");
            ...
            if (strIVBRelIds != null && !(strIVBRelIds.equals("")) && !("null".equalsIgnoreCase(strIVBRelIds)))

            {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    String strProdFeatListRelId = MqlUtil.mqlCommand(context, strMqlCmd, true, strIVBRelId, strFromRelSelect);
                    strProdFeatListRelId = strProdFeatListRelId.trim();

                    strMqlCmd = "print connection $1 select $2 dump";
                    String strFromIdSelect = "from.id ";
                    String strPVId = MqlUtil.mqlCommand(context, strMqlCmd, true, strProdFeatListRelId, strFromIdSelect);
    ...
    /**
     * This method is called from getEvaluatedListsIdBased().It takes all the parts to evaluate and returns final part object id's
     * @param context
    ...
    protected ArrayList gbomParseRulesIdBased(Context context, ArrayList objectsToEvaluate, String calledFrom, boolean isVariantExists, boolean isForPart) throws Exception {
        ...
        MapList selectedOptionsMaplist = emxBatchRuleProcessing_mxJPO.getSelectedOptions(context, getId(context));
        Iterator it = selectedOptionsMaplist.iterator();
        while (it.hasNext()) {
            selectedOptions.put(((Map) it.next()).get(ConfigurationConstants.SELECT_ID), "");
        }
        ...
        if (mxType.isOfParentType(context, strContextType, ConfigurationConstants.TYPE_FEATURES)) {
            strProdContextId = domPC.getInfo(context, "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_PRODUCT_CONFIGURATION + "].from.id");
        ...
        if (objectsToEvaluate != null && objectsToEvaluate.size() > 0) {
            for (int count = 0; count < objectsToEvaluate.size(); count++) {
                ...
                if (strExpression == null || strExpression.length() <= 0) {
                    ...
                    int DVCount1 = 0;
                    DomainObject domGBOM = new DomainObject(strGBOMID);
                    String strSelectExpress = "to[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from." + "attribute[" + ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS + "]";
                    ...
                    if (ruleEvaluationRequired) {
                        if (strContextType.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_VARIANT) && isVariantExists && isForPart && calledFrom.equalsIgnoreCase("Part")) {
                            strExpResult = checkDesignVariantsValidity(context, strGBOMID, strContextId);
                ...
                if (ruleEvaluationRequired) {
                    ...
                    if (strUseDesignVariant != null && !strUseDesignVariant.equals("true")) {
                        if (strAttrRExp != null) {
                            ...
                            // part to the "Object"
                            //
                            strExpResult = emxBatchRuleProcessing_mxJPO.calculateExpressionIdBased(context, strGBOMID, strExpression, selectedOptions);
                    ...
                    /*
                     * strExpResult = ${CLASS:emxBatchRuleProcessing} .calculateExpressionIdBased(context, strGBOMID, strExpression, selectedOptions);
                     */
                } else {
                    strExpResult = STR_FALSE;
                }
     ...
     */
    public String getExpressionForQuantityRule(Context context, String strObjectID) throws Exception {
        ...
        strRightExpressionList = (StringList) (JPO.invoke(context, "emxBooleanCompatibility", null, "getRightExpressionforQuantityEdit", arrJPOArguments, StringList.class));
    ...
    public MapList getEBOMWithSelects(Context context, String[] args) throws Exception {
        ...
        if ((ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION).equalsIgnoreCase(productConfigObject.getInfo(context, ConfigurationConstants.SELECT_TYPE))) {
            strTopLevelObject = productConfigObject.getInfo(context, "from[" + ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART + "].to.id");
            if (strTopLevelObject != null && strTopLevelObject.length() > 0) {
                try {
                    ebomList = productConfigObject.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART, // relationship
                            // pattern
            ...
            try {
                ...
                ebomList = productConfigObject.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_EBOM, // relationship
                        // pattern
                        ConfigurationConstants.TYPE_PART, // object pattern
                        selectStmts, // object selects
                        selectRelStmts, // relationship selects
    ...
    /**
     * This method is called from updateBomXmlMigration(). It returns the list of parts which evaluates to true as a result of the IR/ER evaluation which is done based on Id comparison & not TNR
     * against selected options
     * @param context
     ...
     *             this could not be deprecated as referenced from migration JPO
     */
    protected HashMap getEvaluatedListsIdBased(Context context, ArrayList arrPartObjectIds, boolean isVariantExists) throws Exception {
        ...
        for (int l = 0; l < arrPartObjectIds.size(); l++) {
            ...
            for (int mapListCount = 0; mapListCount < mapListProductPartId.size(); mapListCount++) {
                Map mapProductPartId = (Map) mapListProductPartId.get(mapListCount);

                String type = (String) mapProductPartId.get("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to." + ConfigurationConstants.SELECT_TYPE);
                String partId = (String) mapProductPartId.get("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to." + ConfigurationConstants.SELECT_ID);

                String gbomObjectId = (String) mapProductPartId.get(ConfigurationConstants.SELECT_ID);
                String ruleType = (String) mapProductPartId.get(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + EXPRESSION_CLOSE);
                // added for Bug No. 376243
                ...
                } else if (isOfParentType(context, type, ConfigurationConstants.TYPE_PART)) {
        ...
        /*
         * arrProductPartId = gbomParseRules(context, arrPartsToEvaluate, errorHolder, "Part", isVariantExists, true);
         */

        arrProductPartId = gbomParseRulesIdBased(context, arrPartsToEvaluate, "Part", isVariantExists, true);
        ...
        if (isVariantExists) {
            ...
            arrProductPartFamilyId = gbomParseRulesIdBased(context, arrPartFamilysToEvaluate, "Part", isVariantExists, false);
        }
    ...
    /**
     * This method is used for the updation of the BOM XML attribute on product configuration
     * @param context
     *            the eMatrix Context object
     * @param args
     *            holds the Product configuration id
     * @throws Exception
     *             if the operation fails this could not be deprecated as referenced from migration JPO
     */
    public String updateBomXmlMigration(Context context, String[] args) throws Exception {
        return "";
    }

    /**
     * This method is used by the background job (which gets fired on done of PC to update the BOM XML attribute on product configuration
     * @param context
     *            the eMatrix Context object
     * @param args
     *            holds the Product configuration id
    ...
    /**
     * This method expands featues from Product Configuration which will be displayed in Preview BOM dialog.
     * @param context
     *            the eMatrix Context object
     * @param args
     *            holds the following input arguments: paramMap a HashMap requestMap a HashMap
     * @return MapList Object which holds following parameter id of feature evaluatedParts a MapList of evaluated parts and part family
     * @throws Exception
     *             if the operation fails
     * @author KXB
     * @since R212
    ...
    public Vector getInclusionRuleLinkForDisplay(Context context, String args[]) throws Exception {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (objectMap.get(ConfigurationConstants.SELECT_LEVEL).toString().equals("0")) {
                ...
                sIncLink = sIncLink.append("<a href=\"javascript:showDialog('").append("../configuration/GBOMRuleExpressionDialog.jsp?")
                        .append("modetype=createnull&amp;PCID=" + XSSUtil.encodeForHTMLAttribute(context, strPCID)).append("&amp;LFId=").append(XSSUtil.encodeForHTMLAttribute(context, strObjectId))
                        .append("&amp;PartRelId=").append(XSSUtil.encodeForHTMLAttribute(context, strGBOMRelID));
        ...
        try {
            ...
            if (isOnlyProductConfiguration) {
                ...
                if (ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION.equalsIgnoreCase(objID.getInfo(context, ConfigurationConstants.SELECT_TYPE))) {
                    String strTopLevelObject = objID.getInfo(context, "from[" + ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART + "].to.id");
                    if (strTopLevelObject != null && strTopLevelObject.length() > 0) {
                        String strTopLevelObjectName = objID.getInfo(context, "from[" + ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART + "].to.name");
                        if (exportToExcel) {
    ...
    public Vector getActionIconsForDisplay(Context context, String args[]) throws Exception {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (evaluatedParts != null && parts.size() == 0) {
                if (isECInstalled) {
                    stbNameRev = stbNameRev.append("<a><img src=\"../common/images/iconActionCreateNewPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18CreateNew)
                            .append("\"")
                            .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                    + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "&amp;isQuantityInvalid=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isQuantityInvalid))
                                    + "&amp;LFParentID=" + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf)
                                    + "&amp;ForcePartReuse=" + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;displayGenerateIcon="
                                    + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(displayGenerateIcon)) + "&amp;isUsedInBOM="
                                    + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM)) + "&amp;DuplicateParts=" + Duplicateparts + "&amp;mode=createpart" + "&amp;duplicate=false"
                                    + "&amp;parentId=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;generate=false" + "');\"")
                            .append("/></a>");
                }
                stbNameRev = stbNameRev.append(" <a><img src=\"../common/images/iconActionAddExistingPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18AddExisting)
                        .append("\"")
                        .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "&amp;isQuantityInvalid=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isQuantityInvalid))
                                + "&amp;displayGenerateIcon=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(displayGenerateIcon)) + "&amp;LFParentID="
                                + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;ForcePartReuse=" + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;DuplicateParts="
                                + Duplicateparts + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf) + "&amp;isUsedInBOM="
                                + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM)) + "&amp;mode=addexisting" + "&amp;duplicate=false" + "&amp;parentId="
                                + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;generate=false" + "');\"")
                        .append("/></a>");
                if (partFamily.size() == 1) {
            ...
            if (parts.size() > 0) {
                ...
                if (isECInstalled) {
                    stbNameRev = stbNameRev.append("<a><img src=\"../common/images/iconActionReplaceWithNewPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                            .append(i18ReplabeByCreateNew).append("\"")
                            .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                    + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "&amp;isQuantityInvalid=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isQuantityInvalid))
                                    + "&amp;LFParentID=" + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf)
                                    + "&amp;ForcePartReuse=" + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;displayGenerateIcon="
                                    + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(displayGenerateIcon)) + "&amp;isUsedInBOM="
                                    + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM)) + "&amp;DuplicateParts=" + Duplicateparts + "&amp;mode=replacebycreatepart"
                                    + "&amp;duplicate=false" + "&amp;parentId=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;ResolvedPart="
                                    + XSSUtil.encodeForHTMLAttribute(context, strResolvedPart) + "&amp;ResolvedPartRelID=" + XSSUtil.encodeForHTMLAttribute(context, strGBOMRelID)
                                    + "&amp;generate=false" + "');\"")
                            .append("/></a>");
                }
                stbNameRev = stbNameRev.append(" <a><img src=\"../common/images/iconActionReplaceWithExistingPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                        .append(i18ReplabeByAddExisting).append("\"")
                        .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "&amp;isQuantityInvalid=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isQuantityInvalid))
                                + "&amp;LFParentID=" + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf)
                                + "&amp;ForcePartReuse=" + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;DuplicateParts=" + Duplicateparts + "&amp;displayGenerateIcon="
                                + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(displayGenerateIcon)) + "&amp;isUsedInBOM="
                                + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM)) + "&amp;mode=replacebyaddexisting" + "&amp;duplicate=false" + "&amp;parentId="
                                + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;ResolvedPart=" + XSSUtil.encodeForHTMLAttribute(context, strResolvedPart) + "&amp;ResolvedPartRelID="
                                + XSSUtil.encodeForHTMLAttribute(context, strGBOMRelID) + "&amp;generate=false" + "');\"")
                        .append("/></a>");
                ...
                if (!("".equals(strTLPId) || "null".equals(strTLPId) || null == strTLPId)) {
                    ...
                    if (hasEBOMStructureTo) {
                        ...
                        if (hasEBOMStructureFrom) {
                            ...
                            relatedParts = confUtil.getObjectStructure(context, strTypePattern, strRelPattern, slObjSelects, slRelSelects, false, true, (short) 0, 0, "", "", (short) 0, "");
                    ...
                    if (isBOMDifferent) {
                        ...
                        stbTLPVisualCue = stbTLPVisualCue.append("<img src=\"../common/images/iconStatusValidationError.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                                .append(i18EBOMDifferent).append("\"").append("/>");
                ...
                for (int i = 0; i < objectList.size(); i++) {
                    ...
                    if (evaluatedParts != null) {
                        ...
                        if (parts.size() >= 1) {
                            ...
                            stbVisualCue = stbVisualCue.append("<a><img src=\"../common/images/iconSmallHigherRevision.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")

                                    .append(i18ToolTip).append("\"")
                                    .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + strObjectId + "&amp;displayGenerateIcon=" + displayGenerateIcon
                                            + "&amp;level=" + strObjectLevel + "&amp;ResolvedPart=" + strResolvedPart + "&amp;ResolvedPartRelID=" + strGBOMRelID + "&amp;LFParentID=" + strLFParentId
                                            + "&amp;isLFLeaf=" + strLFisLeaf + "&amp;ForcePartReuse=" + strForcePartReuse + "&amp;isQuantityInvalid=" + isQuantityInvalid + "&amp;mode=duplicate"
                                            + "&amp;duplicate=true" + "&amp;parentId=" + parentId + "');\"")
                                    .append("/></a>");
                        ...
                        if (strResolvedPart != null) {
                            ...
                            if (hasEBOMStructureTo) {
                                if (pc.checkForEBOMDifferent(context, strResolvedPart, strObjectId, parentId)) {
                                    stbVisualCue = stbVisualCue.append("<img src=\"../common/images/iconStatusValidationError.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                                            .append(i18EBOMDifferent).append("\"").append("/>");
                                }
                            }
                            ...
                            if (!isLastRev) {
                                stbVisualCue = stbVisualCue.append("<img src=\"../common/images/iconSmallHigherRevision.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                                        .append(i18HigherRevision).append("\"").append("/>");
                            }
                            // ***************OBSOLETE PART CHECK***************
                            if ((current).equalsIgnoreCase(strObsolete)) {
                                stbVisualCue = stbVisualCue.append("<img src=\"../common/images/iconStatusError.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                                        .append(i18ObsoletePart).append("\"").append("/>");
                            }
                        }
                        // ***************USED IN BOM CHECK***************
                        if (pc.checkForUsedInBOM(context, strObjectId, strResolvedPart, strLFParentId, parentId)) {
                            stbVisualCue = stbVisualCue.append("<img src=\"../common/images/iconStatusUsedInEBOM.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                                    .append(i18UsedInBOM).append("\"").append("/>");
                        }
                        // ******************Check for LF obsolete state********************
                        if (strLFState.equals(strLFObsolete)) {
                            stbVisualCue = stbVisualCue.append("<img src=\"../common/images/iconStatusError.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18ObsoleteLF)
                                    .append("\"").append("/>");
                        }
    ...
    public Vector getPartRevisionForDisplay(Context context, String[] args) throws Exception {
        ...
        if (objListIDs.size() > 0) {
            ...
            mapList = DomainObject.getInfo(context, (String[]) objListIDs.toArray(oidsArray), new StringList(ConfigurationConstants.SELECT_REVISION));
        }
    ...
    public Vector getPartStateForDisplay(Context context, String[] args) throws Exception {
        ...
        if (objListIDs.size() > 0) {
            String[] oidsArray = new String[objListIDs.size()];
            mapList = DomainObject.getInfo(context, (String[]) objListIDs.toArray(oidsArray), new StringList(ConfigurationConstants.SELECT_CURRENT));
        }
        Iterator itr = mapList.iterator();
        Map tempMap;
        for (int j = 0; j < objectList.size(); j++) {
    ...
    public Vector getPartTypeForDisplay(Context context, String[] args) throws Exception {
        ...
        if (objListIDs.size() > 0) {
            String[] oidsArray = new String[objListIDs.size()];
            mapList = DomainObject.getInfo(context, (String[]) objListIDs.toArray(oidsArray), new StringList(ConfigurationConstants.SELECT_TYPE));
        }
        Iterator itr = mapList.iterator();
        Map tempMap;
        for (int j = 0; j < objectList.size(); j++) {
    ...
    public Vector getUOMForDisplay(Context context, String[] args) throws Exception {
        ...
        if (objListIDs.size() > 0) {
            String[] oidsArray = new String[objListIDs.size()];
            mapList = DomainObject.getInfo(context, (String[]) objListIDs.toArray(oidsArray), new StringList("attribute[" + ConfigurationConstants.ATTRIBUTE_UNIT_OF_MEASURE + "]"));
        }
        Iterator itr = mapList.iterator();
        Map tempMap;
        for (int j = 0; j < objectList.size(); j++) {
            ...
            if (b_isAGap) {
                ...
                tempMap = (Map) itr.next();
                vUOM.addElement((String) tempMap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_UNIT_OF_MEASURE + "]"));
            } else {
                vUOM.addElement("");
            }
        }
    ...
    /**
     * This utility method returns MapList which contains featureId, PartId, and quantity as key.
     * @param strResolvedParts
     *            holds the value of Resolved Parts attribute of product configuration
    ...
    public MapList getAllEBOMWithSelects(Context context, String[] args) throws Exception {
        ...
        if (mxType.isOfParentType(context, strCtxtObjType, ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION)) {
            strTopLevelObject = domObject.getInfo(context, "from[" + ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART + "].to.id");
            if (strTopLevelObject != null && strTopLevelObject.length() > 0) {
                try {
                    confUtil = new ConfigurationUtil(productConfigId);
                    ebomList = (MapList) confUtil.getObjectStructure(context, ConfigurationConstants.TYPE_PART, ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART, objectSelects, relSelects, false,
                            true, recurseLevel, 0, ConfigurationConstants.EMPTY_STRING, DomainObject.EMPTY_STRING, (short) 0, "");
                } catch (FrameworkException Ex) {
                    throw Ex;
                }
            }
            ...
            try {
                confUtil = new ConfigurationUtil(productConfigId);
                ebomList = (MapList) confUtil.getObjectStructure(context, ConfigurationConstants.TYPE_PART, ConfigurationConstants.RELATIONSHIP_EBOM, objectSelects, relSelects, false, true,
                        recurseLevel, 0, ConfigurationConstants.EMPTY_STRING, DomainObject.EMPTY_STRING, (short) 0, "");
            } catch (FrameworkException Ex) {
                throw Ex;
            }
        }
     ...
     *             this could not be deprecated as referenced from migration JPO
     */
    public MapList getProductVariantsTechnicalStructre(Context context, String strProductID, String strProductVariantID) throws Exception {
        MapList mlFinalTechStructure = new MapList();
        DomainObject productDomObject = new DomainObject(strProductID);
        String strType = productDomObject.getInfo(context, DomainObject.SELECT_TYPE);

        StringList lstPVObjectSelects = new StringList(ConfigurationConstants.SELECT_ID);

        String strRelType = ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "," + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO;

        StringList selectStmts = new StringList();

        selectStmts.addElement("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + ConfigurationConstants.SELECT_ID);
        selectStmts.addElement("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + ConfigurationConstants.SELECT_NAME);
        selectStmts.addElement("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + ConfigurationConstants.SELECT_TYPE);
        selectStmts.addElement("from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + ConfigurationConstants.SELECT_REVISION);
        selectStmts.addElement(ConfigurationConstants.SELECT_ID);
        selectStmts.addElement(ConfigurationConstants.SELECT_TYPE);
        selectStmts.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + EXPRESSION_CLOSE);
        selectStmts.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_FEATURE_TYPE + EXPRESSION_CLOSE);
        selectStmts.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_FORCE_PART_REUSE + EXPRESSION_CLOSE);
        selectStmts.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_USAGE + EXPRESSION_CLOSE);
        selectStmts.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_QUANTITY + EXPRESSION_CLOSE);
        ...
        StringList lstChildren = ProductLineUtil.getChildrenTypes(context, ConfigurationConstants.TYPE_FEATURES);
        ...
        lstChildren = ProductLineUtil.getChildrenTypes(context, ConfigurationConstants.TYPE_PRODUCTS);
        for (int i = 0; i < lstChildren.size(); i++) {
            sbWhereClause.append("|| (");
            sbWhereClause.append("type== \"");
            sbWhereClause.append((String) lstChildren.elementAt(i));
        ...
        try {
            if (!strType.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_LINE)) {
                if (strProductVariantID != null && !("null".equals(strProductVariantID)) && !("".equals(strProductVariantID))) {
                    DomainObject pvDomObject = new DomainObject(strProductVariantID);
                    String strTypeTemp = pvDomObject.getInfo(context, DomainObject.SELECT_TYPE);

                    if (strTypeTemp.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_VARIANT) && ((!strType.equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_VARIANT)))) {

                        MapList mlPVTechStructure = pvDomObject.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST, ConfigurationConstants.TYPE_FEATURE_LIST,
                                lstPVObjectSelects, new StringList(ConfigurationConstants.SELECT_RELATIONSHIP_ID), false, true, (short) 1, sbPVWhereClause.toString(), "", 0);

                        MapList mlProductTechStructure = productDomObject.getRelatedObjects(context, strRelType, "*", false, true, (short) 0, selectStmts,
                                new StringList(ConfigurationConstants.SELECT_RELATIONSHIP_ID), sbWhereClause.toString(), null, null, ConfigurationConstants.TYPE_FEATURE_LIST, null);

                        String strId = null;

                        for (int i = 0; i < mlProductTechStructure.size(); i++) {
                            Hashtable mpTechFtr = (Hashtable) mlProductTechStructure.get(i);
                            strId = mpTechFtr.get(ConfigurationConstants.SELECT_ID).toString();
                            strRelType = mpTechFtr.get("relationship").toString();
                            for (int j = 0; j < mlPVTechStructure.size(); j++) {
                                if (strId.equals(((Map) mlPVTechStructure.get(j)).get(ConfigurationConstants.SELECT_ID).toString())
                                        && strRelType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM)) {
                                    mlFinalTechStructure.add((Map) mlProductTechStructure.get(i));
                                }
                            }
                        }
    ...
    /**
     * This method will be called to get the part related data for features or products and also effective list of DVs on a tech feature
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param arrFeatureIds
     ...
     * @return Returns a <code>ArrayList</code> object. The return object is ArrayList, which will contain all the Part ObjectId for a given list of Features.
     * @throws Exception
     *             if the operation fails
     * @since PRC 10.5.1.2 this could not be deprecated as referenced from migration JPO
     */
    ...
    public ArrayList getPartRuleData(Context context, ArrayList arrFeatureIds, String strProdVariantId) throws Exception {
        ...
        if (strProdVariantId != null && !"null".equalsIgnoreCase(strProdVariantId) && !"".equals(strProdVariantId)) {
            if ((new DomainObject(strProdVariantId).getInfo(context, ConfigurationConstants.SELECT_TYPE)).equalsIgnoreCase(ConfigurationConstants.TYPE_PRODUCT_VARIANT))
                isVariant = true;
        }
        for (int arrCount = 0; arrCount < arrFeatureIds.size(); arrCount++) {
            try {
                String strFeatureObjectId = (String) arrFeatureIds.get(arrCount);
                DomainObject domFeatureObject = new DomainObject(strFeatureObjectId);
                ...
                objectSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to." + ConfigurationConstants.SELECT_ID);
                objectSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to." + ConfigurationConstants.SELECT_TYPE);
                objectSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to." + ConfigurationConstants.SELECT_NAME);
                objectSelects.addElement("from[" + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to." + ConfigurationConstants.SELECT_REVISION);

                objectSelects.addElement(ConfigurationConstants.SELECT_ID);
                objectSelects.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + EXPRESSION_CLOSE);
                objectSelects.addElement(STR_ATTRIBUTE_OPEN_BRACE + ConfigurationConstants.ATTRIBUTE_EXPRESSION + EXPRESSION_CLOSE);

                short sRecurseLevel = 1;
                // getting the parts related to one feature
                relObjects = domFeatureObject.getRelatedObjects(context, strRelPattern, strObjPattern, objectSelects, relSelects, false, true, sRecurseLevel, "", "", 0);
    ...
    public boolean isInvalid(Context context, String strRelId, String strProdId) throws Exception {
        ...
        strMqlCmd = "print connection \"" + strRelId + "\" select tomid[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY + "].id dump |";
...

        {
            ...
            while (st.hasMoreTokens()) {
                ...
                strMqlCmd = "print connection \"" + strIVBRelId + "\" select fromrel.id dump";
                String strProdFeatListRelId = MqlUtil.mqlCommand(context, strMqlCmd, true);
                strProdFeatListRelId = strProdFeatListRelId.trim();

                strMqlCmd = "print connection \"" + strProdFeatListRelId + "\" select from.id dump";
                String strPVId = MqlUtil.mqlCommand(context, strMqlCmd, true);
                strPVId = strPVId.trim();
    ...
    public void modifyEquipmentListReportXML(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mlEquipmentFeature = product.getLogicalFeatureStructure(context, ConfigurationConstants.TYPE_EQUIPMENT_FEATURE, ConfigurationConstants.RELATIONSHIP_LOGICAL_FEATURES, objectSelects,
                    new StringList(), false, true, -1, 0, ConfigurationConstants.EMPTY_STRING, ConfigurationConstants.EMPTY_STRING, DomainObject.FILTER_ITEM, DomainObject.EMPTY_STRING);
    ...
    /**
     * This method will call for each Equipment Feature and get its Part, depending on the Inclusion Rule , List will be populated which will be finally used to create XML
     * @param context
     * @param args
     * @throws Exception
     * @since R212
    ...
    private void modifyEquipmentListReportXMLGeneration(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mlCompleteFeatureStructure = product.getCompleteFeatureStructure(context, DomainObject.EMPTY_STRING, DomainObject.EMPTY_STRING, objectSelects, relSelects, false, true, 0, 0,
                    ConfigurationConstants.EMPTY_STRING, ConfigurationConstants.EMPTY_STRING, DomainObject.FILTER_ITEM, DomainObject.EMPTY_STRING);
            ...
            StringTokenizer stTempNew = new StringTokenizer(strEqpFeatureId, STR_Check_Token);
            ...
            while (stTempNew.hasMoreTokens()) {
                ...
                if (partList.size() > 0) {
                    ...
                    for (int jx = 0; jx < mapList.size(); jx++) {
                        ...
                        if (map.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]") != null) {
                            strRightExpression = (String) map
                                    .get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]");
                        }
                        if (map.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "]") != null) {
                            strLeftExpression = (String) map
                                    .get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "]");

                        }
                        if (map.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.id") != null) {
                            strInclusionRuleID = (String) map.get("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.id");
                        }
                                            ...
                                            if (strRightExpressionID.contains("AND")) {
                                                ...
                                                for (int k = 0; k < strArrayANDSplit.length; k++) {
                                                    ...
                                                    if (!domTempExpression.exists(context)) {
                                                        ...
                                                        if (mxType.isOfParentType(context, strObjectType, ConfigurationConstants.TYPE_PRODUCTS) && strRightExpressionID.equals(productPhyID)) {
                                                            iIncludeCount++;
                                                        } else {
            ...
            if (strConnect.equalsIgnoreCase(
                    STR_DELETE)) {/*
                                   * Feature featureBean = (Feature) com.matrixone.apps.domain.DomainObject .newInstance(context, ConfigurationConstants.TYPE_FEATURES, "Configuration");
                                   * StringTokenizer stTempDel = new StringTokenizer( strEqpFeatureId, STR_Check_Token); List al = new ArrayList(); while (stTempDel.hasMoreTokens()) { al.add((String)
                                   * stTempDel.nextToken()); } String[] strObjectIdNew = new String[al.size()]; for (int i = 0; i < al.size(); i++) { strObjectIdNew[i] = (String) al.get(i); }
                                   * 
                                   * if(deleteCheck.equalsIgnoreCase(STR_LAST)){ featureBean.delete(context, strObjectIdNew); }
                                   */
     ...
     * @since R212
     */
    private boolean isFeatureConnectedToProduct(Context context, DomainObject rightExpObject, StringList slAllFeaturePhysicalIDs) throws Exception {
        StringList objSel = new StringList("to[" + ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES + "].physicalid");
        objSel.add("to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES + "].physicalid");
        DomainObject.MULTI_VALUE_LIST.add("to[" + ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES + "].physicalid");
        DomainObject.MULTI_VALUE_LIST.add("to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES + "].physicalid");

        Map mpRelIds = rightExpObject.getInfo(context, objSel);

        DomainObject.MULTI_VALUE_LIST.remove("to[" + ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES + "].physicalid");
        DomainObject.MULTI_VALUE_LIST.remove("to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES + "].physicalid");

        StringList allRelIDs = new StringList();
        for (int j = 0; j < mpRelIds.size(); j++) {
            StringList slLFRelIDs = (StringList) (mpRelIds.get("to[" + ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES + "].physicalid"));
            allRelIDs.add(slLFRelIDs);
            StringList slCFRelIDs = (StringList) (mpRelIds.get("to[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES + "].physicalid"));
    ...
    protected List getPartObjectDetails(Context context, List arrFeatureIds) throws Exception {
        List arrPartObjectIds = new ArrayList();
        MapList relObjects = new MapList();
        try {
            for (int arrCount = 0; arrCount < arrFeatureIds.size(); arrCount++) {
                String strFeatureObjectId = (String) arrFeatureIds.get(arrCount);
                LogicalFeature domFeatureObject = new LogicalFeature(strFeatureObjectId);
                StringList objectSelects = new StringList();
                StringList relSelects = new StringList();
                relSelects.add("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]");
                relSelects.add("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "]");
                relSelects.add("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.id");
                relSelects.addElement("attribute[" + ConfigurationConstants.ATTRIBUTE_RULE_TYPE + "]");
                ...
                // getting the parts related to one feature
                relObjects = domFeatureObject.getActiveGBOMStructure(context, typePattern, ConfigurationConstants.EMPTY_STRING, objectSelects, relSelects, false, true, 1, 0,
                        ConfigurationConstants.EMPTY_STRING, ConfigurationConstants.EMPTY_STRING, DomainObject.FILTER_ITEM, ConfigurationConstants.EMPTY_STRING);
     ...
     * @return
     * @throws FrameworkException
     * @since R212 TODO-need to get separate selectable to relationship
     */
                ...
                if (isECInstalled) {
                    stbNameRev = stbNameRev.append("<a><img src=\"../common/images/iconActionCreateNewPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18CreateNew)
                            ...
                            .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                    + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "&amp;isQuantityInvalid=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isQuantityInvalid))
                                    + "&amp;displayGenerateIcon=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(displayGenerateIcon)) + "&amp;LFParentID="
                                    + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf) + "&amp;ForcePartReuse="
                                    + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;isUsedInBOM=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM))
                                    + "&amp;DuplicateParts=" + Duplicateparts + "&amp;mode=createpart" + "&amp;PartMode=custom" + "&amp;duplicate=false" + "&amp;parentId="
                                    + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;generate=false" + "');\"")
                            .append("/></a>");
                }
                stbNameRev = stbNameRev.append(" <a><img src=\"../common/images/iconActionAddExistingPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18AddExisting)
                        ...
                        .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                ...
                                + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf) + "&amp;ForcePartReuse="
                                + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;isUsedInBOM=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM))
                                + "&amp;DuplicateParts=" + Duplicateparts + "&amp;mode=addexisting" + "&amp;PartMode=custom" + "&amp;duplicate=false" + "&amp;parentId="
                                + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;generate=false" + "');\"")
                        .append("/></a>");
                if (partFamily.size() == 1) {
                    String strPartFamily = (String) partFamily.get(0);
                ...
                if (isECInstalled) {
                    stbNameRev = stbNameRev.append("<a><img src=\"../common/images/iconActionCreateNewPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18CreateNew)
                            ...
                            .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                    ...
                                    + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf) + "&amp;ForcePartReuse="
                                    + XSSUtil.encodeForHTMLAttribute(context, strForcePartReuse) + "&amp;isUsedInBOM=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM))
                                    + "&amp;mode=replacebycreatepart" + "&amp;PartMode=custom" + "&amp;duplicate=false" + "&amp;parentId=" + XSSUtil.encodeForHTMLAttribute(context, parentId)
                                    + "&amp;DuplicateParts=" + Duplicateparts + "&amp;ResolvedPart=" + XSSUtil.encodeForHTMLAttribute(context, strResolvedPart) + "&amp;ResolvedPartRelID="
                                    + XSSUtil.encodeForHTMLAttribute(context, strGBOMRelID) + "&amp;parentId=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "&amp;generate=false" + "');\"")
                            .append("/></a>");
                }
                stbNameRev = stbNameRev.append(" <a><img src=\"../common/images/iconActionAddExistingPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(i18AddExisting)
                        ...
                        .append(" onclick=\"javascript:showDialog('../configuration/PreviewBOMProcess.jsp?featureId=" + XSSUtil.encodeForHTMLAttribute(context, strObjectId) + "&amp;level="
                                + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "&amp;isQuantityInvalid=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isQuantityInvalid))
                                + "&amp;LFParentID=" + XSSUtil.encodeForHTMLAttribute(context, strLFParentId) + "&amp;isLFLeaf=" + XSSUtil.encodeForHTMLAttribute(context, strLFisLeaf)
                                + "&amp;ForcePartReuse=" + strForcePartReuse + "&amp;displayGenerateIcon=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(displayGenerateIcon))
                                + "&amp;isUsedInBOM=" + XSSUtil.encodeForHTMLAttribute(context, String.valueOf(isUsedInBOM)) + "&amp;DuplicateParts=" + Duplicateparts + "&amp;ResolvedPart="
                                + XSSUtil.encodeForHTMLAttribute(context, strResolvedPart) + "&amp;ResolvedPartRelID=" + XSSUtil.encodeForHTMLAttribute(context, strGBOMRelID)
                                + "&amp;mode=replacebyaddexisting" + "&amp;PartMode=custom" + "&amp;duplicate=false" + "&amp;parentId=" + XSSUtil.encodeForHTMLAttribute(context, parentId)
                                + "&amp;generate=false" + "');\"")
                        .append("/></a>");
                // IF PART FAMILY SIZE ==
                if (partFamily.size() == 1) {
                    ...
                    if (!("obsolete").equalsIgnoreCase(strPartFamilyState)) {
                        ...
                        stbNameRev.append(XSSUtil.encodeForHTMLAttribute(context, strPartFamily));
                        stbNameRev.append("', '700', '600', 'true', 'listHidden', '')\">");
                        stbNameRev.append("<img border=\"0\" align=\"middle\" src=\"../common/images/iconActionReplaceWithGeneratedpart.gif\" title=\'" + i18Generate + "\'/></a>");
                    }
                }
            }
            vNameRev.add(stbNameRev.toString());
    ...
    public Vector getPartUsageForDisplay(Context context, String args[]) throws Exception {
        ...
        String strPartUsage = "";
        String strLanguage = context.getSession().getLanguage();
        String strStandard = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.Range.Part_Usage.Standard", strLanguage);
        String strCustom = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.Range.Part_Usage.Custom", strLanguage);
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (ProductLineCommon.isNotNull(strPartUsage)) {
                ...
                if (ProductLineCommon.isNotNull(strRelationship) && strRelationship.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_GBOM)) {
                    vectPartUsage.add(strStandard);
                } else if (ProductLineCommon.isNotNull(strRelationship) && strRelationship.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_CUSTOM_GBOM)) {
                    vectPartUsage.add(strCustom);
                } else {
                    vectPartUsage.add("");
                }
     ...
     * @since R212
     */
    public Object checkCustomPartMode(Context context, String[] args) throws FrameworkException {
        try {
     ...
     * @throws Exception
     */
    public void deltaUpdateBOMXMLForProductConfiguration(Context context, String[] args) throws Exception {
        try {
            /*
             * DomainObject domProductConfigurationObject = new DomainObject(); String strObjPcId = args[0]; String strFeatureId = args[1]; String deltaContext = args[2]; StringTokenizer
             * strTknFeatureId = new StringTokenizer(strFeatureId, "][,"); StringList slFeatures = new StringList(); while (strTknFeatureId.hasMoreTokens()) { String strFeaId =
             * strTknFeatureId.nextToken().trim(); slFeatures.addElement(strFeaId); } ProductConfiguration pc = new ProductConfiguration(); pc.setId(strObjPcId); if
             * (deltaContext.equalsIgnoreCase("AddFeatureUpdate")) { pc.updateBOMXMLForAddFeature(context, strObjPcId,slFeatures); }else if (deltaContext.equalsIgnoreCase("RemoveFeatureUpdate")) {
             * pc.updateBOMXMLForRemoveFeature(context, strObjPcId,slFeatures); } else if (deltaContext.equalsIgnoreCase("GBOMUpdate")) { pc.updateBOMXMLForGBOM(context, strObjPcId, slFeatures); }
             * else if (deltaContext.equalsIgnoreCase("QuantityUpdate")|| deltaContext.equalsIgnoreCase("UsageUpdate")) { pc.updateBOMXMLForUsageAndQuantity(context, strObjPcId, slFeatures); }
             */} catch (Exception e) {
            throw new FrameworkException(e.getLocalizedMessage());
        }
    }

    public Vector displayQuantity(Context context, String args[]) throws Exception {
     ...
     * @param context
     * @param args
     *            - parentId is the object id of Product Configuration, - strFeatureId is the object id of feature whose duplicate parts are to be retrived.
     * @throws Exception
     * @author
     */
    public MapList getDuplicatePartsFromXML(Context context, String parentId, String strFeatureId, String strResolvedPart, String strResolvedPartRelId) throws FrameworkException {
        ...
        try {
            ...
            // get the expression
            evaluatedParts = lfbean.getActiveGBOMStructure(context, ConfigurationConstants.TYPE_PART, strRelSel.toString(), objSelects, ConfigurationUtil.getBasicRelSelects(context), false, true, 1,
                    0, null, null, DomainObject.FILTER_STR_AND_ITEM, strFilterCompiledForm);
            ...
            for (int j = 0; j < evaluatedParts.size(); j++) {
                ...
                if (strResolvedPartsRelIds.contains(partrelId)) {
                    ...
                    j--;
                } else if (strResolvedPartsRelIds.isEmpty() && (strResolvedPartRelId != null && strResolvedPartRelId.equals(partrelId))) {
                    evaluatedParts.remove(j);
                    j--;
                }
            }
    ...
    public boolean callIsEBOMDifferent(Context context, String args[]) throws Exception {
        HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
        ...
        return isEBOMDifferent(context, strObjectId, featureId, productConfigurationId);
     ...
     *             this could not be deprecated as referenced from migration JPO
     */
    private boolean isEBOMDifferent(Context context, String PartID, String FeatureID, String parentID) throws Exception {
        boolean isEBOMDifferent;
        ArrayList alSubFeatures = new ArrayList();
        ArrayList GBOMParts = new ArrayList();

        DomainObject objPCID = new DomainObject(parentID);
        String strBOMXML = objPCID.getInfo(context, ConfigurationConstants.SELECT_ATTRIBUTE_BOMXML);
        SAXBuilder saxb = new SAXBuilder();
        saxb.setFeature("http://apache.org/xml/features/disallow-doctype-decl", true);
        saxb.setFeature("http://xml.org/sax/features/external-general-entities", false);
        saxb.setFeature("http://xml.org/sax/features/external-parameter-entities", false);
        ...
        inner: for (int ix = 0; ix < listFeature.size(); ix++) {
            ...
            if (FeatureID.equalsIgnoreCase(featureId)) {
                ...
                List listDuplicate = (List) elementFeature.getChildren("SubFeatures");

                if (listDuplicate.size() > 0) {
                    Element elementDuplicate = (Element) listDuplicate.get(0);
                    List listSubFeature = elementDuplicate.getChildren("Feature");
                    for (int p = 0; p < listSubFeature.size(); p++) {
                        Element elementPart = (Element) listSubFeature.get(p);
                        String subFeatureId = elementPart.getAttributeValue("id");
        ...
        for (int q = 0; q < alSubFeatures.size(); q++) {
            ...
            inner1: for (int ix = 0; ix < listFeature.size(); ix++) {
                ...
                if (sfId.equalsIgnoreCase(featureId)) {
                    ...
                    List listPending = (List) elementFeature.getChildren("Resolved");
                    if (listPending.size() > 0) {
                        Element elementPending = (Element) listPending.get(0);
                        partId = elementPending.getAttributeValue("id");
                        GBOMParts.add(partId);

                    } else {
                        List listResolved = (List) elementFeature.getChildren("Pending");
                        List listDuplicate = (List) elementFeature.getChildren("Duplicate");
                        if (listResolved.size() > 0) {
                            Element elementResolved = (Element) listResolved.get(0);
                            partId = elementResolved.getAttributeValue("id");
                            GBOMParts.add(partId);
                        } else {
                            if (listDuplicate.size() > 0) {
                                Element elementDuplicate = (Element) listDuplicate.get(0);
                                List listPart = elementDuplicate.getChildren("Part");
                                for (int p = 0; p < listPart.size(); p++) {
                                    Element elementPart = (Element) listPart.get(p);
                                    partId = elementPart.getAttributeValue("id");
                                    GBOMParts.add(partId);
                                }
        ...
        StringList objSelects = new StringList();
        objSelects.add(ConfigurationConstants.SELECT_ID);
        MapList mapListEBOMRelatedObject = objPartOfFeature.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_EBOM, ConfigurationConstants.TYPE_PART, objSelects,
                new StringList(ConfigurationConstants.SELECT_RELATIONSHIP_ID), false, true, (short) 1, null, null, 0);
        ...
        while (itrEBOM.hasNext()) {
            ...
            String objFeatureId = (String) hsEBOM.get(ConfigurationConstants.SELECT_ID);
        ...
        try {
            ...
            for (int i = 0; i < objectList.size(); i++) {
                ...
                if (ProductLineCommon.isNotNull(isLeafLF) && isLeafLF.equalsIgnoreCase("Yes")) {
                    stbNameRev = stbNameRev.append("<a><img src=\"../common/images/iconActionDuplicateRow.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                            .append(XSSUtil.encodeForHTMLAttribute(context,
                                    EnoviaResourceBundle.getProperty(context, "Configuration", "emxConfiguration.ToolTip.Duplicate", context.getSession().getLanguage())))
                            .append("\"").append(" onclick=\"javascript:addDuplicateRowForLF('" + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "');\"").append("/></a>&#160;&#160;");

                    stbNameRev = stbNameRev.append("<a><img src=\"../common/images/iconActionRemoveRow.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                            .append(XSSUtil.encodeForHTMLAttribute(context,
                                    EnoviaResourceBundle.getProperty(context, "Configuration", "emxConfiguration.ToolTip.Remove", context.getSession().getLanguage())))
                            .append("\"").append(" onclick=\"javascript:removeLeafLevelLF('" + XSSUtil.encodeForHTMLAttribute(context, strObjectLevel) + "');\"").append("/></a>");
                    vNameRev.add(stbNameRev.toString());
                } else {
                    vNameRev.add("");
                }
     ...
     * @throws FrameworkException
     */
    public static StringList getBackAnnotateForPartOnPreviewBOM(Context context, String[] args) throws FrameworkException {
        try {
                        ...
                        for (int k = 0; k < listParts.size(); k++) {
                            ...
                            if (strGBOMRelID.equals(strPartRelId)) {
                                ...
                                if (bomAttributes != null) {
                                    String strI18Value = (EnoviaResourceBundle.getProperty(context, "Configuration", "emxConfiguration.PreviewBOM.UpdateBOMDetails.Range.Local",
                                            context.getSession().getLanguage()));
                                    returnStringList.addElement(strI18Value);
                                } else {
                                    String strI18Value = (EnoviaResourceBundle.getProperty(context, "Configuration", "emxConfiguration.PreviewBOM.UpdateBOMDetails.Range.Global",
                                            context.getSession().getLanguage()));
                                    returnStringList.addElement(strI18Value);
                                }
                            }
    ...
    public Map getRangeValuesForBOMAttributeUsage(Context context, String[] args) throws Exception {
        ...
        List attributeDisplayRange = i18nNow.getAttrRangeI18NStringList(ConfigurationConstants.ATTRIBUTE_USAGE, (StringList) attributeRange, context.getSession().getLanguage());
