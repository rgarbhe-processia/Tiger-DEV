...
public class emxTeamUtilsBase_mxJPO {
    ...
    public StringList excludeAffectedItems(Context context, String args[]) throws Exception {
        ...
        String typePattern = PropertyUtil.getSchemaProperty(context, "type_PLMCoreReference");
        String whereExpression = "to[" + DomainConstants.RELATIONSHIP_PART_SPECIFICATION + "].from.policy.property[PolicyClassification]==Development";
        StringList objectSelects = new StringList(1);
        objectSelects.addElement(DomainConstants.SELECT_ID);

        MapList excludeProducts = DomainObject.findObjects(context, typePattern, context.getVault().getName(), whereExpression, objectSelects);
    ...
    public StringList getContextProjectUsers(Context context, String args[]) throws Exception {
        return TBEUtil.getContextProjectUsers(context);
    }

    /**
     * Method to check for revise access on development part In TBE, once one of the development part revision is released, we will not allow further development part revisions on the same
     * @param context
     * @param args
     * @return Boolean
     * @throws Exception
    ...
    public Boolean hasReviseAccess(Context context, String[] args) throws Exception {
        ...
        StringList revStateList = FrameworkUtil.split(MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", objectId,
                "from[" + DomainConstants.RELATIONSHIP_PART_SPECIFICATION + "|to.type.kindof[" + EngineeringConstants.TYPE_VPLM_CORE_REF + "]]" + " policy current.access[revise] revisions.current"),
                ",");

        if ("true".equalsIgnoreCase((String) revStateList.get(0))) {

            if (DomainConstants.POLICY_DEVELOPMENT_PART.equalsIgnoreCase((String) revStateList.get(1)) && "TRUE".equalsIgnoreCase((String) revStateList.get(2))) {
    ...
    public String getDynamicReviewrListQuery(Context context, String[] args) throws Exception {
        return "TYPES=type_RouteTemplate:ROUTE_BASE_PURPOSE=Review:CURRENT=policy_RouteTemplate.state_Active:LATESTREVISION=TRUE" + EngineeringUtil.getAltOwnerFilterString(context);
    }
    ...
    public String getDynamicApprovalListQuery(Context context, String[] args) throws Exception {
        return "TYPES=type_RouteTemplate:ROUTE_BASE_PURPOSE=Approval:CURRENT=policy_RouteTemplate.state_Active:LATESTREVISION=TRUE" + EngineeringUtil.getAltOwnerFilterString(context);
    }
    ...
    public String getDynamicDistributionListQuery(Context context, String[] args) throws Exception {
        return "TYPES=type_MemberList:CURRENT=policy_MemberList.state_Active" + EngineeringUtil.getAltOwnerFilterString(context);
    }
    ...
    public String getDynamicReportedAgainstQuery(Context context, String[] args) throws Exception {
        return "TYPES=type_Part,type_Builds,type_CADDrawing,type_CADModel,type_DrawingPrint,type_PartSpecification,type_Products" + EngineeringUtil.getAltOwnerFilterString(context);
    }

    /**
     * Blocks the creation of Part if the user context is Private/Protected Project. This is just a temperory fix to handle only Public access
     * @param context
     * @param args
     * @return String
     * @throws Exception
    ...
    public int allowPartCreateForPublicProj(Context context, String[] args) throws Exception {
        ...
        if (ctxRole.length() > 1) {
            ...
            String strMessage = "";
            String strProjectType = MqlUtil.mqlCommand(context, "print role $1 select $2 dump", ctxProject, "property[vplm_readAbility].value");
