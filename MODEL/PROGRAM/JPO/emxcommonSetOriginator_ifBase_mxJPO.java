...
public class emxcommonSetOriginator_ifBase_mxJPO {
    ...
    public emxcommonSetOriginator_ifBase_mxJPO(Context context, String[] args) throws Exception {
        // if (!context.isConnected())
        // throw new Exception("not supported no desktop client");
    }

    /**
     * This is an initialization procedure used to set the Originator attribute value to the name of the current user and This procedure is designed to be called from the check manager via a creation
     * check The actual Originator attribute name must be looked up as the customer could change it to a different string Because creation is often done as the Shadow Agent, the environment variable
     * APPREALUSER is checked If set, it is used instead of USER for setting the originator.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            hold the following input arguments: 0 - String containing Property name of attribute to be set.
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        ...
        if ((!UIUtil.isNullOrEmpty(sObjectId)) && (sEvent.compareTo("Revision") == 0 || sEvent.compareTo("MinorRevision") == 0)) {
            ...
            String mqlCommand = "print bus " + sObjectId + " select last.id dump;";
            String mqlResult = MqlUtil.mqlCommand(context, mqlCommand);
            sObjectId = mqlResult;
        } else if (!UIUtil.isNullOrEmpty(sObjectId) && sEvent.compareTo("MajorRevision") == 0) {
            String mqlCommand = "print bus " + sObjectId + " select majorid.nextmajorid.bestsofar.id dump;";
            String mqlResult = MqlUtil.mqlCommand(context, mqlCommand);
            sObjectId = mqlResult;
        }
