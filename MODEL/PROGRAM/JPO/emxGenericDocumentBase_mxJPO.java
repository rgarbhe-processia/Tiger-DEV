...
/**
 * The <code>emxGenericDocumentBase</code> represents implementation of anything on the "To Side" of "Has Documents" Relationship in DC Schema
 * @exclude
 */

public class emxGenericDocumentBase_mxJPO extends emxCommonDocument_mxJPO implements DocumentCentralConstants {
    ...
    /*
     * 
     * public ${CLASSNAME} (String id) throws Exception { // Call the super constructor
     * 
     * super(id); }
     */
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (true) {
            throw new Exception("Must specify method on emxGenericDocumentBase invocation");
        }
     ...
     * @param context
     * @param type
     *            - for creating subtype of GD, assumed as GD if null. If not null, it must be a subtype of GD
     * @param name
     *            - Name of the Generic Document
     * @param revision
     *            - revision of the Generic Document , default revision is applied if null
     * @param description
     *            - description of the Generic Document , no description is set if null
     * @param policy
     *            - Policy for the Generic Document, default policy is applied if null
     * @param vault
     *            - Vault for the Generic Document, default vault is applied if null
     * @param approverName
     *            - Approver Name for the Generic Document, User Name of a Person with Reviewer role
     * @param map
     *            - a HashMap containing values for other Attributes
     * @param relationshipName
     *            - name of the relationship to use to connect, if null, Classified Item will be used
     * @param parentId
     *            - parentID to connect, if relationshipName is Classified Item, this object must be of type Classification, If both relationshipName and parentId are null then Created Generic
     *            Document is not connected to any objects
     * @return void
     * @throws Exception
     *             in Failure
     */
    public CommonDocument createAndConnect(Context context, String type, String name, String revision, String description, String policy, String vault, String approverName, Map map,
            String relationshipName, String parentId) throws Exception {

        GenericDocument genericDocument = new GenericDocument();
        genericDocument.createAndConnect(context, type, name, revision, description, policy, vault, approverName, map, relationshipName, parentId);
    ...
    /**
     * The method checks if all DocumentSheet children of this object are beyond WIP state when object is promoted to Released state.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     *             if the operation fails
     */
    public int checkAllDocumentSheetState(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList wipDocSheetList = doc.getRelatedObjects(context, LibraryCentralConstants.RELATIONSHIP_DOCUMENT_SHEETS, "*", null, null, false, // get the "to"
                    true, // get the "from"
            ...
            if (wipDocSheetList.size() > 0) {
                String errorString = EnoviaResourceBundle.getProperty(context, "emxDocumentCentralStringResource", new Locale(context.getSession().getLanguage()),
                        "emxDocumentCentral.ErrorMsg.AllDocumentSheetChildrenNotActive");
