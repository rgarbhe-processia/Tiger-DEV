...
/**
 * The <code>Job</code> class represents Job JPO in common implements methods to access Job lists, and also triggers related to background jobs
...
public class emxJobBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @since AEF V11-0-0-0
     */
    public emxJobBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    /**
     * Returns the Background Jobs owned by the context user, and matching the filter criteria
     ...
     *                if filter = = 'All' call getMyBackgroundJobs(Context context)<br>
     *                else if 'Completed' call getMyCompletedBackgroundJobs(Context context)<br>
     *                else if 'Failed' call getMyFailedBackgroundJobs(Context context)<br>
     */
    public static MapList getBackgroundJobs(Context context, String filter) throws FrameworkException {
        ...
        try {
            resultList = DomainObject.findObjects(context, typeJob, DomainConstants.QUERY_WILDCARD, DomainConstants.QUERY_WILDCARD, user, DomainConstants.QUERY_WILDCARD, whereExp.toString(), null,
                    true, objectSelects, (short) 0, null, null);
     ...
     *             if the operation fails
     * @since AEF V11-0-0-0
     * @mx.pseudocode temp query 'Background Job' * * where 'owner = = context.user'
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getMyBackgroundJobs(Context context, String args[]) throws FrameworkException {
        return getBackgroundJobs(context, "All");
    }

    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getMyCurrentBackgroundJobs(Context context, String args[]) throws FrameworkException {
        return getBackgroundJobs(context, "Current");
    }

    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getMyArchivedBackgroundJobs(Context context, String args[]) throws FrameworkException {
     ...
     *             if the operation fails
     * @since AEF V11-0-0-0
     * @mx.pseudocode temp query 'Background Job' * * where "owner = = context.user && attribute[Completion Status] = = 'Completed'
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getMyCompletedBackgroundJobs(Context context, String[] args) throws FrameworkException {
     ...
     *             if the operation fails
     * @since AEF V11-0-0-0
     * @mx.pseudocode temp query bus 'Background Job' * * where "owner == context.user && attribute[Completion Status] = = 'Failed'
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public static MapList getMyFailedBackgroundJobs(Context context, String[] args) throws FrameworkException {
        return getBackgroundJobs(context, "Failed");
    }

    /**
     * Starts a job running when it is promoted to Running.
     * @mx.whereUsed invoked as a trigger when a Job object is promoted to Running. Calls job.start()
     * @param context
     * @since AEF V11-0-0-0
     * @param args
     */
    public static int startJobOnPromoteToRunning(Context context, String[] args) throws FrameworkException {
        /*
     ...
     * @mx.whereUsed invoked as a trigger when a Job object is is created. Currently there is no concept of queueing; submitted jobs are immediately promoted to Running
     * @param c
     * @param args
     */
    public static int autoPromoteSubmittedJobToRunning(Context context, String[] args) throws MatrixException {
        String jobId = PropertyUtil.getRPEValue(context, "OBJECTID", false);
    ...
    /**
     * Notifies the owner with a message. Notifying is controlled thru "Notify Owner" attribute
     ...
     * @throws Exception
     */
    public static int notifyOwner(Context context, String[] args) throws Exception {
        ...
        for (Iterator itr = objectList.iterator(); itr.hasNext();) {
            ...
            if ((current.equals(Job.STATE_JOB_COMPLETED) || current.equals(Job.STATE_JOB_ARCHIVED)) && (completionStatus.equals("Succeeded"))) {
                if (nextStepCommand != null && !"".equals(nextStepCommand) && !"null".equals(nextStepCommand)) {
                    ...
                    curRowLink = "javascript:showModalDialog('" + commandMap.get("HREF") + "','" + XSSUtil.encodeForJavaScript(context, windowWidth) + "','"
                            + XSSUtil.encodeForJavaScript(context, windowHeight) + "',true,'" + XSSUtil.encodeForJavaScript(context, popupSize) + "')";
     ...
     *            the ematrix Context
     * @param commandName
     *            the name of the command that has been set as the value for the attribute "Next Step Command"
     * @throws Exception
        ...
        for (Iterator itr = objectList.iterator(); itr.hasNext();) {
            ...
            if (current.equals(Job.STATE_JOB_RUNNING)) {
                ...
                try {
                    ...
                    resultList.addElement(unknownProgress);
                }
            } else if (current.equals(Job.STATE_JOB_COMPLETED) || current.equals(Job.STATE_JOB_ARCHIVED)) {
    ...
    /**
     * Deletes the Jobs
     * @mx.whereUsed Invoked by the user from Background Job List view action menu
     * @param context
     *            the eMatrix <code>Context</code> object
    ...
    public void delete(Context context, String[] args) throws FrameworkException {
        ...
        try {
            Map programMap = (Map) JPO.unpackArgs(args);
            String[] jobIds = (String[]) programMap.get("JobIds");
            String current, completionStatus;
            ...
            for (int index = 0; index < jobIds.length; index++) {
                ...
                current = (String) resMap.get(DomainConstants.SELECT_CURRENT);
    ...
    /**
     * Notify if Equipment List Report Job is completed, if job is complete it removes pending job relationship.
     ...
     * @throws Exception
     */
    public static void removePendingJob(Context context, String[] args) throws Exception {
