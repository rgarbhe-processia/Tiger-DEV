...
public class IEFActionAccess_mxJPO {
     ...
     * @since EC 10.5
     */
    public IEFActionAccess_mxJPO(Context context, String[] args) throws Exception {
    }

    /**
     * This is overridden by IEF. For IEF types do not show revise with files option. For minors even revise should be restricted.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds objectId.
    ...
    public Boolean canRevise(Context context, String[] args) throws Exception {
        ...
        try {
            if (isTypeAccessable) {
                ...
                // [NDM] QWJ
                /*
                 * BusinessObject bus = new BusinessObject(objectId); bus.open(context); String type = bus.getTypeName(); bus.close(context);
                 */
    ...
    private boolean isMinorObject(Context context, String objectId, String language) {
         ...
         * 
         * String attrBusTypeMapping = MCADMxUtil.getActualNameForAEFData(context, "attribute_MCADInteg-BusTypeMapping"); IEFSimpleConfigObject simpleGCO = IEFSimpleConfigObject.getSimpleGCO(context,
         * integrationName); Hashtable typeMapping = simpleGCO.getAttributeAsHashtable(attrBusTypeMapping, "\n", "|");
         * 
         ...
         * StringTokenizer(sTypes, ","); while(tokenizer.hasMoreElements()) { String sType = (String) tokenizer.nextElement(); mxTypes.addElement(sType.trim()); } }
         * 
