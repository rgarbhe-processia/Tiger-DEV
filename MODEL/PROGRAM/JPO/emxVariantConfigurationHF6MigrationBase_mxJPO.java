 ...
 */
public class emxVariantConfigurationHF6MigrationBase_mxJPO extends emxCommonMigration_mxJPO {
    protected static final String selectFeatureListToName = "from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.name";

    protected static final String selectFeatureListToMrktName = "from[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME + "]";

    protected static final String selectFeatureListFromName = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.name";

    protected static final String selectFeatureListFromMrktName = "to[" + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.attribute[" + ProductLineConstants.ATTRIBUTE_MARKETING_NAME
            + "]";

    // from[Left Expression].to[GBOM].to[GBOM From].from.from[Varies by].to.id
    protected static final String selectVariesByGBOM = "from[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "].to[" + ConfigurationConstants.TYPE_GBOM + "].to["
            + ConfigurationConstants.RELATIONSHIP_GBOM_FROM + "].from.from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.id";

    // from[Right Expression].to[Feature List].to.from.id
    protected static final String selectVariesByFeatureList = "from[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "].to[" + ConfigurationConstants.TYPE_FEATURE_LIST + "].to["
            + ConfigurationConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.id";

    protected static final String selectInactiveVariesBy = "tomid[" + ConfigurationConstants.RELATIONSHIP_INACTIVE_VARIES_BY + "].fromrel[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST
            + "].from.id";
     ...
     * @grade 0
     */
    public emxVariantConfigurationHF6MigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public void migrateObjects(Context context, StringList objectList) throws Exception {
        ...
        if (isrelId.equals("")) {
            /*
             * String cmd = "print bus " + (String) objectList.get(0) + " select exists dump"; isrelId = MqlUtil.mqlCommand(context, cmd);
             */
        ...
        if (isrelId.equals("TRUE")) {
            ...
            mapList.addAll(DomainObject.getInfo(context, oidsArray, selectStmts));
        } else {

            mapList.addAll(DomainRelationship.getInfo(context, oidsArray, relSelectStmts));
        }
        ...
        while (itr.hasNext()) {
            ...
            } else if (type.equals(ConfigurationConstants.TYPE_INCLUSION_RULE) || type.equals(ConfigurationConstants.TYPE_RULE) || type.equals(ConfigurationConstants.TYPE_BOOLEAN_COMPATIBILITY_RULE)
                    || type.equals(PropertyUtil.getSchemaProperty(context, "type_BOM_Rules")) || type.equals(ConfigurationConstants.TYPE_MARKETING_PREFERENCE)
                    || type.equals(ConfigurationConstants.TYPE_RULE_EXTENSION) || type.equals(ConfigurationConstants.TYPE_FIXED_RESOURCE)
            ...
            } else if (type.equals(ConfigurationConstants.RELATIONSHIP_VARIES_BY)) {
    ...
    public void migrateVariesBy(Context context, Map objMap) throws Exception {
        ...
        String INVALID_CONTEXT = PropertyUtil.getSchemaProperty(context, "attribute_InvalidContexts");
        String objID = (String) objMap.get(DomainConstants.SELECT_ID);
        if (objMap.get(selectInactiveVariesBy) != null) {
            String invalidContexts = (String) objMap.get(selectInactiveVariesBy);

            new DomainRelationship(objID).setAttributeValue(context, INVALID_CONTEXT, FrameworkUtil.findAndReplace(invalidContexts, SelectConstants.cSelectDelimiter, ","));
            convertedOidsLog.write("Varies By =" + objID + newline);
        }

    } // End Of migrating varies by()

    public void migrateFeatureList(Context context, Map objMap) throws Exception {

        String ATTRIBUTE_PARENT_OBJECT_NAME = PropertyUtil.getSchemaProperty(context, "attribute_ParentObjectName");
        String ATTRIBUTE_PARENT_MARKETING_NAME = PropertyUtil.getSchemaProperty(context, "attribute_ParentMarketingName");
        String ATTRIBUTE_CHILD_OBJECT_NAME = PropertyUtil.getSchemaProperty(context, "attribute_ChildObjectName");
        String ATTRIBUTE_CHILD_MARKETING_NAME = PropertyUtil.getSchemaProperty(context, "attribute_ChildMarketingName");
        try {
            ...
            if (objMap.get(selectFeatureListFromName) != null) {
                attvaluesMap.put(ATTRIBUTE_PARENT_OBJECT_NAME, (String) objMap.get(selectFeatureListFromName));
                attvaluesMap.put(ATTRIBUTE_PARENT_MARKETING_NAME, (String) objMap.get(selectFeatureListFromMrktName));

            }
            if (objMap.get(selectFeatureListToName) != null) {
                attvaluesMap.put(ATTRIBUTE_CHILD_OBJECT_NAME, (String) objMap.get(selectFeatureListToName));
                attvaluesMap.put(ATTRIBUTE_CHILD_MARKETING_NAME, (String) objMap.get(selectFeatureListToMrktName));
            ...
            if (attvaluesMap != null && attvaluesMap.size() > 0) {
                ...
                new DomainObject(strObjId).setAttributeValues(context, attvaluesMap);
            }

            convertedOidsLog.write("migrateFeatureList =" + strObjId + newline);

        } catch (Exception e) {
            mqlLogRequiredInformationWriter(".....migrateFeatureList....exception........." + e + "\n");
            e.printStackTrace();
            throw e;
        }
     ...
     * @since X3.
     */
    public void migrateDesignVariants(Context context, Map objMap) throws Exception {

        try {

            String strIncRuleID = (String) objMap.get(DomainConstants.SELECT_ID);

            String ATTRIBUTE_DESIGNVARIANTS = PropertyUtil.getSchemaProperty(context, "attribute_DesignVariants");

            if (objMap.get(selectVariesByGBOM) != null) {
                // mqlLogRequiredInformationWriter("strIncRuleID
                // ="+strIncRuleID+"\n");
                StringList listDVs = (StringList) objMap.get(selectVariesByGBOM);
                ...
                if (objMap.get(selectVariesByFeatureList) != null) {
                    StringList tmplistDV = (StringList) objMap.get(selectVariesByFeatureList);
                ...
                new DomainObject(strIncRuleID).setAttributeValue(context, ATTRIBUTE_DESIGNVARIANTS, tempdvs);
     ...
     * @since X3.
     */
    public void migrateRuleComplexity(Context context, String objectID) throws Exception {

        String ATTRIBUTE_RIGHT_EXPRESSION = PropertyUtil.getSchemaProperty(context, "attribute_RightExpression");
        String ATTRIBUTE_DESIGNVARIANTS = PropertyUtil.getSchemaProperty(context, "attribute_DesignVariants");
        String ATTRIBUTE_RULE_COMPLEXITY = PropertyUtil.getSchemaProperty(context, "attribute_RuleComplexity");
        try {
            ...
            Map incRuleMap = new DomainObject(objectID).getInfo(context, tempselects);

            strIncRuleID = (String) incRuleMap.get(DomainConstants.SELECT_ID);
            dvs = (String) incRuleMap.get("attribute[" + ATTRIBUTE_DESIGNVARIANTS + "]");
            rightExp = (String) incRuleMap.get("attribute[" + ATTRIBUTE_RIGHT_EXPRESSION + "]");
            ...
            if (dvs == null || ("".equals(dvs)) || ("null".equals(dvs))) {
                ...
                String partType = new DomainObject(strIncRuleID).getInfo(context, "from[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "].to[" + ConfigurationConstants.TYPE_GBOM + "].from["
                        + ConfigurationConstants.RELATIONSHIP_GBOM_TO + "].to.type");
            ...
            if (rightExp != null && !("".equals(rightExp)) && !("null".equals(rightExp))) {
                if (rightExp.indexOf(" AND ") >= 0) {
                    ...
                    rightExp = FrameworkUtil.findAndReplace(rightExp, ")", "");
                    rightExp = FrameworkUtil.findAndReplace(rightExp, "AND", "");
                    rightExp = FrameworkUtil.findAndReplace(rightExp, "OR", "");
                    rightExp = FrameworkUtil.findAndReplace(rightExp, "NOT", "");
                    ...
                    for (Iterator iterator = tempList.iterator(); iterator.hasNext();) {
                        ...
                        if (strid != null && !("".equals(strid)) && !("null".equals(strid))) {
                            ...
                            Map objMap = donObjid.getInfo(context, tempObjselects);
                            String type = (String) objMap.get(DomainConstants.SELECT_TYPE);
                            // mqlLogRequiredInformationWriter("
                            // ======type========"+type+"== \n");
                            if (type.equals(ProductLineConstants.TYPE_FEATURES) || type.equals(ProductLineConstants.TYPE_PRODUCTS)) {
                                // new
                                // DomainObject(strIncRuleID).setAttributeValue(context,ATTRIBUTE_RULE_COMPLEXITY,"Complex");
                                // mqlLogRequiredInformationWriter("
                            ...
                            if (type.equals(ProductLineConstants.TYPE_FEATURE_LIST)) {

                                // mqlLogRequiredInformationWriter("
                                // ======feature List========"+strIncRuleID+"==
                                // \n");
                                String strFeature1 = donObjid.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.id");
                            ...
                            if (type.equals(ProductLineConstants.RELATIONSHIP_COMMON_GROUP)) {
                                // new
                                // DomainObject(strIncRuleID).setAttributeValue(context,ATTRIBUTE_RULE_COMPLEXITY,"Complex");
                                // mqlLogRequiredInformationWriter("
                                // ======common gruop========
                                ...
                                MapList comGrplist = new DomainRelationship().getInfo(context, new String[] { strid }, new StringList("tomid.to.id"));
                                String comGrpID = (String) ((Map) comGrplist.get(0)).get("tomid.to.id");
            ...
            new DomainObject(strIncRuleID).setAttributeValue(context, ATTRIBUTE_RULE_COMPLEXITY, "Simple");

            convertedOidsLog.write("migrateRuleComplexity =" + strIncRuleID + newline);
    ...
    public void migrateLeftRightExpresion(Context context, String strIncRuleID) throws Exception {

        try {
            String ATTRIBUTE_LEFT_EXPRESSION = PropertyUtil.getSchemaProperty(context, "attribute_LeftExpression");
            String ATTRIBUTE_RIGHT_EXPRESSION = PropertyUtil.getSchemaProperty(context, "attribute_RightExpression");
            ...
            for (int i = 0; i < expression.size(); i++) {
                ...
                if (strRightExpressionList != null && strRightExpressionList.size() > 0) {
                    // Rule Expression associated with the Object
                    String strRuleExp = (String) strRightExpressionList.get(0);

                    if (tmpExp.equals("Right")) {
                        attvaluesMap.put(ATTRIBUTE_RIGHT_EXPRESSION, strRuleExp);
            ...
            convertedOidsLog.write("migrateLeftRightExpresion =" + strIncRuleID + newline);
