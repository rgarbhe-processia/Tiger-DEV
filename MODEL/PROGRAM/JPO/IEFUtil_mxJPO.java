...
/**
 * The <code>IEFUtil</code> class represents the JPO for obtaining the MS Office integration menus
...
public class IEFUtil_mxJPO {
    ...
    // private static final String MY_CHECKEDOUT_DOCUMENTS = CommonDocument.DEFAULT_DOCUMENT_TYPE + "," + PropertyUtil.getSchemaProperty("type_GenericDocument") + "," +
    // PropertyUtil.getSchemaProperty("type_CADDrawing") + "," + PropertyUtil.getSchemaProperty("type_CADModel");
    ...
    /**
     * Parses the HREF values of a command and puts the parameter value in a Map Returns a map which has the name value pair of the params
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param commandName
     *            the name of the command being evaluated
     * @param commandHRef
     *            the HRef value of the command
     * @throws Exception
     *             if the operation fails
     * @since AEF 10.5
     */
        ...
        try {
            ...
            if (!areParamsProper) {
                ...
                throw (new FrameworkException(
                        "The command " + commandName + "is not UI3 enabled. The HREF does not use program or inquiry or table to display objects in it. Contact your system administrator."));
            }
        } catch (Exception ex) {
                ...
                if (tempObjectMapList != null && !(tempObjectMapList.isEmpty())) {
                    ...
                    if (programName.equals("emxAEFCollection:getCollections")) {
                        for (int index = 0; index < tempObjectMapListSize; index++) {
                            ...
                            // emxAEFCollectionBase JPO -->BPS translates the System generated Colletion (i.e. Clipboard) to locale specific , MSF reads the response and sends the Clipboard name in
                            // that specific locale(e.g. Japanese)
    ...
    public MapList getMyCheckedOutDocs(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String MY_CHECKEDOUT_DOCUMENTS = CommonDocument.DEFAULT_DOCUMENT_TYPE + "," + PropertyUtil.getSchemaProperty(context, "type_GenericDocument") + ","
                    + PropertyUtil.getSchemaProperty(context, "type_CADDrawing") + "," + PropertyUtil.getSchemaProperty(context, "type_CADModel");
    ...
    public boolean isTypeOf(Context context, String sType, String sRootType) throws MatrixException, FrameworkException {
    ...
    /**
     * Generic function that the Applet calls for getting actions menu details. returns an xml response after traversing the object
     * @param args
     *            input details as provided by the Applet
            ...
            try {
                ...
                if (menuMapList != null) {
                    ...
                    while (menuItr.hasNext()) {
                        ...
                        if (componentLabel != null && componentLabel.length() > 0 && (componentLabel.equalsIgnoreCase("emxFramework.Suites.Display.IEF")
                                || componentLabel.equalsIgnoreCase("emxFramework.Suites.Display.Integration") || componentLabel.equalsIgnoreCase("emxFramework.Suites.Display.DesignerCentral"))) {
                        ...
                        String uiComponentSymbolicName = FrameworkUtil.getAliasForAdmin(context, uiComponentType, componentName, true);
    ...
    private String getFormattedComponentHRef(Context context, String componentHRef, String menuRegisteredDir, String sRegisteredSuite, String stringResFileId, String sWindowHeight,
            String sWindowWidth) throws Exception {
