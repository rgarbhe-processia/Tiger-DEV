...
/**
 * The <code>ChangeTemplateBase</code> class contains methods for executing JPO operations related to objects of the admin type Change.
 * @author R3D
...
public class enoECMChangeTemplateBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public Map create(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            // dmObj.setState(context,PropertyUtil.getSchemaProperty(context,"policy",
            // ChangeConstants.POLICY_CHANGE_TEMPLATE,
            // ChangeConstants.STATE_SYMBOLIC_ACTIVE));
    ...
    public MapList getMyTemplatesView(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (sItr.hasNext()) {
                ...
                MapList sParentOrgList = orgObj.getRelatedObjects(context, RELATIONSHIP_DIVISION + "," + RELATIONSHIP_COMPANY_DEPARTMENT, TYPE_ORGANIZATION, new StringList(SELECT_ID), null, true,
                        false, (short) 0, EMPTY_STRING, EMPTY_STRING, null, null, null);
    ...
    /**
     * @author R3D Updates the Owning Organization in CO WebForm.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
        ...
        try {
            ...
            if ("edit".equals(mode)) {
                if ("Enterprise".equals(sAvailabilityOption)) {
                    ...
                    DomainRelationship.connect(context, DomainObject.newInstance(context, strNewToTypeObjId), ChangeConstants.RELATIONSHIP_CHANGE_TEMPLATES,
                            DomainObject.newInstance(context, currentCTObjectID));
                    // ContextUtil.popContext(context);
                } else if ("Personal".equals(sAvailabilityOption)) {
                    // ContextUtil.pushContext(context);
                    DomainRelationship.disconnect(context, relId);
                    dmObj.setOwner(context, user);
                    DomainRelationship.connect(context, DomainObject.newInstance(context, loggedInPersonId), ChangeConstants.RELATIONSHIP_CHANGE_TEMPLATES,
                            DomainObject.newInstance(context, currentCTObjectID));
                    // ContextUtil.popContext(context);
                }
            }
    ...
    public StringList getOwningOrganizations(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            StringBuffer sSelectBU = new StringBuffer("to[");
            sSelectBU.append(RELATIONSHIP_BUSINESS_UNIT_EMPLOYEE);
            sSelectBU.append("].from.id");

            dmObj.setId(loggedInPersonId); // Person Object
            String sPersonBU = dmObj.getInfo(context, sSelectBU.substring(0));

            if (!UIUtil.isNullOrEmpty(sPersonBU)) {

                dmObj.setId(sPersonBU); // Organization Object
                sOrganizationList = dmObj.getRelatedObjects(context, RELATIONSHIP_DIVISION + "," + RELATIONSHIP_COMPANY_DEPARTMENT, TYPE_ORGANIZATION, selectList, null, false, true, (short) 0,
                        EMPTY_STRING, EMPTY_STRING, null, null, null);

                includeOIDList.add(sPersonBU);
            }
            // If the Person is at Host Company Level
                ...
                includeOIDList.add(orgId);

            }
            if (sOrganizationList.size() > 0) {
                Iterator sOrganizationItr = sOrganizationList.iterator();
                Map map;
                while (sOrganizationItr.hasNext()) {
                    map = (Map) sOrganizationItr.next();
                    includeOIDList.add((String) map.get(SELECT_ID));
    ...
    public StringList getResponsibleOrganizations(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            sb.append(RELATIONSHIP_BUSINESS_UNIT_EMPLOYEE);
            sb.append("].from.id");
            ...
            dmObj.setId(loggedInPersonId);
            String sPersonBU = dmObj.getInfo(context, sb.substring(0));
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - String containing one String entry for key "objectId" of current Organization
    ...
    public Vector showEditIconforStructureBrowser(Context context, java.util.HashMap arguMap) throws FrameworkException {
        ...
        try {
            ...
            sbStartHref.append(
                    "&amp;preProcessJavaScript=setOwningOrganization&amp;HelpMarker=emxhelpparteditdetails&amp;commandName=ECMMyChangeTemplates&amp;refreshStructure=false&amp;postProcessURL=../enterprisechangemgt/ECMCommonRefresh.jsp&amp;suiteKey=EnterpriseChangeMgt&amp;objectId=");
    ...
    public StringList getUsageTemplates(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (sItr.hasNext()) {
                ...
                MapList sOrgList = sBUObj.getRelatedObjects(context, // matrix context
    ...
    /**
     * Returns all Person with Technical Assignee Role
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds input arguments.
     ...
     * @since ECM R212
     */
    public String getCATechnicalAssignee(Context context, String[] args) throws Exception {

        String sTechAssignee = "";
        try {

            String objId = args[0];
            String techAssigneeRel = PropertyUtil.getSchemaProperty(context, "relationship_TechnicalAssignee");

            StringBuffer select = new StringBuffer("from[");
            select.append(techAssigneeRel);
            select.append("].to.name");

            DomainObject dmObj = DomainObject.newInstance(context);
            dmObj.setId(objId);
            sTechAssignee = dmObj.getInfo(context, select.substring(0));
            // CA is connected to Person with Relationship "Technical Assignee"
        } catch (Exception e) {
            throw new FrameworkException(e);
        }
        return sTechAssignee;
     ...
     * @since ECM R212
     */
    public static Map getAllPersonValues(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            MapList mlPersons = DomainObject.findObjects(context, TYPE_PERSON, "*", wherClause, strList);
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getInterfaceAttributes(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public MapList getAttributesByQuery(Context context, String nameMatches, String typeFilter, boolean unused) throws Exception {
        ...
        String allAttrData = "";
        try {
            allAttrData = MqlUtil.mqlCommand(context, "list attribute $1 select $2 $3 $4 $5 $6 $7 dump $8 recordsep $9", true, nameMatches, "name", "type", "hidden", "description", "owner",
                    "application", "@", "|").trim();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the list of Attribute Groups
     * @throws Exception
     *             if the operation fails
     * @since ECM R216
     */
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param queryParamSelect
     *            query parameters for query
     * @param interfaceName
     *            to get its related info
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following data 1.Change Template id 2.new Attribute Group Name 3.description for AG 4.Attributes to added to AG
     * @throws Exception
     *             if the operation fails
     * @since ECM R216
     */
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following data 1.Change Template id 2.filter to show the result 3.name matches if matching Ag are required to add.
     ...
     * @param args
     *            holds the following input arguments 0 - objectName (Attribute Group Name)
     * @return String of result which holds details for
     * @throws Exception
     *             if the operation fails
     * @since ECM R216
     */
    public String getAGDescription(Context context, String[] args) throws Exception {
        ...
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        String attributeGroupName = (String) requestMap.get("objectName");
        String result = MqlUtil.mqlCommand(context, "print interface $1 select $2 dump $3", attributeGroupName, ChangeConstants.SELECT_DESCRIPTION, ",").toString();
    ...
    public String getAGOriginatedDate(Context context, String[] args) throws Exception {
        ...
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        String attributeGroupName = (String) requestMap.get("objectName");
        String result = MqlUtil.mqlCommand(context, "print interface $1 select $2 dump $3", attributeGroupName, ChangeConstants.SELECT_ORIGINATED, ",").toString();
        return result.toString();
    }
    ...
    private MapList getDynamicFieldsMapList(Context context, MapList classificationAttributesList, String formName, boolean isCreate) throws Exception {
        ...
        while (classItr.hasNext()) {
            ...
            for (int i = 0; i < AGAttributes.size(); i++) {
                ...
                /*
                 * String attributeGroupFieldName = (isCreate==true?attributeGroupName+"|"+attributeName:attributeName); fieldMap.put("name",attributeGroupFieldName);
                 */
                ...
                /*
                 * if(isCreate){ settingsMap.put(SETTING_UPDATE_PROGRAM,"enoECMChangeTemplate"); settingsMap.put(SETTING_UPDATE_FUNCTION,"dummyUpdateFunction"); }else{
                 * 
                 * }
                 */
    ...
    /***
     * This method returns details all attributes of context AG having given selectables. settings for each of the attribute type.
     * @param context
     * @param agName
     *            =AG name
     * @param selectables
