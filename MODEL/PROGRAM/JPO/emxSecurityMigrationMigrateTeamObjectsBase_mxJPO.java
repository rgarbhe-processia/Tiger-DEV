...
public class emxSecurityMigrationMigrateTeamObjectsBase_mxJPO extends emxCommonMigration_mxJPO {
        ...
        if ((templateMemberIds.size() == templateMemberNames.size()) && (templateMemberIds.size() == templateMemberRoles.size())) {
            for (int i = 0; i < templateMemberIds.size(); i++) {
                ...
                if (USER_PROJECTS.contains(memberName + "_PRJ")) {
                    ...
                    mqlLogRequiredInformationWriter("Adding Multiple Ownership for Member '" + memberName
                            + "' is not supported due to two reason 1. The member is a Role or Group 2. The member is not Migrated to be added....");
                    unconvertable = true;
                    unConvertableComments += "Adding Multiple Ownership for Member '" + memberName
                            + "' is not supported due to two reason 1. The member is a Role or Group 2. The member is not Migrated to be added.... \n";
                }
            }
        }
    ...
    private void migrateWorkspaceFolder(Context context, Map<?, ?> m) throws Exception {
        ...
        // If parent Workspace object has a grentee with Basic access(read,show,checkout) then as per the old access mechanism, the connected folders and Documents will not have any access.
        // But they have to be migrated as per the new access mechanism. read show checkout access has to be given to connected folders and Documents

        String parentGrantee = getGranteeifParentHasBasicAccess(context, m, parentAccessMap);
        if (UIUtil.isNotNullAndNotEmpty(parentGrantee)) {
            migrateObjectforBasicAccess(context, new StringList(contentParentId), parentGrantee);
        }
        ...
        if (contentList != null && contentList.size() > 0) {
            ...
            while (itr.hasNext()) {
                ...
                mqlLogRequiredInformationWriter("Started Migrating Object " + (String) cMap.get("type") + "  " + (String) cMap.get("name") + "  " + (String) cMap.get("revision")
                        + " Access for Grantees = " + contetnGrantees);
            ...
            // If the folder objects parent has basic access then migrate all the document contents has to be migrated to grant read, show, checkout access
            if (UIUtil.isNotNullAndNotEmpty(parentGrantee)) {
                migrateObjectforBasicAccess(context, contentList, parentGrantee);
            }
        }
    }
    ...
    /**
     * @param m
     * @param parentAccessMap
     * @return
     * @throws Exception
     */
    private String getGranteeifParentHasBasicAccess(Context context, Map<?, ?> m, Map<String, Collection> parentAccessMap) throws Exception {
        String parentGrantee = "";
        StringList parentGrantees = (StringList) m.get(parentWorkspaceGrantee);
        // if parentGrantees is null then iterate through 'Sub Vaults' and 'Data Vaults' relationships to get the Workspace Grantee
        if (parentGrantees == null) {
            String folderId = (String) m.get(parentFolderId);
            DomainObject folderObject = new DomainObject(folderId);
            StringList objectSelects = new StringList("grantee");
            objectSelects.add("grant.granteeaccess");
            String relationshipPattern = relationshipDataVaults + "," + relationshipSubVaults;
            MapList workspaceData = folderObject.getRelatedObjects(context, relationshipPattern, DomainConstants.TYPE_WORKSPACE, true, false, 0, objectSelects, null, null, null, 0, null, null, null);
            Iterator<?> itr = workspaceData.iterator();
            while (itr.hasNext()) {
                Map cMap = (Map) itr.next();
                StringList workspaceGrantees = new StringList();
                Object granteeInfo = (Object) cMap.get("grantee");
                if (granteeInfo instanceof String) {
                    workspaceGrantees.add((String) granteeInfo);
                } else if (granteeInfo instanceof StringList) {
                    workspaceGrantees.addAll((StringList) granteeInfo);
                }
                workspaceGrantees = removeDuplicates(workspaceGrantees);
                Map<String, Collection> parentWorkspaceAccessMap = (Map<String, Collection>) ((Map<String, Map>) getTotalAccessMap(context, cMap)).get("OBJECTACCESSMAP");
                parentGrantee = getParentWorkspaceGrantee(parentWorkspaceAccessMap, workspaceGrantees);
            }
        } else {
            parentGrantees = removeDuplicates(parentGrantees);
            parentGrantee = getParentWorkspaceGrantee(parentAccessMap, parentGrantees);
        }
        return parentGrantee;
    }
     ...
     * @throws Exception
     */
    private String getParentWorkspaceGrantee(Map<String, Collection> parentAccessMap, StringList parentGrantees) throws Exception {
        String parentGrantee = "";
        Iterator pgItr = parentGrantees.iterator();
        while (pgItr.hasNext()) {
            parentGrantee = (String) pgItr.next();
            HashSet parentAccessSet = (HashSet) parentAccessMap.get(parentGrantee);
            ...
            if (getBasicAccessLong() == parentAccessLong) {
                break;
            }
        }
        return parentGrantee;
     ...
     * @throws Exception
     */
    private long getBasicAccessLong() throws Exception {
        StringList accessList = new StringList();
        accessList.add("read");
        accessList.add("show");
        accessList.add("checkout");

        return getAccessFlag(accessList);
    }

    private void migrateObjectforBasicAccess(Context context, StringList objectIds, String grantee) throws Exception {
        mqlLogRequiredInformationWriter("------------------------------------------------------");

        String basicAccessList = "read,show,checkout";

        for (int count = 0; count < objectIds.size(); count++) {
            String objectId = (String) objectIds.get(count);
            mqlLogRequiredInformationWriter("Migrating object " + objectId + " for basic access start...");
            if (USER_PROJECTS.contains(grantee + "_PRJ")) {
                if (!DomainAccess.hasObjectOwnership(context, objectId, null, grantee + "_PRJ", DomainAccess.COMMENT_MULTIPLE_OWNERSHIP)) {
                    // mqlLogRequiredInformationWriter("Modify Folder " + objectId + " Add objectOwnership for user " + grantee +" with access as "+ basicAccessList);
                    mqlLogRequiredInformationWriter("Migrating object " + objectId + " Add objectOwnership for user " + grantee + " with access as " + basicAccessList);
                    DomainAccess.createObjectOwnership(context, objectId, null, grantee + "_PRJ", basicAccessList, DomainAccess.COMMENT_MULTIPLE_OWNERSHIP);
                }
            } else {
                mqlLogRequiredInformationWriter("Modify Folder " + objectId + " Add objectOwnership for user '" + grantee
                        + "' is not supported due to two reason 1. The member is a Role or Group 2. The member is not Migrated to be added....");
                unconvertable = true;
                unConvertableComments += "Modify Folder " + objectId + " Add objectOwnership for user '" + grantee
                        + "' is not supported due to two reason 1. The member is a Role or Group 2. The member is not Migrated to be added.... \n";
            }
            mqlLogRequiredInformationWriter("Migrating object " + objectId + " for basic access end...");
        }

        mqlLogRequiredInformationWriter("------------------------------------------------------");
    }

    private void migrateObjectAccess(Context context, Map<String, Collection> objectAccessMap, Map<String, Collection> parentAccessMap, StringList grantees, Map<?, ?> m, String parentId)
            throws Exception {
        if (grantees != null) {
    ...
    public static boolean hasObjectOwnership(Context context, String objectId, String parentId, String comment) throws FrameworkException {
        ...
        String result = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", objectId, "ownership[" + parentId + "|" + comment + "]");
    ...
    private void migrateWorkspace(Context context, Map<?, ?> m) throws Exception {
        ...
        if (projectMemberIds != null) {
            ...
            while (gItr.hasNext()) {
                ...
                if (USER_PROJECTS.contains(grantee + "_PRJ")) {
                    mqlLogRequiredInformationWriter("------------------------------------------------------");
                    mqlLogRequiredInformationWriter("Modify Workspace " + m.get("name") + " for user " + grantee + " with access as " + StringUtil.join(accessList, ",") + " with comment as "
                    ...
                    mqlLogRequiredInformationWriter("Modify Workspace " + (String) m.get("id") + " adding Multiple Ownership for user '" + grantee
                            + "' is not supported due to two reason 1. The member is a Role or Group 2. The member is not Migrated to be added....");
                    unconvertable = true;
                    unConvertableComments += "Modify Workspace " + (String) m.get("id") + " adding Multiple Ownership for user '" + grantee
                            + "' is not supported due to two reason 1. The member is a Role or Group 2. The member is not Migrated to be added.... \n";
                }
            }
    ...
    /**
     * This method is used to modify the policy of Document objects In migration we are modifying the policy to Document Release if it is Document
     * @param context
     * @param m
     * @throws Exception
     */
