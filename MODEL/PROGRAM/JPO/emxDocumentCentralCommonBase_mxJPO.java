 ...
 **
 ** FileName : "$RCSfile: ${CLASSNAME}.java.rca $" Author : rajeshv Version : "$Revision: 1.17 $" Date : "$Date: Wed Oct 22 16:02:17 2008 $"
 **
 */
...
public class emxDocumentCentralCommonBase_mxJPO extends emxDocumentCentralRoot_mxJPO {
     ...
     * @param args
     *            the Java <code>String[]</code> object, contains the object Id for the object
     * @throws Exception
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (true) {
            throw new Exception("must specify method on emxDocumentCentralCommonBase invocation");
        }
    ...
    public static Map subAndRelatedTypes(Context context, String[] args) throws Exception {
        /*
     ...
     * @since AEF 9.5.0.0
     */
    protected static Map subAndRelatedTypes(Context context, Map aMap) throws Exception {
        /*
         * Author : Rajesh Venugopalan Date : 11/12/02 Notes : History :
         */
        ...
        MxDebug.location(MxDebug.DL_10, "Input arguments:" + "\n\tobjectType:   \"" + objectType + "\"" + "\n\tdirection:    \"" + direction + "\"" + "\n\trelationship: \"" + relationShip + "\"");
        ...
        try {
            ...
            if ((relationShip != null) && (relationShip.trim().length() > 1)) {
                if ((direction != null) && direction.equalsIgnoreCase("To")) {
                    busList = new RelationshipType(relationShip).getToTypes(context);
                } else if ((direction != null) && direction.equalsIgnoreCase("From")) {
                    busList = new RelationshipType(relationShip).getFromTypes(context);
                }
                ...
                try {
                    ...
                    while (tokenizer.hasMoreTokens()) {
                        BusinessType tmpType = new BusinessType(tokenizer.nextToken(), context.getVault());
                        busList.addElement(tmpType);
                    }
                } catch (Exception e) {
                }

                String strAbstract = (String) new Boolean((new BusinessType(objectType, context.getVault())).isAbstract(context)).toString();

                map.put(objectType, DomainObject.EMPTY_STRING + "," + strAbstract);
            ...
            MxDebug.location(MxDebug.DL_10, "Raw return:  " + busList.toString());
            ...
            while (typeListItr.next()) {
                ...
                if (compareMap.containsKey(objParentName)) {
                    map.put(typeName, busType.getParent(context) + "," + (new Boolean(busType.isAbstract(context))).toString());
                } else {
                    map.put(typeName, DomainObject.EMPTY_STRING + "," + (new Boolean(busType.isAbstract(context))).toString());
                }
            }
        } catch (FlowControlException fce) {
            MxDebug.location(MxDebug.DL_10, "Proper request condition was not specified!");
        }

        MxDebug.location(MxDebug.DL_10, "Final subAndRelatedTypes return:  " + map.toString());
