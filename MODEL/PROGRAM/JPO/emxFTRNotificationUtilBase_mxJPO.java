...
public class emxFTRNotificationUtilBase_mxJPO extends emxPLCNotificationUtil_mxJPO {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: objectId - the Id for the To side object for the EC Affected Item relationship notificationName - FTREngineeringChangeRaisedEvent toType - the
    ...
    public static int notifyOnEngineeringChangeRaisedEvent(Context context, String[] args) throws Exception {
        ...
        if (mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_PRODUCTS) || mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_LOGICAL_STRUCTURES)
                || mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES) || mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_TEST_CASE)) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: objectId - the Id for the To side object for the Issue relationship notificationName - FTRIssueRaisedEvent toType - the type for the To side
    ...
    public static int notifyOnIssueRaisedEvent(Context context, String[] args) throws Exception {
        ...
        if (fromType.equals(typeIssue) && (mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_PRODUCTS)
                || mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_LOGICAL_STRUCTURES) || mxType.isOfParentType(context, toType, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES)
    ...
    /**
     * This trigger method creates a mail notification when an inclusion rule is updated for a feature or a product's GBOM part.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: relId - the Id for the Right Expression relationship created each time when a part inclusion rule is changed or created. notificationName -
     *            FTRPartInclusionRuleUpdatedEvent A notification is generated only if the From side object is of type Inclusion Rule and the To side object is of type Feature List.
     * @return an int 0 status code
     * @throws Exception
     *             if the operation fails
     */
    public static int notifyOnPartInclusionRuleUpdatedEvent(Context context, String[] args) throws Exception {
        ...
        while (itrIds.hasNext()) {
            ...
            if (ctxObjectId == null || ctxObjectId.trim().length() == 0)
                continue;
    ...
    private static String _getObjectForRightExpression(Context context, String ruleId) throws Exception {
        ...
        MapList mapList = domObj.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_RIGHT_EXPRESSION, typePattern.getPattern(), objSelects, null, false, true, (short) 1, null, null, 0);
        if (mapList.isEmpty())
            return objectId; // ""; //not the right relationship
