...
public class emxGeneratePreciseBOMBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public emxGeneratePreciseBOMBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public Vector displayActionIcons(Context context, String args[]) throws Exception {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            if (!(strObjectLevel.equalsIgnoreCase("0"))) {
                ...
                gbomList = domProductObject.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_GBOM, "", false, true, 1, selectable, null, stbWhereClause.toString(), null, null, null,
                        null);

                // get list of parts for which part inclusion rule is evaluated to true
                try {

                    stbNameRev = stbNameRev.append("<img src=\"../common/images/iconActionCreateNewPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append("Create New")
                            .append("\"")
                            .append(" onclick=\"javascript:showDialog('../configuration/ClonePartUtil.jsp?featureId=" + strObjectId + "&amp;level=" + strObjectLevel + "&amp;isQuantityInvalid="
                                    + isQuantityInvalid + "&amp;displayGenerateIcon=" + displayGenerateIcon + "&amp;mode=createpart" + "&amp;duplicate=false" + "&amp;parentId=" + parentId
                                    + "&amp;generate=false" + "');\"")
                            .append("/>");
                    stbNameRev = stbNameRev.append(" <img src=\"../common/images/iconActionAddExistingPart.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append("Add Existing")
                            .append("\"")
                            .append(" onclick=\"javascript:showDialog('../configuration/ClonePartUtil.jsp?featureId=" + strObjectId + "&amp;level=" + strObjectLevel + "&amp;isQuantityInvalid="
                                    + isQuantityInvalid + "&amp;displayGenerateIcon=" + displayGenerateIcon + "&amp;mode=addexisting" + "&amp;duplicate=false" + "&amp;parentId=" + parentId
                                    + "&amp;generate=false" + "');\"")
                            .append("/>");
                    if (gbomList.size() == 1) {
                        ...
                        if (!("obsolete").equalsIgnoreCase(strPartFamilyState)) {
                            stbNameRev = stbNameRev.append(" <img src=\"../common/images/iconActionGenerateFromPartFamily.gif").append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"")
                                    .append("Generate").append("\"")
                                    .append(" onclick=\"javascript:showDialog('../configuration/ClonePartUtil.jsp?featureId=" + strObjectId + "&amp;level=" + strObjectLevel + "&amp;isQuantityInvalid="
                                            + isQuantityInvalid + "&amp;displayGenerateIcon=" + displayGenerateIcon + "&amp;mode=generate" + "&amp;duplicate=false" + "&amp;parentId=" + parentId
                                            + "&amp;generate=true" + "');\"")
                                    .append("/>");
                        }
                    }
     ...
     *            holds the selected features String from the calling method
     * @return MapList
     * @throws Exception
     *             if the operation fails
     * @since R212
    ...
    public MapList getLogicalFeatureStructureForSelectFeatures(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (strSelFeatureIds != null && !strSelFeatureIds.equals("")) {
                int start = 0;
                StringTokenizer FIdTokenizer = new StringTokenizer(strSelFeatureIds, "-");
            ...
            sbWhereClause.append("(id matchlist(" + strMatchList.toString() + ")\",\")");

            LogicalFeature cfBean = new LogicalFeature(strObjectid);
            mapLogicalStructure = (MapList) cfBean.getLogicalFeatureStructure(context, "", null, null, null, false, true, iLevel, limit, sbWhereClause.toString(), DomainObject.EMPTY_STRING,
