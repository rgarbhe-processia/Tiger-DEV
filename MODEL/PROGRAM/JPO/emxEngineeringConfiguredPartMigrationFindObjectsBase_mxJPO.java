...
public class emxEngineeringConfiguredPartMigrationFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO {
     ...
     * @grade 0
     */
    public emxEngineeringConfiguredPartMigrationFindObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public void getIds(Context context, int chunkSize) throws Exception {
        ...
        try {
            // String whereclause = "policy == \"" + UnresolvedEBOMConstants.POLICY_CONFIGURED_PART + "\" && (from[" + DomainConstants.RELATIONSHIP_EBOM + "] == True || from[" +
            // UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING + "] == True) && program[" + migrationProgramName + " -method writeOID ${OBJECTID} {$TYPE}] == true";
            String whereclause = "policy == \"" + UnresolvedEBOMConstants.POLICY_CONFIGURED_PART + "\" && (from[" + DomainConstants.RELATIONSHIP_EBOM + "] == True || from["
                    + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING + "] == True)";
            String command = "temp query bus \"$1\" \"$2\" \"$3\" vault \"$4\" where \"$5\" select \"$6\" dump \"$7\"";

            writer.write("command = " + command + "\n");
            String connections = MqlUtil.mqlCommand(context, command, type, name, revision, vaultList, whereclause, "id", ",");
            String[] connect = connections.split("\n");
            HashSet<String> uniqueObjectIds = new HashSet<String>(connect.length);

            for (int i = 0; i < connect.length; i++) {
                String[] eachConn = connect[i].split(",");

                if (uniqueObjectIds.add(eachConn[3])) {
                    String[] arrJPOArgs = new String[2];
                    arrJPOArgs[0] = eachConn[3];
                    arrJPOArgs[1] = eachConn[0];
                    JPO.invoke(context, migrationProgramName, null, "writeOID", arrJPOArgs);
                }
            }
        } catch (Exception me) {
            throw me;
