...
/**
 * DSCReplace.java Copyright Dassault Systemes, 1992-2007. All Rights Reserved. This program contains proprietary and trade secret information of Dassault Systemes and its subsidiaries, Copyright
...
public class DSCReplace_mxJPO {
     ...
     *            The attribute value.
     */
    public void setRelAttribute(Context _context, Relationship childParentRel, String attribName, String attrValue) {
        try {
            childParentRel.open(_context);
            AttributeList childParentAttr = childParentRel.getAttributes(_context);
            for (int childParAttr = 0; childParAttr < childParentAttr.size(); childParAttr++) {
                Attribute attrib = (Attribute) childParentAttr.get(childParAttr);
                if (attrib.getName().compareTo(attribName) == 0) {
                    util.setRelationshipAttributeValue(_context, childParentRel, attribName, attrValue);
                }
    ...
    /**
     * @param busID
     *            The bus id(object) from which the attributes are obtained.
     * @param attribName
     *            The name for which the value will be queried.
     * @param hidden
     *            True for retrieving hidden values other wise false.
     * @return
     */
     ...
     * @param busID
     *            The business object id on which the attributes will be set.
     * @param attribName
     *            The attribute name
     * @param attribValue
     *            The attribute value
     * @param hidden
     *            If true the hidden attributes can be set If false the hidden attributes cannot be set.
     */
    public void setBusAttribute(Context context, String busID, String attribName, String attribValue, boolean hidden) {
        try {
            BusinessObject busObj = new BusinessObject(busID);
            busObj.open(context);
            ...
            for (int i = 0; i < busAttribLst.size(); i++) {
                Attribute attrib = (Attribute) busAttribLst.get(i);
                if (attrib.getName().compareToIgnoreCase(attribName) == 0) {
                    util.setAttributeOnBusObject(context, busObj, attribName, attribValue);
                }
            }
    ...
    /**
     * @param relID
     *            The relationship object from which the attribute value is queried.
     * @param attribName
     *            The attribute name for which the value is fetched.
     * @param hidden
     *            "true" if hidden attributes are to be searched OR false if hidden a attributes are to be ignored.
     ...
     * @param currentParent
     *            The currentParent on which attributes are set.
     * @param child
     *            The child(old) of the parent which got replaced.
     * @param replaceWith
     *            The replaceWith object on which attributes are set.
     * @param relID
     *            The relationship between replaceWith and the parent.
     * @param oldRelD
     *            The old relationship between parent and old child.
     */
    private void modifyRelBusAttributes(Context context, String currentParent, String child, String replaceWith, Relationship relID, Relationship oldRelD) {
        try {
    ...
    /**
     * @param currentParent
     *            The business object on which we need to perform lock.
     * @param lock
     *            If true , the bus object is locked, if false bus object is unlocked.
     */
    ...
    /**
     * @param child
     *            The child object that gets connected to the parent.
     * @param currentParent
     *            The parent object
     * @param relationShip
     *            The type of relationship like CAD SubComponent
     * @param isFrom
     *            false if "FROM end and true if TO end
     * @return The relationship between the child and the parent.
     */
    public Relationship connectChildWithParent(Context _context, String child, String currentParent, String relationShip, boolean isFrom) {
        ...
        try {
            ...
            rel = util.ConnectBusinessObjects(_context, currentObj, currentParObj, relationShip, isFrom);
    ...
    /**
     * @param child
     *            The child object that will be connected to the parent.
     * @param currentParent
     *            The parent object.
     * @param replaceWith
     *            The object that will replace the child.
     * @param relID
     *            The relationship between replaceWith and the parent.
     * @param isFrom
     *            false for CAD SubComponent relationship.
     * @param lock
     *            true For locking the parent.
     */
    public void lockConnectAndModifyAttribtues(Context _context, String child, String currentParent, String replaceWith, String relID, boolean isFrom, boolean lock) throws MCADException {

        // Lock the parent
        lockUnlockObject(_context, currentParent, lock);

        // Connect:- Create a relationship between the child and the parent.
        Relationship oldRelID = connectChildWithParent(_context, child, currentParent, MCADMxUtil.getActualNameForAEFData(_context, "relationship_CADSubComponent"), isFrom);
        ...
        modifyRelBusAttributes(_context, currentParent, child, replaceWith, newRelID, oldRelID);

    }

    /**
     * This function will replace the current design by the user selected replacewith design This function will be called from the page.
     * @param context
     *            Context
     * @param args
     * @return
    ...
    public Hashtable replace(Context context, String[] args) throws Exception {
        ...
        _generalUtil = new MCADServerGeneralUtil(context, globConf, serverResourceBundle, cache);
        ...
        Hashtable childVsParent = (Hashtable) paramMap.get("ChildVsParentForReplace");
        ...
        while (keys.hasMoreElements()) {
            ...
            for (int par = 0; par < parent.size(); par++) {
                ...
                if (hasLockAccess) {
                    ...
                    Vector relLst = util.findRelationShip(context, child, currentParent, true, "all", MCADMxUtil.getActualNameForAEFData(context, "relationship_CADSubComponent"));
                    ...
                    for (int i = 0; i < relLst.size(); i++) {
                        ...
                        Vector replaceWithChildren = util.getImmParentOrImmChild(context, replaceWith, MCADMxUtil.getActualNameForAEFData(context, "relationship_CADSubComponent"), "to");

                        // Get the list of the parents
                        Vector parentList = util.getImmParentOrImmChild(context, child, MCADMxUtil.getActualNameForAEFData(context, "relationship_CADSubComponent"), "from");

                        for (int k = 0; k < replaceWithChildren.size(); k++) {
                            // Get children of the replacewith if replacewith is
                            // as sub assembly.
                            String replaceChild = (String) replaceWithChildren.get(k);
                            ...
                            if (parentList.contains(replaceChild)) {
                                ...
                                boolean outcome = util.replaceWithRelationshipEnd(context, relID, true, replaceWith);
                                if (outcome == true) {
                                ...
                                // the relationship.
                                lockConnectAndModifyAttribtues(context, child, currentParent, replaceWith, relID, false, true);
                            }
                        }
