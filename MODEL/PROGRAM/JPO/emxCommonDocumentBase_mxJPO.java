 ...
 **
 ** Copyright (c) 2002-2015 Dassault Systemes. All Rights Reserved. This program contains proprietary and trade secret information of MatrixOne, Inc. Copyright notice is precautionary only and does not
...
public class emxCommonDocumentBase_mxJPO extends emxDocument_mxJPO {
     ...
     * @param args
     *            holds no arguments
     * @returns Map objectMap which contains objectId, filename, format pairs and errorMessage if any error.
     * @throws Exception
     *             if the operation fails
     * @since VCP 10.5.0.0
     */
    public Map commonDocumentCheckin(Context context, String[] args) throws Exception {
        try {
            ...
            if (objectAction.equalsIgnoreCase(CommonDocument.OBJECT_ACTION_CREATE_MASTER)) {
                /*
                 * create Master with given 'type' 'name' 'revision' and other attributes and checckin all file selected in second step to the object created. and Crete Version object for each file.
                 */
                ...
                /*
                 * create Master Document with given 'Type' = Document 'name' = autoName 'revision' = default and checckin each file in separate object created. and Crete Version object for each file.
                 */
                ...
                /*
                 * create Document with given 'Type' = Document 'name' = autoName 'revision' = default and checckin each file in separate object created.
                 */
                ...
                /*
                 * Checkin the files into the given object and for each file create version object.
                 */
     ...
     *            holds all arguments passed through checkin screens.
     * @param objectMap
     *            holds all objectDetails for checkin.
     * @returns objectMap which contains objectId, filename, format pairs and errorMessage if any error.
     * @throws Exception
     *             if the operation fails
     * @since VCP 10.5.0.0
     */
    public Map objectCheckin(Context context, HashMap uploadParamsMap, HashMap objectMap) throws Exception {
        try {
            ...
            if (objectAction.equals(CommonDocument.OBJECT_ACTION_CREATE_MASTER) || objectAction.equals(CommonDocument.OBJECT_ACTION_CREATE_MASTER_PER_FILE)
                    || objectAction.equals(CommonDocument.OBJECT_ACTION_UPDATE_MASTER) || objectAction.equals(CommonDocument.OBJECT_ACTION_VERSION_FILE)
                    || objectAction.equals(CommonDocument.OBJECT_ACTION_UPDATE_HOLDER) || objectAction.equals(CommonDocument.OBJECT_ACTION_CHECKIN_WITH_VERSION)
     ...
     *            holds all arguments passed through checkin screens.
     * @param createVersion
     *            boolean to create version objects for master object.
     * @returns Map objectMap which contains objectId, filename, format pairs and errorMessage if any error.
     * @throws Exception
     *             if the operation fails
     * @since VCP 10.5.0.0
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (fileName != null && !"".equals(fileName) && !"null".equals(fileName)) {
                    if (!checkDuplicate(context, uploadParamsMap, fileName, null)) {
                        object.createAndConnect(context, type, name, revision, policy, masterDescription, vault, title, language, parentObject, parentRelName, isFrom, mAttrMap,
     ...
     *            the eMatrix <code>Context</code> object
     * @param objectId
     *            the String Object Id of the Object for Update
     * @param store
     *            String to specify in which store these files need to get checked in
     * @param format
     *            String the specifies in which format the files should get checked in if this is null then file will be checked in to generic format
     * @param fileName
     *            the String fileName moved to the store
     * @param append
     *            String true if these files need to get appended to the existing file false if these files need to over-write existing files
     * @param unlock
     *            String true to unlock this file after checkin false to hold lock on this document
     * @param receiptValue
     *            String specifies the receiptValue of the file moved to store
     ...
     * @since Common 10.0.0.0
     */
    public void multiFileCheckinUpdate(Context context, StringList objectIds, String store, StringList formats, StringList fileNames, StringList appends, StringList unlocks, String receiptValue)
            throws Exception {
        try {
            if (appends != null)
    ...
    public boolean checkDuplicate(Context context, HashMap uploadParamsMap, String fileName, String objectId) throws Exception {
        ...
        if (objectAction.equalsIgnoreCase(CommonDocument.OBJECT_ACTION_UPDATE_MASTER) || objectAction.equalsIgnoreCase(CommonDocument.OBJECT_ACTION_VERSION_FILE)
                || objectAction.equalsIgnoreCase(CommonDocument.OBJECT_ACTION_CHECKIN_WITH_VERSION) || objectAction.equalsIgnoreCase(CommonDocument.OBJECT_ACTION_UPDATE_HOLDER)
    ...
    /**
     * This method is used to initiate subscription notifications. Args Array takes objectid, event, boolean if object is a version
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * This is method is used to purge old version of checked in file this method need to be called from checkin action trigger from any DOCUMENTS type object
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments args[0] should be ${OBJECTID} args[1] should be ${FILENAME_ORIGINAL} These arguments need to be set as eService Program Arguments 1 and 2 in eService Trigger
     *            Program Parameters object.
     * @throws Exception
     *             if the operation fails
     * @since VCP 10.5.0.0
     * @grade 0
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no arguments args[0] is DOCUMENTS or its derived object - objectId to cehckin args[1] is File Path where the file is located in the disk EX: c:\temp\ args[2] is File Name which
     *            need to be checked in given path EX:xyz.doc args[3] is File Format in which this file need to be checked in EX:generic args[4] is store Name where this file should be checked in
     *            EX:STORE args[5] is unlock this should be 'true' or 'false' true to unlock the version object. args[6] is server key word to checkin from from server this should be server when
     *            calling from thinck client this should be client when calling from powerweb args[7] is Comments to the file getting checked in Example to call this method through mql execute program
     *            emxCommonDocument -method checkinBus objectId c:\temp\ tempFile.txt generic STORE false server 'New File Added';
     * @returns nothing
     * @throws Exception
     *             if the operation fails
     */
    ...
    public int checkContentAdded(Context context, String[] args) throws Exception {
        try {
            ...
            if (("True".equalsIgnoreCase(appendFlag) && !checkedInFiles.contains(fileName)) || ("False".equalsIgnoreCase(appendFlag) && checkedInFiles.size() == 0)) {
                return 0;
            } else {
                return 1;
            }
        ...
        try {
            ...
            if (event.equalsIgnoreCase("create") && checkVersionable(context, objectId)) {
                ...
                if (toObjectId != null || "".equalsIgnoreCase(toObjectId)) {
                    ...
                    if ("false".equalsIgnoreCase(Info[1])) {
                        return 1;
                    }
                }
                return 0;
     ...
     * @param args
     *            the parameters for this request. args[0] = objectid of document object.
     */
    ...
    public boolean hasDocumentDownloadAccess(Context context, String args[]) throws Exception {
        ...
        return (CommonDocument.canDownload(context, docObjectInfo) && !(Boolean.valueOf((String) docObjectInfo.get(CommonDocument.SELECT_IS_VERSION_OBJECT))));
    ...
    public boolean hasDocumentCheckinAccess(Context context, String args[]) throws Exception {
        ...
        return (CommonDocument.canCheckin(context, docObjectInfo) && !(Boolean.valueOf((String) docObjectInfo.get(CommonDocument.SELECT_IS_VERSION_OBJECT))).booleanValue()
                && (latestRevision.equalsIgnoreCase(docRevision)) && isAnyFileLockedByContext);
    }
    ...
    /**
     * This method gets called from 'APPFileCheckOutFolderPreference' command to check if applet is on or not and accordingly the command is displayed in the preference.
     * @deprecated since V6R2014x as the command will always be displayed in the preference(IR-253253V6R2014x). This method should be removed in V6R2015.
     */
    @Deprecated
    ...
    /**
     * Trigger to Auto promote the Document during creation conditionally based on a RACE VPLMAutoPromoteNextMinorRev setting. This trigger will check the setting and auto-promote if needed to support
     * "collaborative creation mode".
     * @param context
     *            the Enovia <code>Context</code> object
     * @param args
     *            trigger parameters which include the list of policies to promote against and trigger event. for copy event, additional NEWTYPE, NEWNAME, NEWREV & NEWVAULT must be passed.
     * @throws Exception
     */
    public void autoPromoteCollaborativeMode(Context context, String args[]) throws Exception {
        String autoPrmote = MqlUtil.mqlCommand(context, "list expression $1 select $2 dump", "VPLMAutoPromoteNextMinorRev", "value");
    ...
    /**
     * Check Trigger to block the promotion of locked documents to release state This trigger will check if the document is locked, it wont allow it to be released
     * @param context
     *            the Enovia <code>Context</code> object
     * @param args
     *            trigger parameters which include the OBJECTID.
     * @throws Exception
     */
    public int blockPromotionForLockedDocuments(Context context, String[] args) throws Exception {
        String oid = args[0];
        String strLockedDocError = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(context.getSession().getLanguage()),
                "emxComponents.Common.LockedDocumentAlertMsg");

        int status = 0;

        boolean bHasFiles = hasFiles(context, oid);
        if (!bHasFiles) {
            strLockedDocError = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(context.getSession().getLanguage()), "emxComponents.Common.NoFilesAlertMsg");
        }
    ...
    private boolean hasFiles(Context context, String oid) throws Exception, FrameworkException {
        ...
        if ("false".equalsIgnoreCase(strPromotionValue)) {
            return true;
        }
     ...
     * @throws Exception
     */
    public String getDocumentPromoteConfirmMessage(Context context, String[] args) throws Exception {
        Map info = (Map) JPO.unpackArgs(args);
        String oid = (String) info.get("objectId");
        String relActiveVersion = PropertyUtil.getSchemaProperty(context, DomainObject.SYMBOLIC_relationship_ActiveVersion);
        StringList busSelects = new StringList("from[" + relActiveVersion + "].to.locker");
        DomainObject bus = new DomainObject(oid);
        Map objDetails = bus.getInfo(context, busSelects);
        StringList lockers = (StringList) objDetails.get("from[" + relActiveVersion + "].to.locker");
        boolean isLocked = false;
        String locker;
        StringList lockersList = new StringList();
        if (lockers != null) {
            for (int count = 0; count < lockers.size(); count++) {
                locker = (String) lockers.get(count);
                if (UIUtil.isNotNullAndNotEmpty(locker)) {
                    if (!isLocked) {
                        isLocked = true;
                    }
                    if (!lockersList.contains(locker)) {
        ...
        if (isLocked) {
            String lockerNames = "";
            if (lockersList.size() > 5) {
                lockerNames = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", context.getLocale(), "emxComponents.Common.LockedDocumentPromotionConfirm.MultipleUsers");
            } else {
                for (int count = 0; count < lockersList.size(); count++) {
                    if (count != 0) {
                        lockerNames = lockerNames + ", ";
                    }
                    lockerNames = lockerNames + PersonUtil.getFullName(context, (String) lockersList.get(count));
                }
            }
            String confirmMessage = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", context.getLocale(), "emxComponents.Common.LockedDocumentPromotionConfirm.Message");
            confirmMessage = FrameworkUtil.findAndReplace(confirmMessage, "$<USERNAME>", lockerNames);
    ...
    public void unlockDocumentsForPromotion(Context context, String[] args) throws Exception {
        Map info = (Map) JPO.unpackArgs(args);
        String oid = (String) info.get("objectId");
        try {
            ...
            String relActiveVersion = PropertyUtil.getSchemaProperty(context, DomainObject.SYMBOLIC_relationship_ActiveVersion);
            StringList busSelects = new StringList("from[" + relActiveVersion + "].to.locker");
            busSelects.addElement("from[" + relActiveVersion + "].to.id");

            DomainObject bus = new DomainObject(oid);
            Map objDetails = bus.getInfo(context, busSelects);
            StringList locker = (StringList) objDetails.get("from[" + relActiveVersion + "].to.locker");
            StringList versionObjects = (StringList) objDetails.get("from[" + relActiveVersion + "].to.id");

            if (locker != null) {
                DomainObject versionObject = new DomainObject();
                for (int count = 0; count < locker.size(); count++) {
                    if (UIUtil.isNotNullAndNotEmpty((String) locker.get(count))) {
                        String versionObjId = (String) versionObjects.get(count);
                        versionObject = DomainObject.newInstance(context, versionObjId);
