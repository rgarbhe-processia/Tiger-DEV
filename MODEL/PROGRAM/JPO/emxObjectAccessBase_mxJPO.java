 ...
 * emxObjectAccessBase.
 **
 ** Copyright (c) 2006-2015 Dassault Systemes. All Rights Reserved. This program contains proprietary and trade secret information of MatrixOne, Inc. Copyright notice is precautionary only and does not
...
public class emxObjectAccessBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @grade 0
     */
    public emxObjectAccessBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: objectId - a String containing the concerned object id. pushGrantor - a String containing the name of the user with whom the
     *            push context is done for granting the access. editMode - a String either true or false for deciding the mode. charSet - a String containing the character setting. showAllProgram - a
     *            String containing the program name which is called for deciding if all access grantees are to be shown to context user. showAllFunction - a String containing the function name which
     *            is called for the above purpose. accessChoice - a String containing the comma seperated values of the access rights available.
    ...
    public MapList getAccessGrantees(Context context, String[] args) throws Exception {
        ...
        while (itr.hasNext()) {
            ...
            String cmd = MqlUtil.mqlCommand(context, "print user \"" + user + "\" select isaperson isarole isagroup dump |");
            ...
            } else { // For the user whom only limited view is available
                if ("false".equalsIgnoreCase(editMode)) {
    ...
    public Vector getUserNames(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramList = (HashMap) programMap.get("paramList");
        ...
        for (int k = 0; k < objList.size(); k++) {
            ...
            if ("false".equalsIgnoreCase(editMode) && type.equalsIgnoreCase("Person")) {
                ...
                strBuffName.append("<b><a href=\"javascript:emxTableColumnLinkClick('../common/emxTree.jsp?mode=insert&jsTreeID=" + (String) paramList.get("jsTreeID")
                        + "&emxSuiteDirectory=components&objectId=");
    ...
    public Vector getUserTypes(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
        HashMap paramList = (HashMap) programMap.get("paramList");
        String languageStr = (String) paramList.get("languageStr");
    ...
    public Vector getUserOrganizations(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
        Vector columnVals = new Vector(objList.size());
    ...
    public Vector getUserAccess(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
        HashMap paramList = (HashMap) programMap.get("paramList");
        String languageStr = (String) paramList.get("languageStr");
    ...
    public Vector getUserNewWindow(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objList = (MapList) programMap.get("objectList");
        int objListSize = objList.size();
        HashMap paramList = (HashMap) programMap.get("paramList");
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: requestMap - a HashMap containing the following parameters. languageStr - a String containing the language information.
    ...
    public HashMap getAccessChoices(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            contains a packed HashMap with the following entries: editMode - a String either true or false for deciding the mode.
    ...
    public Boolean checkViewMode(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String editMode = (String) programMap.get("editMode");
    ...
    public Boolean hasLibrarianRole(Context context, String[] args) throws Exception {
        ...
        return Boolean.valueOf(isLibrarian || isVPLMPrjAdmin || isVPLMPrjLeader);
    }
     ...
     *            the eMatrix <code>Context</code> object.
     * @param access
     *            - a String containing the comma seperated values of the access rights.
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            - contains a packed HashMap with the following entries: dataMap - a HashMap containing the user and access information. boList - a BusinessObjectList containing the Business Objects.
     *            accessGrantor - a String representing the access grantor for the push context. charSet - a MapList containing the actual maps "dataMap" containing the data. languageStr - a String
     *            containing the language information. action - a String representing the action to be performed. It can be add, remove or edit
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since CommonComponents 11.0
     ...
     *            the eMatrix <code>Context</code> object.
     * @param userMap
     *            - a HashMap containing the user and access information.
     * @param action
     *            - a String denoting the action taking place ie. add or revoke.
     * @param boList
     *            - a BusinessObjectList containing the business objects passed.
     * @param languageStr
     *            - a String containing the language information.
     * @return void.
     * @throws Exception
     *             if the operation fails.
     * @since CommonComponents 11.0
    ...
    public static void sendAccessGrantNotification(Context context, HashMap userMap, String action, BusinessObjectList boList, String languageStr) throws Exception {
        ...
        while (itr.hasNext()) {
            ...
            String cmd = MqlUtil.mqlCommand(context, "print user \"" + userName + "\" select isaperson isarole isagroup dump |");
            boolean isPerson = "TRUE|FALSE|FALSE".equalsIgnoreCase(cmd);
            boolean isRole = "FALSE|TRUE|FALSE".equalsIgnoreCase(cmd);
            boolean isGroup = "FALSE|FALSE|TRUE".equalsIgnoreCase(cmd);
            String userType = "";
    ...
    /**
     * This method is used to check whether Context user is owner of the business object passed or has any of roles listed as property emxComponents.AccessComponent.ObjectAccess in the
     * emxComponents.properties file
     * @param context
