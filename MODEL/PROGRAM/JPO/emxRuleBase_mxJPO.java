...
public class emxRuleBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    /**
     * This method will inherit Mandatory Rules from Product Line to SubProductLine upon creation of a SubProductLine.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     */
    public void inheritMandatoryRulesToSubProductLine(Context context, String args[]) throws Exception {
        try {
            String prodLineId = args[0];
            String subProdLineId = args[1];
            String relName = PropertyUtil.getSchemaProperty(context, "relationship_BooleanCompatibilityRule");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, subProdLineId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_RuleExtension");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, subProdLineId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_ResourceLimit");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, subProdLineId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_MarketingPreference");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, subProdLineId, relName);
        } catch (Exception e) {
    ...
    /**
     * This method will inherit Mandatory Rules from Product Line to Model upon creation of a Model.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the ${FROMOBJECTID} and ${TOOBJECTID}
     ...
     * @since since Feature Management Module X3
     */
    public void inheritMandatoryRulesToModel(Context context, String args[]) throws Exception {
        try {
            String prodLineId = args[0];
            String modelId = args[1];
            String relName = PropertyUtil.getSchemaProperty(context, "relationship_BooleanCompatibilityRule");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, modelId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_RuleExtension");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, modelId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_ResourceLimit");
            processMandatoryRulesOnChildObjCreate(context, prodLineId, modelId, relName);
            ...
            /*
             * }else{ StringList strProduct = domModel.getAllProductRevisionUnderModel(context); if(strProduct!=null && strProduct.size()>0 ){ for(int i=0;i<strProduct.size();i++){ strPrdMdl[1] =
             * (String) strProduct.get(i); inheritMandatoryRulesToProduct(context,strPrdMdl); } } }
             */
    ...
    /**
     * This method will inherit Mandatory Rules from Model to Product to upon creation of a Product.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the ${FROMOBJECTID} and ${TOOBJECTID}
     ...
     * @since since Feature Management Module X3
     */
    public void inheritMandatoryRulesToProduct(Context context, String args[]) throws Exception {
        try {
            ...
            String relName = PropertyUtil.getSchemaProperty(context, "relationship_BooleanCompatibilityRule");
            processMandatoryRulesOnChildObjCreate(context, modelId, productId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_RuleExtension");
            processMandatoryRulesOnChildObjCreate(context, modelId, productId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_ResourceLimit");
            processMandatoryRulesOnChildObjCreate(context, modelId, productId, relName);
    ...
    /**
     * This method will inherit Mandatory Rules from Product to Product Variant upon creation of a Product.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the ${FROMOBJECTID} and ${TOOBJECTID}
     ...
     * @since since Feature Management Module X3
     */
    public void inheritMandatoryRulesToProductVariantBackgroundProcess(Context context, String args[]) throws Exception {
        try {
            ...
            processMandatoryRulesOnChildObjCreate(context, productId, verId, relName);
            relName = PropertyUtil.getSchemaProperty(context, "relationship_RuleExtension");
    ...
    public void inheritMandatoryRulesToProductVariant(Context context, String args[]) throws Exception {
    ...
    /**
     * This method will inherit all the Mandatory Rules that are connected to Parent Object to its immediate Child Object.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the ${FROMOBJECTID} and ${TOOBJECTID}
     ...
     * @since since Feature Management Module X3
     */
    private void processMandatoryRulesOnChildObjCreate(Context context, String fromId, String toId, String relName) throws Exception {
        ...
        try {
            ...
            String strChildType = childObj.getInfo(context, DomainConstants.SELECT_TYPE);

            List lstObjectSelects = new StringList(DomainConstants.SELECT_ID);
            List lstRelationshipSelects = new StringList(DomainConstants.SELECT_RELATIONSHIP_ID);

            // get all the Feature List objects under the parent
            // Here relName can be Candidate Item or Feature List From
            MapList objRuleList = parentObj.getRelatedObjects(context, relName, DomainConstants.QUERY_WILDCARD, (StringList) lstObjectSelects, (StringList) lstRelationshipSelects, false, true,
            ...
            MapList mapRelIds = configChildObject.getObjectStructure(context, DomainConstants.QUERY_WILDCARD, // Type Pattern
            ...
            for (int i = 0; i < rulesDO; i++) {
                ...
                if (mapRelIdSize > 0) {
                    if (childRuleIdList.contains(domId)) {
                        ...
                        domRel = DomainRelationship.connect(context, childObj, relName, objDom);
                    }
                ...
                /*
                 * DomainRelationship domRel = DomainRelationship.connect(context, childObj, relName, objDom);
                 */

                domRel.openRelationship(context);
                if (!strChildType.equals(ConfigurationConstants.TYPE_PRODUCT_VARIANT)) {
                    domRel.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, "Yes");
                }
                // always set Inherited attribute
                domRel.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_INHERITED, "True");

                domRel.closeRelationship(context, true);

                // added for CR 138-2 -- starts
                String strParentRelId = (String) rulesMap.get(DomainConstants.SELECT_RELATIONSHIP_ID);
                StringList s2RelSelects = new StringList(1);
                s2RelSelects.addElement(DomainRelationship.SELECT_ID);
                Hashtable currentRelData = domRel.getRelationshipData(context, s2RelSelects);
                StringList s2RelIds = (StringList) currentRelData.get(DomainRelationship.SELECT_ID);
                ...
                selectable.append("tomid[").append(relInheritedFrom).append("].id");
    ...
    /**
     * This method will get only the Mandatory Rules from a Maplist of Rule objects.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objRulesList
     ...
     * @since since Feature Management Module X3
     */
    private MapList getOnlyMandatoryRules(Context context, MapList objRulesList) throws Exception {
        try {
            ...
            for (int i = 0; i < ruleObj; i++) {
                ...
                String mRule = objDom.getAttributeValue(context, ConfigurationConstants.ATTRIBUTE_MANDATORYRULE);
                if (mRule.equals("Yes")) {
                    mandRules.add(rMap);
                }
            }
    ...
    public void inheritMandatoryRulesOnModifyAttribute(Context context, String[] args) throws Exception {
        ...
        if (strCreate != null && !strCreate.equalsIgnoreCase("null") && !strCreate.equalsIgnoreCase("")) {
        ...
        if (!isCreate && (strAttrVal != null && !"".equals(strAttrVal) && strAttrVal.equalsIgnoreCase("Yes"))) {
            ...
            StringList slFromObjIds = (StringList) htRelData.get(DomainRelationship.SELECT_FROM_ID);
            StringList slToObjIds = (StringList) htRelData.get(DomainRelationship.SELECT_TO_ID);
        ...
        if (strAttrVal != null && !"".equals(strAttrVal) && strAttrVal.equalsIgnoreCase("No")) {
            ...
            List lstObjectSelects = new StringList(DomainConstants.SELECT_ID);
            MapList childObjList = domfromObj.getRelatedObjects(context, strChildRel, strChildType, (StringList) lstObjectSelects, null, false, true, (short) 1, DomainConstants.EMPTY_STRING,
            ...
            if (mxType.isOfParentType(context, strParentType, ConfigurationConstants.TYPE_PRODUCT_LINE)) {
                MapList childObjListPl = domfromObj.getRelatedObjects(context, strChildRelPl, strChildTypePl, (StringList) lstObjectSelects, null, false, true, (short) 1, DomainConstants.EMPTY_STRING,
                        DomainConstants.EMPTY_STRING, 0);
                ...
                for (int iCnt = 0; iCnt < childObjListPl.size(); iCnt++) {
                    ...
                    MapList relList = domObj.getRelatedObjects(context, strRelType, strRuleType, strObjectSelectablesPV, (StringList) lstRelSelects, false, true, (short) 1, strWhere,
                            DomainConstants.EMPTY_STRING, 0);
                    for (int count = 0; count < relList.size(); count++) {
                        Hashtable htTypeObjMap = (Hashtable) relList.get(count);
                        String strrelId = (String) htTypeObjMap.get(DomainRelationship.SELECT_ID);
                        ...
                        String tomidId = MqlUtil.mqlCommand(context, strMqlCmd1, true, strrelId, selectable.toString(), ConfigurationConstants.DELIMITER_PIPE);
            ...
            try {
                ...
                for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
                    ...
                    for (int count = 0; count < relList.size(); count++) {
                        ...
                        selectable3.append("tomid[").append(relInheritedFrom).append("].id");
                        String tomidId = MqlUtil.mqlCommand(context, strMqlCmd1, true, strrelId, selectable3.toString(), ConfigurationConstants.DELIMITER_PIPE);
                        String strMqlCmd2 = "disconnect connection $1";
                        MqlUtil.mqlCommand(context, strMqlCmd2, true, tomidId);
        ...
        if (mxType.isOfParentType(context, strParentType, ConfigurationConstants.TYPE_PRODUCT_LINE)) {
            ...
            for (int iCnt = 0; iCnt < childObjListPl.size(); iCnt++) {
                ...
                MapList objRuleList = domObj.getRelatedObjects(context, strRelType, strToType, true, true, 1, objSelects, relSelects, "id==" + strRuleId, null, null, null, null);
                ...
                if (objRuleList.size() > 0) {
                    ...
                    relId = (String) objTable.get(DomainConstants.SELECT_RELATIONSHIP_ID); // modified
                    // for
                    // CR
                    // 138-2
                    domRelationship = new DomainRelationship(relId);
                } else {
                    domRelationship = DomainRelationship.connect(context, domObj, strRelType, domRelObj);

                    // added for CR 138-2 -- starts
                    StringList s2RelSelects = new StringList(1);
                    s2RelSelects.addElement(DomainRelationship.SELECT_ID);
                    Hashtable currentRelData = domRelationship.getRelationshipData(context, s2RelSelects);
                    StringList s2RelIds = (StringList) currentRelData.get(DomainRelationship.SELECT_ID);
                ...
                if (!strChildRelPl.equals(ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION)) {
                ...
                String relInheritedFrom = (String) PropertyUtil.getSchemaProperty(context, "relationship_InheritedFrom");
                /*
                 * String strMqlCmd2 = "add connection \"" + relInheritedFrom + "\" fromrel \"" + strRlId + "\" torel \"" + relId;
                 */
                String strMqlCmd2 = "add connection $1 fromrel $2 torel $3";
        ...
        for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
            ...
            if (!strChildRel.equals(ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION)) {
                domRelationship.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, "Yes");
            }
            domRelationship.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_INHERITED, "True");
            domRelationship.closeRelationship(context, true);
     ...
     *             if the operation fails
     */
    public Map getRangeValuesForMandatoryRuleAttribute(Context context, String[] args) throws Exception {
        ...
        List attributeDisplayRange = i18nNow.getAttrRangeI18NStringList(ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, (StringList) attributeRange, context.getSession().getLanguage());
     ...
     */
    public void updateValuesForMandatoryRuleAttribute(Context context, String[] args) throws Exception {
        ...
        if (strRelId != null && !"".equals(strRelId)) {
            ...
            domRel.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, strNewAttribval);
        }
    }
     ...
     * @throws Exception
     */
    public String getTNRForObject(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        sbTemp = sbTemp.append(i18ObjectType).append(FEATURE_TNR_DELIMITER).append((String) mapInfo.get(DomainConstants.SELECT_NAME)).append(FEATURE_TNR_DELIMITER)
     ...
     * @throws Exception
     */
    public String getMarketingName(Context context, String[] args) throws Exception {
    ...
    public Map getRangeValuesForRuleClassificationAttribute(Context context, String[] args) throws Exception {
        ...
        List attributeDisplayRange = i18nNow.getAttrRangeI18NStringList(ConfigurationConstants.ATTRIBUTE_RULE_CLASSIFICATION, (StringList) attributeRange, context.getSession().getLanguage());
        rangeMap.put(FIELD_CHOICES, attributeRange);
        rangeMap.put(FIELD_DISPLAY_CHOICES, attributeDisplayRange);
    ...
    /**
     * This method will be executed only when the Scenario is Edit of the Configurable Rule. This is a background job method.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
    ...
    public void ruleInheritanceOnEdit(Context context, String args[]) throws Exception {
        try {
            ...
            if (mxType.isOfParentType(context, strParentType, ConfigurationConstants.TYPE_PRODUCT_LINE)) {
                ...
                strChildRel = ConfigurationConstants.RELATIONSHIP_SUB_PRODUCT_LINES + "," + ConfigurationConstants.RELATIONSHIP_PRODUCTLINE_MODELS + "," + ConfigurationConstants.RELATIONSHIP_PRODUCTS
            ...
            MapList childObjList = dom.getRelatedObjects(context, strChildRel, strChildType, (StringList) lstObjectSelects, relSelects, false, true, (short) 0, DomainConstants.EMPTY_STRING,
                    DomainConstants.EMPTY_STRING, 0);
            ...
            for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
                ...
                MapList relList = domObj.getRelatedObjects(context, strRelType, strRuleType, (StringList) lstObjectSelects, (StringList) lstRelSelects, false, true, (short) 1, strWhere,
                        DomainConstants.EMPTY_STRING, 0);
                ...
                if (relList.size() == 0) {
                    ...
                    // added for CR 138-2 -- starts
                    /*
                     * String strMqlCmd2 = "add connection \"" + relInheritedFrom + "\" fromrel \"" + strRuleRelId + "\" torel \"" + relId; MqlUtil.mqlCommand(context, strMqlCmd2, true);
                     */
                    String strMqlCmd2 = "add connection $1 fromrel $2 torel $3";
                    ...
                    for (int j = 0; j < relList.size(); j++) {
                        ...
                        MapList MlRelIds = domObj.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_BOOLEAN_COMPATIBILITY_RULE, // String relPattern
                        ...
                        if (subTypes.contains(strchildType) && strChildRelsForEdit.contains(strchildRel) && MlRelIds.isEmpty())
                        // )
                        {
                            // relId = (String) htChMap.get(DomainRelationship.SELECT_ID);
    ...
    /**
     * This method will be executed only when the Scenario is Create of the Configurable Rule. This is a background job method.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
    ...
    public void ruleInheritanceOnCreate(Context context, String args[]) throws Exception {
        try {
            ...
            if (mxType.isOfParentType(context, strContextObjType, ConfigurationConstants.TYPE_PRODUCT_LINE)) {
                ...
                strChildType = ConfigurationConstants.TYPE_PRODUCTS;
                strChildRel = ConfigurationConstants.RELATIONSHIP_PRODUCTS + "," + ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION;
            } else if (mxType.isOfParentType(context, strContextObjType, ConfigurationConstants.TYPE_PRODUCTS) && !strContextObjType.equals(ConfigurationConstants.TYPE_PRODUCT_VARIANT)) {
                strChildType = ConfigurationConstants.TYPE_PRODUCTS;
                strChildRel = ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION;
            }
            ...
            for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
                ...
                // added for CR 138-2 -- starts
                /*
                 * String strMqlCmd2 = "add connection \"" + relInheritedFrom + "\" fromrel \"" + strRelIdBCRContext + "\" torel \"" + relId; MqlUtil.mqlCommand(context, strMqlCmd2, true);
                 */
    ...
    /**
     * This is a Access function for Resource Usage command under the Resources Rules added to fix IR -- Mx377985
     * @param context
     * @param args
     * @return boolean - false if the parent object is Product Line or Model - true if the above condition doesn't satisfy.
     * @throws Exception
     * @author IVU
     */
    ...
    /**
     * Method to return a string in the <Marketing Name Revision > from an Object id. Added for IR Mx376196
     * @param context
     * @param args
     * @return
     * @throws Exception
     */
    public String getMarketingNameRev(Context context, String[] args) throws Exception {
        ...
        strMarketingName = strMarketingName + " " + (String) contextNameMap.get(ConfigurationConstants.SELECT_REVISION);

        return strMarketingName;
    }

    public void updateExpressionAttributes(Context context, String args[]) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String strRuleId = (String) programMap.get("ruleId");
        DomainObject objRule = new DomainObject(strRuleId);
        String strRuleRightExp = getExpression(context, ConfigurationConstants.RELATIONSHIP_RIGHT_EXPRESSION, strRuleId);
        String strRuleLeftExp = getExpression(context, ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION, strRuleId);
        objRule.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION, strRuleLeftExp);
        objRule.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION, strRuleRightExp);

    }

    public String getExpression(Context context, String strType, String strIncRuleID) throws Exception {
        ...
        if (strType.equalsIgnoreCase(ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION)) {
            programMap.put("strRelType", ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION);
        } else {
            programMap.put("strRelType", ConfigurationConstants.RELATIONSHIP_RIGHT_EXPRESSION);
        }
        programMap.put("strRuleDisplay", ConfigurationConstants.RULE_DISPLAY_FULL_NAME);
        arrJPOArguments = JPO.packArgs(programMap);

        StringList strRightExpressionList = new StringList();
        strRightExpressionList = (StringList) (JPO.invoke(context, "emxBooleanCompatibility", null, "getExpression", arrJPOArguments, StringList.class));
    ...
    /**
     * Method call to get the Edit Link for Rule Extension Summary Page. @param context the eMatrix <code>Context</code> object @param args - Holds the following arguments 0 - HashMap containing the
     * following arguments: 1:Relationship Id. 2:Feature Id. @return Object - MapList containing the context name and ids. @throws Exception if the operation fails @author IXH @since R212 @category
     */
    public Vector getEditColumnDisplayInTable(Context context, String[] args) throws Exception {
        ...
        while (i.hasNext()) {
            ...
            if (!strType.equals(ConfigurationConstants.TYPE_PRODUCT_COMPATIBILITY_RULE)) {
                ...
                /*
                 * strMqlCmd = "expand bus \"" + parentId + "\" from relationship \"" + strRelationShip + "\" select rel id where to.id=='" + id + "' dump | ";
                 * 
            ...
            if (strType.equals(ConfigurationConstants.TYPE_RULE_EXTENSION)) {
                strEditlink = "<a href=\"javascript:emxTableColumnLinkClick('../common/emxForm.jsp?form=FTRRuleExtensionForm%26mode=edit%26formHeader=emxProduct.Heading.RuleExtension.EditRule%26HelpMarker=emxhelpruleextensionedit%26suiteKey=Configuration%26StringResourceFileId=emxConfigurationStringResource%26SuiteDirectory=configuration%26submitAction=doNothing%26postProcessURL=../configuration/RuleExtensionCreateEditPostProcess.jsp%26relId="
                        + XSSUtil.encodeForHTMLAttribute(context, relId) + "%26suiteKey=configuration%26SuiteDirectory=configuration%26parentOID=" + XSSUtil.encodeForHTMLAttribute(context, parentId)
                        + "%26objectId=" + XSSUtil.encodeForHTMLAttribute(context, id) + "', '800', '700', 'true', 'slidein', '')\"><img border=\"0\" src=\"images/iconActionEdit.gif\" alt=\"\"/></a>";
            } else if (strType.equals(ConfigurationConstants.TYPE_MARKETING_PREFERENCE)) {
                strEditlink = "<a href=\"javascript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?modetype=edit%26commandName=FTRMarketingPreferenceRuleSettings%26ruleType=MarketingPreferenceRule%26SuiteDirectory=configuration%26relId="
                        + XSSUtil.encodeForHTMLAttribute(context, relId) + "%26suiteKey=configuration%26parentOID=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "%26objectId="
                        + XSSUtil.encodeForHTMLAttribute(context, id) + "%26submitURL=" + "../configuration/MPREditPostProcess.jsp?mode=edit%26ruleType=MarketingPreferenceRule"
                ...
                strEditlink = "<a href=\"javascript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?modetype=edit%26commandName=FTRBooleanCompatibilityRuleSettings%26ruleType=BooleanCompatibilityRule%26SuiteDirectory=configuration%26relId="
                        + XSSUtil.encodeForHTMLAttribute(context, relId) + "%26suiteKey=configuration%26parentOID=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "%26objectId="
                        + XSSUtil.encodeForHTMLAttribute(context, id) + "%26submitURL=" + "../configuration/BCREditPostProcess.jsp?mode=edit%26ruleType=BooleanCompatibilityRule"
                        + "', '800', '700', 'true', 'popup', '','','false','Large')\"><img border=\"0\" src=\"images/iconActionEdit.gif\" alt=\"\"/></a>";
            } else if (strType.equals(ConfigurationConstants.TYPE_PRODUCT_COMPATIBILITY_RULE)) {
                strEditlink = "<a href=\"javascript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?modetype=edit%26commandName=FTRProductCompatibilityRuleSettings%26ruleType=ProductCompatibilityRule%26SuiteDirectory=configuration%26relId="
                        + XSSUtil.encodeForHTMLAttribute(context, relId) + "%26suiteKey=configuration%26parentOID=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "%26objectId="
                        + XSSUtil.encodeForHTMLAttribute(context, id) + "%26submitURL=" + "../configuration/PCREditPostProcess.jsp?mode=edit%26ruleType=ProductCompatibilityRule"
                        + "', '800', '700', 'true', 'popup', '','','false','Large')\"><img border=\"0\" src=\"images/iconActionEdit.gif\" alt=\"\"/></a>";
                ...
                strEditlink = "<a href=\"javascript:emxTableColumnLinkClick('../configuration/CreateRuleDialog.jsp?modetype=edit%26commandName=FTRQuantityRuleSettings%26ruleType=QuantityRule%26SuiteDirectory=configuration%26relId="
                        + XSSUtil.encodeForHTMLAttribute(context, relId) + "%26suiteKey=configuration%26parentOID=" + XSSUtil.encodeForHTMLAttribute(context, parentId) + "%26objectId="
                        + XSSUtil.encodeForHTMLAttribute(context, id) + "%26submitURL=" + "../configuration/QuantityRuleEditPostProcess.jsp?mode=edit%26ruleType=QuantityRule"
                        + "', '800', '700', 'true', 'popup', '','','false','Large')\"><img border=\"0\" src=\"images/iconActionEdit.gif\" alt=\"\"/></a>";
            } else if (strType.equals(ConfigurationConstants.TYPE_FIXED_RESOURCE)) {
    ...
    /**
     * Method call to get the "Mandatory Column" in Rule list Page. @param context the eMatrix <code>Context</code> object @param args - @return Object -Vector containing Mandatory Value Yes or
     * No. @throws Exception if the operation fails @author IXH @since R212 @category
     */
    ...
    public Vector getMandatoryColumnInTable(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < lstObjectIdsList.size(); i++) {
            ...
            // TO DO:MQL command
            /*
             * strMqlCmd = "expand bus \"" + strObjId1 + "\" from relationship \"" + strRel + "\" select rel id where to.id=='" + strObjId + "' dump | ";
             * 
             * String strRelId = MqlUtil.mqlCommand(context, strMqlCmd, true);
             */
            strMqlCmd = "expand bus $1 from relationship $2 select $3 $4 where $5 dump $6 ";
            ...
            String strAttrib = (String) domRel.getAttributeValue(context, ConfigurationConstants.ATTRIBUTE_MANDATORYRULE);
            if (strAttrib == null) {
                strAttrib = "";
            }
    ...
    /**
     * Method call to get the "Inherited From" in Rule list Page. @param context the eMatrix <code>Context</code> object @param args - @return Object - StringList containing the Inherited
     * Values @throws Exception if the operation fails @author IXH @since R212 @category
     */

    public StringList getInheritedFromColumnInTable(Context context, String[] args) throws Exception {
        ...
        while (i.hasNext()) {
            Map m = (Map) i.next();
            String relId = (String) m.get(DomainConstants.SELECT_RELATIONSHIP_ID);
            ...
            while (true) {
                ...
                iCnt = iCnt + 1;
                /*
                 * String strMqlCmd1 = "print connection \"" + itrRelId + "\" select tomid[" + relInheritedFrom + "].id dump | "; tomidId = MqlUtil.mqlCommand(context, strMqlCmd1, true);
                 */
                String strMqlCmd1 = "print connection $1 select $2 dump $3 ";
                StringBuffer selectable = new StringBuffer();
                selectable.append("tomid[").append(relInheritedFrom).append("].id");
                ...
                if (null == tomidId || "null".equals(tomidId) || "".equals(tomidId)) {
                    ...
                    /*
                     * String strMqlCmd2 = "print connection \"" + tomidId + "\" select fromrel[" + relType + "].id dump"; itrRelId = MqlUtil.mqlCommand(context, strMqlCmd2, true);
                     */
                    String strMqlCmd2 = "print connection $1 select $2 dump";
                    StringBuffer selectable2 = new StringBuffer();
                    selectable2.append("fromrel[").append(relType).append("].id");
    ...
    /**
     * Method call to get the "Inherited" in Rule list Page. @param context the eMatrix <code>Context</code> object @param args - @return Object - StringList containing the Inherited Values @throws
     * Exception if the operation fails @author IXH @since R212 @category
     */
    public Vector getInheritedColumnInTable(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < lstObjectIdsList.size(); i++) {
            ...
            // TO DO
            /*
             * strMqlCmd = "expand bus \"" + strObjId + "\" to relationship \"" + strRel + "\" select rel id where from.id=='" + strObjId1 + "' dump | ";
             * 
             * String strRelId = MqlUtil.mqlCommand(context, strMqlCmd, true);
             */
            ...
            DomainRelationship domRel = new DomainRelationship(strRelId);
            String strAttrib = (String) domRel.getAttributeValue(context, ConfigurationConstants.ATTRIBUTE_INHERITED);
            if (strAttrib == null) {
                strAttrib = "";
            }
    ...
    public List getDesignResponsibilityEditInTable(Context context, String[] args) throws Exception {
        ...
        while (objectListItr.hasNext()) {
            ...
            /*
             * If RDO is set for this object then check whether the context user has read access on the RDO object. If yes then hyperlink the RDO name to its properties page otherwise return the RDO
             * name. If the mode is edit , display the design responsibility field as textbox with a chooser
             */
            ...
            if (strFromWhere != null && strFromWhere.equalsIgnoreCase("EditTable")) {
                ...
                if (strRDOName != null && !strRDOName.equals("") && !strRDOName.equalsIgnoreCase("null")) {
                    ...
                    if (bHasReadAccess) {
                        ...
                        /*
                         * sbBuffer.append("&nbsp;"); sbBuffer.append("<A HREF=\"javascript:showDetailsPopup('../common/emxTree.jsp?objectId="); sbBuffer.append(strRDOId);
                         * sbBuffer.append("&amp;mode=replace"); sbBuffer.append("&amp;AppendParameters=true"); sbBuffer.append("&amp;reloadAfterChange=true");
                         * sbBuffer.append("')\"class=\"object\">"); sbBuffer.append("</A>");
                         */
    ...
    public void updateMandatoryAttr(Context context, String[] args) throws FrameworkException {
        try {
            ...
            // IR-151491V6R2013
            String mandRuleAttibute = (String) requestMap.get(ConfigurationConstants.MANDATORY_RULE_ATTRIBUTE_NAME);
            ...
            DomainRelationship domresoLimitRel = new DomainRelationship(resourceLimitRel);
            domresoLimitRel.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, mandRuleAttibute);
     ...
     * @throws Exception
     */
    public void emptyUpdateProgramForRule(Context context, String[] args) throws Exception {
        // DO Nothing
    }
     ...
     * @throws Exception
     */
    public String getDefaultValueForMandatoryRuleAttribute(Context context, String[] args) throws Exception {
        String strAttributeName = ConfigurationConstants.ATTRIBUTE_MANDATORYRULE;
        matrix.db.AttributeType attribName = new matrix.db.AttributeType(strAttributeName);
        attribName.open(context);
    ...
    public void disconnectInheritedFromRuleIds(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String strComma = ",";
            String strRelPattern = ConfigurationConstants.RELATIONSHIP_RULE_EXTENSION + strComma + ConfigurationConstants.RELATIONSHIP_RESOURCE_LIMIT + strComma
                    + ConfigurationConstants.RELATIONSHIP_BOOLEAN_COMPATIBILITY_RULE + strComma + ConfigurationConstants.RELATIONSHIP_MARKETING_PREFERENCE;
            ...
            for (int iCnt = 0; iCnt < programArray.size(); iCnt++) {
                ...
                mapRelIds = confUtil.getObjectStructure(context, DomainConstants.QUERY_WILDCARD, // Type Pattern
                ...
                while (relIdsItr.hasNext()) {
                    ...
                    domRel.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_INHERITED, "False");
                    /*
                     * String strMqlCmd1 = "print connection \"" + relationshipId + "\" select tomid[" + relInheritedFrom + "].id dump | "; tomidId = MqlUtil.mqlCommand(context, strMqlCmd1, true);
                     */
                    String strMqlCmd1 = "print connection $1 select $2 dump $3";
                    StringBuffer selectable = new StringBuffer();
                    selectable.append("tomid[").append(relInheritedFrom).append("].id");
    ...
    /**
     * This method is called on create Left/Right Expression relationship as action trigger,this will check Varies By Relationship and Configuration Type attribute on Varies By Relationship.
     * @param context
     *            The eMatrix Context object
     * @param args
     *            This will be the Macro {FROMOBJECTID} {FROMTYPE} {TOOBJECTID} for Create Action Trigger for Left/Right Expression.
     * @throws Exception
     * @since R216
     */
    public void updateConfigurationTypeAttributeForVBRel(Context context, String args[]) throws Exception {
        try {
            String ToObjectId = args[2];

            // 1.Check If TO side is Relationship Object
            if (ProductLineCommon.isNotNull(ToObjectId) && !new DomainObject(ToObjectId).exists(context)) {
                ...
                MapList domRelMapList = domRelId.getInfo(context, relationshipIds, relSelects);
                Map domRelMap = (Map) domRelMapList.get(0);

                // Check If Type and Relationship Attribute is present in Map
                if (domRelMap.containsKey(DomainRelationship.SELECT_TYPE) && domRelMap.containsKey(ConfigurationConstants.SELECT_ATTRIBUTE_CONFIGURATION_TYPE)) {
                    // 2.Check If Relationship is Varies By And Check If
                    // Configuration Type Attribute On Varies By Relationship Is
                    // System
                    if (ConfigurationUtil.isKindOfRel(context, (String) domRelMap.get(DomainRelationship.SELECT_TYPE), ConfigurationConstants.RELATIONSHIP_VARIES_BY)
                            && ((String) domRelMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_CONFIGURATION_TYPE)).equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_SYSTEM)) {
