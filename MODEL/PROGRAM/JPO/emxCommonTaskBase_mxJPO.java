...
/**
 * The <code>emxTaskBase</code> class represents the Task JPO functionality for the AEF type.
...
public class emxCommonTaskBase_mxJPO extends com.matrixone.apps.common.Task {
    ...
    public static final String STATE_PROJECT_TASK_ASSIGN = PropertyUtil.getSchemaProperty("policy", POLICY_PROJECT_TASK, "state_Assign");

    /** state "Active" for the "Project Task" policy. */
    public static final String STATE_PROJECT_TASK_ACTIVE = PropertyUtil.getSchemaProperty("policy", POLICY_PROJECT_TASK, "state_Active");

    /** state "Review" for the "Project Task" policy. */
    public static final String STATE_PROJECT_TASK_REVIEW = PropertyUtil.getSchemaProperty("policy", POLICY_PROJECT_TASK, "state_Review");
    ...
    /** state "Complete" for the "Project Task" policy. */
    public static final String STATE_BUSINESS_GOAL_CREATED = PropertyUtil.getSchemaProperty("policy", POLICY_BUSINESS_GOAL, "state_Created");

    /** state "Complete" for the "Project Task" policy. */
    public static final String STATE_BUSINESS_GOAL_ACTIVE = PropertyUtil.getSchemaProperty("policy", POLICY_BUSINESS_GOAL, "state_Active");

    /*
     * Added:nr2:PRG:R210:For Project Hold and Cancel Highlight
     */

    static protected final String POLICY_PROJECT_SPACE_HOLD_CANCEL = PropertyUtil.getSchemaProperty("policy_ProjectSpaceHoldCancel");

    /** The parent type of the task. */
    static protected final String SELECT_SUBTASK_TYPE = "to[" + RELATIONSHIP_SUBTASK + "].from.type";
     ...
     * @param args
     *            an array of String arguments for this method
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.1.1
     */
    public emxCommonTaskBase_mxJPO(Context context, String[] args) throws Exception {
     ...
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            - args contains the Ids of Context Task Object, Document object and RelationshipId
    ...
    public int checkStateAndUser(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String strTaskPolicy = PropertyUtil.getSchemaProperty(context, "policy_ProjectTask");
            // Getting and representing the state 'Create'
            String strActualStateName = FrameworkUtil.lookupStateName(context, strTaskPolicy, "state_Active");
            ...
            if (!strCurrentState.equals(strActualStateName)) {
                strError = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", strLocale, "emxComponents.WBSTask.CheckActiveState");
                emxContextUtil_mxJPO.mqlNotice(context, strError);
            } else {

                String currentUserId = com.matrixone.apps.common.Person.getPerson(context, context.getUser()).getObjectId();
                ...
                MapList taskAssignee = dmObj.getRelatedObjects(context, RELATIONSHIP_ASSIGNED_TASKS, QUERY_WILDCARD, true, false, 1, busSelects, null, busWhere, null, null, null, null);

                if (taskAssignee != null && taskAssignee.size() > 0) {
                    iFlag = 0;
                } else {
                    strError = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", strLocale, "emxComponents.WBSTask.CheckTaskAssignee");
                    emxContextUtil_mxJPO.mqlNotice(context, strError);
                }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - [PROJECT_MEMBER|PROJECT_LEAD|TASK_ASSIGNEE|TASK_OWNER] <BR>
     ...
     *            TASK_ASSIGNEE to see if the context user is an assignee of this task, <BR>
     *            TASK_OWNER to see if the context user is an owner of any of the direct parent tasks
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.1.0
     * @grade 0
    ...
    public boolean hasAccess(Context context, String args[]) throws Exception {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (accessListObject == null) {
                ...
                if (accessListObject != null) {
                    String output = MqlUtil.mqlCommand(context, "print bus " + accessListObject.getId() + " select from[Project Access Key].to.id dump |");
            ...
            } else if ("PROJECT_MEMBER".equals(accessType) || "PROJECT_LEAD".equals(accessType) || "PROJECT_OWNER".equals(accessType)) {
                if (accessListObject != null) {
                    int iAccess;
     ...
     * @return true if the person has either "Supplier Quality Engineer" or "Supplier Representative" role or both.
     * @throws Exception
     *             if the operation fails
     * @since AEF 10.5.0.0
     * @grade 0
     */
    public boolean hasSupplierRoles(Context context, String args[]) throws Exception {
    ...
    /**
     * When the task is promoted this function is called. Depending on which state the promote is triggered from it performs the necessary actions based on the arg-STATENAME value Note: object id must
     * be passed as first argument.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            OBJECTID STATENAME NEXTSTATE
     * @throws Exception
     *             if operation fails
     * @since AEF 9.5.1.3
     * @grade 0
     */
    public void triggerPromoteAction(Context context, String[] args) throws Exception {
        ...
        if (fromState.equals(STATE_PROJECT_TASK_CREATE)) {
            ...
            if (utsList.size() > 0) {
                ...
                while (itr.hasNext()) {
                    ...
                    if (taskPromoteToPosition != -1 && taskCurrentPosition < taskPromoteToPosition) {
             ...
             * 
             * 
             * ContextUtil.pushContext(context); try { com.matrixone.apps.program.BusinessGoal businessGoal = (com.matrixone.apps.program.BusinessGoal) DomainObject.newInstance(context,
             * DomainConstants.TYPE_BUSINESS_GOAL, DomainConstants.PROGRAM); com.matrixone.apps.program.ProjectSpace project = (com.matrixone.apps.program.ProjectSpace)
             * DomainObject.newInstance(context, DomainConstants.TYPE_PROJECT_SPACE,DomainConstants.PROGRAM); project.setId(objectId); MapList businessGoalList = new MapList(); businessGoalList =
             * businessGoal.getBusinessGoals(context, project, busSelects, null); if (null != businessGoalList && businessGoalList.size()>0){ Iterator businessGoalItr = businessGoalList.iterator();
             * while(businessGoalItr.hasNext()) { Map businessGoalMap = (Map) businessGoalItr.next(); String businessGoalId = (String) businessGoalMap.get(businessGoal.SELECT_ID); String
             * businessGoalState = (String) businessGoalMap.get(businessGoal.SELECT_CURRENT); businessGoal.setId(businessGoalId); if(fromState.equals(STATE_PROJECT_TASK_ASSIGN) &&
             * businessGoalState.equals(STATE_BUSINESS_GOAL_CREATED)){ businessGoal.changeTheState(context); } //ends if }//ends while }//ends if }//ends try catch (Exception e) { DebugUtil.debug(
             * "Exception Task triggerPromoteAction- ", e.getMessage()); throw e; } finally { ContextUtil.popContext(context); }
             */
             ...
             * businessGoal.getBusinessGoals(context, project, busSelects, null); if (null != businessGoalList && businessGoalList.size()>0){ Iterator businessGoalItr = businessGoalList.iterator();
             * while(businessGoalItr.hasNext()) { Map businessGoalMap = (Map) businessGoalItr.next(); String businessGoalId = (String) businessGoalMap.get(businessGoal.SELECT_ID); String
             * businessGoalState = (String) businessGoalMap.get(businessGoal.SELECT_CURRENT); businessGoal.setId(businessGoalId); if(fromState.equals(STATE_PROJECT_TASK_REVIEW) &&
             * businessGoalState.equals(STATE_BUSINESS_GOAL_ACTIVE)){ MapList projectList = businessGoal.getProjects(context, busSelects, null); boolean changeState = true; if (null != projectList &&
             * projectList.size()>0){ Iterator projectItr = projectList.iterator(); while(projectItr.hasNext()) { Map projectMap = (Map) projectItr.next(); String projectId = (String)
             * projectMap.get(project.SELECT_ID); String projectState = (String) projectMap.get(project.SELECT_CURRENT); if(!projectState.equals(STATE_PROJECT_TASK_COMPLETE) &&
             * !projectState.equals(STATE_PROJECT_TASK_ARCHIVE) && !projectId.equals(objectId)){ changeState = false; } } if(changeState){ businessGoal.changeTheState(context); }//ends if }//ends if
             * }//ends if }//ends while }//ends if }//ends try catch (Exception e) { DebugUtil.debug("Exception Task triggerPromoteAction- ", e.getMessage()); throw e; } finally {
             * ContextUtil.popContext(context); }
             */
            ...
            if (parentList.size() > 0) {
                ...
                if (state.equals(STATE_PROJECT_TASK_ACTIVE)) {
                    ...
                    setId(parentId);
                    boolean checkPassed = checkChildrenStates(context, STATE_PROJECT_TASK_COMPLETE, null);
                    ...
                    if (checkPassed) {
                        ...
                        // state list
                        int taskPromoteToPosition = parentStateList.indexOf(STATE_PROJECT_TASK_REVIEW);
                        if (parentType.equals(TYPE_PART_QUALITY_PLAN)) {
                            taskPromoteToPosition = parentStateList.indexOf(STATE_PART_QUALITY_PLAN_COMPLETE);

                        }
                        // Review state exists and is the state next to Active.
                        // Promote the parent
                        if (taskPromoteToPosition != -1 && taskPromoteToPosition == (taskCurrentPosition + 1)) {
                            // use super user to overcome access issue
                            ContextUtil.pushContext(context);
                            try {
                                setId(parentId);
    ...
    /**
     * When the task is demoted this function is called. Depending on which state the promote is triggered from it performs the necessary actions based on the arg-STATENAME value
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            OBJECTID STATENAME NEXTSTATE
     ...
     * @grade 0
     */
    public void triggerDemoteAction(Context context, String[] args) throws Exception {
                ...
                if (state.equals(STATE_PROJECT_TASK_ASSIGN)) {
                    ...
                    if (utsList.size() > 0) {
                        ...
                        while (itr.hasNext()) {
                            ...
                            int taskAssignStatePosition = taskStateList.indexOf(STATE_PROJECT_TASK_ASSIGN);

                            // if the current task position is less then the
                            // taskAssignStatePosition, demote the task
                            if (taskAssignStatePosition == -1 || taskCurrentPosition >= taskAssignStatePosition) {
                                demoteParent = false;
                                break;
                            }
                        }
                ...
                if (state.equals(STATE_PROJECT_TASK_ACTIVE) && !TYPE_PART_QUALITY_PLAN.equals(type)) {
                    ...
                    if (utsList.size() > 0) {
                        ...
                        while (itr.hasNext()) {
                            ...
                            // the parent task
                            if (taskActiveStatePosition != -1 && taskCurrentPosition >= taskActiveStatePosition) {
                                demoteParent = false;
                                break;
                            }
                        }
     ...
     *            OBJECTID
     * @param args
     *            sideDoorFeature. True if URL needs to sent in notification
     * @throws Exception
     *             if operation fails
     * @since AEF 9.5.1.3
     * @grade 0
    ...
    public void notifyTaskAssignees(Context context, String[] args) throws Exception {
        ...
        String taskEstStartDate = (String) objMap.get(SELECT_TASK_ESTIMATED_START_DATE);
        String taskEstFinishDate = (String) objMap.get(SELECT_TASK_ESTIMATED_FINISH_DATE);
        String taskEstDuration = (String) objMap.get(SELECT_TASK_ESTIMATED_DURATION);
        ...
        if (assigneeList.size() > 0) {
            ...
            sMailSubject = mailUtil.getMessage(context, sMailSubject, null, null, companyName);

            // get the mail message
            String sMailMessage = FrameworkStringResource.WBS_Task_Notify_Message;
            String mKey[] = { "TaskType", "TaskName", "TaskEstStartDate", "TaskEstFinishDate", "TaskEstDuration" };
            String mValue[] = { taskType, taskName, taskEstStartDate, taskEstFinishDate, taskEstDuration };
            sMailMessage = mailUtil.getMessage(context, sMailMessage, mKey, mValue, companyName);

            MailUtil.setAgentName(context, context.getUser());

            MailUtil.sendMessage(context, mailToList, mailCcList, null, sMailSubject, sMailMessage, objectIdList);
        }
    ...
    /**
     * This function modifies the attributes Sets the actual start date on promoting a task to Active state Resets the actual start date on demoting a task from Active state Sets the actual completion
     * date on promoting a task to Complete state Resets the completion date on demoting a task from Complete state
     * @param context
     *            the eMatrix <code>Context</code> object
     * @throws Exception
     *             if operation fails
    ...
    public void triggerModifyAttributes(Context context, String[] args) throws Exception {
        ...
        if (sEvent.equals("Promote")) {
            if (toState.equals(STATE_PROJECT_TASK_ACTIVE)) {
                ...
                attributes.put(ATTRIBUTE_TASK_ACTUAL_DURATION, duration.toString());
    ...
    /**
     * This function checks the states of the child tasks have reached the state defined in the trigger object Parameters arg[0] = objectId arg[1] = relationship to check for arg[2] = state to check
     * for
     * @param context
     *            the eMatrix <code>Context</code> object
     * @throws Exception
     ...
     * @grade 0
     */
    public int triggerCheckChildrenStates(Context context, String[] args) throws Exception {
        // get values from args.
        String objectId = args[0];
        ...
        String checkState = PropertyUtil.getSchemaProperty(context, "policy", POLICY_PROJECT_TASK, checkStateArg);
     ...
     *            the eMatrix <code>Context</code> object
     * @param targetState
     *            the state name to compare against
     * @param whereClause
     *            optional where clause to apply against the children
     * @return true if all children are at least at the target state; false if any of the children do not meet the target state requirement.
     * @throws Exception
     *             if operation fails
     * @since AEF 9.5.1.3
     * @grade 0
     */
    protected boolean checkChildrenStates(Context context, String targetState, String whereClause) throws Exception {
        // get children one level only
        StringList busSelects = new StringList(2);
        busSelects.add(SELECT_CURRENT);
        busSelects.add(SELECT_STATES);

        MapList utsList = getRelatedObjects(context, RELATIONSHIP_SUBTASK, QUERY_WILDCARD, busSelects, null, // relationshipSelects
                false, // getTo
                true, // getFrom
    ...
    /**
     * Sets the %age Completion based on whether the event is promote or demote and the state on which this event occurs Parameters arg[0] = objectId arg[1] = fromState arg[2] = toState
     * @param context
     *            the eMatrix <code>Context</code> object
     * @throws Exception
     *             if operation fails
     ...
     * @grade 0
     */
    public void triggerSetPercentageCompletion(Context context, String[] args) throws Exception {
        // get values from args.
        String objectId = args[0];
        String fromState = args[1];
        String toState = args[2];
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - ${OBJECTID}, ${STATENAME}, ${TOSTATE}
     * @throws Exception
     *             if operation fails
     * @since AEF 9.5.1.3
     * @grade 0
     */
    public int triggerDemoteCheck(Context context, String[] args) throws Exception {
        // get values from args.
        String objectId = args[0];
        String fromState = args[1];
        String toState = args[2];
        ...
        if (fromState.equals(STATE_PROJECT_TASK_ACTIVE)) {
            ...
            if (utsList.size() > 0) {
                ...
                while (itr.hasNext()) {
                    ...
                    int taskActiveStatePosition = stateList.indexOf(STATE_PROJECT_TASK_ACTIVE);
    ...
    /**
     * Promote check trigger Checks if all the dependency requirements are met
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - ${OBJECTID}, ${STATENAME}, ${TOSTATE}
    ...
    public int triggerCheckDependency(Context context, String[] args) throws Exception {
        ...
        if (hasPredecessors.equals("True")) {
            ...
            while (itr.hasNext()) {
                ...
                StringList stateList = (StringList) predecessorMap.get(SELECT_STATES);
                ...
                int pActiveStatePosition = stateList.indexOf(STATE_PROJECT_TASK_ACTIVE);
                int pCompleteStatePosition = stateList.indexOf(STATE_PROJECT_TASK_COMPLETE);
    ...
    /**
     * Modify action for Percentage complete attribute Based on the %age set the trigger will promote or demote the object to the required state
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - ${OBJECTID}, ${STATENAME}, ${TOSTATE}
     ...
     * @grade 0
     */
    public void triggerModifyPercentCompleteAction(Context context, String[] args) throws Exception {
        // get values from args.
        String objectId = args[0];
        ...
        /*
         * gqh: not necessary as rollup takes care of PC. if (parentId != null) { //peform percent rollup String[] jpoArgs = new String[1]; jpoArgs[0] = parentId; calculatePercentComplete(context,
         * jpoArgs); }
         */
    }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            the task id to update as argument 0
     * @throws Exception
     *             if operation fails
     * @since PC 10.0.0.0
     * @grade 0
     */
    public void calculatePercentComplete(Context context, String[] args) throws Exception {
        ContextUtil.pushContext(context);
        try {
            setId(args[0]);
     ...
     * @grade 0
     */
    protected void calculatePercentComplete(Context context) throws Exception {
        /*
         * gqh: removed entire logic as PC is now part of date rollup. //get the children of the parent StringList busSelects = new StringList(2);
         * 
         * busSelects.add(SELECT_PERCENT_COMPLETE); busSelects.add(SELECT_TASK_ESTIMATED_DURATION);
         * 
         * MapList tasks = getTasks(context, this, 1, busSelects, null); double totalDuration = 0; double completed = 0; String percentCompleted=""; //Iterator itr = tasks.iterator(); //while
         * (itr.hasNext()) for (int i=0; i < tasks.size(); i++) { //Map map = (Map) itr.next(); Map map = (Map) tasks.get(i);
         * 
         * double percent = Double.parseDouble((String) map.get(SELECT_PERCENT_COMPLETE));
         * 
         * double duration = Double.parseDouble((String) map.get(SELECT_TASK_ESTIMATED_DURATION));
         * 
         * //Bug# 306151 - (duration = 0) problem if(duration <= 0) { duration = 1; }
         * 
         * totalDuration += duration; completed = completed + (percent * duration); }
         * 
         * if (totalDuration != 0) { completed = completed / totalDuration; }
         * 
    ...
    /**
     * Generic Trigger to be used as Promote or any other event Deliverables - when a deliverable is completed, the attached task will be completed Deliverables are often of types
     * ECR,ECO,Document,Part, etc
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - ${OBJECTID}
     ...
     * @grade 0
     */
    public void triggerSetParentTaskToCompleteAction(Context context, String[] args) throws Exception {
        // get values from args.
        String objectId = args[0];
        ...
        MapList boTasks = getRelatedObjects(context, RELATIONSHIP_TASK_DELIVERABLE, QUERY_WILDCARD, selects, EMPTY_STRINGLIST, true, false, (short) 1, EMPTY_STRING, EMPTY_STRING);
    ...
    /**
     * Check to see if the object is a Project Space if it is not then call thru to the CreateObjectOwnership method to inherit ownership from the parent object.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
    ...
    public boolean PRGcreateObjectOwnershipInheritance(Context context, String[] args) throws Exception {
        ...
        try {
            // if the Object being added is a Project Space don't inherit access
