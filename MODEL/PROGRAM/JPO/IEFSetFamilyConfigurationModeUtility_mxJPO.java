 ...
 *
 * The utility script should be run to correct the existing Family Tables if they have different value of IEF-ObjectBasedConfigurationRevisionMode across Revisions or versions (mixed mode data). The
 * script will take 2 arguments as an input when run through MQL. The 2 arguments should be as listed below:
 * 
 * 1. Input major bus id/path of text file containing list of major bus ids: The first argument can be the major id of the family of any Revision OR the path of the text file which contains the list
 * of major ids of different families. The extension of the file should be ".txt". 2. Mode to set: The second argument will be the IEF-ObjectBasedConfigurationRevisionMode which needs to be set on all
 * input object ids.
 * 
 * e.g. exec prog IEFSetFamilyConfigurationModeUtility 39184.13709.60768.40155 together; OR exec prog IEFSetFamilyConfigurationModeUtility E:\MajorIds.txt individual;
 * 
 */
...
public class IEFSetFamilyConfigurationModeUtility_mxJPO {
     ...
     * @grade 0
     */
    public IEFSetFamilyConfigurationModeUtility_mxJPO(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param chunksize
     *            has the no. of objects to be stored in file.
     * @return void
     * @exception Exception
     *                if the operation fails.
     */
                ...
                for (int j = 0; j < majorFamilyRevisions.size(); j++) {
                    ...
                    if (null != minorFamilyRevisions) {
                        for (int k = 0; k < minorFamilyRevisions.size(); k++) {
                            ...
                            String minorRevisionMode = busWithSelect
                                    .getSelectData("revisions[" + majorRevision + "]." + SELECT_ON_ACTIVE_MINOR + "revisions[" + minorRevision + "]." + SELECT_ATTR_REVISION_MODE);
