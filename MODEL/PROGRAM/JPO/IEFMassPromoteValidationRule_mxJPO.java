...
public class IEFMassPromoteValidationRule_mxJPO {
    ...
    /**
     * This method contains the implementation for getting the remark for a particular child node in the finalization page if it is selected for promotion. The method is passed a business object ID of
     * the child node as argument. If the child object is not a major object, the corresponding major object and it's state is found. If the method returns a non-blank string starting with 'false|',
     * the user is not allowed to select the node and the string that is returned is shown to the user as a pop-up dialog. If the selection is implicit (e.g. if "select all children" flag is true in
     * the Promote dialog) then no message is shown but the node will not be selected for promotion.
     * @param context
     *            The user context
     * @param args
     *            A string array of arguments used. The first element of the array MUST be the objectId
     * @returns String
     */
    ...
    /**
     * This function contains the implementation for validating the promotion of the objects in the finalization page. The function is passed the following arguments - child object ID,Root Object
     * Id,state of this Child node,strings representing the its selection and that of any of its parents. If this method returns a non-blank String starting with 'false|', is is treated as an error
     * condition and the promote operation is aborted. In this case, the String that is returned is shown to the user as an error message.
     * @param context
     *            The user context
     * @param args
     *            A string array of a packed Hashtable, which has the follwing keys and values: key value isAnyParentSelected "true" if any of the parents of this child object is select in the
     *            promotion page rootBusObjId the object ID of the root node in the promotion page childBusObjId the object Id of this child node childNodeState the state of this child node's major
     *            object isChildSelected "true" if this child has been selected
     * @returns String
     */
