 ...
 *
 * Copyright (c) 2004-2015 Dassault Systemes.
 *
 ...
 *
 * static const char RCSID[] = $Id: /ENOVariantConfigurationBase/CNext/Modules/ENOVariantConfigurationBase/JPOsrc/base/${CLASSNAME}.java 1.30.2.8.1.1 Wed Oct 29 22:27:16 2008 GMT przemek
 * Experimental${CLASSNAME}.java 1.5 Mon Oct 08 16:16:54 2007 GMT ds-mberi Experimental$
 */
...
/**
 * The <code>emxProductConfigurationBase</code> class holds methods for executing JPO operations related to objects of the type ProductConfiguration.
...
public class emxProductConfigurationBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public emxProductConfigurationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
     ...
     * @throws Exception
     *             when problems occurred in the ProductCentral 10-0-0-0
     * @since ProductCentral 10-0-0-0
     */
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing one String entry for the key "objectId"
    ...
    public MapList getAllProductConfigurations(Context context, String[] args) throws Exception {
        ...
        if (objectId == null) {
            relBusObjPageList = DomainObject.findObjects(context, ProductLineConstants.TYPE_PRODUCT_CONFIGURATION, DomainConstants.QUERY_WILDCARD, null, objectSelects);
        } else {
            DomainObject domainObject = new DomainObject(objectId);
            relBusObjPageList = domainObject.getRelatedObjects(context, strRelPattern, ProductLineConstants.TYPE_PRODUCT_CONFIGURATION, objectSelects, relSelects, false, true, shRecursionLevel,
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getOwnedProductConfigurations(Context context, String[] args) throws Exception {
        HashMap parameterMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) parameterMap.get("objectId");
        ...
        if (objectId == null || "".equals(objectId)) {
            relBusObjPageList = DomainObject.findObjects(context, ProductLineConstants.TYPE_PRODUCT_CONFIGURATION, DomainConstants.QUERY_WILDCARD, whereExpression, objectSelects);
        } else {
            DomainObject domainObject = new DomainObject(objectId);
            relBusObjPageList = domainObject.getRelatedObjects(context, strRelPattern, ProductLineConstants.TYPE_PRODUCT_CONFIGURATION, objectSelects, relSelects, false, true, shRecursionLevel,
                    whereExpression, DomainConstants.EMPTY_STRING, 0);
        }
        return relBusObjPageList;
    }
    ...
    public Vector getLevelOfPartsInPrecBOM(Context context, String[] args) throws Exception {
    ...
    public MapList getPreciseBOMForPC(Context context, String[] args) throws Exception {
        ...
        String sPBOMExists = objPCID.getInfo(context, "from[" + ConfigurationConstants.RELATIONSHIP_PRECISE_BOM + "]");
        MapList evaluatedPreciseParts = new MapList();

        if (ConfigurationConstants.RANGE_VALUE_TRUE.equalsIgnoreCase(sPBOMExists)) {
            evaluatedPreciseParts = pcBean.getAllPreciseBOMParts(context, pcObjId);
            ...
            String contextId = (String) dataMap.get("to[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_CONFIGURATION + "].from.id");

            if (contextId != null && !contextId.equals("")) {
                ProductConfiguration pcInstance = new ProductConfiguration(pcObjId, ProductConfiguration.NONINTERACTIVE_MODE);
                MapList logicalFeatureList = pcInstance.getLogicalFeatureStructure(context, contextId, false);
                ...
                for (int i = 0; i < logicalFeatureList.size(); i++) {
                    ...
                    String lfId = (String) feature.get(ConfigurationConstants.SELECT_ID);
                    LogicalFeature lfbean = new LogicalFeature(lfId);
                    // call the API to retrieve the evaluated Parts of the Logical
                    // Feature.
                    evaluatedParts = lfbean.getActiveGBOMStructure(context, "", relSelect, objselect, ConfigurationUtil.getBasicRelSelects(context), false, true, 1, 0, null, null,
                            DomainObject.FILTER_STR_AND_ITEM, strFilterCompiledForm);
                    if (evaluatedParts.size() > 0) {

                        for (int k = 0; k < evaluatedParts.size(); k++) {
                            Map infoMap = new HashMap();
                            MapList tempPartlist = new MapList();
                            Map partMap = (Map) evaluatedParts.get(k);
                            infoMap.put(ConfigurationConstants.SELECT_ID, partMap.get(ConfigurationConstants.SELECT_ID));
                            infoMap.put(ConfigurationConstants.SELECT_NAME, partMap.get(ConfigurationConstants.SELECT_NAME));
                            infoMap.put(ConfigurationConstants.SELECT_RELATIONSHIP_ID, (String) partMap.get(ConfigurationConstants.SELECT_RELATIONSHIP_ID));

                            infoMap.put(ConfigurationConstants.SELECT_INTERFACE_EBOM, (String) partMap.get(ConfigurationConstants.SELECT_INTERFACE_EBOM));
                            infoMap.put(ConfigurationConstants.RELATIONSHIP_TYPE, (String) partMap.get(ConfigurationConstants.RELATIONSHIP_TYPE));
                            infoMap.put(ConfigurationConstants.SELECT_PHYSICAL_ID, (String) partMap.get(ConfigurationConstants.SELECT_PHYSICAL_ID));
                            infoMap.put(ConfigurationConstants.SELECT_TYPE, (String) partMap.get(ConfigurationConstants.SELECT_TYPE));
                            infoMap.put(ConfigurationConstants.SELECT_REVISION, (String) partMap.get(ConfigurationConstants.SELECT_REVISION));
                ...
                MapList prdGbomList = prdObj.getActiveGBOMStructure(context, "", relSelect, objselect, ConfigurationUtil.getBasicRelSelects(context), false, true, 1, 0, null, null,
                        DomainObject.FILTER_STR_AND_ITEM, strFilterCompiledForm);
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getSelectedOptions(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing one Map entry for the key "paramMap" This Map contains the arguments passed to the jsp which called this method.
    ...
    public StringList getStandardCustomItem(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        String strCustomConfiguration = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Value.CustomConfiguration", language);
        String strStandardConfiguration = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Value.StandardConfiguration", language);

        /*
         * * Commented out... This is not required. String sUserName = context.getUser(); com.matrixone.apps.common.Person personLoggedIn = com.matrixone.apps.common.Person.getPerson ( context,
         * sUserName); com.matrixone.apps.common.Company userCompany = personLoggedIn.getCompany ( context );
         * 
         * String sUserCompanyId = userCompany.getObjectId();
         */

        StringList strCompanyInfo = ProductLineUtil.getUserCompanyIdName(context);
        String sUserCompanyId = (String) strCompanyInfo.get(0);

        // Initialize the relationships
        String rel_StandardItem = ProductLineConstants.RELATIONSHIP_STANDARD_ITEM;
        String rel_CustomItem = ProductLineConstants.RELATIONSHIP_CUSTOM_ITEM;
        ...
        if (objectId != null) // process if not null
        {
            ...
            String sTempCompanyId = dmnObject.getInfo(context, strListRequiredInfo);
            ...
            if (sTempCompanyId != null) {
                ...
                sTempCompanyId = dmnObject.getInfo(context, strListRequiredInfo);
                if (sTempCompanyId != null) {
                    if (!(sTempCompanyId.equals(DomainConstants.EMPTY_STRING))) {
                        if (sTempCompanyId.equals(sUserCompanyId)) {
                            if (strMode.equalsIgnoreCase("Edit"))
    ...
    public Object getStandardCustomRange(Context context, String[] args) throws Exception {
    ...
    public int updateStandardCustomRange(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
            ...
            try {
                ...
                if (strOldValue != null && !strOldValue.equals("") && !("null".equalsIgnoreCase(strOldValue))) {
                    if (strOldValue.equals(strStandardConfiguration)) {
                        strListRequiredInfo = "to[" + ProductLineConstants.RELATIONSHIP_STANDARD_ITEM + "].businessobject.id";
                    } else if (strOldValue.equals(strCustomConfiguration)) {
                        strListRequiredInfo = "to[" + ProductLineConstants.RELATIONSHIP_CUSTOM_ITEM + "].businessobject.id";
                    }
                    strCompanyId = dmnPCObject.getInfo(context, strListRequiredInfo);

                    // Disconnect the existing relationship
                    if (strNewValue.equals(strCustomConfiguration))
                    // If the new value is "Custom Configuration"
                    {
                        strListRequiredInfo = "to[" + ProductLineConstants.RELATIONSHIP_STANDARD_ITEM + "].id";
                    } else if (strNewValue.equals(strStandardConfiguration))
                    // If the new value is "Standard Configuration"
                    {
                        strListRequiredInfo = "to[" + ProductLineConstants.RELATIONSHIP_CUSTOM_ITEM + "].id";
                    }
                    strOldRelationshipId = dmnPCObject.getInfo(context, strListRequiredInfo);
                ...
                // If the new value is "Custom Configuration"
                {
                    strNewRelationshipType = ProductLineConstants.RELATIONSHIP_CUSTOM_ITEM;
                } else if (strNewValue.equals(strStandardConfiguration))
                // If the new value is "Standard Configuration"
                {
                    strNewRelationshipType = ProductLineConstants.RELATIONSHIP_STANDARD_ITEM;
                }
                ...
                DomainRelationship.connect(context, dmnCompanyObject, strNewRelationshipType, dmnPCObject);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds Map containg the following input arguments: 0 - Haspmap containg SETTINGS 1- string of command
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the arguments passed by trigger.
     * @return StringList
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.5.SP1-Semi
     */
    public StringList getStandardCustomItemforTable(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @since PRC 10.6
     */
    public Boolean canCreateProductConfig(Context context, String[] args) throws Exception {
        Boolean retVal = null;
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the arguments passed by trigger. ObjectId
     * @return Boolean
     * @throws Exception
     *             if the operation fails
     * @since EC 10.5.1.2
     */
    public int isSalableProductConfiguration(Context context, String[] args) throws Exception {
        ...
        if (bSalable) {
            ...
            String strSalable = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.TiggerMsg.SalableProductConfiguration", language);
    ...
    /**
     * Trigger Method to check if the selected Options in the product Configuration are present in the Product Structure.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the Object Id of the Product Configuration.
     * @return int 0 - succcess 1 - failure.
     * @throws Exception
     *             if the operation fails
     * @since R212
     */

    public int checkSelectedOptionsExist(Context context, String[] args) throws Exception {
        ...
        ProductConfiguration productConfigObj = new ProductConfiguration(productConfigurationId, ProductConfiguration.NONINTERACTIVE_MODE);
        ...
        productConfigObj.setStartEffectivity(context, (String) mpProduct.get("attribute[" + ConfigurationConstants.ATTRIBUTE_START_EFFECTIVITY + "]"), ProductConfiguration.ACTION_PC_EDIT);
        ...
        MapList relatedObjectsList = contextUtilObj.getObjectStructure(context, "*", relTypes.toString(), objAttriSel, relAttriSel, false, true, 0, 0, ConfigurationConstants.EMPTY_STRING,
                ConfigurationConstants.EMPTY_STRING, (short) -1, strEffectivityExpression);
        ...
        /*
         * Only Configuration Features check if(contextDomObj.isKindOf(context, ConfigurationConstants.TYPE_PRODUCT_VARIANT)) { structureRelIds.addAll(contextUtilObj.getInfoList(context,
         * "from["+ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST+"].torel.id")); }
         */
        ...
        for (int i = 0; i < selectedRelIds.size(); i++) {
            ...
            if (!structureRelIds.contains(selectedRel)) {
                String strPromoteStateError = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Alert.SelectedOptionsNotPresent", context.getSession().getLanguage());
    ...
    /**
     * Trigger Method to check if dyamically filtered Logical Features exist in the Product/Product Variant/Logical Structure Configuration are present in the Product Structure.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the Object Id of the Product Configuration.
    ...
    public int checkSelectedLogicalFeaturesExist(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            a String array holding the arguments from the calling method. It contains a string representing the strProductConfigurationID.
     * @return int 0 or 1
     * @throws Exception
    ...
    public int validateBatchRulesForPromoteCheck(Context context, String[] args) throws Exception {
        ...
        if (skipTriggerCheck != null && "true".equals(skipTriggerCheck)) {
            ...
            if (false == attrValueMap.isEmpty() && attrValueMap.get(attrNameList.get(0)).equalsIgnoreCase("Valid")) {
            ...
            /*
             * ProductConfiguration productConf = new ProductConfiguration(); productConf.setId(strProductConfigurationID); //Marketing Rules Evaluation productConf.initContext(context);
             * productConf.loadContextStructure(context, productConf.getContextId(), productConf.getParentProductId()); productConf.loadSelectedOptions(context); boolean marketingRulesPassed =
             * productConf.isValidProductConfiguration(context);
             * 
             * BackgroundProcess backgroundProcess = new BackgroundProcess(); String strAttributeValidationStatus=ConfigurationConstants.ATTRIBUTE_VALIDATION_STATUS; String
             * strAttributeValidationStatusValue=""; if (marketingRulesPassed) { strAttributeValidationStatusValue=ConfigurationConstants.RANGE_VALUE_VALIDATION_PASSED; //
             * setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_VALIDATION_STATUS,ConfigurationConstants.RANGE_VALUE_VALIDATION_PASSED);
             * 
             * }else { strAttributeValidationStatusValue=ConfigurationConstants.RANGE_VALUE_VALIDATION_FAILED; //
             * setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_DESIGN_VALIDATION_STATUS,ConfigurationConstants.RANGE_VALUE_VALIDATION_FAILED); iFlag = 1; } String[] args1= new String
             * []{strProductConfigurationID,strAttributeValidationStatus,strAttributeValidationStatusValue}; backgroundProcess.submitJob(context.getFrameContext(context.getSession().toString()),
             * "emxProductConfiguration", "setValidationStatusOnPromote", args1 , (String)null);
             * 
             * 
             * if(iFlag == 1) { String strLanguage = context.getSession().getLanguage(); String strPromoteStateError = EnoviaResourceBundle.getProperty(context,
             * SUITE_KEY,"emxConfiguration.ProductConfiguration.PromoteFailMarketingRuleEvaluationFailure", strLanguage); throw new FrameworkException(strPromoteStateError); }
             */
    ...
    public boolean isSalableProductConfiguration(Context context, String objectId) throws Exception {
        ...
        if (sValidationStatus.equals("Not Validated") && !curState.equals(ProductLineConstants.STATE_PRODUCT_CONFIGURATION_PRELIMINARY)) {
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList expandAllFeatureStructureBasedOnSelectedOptions(Context context, String[] args) throws Exception {
        return getSelectedOptionsList(context, args, null);
    }

    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList expandMarketingFeatureStructureBasedOnSelectedOptions(Context context, String[] args) throws Exception {
        return getSelectedOptionsList(context, args, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES);
    }

    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList expandTechnicalFeatureStructureBasedOnSelectedOptions(Context context, String[] args) throws Exception {
     ...
     * @throws Exception
     */
    private MapList getSelectedOptionsList(Context context, String[] args, String featureType) throws Exception {
        ...
        ProductConfiguration pConf = new ProductConfiguration(productConfigId, ProductConfiguration.NONINTERACTIVE_MODE);
        ...
        MapList _selectedOptionList = pConf.getSelectedOptions(context, productConfigId, true, true);
        ...
        for (int i = 0; i < _selectedOptionList.size(); i++) {
            ...
            String _SOFeatureRelID = (String) selectedOption.get(DomainRelationship.SELECT_ID);
        ...
        String objType = objectToBeExpanded.getInfo(context, DomainConstants.SELECT_TYPE);
        ...
        if (mxType.isOfParentType(context, objType, ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION)) {
            ...
            if (topLevelfeatures != null) {
                for (IProductConfigurationFeature pSelectedFeature : topLevelfeatures) {
                    if ((featureType == null || ProductLineUtil.getChildrenTypes(context, featureType).contains(pSelectedFeature.getType()))) {
                        pSelectedFeature.getSelectedFeaturesForSB(context, _selectedOptionsRelIds, out_selectedFeaturesMapList, iLevel);
                    }
                }
            }
        } else {
            String featureRelId = (String) paramMap.get("relId");
            IProductConfigurationFeature featureToBeExpanded = (IProductConfigurationFeature) pConf.getFeature(featureRelId).get(0);
            childFeatures = featureToBeExpanded.getChildren();
            if (childFeatures != null) {
                for (int i = 0; i < childFeatures.size(); i++) {
                    IProductConfigurationFeature pSelectedFeature = (IProductConfigurationFeature) childFeatures.get(i);
    ...
    public String getTopLevelPartChooser(Context context, String[] args) {
        ...
        try {
            ...
            if (strMode != null && !strMode.equals("") && !strMode.equalsIgnoreCase("null") && strMode.equalsIgnoreCase("edit")) {
                ...
                sbBuffer.append(
                        "onClick=\"javascript:showChooser('../common/emxFullSearch.jsp?field=TYPES=type_Part:HAS_EBOM_CONNECTED=false:IS_TOP_LEVEL_PART=false:CURRENT!=policy_DevelopmentPart.state_Obsolete,policy_StandardPart.state_Obsolete:POLICY!=policy_ECPart,policy_ConfiguredPart&ParentProductId="
                                + XSSUtil.encodeForHTMLAttribute(context, strProductId)
                                + "&table=PLCSearchPartsTable&showInitialResults=false&selection=single&submitAction=refreshCaller&hideHeader=true&HelpMarker=emxhelpfullsearch&mode=Chooser&chooserType=FormChooser&fieldNameActual="
                                + XSSUtil.encodeForHTMLAttribute(context, strFieldName) + "OID&fieldNameDisplay=" + XSSUtil.encodeForHTMLAttribute(context, strFieldName)
                                + "Display&formName=editDataForm&frameName=formEditDisplay&suiteKey=Configuration&submitURL=../configuration/SearchUtil.jsp");
                ...
                String strClear = EnoviaResourceBundle.getProperty(context, "ProductLine", "emxProduct.Button.Clear", context.getSession().getLanguage());
     ...
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            1- Hold the id of the object for which the selectable is obtained
     * @param args
     *            2- Defines the relationship for which the check is done
     * @param args
     *            3- Defines the direction of expansion to be used
     * @return String- Boolean to indicate if the relationship exists or not
     * @throws Exception
     *             : Returns false in the case of exception
     */
    ...
    public boolean showCommandForPCCreate(Context context, String args[]) throws Exception {
        ...
        String strContextId = (String) configurableRulesMap.get("objectId");
        boolean showCommand = true;

        if (strContextId != null) {
            if (!strContextId.equals("")) {
                ...
                if (strObjectState.equals(ProductLineConstants.STATE_INACTIVE)) {
                    showCommand = false;
                }
            }
        }
    ...
    /**
     * Override trigger for the context object delete deletes the Product Configurations attached to this context object
     * @param context
     * @param args
     * @throws Exception
     */
    public void onContextObjectDelete(Context context, String args[]) throws Exception {
        try {
            ...
            StringList relSelects = new StringList(DomainConstants.SELECT_RELATIONSHIP_ID);
            short shRecursionLevel = 1;
            MapList PCList = contextDom.getRelatedObjects(context, sbBuffer.toString(), ProductLineConstants.TYPE_PRODUCT_CONFIGURATION, objectSelects, relSelects, false, true, shRecursionLevel,
                    DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);
    ...
    /**
     * Check trigger for context object delete checks whether the Product Configurations attached to this context object can abe deleted
     * @param context
     * @param args
     * @return
     * @throws Exception
    ...
    /**
     * Trigger method to check for the relationship Inactive Varies By, from Product to Feature If any of them is present in the Selected Options, then fail
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the object id for product configuration.
    ...
    public int inactiveFeaturePromoteStateCheck(Context context, String[] args) throws Exception {
        ...
        Map htbProduct = (Hashtable) productConfiguration.getRelatedObject(context, ProductLineConstants.RELATIONSHIP_PRODUCT_CONFIGURATION, false, attributeList,
        ...
        MapList relBusObjList = lfBean.getInactiveDesignVariants(context, sbType.toString(), relationshipName, objectSelects, relSelects, false, true, shRecursionLevel, 0, null, null, (short) 0,
                null);
        for (int iCount = 0; iCount < relBusObjList.size(); iCount++) {
            String string1 = (String) ((Map) relBusObjPageList.get(iCount)).get(DomainConstants.SELECT_ID);
            bFlag = false;
        ...
        if (bFlag) {
            // Check Trigger blocked
            String strPromoteStateError = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Alert.InactiveFeaturePresent", context.getSession().getLanguage());
            emxContextUtil_mxJPO.mqlNotice(context, strPromoteStateError);
            return 1;
        } else {
    ...
    /**
     * Trigger Method called to check if the Parent Product on which the Product Configuration is based on is in release state
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the Object Id of the Product Configuration.
    ...
    public int isProdConfContextObjectReleased(Context context, String[] args) throws Exception {
        ...
        if (strState.equals(strRelease)) {
            ...
            // Check Trigger blocked
            String strPromoteStateError = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Alert.ParentNotInRelease", context.getSession().getLanguage());
            emxContextUtil_mxJPO.mqlNotice(context, strPromoteStateError);
            return 1;
        }
    }
    ...
    public StringList getMPRIncludeOIDs(Context context, String[] args) throws Exception {
        ...
        String strSelectOptions = "from[" + ConfigurationConstants.RELATIONSHIP_RIGHT_EXPRESSION + "].torel.to.id";
        ...
        MapList mlMprResultIds = DomainObject.getInfo(context, (String[]) ids.toArray(new String[ids.size()]), objSelects);
        ...
        if (mlMprResultIds != null && mlMprResultIds.size() > 0) {
            ...
            List slConfOptionsBase = (StringList) ((Map) mprObjBase).get(strSelectOptions);
            // First set of CO's
            ...
            for (int j = 1; j < mlMprResultIds.size(); j++) {
                ...
                List slConfOptions = (StringList) ((Map) mprObj).get(strSelectOptions);
     ...
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            Holds ParamMap having the key "parentOID" which will determine if the Object is of type Product, only then command will be dispplayed in Reports Menu
     * @return bResult - true - if the Object from which the command callled is of type Product false - if the Object from which the command callled is NOT of type Product
     * @throws Exception
     ...
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            Holds ParamMap having the key "parentOID" which will determine if the Object is of type Product Variant, only then command will be dispplayed in Reports Menu
     * @return bResult - true - if the Object from which the command callled is of type Product Variant false - if the Object from which the command callled is NOT of type Product Variant
     * @throws Exception
     *             - throws Exception if any operation fails.
     */
     ...
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            Holds ParamMap having the key "parentOID" which will determine if the Object is of type Logical Feature, only then command will be dispplayed in Reports Menu
     * @return bResult - true - if the Object from which the command callled is of type Logical Feature false - if the Object from which the command callled is NOT of type Logical Feature
     * @throws Exception
     *             - throws Exception if any operation fails.
     */
    ...
    /**
     * Gets Columns to view Product Configuration grid and pass it on to GridComponent to view product configuration *
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public MapList getProductConfigurationForGrid(Context context, String[] args) throws FrameworkException {
        ...
        try {
            String colGroupValue = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.Table.ColumnGroupHeader", context.getSession().getLanguage());
            ...
            if (ProductLineCommon.isNotNull(selectIds)) {
                ...
                colList = new DomainObject(stprObjectId).getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCT_CONFIGURATION, ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION,
                        strListSelectstmts, new StringList(), false, true, (short) 0, DomainConstants.EMPTY_STRING, "", 0);
            }
            Map tempmap;
    ...
    /**
     * Gets rows to view Product Configuration grid and pass it on to GridComponent to view product configuration This method will fetch all the CONFIGURATION STRUCTURES for a context product and
     * check if its connected to a product configuration by "Selected Options" If connected to a product configuration, it will put the feature ID against CELL_VALUEE for a particular CELL_ID(Product
     * Configuration ID)
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public MapList getCFStructureForPCCompareGrid(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            String cellIdKeyInValue = "tomid[" + ProductLineConstants.RELATIONSHIP_SELECTED_OPTIONS + "].attribute[" + ConfigurationConstants.ATTRIBUTE_KEY_IN_VALUE + "]";
            String languageStr = context.getSession().getLanguage();
            String shortHandNotationForKeyIn = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.Table.SubHeader.SelectedOptionsComparison_ShortNotation", languageStr);
            ...
            MapList mapCFStructure = (MapList) cfBean.getConfigurationFeatureStructure(context, strTypePattern, strRelPattern, slObjSelects, slRelSelects, false, true, 0, 0,
            ...
            for (int i = 0; i < mapCFStructure.size(); i++) {
                ...
                /**
                 * Checking for each feature or options, whether it qualifies for a ROW_ID If a feature is a top level Configuration Feature or Configuration Feature with children it qualifies for
                 * ROW_ID
                 */
                ...
                if (tempObject.isKindOf(context, ConfigurationConstants.TYPE_CONFIGURATION_FEATURE) && (subFeatureCount > 0 || isTopLevelFeature || isListOfNull(tempMap.get(cellIdKeyInValue)))) {
                    ...
                    /**
                     * IF a Feature has Key In value on it , it need to be processed differently. All Options or features that are cell values, has to appear first on UI So all key in values are
                     * stored in different Map and at the end appended to the final Map of options
                     */
                ...
                /**
                 * For each Option or Feature that can be a CELL_VALUE need to be put against correct ROW_ID. If current ROW_ID is not correct Parent for a feature or option, Final Map list is
                 * traversed and it is updated with "ActualRowId".
                 */
                ...
                if (tempMap.get(DomainConstants.SELECT_ID) != null) {
                    if (currentRowId.equals(fromId)) {
                        if (tempMap.get(cellIdSelect) != null) {
                            putCellValues(context, tempMap, cellIdvaluesMap, exportToExcel);
                        }
                        aOptionCount++;
                    } else {
                ...
                /**
                 * For each ROW_ID feature, once its all chinldren are marked as CELL_VALUE, for UI, Key In values need to be added after option values. Below code checks whether all options are
                 * traversed and appends Key In value after Option values Since we are using tabular HTML view all cells , HTML tags are added in CELL_VALUE itself
                 */
     ...
     *            This map is the feature ot option map if in case of Key In value processing or In case called for option processing it a Map containing all the options as CELL_VALUE
     * @param cellId
        ...
        try {
            ...
            if (cellIdvaluesMap.containsKey(cellId)) {
                ...
                if (exportToExcel) {
                    ...
                    strcellval = ((String) cellIdvaluesMap.get(cellId)).replace("</table>", "<tr><td>") + line + strName + "</td>" + str + "</tr></table>";
                }
     ...
     */
    private void putCellValues(Context context, Map tempMap, Map cellIdvaluesMap, boolean exportToExcel) throws FrameworkException {
        String cellIdSelect = "tomid[" + ProductLineConstants.RELATIONSHIP_SELECTED_OPTIONS + "].from.id";
    ...
    /**
     * Private method used to create Final Maplist It will check if a Map contains a key "ActualRowID" , it will map the cell value to correct ROW_ID Else feature/option cell value is mapped against
     * last updated ROW_ID
     */
                    ...
                    if (internalMap.get(UITableGrid.KEY_ROW_ID).toString().equals(cellIdvaluesMap.get("ActualRowId").toString())) {
                        ...
                        if (internalMap.get(UITableGrid.KEY_CELL_ID) != null && internalMap.get(UITableGrid.KEY_CELL_VALUE) != null) {
                            ...
                            if (cellIdvaluesMap.get(cellIdSelect) instanceof StringList) {
                                ...
                                for (int tempCount = 0; tempCount < tempList.size(); tempCount++) {
                                    ...
                                    if (!found) {
                                        ...
                                        if (exportToExcel) {
                                            ...
                                            tempStr = "<table width=\"100%\" ><tr><td width=\"100%\" >" + (String) cellIdvaluesMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_DISPLAY_NAME) + "</td>"
                                                    + strFirstTag + "</tr></table>";
                                        }
                                ...
                                String tempStr = "<table width=\"100%\" ><tr><td width=\"100%\" >" + (String) cellIdvaluesMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_DISPLAY_NAME) + "</td>"
                                        + strFirstTag + "</tr></table>";
                                listKeyCellValue.add(tempStr);
                            }
                        } else {
    ...
    public Vector showEffectivtyStatus(Context context, String[] args) throws Exception {
        ...
        String strIneffective = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxConfiguration.ConfigurationFeature.UsageEffectivity.Ineffective", context.getSession().getLanguage());
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList lstObjectIdsList = (MapList) programMap.get("objectList");
    ...
    public StringList getDBChooserFeatureOptions(Context context, String[] args) throws Exception {
        StringList includeOIDList = new StringList();
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        if (!UIUtil.isNullOrEmpty(contextObjectId)) {
            ...
            MapList mpIncludeOIDList = domContextBus.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES, ConfigurationConstants.TYPE_CONFIGURATION_FEATURES,
                    slObjSelects, slRelSelects, false, true, (short) 0, ConfigurationConstants.EMPTY_STRING, ConfigurationConstants.EMPTY_STRING, (short) 0, DomainObject.CHECK_HIDDEN,
                    DomainObject.PREVENT_DUPLICATES, (short) DomainObject.PAGE_SIZE, null, null, null, DomainObject.EMPTY_STRING, strEffectivityExpression, (short) 1);
        ...
        if (UIUtil.isNotNullAndNotEmpty(MPRIds)) {
            ...
            _slMPRFilteredOptions = (StringList) (JPO.invoke(context, "emxProductConfiguration", null, "getMPRIncludeOIDs", jpoArgs, StringList.class));
     ...
     * @throws Exception
     */
    public boolean showLinkForPBOM(Context context, String args[]) throws Exception {
        // Getting programMap from the args
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) programMap.get("objectId");
        boolean bDisplay = false;
        ...
        selectStmts.addElement(DomainConstants.SELECT_CURRENT);
        selectStmts.addElement("from[" + ConfigurationConstants.RELATIONSHIP_PRECISE_BOM + "]");
        selectStmts.addElement("attribute[" + ProductLineConstants.ATTRIBUTE_VALIDATION_STATUS + "]");
        DomainObject productConfigObj = new DomainObject(objectId);
        // Getting the current state
        Map productConfigInfomap = productConfigObj.getInfo(context, selectStmts);
        String curState = (String) productConfigInfomap.get(DomainConstants.SELECT_CURRENT);
        String sPBOMExists = (String) productConfigInfomap.get("from[" + ConfigurationConstants.RELATIONSHIP_PRECISE_BOM + "]");
        String sValidationStatus = (String) productConfigInfomap.get("attribute[" + ProductLineConstants.ATTRIBUTE_VALIDATION_STATUS + "]");
        if (curState.equalsIgnoreCase(ConfigurationConstants.STATE_PRODUCT_CONFIGURATION_ACTIVE) && ConfigurationConstants.RANGE_VALUE_FALSE.equalsIgnoreCase(sPBOMExists)
                && !sValidationStatus.equals("Not Validated")) {
     ...
     * @throws Exception
     */
    public boolean showLinkForPreviewBOM(Context context, String args[]) throws Exception {
        // Getting programMap from the args
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) programMap.get("objectId");
        boolean bDisplay = true;
        ...
        sb.append("].from.");
        sb.append(ConfigurationConstants.SELECT_ATTRIBUTE_LEAF_LEVEL);
        sb.append(".value");
        String selresult = sb.toString();
        selectStmts.addElement(selresult);
        selectStmts.addElement("attribute[" + ConfigurationConstants.ATTRIBUTE_COMPLETENESS_STATUS + "]");
        DomainObject productConfigObj = new DomainObject(objectId);
        // Getting the current state
        Map productConfigInfomap = productConfigObj.getInfo(context, selectStmts);
        String isLeafLevel = (String) productConfigInfomap.get(selresult);
        if (ProductLineCommon.isNotNull(isLeafLevel) && isLeafLevel.equalsIgnoreCase(ConfigurationConstants.RANGE_VALUE_LEAFLEVEL_YES)) {
            bDisplay = false;
        }

        String pcCreateMode = EnoviaResourceBundle.getProperty(context, "emxConfiguration.ProductConfiguration.UIMode");
        String OS = System.getProperty("os.name").toLowerCase();
        if (OS.indexOf("sunos") >= 0 || OS.indexOf("aix") > 0)
            pcCreateMode = "NonSolver";

        if (pcCreateMode != null && "Solver".equals(pcCreateMode)) {
            String rulesCompliancyStatus = (String) productConfigInfomap.get("attribute[" + ConfigurationConstants.ATTRIBUTE_COMPLETENESS_STATUS + "]");

            if (!"Complete".equals(rulesCompliancyStatus))
                bDisplay = false;
        }
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getNonValidateRuleConnected(Context context, String[] args) throws Exception {
        MapList mapLogicalStructure = new MapList();
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        StringList stconflictRules = (StringList) programMap.get("conflictRules");
     ...
     * @throws Exception
     */
    public List getErrorMessage(Context context, String[] args) throws Exception {
        List errorList = new StringList();
        MapList mapRuleList = new MapList();
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        Map ruleIdsList = (Map) programMap.get("paramList");
        StringList stconflictRules = (StringList) ruleIdsList.get("conflictRules");
        mapRuleList = ConfigurableRulesUtil.getRequiredMapDesignRuleTable(context, stconflictRules);
    ...
    public static List getRuleCondition(Context context, String[] args) throws FrameworkException {
        MapList mapRuleList = new MapList();
        ...
        try {
            ...
            for (int i = 0; i < mapRuleList.size(); i++) {
                Map tempMAp = (Map) mapRuleList.get(i);
                String operator = (String) tempMAp.get(ConfigurationConstants.SELECT_ATTRIBUTE_COMPARISION_OPERATOR);
            ...
            leftExpList = ruleProcess.getExpressionForRuleDisplay(context, JPO.packArgs(newProgramMap), ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION);
            // getting the Right expression
            rightExpList = ruleProcess.getExpressionForRuleDisplay(context, JPO.packArgs(newProgramMap), ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION);
    ...
    public List getContext(Context context, String[] args) throws Exception {
        ...
        if (strType != null && !strType.equalsIgnoreCase("")) {
            if ((mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_PRODUCTS) || mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_PRODUCT_VARIANT))) {
                strTypeIcon = "iconSmallProduct.gif";
            } else if (strType.equalsIgnoreCase(ConfigurationConstants.TYPE_SOFTWARE_FEATURE)
                    || ProductLineUtil.getChildrenTypes(context, ConfigurationConstants.TYPE_SOFTWARE_FEATURE).contains(strType)) {
        ...
        if (ProductLineCommon.isNotNull(strReportFormat)) {
            ...
            sbBuffer = sbBuffer.append("<img src=\"../common/images/").append(strTypeIcon).append("\" border=\"0\"  align=\"middle\" ").append("/><B>")
                    .append(XSSUtil.encodeForXML(context, contextName)).append("</B>");
        }
    ...
    public Vector ruleEdit(Context context, String[] args) throws Exception {
        ...
        StringBuffer sb = new StringBuffer();
        sb.append("to[");
        sb.append(ConfigurationConstants.RELATIONSHIP_BOOLEAN_COMPATIBILITY_RULE);
        sb.append("].from.id");
        ...
        selectable.add(ConfigurationConstants.SELECT_ATTRIBUTE_RIGHT_EXPRESSION);
        ...
        mapLogicalStructure = dom.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_BOOLEAN_COMPATIBILITY_RULE + "," + ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES,
                ConfigurationConstants.TYPE_BOOLEAN_COMPATIBILITY_RULE + "," + ConfigurationConstants.TYPE_LOGICAL_STRUCTURES, selectable, DomainConstants.EMPTY_STRINGLIST, true, true, sLevel,
                DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);

        if (ConfigurationConstants.TYPE_PRODUCT_VARIANT.equalsIgnoreCase(domType)) {
            dom.setId(parentProduct);
            MapList parentProductList = dom.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_BOOLEAN_COMPATIBILITY_RULE + "," + ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES,
                    ConfigurationConstants.TYPE_BOOLEAN_COMPATIBILITY_RULE + "," + ConfigurationConstants.TYPE_LOGICAL_STRUCTURES, selectable, DomainConstants.EMPTY_STRINGLIST, true, true, sLevel,
                    DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);
        ...
        for (int i = 0; i < mapLogicalStructure.size(); i++) {
            ...
            if (stconflictRules.contains(id)) {
                ...
                ruleMap.put("revision", tempMAp.get(DomainConstants.SELECT_REVISION));
                ruleMap.put(ConfigurationConstants.SELECT_PHYSICAL_ID, tempMAp.get(ConfigurationConstants.SELECT_PHYSICAL_ID));

                Object obj = tempMAp.get(parentId);
                Object ruletype = tempMAp.get(ruleId);
                StringList filter = new StringList();
                if (obj != null) {
                    if (obj instanceof StringList && ruletype instanceof StringList) {
                        StringList ruleIds = ConfigurationUtil.convertObjToStringList(context, ruletype);
                ...
                ruleMap.put("attribute[Left Expression]", tempMAp.get(ConfigurationConstants.SELECT_ATTRIBUTE_LEFT_EXPRESSION));
                ruleMap.put("attribute[Right Expression]", tempMAp.get(ConfigurationConstants.SELECT_ATTRIBUTE_RIGHT_EXPRESSION));
        ...
        newProgramMap.put(contextId, contextId);
        Vector exper = (Vector) JPO.invoke(context, "emxRule", jpoArgs, "getEditColumnDisplayInTable", jpoArgs, Vector.class);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            a String array holding the arguments from the calling method. It contains a string representing Transaction History.
     * @return int 0 or 1
     * @throws Exception
     *             when the operation fails. s
     */
    ...
    /**
     * This is a method to Update the Validation Status Attribute on the Product Configuration Property Page when the PC is to be Promoted to Active State
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            a String array holding the arguments from the calling method.
     * @throws Exception
     *             when the operation fails.
     */
    ...
    /**
     * This method is used in the Product Configuration Properties Page to show the Milestones selected for Product Configuration.
     * @param context
     * @param args
     * @return
     */
        ...
        try {
            ...
            if (strMode != null && !strMode.equals("") && !strMode.equalsIgnoreCase("null") && strMode.equalsIgnoreCase("edit")
                ...
                sbBuffer.append("onClick=\"javascript:showChooser('../common/emxFullSearch.jsp?field=TYPES=type_Milestone&includeOIDprogram=emxProductConfiguration:includeMilestones&strModelId="
                        + XSSUtil.encodeForHTMLAttribute(context, strModelId)
                        + "&table=CFFMilestoneDefinitionTable&showInitialResults=true&selection=single&submitAction=refreshCaller&hideHeader=true&HelpMarker=emxhelpfullsearch&mode=Chooser&chooserType=Milestone&fieldNameActual="
                        + XSSUtil.encodeForHTMLAttribute(context, strFieldName) + "OID&fieldNameDisplay=" + XSSUtil.encodeForHTMLAttribute(context, strFieldName)
                        + "Display&formName=editDataForm&frameName=formEditDisplay&suiteKey=Configuration&submitURL=../configuration/SearchUtil.jsp");
                sbBuffer.append("','900','600')\">");
                sbBuffer.append("&nbsp;&nbsp;");
    ...
    public StringList includeMilestones(Context context, String[] args) throws Exception {
        ...
        mapProductObjsList = com.matrixone.apps.productline.Model.getModelMilestoneTracks(context, strModelId, new StringList("Engineering"), null, null, false, false);
    ...
    /**
     * Access function to show the Milestone Field only in Product Revision case. For PC in Logical Feature and Product Variant, the Milestone Field will not be visible.
     * @param context
     * @param args
     * @return
     * @throws Exception
     */
    public static boolean showMilestoneField(Context context, String[] args) throws Exception {
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     * @throws Exception
     */
    public int updateProductConfigurationMilestone(Context context, String[] args) throws Exception {
        Map programMap = (HashMap) JPO.unpackArgs(args);
        Map paramMap = (HashMap) programMap.get("paramMap");
        ...
        } else if (strMilestoneOID != null && strMilestoneOID.equals("") && pcDetails.get("from[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FILTER_CRITERIA + "].physicalid") != null) { // if
                                                                                                                                                                                                   // Milestone
                                                                                                                                                                                                   // is
                                                                                                                                                                                                   // cleared.
            DomainRelationship.disconnect(context, pcDetails.get("from[" + ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FILTER_CRITERIA + "].physicalid").toString());
