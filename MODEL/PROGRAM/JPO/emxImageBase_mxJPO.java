...
public class emxImageBase_mxJPO extends emxImageManager_mxJPO {
    ...
    /**
     * Default constructor. Create a new emxImageBase object from a given id.
     * @param context
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (!context.isConnected()) {
            String sContentLabel = EnoviaResourceBundle.getProperty(context, SUITE_KEY, ERROR_UNSUPPORTED_CLIENT, context.getSession().getLanguage());
     ...
     *            - string containing relationship name
    ...
    protected MapList expandForAll(Context context, String args[], String strRelationshipName) throws Exception {
        ...
        MapList relBusObjPageList = domainObject.getRelatedObjects(context, strRelationshipName, strType, objectSelects, relSelects, false, true, sh, "", "");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the Maps which contains string for ObjectId and Mode.
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds Hashmap containing the ObjectId and OBJECTLIST.
    ...
    public Vector primaryImageIcon(Context context, String[] args) throws Exception {
        ...
        if (!(relBusObjPageList != null)) {
            String sContentLabel = EnoviaResourceBundle.getProperty(context, SUITE_KEY, ERROR_NO_OBJECTS, context.getSession().getLanguage());
            throw new Exception(sContentLabel);
        }
        ...
        DomainObject domainObject = newInstance(context, strObjectId);
        // The associated object details are retreived onto a MapList
        MapList relBusObjPageList1 = domainObject.getRelatedObjects(context, strPrimaryRelationshipName, "*", objectSelects, relSelects, true, true, sh, "", "");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a Map with the following entries: strObjectId - string containing the object id. strRelId - string containing the relationship id. strNewValue - string containing the new
     *            value. strOldValue - string containing the old value
     * @return Int value.
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.0.0.0
    ...
    public static int updatePrimaryImageStatus(Context context, String[] args) throws Exception {
        ...
        int iReturnValue = 0;
        String strRelationshipName = ProductLineConstants.RELATIONSHIP_IMAGES;
        String strParentObjectId = getParentObjectId(context, strObjectId, strRelId, strRelationshipName);
        if (strOldValue.equalsIgnoreCase(YES) && strNewValue.equalsIgnoreCase(NO)) {
            ...
            // The associated object details are retreived onto a MapList
            MapList relBusObjPageList = dom.getRelatedObjects(context, strPrimaryRelationshipName, "*", objectSelects, relSelects, true, true, sh, "", "");
            ...
            imageBean.setAsPrimaryImage(context, strObjectId, strParentObjectId);
        }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a Map with the following entries: strObjectId - string containing the object id. strRelId - string containing the relationship id.
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a Map with the following entries: strObjectId - string containing the object id. strPrimaryUrl - string containing the Primary Image URl. attachment - string containing the
     *            attachment. file - string containing the file name
    ...
    public String primaryImageURL(Context context, String[] args) throws Exception {
        ...
        String strShowPrimaryUrl = "<img src=" + strPrimaryUrl + "&attachment=" + strattachment + "&file=" + strfile + " height=120>";
     ...
     * @since ProductCentral 10.0.0.0
     */
    protected static String getParentObjectId(Context context, String strObjectId, String strRelId, String strRelationshipName) throws Exception {
        // Stringlist for querying is formulated.
        ...
        MapList relBusObjPageList = dom.getRelatedObjects(context, strRelationshipName, "*", null, relSelects, true, true, sh, "", strRelationshipWhere);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the map containing following input arguments: objetlist - list of object ids objectid - the object id
        ...
        try {
            for (int i = 0; i < iNoOfObjects; i++) {
                ...
                if (strFile != null) {
                    ...
                    String strURL = "<b><A HREF='../productline/ImageUtil.jsp?mode=Display&objectId=" + strObjectId + "&file=" + strFile + "&format=" + strFormat + "'>" + strFile + "</b>";
    ...
    /**
     * Creates an Image Object and checks in a file to it. Also sets the primary image it this is the one.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public Map checkin(Context context, String[] args) throws Exception {
        try {
            ...
            /*
             * The different values in the formBean is obtained by specifying thekey value
             */
            ...
            /*
             * String are initialized to store the value of relationship namesthat will be used in this method.
             */
            String strRelationshipName = ProductLineConstants.RELATIONSHIP_IMAGES;
            String strPrimaryRelationshipName = ProductLineConstants.RELATIONSHIP_PRIMARY_IMAGE;
            ...
            String strRevision = "";
            /*
             * boolean varaible to specify to connect the new createdimage object in the from side of relationship
             */
            boolean bIsFrom = true;
            /*
             * HashMap to store the attribute name value pairs.The attribute values to be set for the object are put in the map.
             */
            HashMap attributeMap = new HashMap();
            attributeMap.put(ProductLineConstants.ATTRIBUTE_IMAGE_VERTICAL_SIZE, strImageVerticalSize);
            attributeMap.put(ProductLineConstants.ATTRIBUTE_IMAGE_HORIZONTAL_SIZE, strImageHorizontalSize);
            attributeMap.put(ProductLineConstants.ATTRIBUTE_IMAGE_UOM, strImageUnitOfMeasure);
            attributeMap.put(ProductLineConstants.ATTRIBUTE_MARKETING_NAME, strImageMarketingName);
            attributeMap.put(ProductLineConstants.ATTRIBUTE_MARKETING_TEXT, strImageMarketingText);
            ...
            strNewObjId = image.create(context, strImageType, strImageName, strRevision, strImageDescription, strPolicy, strVaultName, attributeMap, strImageOwner, strParentOId, strRelationshipName,
                    bIsFrom);
            ...
            /*
             * The setSequenceOrder method is called with the required parameters to set the attribute value of relationship
             */
            image.setSequenceOrder(context, strParentOId, strImageType, strRelationshipName, strNewObjId);
            /*
             * If the Primary Image field in made "Yes" in Create Page then the Image object is connectedto the Parent object through relationship "Primary Image".
             */
            ...
            if (strPrimaryImageId == null) {
                 ...
                 */
                image.setImageAsPrimary(context, strParentOId, strImageType, strPrimaryRelationshipName, strNewObjId);
     ...
     *            string containing the object id of from side.
     * @param strToObjectId
     *            string containing the object id of to side.
     * @param strRelationshipName
     *            string containing the relationship name.
     ...
     * @since ProductCentral 10.0.0.0
     */
    protected void imageConnect(Context context, String strFromObjectId, String strToObjectId, String strRelationshipName) throws FrameworkException, Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the Map containing the following elements reqMap - Map containing the objectList strNewObjIdArr - string array containing the Table row ids strParentOIdArr - string array
    ...
    public void addExisting(Context context, String[] args) throws Exception {
        try {
            ...
            String strPrimaryImageId = domParent.getInfo(context, "from[" + ProductLineConstants.RELATIONSHIP_PRIMARY_IMAGE + "].to.id");

            if (strPrimaryImageId == null) {
                imageList = domParent.getRelatedObjects(context, strRelationshipName, DomainConstants.QUERY_WILDCARD, objectSelects, null, false, true, (short) 1, DomainConstants.EMPTY_STRING,
                        strRelWhereCondition);

                if (imageList.size() != 0) {
                    String strImageId = (String) ((Map) imageList.get(0)).get(DomainConstants.SELECT_ID);

                    image.setImageAsPrimary(context, strParentOId, strImageType, ProductLineConstants.RELATIONSHIP_PRIMARY_IMAGE, strImageId);
                }
            }
    ...
    /**
     * This trigger method is used to set the icon of Image object when a new image file is checked into the Image Object. This method first converts the image file checked into the object to gif icon
     * of particular size and then modifies the object to set the icon. To convert the image file into icon a third party tool called image magick is used. The path of this application is obtained
     * from the system.properties. If there is no entry present for image magick in system.properties then the image iteself is set as icon of the object without any conversion.
     * @param context
     *            The ematrix context of the request.
     * @param args
     *            This string array contains following elements: 0 - The id of the image object 1 - The workspace for image magick
     * @throws Exception
     * @throws FrameworkException
     * @since ProductCentral10.6
     */
    public void createPrimaryImage(Context context, String[] args) throws Exception, FrameworkException {
        ...
        try {
            ...
            try {
                strGenerateImage = EnoviaResourceBundle.getProperty(context, "emxProduct.PrimaryImage.type_Image.GenerateImage");

                strImageSize = EnoviaResourceBundle.getProperty(context, "emxProduct.PrimaryImage.type_Image.Size");
            } catch (Exception e) {
                strGenerateImage = "false";
            }

            // Get the name of the file checked into the image object
            DomainObject domImage = newInstance(context, strImageId);
            // Modified by Enovia MatrixOne for Bug # 311012 Date Oct 25, 2005
            String strFileName = domImage.getInfo(context, "format[" + strImageFormat + "].file.name");

            // Modified by Enovia MatrixOne for Bug # 311012 Date Oct 25, 2005
            // Checkout the file into the workspace folder
            domImage.checkoutFile(context, false, strImageFormat, strFileName, strWorkspacePath);
            ...
            if ("true".equals(strGenerateImage)) {
                ...
                StringBuffer sbNewFileName = new StringBuffer(getFileBaseName(strFileName));
                ...
                try {
                    ...
                    String imageMagick = EnoviaResourceBundle.getProperty(context, "emxProduct.ImageMagick.Directory");
                    // Call the generate thumbnail method to conver the image into
                    // thumbnail
                    generateThumbnail(context, strWorkspacePath, strFileName, sbNewFileName.toString(), strImageSize, imageMagick);
                    // Set the newly generated image as icon for the object
                    setIconImage(context, strImageId, strWorkspacePath + File.separatorChar + sbNewFileName.toString());
            ...
            if (!bImageMagic) {
                ...
                // as icon
                setIconImage(context, strImageId, strWorkspacePath + File.separatorChar + strFileName);
            }
     ...
     * @param strImageMagickLoc
     *            Location of the image magick.
     * @throws Exception
     * @since ProductCentral10.6
     */
    static public void generateThumbnail(Context context, String strWorkspacePath, String strSourceFileName, String strNewFileName, String strImageSize, String strImageMagickLoc) throws Exception {
        ...
        Process proc = app.exec(strCmd.toString(), null, workingDirectory);
        ...
        if (!bSucess) {
            String strErrorMessage = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Error.ImageConvert", context.getSession().getLanguage());
     ...
     * @since ProductCentral10.6
     */
    static public void setIconImage(Context context, String strObjectId, String strFileLocation) throws Exception {
        /*
         * StringBuffer strCmd = new StringBuffer(); strCmd.append("mod bus "); strCmd.append(strObjectId); strCmd.append(" image \""); strCmd.append(strFileLocation); strCmd.append("\"");
         */

        // Begin of modify for Bug# 300329 by Enovia MatrixOne on 16 Mar 05
        try {
            /*
             * MqlUtil.mqlCommand( context, strCmd.toString(), true);
             */
            MqlUtil.mqlCommand(context, "mod bus $1 image $2", true, strObjectId, strFileLocation);

        } catch (Exception e) {
            String strErrorMsg = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Alert.ImageIconFailed", context.getSession().getLanguage());
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains the argument Map of the context object
     * @throws Exception
     *             if operation fails
        ...
        while (imageItr.hasNext()) {
            ...
            if (!imageURL.equalsIgnoreCase("&nbsp;")) {
                if (strImageURLs.length > count) {
                    ...
                    count++;
                    vectImage.add(
                            "<img border='0' align='absmiddle' src='" + XSSUtil.encodeForHTMLAttribute(context, imageURL) + "' title='" + XSSUtil.encodeForHTMLAttribute(context, strFile) + "'/>");
