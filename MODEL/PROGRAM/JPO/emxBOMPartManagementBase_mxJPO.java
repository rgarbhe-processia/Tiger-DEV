...
public class emxBOMPartManagementBase_mxJPO extends emxCommonPart_mxJPO {
    ...
    public emxBOMPartManagementBase_mxJPO(Context context, String[] args) throws Exception {
    ...
    // PCM TIGTK-3622 : 2/12/2016 : KWagh : Start
    ...
    public void floatEBOMToEnd(Context context, String[] args) throws Exception {
        ...
        String sImplementedItemCO = "to[" + RELATIONSHIP_IMPLEMENTED_ITEM + "].from.to[" + RELATIONSHIP_CHANGE_ACTION + "|from.current != \"" + completeState + "\"&& from.type == '"
                + TYPE_CHANGE_ORDER + "'].from.id";
        ;
        String sAffectedItemCO = "to[" + RELATIONSHIP_CHANGE_AFFECTED_ITEM + "].from.to[" + RELATIONSHIP_CHANGE_ACTION + "|from.current != \"" + completeState + "\"&& from.type == '"
                + TYPE_CHANGE_ORDER + "'].from.id";
    ...
    /**
     * Sets the Start Effectivity date on the "EBOM" or "Manufacturer Equivalent" connection between this assembly and any child components to the released date of the assembly.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - the symbolic name of the relationship to float, either relationship_ManufacturerEquivalent or relationship_EBOM (default if none specified)
     ...
     * @trigger PolicyECPartStateReviewPromoteAction.
     */
    public void setEBOMStartEffectivity(Context context, String[] args) throws Exception {
        SimpleDateFormat _mxDateFormat = new SimpleDateFormat(eMatrixDateFormat.getEMatrixDateFormat(), Locale.US);
        ...
        try {
            ...
            MapList mapList = getRelatedObjects(context, relToExpand, // relationship pattern
                    "*", // object pattern
                    null, // object selects
    ...
    public int checkRelatedObjectState(Context context, String[] args) throws Exception {
        ...
        if (intOutput == 0) {
            ...
            MapList mapList = domObj.getRelatedObjects(context, sRel, sTargObject, strListObj, null, getToRelationships, // getTo relationships
     ...
     * @since AEF 9.5.0.0.
     */
    public MapList getECO(Context context, StringList selectStmts, StringList selectRelStmts) throws FrameworkException {
        try {
    ...
    /**
     * Sets the End Effectivity date between this part and its parent assemblies to the date this part is made obsolete.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - the symbolic name of the relationship to float, either relationship_ManufacturerEquivalent or relationship_EBOM (default if none specified)
     ...
     */
    public void setEBOMEndEffectivity(Context context, String[] args) throws Exception {
        SimpleDateFormat _mxDateFormat = new SimpleDateFormat(eMatrixDateFormat.getEMatrixDateFormat(), Locale.US);
    ...
    /**
     * Checks if an object can be connected as an EBOM. If the system property emxBOMPartManagement.AllowUnReleasedRevs is false, then it makes the following checks: - makes sure the part is not in
     * one of the frozen states as specified in emxBOMPartManagement.EBOMFrozenStates. - checks that only the latest revision can be connected.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
    ...
    public int CheckForEBOMConnection(Context context, String[] args) throws Exception {
        ...
        // property entry.
        String strAllowUnreleasedRevs = FrameworkProperties.getProperty(context, "emxBOMPartManagement.AllowUnReleasedRevs");
        // Allow all revisions to connect irrespective of state if property
        // entry is true.
        if (strAllowUnreleasedRevs != null && strAllowUnreleasedRevs.equalsIgnoreCase("true")) {
            return 0;
        }

        EBOM_FROZEN_STATES = FrameworkProperties.getProperty(context, "emxBOMPartManagement.EBOMFrozenStates");
        StringTokenizer sTok = new StringTokenizer(EBOM_FROZEN_STATES, ",");

        // Added for IR-021267
        if (EBOM_FROZEN_STATES == null || EBOM_FROZEN_STATES.trim().length() == 0 || (sTok.countTokens() % 2) != 0) {
        ...
        while (sTok.hasMoreTokens()) {
            ...
            String sPolicyRealName = PropertyUtil.getSchemaProperty(context, tokenPolicy);
            if (sPolicyRealName == null || "".equals(sPolicyRealName.trim())) {
                ...
                emxContextUtil_mxJPO.mqlError(context, tokenPolicy + " " + sErrorMsg);
                return 1;
            }
            String sStateRealName = PropertyUtil.getSchemaProperty(context, "policy", sPolicyRealName, tokenState);
            if (sStateRealName == null || "".equals(sStateRealName.trim())) {

                String sErrorMsg = EnoviaResourceBundle.getProperty(context, "emxBOMPartManagementStringResource", context.getLocale(), "emxBOMPartManagement.Common.SymbolicNameMapError");
                emxContextUtil_mxJPO.mqlError(context, tokenState + " " + sErrorMsg);
                return 1;
            }
     ...
     * @since EC V6R2011
     */
    public int CheckForParentInRelease(Context context, String[] args) throws Exception {
        String sType = getInfo(context, SELECT_TYPE);
        String sName = getInfo(context, SELECT_NAME);
        String sRev = getInfo(context, SELECT_REVISION);
    ...
    /**
     * Gets the Policy Classification for a policy The value will be either 'Development', 'Production', 'Equivalent',or 'Other'
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param policyName
     ...
     *             if the operation fails
     */
    static public String getPolicyClassification(Context context, String policyName) throws FrameworkException {
        MQLCommand command = new MQLCommand();
        String sPolicyName = policyName;

        try {
            command.executeCommand(context, "print policy $1 select $2 dump", sPolicyName, "property[PolicyClassification].value");
    ...
    /**
     * Removes the Substitutes for an EBOM relationship that is being deleted. When a part is disconnected from the assembly, all subsitutes for that part must also be deleted. The context of this
     * Part should be the Assembly Part.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: 0 - the EBOM relationship id that is being deleted
     * @throws Exception
     *             if the operation fails.
     * @since X3.
    ...
    public void removeSubstitutes(Context context, String[] args) throws Exception {
    ...
    /**
     * Checks that if there are Equivalent Parts connected to the context part at least one of the Equivalent Parts must be Released. If none of the attached Equivalent Parts are Released, the error
     * message "The Part has 1 or more Equivalent Parts connected.One or more of the Equivalent Parts must be in the Release state" is thrown.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds no arguments.
    ...
    public void checkEquivalentPartState(matrix.db.Context context, String[] args) throws Exception {
        try {
            ...
            if (partId != null) {
                ...
                ExpansionIterator iter = expandPartObj.getExpansionIterator(context, RELATIONSHIP_MANUFACTURER_EQUIVALENT, TYPE_PART, objectSelects, relSelects, false, true, (short) 1, null, null,
                        (short) 0, false, false, (short) 1, false);

                MapList objList = FrameworkUtil.toMapList(iter, (short) 0, null, null, null, null);
    ...
    /**
     * Checks the current state of the object with the target state, using the comparison operator and returns the result.
     * @return int representing the following values. 0 if object state logic satisfies Comparison Operator. 1 if object state logic didn't satisfies Comparison Operator. 1 if a program error is
     *         encountered. 1 if state in state argument does not exist in objects policy. 1 if an invalid comparison operator is passed in.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param id
     *            the id of the object whose state to be checked.
     * @param targetState
     *            the target state against which the current state of the object is compared.
     * @param comparisonOperator
     *            the operator used for comparison LT, GT, EQ, LE, GE, NE.
     * @since EC 9.5.ROSSINI.0.
     */
