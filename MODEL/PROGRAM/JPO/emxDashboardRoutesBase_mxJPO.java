...
public class emxDashboardRoutesBase_mxJPO {
    ...
    public String createRoute(Context context, String[] args) throws Exception {
        ...
        if (!sRecipients.equals("")) {
            ...
            for (int i = 0; i < mlRecipients.size(); i++) {
                ...
                if (sType.equals(DomainConstants.TYPE_PERSON)) {
                    ...
                    MapList mlMembers = doList.getRelatedObjects(context, DomainConstants.RELATIONSHIP_LIST_MEMBER, DomainConstants.TYPE_PERSON,
                            new StringList(new String[] { DomainConstants.SELECT_TYPE, DomainConstants.SELECT_ID }), null, false, true, (short) 1, "current == 'Active'", "", 0);
            ...
            if (sbMembers.length() > 0) {
                sbMembers.setLength(sbMembers.length() - 1);
            }
            ...
            for (int i = 0; i < mlRouteNodes.size(); i++) {
                ...
                if (!sInstructions.equals("")) {
                    dRel.setAttributeValue(context, DomainConstants.ATTRIBUTE_ROUTE_INSTRUCTIONS, sInstructions);
                }
                if (!sRequired.equals("")) {
                    dRel.setAttributeValue(context, PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_attribute_ParallelNodeProcessionRule), sRequired);
                }
                if (!sAction.equals("")) {
                    dRel.setAttributeValue(context, DomainConstants.ATTRIBUTE_ROUTE_ACTION, sAction);
                }
                if (!sDueDate.equals("")) {
                    dRel.setAttributeValue(context, DomainConstants.ATTRIBUTE_SCHEDULED_COMPLETION_DATE, sDueDate + " 5:00:00 PM");
                }
        ...
        if (sStart.equalsIgnoreCase("TRUE")) {
    ...
    public String[] getRoutesDashboardData(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < mlRoutes.size(); i++) {
            ...
            if (sHasTask.equalsIgnoreCase("TRUE")) {
                ...
                String sFirstName = (String) mRoute
                        .get("to[" + relRouteTask + "].from.from[" + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].to.attribute[" + DomainConstants.ATTRIBUTE_FIRST_NAME + "]");
                String sLastName = (String) mRoute
                        .get("to[" + relRouteTask + "].from.from[" + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].to.attribute[" + DomainConstants.ATTRIBUTE_LAST_NAME + "]");
                ...
                if (sStatus.contains("")) {
                    ...
                    if (sDateTarget.startsWith("")) {
                        sDateTarget = " " + sDateTarget;
                    }
                    if (sDateTarget.endsWith("")) {
                        sDateTarget = sDateTarget + " ";
                    }

                    sDateActual = sDateActual.replaceAll("", " ");
                    if (sDateActual.startsWith("")) {
                        sDateActual = " " + sDateActual;
                    }
                    if (sDateActual.endsWith("")) {
                        sDateActual = sDateActual + " ";
                    }
                    ...
                    if (sLastName.endsWith("")) {
                        sLastName = sLastName + " ";
                    }

                    sFullNames = sFullNames.replaceAll("", " ");
                    if (sFullNames.startsWith("")) {
                        sFullNames = " " + sFullNames;
                    }
                    if (sFullNames.endsWith("")) {
                        sFullNames = sFullNames + " ";
                    }
                    ...
                    for (int k = 0; k < aStatus.length; k++) {
                        ...
                        if (k < aFirstName.length) {
                            sParamFirstName = aFirstName[k];
                        }
                        if (k < aLastName.length) {
                            sParamLastName = aLastName[k];
                        }
                        String sFullName = "";
                        try {
                        ...
                        if (sFullName.equals(context.getUser())) {
                            ...
                            addRecentTaskToMap(context, mlTasksRecent, cRecent, sId, sName, aStatus[k], aDateTarget[k], aDateActual[k], aTitle[k], aAction[k], sParamFirstName, sParamLastName,
                                    sLanguage);
                        }
                    }
            ...
            if (null == sTemplate || sTemplate.equals("")) {
                sTemplate = "-";
        ...
        for (int i = 0; i < mlTasksPending.size(); i++) {
            ...
            if (sDateLabel.contains(" ")) {
                sDateLabel = sDateLabel.substring(0, sDateLabel.indexOf(" "));
            }
            if (sTitle.equals("")) {
                sTitle = "(" + sAction + ")";
            ...
            if (!"".equals(sDate)) {
                ...
                sbToAppend.append("{ id:'").append(sId).append("', title:'").append(sTitle).append("', route:'").append(sRoute).append("', person:'").append(sPerson).append("', action:'")
                        .append(sAction).append("',person:'").append(sPerson);
                ...
                if (cDate.before(cNow)) {
                    sbDataPending1.append(sbToAppend.toString());
                    aCountPending[2]++;
                } else {
                    if (cDate.after(cFuture)) {
                        sbDataPending3.append(sbToAppend.toString());
                    } else {
                        sbDataPending2.append(sbToAppend.toString());
                    }
        ...
        String sPrefixTasks = "<a onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=APPTaskSummary&freezePane=Name,Title&program=emxDashboardRoutes:getRouteTasksPending&mode=";
        String sSuffixTasks = "&suiteKey=Framework&header=";

        sbPendingWeek.append("<span style='font-weight:bold;'>").append(aCountPending[0]).append("</span> ").append(sPrefixTasks).append("week").append(sSuffixTasks)
                .append("emxFramework.String.PendingTasksThisWeek").append("\")'>").append(sLabelThisWeek).append("</a>");
        sbPendingMonth.append("<span style='font-weight:bold;'>").append(aCountPending[1]).append("</span> ").append(sPrefixTasks).append("month").append(sSuffixTasks)
                .append("emxFramework.String.PendingTasksThisMonth").append("\")'>").append(sLabelThisMonth).append("</a>");
        sbPendingOverdue.append("<span style='font-weight:bold;'>").append(aCountPending[2]).append("</span> ").append(sPrefixTasks).append("overdue").append(sSuffixTasks)
                .append("emxFramework.String.PendingTasksOverdue").append("\")'>").append(sLabelOverdue).append("</a>");
        ...
        for (int i = 0; i < mlTasksRecent.size(); i++) {
            ...
            if (sTargetLabel.contains(" ")) {
                sTargetLabel = sTargetLabel.substring(0, sTargetLabel.indexOf(" "));
            }
            if (sTitle.equals("")) {
                sTitle = "(" + sAction + ")";
            } else if (sTitle.equals(" ")) {
            ...
            if (!"".equals(sActual)) {
                ...
                if ("".equals(sTarget) || " ".equals(sTarget)) {
                    sbDataRecent2.append(sbToAppend.toString());
                } else {
                    ...
                    sbDataRecent1.append("{ id:'").append(sId).append("', title:'").append(sTitle).append("', route:'").append(sRoute).append("', person:'").append(sPerson).append("', action:'")
                            .append(sAction).append("',person:'").append(sPerson).append("', status:'").append(statusDueDate).append("', date:").append(cTarget.getTimeInMillis()).append(",")
                            .append(" x:").append(cTarget.getTimeInMillis()).append(",");
        ...
        sbDataPurpose.append("{ color:'#8BBA00', y:").append(aCountPurpose[0]).append(", value:'Approval', name:\"")
                .append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.Range.Route_Base_Purpose.Approval", sLanguage)).append("\"},");
        sbDataPurpose.append("{ color:'#329CEE', y:").append(aCountPurpose[1]).append(", value:'Review',   name:\"")
                .append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.Range.Route_Base_Purpose.Review", sLanguage)).append("\"},");
        sbDataPurpose.append("{ color:'#F6BD0F', y:").append(aCountPurpose[2]).append(", value:'Standard', name:\"")
                .append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.Range.Route_Base_Purpose.Standard", sLanguage)).append("\"}");
        ...
        // Remove commas
        if (sbCategoriesPending.length() > 0) {
            sbCategoriesPending.setLength(sbCategoriesPending.length() - 1);
        }
        if (sbDataPending1.length() > 0) {
            sbDataPending1.setLength(sbDataPending1.length() - 1);
        }
        if (sbDataPending2.length() > 0) {
            sbDataPending2.setLength(sbDataPending2.length() - 1);
        }
        if (sbDataPending3.length() > 0) {
            sbDataPending3.setLength(sbDataPending3.length() - 1);
        }
        if (sbCategoriesRecent.length() > 0) {
            sbCategoriesRecent.setLength(sbCategoriesRecent.length() - 1);
        }
        if (sbDataRecent1.length() > 0) {
            sbDataRecent1.setLength(sbDataRecent1.length() - 1);
        }
        if (sbDataRecent2.length() > 0) {
            sbDataRecent2.setLength(sbDataRecent2.length() - 1);
        }
        if (sbDataRecent3.length() > 0) {
            sbDataRecent3.setLength(sbDataRecent3.length() - 1);
        }
        if (sbDataRecent4.length() > 0) {
            sbDataRecent4.setLength(sbDataRecent4.length() - 1);
        }
        if (sbDataType.length() > 0) {
            sbDataType.setLength(sbDataType.length() - 1);
        }
        if (sbCategoriesTemplate.length() > 0) {
            sbCategoriesTemplate.setLength(sbCategoriesTemplate.length() - 1);
        }
        if (sbDataTemplate.length() > 0) {
            sbDataTemplate.setLength(sbDataTemplate.length() - 1);
        }
    ...
    private void addPendingTaskToMap(Context context, MapList mlTasksPending, String sId, String sName, String sStatus, String sDateTarget, String sTitle, String sAction, String sFirstName,
            String sLastName, String sLanguage) {
    ...
    private void addRecentTaskToMap(Context context, MapList mlTasksRecent, Calendar cRecent, String sId, String sName, String sStatus, String sDateTarget, String sDateActual, String sTitle,
            String sAction, String sFirstName, String sLastName, String sLanguage) throws ParseException {
    ...
    public MapList retrieveRoutesPending(Context context, String[] args, StringList busSelects, String sFilter) throws FrameworkException {
        ...
        String sWhere = "(current == 'In Process') && (attribute[Route Status] == 'Started') && (to[" + DomainConstants.RELATIONSHIP_ROUTE_TASK + "].from.from["
                + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].to.name == \"" + context.getUser() + "\")";
    ...
    public String[] getUserDashboardData(Context context, String[] args) throws Exception {
        ...
        for (int i = 0; i < mlInboxTasks.size(); i++) {
            ...
            if (null != sDate) {
                if (!"".equals(sDate)) {
                    ...
                    sbToAppend.append("{ id:'").append(sId).append("', title:'").append(sTitle).append("', route:'").append(sRoute).append("', action:'")
                            .append(EnoviaResourceBundle.getProperty(context, "Framework", actionKey, sLanguage)).append("', date:").append(cTarget.getTimeInMillis()).append(", y:")
                            .append(iCountTimeline).append(",");
                    sbToAppend.append(" x:").append(cTarget.getTimeInMillis()).append("},");

                    if (cTarget.before(cNow)) {
                        iCounters[0]++;
                        sbDataTimeline1.append(sbToAppend.toString());
                    } else {
                        if (cTarget.before(cFuture)) {
                            sbDataTimeline2.append(sbToAppend.toString());
                        } else {
                            sbDataTimeline3.append(sbToAppend.toString());
                        }
                    }
                    if (iYear == iYearNow) {
                        if (iMonth == iMonthNow) {
        ...
        sbInfo1.append("<b>").append(iCounters[0]).append("</b>").append(sInfoPrefix).append("getRouteTasksAssignedPending&mode=Overdue&header=emxFramework.String.RouteTasksOverdue\")'>")
                .append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.Overdue", sLanguage)).append("</a>");
        sbInfo2.append("<b>").append(iCounters[1]).append("</b>").append(sInfoPrefix).append("getRouteTasksAssignedPending&mode=Month&header=emxFramework.String.RouteTasksDueThisMonth\")'>")
                .append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.ThisMonth", sLanguage)).append("</a>");
        sbInfo3.append("<b>").append(iCounters[2]).append("</b>").append(sInfoPrefix).append("getRouteTasksAssignedPending&mode=Week&header=emxFramework.String.RouteTasksDueThisWeek\")'>")
                .append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.ThisWeek", sLanguage)).append("</a>");

        // Dashboard Counters
        StringBuilder sbCounter = new StringBuilder();
        sbCounter.append(
                "<td onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=APPTaskSummary&program=emxDashboardRoutes:getRouteTasksAssignedPending&header=emxFramework.String.AssignedTasksPending&freezePane=Name,NewWindow&suiteKey=Framework\")' ");
        sbCounter.append(" class='counterCell ");
        if (mlInboxTasks.size() == 0) {
            sbCounter.append("grayBright");
        } else {
            sbCounter.append("purple");
        }
        ...
        if (iCountMRU > 0) {
            sbUpdates.append(
                    " onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=APPTaskSummary&program=emxDashboardRoutes:getRouteTasksAssignedPending&mode=MRU&header=emxFramework.String.MRURouteTasks&freezePane=Name,NewWindow&suiteKey=Framework\")' ");
    ...
    public MapList getRouteTasksAssignedPending(Context context, String[] args) throws Exception {
        ...
        if (sMode.equals("MRU")) {
            ...
            sbWhere.append(" && ");
            sbWhere.append("(attribute[" + attrScheduledCompletionDate + "] >= '").append(cStart.get(Calendar.MONTH) + 1).append("/").append(cStart.get(Calendar.DAY_OF_MONTH)).append("/")
                    .append(cStart.get(Calendar.YEAR)).append("')");
            sbWhere.append(" && ");
            sbWhere.append("(attribute[" + attrScheduledCompletionDate + "] < '").append(cEnd.get(Calendar.MONTH) + 1).append("/").append(cEnd.get(Calendar.DAY_OF_MONTH)).append("/")
                    .append(cEnd.get(Calendar.YEAR)).append("')");
    ...
    public int triggerCreateRoute(Context context, String[] args) throws Exception {
        ...
        if (sRestart.equalsIgnoreCase("TRUE")) {
            ...
            MapList mlStateBlocks = dObject.getRelatedObjects(context, DomainConstants.RELATIONSHIP_OBJECT_ROUTE, DomainConstants.TYPE_ROUTE, busSelects, relSelects, false, true, (short) 1,
                    sbWhere.toString(), "attribute[" + DomainConstants.ATTRIBUTE_ROUTE_BASE_STATE + "] == '" + sSymbolicState + "'", 0);
    ...
    public MapList getMyPendingRoutes(Context context, String[] args) throws Exception {
        try {
            ...
            String sWhere = "(to[" + DomainConstants.RELATIONSHIP_ROUTE_TASK + "].from.from[" + DomainConstants.RELATIONSHIP_PROJECT_TASK + "].to.name == \"" + context.getUser()
                    + "\") && (current == 'In Process') && (attribute[Route Status] == 'Started')";
