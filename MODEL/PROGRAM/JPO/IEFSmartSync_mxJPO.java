 ...
 ** IEFSmartSync
 **
 ** Copyright Dassault Systemes, 1992-2012. All Rights Reserved. This program contains proprietary and trade secret information of Dassault Systemes and its subsidiaries, Copyright notice is
...
public class IEFSmartSync_mxJPO implements ReplicationTrigger {
    ...
    private void buildProximityMap() {
        /*
         * Following steps are required for setting up the proximity map of locations. It will be used to find out the location/Store from which file will be picked up and synchronized to the
         * destination location. 1. Uncomment the following sample entry (by removing "//" characters at the beginning) 2. For each location/store, atleast one entry is required. 3. In the sample
         * entry below, "loc1" is the name of the destination for which preferred locations are to be entered 4. the String curly braces (inside {}) has the list of preferred locations 5. The sample
         * entry below indicates that the first preferred location is loc2. 6. You can add as many entries for preferred locations, in the order of preference. The first one is taken as the most
         * preferred location 7. Note that the format of the entries should be exactly as indicated below.
         */
     ...
     *            - format name
     * @param locationName
     *            - location name containg the file
