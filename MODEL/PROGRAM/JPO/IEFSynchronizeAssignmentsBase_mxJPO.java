...
/**
 * IEFSynchronizeAssignmentsBase.java jpo Copyright Dassault Systemes, 1992-2007. All Rights Reserved. This program contains proprietary and trade secret information of Dassault Systemes and its
 * subsidiaries, Copyright notice is precautionary only and does not evidence any actual or intended publication of such program This JPO assigns integrations to given person and updates values of
 * attributes in the corresponding local config object for assigned integrations.
 */
...
public class IEFSynchronizeAssignmentsBase_mxJPO {
    ...
    /**
     * Logic for synchronizing assignments is as follows :- For each person, all integrations assigned to all the roles and groups to which this person is assigned, are collected and then lco is
     * updated based on this list.If an integration belonging to this list is not found in the IEF-IntegrationToGCOMapping in lco, an entry is made for this integration in the mapping.Also default
     * values for this integration for all lco attributes are retreived from given gco (from "IEF-Pref-" attributes values) and lco attributes are updated with values for this integrations. The same
     * process is repeated even if the integraion exists.
     * @param personName
    ...
    /**
     * Iterate through the list which contains all the assigned integrations to the person. If any integration belonging to this list is not found in the mapping in local config object then add an
     * entry for this integration in the local config object mapping. Also copy the attribute values from "IEF-Pref-" type attributes in given gco to corresponding attributes in lco. Do the same for
     * those integrations which already exist in the mapping in local config object.
     * @param lcoAttrNameValMap
     *            - hashtable containing the existing values of lco attributes
     * @param assignedIntegrationsGCONamesMap
     *            - hashtable containing all the assigned integrations and corresponding gco names as key-value pairs
     */
    protected void updateLCOAttributesForAssignment(Context context, String personName, Hashtable lcoAttrNameValMap, Hashtable assignedIntegrationsGCONamesMap, boolean isAssignRoleOnly)
            throws Exception {
    ...
    /**
     * This method gets the attributes list with values updated for all the assigned integrations If an integration is already assigned to the person, attribute values are not updated for that
     * integration.If an integration is not previously assigned, attribute values are copied from corresponding attributes in the assigned gco.
     * @param localConfigObj
     * @param assignedIntegrationsGCONamesMap
     *            - array containing assigned integration names as keys and corresponding gco names as values
     */
    ...
    /**
     * This method takes a hashtable and returns string For example,inTable contains key1="a" value1="1,2" and key2="b" value2="3,4", and separator1=";" and separator2="|", then this method will
     * return "a|1,2;b|3,4"
     */
     ...
     *            - person name
     * @param roleName
     *            - role name (should pass actual name which can be get by using getActualNameForAEFData to get the actual name)
