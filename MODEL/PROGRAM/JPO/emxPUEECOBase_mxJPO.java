...
import com.matrixone.apps.unresolvedebom.PUEChange;
...
public class emxPUEECOBase_mxJPO extends emxChange_mxJPO {
     ...
     */
    public emxPUEECOBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
        // Added for the fix 366148
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            contains a MapList with the following as input arguments or entries: objectId holds the context PUEECO object Id
     * @throws Exception
    ...
    public void updateResponsibleManufacturingEngineer(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            String strPUEECOId = (String) paramMap.get("objectId");
            ...
            // Connected RDE and RME
            String strResponsibleMEOldId = domPUEECO.getInfo(context, "relationship[" + strRelResponsibleME + "].to.id");

            // Modified RDE and RME
            String strResponsibleMENewID = ("".equals((String) paramMap.get("New OID"))) ? (String) paramMap.get("New Value") : (String) paramMap.get("New OID");

            // Disconnect RME if the Update RME is different from the new ID
            if (strResponsibleMEOldId != null) {
                if (!strResponsibleMEOldId.equalsIgnoreCase(strResponsibleMENewID)) {
                    String strResponsibleMERelID = domPUEECO.getInfo(context, "relationship[" + strRelResponsibleME + "].id");
                    DomainRelationship.disconnect(context, strResponsibleMERelID);

                }
            }

            if (strResponsibleMENewID != null && !"".equals(strResponsibleMENewID) && !strResponsibleMENewID.equalsIgnoreCase(strResponsibleMEOldId)) {
                setId(strResponsibleMENewID);
                DomainRelationship.connect(context, domPUEECO, strRelResponsibleME, this);
            }
        } catch (Exception ex) {
        }
    ...
    /*
     * This method is called for retrieving the related items of the object in the given context. Related items are populated in the MaplList structure.
     */
    ...
    public MapList getallProductBuilds(Context context, String[] args) throws Exception {
        ...
        relObjIdList = domainObject.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD, ProductLineConstants.TYPE_BUILDS, ObjectSelectsList, RelSelectsList, false, true,
                (short) 1, "", "");
     ...
     * @param context
     * @param args
     *            args[0] -object Id args[1] - relationship name (Prerequisite)
     * @return int
     * @throws FrameworkException
     */
     ...
     * @param context
     * @param args
     *            0 - Object Id 1 - RelationShip symbolic name
     * @return int
     * @throws FrameworkException
     */
     ...
     * @param context
     * @param args
     *            args[0] - Object Id args[1] - relationship name Unresolved EBOM
     * @return int
     * @throws FrameworkException
     */
    ...
    public int checkResponsibleDesignEngineer(Context context, String args[]) throws Exception {
        if (args == null || args.length < 1) {
            throw (new IllegalArgumentException());
        }
        ...
        sList.addElement(DomainConstants.SELECT_ID);
        selectRelStmts.addElement(DomainConstants.SELECT_RELATIONSHIP_ID);
        MapList mListPerson = domPUEECO.getRelatedObjects(context, strRelResponsibleDE, "Person", sList, selectRelStmts, false, true, (short) 1, DomainConstants.EMPTY_STRING,
    ...
    public int checkResponsibleManufacturingEngineer(Context context, String args[]) throws Exception {
        if (args == null || args.length < 1) {
            throw (new IllegalArgumentException());
        }
        String objectId = args[0];
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - Holds the HashMap containing the following arguments paramMap - contains part object id
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getRelatedPUEEcosSummary(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            pueecoList = partObj.getRelatedObjects(context, relPattern.getPattern(), // relationship pattern
    ...
    public String getPolicyFieldPUEECO(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (typeName.indexOf("type_") > 0) {
                typeName = typeName.substring(typeName.indexOf("type_"), typeName.length());
            }
            ...
            if (objectId != null && !objectId.equals("") && !objectId.equals("null")) {
                ...
                String partType = PropertyUtil.getSchemaProperty(context, typeName);
                try {
    ...
    public void floatParentEBOMRels(Context context, String oldRevId, String newRevId) throws Exception {
        if (UnresolvedPart.isTopLevelPart(context, newRevId)) {
            return;
        }
        ...
        MapList mapList = oldRev.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, null, relSels, true, false, (short) 1, "revision==last", null);
    ...
    private void floatAffectedItemRels(Context context, String newPartId, String oldPartId) throws Exception {
        ...
        MapList pendingPUEECOs = part.getRelatedObjects(context, RELATIONSHIP_AFFECTED_ITEM, UnresolvedEBOMConstants.TYPE_PUE_ECO, null, slRelSelect, true, false, (short) 1,
                "current!=" + UnresolvedEBOMConstants.STATE_PUE_ECO_RELEASE, null);
        String sRelId = "";
    ...
    private void updateRequestedChange(Context context, String ecoId, DomainObject parentPart) throws Exception {
        ...
        String strRequestedChangeValue = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", new Locale(strLanguage), "emxFramework.Range.Requested_Change.For_Revise");
        ...
        MapList affectedPartECOIds = parentPart.getRelatedObjects(context, RELATIONSHIP_AFFECTED_ITEM, UnresolvedEBOMConstants.TYPE_PUE_ECO, slBusSelect, slRelSelect, true, false, (short) 1,
    ...
    /**
     * Method to handle PUE ECO release action.
     * @param context
     * @param args
     * @throws Exception
     */
    public int releaseECO(Context context, String[] args) throws Exception {
        String ecoId = args[0];
        StringList selectStmts = new StringList();
        StringList selectRelStmts = new StringList();
        DomainObject eco = new DomainObject(ecoId);
        String ecoName = eco.getName(context);
        Map mapEffectivity = null;
        MapList mlEffectivity = new MapList();
        MapList ebom = new MapList();
        selectStmts.addElement(DomainConstants.SELECT_ID);
        selectRelStmts.addElement("logicalid");
        ...
        selectRelStmts.addElement("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE + "].from.id");
        selectRelStmts.addElement("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE + "].id");
        selectRelStmts.addElement("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY + "].from.id");
        selectRelStmts.addElement("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE + "].attribute[" + UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION + "].value");
        selectRelStmts.addElement("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY + "].attribute[" + UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION + "].value");

        // to process intermediate CFF relationships, on revise
        String RELATIONSHIP_EFFECTIVITY_USAGE = EffectivityFramework.RELATIONSHIP_EFFECTIVITY_USAGE;
        String RELATIONSHIP_NAMED_EFFECTIVITY_USAGE = EffectivityFramework.RELATIONSHIP_NAMED_EFFECTIVITY_USAGE;
        String ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX = EffectivityFramework.ATTRIBUTE_EFFECTIVITY_USAGE_INDEX;
        selectRelStmts.addElement("frommid[" + RELATIONSHIP_EFFECTIVITY_USAGE + "].to.id");
        selectRelStmts.addElement("frommid[" + RELATIONSHIP_EFFECTIVITY_USAGE + "].attribute[" + ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX + "].value");
        selectRelStmts.addElement("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].id");
        // eff. usage selectable will return this too.
        // selectRelStmts.addElement("frommid["+RELATIONSHIP_NAMED_EFFECTIVITY_USAGE+"].to.id");
        selectRelStmts.addElement("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].to.name");
        selectRelStmts.addElement("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].attribute[" + ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX + "].value");

        String strLanguage = "en";
        String CONST_FOR_RELEASE = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", new Locale(strLanguage), "emxFramework.Range.Requested_Change.For_Release");
        String CONST_FOR_REVISE = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", new Locale(strLanguage), "emxFramework.Range.Requested_Change.For_Revise");

        StringList slBusSelect = new StringList(1);
        slBusSelect.add(DomainConstants.SELECT_ID);
        slBusSelect.add(DomainConstants.SELECT_CURRENT);
        StringList slRelSelect = new StringList(1);
        slRelSelect.add("attribute[" + ATTRIBUTE_REQUESTED_CHANGE + "].value");
        MapList affectedPartIds = eco.getRelatedObjects(context, RELATIONSHIP_AFFECTED_ITEM, DomainConstants.TYPE_PART, slBusSelect, slRelSelect, false, true, (short) 1, "", "");

        if (affectedPartIds.isEmpty())
            return 0;

        try {
            ContextUtil.pushContext(context);
            Iterator affectedItemItr = affectedPartIds.iterator();
            Map mapAffectedItem = null;
            Map map = null;
            DomainObject affectedPart = new DomainObject();
            String affectedPartId = "";
            String requestedChange = "";
            String objectState = "";
            StringList slChangeECOIds = new StringList();
            StringList slChangeRelIds = new StringList();
            StringList slEBOMChangeHistoryOp = new StringList();
            StringList slEBOMChangeHistoryRels = new StringList();
            StringList ebomChangeList = new StringList();
            StringList ebomPendingList = new StringList();
            String relType = "";
            boolean updateRC = false;
            String objOwner = "";
            DomainObject newPart = null;
            String newPartId = "";
            String ebomrelId = "";
            String newEBOMRelId = "";
            StringList changeRelsDisconnectList = new StringList();
            String[] strArrRelIds = null;
            String logicalid = "";
            MapList ebomNewRev = new MapList();
            Map newRevMap = new HashMap();
            StringList newRelSelects = new StringList();
            newRelSelects.addElement("logicalid");
            newRelSelects.addElement(SELECT_RELATIONSHIP_ID);
            String childPartState = "";
            String childPartPolicy = "";
            Map relMap = new HashMap();
            MapList affectedRelIds = new MapList();

            StringList slEUObjects = new StringList();
            StringList slEUIndex = new StringList();
            StringList slNUERels = new StringList();
            StringList slNEUObjects = new StringList();
            StringList slNEUIndex = new StringList();
            StringList slNEUNames = new StringList();

            // IR-273413V6R2014x
            StringList pendingChangeStateList = new StringList(4);
            pendingChangeStateList.addElement(PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_PUE_ECO, "state_Create"));
            pendingChangeStateList.addElement(PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_PUE_ECO, "state_DefineComponents"));
            pendingChangeStateList.addElement(PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_PUE_ECO, "state_DesignWork"));
            pendingChangeStateList.addElement(PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_PUE_ECO, "state_Review"));

            while (affectedItemItr.hasNext()) {
                updateRC = false;
                mapAffectedItem = (Map) affectedItemItr.next();
                affectedPartId = (String) mapAffectedItem.get(DomainConstants.SELECT_ID);
                affectedPart = new DomainObject(affectedPartId);
                ebom = affectedPart.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM + "," + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING, DomainConstants.TYPE_PART, selectStmts,
                        selectRelStmts, false, true, (short) 1, "", "");

                requestedChange = mapAffectedItem.get("attribute[" + ATTRIBUTE_REQUESTED_CHANGE + "].value").toString();
                objectState = (String) mapAffectedItem.get(DomainConstants.SELECT_CURRENT);

                if (CONST_FOR_RELEASE.equalsIgnoreCase(requestedChange) && UnresolvedEBOMConstants.STATE_PART_PRELIMINARY.equals(objectState)) {
                    affectedPart.promote(context);
                }

                if (CONST_FOR_REVISE.equalsIgnoreCase(requestedChange)) {
                    // revise affected item
                    newPart = new DomainObject(affectedPart.reviseObject(context, false));
                    objOwner = affectedPart.getInfo(context, DomainConstants.SELECT_OWNER);
                    newPart.promote(context);
                    newPart.setOwner(context, objOwner);
                    newPart.setAttributeValue(context, DomainConstants.ATTRIBUTE_ORIGINATOR, objOwner);
                    newPartId = newPart.getId();

                    // float parent EBOM rels to new revision
                    floatParentEBOMRels(context, affectedPartId, newPartId);

                    // promote affected item to superseded state
                    affectedPart.promote(context);

                    // float pending affected item rels to new revision
                    floatAffectedItemRels(context, newPartId, affectedPartId);

                    // save ebom ids of new revision
                    ebomNewRev = newPart.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, null, newRelSelects, false, true, (short) 1, "", "");
                    Iterator itr = ebomNewRev.iterator();
                    while (itr.hasNext()) {
                        map = (Map) itr.next();
                        ebomrelId = (String) map.get(DomainRelationship.SELECT_RELATIONSHIP_ID);
                        logicalid = (String) map.get("logicalid");
                        newRevMap.put(logicalid, ebomrelId);
                    }
                }

                Iterator itr = ebom.iterator();
                Locale Local = context.getLocale();
                while (itr.hasNext()) {
                    map = (Map) itr.next();
                    // if any un-configured child part is not released, block PUE ECO promotion
                    childPartState = (String) map.get(DomainConstants.SELECT_CURRENT);
                    childPartPolicy = (String) map.get(DomainConstants.SELECT_POLICY);

                    if (!(UnresolvedEBOMConstants.POLICY_CONFIGURED_PART).equals(childPartPolicy) && !(STATE_ECPART_RELEASE.equals(childPartState))) {
                        String strAlertMessage = EnoviaResourceBundle.getProperty(context, "emxUnresolvedEBOMStringResource", Local, "emxUnresolvedEBOM.PUEECO.CheckUnreleasedParts");
                        emxContextUtilBase_mxJPO.mqlNotice(context, strAlertMessage);
                        return 1;
                    }

                    relType = (String) map.get(DomainRelationship.SELECT_RELATIONSHIP_TYPE);
                    ebomrelId = (String) map.get(DomainRelationship.SELECT_RELATIONSHIP_ID);
                    logicalid = (String) map.get("logicalid");
                    slChangeRelIds = toStringList(map.get("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE + "].id"));
                    slChangeECOIds = toStringList(map.get("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE + "].from.id"));

                    // EBOM Pending relationships need to be processed for affected items that are For Revise or For Release
                    if (UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING.equals(relType)) {

                        for (int i = 0; i < slChangeECOIds.size(); i++) {
                            if (ecoId.equals(slChangeECOIds.get(i))) {
                                // if pending change is same as the ECO being released
                                // update EBOM Change to EBOM Change History and EBOM Pending to EBOM
                                ebomChangeList.add((String) slChangeRelIds.get(i));
                                ebomPendingList.add(ebomrelId);
                                mapEffectivity = new HashMap();
                                mapEffectivity.put(DomainConstants.SELECT_RELATIONSHIP_ID, ebomrelId);
                                mlEffectivity.add(mapEffectivity);
                            } else if (CONST_FOR_RELEASE.equalsIgnoreCase(requestedChange)) {
                                // if pending change is NOT same as the ECO being released
                                // update requested change on affected item relationships of pending changes
                                updateRC = true;
                            }
                        }

                    } else {
                        // EBOM relationships need to be processed only for affected items that are for revise
                        if (CONST_FOR_REVISE.equalsIgnoreCase(requestedChange)) {
                            slEBOMChangeHistoryRels = toStringList(map.get("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY + "].from.id"));
                            slEBOMChangeHistoryOp = toStringList(
                                    map.get("tomid[" + UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY + "].attribute[" + UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION + "].value"));

                            slEUObjects = toStringList(map.get("frommid[" + RELATIONSHIP_EFFECTIVITY_USAGE + "].to.id"));
                            slEUIndex = toStringList(map.get("frommid[" + RELATIONSHIP_EFFECTIVITY_USAGE + "].attribute[" + ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX + "].value"));
                            slNUERels = toStringList(map.get("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].id"));
                            slNEUObjects = toStringList(map.get("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].to.id"));
                            slNEUNames = toStringList(map.get("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].to.name"));
                            slNEUIndex = toStringList(map.get("frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "].attribute[" + ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX + "].value"));

                            // get ebom rel on the new revision corresponding to the ebom rel on the old revision
                            newEBOMRelId = (String) newRevMap.get(logicalid);

                            for (int i = 0; i < slEBOMChangeHistoryRels.size(); i++) {
                                // connect ebom change history rels from previous revision to the new revision
                                connectObjToRel(context, (String) slEBOMChangeHistoryRels.get(i), newEBOMRelId, UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION, (String) slEBOMChangeHistoryOp.get(i),
                                        UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY);
                            }

                            // create a list of rel ids to clear proposed expression, which have pending change
                            if (!slChangeECOIds.isEmpty()) {
                                relMap = new HashMap();
                                relMap.put(DomainObject.SELECT_RELATIONSHIP_ID, ebomrelId);
                                affectedRelIds.add(relMap);
                            }

                            // copy EU rels to replicated EBOM rel
                            for (int i = 0; i < slEUObjects.size(); i++) {
                                connectRelToObj(context, (String) slEUObjects.get(i), newEBOMRelId, ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX, (String) slEUIndex.get(i), RELATIONSHIP_EFFECTIVITY_USAGE);
                            }

                            StringList pendingNEList = new StringList();
                            // copy NEU rels to replicated EBOM rel
                            for (int i = 0; i < slNEUObjects.size(); i++) {
                                connectRelToObj(context, (String) slNEUObjects.get(i), newEBOMRelId, ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX, (String) slNEUIndex.get(i),
                                        RELATIONSHIP_NAMED_EFFECTIVITY_USAGE);

                                // disconnect NEU rel corresponding to the PUE ECO being released
                                // IR-273413V6R2014x
                                if (ecoName.equals((String) slNEUNames.get(i)))
                                    DomainRelationship.disconnect(context, (String) slNUERels.get(i));
                                else {
                                    pendingNEList.addElement((String) slNUERels.get(i));
                                }
                            }

                            // IR-273413V6R2014x
                            if (pendingNEList.size() > 0) {
                                String SELECT_CHANGEOBJ_CURRENT = "to.to[" + EffectivityFramework.RELATIONSHIP_NAMED_EFFECTIVITY + "].from.current";

                                StringList selects = new StringList(2);
                                selects.addElement(SELECT_CHANGEOBJ_CURRENT);
                                selects.addElement("id");

                                MapList infoMapList = DomainRelationship.getInfo(context, (String[]) pendingNEList.toArray(new String[0]), selects);

                                for (int j = 0; j < infoMapList.size(); j++) {
                                    String changeState = (String) ((Map) infoMapList.get(j)).get(SELECT_CHANGEOBJ_CURRENT);

                                    if (pendingChangeStateList.contains(changeState)) {
                                        DomainRelationship.disconnect(context, (String) ((Map) infoMapList.get(j)).get("id"));
                                    }
                                }
                            }
                            ...
                            for (int i = 0; i < slChangeECOIds.size(); i++) {
                                if (!ecoId.equals(slChangeECOIds.get(i))) {
                                    // if pending change is NOT same as the ECO being released
                                    // copy ebom change to new rev (with an operation of remove)
                                    // create a list of pending ebom change rels to disconnect from the old rev
                                    connectObjToRel(context, slChangeECOIds.get(i).toString(), newEBOMRelId, UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION, "Remove",
                                            UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE);
                                    changeRelsDisconnectList.add((String) slChangeRelIds.get(i));

                                } else {
                                    // if pending change is same as the ECO being released
                                    // copy ebom change to new rev as ebom change history (with an operation of remove)
                                    // create a list of pending ebom change rels to disconnect from the old rev
                                    connectObjToRel(context, slChangeECOIds.get(i).toString(), newEBOMRelId, UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION, "Remove",
                                            UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY);
                                    changeRelsDisconnectList.add((String) slChangeRelIds.get(i));
                                    mapEffectivity = new HashMap();
                                    mapEffectivity.put(DomainConstants.SELECT_RELATIONSHIP_ID, newEBOMRelId);
                                    mlEffectivity.add(mapEffectivity);
                                }
                            }
                        }
                ...
                // update requested change if pending ECOs exist
                if (updateRC) {
                    updateRequestedChange(context, ecoId, affectedPart);
                }
            }

            if (mlEffectivity.size() > 0)
                effectivity.validateRelEffectivity(context, ecoId, mlEffectivity);

            updateRelTypes(context, ebomPendingList, ebomChangeList);

            // disconnect all pending ebom change rels
            if (!changeRelsDisconnectList.isEmpty()) {
                strArrRelIds = (String[]) changeRelsDisconnectList.toArray(new String[changeRelsDisconnectList.size()]);
                DomainRelationship.disconnect(context, strArrRelIds);
            }

            // clear proposed expression
            if (affectedRelIds.size() > 0) {
                effectivity.setChangeId("");
                effectivity.refreshProposedEffectivity(context, affectedRelIds);
            }
    ...
    public void floatOldRevRels(Context context, String oldRevId, String newRevId) throws Exception {
        ...
        relSels.addElement(DomainRelationship.SELECT_RELATIONSHIP_ID);
        MapList mapList = oldRev.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, objSels, relSels, true, false, (short) 1, null, null);
        Iterator itr = mapList.iterator();
        while (itr.hasNext()) {
            ...
            DomainRelationship.setToObject(context, ebomRelId, newRev);
            DomainRelationship.connect(context, new DomainObject(partId), UnresolvedEBOMConstants.RELATIONSHIP_EBOM_HISTORY, new DomainObject(oldRev));
        }
    }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments effectivityExpression: the actual effectivity expression from which the context Ids have to be retrieved
    ...
    /**
     * Added for IR-066748V6R2011x Method to construct StringList of products for the given StringList of connectionids..
     * @param context
     *            the Ematrix code context object.
     * @param slProductList
     *            the products list.
     * @param isFromEBOM
     *            value true for EBOM and false for EBOM Pending
     ...
     * @param context
     *            the Matrix code context object
     * @param args
     *            the packed hashMap of request parameters
    ...
    public StringList getAffectedProducts(Context context, String[] args) throws Exception {
        ...
        for (int index = 0; index < objectList.size(); index++) {
            ...
            StringList prodList = PUEChange.getStringListOfProductsForChangeId(context, strObjId);
     ...
     *            the eMatrix code context object
     * @param prodList
     *            the given stringlist of products
...

    {
        ...
        while (prodIterator.hasNext()) {
            ...
            prodBuffer.append("<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?emxSuiteDirectory=" + "unresolvedebom" + "&amp;jsTreeID=" + "null" + "&amp;mode=insert&amp;suiteKey="
                    + "unresolvedebom" + "&amp;objectId=" + prodMap.get(DomainConstants.SELECT_ID) + "', '700', '600', 'false', 'popup', '')\">" + prodMap.get(DomainConstants.SELECT_NAME) + "</a>");
        }
    ...
    /**
     * Gets all the PUE ECO id which is not cancelled in MapList which is connected to Model with Unit and Feature option effectivity. context ematrix context boolean addMultiPart args required
     * arguments throws if any operation fails returns MapList which contains valid ecoIds.
     */
    public MapList getPUEECOsImpl(Context context, boolean addMultiPart, String[] args) throws Exception {
        ...
        MapList mapListPUEChangs = domObj.getRelatedObjects(context, EffectivityFramework.RELATIONSHIP_EFFECTIVITY_USAGE, EffectivityFramework.TYPE_NAMED_EFFECTIVITY, objectSelect, null, true, false,
                (short) 1, null, null, (short) 0, false, false, (short) 0, null, null, null, null);
    ...
    /**
     * This method checks the expression for single model or multi model expression by comparing model Ids. expression effectivity expression throws exception if any operation fails returns true if it
     * is single model effectivity
     */
    ...
    /**
     * This method gets all the PUE ECO ids connected to Model with Feature option effectivity. context ematrix context modelId Model Id throws exception if any operation fails returns StringList
     * which contains ECO ids
     */
    private StringList getFeatureOptionEffectivityECOIds(Context context, String modelId) throws Exception {
        ...
        String REL_CANDIDATE_CONFIG_FEATURES = PropertyUtil.getSchemaProperty(context, "relationship_CandidateConfigurationFeatures");
        ...
        /*
         * //selectable for getting CCA / PUE ECO from Model Context. String SELECT_CCA_PUEECO_ID_MODEL_CONTEXT = "from[" + REL_CANDIDATE_CONFIG_FEATURES + "].to.from[" + REL_CONFIG_OPTIONS +
         * "].tomid[" + EffectivityFramework.RELATIONSHIP_EFFECTIVITY_USAGE + "].from.to[" + EffectivityFramework.RELATIONSHIP_NAMED_EFFECTIVITY + "].from[" + types + "].id";
         */

        // selectable for getting CCA / PUE ECO from Product Context.
        String SELECT_CO_PUEECO_ID_PRODUCT_CONTEXT = "from[" + REL_MAIN_PRODUCT + "].to.from[" + REL_CONFIG_FEATURES + "].to.from[" + REL_CONFIG_OPTIONS + "].tomid["
                + EffectivityFramework.RELATIONSHIP_EFFECTIVITY_USAGE + "].from.to[" + EffectivityFramework.RELATIONSHIP_NAMED_EFFECTIVITY + "].from[" + types + "].id";
    ...
    /**
     * This method checks for PUE ECO which is cancelled and adds all the valid PUE ECO ids to dataList connected to Model with Feature option effectivity. context ematrix context dataList MapList
     * ecoIdSet HashSet with unigue ECO ids boolean addmultipart throws exception if any operation fails returns void
     */
    ...
    /**
     * Method is called when CO Create/Edit with effectivity, creates CCA with Effectivity, and also called from COnfigured BOM Add To Create CO.
     * @param context
     *            ematrix context
     * @param args
     * @throws FrameworkException
     * @throws Exception
     */
    public HashMap createCCA(Context context, String[] args) throws FrameworkException, Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        if (paramMap != null && requestMap != null) {
            String effectivityExpr = (String) requestMap.get("ChangeEffectivityOID");

            if (ChangeUtil.isCFFInstalled(context)) {
                String fromConfiguredBOMViewValue = (String) requestMap.get("fromConfiguredBOMView");

                if (UIUtil.isNotNullAndNotEmpty(effectivityExpr)) {
                    String coId = ("create".equals((String) requestMap.get("mode"))) ? (String) paramMap.get("newObjectId") : (String) requestMap.get("objectId");

                    String ccaId = PUEChange.createCCA(context, coId, effectivityExpr);

                    if ("create".equals((String) requestMap.get("mode"))) {
                        resultMap.put("caId", ccaId);
                    }

                    String selectedObjIds = (String) ((Map) programMap.get(ChangeConstants.REQUEST_MAP)).get("selectedPartsList"); // if it is coming from configured BOM Add to
                    StringList selectedObjIdList = new StringList();
                    if (ChangeUtil.isNullOrEmpty(selectedObjIds)) {
                        String[] selectedRowIds = (String[]) requestMap.get("sourceAffectedItemRowIds"); // if it is coming from My ENG view.

                        selectedObjIdList = getConfiguredPartIds(context, selectedRowIds);
                    } else {
                        selectedObjIdList = (StringList) ChangeUtil.getObjectIdsRelIdsMapFromTableRowID(selectedObjIds.split("~")).get("ObjId");
                    }

                    if (selectedObjIdList.size() > 0 && "true".equalsIgnoreCase(fromConfiguredBOMViewValue)) {
                        programMap.put("CreateMode", "CreateECO");
                        programMap.put("affectedItems", selectedObjIdList);
                        programMap.put("targetECOId", ccaId);

                        connectOrDisconnectAffectedItems(context, JPO.packArgs(programMap));
                    }
                } else { // if effectivity is not defined then check any part is selected
                    String selectedObjIds = (String) ((Map) programMap.get(ChangeConstants.REQUEST_MAP)).get("selectedPartsList");
                    String[] selectedRowIds = (String[]) requestMap.get("sourceAffectedItemRowIds");

                    if (UIUtil.isNotNullAndNotEmpty(selectedObjIds) || new ChangeUtil().isConfiguredPartSelected(context, selectedRowIds)) {
                        action = "error";
                        mesage = EnoviaResourceBundle.getProperty(context, "emxUnresolvedEBOMStringResource", context.getLocale(), "emxUnresolvedEBOM.Select.Effectivity");
                    }
                }
            }
        }
     ...
     */
    public void cancelCCA(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);

        String changeId = (String) programMap.get("objectId");
        String cancelReason = (String) programMap.get(PropertyUtil.getSchemaProperty(context, "attribute_ReasonForCancel"));

        DomainObject domObj = DomainObject.newInstance(context, changeId);

        String cancelState = PropertyUtil.getSchemaProperty(context, "policy", UnresolvedEBOMConstants.POLICY_PUE_ECO, "state_Cancelled");

        if (!domObj.getCurrentState(context).getName().equals(cancelState)) { // if CCA is cancelled manually from CCA properties page then dont call CCA cancel
            new PUEChange().cancelPUEECO(context, new String[] { changeId }, cancelReason);
        }
    }

    /**
     * This api will be called when CCA OR PUEECO promoted from Create to Define Components. api returns 1 if user trying to promote CCA which does not have any affected items connected to it.
     * @param context
     *            ematrix context
     * @param args
     * @return check trig result
     * @throws Exception
     */
    public int checkAffectedItem(Context context, String[] args) throws Exception {
        String objectId = args[0];
        String type = args[1];
        ...
        if (ChangeConstants.TYPE_CCA.equals(type)) {
            ...
            if (!stateCancelled.equals(nextState)) {
                String affetcedItemRelExits = DomainObject.newInstance(context, objectId).getInfo(context, "from[" + DomainConstants.RELATIONSHIP_AFFECTED_ITEM + "]");

                if ("False".equalsIgnoreCase(affetcedItemRelExits)) {
                    emxContextUtilBase_mxJPO.mqlNotice(context,
                            EnoviaResourceBundle.getProperty(context, "emxUnresolvedEBOMStringResource", context.getLocale(), "emxUnresolvedEBOM.Warning.AffectedItemConnected"));
                    checkResult = 1;
                }
            }
        }
