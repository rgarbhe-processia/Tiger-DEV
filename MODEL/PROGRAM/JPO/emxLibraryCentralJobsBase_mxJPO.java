 ...
 *
 *
 * static const char RCSID[] = $Id: /java/JPOsrc/${CLASSNAME} 1.30.2.15.1.1.1.1 Thu Nov 13 08:27:30 2008 GMT ds-skumaran Experimental${CLASSNAME}.java 1.2 Mon Oct 01 04:47:29 2007 GMT sgudlavalleti
 * Experimental$
 */
import java.io.File;
...
public class emxLibraryCentralJobsBase_mxJPO extends emxJob_mxJPO {
     ...
     * @since AEF 9.5.1.1
     */
    public emxLibraryCentralJobsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
        if (args.length == 2) {
     ...
     * @param args
     *            , String array containing the Library Id
     * @return int
     * @throws Exception
     */
     ...
     *            the ENOVIA <code>Context</code> object
     * @param args
     *            , containing the names of Attribute Group
     * @return int
     * @throws Exception
     */
     ...
     *            the ENOVIA <code>Context</code> object
     * @param args
     *            , String containing Attribute Group name
    ...
    public void importAttributeGroupASync(Context context, String args[]) throws FrameworkException, Exception {
        try {
            ...
            messageBuff
                    .append(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()), "emxMultipleClassification.Command.Import"));
            if (args[0].indexOf("|") != -1)
                messageBuff.append("\t").append(args[0]);
            else
                messageBuff.append("\t").append(args[0].substring(args[0].indexOf("|") + 1, args[0].length()));
            messageBuff.append("\t").append(
                    EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()), "emxMultipleClassification.Command.ImportHeading"));
            messageBuff.append("\t").append(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                    "emxMultipleClassification.Job.AGImportInitialSuccess"));
            messageBuff.append("\t");
            LibraryCentralJobs job = (LibraryCentralJobs) Job.getInstance(context, jobId, "CLASSIFICATION");
            String logMsg = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                    "emxMultipleClassification.Job.InitiateAGImport");
    ...
    public void importLibraryASync(Context context, String args[]) throws FrameworkException, Exception {
        try {
            ...
            messageBuff.append("\t").append(args[0]);
            messageBuff.append("\t").append(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                    "emxMultipleClassification.Command.ImportHeadingLibrary"));
            messageBuff.append("\t").append(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                    "emxMultipleClassification.Job.GLImportInitialSuccess"));
            messageBuff.append("\t");
            LibraryCentralJobs job = (LibraryCentralJobs) Job.getInstance(context, jobId, "CLASSIFICATION");
            String logMsg = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                    "emxMultipleClassification.Job.InitiateGLImport");
            job.logEntry(context, "import_log.txt", logMsg, messageBuff.toString() + logMsg + " " + args[1]);
            job.setProgressPercent(5);
     ...
     */
    public static MapList getLibraryJobs(Context context, String args[], String filter) throws FrameworkException {
    ...
    public Vector getDownloadedFile(Context context, String args[]) throws FrameworkException {
        ...
        try {
            ...
            for (Iterator itr = objectList.iterator(); itr.hasNext();) {
                ...
                String initMsg = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                        "emxMultipleClassification.Command.ExportHeading");
