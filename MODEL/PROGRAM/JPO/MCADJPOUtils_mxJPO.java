...
public class MCADJPOUtils_mxJPO extends IEFBaseJPO_mxJPO {
    ...
    /**
     * Common Entry Point to JPO After initialization, the controll is given to the apropriate method depending on requested action
     */
     ...
     *            matrix context.
     * @param args
     *            [0] Global config object in serialized form.
     * @param args
     *            [1] business object id of the major component.
    ...
    public void getBoIdForCheckout(Context _context, Hashtable resultAndStatusTable) throws MCADException {
        ...
        try {
            ...
            // [NDM : QWJ] START
            /*
             * else if(sBusIdToCheckout.equals(_busObjectID)) { resultAndStatusTable.put(MCADServerSettings.JPO_EXECUTION_RESULT, sBusIdToCheckout); }
             */
            else {
                /*
                 ...
                 * bo.open(_context);
                 * 
                 * Hashtable msgTable = new Hashtable(); msgTable.put("TYPE1", sType); msgTable.put("NAME1", sName); msgTable.put("REVISION1", sRev); msgTable.put("TYPE2", bo.getTypeName());
                 * msgTable.put("NAME2", bo.getName()); msgTable.put("REVISION2", bo.getRevision());
                 */
    ...
    /**
     * Description: Decides whether the business object is newly created using "Generate New Structure". The logic is : if the value of attribute "Newly Created in Matrix" on CAD Model is "true", then
     * it is created using "Generate New Structure".
     * @param Business
     *            object
     * @return boolean value which is the information about whether given business object is generated by "Generate New Structure"
     */
    ...
    /**
     * Description: Returns the latest version of the Business object which has files.
     * @param Business
     *            object list
     * @return ID of the versioned object. Blank string if none of the versions have files.
     */
     ...
     * @param Business
     *            object (in opened state) in question.
    ...
    public void getBoIdToFinalize(Context _context, Hashtable resultAndStatusTable) throws MCADException {
        ...
        try {
            ...
            /*
             * else if(_util.isMajorObject(_context, _busObjectID)) // [NDM] is Major Object { String busIdToCheckout = null;
             * 
             * //if object is finalized, get its id // [NDM] Comment if(_generalUtil.isBusObjectFinalized(_context, _busObject)) { busIdToCheckout = _busObjectID; } else { //Get the id of the latest
             * minor. busIdToCheckout = getLatestMinorObjectId(_context, _busObject);
             * 
             * if(busIdToCheckout.equals("")) busIdToCheckout = _busObjectID; }
             * 
             * resultAndStatusTable.put(MCADServerSettings.JPO_EXECUTION_RESULT, busIdToCheckout); }
             */
            else {
    ...
    /**
     * This method returns the id of the relevant object which has files If the input id is that of a versioned type or if the input object is finalized, the input id is returned back.Otherwise id of
    ...
    // [NDM:QWJ} : Start
    /**
     * Description: Returns the id of business object to be checked out given an id of any MCAD type object based on the following set of rules. Rules for NDM: 1. If object is major object, it must
     * have file, return its id. 2. If object is minor object and object has files (means older version), return its id. 3. If object is minor object and object has NO files (means latest minor), get
     * major object and returns its id.
     * @throws Exception
     */
    ...
    /**
     * Description: Returns the id of business object to be checked out given an id of any MCAD type object based on the following set of rules. Rules: 1.If object is major type,is finalized and has
     * files, return its id.If there are NO files, return blank string 2.If object is major type, is NOT finalized and if the latest minor has files return the lates minor object's id.If the latest
     * minor object has NO files, return blank string 3.If object is minor type and has files return its id.If it has NO files return blank string
     */
    /*
     * public String getBoIdToCheckout(Context _context, String sBusId) { String busIdToCheckout = "";
     * 
     * try { MCADMxUtil mxUtil = new MCADMxUtil(_context, new MCADServerResourceBundle(""), new IEFGlobalCache());//[NDM]:for getting mcadmxutil BusinessObject bo = new BusinessObject(sBusId);
     * bo.open(_context); String boType = bo.getTypeName();
     * 
     * //check if the object is major type if(mxUtil.isMajorObject(_context, sBusId)) // [NDM] is Major Object { //if object is finalized, get its id //if(_generalUtil.isBusObjectFinalized(_context,
     * bo)) //{ // busIdToCheckout = sBusId; //} //else //{ //object is not finalized.Get the id of the latest minor. //busIdToCheckout = getLatestMinorObjectId(_context, bo); busIdToCheckout =
     * sBusId; // [NDM:QWJ] //} } else { //object is a minor object.Get its id //check if the object is with BulkLoading and return Major ID if(_generalUtil.isObjectBulkLoaded(_context, sBusId)) {
     * busIdToCheckout = _util.getMajorIDByLatestVersion(_context, sBusId); } else if(_generalUtil.isBusObjectFinalized(_context, bo)) { //if object is finalized, get the id of the major object bo =
     * _util.getMajorObject(_context, bo); if(bo!=null) { bo.open(_context); busIdToCheckout = bo.getObjectId(); bo.close(_context); } else { System.out.println("No Business Object Found"); } } else {
     * //object is not finalized, get its id busIdToCheckout = sBusId; } }
     * 
     * bo.close(_context);
     * 
     * if(!busIdToCheckout.trim().equals("")) { BusinessObject boToCheckout = new BusinessObject(busIdToCheckout); } } catch(Exception _ex) { _ex.printStackTrace(); busIdToCheckout = "false|" +
     * _ex.getMessage(); }
     * 
     * return busIdToCheckout; }
     */
