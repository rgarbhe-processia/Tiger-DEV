...
/**
 * The class ${CLASSNAME} provides API for validation and checks on Classifications and classified end items.
 * @exclude
 */
public class emxLibraryCentralClassificationRuleBase_mxJPO implements LibraryCentralConstants {
     ...
     */
    public emxLibraryCentralClassificationRuleBase_mxJPO(Context context, String[] args) throws Exception {
     ...
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param releasedObjectId
     *            object ID of the released revision
     * @param flgDisconnectLaterRevs
     *            flag indicating whether to disconnect later revisions
     * @return String 0.0
     * @throws Exception
     *             if the operation fails
     */
    public static String releasedRevDisplaceOtherRevs(Context context, String releasedObjectId, boolean flgDisconnectLaterRevs) throws Exception {
        try {
            ...
            Vector revList = getClassifiedItemRevisions(context, releasedObjectId, flgDisconnectLaterRevs);
            DomainObject releasedObj = new DomainObject(releasedObjectId);
            String strType = releasedObj.getInfo(context, DomainConstants.SELECT_TYPE);
            ...
            while (revIter.hasNext()) {
                ...
                String strOriginalInterfacesResult = MqlUtil.mqlCommand(context, strOriginalInterfacesCmd, true, strObjectId, "interface", ",").trim();
                StringList originalInterfaces = FrameworkUtil.split(strOriginalInterfacesResult, ",");

                // If this object is not classified at all, or was previously
                // classified,
                // but is already in the "hidden" state, no need to do anything
                // with it.
                if (originalInterfaces.isEmpty() || originalInterfaces.contains(LibraryCentralConstants.INTERFACE_CLASSIFICATION_SEARCH_FILTER)) {
                    continue;
                }
                ...
                MapList containingClassesInfoList = doObj.getRelatedObjects(context, RELATIONSHIP_CLASSIFIED_ITEM, TYPE_CLASSIFICATION, sSelects, relSelects, true, false, (short) 1, null, null, 0);

                // Where ever a classification object contains this rev, replace
                // it
                // with the released rev
                Iterator containerInfoIter = containingClassesInfoList.iterator();
                while (containerInfoIter.hasNext()) {
                    ...
                    strBusWhere.append("'");
                    MapList alreadyContainingReleasedObj = releasedObj.getRelatedObjects(context, RELATIONSHIP_CLASSIFIED_ITEM, TYPE_CLASSIFICATION, sSelects, relSelects, true, false, (short) 1,
                            strBusWhere.toString(), null, 0);
                    ...
                    String strReleasedInterfacesResult = MqlUtil.mqlCommand(context, strReleasedInterfacesCmd, true, releasedObjectId, "interface", ",").trim();
                    StringList strReleasedInterfaces = FrameworkUtil.split(strReleasedInterfacesResult, ",");
                    ...
                    if (alreadyContainingReleasedObj.size() == 0 && strReleasedInterfaces.size() == 0) {
                        DomainRelationship.connect(context, fromObject, RELATIONSHIP_CLASSIFIED_ITEM, releasedObj);
                    }
                }
                ...
                String strRemainingInterfacesResult = MqlUtil.mqlCommand(context, strRemainingInterfacesCmd, true, strObjectId, "interface", ",").trim();
                StringList remainingInterfaces = FrameworkUtil.split(strRemainingInterfacesResult, ",");
                ...
                if (!remainingInterfaces.contains(LibraryCentralConstants.INTERFACE_CLASSIFICATION_SEARCH_FILTER)) {
     ...
     *            object ID of the released revision
     * @param flgDisconnectLaterRevs
     *            flag indicating whether to include later revision
     * @return vector containg Object IDs of the revisions
     * @throws Exception
     *             if the operation fails
     */
    public static Vector getClassifiedItemRevisions(Context context, String strObjectId, boolean flgDisconnectLaterRevs) throws Exception {
        Vector vecResult = new Vector();
        ...
        if (flgDisconnectLaterRevs) {
            MapList revList = doObj.getRevisionsInfo(context, sSelects, new StringList());
     ...
     *             if the operation fails
     */
    public static String getPreviousRevision(Context context, DomainObject doObj) throws Exception {

        String strObjectId = "";
        BusinessObject busObj = (BusinessObject) doObj.getPreviousRevision(context);
        if (busObj != null) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param DomainObject
     *            doObj of the released revision
     * @return Vector containg non released revision
     * @throws Exception
     *             if the operation fails
     */
    public static Vector getPreviousRevisions(Context context, DomainObject doObj) throws Exception {
        ...
        if (doObj != null) {
            ...
            while (strObjectId != null && !strObjectId.equals("")) {
                ...
                strObjectId = getPreviousRevision(context, new DomainObject(strObjectId));
            }
        }
     ...
     * @param args
     *            contains a Map with the following entries: 0 - Object ID 1 - whether to delete later revisions true/false @ return String 0.0
     * @throws Exception
     *             if a major failure occurs
     */
    public static String releasedRevDisplaceOtherRevs(Context ctx, String[] args) throws Exception {
