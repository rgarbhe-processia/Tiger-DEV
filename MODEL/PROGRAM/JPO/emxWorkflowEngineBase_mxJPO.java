...
public class emxWorkflowEngineBase_mxJPO {
    ...
    public emxWorkflowEngineBase_mxJPO(Context context, String[] args) throws Exception {
        ...
        /*
         * String command = "print bus \"" + workflowType + "\" \"" + _processName + "\" * select id dump"; HashMap resMap = processMql(context, command);
         */
        String command = "";
        String result = null;
            ...
            if (_parentProcessName != null && !_parentProcessName.equals("")) {
                ...
                if (result != null) {
                    ...
                    if (obj != null) {
                        ...
                        obj.addRelatedObject(context, new RelationshipType(PropertyUtil.getSchemaProperty(context, "relationship_WorkflowSubProcess")), false, _workflow.getInfo(context, "id"));
        ...
        try {
            ...
            if (event.equalsIgnoreCase("start")) {
                ...
                MapList mList = _workflow.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, "relationship_WorkflowSubProcess"),
                        PropertyUtil.getSchemaProperty(context, "type_Workflow"), list, new StringList(), false, true, (short) 0, null, null);
            ...
            // Delete all tasks connected to the Business object.
                ...
                MapList mList = _workflow.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, "relationship_WorkflowTask"), PropertyUtil.getSchemaProperty(context, "type_WorkflowTask"),
    ...
    public void deliver(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            /*
             * String command = "print bus \"" + workflowTaskType + "\" \"" + taskName + "\" '-' select id dump"; HashMap resMap = processMql(context, command);
             */
            StringList selects = new StringList();
            selects.add(DomainObject.SELECT_ID);
            ...
            if (objectId == null) {
                ...
                try {
                    ...
                    Map objectMap = taskObj.getRelatedObject(context, PropertyUtil.getSchemaProperty(context, "relationship_WorkflowTaskAssignee"), true, null, relSelect);
                    if (objectMap != null) {
            ...
            if (assignees.size() == 1) {
                ...
                String messageValues[] = { PropertyUtil.getSchemaProperty(context, "type_WorkflowTask"), taskObj.getInfo(context, "name"), PropertyUtil.getSchemaProperty(context, "type_Workflow"),
                        _workflow.getInfo(context, "name"), _workflow.getInfo(context, "owner"), instructions, dueDate };
                ...
                // Bug No 318930 start
                /*
                 * The below code executes only when a) Assignee of an activity is Role, Group having more than one user 1. read the process name from workflow 2. Got the assignee of activity 3. set
                 * the assignee as owner
                 */
                ...
                String messageValues[] = { PropertyUtil.getSchemaProperty(context, "type_WorkflowTask"), taskObj.getInfo(context, "name"), PropertyUtil.getSchemaProperty(context, "type_Workflow"),
                        _workflow.getInfo(context, "name"), _workflow.getInfo(context, "owner"), instructions, dueDate, assignee, assigneeType };
                MailUtil.sendNotification(context, toList, // To List
                        null, // Cc List
                        null, // Bcc List
    ...
    public void resolve(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String messageValues[] = { PropertyUtil.getSchemaProperty(context, "type_WorkflowTask"), assignee };
    ...
    private void deleteTasks(Context context, String objectId) throws Exception {
        ...
        StringList list = new StringList(1);
        list.add(DomainObject.SELECT_ID);
        MapList mList = obj.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, "relationship_WorkflowTask"), PropertyUtil.getSchemaProperty(context, "type_WorkflowTask"), list,
    ...
    private void promoteConnectedObjects(Context context) throws Exception {
        ...
        try {
            ...
            if (promoteConnected != null && promoteConnected.equalsIgnoreCase("true")) {
                ...
                MapList mList = _workflow.getRelatedObjects(context, relPattern.getPattern(), "*", objectSelects, relSelects, false, true, (short) 1, null, // objectWhere
                        null, // relWhere,
                        null, null, null);
