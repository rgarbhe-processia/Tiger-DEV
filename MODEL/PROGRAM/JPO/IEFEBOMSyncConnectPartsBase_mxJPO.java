...
public class IEFEBOMSyncConnectPartsBase_mxJPO {
    ...
    public Hashtable connectPartWithCADObject(Context context, String[] args) throws Exception {
        ...
        if (matchingPartString.equals(MATCH_CADMODEL_REV)) {
            ...
            /*
             * if(isMinorType) //[NDM] { BusinessObject majorObject = mxUtil.getMajorObject(context, cadObject); majorObject.open(context); cadModelRevision = majorObject.getRevision(); cadModelType =
             * majorObject.getTypeName();
             * 
             * majorObject.close(context); } else
             */
            {
        ...
        /*
         * if(isMinorType) //[NDM] { relatedDocsList = getRelatedDocuments(context, cadObjId, actCadModelType, cadModelName, actCadModelRev, isMinorType, new Vector(), new Vector()); } else
         */
        {
        ...
        for (int i = 0; i < relatedDocsList.size(); i++) {
            ...
            /*
             * if("true".equalsIgnoreCase(assignPartToMajor)) //[NDM] { BusinessObject busObj = new BusinessObject(boId); busObj.open(context);
             * 
             * BusinessObject majorObj = mxUtil.getMajorObject(context, busObj); if(majorObj!= null) { majorObj.open(context); boId = majorObj.getObjectId(); majorObj.close(context); isMinorType =
             * false; }
             * 
    ...
    private String getVersion(Context context/* {NDM],String confAttrAssignPartToMajor */, BusinessObject cadObject) throws Exception {
        ...
        /*
         * {NDM] if("true".equalsIgnoreCase(confAttrAssignPartToMajor)) { BusinessObject majorBusObject = mxUtil.getMajorObject(context, cadObject); if(majorBusObject == null) { majorBusObject =
         * cadObject; }
         * 
         * majorBusObject.open(context); version = majorBusObject.getRevision(); majorBusObject.close(context); } else
         */
        {
            cadObject.open(context);
    ...
    private void managePartSpecificationRelationship(Context context, String cadObjId, String partObjId/* [NDM],String assignPartToMajor */, MCADGlobalConfigObject globalConfigObject,
            String partObjName, String sourceName) throws Exception {
        ...
        if (globalConfigObject.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_INSTANCE_LIKE)) {
            ...
            if ("single".equalsIgnoreCase(ebomExpositionMode)) {
                ...
                while (alreadySynchedInstObjListItr.next()) {
                    ...
                    /*
                     * if(!"true".equals(assignPartToMajor) && globalConfigObject.isMajorType(instObjObject.getTypeName())&& !globalConfigObject.isMajorType(currentObjType)) { String minorType =
                     * globalConfigObject.getCorrespondingType(instObjObject.getTypeName()); keyString.append(minorType); keyString.append("|"); keyString.append(instObjObject.getName()); } else
                     */ // [NDM] End
                    {
    ...
    private Hashtable getTypeNameKeyIdTable(Context context, ArrayList objList) throws MatrixException {
        ...
        // To be done
        /*
         * {NDM] busSelectList.add(SELECT_ON_MAJOR + "id"); busSelectList.add(SELECT_ON_MAJOR + "type"); busSelectList.add(SELECT_ON_MAJOR + "name");
         */
        ...
        for (int i = 0; i < busWithSelectList.size(); i++) {
            ...
            /*
             * [NDM]keyString = new StringBuffer(); keyString.append(busWithSelect.getSelectData(SELECT_ON_MAJOR + "type")); keyString.append("|");
             * keyString.append(busWithSelect.getSelectData(SELECT_ON_MAJOR + "name")); typeNameKeyIdTable.put(keyString.toString(),busWithSelect.getSelectData(SELECT_ON_MAJOR + "id"));
             */
        }
    ...
    public boolean updateEBOMAttribute(Context context, Relationship ebomRel, String cadrelId, String sourceName, IEFEBOMConfigObject ebomConfObj, String findNumber, String updateFindNumber) {
        ...
        try {
            ...
            if (findNumber != null && findNumber.length() > 0 && updateFindNumber != null && updateFindNumber.equals("true")) {
                ...
                findNumber = numFormatFour.format(new Integer(Integer.parseInt(findNumber)));
    ...
    protected Hashtable getAttributeMap(Context context, IEFEBOMConfigObject ebomConfObj, Relationship relObject, String cadRel, String partRel, String sourceName) throws Exception {
    ...
    /*
     * {NDM] protected boolean updateRelationship(Context context, String sRelName, String fromSideObjId, String toSideObjId) throws MatrixException, MCADException { boolean bRelExists = false;
     * 
     * BusinessObject fromSideBO = new BusinessObject(fromSideObjId);
     * 
     * // Open from side BOf //fromSideBO.open(_context); short level = 1; Visuals vis = new Visuals();
     * 
     * //build the relationship where clause to be used in expand on bus object StringBuffer relationshipWhereClause = new StringBuffer(); relationshipWhereClause.append("(name == const\"");
     * relationshipWhereClause.append(sRelName); relationshipWhereClause.append("\""); relationshipWhereClause.append(")");
     * 
     * //call the expand method on the business object Expansion expansion = mxUtil.expandBusObject(context, fromSideBO, level, "",relationshipWhereClause.toString(),(short)0, false, vis);
     * 
     * ExpandRelationshipList filteredRelationshipList = expansion.getRelationships(); ExpandRelationshipItr expandItr = new ExpandRelationshipItr(filteredRelationshipList);
     * 
     * BusinessObject childObject = null;
     * 
     * while (expandItr.next()) { ExpandRelationship expandRel = expandItr.obj();
     * 
     * // get the to side object and add it to ret list childObject = expandRel.getTo();
     * 
     * if (childObject.getObjectId().equals(toSideObjId)) { // Modify the relationship String qtyAttrVal = (expandRel.getAttributeValues(context,ATTR_QUANTITY)).getValue(); float qty = 0; try {
     * java.text.NumberFormat nf = java.text.NumberFormat.getInstance(serverResourceBundle.getLocale()); Number quantityNumber = nf.parse(qtyAttrVal); float fQuantity = quantityNumber.floatValue();
     * qty = fQuantity; } catch(Exception e) { qty = 0; } qty = qty + 1; String updatedQtyAttrVal = String.valueOf(qty); mxUtil.setRelationshipAttributeValue(context, expandRel, ATTR_QUANTITY,
     * updatedQtyAttrVal);
     * 
     * bRelExists = true; break; } } return bRelExists; }
     */
    ...
    protected Vector getRelatedDocuments(Context context, String parentObjId, boolean isMinorType, Vector retListNew) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < associatedSpecificationInfoList.size(); i++) {
                ...
                /*
                 * if(isMinorType) //[NDM] { String typeExists = mxUtil.executeMQL(context ,"list type \"" + releventType + "\""); if(typeExists.startsWith("true|") && typeExists.length() > 5) {
                 * String correspondingType = mxUtil.getCorrespondingType(context, releventType); if(correspondingType != null && !"".equals(correspondingType)) releventType = correspondingType; } }
                 */
    ...
    /*
     * protected Vector getRelatedDocuments(Context context, String parentObjId, String parentObjType, String parentObjName, String parentObjRevision, boolean isMinorType, Vector retListNew, Vector
     * alreadyTraversedListNew) throws Exception { Vector retList = retListNew; Vector alreadyTraversedList = alreadyTraversedListNew; Hashtable objectIdTypeNameMap = new Hashtable(); Hashtable
     * typeNameObjectIdMap = new Hashtable(); Hashtable typeNameObjectRevMap = new Hashtable(); QueryIterator resultItr=null;
     * 
     * try { String typePattern = parentObjType; String namePattern = parentObjName; String revPattern = "*"; StringList busSelects = new StringList(10); String selectOnDrwRelFrom = "from[" +
     * relAssociateDrawing + "].to."; String selectOnDrwRelTo = "to[" + relAssociateDrawing + "].from.";
     * 
     * busSelects.add("id"); busSelects.add("type"); busSelects.add("name"); busSelects.add("revision"); busSelects.add("revisions"); busSelects.add(selectOnDrwRelFrom + "id");
     * busSelects.add(selectOnDrwRelFrom + "type"); busSelects.add(selectOnDrwRelFrom + "name"); busSelects.add(selectOnDrwRelFrom + "revision"); busSelects.add(selectOnDrwRelFrom + "revisions");
     * busSelects.add(selectOnDrwRelFrom + "attribute[" + attrCADType + "]"); busSelects.add(selectOnDrwRelTo + "id"); busSelects.add(selectOnDrwRelTo + "type"); busSelects.add(selectOnDrwRelTo +
     * "name"); busSelects.add(selectOnDrwRelTo + "revision"); busSelects.add(selectOnDrwRelTo + "revisions"); busSelects.add(selectOnDrwRelTo + "attribute[" + attrCADType + "]");
     * 
     * resultItr = mxUtil.executeQuery(context, typePattern, namePattern, revPattern, busSelects);
     * 
     * while(resultItr.hasNext()) { BusinessObjectWithSelect busWithSelect = resultItr.next();
     * 
     * String parentRevision = busWithSelect.getSelectData("revision"); StringList parentRevisions = busWithSelect.getSelectDataList("revisions"); StringList connectedObjIds =
     * busWithSelect.getSelectDataList(selectOnDrwRelFrom + "id"); StringList connectedObjTypes = busWithSelect.getSelectDataList(selectOnDrwRelFrom + "type"); StringList connectedObjNames =
     * busWithSelect.getSelectDataList(selectOnDrwRelFrom + "name"); StringList connectedObjRevisions = busWithSelect.getSelectDataList(selectOnDrwRelFrom + "revision"); StringList connectedObjAllRevs
     * = busWithSelect.getSelectDataList(selectOnDrwRelFrom + "revisions"); StringList cadTypes = busWithSelect.getSelectDataList(selectOnDrwRelFrom + "attribute[" + attrCADType + "]"); StringList
     * connectedObjIdsTo = busWithSelect.getSelectDataList(selectOnDrwRelTo + "id"); StringList connectedObjTypesTo = busWithSelect.getSelectDataList(selectOnDrwRelTo + "type"); StringList
     * connectedObjNamesTo = busWithSelect.getSelectDataList(selectOnDrwRelTo + "name"); StringList connectedObjRevisionsTo = busWithSelect.getSelectDataList(selectOnDrwRelTo + "revision"); StringList
     * connectedObjAllRevsTo = busWithSelect.getSelectDataList(selectOnDrwRelTo + "revisions"); StringList cadTypesTo = busWithSelect.getSelectDataList(selectOnDrwRelTo + "attribute[" + attrCADType +
     * "]");
     * 
     * if(null != connectedObjIdsTo) { if(null == connectedObjIds) { connectedObjIds = new StringList(); connectedObjTypes = new StringList(); connectedObjNames = new StringList();
     * connectedObjRevisions = new StringList(); connectedObjAllRevs = new StringList(); cadTypes = new StringList(); }
     * 
     * connectedObjIds.addAll(connectedObjIdsTo); connectedObjTypes.addAll(connectedObjTypesTo); connectedObjNames.addAll(connectedObjNamesTo); connectedObjRevisions.addAll(connectedObjRevisionsTo);
     * connectedObjAllRevs.addAll(connectedObjAllRevsTo); cadTypes.addAll(cadTypesTo); }
     * 
     * if(null != connectedObjIds) { for(int i=0; i<connectedObjIds.size(); i++) { String connectedObjId = (String)connectedObjIds.get(i); String connectedObjType = (String)connectedObjTypes.get(i);
     * String connectedObjName = (String)connectedObjNames.get(i); String connectedObjRevision = (String)connectedObjRevisions.get(i); String cadType = (String)cadTypes.get(i);
     * 
     * if(null != connectedObjId && !connectedObjId.equals("") && isLatestRevision(parentObjRevision, parentRevision, parentRevisions) && ("presentation".equalsIgnoreCase(cadType) ||
     * "drawing".equalsIgnoreCase(cadType))) { StringBuffer typeNameKey = new StringBuffer(); typeNameKey.append(connectedObjType); typeNameKey.append("|"); typeNameKey.append(connectedObjName);
     * 
     * if(!typeNameObjectIdMap.containsKey(typeNameKey.toString())) { objectIdTypeNameMap.put(connectedObjId, typeNameKey.toString()); typeNameObjectIdMap.put(typeNameKey.toString(), connectedObjId);
     * typeNameObjectRevMap.put(typeNameKey.toString(), connectedObjRevision); } else { // This is to replace existing version with later version to ensure latest version is kept in the retList.
     * String existingEntryRevision = (String)typeNameObjectRevMap.get(typeNameKey.toString());
     * 
     * if(isLatestRevision(connectedObjRevision, existingEntryRevision, connectedObjAllRevs)) { objectIdTypeNameMap.put(connectedObjId, typeNameKey.toString());
     * typeNameObjectIdMap.put(typeNameKey.toString(), connectedObjId); typeNameObjectRevMap.put(typeNameKey.toString(), connectedObjRevision); } }
     * 
     * if(!retList.contains(connectedObjId)) retList.add(connectedObjId); } } } }
     * 
     * // Recursive call to check if the drawing or presentation has any associated_drawing relationship
     * 
     * Iterator itr = retList.iterator(); if(itr.hasNext()) { String connectedObjId = (String) itr.next(); if(!alreadyTraversedList.contains(connectedObjId)) {
     * alreadyTraversedList.add(connectedObjId); String typeNameKey = (String)objectIdTypeNameMap.get(connectedObjId); Enumeration typeName = MCADUtil.getTokensFromString(typeNameKey, "|"); String
     * connectedObjType = (String)typeName.nextElement(); String connectedObjName = (String)typeName.nextElement(); String connectedObjRevision = (String)typeNameObjectRevMap.get(typeNameKey);
     * getRelatedDocuments(context, connectedObjId, connectedObjType, connectedObjName, connectedObjRevision, isMinorType, retList,alreadyTraversedList ); } } } catch (Exception _ex) {
     * MCADServerException.createManagedException(MCADAppletServletProtocol.DEFAULT_ERROR_CODE, serverResourceBundle.getString("mcadIntegration.Server.Message.RelatedDocumentError"), _ex); } finally {
     * resultItr.close(); }
     * 
     * return retList; }
     */
    ...
    protected void matchRevisionSequence(Context context, String busObjectID, String matchingPartID, boolean isMinorType) throws MCADException {
        try {
            ...
            /*
             * if(isMinorType) //[NDM] { BusinessObject _busObject = mxUtil.getMajorObject(context, partBusObject); _busObject.open(context); policyObject = _busObject.getPolicy(context);
    ...
    private void copyAttribsFromCadObjToPart(Context _context, String partObjId, String busObjectID, String busType, String instanceName, MCADGlobalConfigObject globalConfigObject, String language)
            throws Exception {
