 ...
 **
 ** $RCSfile: ${CLASS:emxCommonDocumentBase}.java $ $Revision: 2.10 $ $Date: Jul 12 2012 $
 **
...
public class emxVCDocumentBase_mxJPO extends emxCommonDocument_mxJPO {
    ...
    public static final String PROJECTSTORE_SELECT = "to[" + DomainConstants.RELATIONSHIP_PROJECT_ACCESS_KEY + "].from.from[" + DomainConstants.RELATIONSHIP_PROJECT_ACCESS_LIST
            + "].to.vcfile[1].store";
    ...
    public Map vcDocumentConnectCheckin(Context context, String[] args) throws Exception {
        try {
            ...
            if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_CREATE_VC_ZIP_TAR_GZ, VCDocument.OBJECT_ACTION_CREATE_VC_FILE_FOLDER, VCDocument.OBJECT_ACTION_CREATE_VC_ON_DEMAND)) {
            ...
            } else if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_STATE_SENSITIVE_CONNECT_VC_FILE_FOLDER, VCDocument.OBJECT_ACTION_CONNECT_VC_FILE_FOLDER,
                    VCDocument.OBJECT_ACTION_CONNECT_VC_ON_DEMAND, VCDocument.OBJECT_ACTION_CONNECT_VC_FROM_ISSUE_SEARCH)) {
                uploadParamsMap.put("incomplete", "false");
                objectMap = createConnect(context, uploadParamsMap);

                // create DOCUMENTs object with given 'Type' 'name' 'revision'
                // and copy selected file/folder to local design sync file/folder.
            } else if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_COPY_FROM_VC, VCDocument.OBJECT_ACTION_CONVERT_COPY_FROM_VC)) {
                uploadParamsMap.put("incomplete", "true");
                objectMap = createCopy(context, uploadParamsMap);

                // checkin file/folder into DesignSync from the given object
            } else if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_CHECKIN_VC_FILE, VCDocument.OBJECT_ACTION_CHECKIN_VC_FOLDER)) {
                objectMap = checkinUpdate(context, uploadParamsMap);
            }

            // convert/checkin?
            else if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_CONVERT_CHECKIN_VC_FILE_FOLDER, VCDocument.OBJECT_ACTION_CONVERT_VC_FILE_FOLDER)) {
    ...
    public HashMap connect(Context context, HashMap uploadParamsMap) throws Exception {
        try {
            ...
            if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_CONNECT_VC_FILE_FOLDER, VCDocument.OBJECT_ACTION_STATE_SENSITIVE_CONNECT_VC_FILE_FOLDER)) {
                incomplete = "false";
            }

            if (hasRealValue(objectId) && objectAction.equalsIgnoreCase(VCDocument.OBJECT_ACTION_CONVERT_CHECKIN_VC_FILE_FOLDER)) {
    ...
    /**
     * This is the method executed in common Document model to create master using FCS/NonFCS and connect to a VC file/folder.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param uploadParamsMap
     *            holds all arguments passed through checkin screens.
    ...
    public HashMap create(Context context, HashMap uploadParamsMap) throws Exception {
        try {
            ...
            if (UIUtil.isNullOrEmpty(parentId)
                    || matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_CONNECT_VC_FROM_ISSUE_SEARCH, VCDocument.OBJECT_ACTION_CREATE_VC_ON_DEMAND, VCDocument.OBJECT_ACTION_CONNECT_VC_ON_DEMAND))

            {
                ...
                String docId = FrameworkUtil.autoName(context, symDocumentType, objectGeneratorRevision, symDocumentPolicy, vault, custrev, true);
                object.setId(docId);
                ...
                if (objectAction.equalsIgnoreCase(VCDocument.OBJECT_ACTION_CONNECT_VC_ON_DEMAND)) {
                    final String relVaultedObjects = PropertyUtil.getSchemaProperty(context, "relationship_VaultedDocuments");
                    ...
                    if (parentRelName == relVaultedObjects) {
                        ...
                        domWSFolder.addRelatedObject(context, new matrix.db.RelationshipType(parentRelName), false, docId);
                    }
                }
            } else {
    ...
    /**
     * This is the method executed in DesignSync Document model to modify object When the previous button is selected.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param uploadParamsMap
     *            holds all arguments passed through checkin screens.
    ...
    /**
     * This is the method executed in common Document model to create a copy of a remote design sync file/folder. The file is copied from the specified server/path and checked in to the configured
     * server/path.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param uploadParamsMap
    ...
    public HashMap createCopy(Context context, HashMap uploadParamsMap) throws Exception {
        try {
            ...
            try {
                ...
                throw (new FrameworkException(EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(language), "emxComponents.VersionControl.DesignSyncServerNotSet")));
            }
    ...
    /**
     * Returns the local DesignSync path to store files. The path is computed based on a defined directory concatenated with the Document name.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param uploadParamsMap
     *            holds all arguments passed through checkin screens.
    ...
    public Map checkinUpdate(Context context, String[] args) throws Exception {
        try {
            ...
            if (matchesOneOf(objectAction, VCDocument.OBJECT_ACTION_CHECKIN_VC_FILE, VCDocument.OBJECT_ACTION_CHECKIN_VC_FOLDER)) {
                emxNotificationUtil_mxJPO notifyUtil = new emxNotificationUtil_mxJPO(context, null);
    ...
    public Map connectStateSensitiveICDocument(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (hasRealValue(parentId)) {
                ...
                MapList relList = taskObject.getRelatedObjects(context, DomainConstants.RELATIONSHIP_TASK_DELIVERABLE, "*", null, relSelects, false, true, (short) 1, whereClause, null);
    ...
    /**
     * This method is executed after create of document object to handle subscriptions.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param uploadParamsMap
     *            HashMap
    ...
    /**
     * This method will process DS Selectors to ensure proper format. The selector should be Branch:Version, and will default to Branch:Latest if no Version is given
     * @param selector
     *            String
     * @return String
     * @since Common 11-0
    ...
    /*
     * Gets the Checkin Status to be displayed in the form for CAD Drawing,CAD Model,Drawing Print,Mark-up This is used in the display of properties page of these objects, normally called from
     * Webform.
     * 
     * @param context the eMatrix <code>Context</code> object.
     * 
     * @param args holds the following input arguments: 0 - String containing the object id.
     * 
     * @return String.
     * 
    ...
    public String getVCCheckinStatus(Context context, String[] args) throws Exception {
        ...
        String STR_DOCUMENT_CHECKIN_STATUS_ERROR = ComponentsUtil.i18nStringNow("emxComponents.VCDocument.NoCheckInStatusError", languageStr);
    ...
    /**
     * gets the vcpath Value to be displayed in the form. This is used in the display of properties page of these objects, normally called from Webform.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
    ...
    /*
     * This method is called for updating selector for CAD Drawing, CAD Model, Drawing Print, spec Objects as update function in their respective Edit pages.
     * 
     * @param context the eMatrix <code>Context</code> object
     * 
     * @param args holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, "objectId", "old RDOIds", "New OID", "Old RDO Rel Ids".
     * 
    ...
    public boolean hasVCFileCheckinAccess(Context context, String args[]) throws Exception {
        ...
        return ("TRUE".equalsIgnoreCase(docInfo.get("vcfile")) && CommonDocument.canCheckin(context, docInfo));
    }
    ...
    public boolean hasVCFolderCheckinAccess(Context context, String args[]) throws Exception {
        ...
        return ("TRUE".equalsIgnoreCase(docInfo.get("vcfolder")) && CommonDocument.canCheckin(context, docInfo));
    }
    ...
    public boolean hasVCModuleCheckinAccess(Context context, String args[]) throws Exception {
        ...
        return ("TRUE".equalsIgnoreCase(docInfo.get("vcmodule")) && CommonDocument.canCheckin(context, docInfo));
    }
     ...
     *            - the matrix Object ID
     * @param vctype
     *            - the vc/dsfa connection type - vcfile, vcfolder, vcmodule
