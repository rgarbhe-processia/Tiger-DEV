...
/**
 * The emxSpoolBase class represents the Spool JPO Functionality Copyright (c) 1992-2015 Dassault Systemes.
 * @since R210
 **/
...
public class emxSpoolBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @param strMessage
     *            the string argument for storing message to print
     * @since R210
     */
    ...
    /**
     * This method processes the Notification Spool. To consolidate, the method finds the corresponding Notification object and the attribute Consolidation JPO. If this attribute is blank then default
     * consolidation program will be called i.e. emxSpool:consolidateNotifications otherwise custom specified consolidation program will be executed. This method will group the notification requests
     * coming from same notification objects before sending them to consolidation program.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - an array of String arguments, args[0] = name of the spool object
    ...
    public void processSpool(Context context, String[] args) throws MatrixException {
        ...
        try {
            ...
            MapList mlSpool = DomainObject.findObjects(context, // The current context object
            ...
            MapList mlNotificationRequests = domNotificationSpool.getRelatedObjects(context, // context.
    ...
    /**
     * This method consolidates the Notification Requests. Those are consolidated based on to, cc, bcc of the notification requests. After sending the consolidated notification the notification
     * request from spool are deleted. This method assumes that the notification requests passed in are from same notification object i.e. they all have same subject
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - packed MapList containing Map of Notification Request information
     * @throws MatrixException
     *             if the operation fails
     * @return void
     * @since R210
    ...
    /**
     * This method sends the Email of Consolidated Notification Requests according to toList, ccList, bccList
     * @param context
     *            The Matrix Context object
     * @param mlNotificationRequests
     *            The MapList with each Map representing the Notification Request relationship information. SELECT_ATTRIBUTE_ATTACHMENTS key represents the EMAIL_ATTACHMENT_SEPARATOR object ids
     * @throws MatrixException
     *             if operation fails
     * @return void
     * @since R210
    ...
    protected void sendNotifications(Context context, MapList mlNotificationRequests) throws MatrixException {
        try {
            ...
            for (int i = 0; i < mlNotificationRequests.size(); i++) {
                ...
                debug("sendNotification To:" + slToList + "fromAgent:" + strFromAgent + " Cc:" + slCcList + " Bcc:" + slBccList + " Subject:" + strSubject + " Attachments:" + slAttachments
                        + " strNotificationName:" + strNotificationName);
    ...
    /*
     * protected void sendNotification(Context context, StringList to, StringList cc, StringList bcc, String subject, String body, String message, StringList attachments) throws FrameworkException {
     * MailUtil.sendNotification(context, to, cc, bcc, subject, null, null, message, null, null, attachments, null); }
     */
    protected void sendNotification(Context context, StringList to, StringList cc, StringList bcc, String subject, String messageText, String messageHTML, String fromAgent, StringList replyTo,
            StringList attachments, String notifyType) throws Exception {
        emxNotificationUtil_mxJPO.sendJavaMail(context, to, cc, bcc, subject, messageText, messageHTML, fromAgent, replyTo, attachments, notifyType);
    }
    ...
    /**
     * This method checks if the given two notifications are similar by comparing the to, cc and bcc lists. It assumes they have the same subject.
     * @param mapNotificationRequest1
     *            The first notification request
     * @param mapNotificationRequest2
     *            The second notification request
     ...
     * @param objKey
     *            the key to be used to get the value from the map
     * @param map1
     *            first map
     * @param map2
     *            second map
    ...
    /**
     * This method sorts the recipients addresses in the map, which are separated by EMAIL_ADDRESS_SEPARATOR and kept against key given by strSelectKey
     * @param mapNotificationRequest
     *            The map containing Notification Request information
     * @param strSelectKey
     *            The key to be used to get the recipient addresses from map
     * @return void
     * @since R210
     */
    ...
    /**
     * This method checks if the given two notifications are similar by comparing the Notification Name. It assumes they have the same subject.
     * @param mapNotificationRequest1
     *            The first notification request
     * @param mapNotificationRequest2
     *            The second notification request
    ...
    /**
     * This method processes the notification requests where Notification Name are same. It invokes application specific consolidation JPO or Notification object if present else invokes default
     * consolidation program
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param mlSimilarNotifications
     *            - MapList containing Map of similar Notification Request information
     * @throws MatrixException
     *             if the operation fails
     * @return void
     * @since R210
    ...
    protected void processSimilarRequests(Context context, MapList mlSimilarNotifications) throws MatrixException {
        try {
            ...
            // Queries on the given criteria into a map list containing a mapping of selectable names to values
            MapList mlNotification = DomainObject.findObjects(context, // The current context object
            ...
            if ((strProgramName != null && !"".equals(strProgramName)) && (strMethodName != null && !"".equals(strMethodName))) {
