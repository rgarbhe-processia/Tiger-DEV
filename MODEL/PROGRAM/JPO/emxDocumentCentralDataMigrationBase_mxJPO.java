...
public class emxDocumentCentralDataMigrationBase_mxJPO extends DomainObject {
     ...
     * @since AEF 9.5.6.0
     */
    public emxDocumentCentralDataMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        /*
         * Author : Mike Terry Date : 02/10/03 Notes : History :
         */

        super();

        // initialize member variables.
        TYPE_PERSONALFOLDER = PropertyUtil.getSchemaProperty(context, "type_PersonalFolder");
        TYPE_PUBLICFOLDER = PropertyUtil.getSchemaProperty(context, "type_PublicFolder");
        TYPE_PROJECTVAULT = PropertyUtil.getSchemaProperty(context, "type_ProjectVault");
        TYPE_DOCUMENTSHEET = PropertyUtil.getSchemaProperty(context, "type_DocumentSheet");
        TYPE_VERSIONDOCUMENT = PropertyUtil.getSchemaProperty(context, "type_VersionDocument");
        RELATIONSHIP_VAULTEDDOCUMENTREV2 = PropertyUtil.getSchemaProperty(context, "relationship_VaultedDocumentsRev2");
        RELATIONSHIP_CONTAINS = PropertyUtil.getSchemaProperty(context, "relationship_Contains");
        RELATIONSHIP_SUBVAULTS = PropertyUtil.getSchemaProperty(context, "relationship_SubVaults");
        RELATIONSHIP_VERSION = PropertyUtil.getSchemaProperty(context, "relationship_Version");
        RELATIONSHIP_PUBLISHSUBSCRIBE = PropertyUtil.getSchemaProperty(context, "relationship_PublishSubscribe");
        POLICY_PROJECTVAULT = PropertyUtil.getSchemaProperty(context, "policy_ProjectVault");
        POLICY_VERSIONDOCUMENT = PropertyUtil.getSchemaProperty(context, "policy_VersionDocument");
        ...
        ATTRIBUTE_GLOBALREAD = PropertyUtil.getSchemaProperty(context, "attribute_GlobalRead");
        ATTRIBUTE_FILEVERSION = PropertyUtil.getSchemaProperty(context, "attribute_FileVersion");
        ATTRIBUTE_VERSIONDATE = PropertyUtil.getSchemaProperty(context, "attribute_VersionDate");
        ATTRIBUTE_CHECKINREASON = PropertyUtil.getSchemaProperty(context, "attribute_CheckinReason");
        ATTRIBUTE_DCTYPENAME = PropertyUtil.getSchemaProperty(context, "attribute_DCTypeName");
        ATTRIBUTE_DCNAME = PropertyUtil.getSchemaProperty(context, "attribute_DCName");
        ATTRIBUTE_DCGROUP = PropertyUtil.getSchemaProperty(context, "attribute_DCGroup");
        ATTRIBUTE_DCROLE = PropertyUtil.getSchemaProperty(context, "attribute_DCRole");
        ATTRIBUTE_DCSEQUENCE = PropertyUtil.getSchemaProperty(context, "attribute_DCSequence");
        ATTRIBUTE_DCSIZE = PropertyUtil.getSchemaProperty(context, "attribute_DCSize");
        ATTRIBUTE_DCREQUIRED = PropertyUtil.getSchemaProperty(context, "attribute_DCRequired");
        ATTRIBUTE_DCPREFIX = PropertyUtil.getSchemaProperty(context, "attribute_DCPrefix");
        ATTRIBUTE_DCSEARCHABLE = PropertyUtil.getSchemaProperty(context, "attribute_DCSearchable");
        ATTRIBUTE_DCEDITABLE = PropertyUtil.getSchemaProperty(context, "attribute_DCEditable");
        ATTRIBUTE_DCSUFFIX = PropertyUtil.getSchemaProperty(context, "attribute_DCSuffix");
        ROLE_EMPLOYEE = PropertyUtil.getSchemaProperty(context, "role_Employee");
    ...
    /**
     * Converts all type_PersonalFolder and type_PublicFolder objects to be of type_ProjectVault. The existing objects are cloned and the types changed to be the new type. The relations are preserved
     * and changed to be relationships applicable for the type_ProjectVault. The conversion happens as one transaction, all are converted or none on an exception. The relationships used by
     * type_ProjectVault are different than the previous folder relationships. The cardinality is different for type_ProjectVault connected to a type_ProjectVault. The relationship is one to many in
     * release 2. The conversion method will connect children as the relationship will allow. Folders that are connected to multiple parents, the first one will connected and the others will be caught
     * as an exception and written to the log file. These children relationships can't be migrated. If type_PersonalFolder or type_PublicFolder are sub-typed, then the method needs modified to include
     * the sub-types properly.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            input parameters, none expected.
     * @return <code>int</code> return 0 if no exceptions.
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.6.0
     */
    public int convertFoldersToWorkspaceVault(Context context, String[] args) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
        ...
        MapList queryResultList = DomainObject.findObjects(context, searchTypes, "*", null, busSelects);

        MxDebug.message(MxDebug.DL_3, "Search Types = " + searchTypes + "\n" + "\tqueryResultList.size() = " + queryResultList.size());
        ...
        try {
            ...
            while (queryResultItr.hasNext()) {
                ...
                MxDebug.message(MxDebug.DL_5, "objectType  info ...\n" + "\tobjectId = " + objectId + "\n" + "\tobjectName = " + objectName + "\n" + "\tobjectVault = " + objectVault + "\n"
                ...
                MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2a ...");

                // clone the existing folder.
                BusinessObject cloneBo = doExistingObject.clone(context, objectName, autoRev, objectVault);

                MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2b ...");

                // change the new clone to type_ProjectVault(Workspace Vault).
                BusinessObject workspaceVaultBo = cloneBo.change(context, TYPE_PROJECTVAULT, objectName, autoRev, objectVault, POLICY_PROJECTVAULT);

                MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2c ...");

                String newObjectId = workspaceVaultBo.getObjectId();

                // *** create bean instance and set other attribute info
                WorkspaceVault workspaceVault = (WorkspaceVault) DomainObject.newInstance(context, newObjectId);

                workspaceVault.open(context);
                workspaceVault.setOwner(objectOwner);
                workspaceVault.setDescription(objectDesc);

                MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2d ...");

                if (objectType.equals(TYPE_PERSONALFOLDER)) {
                    workspaceVault.setAttributeValue(context, ATTRIBUTE_GLOBALREAD, "False");
                } else {
                    workspaceVault.setAttributeValue(context, ATTRIBUTE_GLOBALREAD, "True");

                    MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2e ...");
                    ...
                    AccessUtil.grantAccess(context, boList, accessList, AccessUtil.WORKSPACE_ACCESS_GRANTOR);

                    MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2f ...");
                }

                // *** preserve the any subscription to the folder
                preserveSubscriptions(context, doExistingObject, workspaceVault);

                MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2g ...");
                ...
                MxDebug.message(MxDebug.DL_7, "convertFoldersToWorkspaceVault 2h ...");
    ...
    /**
     * Each user in release 1 has a set automatically created that is named 'My Workspace'. The set is the root node for the tree in the user interface. The set can contain objects created within the
     * application. This method converts the set to a type_ProjectVault. There is a transaction around each set migration. The method makes a copy of each set, gets the children objects, creates a
     * type_ProjectVault that is identical to the set.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            input parameters, none expected.
     ...
     * @since AEF 9.5.6.0
     */
    public int convertMyWorkspaceToWorkspaceVault(Context context, String[] args) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
        ...
        MxDebug.message(MxDebug.DL_3, "queryResultList.size() = " + personList.size());

        MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 1");

        try {
            if (personList.size() > 0) {
                MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 2");

                preModifySchema(context);

                schemaAltered = true;
                MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 3");
            } else {
                return 0;
            }
            ...
            MapList queryResultList = DomainObject.findObjects(context, TYPE_PROJECTVAULT, "*", whereExpression, busSelects);

            MxDebug.message(MxDebug.DL_3, "queryResultList.size() = " + queryResultList.size());
            ...
            while (personItr.next()) {
                ...
                MxDebug.message(MxDebug.DL_5, "userName = " + userName);
                ...
                if (migrationAlreadyRun) {
                    MxDebug.message(MxDebug.DL_5, "migrationAlreadyRun  " + migrationAlreadyRun + "");
                    continue;
                }
                ...
                try {
                    ...
                    MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 4");
                    ...
                    MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 5");
                    ...
                    if (mqlError.equals("")) {
                        ...
                        BusinessObjectWithSelectList boList = tempSet.select(context, objectSelects);

                        tempSet.remove(context);
                        tempSet.close(context);

                        MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 6");

                        // *** create new object
                        DomainObject domObject = new DomainObject();
                        BusinessObject newBo = new BusinessObject(TYPE_PROJECTVAULT, "My Workspace", domObject.getUniqueName(), VAULT_ESERVICEPRODUCTION);

                        newBo.create(context, POLICY_PROJECTVAULT);

                        String newObjectId = newBo.getObjectId();

                        MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 7");
                        ...
                        MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 8");

                        // propagate the child relations to the new object.
                        adjustWorkspaceRelationship(context, workspaceVault, boList);

                        workspaceVault.update(context);
                        workspaceVault.close(context);

                        MxDebug.message(MxDebug.DL_7, "convertMyWorkspaceToWorkspaceVault  .. 9");
    ...
    /**
     * Creates a type_VersionDocument for each file of a type_DocumentSheet that doesn't have one. Inspect all type_DocumentSheet, create type_VersionDocument as needed. For each file that a
     * type_VersionDocument that can't be created display message about the file. Transaction around the creation of the each type_VersionDocument. Data migration for V10. This is an addition beyond
     * the original purpose of the method. Method invocation was added to avoid changing the tcl file. The method updateVersionDocuments sets the title attribute to contain the value of the file name
     * that is checked into the object.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            input parameters, none expected.
     ...
     * @since AEF 9.5.6.0
     */
    public int createVersionDocuments(Context context, String[] args) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
        ...
        MapList queryResultList = DomainObject.findObjects(context, TYPE_DOCUMENTSHEET, "*", null, busSelects);
        ...
        while (queryResultItr.hasNext()) {
            ...
            MxDebug.message(MxDebug.DL_7, "createVersionDocuments 1 ...");

            try {
                startTransaction(context, true);

                MxDebug.message(MxDebug.DL_7, "createVersionDocuments 2 ...");
                ...
                MxDebug.message(MxDebug.DL_7, "createVersionDocuments 3 ...");
                ...
                MapList versionDocumentsList = docSheet.getRelatedObjects(context, RELATIONSHIP_VERSION, "*", childSelectList, new StringList(), false, true, (short) 1, "", "");
                ...
                while (docSheetFileItr.next()) {
                    ...
                    MxDebug.message(MxDebug.DL_7, "createVersionDocuments 4 ...");

                    File checkinFile = (File) docSheetFileItr.obj();
                    String fileName = checkinFile.getName();

                    MxDebug.message(MxDebug.DL_7, "createVersionDocuments 5 ...");
                    ...
                    while (versionDocumentsItr.hasNext()) {
                        ...
                        MxDebug.message(MxDebug.DL_7, "fileName / versionDocTitle = " + fileName + "/" + versionDocTitle);
                    ...
                    MxDebug.message(MxDebug.DL_7, "versionDocumentExists = " + versionDocumentExists);
                    ...
                    if (versionDocumentExists == false) {
                        ...
                        if (results == 1) // there was an error
                        {
                            ...
                            MxDebug.message(MxDebug.DL_5, "create VD results =" + results);
        ...
        MxDebug.message(MxDebug.DL_3, "dsObjectErrors =" + dsObjectErrors);
    ...
    /**
     * Creates a type_VersionDocument for a the parent object(type_DocumentSheet) connects the two objects.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param docSheet
     *            the type_DocumentSheet, should be open
     * @return <code>int</code> return 0 if no exceptions.
     * @throws Exception
     *             if the operation fails
     * @since AEF 10.0.0.0
     */
    protected int createVersionDocumentObject(Context context, DomainObject docSheet, String fileName) throws Exception {
        /*
         * Author : Mike Terry Date : 06/24/03 Notes : History :
         */
        ...
        try {
            ...
            MxDebug.message(MxDebug.DL_7, "createVersionDocumentObject 1 ...");

            // DomainObject versionDocumentDo = new DomainObject();

            // versionDocumentDo.
            createAndConnect(context, TYPE_VERSIONDOCUMENT, vdName, "1", POLICY_VERSIONDOCUMENT, objectVault, RELATIONSHIP_VERSION, docSheet, true);

            MxDebug.message(MxDebug.DL_7, "createVersionDocumentObject 2 ...");
            ...
            MxDebug.message(MxDebug.DL_7, "createVersionDocumentObject 3 ...");
            ...
            MxDebug.message(MxDebug.DL_7, "createVersionDocumentObject 4 ...");
            closeObject(context, closeBO);

            docSheet.closeObject(context, docSheetClose);

            MxDebug.message(MxDebug.DL_7, "createVersionDocumentObject 5 ...");
        } catch (Exception e) {
            MxDebug.message(MxDebug.DL_1, "Error = " + "'" + e + "'");
    ...
    /**
     * The V10 common file functionality requires that the name of the file checked into the object be stored in the Title attribute. This method queries for all Version Documents in the system and
     * sets the attribute.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @return <code>int</code> return 0 if no exceptions.
     ...
     * @since AEF 10.0.0.0
     */
    public int updateVersionDocuments(Context context) throws Exception {
        /*
         * Author : Mike Terry Date : 06/23/03 Notes : History :
         */
        ...
        MapList queryResultList = DomainObject.findObjects(context, TYPE_VERSIONDOCUMENT, "*", null, busSelects);
        ...
        while (queryResultItr.hasNext()) {
            ...
            MxDebug.message(MxDebug.DL_7, "updateVersionDocuments 1 ...");

            try {
                startTransaction(context, true);

                MxDebug.message(MxDebug.DL_7, "updateVersionDocuments 2 ...");
                ...
                MxDebug.message(MxDebug.DL_7, "versionDocFileList.size () = " + versionDocFileList.size());

                // the type_VersionDocument should only have one file.
                if (versionDocFileList.size() == 1) {

                    MxDebug.message(MxDebug.DL_7, "updateVersionDocuments 4 ...");

                    File checkinFile = (File) versionDocFileItr.obj();
                    MxDebug.message(MxDebug.DL_7, "updateVersionDocuments 4a ..." + checkinFile);
                    String fileName = checkinFile.getName();

                    MxDebug.message(MxDebug.DL_7, "fileName ..= '" + fileName + "'");
                ...
                MxDebug.message(MxDebug.DL_7, "updateVersionDocuments 6 ...");
     ...
     * @since AEF 9.5.6.0
     */
    public void deleteOldSystemObjects(Context context, String[] args) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
        ...
        String searchTypes = "vailPDF Rendering Queue" + "," + "vailSystemSettings";
        ...
        try {
            ...
            DomainObject.deleteObjects(context, getStringArrayOfIds(context, queryResultList));
     ...
     * @since AEF 9.5.6.0
     */
    protected void preModifySchema(Context context) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
     ...
     * @since AEF 9.5.6.0
     */
    protected void postModifySchema(Context context) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
    ...
    /**
     * Create new relationship object(s) connecting the children to the new object. If the relationship can't be created a message is written to the log file. If type_PersonalFolder or
     * type_PublicFolder are sub-typed, then the method needs modified to include the sub-types properly. WorkspaceVault object must be open.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param workspaceVault
     *            the business object that needs adjusted
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.6.0
     */
    protected void adjustWorkspaceRelationship(Context context, WorkspaceVault workspaceVault, BusinessObjectWithSelectList busSelList) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
        ...
        RelationshipType relTypeFolder = new RelationshipType(RELATIONSHIP_SUBVAULTS);

        RelationshipType relTypeOther = new RelationshipType(RELATIONSHIP_VAULTEDDOCUMENTREV2);

        BusinessObjectWithSelectItr itr = new BusinessObjectWithSelectItr(busSelList);
        ...
        while (itr.next()) {
            ...
            try {
                if (toTypeNameBo.equals(TYPE_PERSONALFOLDER) || toTypeNameBo.equals(TYPE_PUBLICFOLDER) || toTypeNameBo.equals(TYPE_PROJECTVAULT)) {
                ...
                MxDebug.message(MxDebug.DL_5, "Warning/Error.... = " + objectId + " '" + e + "' " + "add to object " + toTypeNameBo + " " + bo.getName());
            }
    ...
    /**
     * Preserves subscription relationships. Connect the existing folder to the type_PublishSubscribe object. WorkspaceVault object must be open.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param doExistingObject
     *            existing business object that may have subscriptions.
     * @param workspaceVault
     *            the business object that needs adjusted
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.6.0
     */
    protected void preserveSubscriptions(Context context, DomainObject doExistingObject, WorkspaceVault workspaceVault) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
        ...
        RelationshipType relPublishSubscribe = new RelationshipType(RELATIONSHIP_PUBLISHSUBSCRIBE);
        ...
        MxDebug.message(MxDebug.DL_3, "relList.size() = " + relList.size());
        ...
        while (itr.next()) {
            ...
            MxDebug.message(MxDebug.DL_5, "rel.getTypeName () = " + rel.getTypeName());

            if (rel.getTypeName().equals(RELATIONSHIP_PUBLISHSUBSCRIBE)) {
                rel.open(context);
                rel.remove(context);
                rel.update(context);
                rel.close(context);
                ...
                try {
                    MxDebug.message(MxDebug.DL_5, "Creating rel between " + "\tworkspaceVault.getName () = " + workspaceVault.getName() + "\n" + "\tbo.getName () = " + bo.getName() + "\n"
                            + "\trel name = " + relPublishSubscribe);

                    workspaceVault.connect(context, relPublishSubscribe, true, bo);
                } catch (Exception e) {
                    MxDebug.message(MxDebug.DL_5, "ERROR:  maintaining subscriptions.... = " + objectId + " - " + e);
                }
     ...
     *            the eMatrix <code>Context</code> object
     * @param mapList
     *            a MapList business object. id must be a key in object map.
     * @return <code>String []</code>
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.6.0
     */
    protected String[] getStringArrayOfIds(Context context, MapList mapList) throws Exception {
        /*
         * Author : Mike Terry Date : 02/12/03 Notes : History :
         */
    ...
    /**
     * Creates a type_VersionDocument for a file. Copies the type_DocumentSheet object, changes the type, removes the non-pertinent files, connects the object to the type_DocumentSheet.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectId
     *            the type_DocumentSheet id
     * @param fileName
     *            the file name to versioned
     * @param fileObjects
     *            a Map of FileObj objects
     * @return <code>int</code> return 0 if no exceptions.
     * @throws Exception
     *             if the operation fails
     * @since AEF 9.5.6.0
     ...
     */
    /*
     * protected int createVersionDocumentObject ( Context context, String objectId, String fileName, Map fileObjects) throws Exception {
     * 
     * Author : Mike Terry Date : 02/26/03 Notes : History :
     * 
     * 
     * 
     * MxDebug.enter ();
     ...
     * try {
     * 
     * MxDebug.message (MxDebug.DL_5, "objectId = " + objectId + "\n" + "fileName = " + fileName + "\n" + "fileObjects = " + fileObjects );
     * 
     * DomainObject docSheet = new DomainObject (objectId); docSheet.open(context);
     * 
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 1 ...");
     * 
     * String vdName = docSheet.getUniqueName("VD_"); String objectVault = docSheet.getVault();
     * 
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 2 ...");
     * 
     * // clone the Document Sheet BusinessObject cloneBo = docSheet.clone(context, vdName, "1", objectVault );
     * 
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 3 ...");
     * 
     * MxDebug.message (MxDebug.DL_5, "New object details: \n" + "TYPE_VERSIONDOCUMENT = " + TYPE_VERSIONDOCUMENT + "\n" + "vdName = " + vdName + "\n" + "Revision = 1 \n" + "objectVault = " +
     * objectVault + "\n" + "POLICY_VERSIONDOCUMENT = " + POLICY_VERSIONDOCUMENT + "\n" );
     * 
     * // change the new clone to type_VersionDocument BusinessObject versionDocumentBo = cloneBo.change( context, TYPE_VERSIONDOCUMENT, vdName, "1", objectVault, POLICY_VERSIONDOCUMENT );
     * 
     ...
     * 
     * String vdObjectId = versionDocumentDo.getObjectId(); String owner = docSheet.getOwner().getName();
     * 
     * String originated = versionDocumentDo.getInfo ( context, DomainConstants.SELECT_ORIGINATED);
     * 
     * //*** set metadata on type_VersionDocument versionDocumentDo.setOwner (owner);
     * 
     * Map attributes = new HashMap(3); attributes.put( ATTRIBUTE_FILEVERSION, "1" ); attributes.put( ATTRIBUTE_VERSIONDATE, originated ); attributes.put( ATTRIBUTE_CHECKINREASON,
     * "Generated during installation" ); attributes.put( ATTRIBUTE_TITLE, fileName );
     * 
     * 
     ...
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 5 ...");
     * 
     * // connect to type_DocumentSheet to type_VersionDocument DomainRelationship.connect(context, docSheet, RELATIONSHIP_VERSION, versionDocumentDo);
     * 
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 6 ...");
     * 
     * // remove other file(s) from the Version Document Iterator keyItr = fileObjects.keySet().iterator(); while (keyItr.hasNext()) {
     * 
     * String keyName = (String) keyItr.next();
     * 
     * // pull the FileObj object out of the Map, get name FileObj fileObj = (FileObj) fileObjects.get(keyName); String thisFileName = fileObj.getFilename(); String thisFileFormat =
     * fileObj.getFormat();
     * 
     * // if not the desired attached file, delete from object. if (!thisFileName.equals(fileName)) { // Create the mql command instance, delete the file, // and close the mql command instance String
     * [] methodArgs = {vdObjectId, thisFileName}; String mqlString = "delete businessobject $1 file \'$2\'"; MqlUtil.mqlCommand(context, mqlString, methodArgs);
     * 
     * } }
     * 
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 7 ...");
     * 
     * versionDocumentDo.update(context); versionDocumentDo.close(context);
     * 
     * docSheet.close(context);
     * 
     * MxDebug.message (MxDebug.DL_7, "createVersionDocumentObject 8 ..."); } catch (Exception e) { MxDebug.message (MxDebug.DL_1, "Error = " + "'" + e + "'");
     * 
     * return 1; }
     * 
     * 
    ...
    public int updateVersionDocumentsWithTitle(Context context, String[] args) throws Exception {
        try {
            ...
            while (itr.hasNext()) {
                ...
                MxDebug.message(MxDebug.DL_3, "id = " + id + "\n" + "title = " + title);
