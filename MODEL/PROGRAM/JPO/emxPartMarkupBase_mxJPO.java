...
public class emxPartMarkupBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public static final String STATE_ORGANIZATION_ACTIVE = PropertyUtil.getSchemaProperty("policy", DomainConstants.POLICY_ORGANIZATION, "state_Active");
    ...
    // 377755 start
    public static final String STATE_PART_RELEASE = PropertyUtil.getSchemaProperty("policy", DomainConstants.POLICY_EC_PART, "state_Release");
    ...
    public static int checkProposedMarkupsinPeerReview(Context context, String[] args) throws Exception {
        ...
        MapList mapListMarkups = doPart.getRelatedObjects(context, strRelPattern, // relationship pattern
    ...
    public static int CheckProposedMarkupsECParts(Context context, String[] args) throws Exception {
        ...
        MapList mapListMarkups = chgobj.getRelatedObjects(context, strRelPattern, // relationship pattern
                strTypePattern, // object pattern
                strlObjectSelects, // object selects
                strlRelSelects, // relationship selects
                false, // to direction
    ...
    public static String mergeMarkup(Context context, String[] args) throws Exception {
        ...
        if (bolIsConflict) {
            ...
            if (blnSave && strlMarkupIds.size() > 1) {
                ...
                if (strChangeId != null && (DomainConstants.TYPE_ECO.equalsIgnoreCase(strChangeType) || DomainConstants.TYPE_ECR.equalsIgnoreCase(strChangeType)
                        || TYPE_DCR.equalsIgnoreCase(strChangeType) || (ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)))) {

                    DomainObject doChange = new DomainObject(strChangeId);

                    String strRelToConnectToChange = null;

                    if (doChange.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION) || doChange.isKindOf(context, DomainConstants.TYPE_ECO) || doChange.isKindOf(context, TYPE_MECO)) // Modified for
                                                                                                                                                                                         // ENG
                                                                                                                                                                                         // Convergence
                    {
                    ...
                    if (strChangeCurrentState.equals(ECO_STATE_REVIEW) || strChangeCurrentState.equals(ECR_STATE_REVIEW) || strChangeCurrentState.equals(ECR_STATE_PLAN_ECO)
                            || strChangeCurrentState.equals(STATE_PLAN_DCO) || strChangeCurrentState.equals(CA_STATE_IN_APPROVAL)) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a StringList containing the following entries: -0 holds the Change Process Id paramMap - a StringList containing all selected Markup Ids
     * @return void
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral X3
    ...
    public static int mergeMarkupsonChange(Context context, String args[]) throws Exception {
        ...
        String strAIRelPattern = RELATIONSHIP_AFFECTEDITEM + "," + RELATIONSHIP_CHANGE_AFFECTED_ITEM;
        ...
        MapList mapListParts = doChange.getRelatedObjects(context, strAIRelPattern, // relationship pattern
        ...
        while (itrParts.hasNext()) {
            ...
            if (doChange.isKindOf(context, DomainConstants.TYPE_ECO) || doChange.isKindOf(context, TYPE_MECO) || doChange.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)) {
                strRelPattern = RELATIONSHIP_APPLIED_MARKUP;
                strWhereClause = "policy ==" + "\"" + POLICY_PARTMARKUP + "\"" + " " + "&&" + " " + "current==" + approvedState + " " + "&&" + " " + "to[" + RELATIONSHIP_EBOM_MARKUP + "].from.id == "
                        + strPartId;
            } else if (doChange.isKindOf(context, DomainConstants.TYPE_ECR) || doChange.isKindOf(context, TYPE_DCR)) {
                strRelPattern = RELATIONSHIP_PROPOSED_MARKUP;
                strWhereClause = "policy ==" + "\"" + POLICY_PARTMARKUP + "\"" + " " + "&&" + " " + "current==" + approvedState + " " + "&&" + " " + "to[" + RELATIONSHIP_EBOM_MARKUP + "].from.id == "
            ...
            MapList mapListMarkups = doChange.getRelatedObjects(context, strRelPattern, // relationship pattern
                    strTypePattern, // object pattern
                    strlObjectSelects, // object selects
                    strlRelSelects, // relationship selects
                    false, // to direction
    ...
    public MapList getAllMarkupSummary(Context context, String[] args) throws Exception {
        ...
        MarkupIds = domobj.getRelatedObjects(context, RELATIONSHIP_EBOMMARKUP, strTypes, selectStmts, selectRelStmts, false, true, (short) 1, null, null);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a StringList containing the following entries: paramMap - a StringList containing all selected Markup Ids
     * @return int
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral X3
        ...
        try {
            ...
            if (mapListParts.size() > 0) {
                while (itrParts.hasNext()) {
                    ...
                    strRelPattern = RELATIONSHIP_APPLIED_MARKUP;
                    strWhereClause = "(policy == " + "\"" + POLICY_PARTMARKUP + "\"" + " " + "||" + " " + "policy == " + "\"" + POLICY_EBOMMARKUP + "\"" + ") " + "&&" + " " + "current=="
                            + approvedState + " " + "&&" + " " + "to[" + RELATIONSHIP_EBOM_MARKUP + "].from.id == " + strPartId;
                    ...
                    if (mapListMarkups.size() > 0) {
                        while (itrMarkupIds.hasNext()) {
                            ...
                            if (changeId != null) {
                                ...
                                if (changeobj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)) {
                                    ...
                                    strNewPartId = (String) JPO.invoke(context, "emxChange", null, "getIndirectAffectedItems", inputArgs, String.class);
                                }
                            }
                        ...
                        // End of If Loop Markup Iterator Size >0
                ...
                // End of If Loop Parts connected >0
    ...
    /**
     * Method to check the validations on Find Number, Ref Des for uniqueness Added for the Bug#362472
     * @param context
     * @param requestMap
     * @return int
     * @throws Exception
     */
    public static int checkForUniqueness(Context context, Element eleRoot, List fnVals, List refDesVals) throws Exception {
        ...
        while (itrRelationships.hasNext()) {
            ...
            if ("A".equals(strEBOMChgType) || "C".equals(strEBOMChgType) || "D".equals(strEBOMChgType)) {
                ...
                while (itrAttributes.hasNext()) {
                    eleAttribute = (Element) itrAttributes.next();
                    com.matrixone.jdom.Attribute changeAttr = eleAttribute.getAttribute("chgtype");
                    String change = "";
                    if (changeAttr != null) {
        ...
        while (itrRelationships.hasNext()) {
            ...
            if ("A".equals(strEBOMChgType)) {
                ...
                if (!doChild.isLastRevision(context)) {
                    ...
                    MapList mapListLatestReleasedParts = DomainObject.findObjects(context, (String) doChild.getInfo(context, DomainObject.SELECT_TYPE),
                            (String) doChild.getInfo(context, DomainObject.SELECT_NAME), "*", null, (String) doChild.getInfo(context, DomainObject.SELECT_VAULT), strWhereClause, false, strlSelects);
                    String strId = "";
                ...
                if (strChangeId != null) {
                    ...
                    if (strCurrent.equals(DomainObject.STATE_PART_OBSOLETE)) {
                        if (strLastRevCurrent.equals(DomainObject.STATE_PART_PRELIMINARY) || strLastRevCurrent.equals(DomainObject.STATE_PART_REVIEW)
                                || strLastRevCurrent.equals(DomainObject.STATE_PART_APPROVED)) {
                        ...
                        if (!(strParentState.equals(DomainObject.STATE_PART_APPROVED))
                                || (!(strCurrent.equals(DomainObject.STATE_PART_APPROVED)) && !(strCurrent.equals(DomainObject.STATE_PART_RELEASE)))) {
                            String[] mailArguments = new String[12];
                            mailArguments[0] = "emxEngineeringCentral.Alert.ObjectState";
                            mailArguments[1] = "4";
                            mailArguments[2] = "strObjType";
                ...
                if (strCurrent.equals(DomainObject.STATE_PART_OBSOLETE) && strLastRevCurrent.equals(DomainObject.STATE_PART_RELEASE)) {
                    ...
                    if (blnIsEBOMSub) {
                        ...
                        // End - Added for EBOM Substitute Apply
                        DomainRelationship doRelNew = DomainRelationship.connect(context, doParentPart, strEBOMRelType, doLastRev);
                        doRelNew.setAttributeValues(context, hmpAttributes);
                        // Start - Added for EBOM Substitute Apply
                    }
                    // End - Added for EBOM Substitute Apply
                    ...
                    if (blnIsEBOMSub) {
                        ...
                        if ("Yes".equals(strSubstitute)) {
                            ...
                            for (int l = 0; l < substList.size(); l++) {
                                ...
                                part.createSubstitutePart(context, strPartId, strEBOMRel, strSubsId);
                            }
                        }
                ...
                if (blnIsEBOMSub) {
                    ...
                    String strObjWhereClause = "name==\"" + strPartName + "\" " + "&&" + " " + "revision==\"" + strPartRev + "\"";
                    String strRelWhereClause = null;

                    if (!"".equalsIgnoreCase(strEBOMFN)) {
                        if (!"".equalsIgnoreCase(strEBOMRD)) {

                            strRelWhereClause = "attribute[" + DomainConstants.ATTRIBUTE_FIND_NUMBER + "] == \"" + strEBOMFN + "\"  && attribute[" + DomainConstants.ATTRIBUTE_REFERENCE_DESIGNATOR
                                    + "] == \"" + strEBOMRD + "\"";
                        } else {
                    ...
                    MapList mapListBOMs = doParentPart.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, // relationship pattern
                    ...
                    if (mapListBOMs.size() > 0) {
                        ...
                        String[] messageValues = new String[7];
                        messageValues[0] = strMarkupName;
                        messageValues[1] = messageValues[4] = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),
                                "emxEngineeringCentral.Common.Part");
                        messageValues[2] = strPartName;
                        messageValues[3] = strPartRev;
                        messageValues[5] = doParentPart.getName();
                        messageValues[6] = doParentPart.getRevision();

                        String strErrorMsg = MessageUtil.getMessage(context, null, "emxEngineeringCentral.MarkupActions.ApplyErrorMsg", messageValues, null, context.getLocale(),
        ...
        if ("create".equals(sAction)) {
            if (relId != null && !"".equals(relId)) {
                ...
                if (!(DomainConstants.TYPE_ECR.equals(strChangeType) || DomainConstants.TYPE_ECO.equals(strChangeType))) {
                    ...
                    boolValueReturn = (!("Create".equals(strChangeCurrent) || "Define Components".equals(strChangeCurrent) || "Design Work".equals(strChangeCurrent)));
                    ...
                    if (strFromId != null && !strFromId.equals("")) {
                        ...
                        boolValueReturn = (!("Create".equals(strChangeCurrent) || "Submit".equals(strChangeCurrent) || "Evaluate".equals(strChangeCurrent)));
                    }
                }
    ...
    public Boolean showItemMarkupCreateCommand(Context context, String[] args) throws Exception {
        ...
        if (sStatus.equals(DomainConstants.STATE_PART_PRELIMINARY) || sStatus.equals(DomainConstants.STATE_PART_OBSOLETE)) {
            ...
            objSelects.add("from[" + DomainConstants.RELATIONSHIP_EBOM_MARKUP + "|" + DomainConstants.SELECT_TO_TYPE + "==\"" + sType + "\"]");
     ...
     *            ematrix context.
     * @param args
     *            selected markup objectids.
     * @throws Exception
     *             if any operation fails. This method first validates all the selected markup objects, if all the markup objects is valid to reject only then markup objects will be rejected.
     */
    public void rejectMarkup(Context context, String[] args) throws Exception {
        ...
        /*
         * below for loop is for doing validations: 1. context user should be an owner of the markup object OR context user should be an ECO Assignee. 2. For development part markups which are only in
         * Proposed state can be rejected. 3. For part markup, EBOM markup Proposed OR Approved state markups can be rejected. 4. No markups can be rejected which is Applied.
         */
        ...
        for (int i = 0; i < dataSize; i++) {
            ...
            if (strMarkupOwner.equals(strContextUser) || "User Agent".equals(strMarkupOwner) || getListValue(dataMap, SELECT_ASSIGNEES).contains(strContextUser)
                    || getListValue(dataMap, SELECT_ECR_ASSIGNEES_FROM_MARKUP_OBJ).contains(strContextUser) || getListValue(dataMap, SELECT_TECHNICAL_ASSIGNEES).contains(strContextUser)) { // Modified
     ...
     * @since EngineeringCentral X3
     */
    public void approveMarkup(Context context, String[] args) throws Exception {
        try {
    ...
    public MapList PersonSearch(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            companylist = domobj.getRelatedObjects(context, RELATIONSHIP_CHANGE_RESPONSIBILITY + "," + RELATIONSHIP_DESIGN_RESPONSIBILITY, // relationship pattern
            ...
            for (int i = 0; i < size; i++) {
                ...
                personlist = domObj.getRelatedObjects(context, strrel, // relationship pattern
    ...
    public void deleteMarkup(Context context, String[] args) throws Exception {
        ...
        while (objDtlItr.hasNext()) {
            ...
            if (canDelete) {
                ...
                if (strContextUser.equals((String) map.get(SELECT_OWNER)) || "User Agent".equals((String) map.get(SELECT_OWNER)) || getListValue(map, SELECT_ASSIGNEES).contains(strContextUser)
                        || getListValue(map, SELECT_ECR_ASSIGNEES_FROM_MARKUP_OBJ).contains(strContextUser) || getListValue(map, SELECT_TECHNICAL_ASSIGNEES).contains(strContextUser)) { // Modified for
                    deleteIds[count++] = markupId;
                } else {

                    // Multitenant
                    emxContextUtil_mxJPO.mqlNotice(context,
                            EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.MarkupActions.DeleteAccess"));
                    return;
                }
            } else {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a HashMap containing the following entries: paramMap - a HashMap containing the following keys, markup object ids
     * @return void
     * @throws Exception
     *             if operation fails
     * @since EngineeringCentral X3
     */
    public void deleteAffectedItemMarkups(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        for (int i = 0; i < strObjectIds.length; i++) {
            ...
            if (selectedIdType.equals(typeBOMMarkup) || selectedIdType.equals(typeItemMarkup) || selectedIdType.equals(typePlantItemMarkup) || selectedIdType.equals(typePlantBOMMarkup)) {
     ...
     * @since EngineeringCentral X3
     */
    public int applyMarkup(Context context, String[] args) throws Exception {
            ...
            while (stritr.hasNext()) {
                ...
                if (currentpolicyName.equals(POLICY_PARTMARKUP) || currentpolicyName.equals(POLICY_EBOMMARKUP)) {
                    if (!DomainConstants.POLICY_DEVELOPMENT_PART.equals(strPartPolicy) && (currentState.equals(approvedState))) {
                        ...
                        if ((strPartCurrentRev.equals(strPartCurrentFirstRev) && "FALSE".equals(strPartVer) && strPartState.equals(DomainConstants.STATE_PART_PRELIMINARY))
                                || strPartState.equals(DomainConstants.STATE_PART_APPROVED) || TYPE_PLANT_BOM_MARKUP.equals(strMarkupType))
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing "requestMap" 1 - HashMap containing one String entry for key "objectId"
     * @return a eMatrix <code>MapList</code> object having the list of all Objects which are required checkbox to be displayed in the Table List Page.
     * @throws Exception
     *             if the operation fails
     * @since EngineeringCentral X3
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing "commandType"
     * @return a eMatrix <code>MapList</code> object having the list of all Objects which are required checkbox to be displayed in the Table List Page.
     * @throws Exception
     *             if the operation fails
     * @since EngineeringCentral X3
        ...
        try {
            ...
            if (changeOID != null && changeOID.length() != 0) {
                ...
                if (doChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ORDER) || doChgObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST)) {
                    Map changeActionMap = checkPartAlreadyConnectedToThisChange(context, strPartId, changeOID);
            ...
            // For EC Parts
                ...
                if (sCommandType == null || sCommandType.equalsIgnoreCase("Save")) {
                    ...
                    if (changeOID != null) {
                        strNewPartId = (String) JPO.invoke(context, "emxChange", null, "getDirectAffectedItems", inputArgs, String.class);
                    }
                    ...
                    if (ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)) {
                        isChangeconnected = isConnectedtoCA(context, strPartId, changeOID, strChangeRelNew);
                    } else {
                    ...
                    if (isChangeconnected.equalsIgnoreCase("true")) {
                        ...
                        if (ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)) {
                            DomainRelationship.connect(context, new DomainObject(changeOID), RELATIONSHIP_APPLIED_MARKUP, new DomainObject(sMarkupId));
                            // DomainRelationship.connect(context,new DomainObject(changeOID),strChangeRelNew,new DomainObject(strPartId));
    ...
    public String getMarkupXML(Context context, String[] args) throws Exception {
        ...
        str += "callback = eval(top.opener.emxEditableTable.prototype.getMarkUpXML);";
    ...
    public void generateBOMMarkupXML(Context context, String[] args) throws Exception {
        ...
        if (massUpdateAction != null) {
            try {
                ...
                MapList ebomList = domobj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, selStmts, selRelStmts, false, true, (short) 1, null, null);
                ...
                if (lr != null) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (markupAction.equalsIgnoreCase("cut")) {
                            ...
                            // hmpChanged.put(strObjectId+"|"+strRelId , "changed");
                        }
                    ...
                    if ("add".equals(markup)) {
                        ...
                        if (sRelType != null) {
                            ...
                            if (sRelType.equalsIgnoreCase(relAlternate)) {
                                ...
                                dmRelPartPM = DomainRelationship.connect(context, partObj, sRelType, selObj);
                            ...
                            // If selected part is Substitute
                                ...
                                String strConnectSubPart = (String) relObj.connect(context, sRelType, sRelId, sObjectId, false, true);
                            ...
                            // Added for Primary Parts
                                ...
                                ContextUtil.pushContext(context);
                                RelToRelUtil relObj = new RelToRelUtil();
                                String sRelid = relObj.connect(context, sRelType, sParentOID, sObjectId, true, true);
                                ...
                                if (strParam2 != null && strParam2.length() > 0) {
                                    // 372458
                                    String connectAsDerived = FrameworkProperties.getProperty(context, "emxEngineeringCentral.ReplaceBOM.Derived");
                                    if (connectAsDerived.equalsIgnoreCase("true")) {
                                        ...
                                        DomainRelationship doRelDerived = DomainRelationship.connect(context, new DomainObject(strParam2), RELATIONSHIP_DERIVED, new DomainObject(sObjectId));
                            ...
                            // Split Quantity
                                ...
                                RelToRelUtil relObj = new RelToRelUtil();
                                String sRelid = relObj.connect(context, sRelType, sRelId, sObjectId, false, true);
                                ...
                                if ("Yes".equalsIgnoreCase(sSwitch)) {
                                    ...
                                    if (mlList.size() > 0) {
                                        ...
                                        if (sMRrelId != null && !"".equals(sMRrelId)) {
                                            ...
                                            relObj.connect(context, sSplitQuantityMFR, sMRrelId, sRelid, false, false);
                                        }
                                    }
                                }
    ...
    /*
     * This method used to store some data that can be used in Creation/editing of Plant Item Markup
     * 
     * @param context, args
     * 
     * @return String value
     ...
     * @grade 0
     * 
     * @author Srikanth Note: as of now this method is no where is used. if webform comes in to the picture in future for Change Process then this method is requred.
     */
    public String generateXMLforMassUpdateAction(Context context, String[] args) throws Exception {
        ...
        if (sReleasedPlants != null && !sReleasedPlants.equals("")) {
            ...
            // Multitenant
            String sNoteMsg = EnoviaResourceBundle.getProperty(context, "emxMBOMStringResource", context.getLocale(), "emxMBOM.Markup.MarkupNoteMessage");
    ...
    public DomainRelationship connectChangeToPart(Context context, DomainObject domPart, DomainObject domChange, String sConnectRelName) throws Exception {
        ...
        try {
            ...
            dmRel = DomainRelationship.connect(context, domChange, sConnectRelName, domPart);
        } catch (Exception e) {
            throw e;
        }
        return dmRel;
    ...
    public MapList getConnectedManufacturingPlants(Context context, String sPartId, String sPlantIds) throws Exception {
        ...
        try {
            ...
            // Database hit
            mlPlants = domPart.getRelatedObjects(context, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY, TYPE_PLANT, slBusSelects, slRelSelects, true, false, (short) 1, busWhere.toString(),
                    relWhere.toString());
        } catch (Exception e) {
            throw e;
        }
    ...
    /*
     * This method used to create a new markup object and it will be connected to both Change and Part
     * 
     * @param context, domPart, domChange
     * 
     * @return String
    ...
    public String createAndConnectMarkup(Context context, DomainObject domPart, DomainObject domChange) throws Exception {
        ...
        DomainObject domMarkup = null;
        DomainRelationship dmRelPartMarkup = null;
        DomainRelationship dmRelChangeMarkup = null;
        ...
        try {
            ...
            dmRelPartMarkup = DomainRelationship.newInstance(context);
            // 088962V6R2012 PUT THE CONNECTION BETWEEN PUSH CONTXT AND POP CONTEXT
            ContextUtil.pushContext(context);
            dmRelPartMarkup = DomainRelationship.connect(context, domPart, DomainConstants.RELATIONSHIP_EBOM_MARKUP, domMarkup);

            // connect the markup with the change
            dmRelChangeMarkup = DomainRelationship.newInstance(context);
            dmRelChangeMarkup = DomainRelationship.connect(context, domChange, sConnectRelName, domMarkup);
            ContextUtil.popContext(context);
        } catch (Exception e) {
            throw e;
        }
    ...
    public MapList getAllManuSites(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            if (UIUtil.isNotNullAndNotEmpty(sPartId)) {
                ...
                mLocationList = doPart.getRelatedObjects(context, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY, TYPE_PLANT, busSelList, relSelList, true, false, (short) 1, "", sbWhere.toString(), null,
                        null, null);
    ...
    public Map getHeighestMRSequence(Context context, DomainObject dmPartMaster, DomainObject dmLoc) throws Exception {
        ...
        try {
            ...
            MapList mlMRObjs = dmPartMaster.getRelatedObjects(context, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY, TYPE_PLANT, busSelList, relSelList, true, false, (short) 1, strBusWhere.toString(),
                    null);
    ...
    public DomainRelationship checkForPendingMRRel(Context context, String sPartId, String sObjectId) throws Exception {
        ...
        try {
            ...
            MapList plantsList = domPartMaster.getRelatedObjects(context, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY, TYPE_PLANT, slObjectSelects, slRelSelects, true, false, (short) 1,
                    sbBusWhere.toString(), sbRelWhere.toString());
    ...
    public boolean showPIMSaveMarkupCommand(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if ("Production".equalsIgnoreCase(policyClassification)) {
                ...
                String sECPartStates = (String) FrameworkProperties.getProperty(context, "emxEngineeringCentral.ECPart.AllowSaveMarkup");
                ...
                if (!slStatesList.contains(sStatus)) {
                    bReturn = false;
                } else if (sFirst.equals(sRevision) && !(sStatus.equals(DomainConstants.STATE_PART_RELEASE) || sStatus.equals(DomainConstants.STATE_PART_PRELIMINARY))) { // Checking for number of
                                                                                                                                                                          // revisions.
                    bReturn = false;
                }
            } else {
    ...
    public String generatedItemMarkupView(Context context, String args[]) throws Exception {
        ...
        String attrName1 = StringUtils.replaceAll(attrName, " ", "_");
        // IR-017226 - Starts
        ...
        if ("create".equals(sAction)) {
            ...
            if ("view".equals(sAction)) {
                String arrFile[] = StringUtils.split(fileContent, "#");
                for (int index = 0; index < arrFile.length; index += 3) {
                    if (arrFile[index].equals(attrName)) {
                        ...
                        if (arrFile[index + 1].indexOf("_") != -1) {
                            attrList = FrameworkUtil.split(arrFile[index + 1], "_");
                        ...
                        if (arrFile[index + 2].indexOf("_") != -1) {
                            attrList = FrameworkUtil.split(arrFile[index + 2], "_");
                        ...
                        if (arrFile[index + 1].indexOf("_") != -1)
                            arrFile[index + 1] = StringUtils.replaceAll(arrFile[index + 1], "_", "");
                        if (arrFile[index + 2].indexOf("_") != -1)
                            arrFile[index + 2] = StringUtils.replaceAll(arrFile[index + 2], "_", "");
                        arrFile[index + 2] = StringUtils.replaceAll(arrFile[index + 2], " ", "_");
                        arrFile[index + 1] = StringUtils.replaceAll(arrFile[index + 1], " ", "_");

                        if (arrFile[index + 1].equals(arrFile[index + 2])) {
                            // IR-017226 - Starts
                            if (strWeight.equals(attrName) || strTargetCost.equals(attrName) || strEstimatedCost.equals(attrName) || strEffectivityDate.equals(attrName)) {
                                // Modified for IR-097616V6R2012 starts
                                if (!"".equals(attrwithdimensiondisp)) {
                                    sReturn = attrwithdimensiondisp;
                                } else {
                                    sReturn = arrFile[index + 1];
                                    if ("timestamp".equals(attrType)) {
                                        sReturn = StringUtils.replaceAll(arrFile[index + 1], "_", " ");
                                ...
                                // Multitenant
                                arrFile[index + 2] = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(),
                                        "emxFramework.Range." + attrName1 + "." + arrFile[index + 2]);

                                // Multitenant
                                sReturn = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), arrFile[index + 2]);
                                // IR-017226 - Starts
                            }
                            // IR-017226 - Ends
                        } else {
                            ...
                            if (!strWeight.equals(attrName) && !strTargetCost.equals(attrName) && !strEstimatedCost.equals(attrName) && !strEffectivityDate.equals(attrName)) {
                                ...
                                arrFile[index + 2] = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(),
                                        "emxFramework.Range." + attrName1 + "." + arrFile[index + 2]);
                                // Multitenant
                                arrFile[index + 1] = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(),
                                        "emxFramework.Range." + attrName1 + "." + arrFile[index + 1]);
                                // IR-017226 - Starts
                            }
                            // IR-017226 - Ends
                            ...
                            // Modified for 071743
                            if ("timestamp".equals(attrType)) {
                                String strShowOldValue = StringUtils.replaceAll(arrFile[index + 1], "_", " ");
                                String strShowNewValue = StringUtils.replaceAll(arrFile[index + 2], "_", " ");
                                ...
                                if (!"".equals(attrwithdimensiondisp)) {
                                    ...
                                    sReturn = "<label style=\"color:red\" ><s>" + XSSUtil.encodeForHTML(context, arrFile[index + 1]) + "</s></label>&nbsp;";
                                    sReturn += "<label style=\"color:green\" >" + XSSUtil.encodeForHTML(context, arrFile[index + 2]) + "</label>";
        ...
        if ("edit".equals(sAction)) {
            String arrFile1[] = StringUtils.split(fileContent, "#");
            for (int index = 0; index < arrFile1.length; index += 3)
                if (arrFile1[index].equals(attrName)) {
                    sShowValue = arrFile1[index + 2];
                    sShowValue2 = arrFile1[index + 1];
                }
        }
            ...
            if (UOMUtil.isAssociatedWithDimension(context, attrName)) {
                ...
                if ("edit".equals(sAction)) {
                    if (sShowValue.equals(sShowValue2))
                        ...
                        sOldValues.append(
                                "<table><tr><td width=150><label style=\"color:red\" id=\"label_" + attrName + "\"> <s>" + XSSUtil.encodeForHTML(context, sAttValueWithDimensionDisp) + "</s></label>");
                ...
                // X6 Bug #: 351866
                String Name1 = StringUtils.replace(attrName, " ", "_");
                String Rang1 = StringUtils.replace(sAttValue, " ", "_");
        ...
        if (attrChoices == null || attrChoices.size() <= 1) {
            ...
            // Added for IR-077316V6R2013
            displayUOM = StringUtils.replaceAll(displayUOM, "<select", "<select onchange=\"showChange(this,'string');\"");

            if (UOMUtil.isAssociatedWithDimension(context, attrName) && "edit".equals(sAction)) {
                String sShowValueArr[] = StringUtils.split(sShowValue, "_");
                String val = sShowValueArr[1];
                ...
                strText.append("\" value=\"" + XSSUtil.encodeForHTML(context, sShowValueArr[0]) + "\" size=\"20\" extra=\"yes\"    onkeyup = \" showChange(this,'" + attrType + "'); \" >");
        ...
        if ("timestamp".equals(attrType)) {
            ...
            strText.append(XSSUtil.encodeForHTML(context, strTxtName));
            strText.append("\" id=\"" + XSSUtil.encodeForHTML(context, strTxtName) + "\" value=\"" + XSSUtil.encodeForHTML(context, sShowValue)
                    + "\" size=\"20\" extra=\"yes\" onfocus = \'var v = document.getElementById(\"" + strTxtName
                    + "\"); showChange(v,\"string\");\'  >&nbsp;&nbsp; <a href='javascript:showCalendar(\"editDataForm\",\"");
            strText.append(XSSUtil.encodeForHTML(context, strTxtName));
            strText.append("\",\"\",true,validateEffectivityDateForItemMarkup); '  onfocus = \'var v = document.getElementById(\"" + strTxtName
                    + "\"); showChange(v,\"string\");\'><img src=\"../common/images/iconSmallCalendar.gif\" border=0></a>");
        ...
        if (attLeadTime.equalsIgnoreCase(attrName)) {
            ...
            attrChoices.addElement("1 Week");
            attrChoices.addElement("2 Weeks");
            attrChoices.addElement("3 Weeks");
            attrChoices.addElement("4 Weeks");
            attrChoices.addElement("6 Weeks");
            attrChoices.addElement("8 Weeks");
            attrChoices.addElement("12 Weeks");
            attrChoices.addElement("20 Weeks");
        }
        ...
        if (attrChoices != null && attrChoices.size() > 1) {
            ...
            for (int j = 0; j < attrChoices.size(); j++) {
                ...
                // X6 Bug #: 351866
                String Name1 = StringUtils.replace(attrName, " ", "_");
                String Rang1 = StringUtils.replace(attrrange, " ", "_");
                String attrName2 = "emxFramework.Range." + Name1 + "." + Rang1;
        ...
        // Modified for IR-077316V6R2013 start
        sScript.append(
                "var sStatus;  if(attrType == 'string') { if(sName.match('Estimated')!=null){  if((parseFloat(sHidElement.value)==parseFloat(txtEstimatedCost))&&(sHidElement.value.match(ele.value)!=null) )sStatus='true'; else sStatus = 'false'; } "); // IR-077316V6R2012
                                                                                                                                                                                                                                                           // fix
        sScript.append(
                " else if(sName.match('Weight')!=null){if(parseFloat(sHidElement.value)==parseFloat(txtWeight+' '+ele.value)&&(sHidElement.value.match(' '+ele.value)!=null)) sStatus='true'; else sStatus = 'false';}");
        sScript.append(
                "else{ if(sName.match('Date')==null) {var num = ele.selectedIndex;if(sHidElement.value==ele.options[num].text) sStatus='true'; else sStatus = 'false'; }if(sName.match('Date')!=null) {if( sHidElement.value==ele.value) sStatus='true'; else sStatus = 'false';} }} "); // IR-077316V6R2012
                                                                                                                                                                                                                                                                                         // fix

        sScript.append(
                "  if(attrType == 'real') { if(sName.match('Estimated')!=null){  if((parseFloat(sHidElement.value)==parseFloat(value))&&((sHidElement.value).match(unitsEstimatedCost)!=null))sStatus='true'; else sStatus = 'false'; }  ");
        sScript.append(
                "  else if(sName.match('Weight')!=null){  if((parseFloat(sHidElement.value)==parseFloat(value))&&((sHidElement.value).match(' '+unitsWeight)!=null))sStatus='true'; else sStatus = 'false'; }  ");
    ...
    public void createItemMarkup(Context context, String args[]) throws Exception {
        ...
        String attrValuesArr[] = StringUtils.split(attrValues.toString(), "#");
        ...
        if ("create".equals(sAction)) // if action is create
        {
            ...
            if ((ChangeConstants.TYPE_CHANGE_REQUEST.equalsIgnoreCase(strChgObjType) || ChangeConstants.TYPE_CHANGE_ORDER.equalsIgnoreCase(strChgObjType))) {
                /*
                 * ChangeOrder changeOrder = new ChangeOrder(sChangeOID); StringList objIdList = new StringList(); objIdList.add(objectId); HashMap mCAList = (HashMap)
                 * changeOrder.connectAffectedItems(context,objIdList); HashMap mCAIdList = (HashMap)mCAList.get("objIDCAMap"); String strCAId = (String)mCAIdList.get(objectId); sChangeOID = strCAId;
                 */
                Map changeActionMap = checkPartAlreadyConnectedToThisChange(context, objectId, sChangeOID);
                String changeConnected = (String) changeActionMap.get("ALREADY_CONNECTED");
                if ("TRUE".equalsIgnoreCase(changeConnected)) { //
            ...
            if (ChangeConstants.TYPE_CHANGE_ACTION.equalsIgnoreCase(strChangeType)) {
                relType = new RelationshipType(RELATIONSHIP_CHANGE_AFFECTED_ITEM);
                isChangeConnected = isConnectedtoCA(context, strPartId, sChangeOID, RELATIONSHIP_CHANGE_AFFECTED_ITEM);
        ...
        for (int index = 0; index < attrValuesArr.length; index += 2) {
            ...
            attNameElement.setText(attrValuesArr[index]);
            if (UOMUtil.isAssociatedWithDimension(context, attrValuesArr[index]))
                attOldElement.setText(doPart.getAttributeValue(context, attrValuesArr[index]) + "_" + UOMUtil.getDBunit(context, attrValuesArr[index]));
            else
                attOldElement.setText(doPart.getAttributeValue(context, attrValuesArr[index]));

            // IR-055011V6R2011x -Starts
            if (attrValuesArr[index + 1].equals(" "))
                attNewElement.setText("");
            else
                attNewElement.setText(attrValuesArr[index + 1]);
            // IR-055011V6R2011x -Ends
    ...
    public void commonViewPlantBOMMarkupInitialProcess(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            while (itrMapList.hasNext()) {
                ...
                if (sAction == null || (commandType.equalsIgnoreCase("SaveAs") && !rowId.equalsIgnoreCase("0") && sAction != null && sAction.equalsIgnoreCase("open"))) {
    ...
    public void createPlantBOMMarkup(Context context, String args[]) throws Exception {
                ...
                /* For Production Parts */
                    ...
                    if ("true".equals(hasPlantChanges)) {
                        ...
                        if (sCommandType == null || sCommandType.equalsIgnoreCase("Save")) {
                            ...
                            String sRevision = "-";
                            domMarkup.createObject(context, TYPE_PLANT_BOM_MARKUP, sName, sRevision, POLICY_PARTMARKUP, vaultName);
                            sMarkupId = domMarkup.getInfo(context, SELECT_ID);
                        }
                    ...
                    if ("true".equals(hasBOMChanges)) {
                        ...
                        if (sCommandType == null || sCommandType.equalsIgnoreCase("Save")) {
                            ...
                            String vaultName = Person.getPerson(context).getVaultName(context);
                            String sRevision = "-";
                            domMarkup.createObject(context, TYPE_BOMMARKUP, sName, sRevision, POLICY_PARTMARKUP, vaultName);
                            sBOMMarkupId = domMarkup.getInfo(context, SELECT_ID);
                        }
                    ...
                    if (isChangeConnected.equalsIgnoreCase("true") || isPlantChangeConnected.equalsIgnoreCase("true")) {
                        ...
                        if ("true".equals(hasBOMChanges)) {
                            ...
                            DomainRelationship.connect(context, new DomainObject(changeOID), relName_2, new DomainObject(sBOMMarkupId));
                        ...
                        if ("true".equals(hasPlantChanges)) {
                            if (changePlantID == null) {
                                ...
                                DomainRelationship.connect(context, new DomainObject(changeOID), relName_2, new DomainObject(sMarkupId));
                            } else {
                                relName_2 = RELATIONSHIP_PROPOSED_MARKUP;
                                /* Connect Change-Markup */
                                if (plantChangetype.equals(TYPE_MECO)) {
                                    ...
                                    if ("false".equals(isChangeConnected))
                                        DomainRelationship.connect(context, new DomainObject(changePlantID), relName_1, new DomainObject(selectedPartId));
                                }

                                ContextUtil.pushContext(context);
                                DomainRelationship.connect(context, new DomainObject(changePlantID), relName_2, new DomainObject(sMarkupId));
                                ContextUtil.popContext(context);
                        ...
                        if ("true".equals(hasBOMChanges)) {
                            ...
                            if ("false".equals(isChangeConnected)) {
                                DomainRelationship.connect(context, new DomainObject(changeOID), relName_1, new DomainObject(selectedPartId));
                            }
                            /* Connect Change-Markup */
                            if (changeOID != null && (changetype.equals(DomainConstants.TYPE_ECO) || (changetype.equals(ChangeConstants.TYPE_CHANGE_ACTION)))) {
                                relName_2 = RELATIONSHIP_APPLIED_MARKUP;
                        ...
                        if ("true".equals(hasPlantChanges)) {
                            ...
                            if (changePlantID == null) {
                                ...
                                if ("false".equals(isChangeConnected)) {
                                    // R209 MECO changes - Starts
                                    doAffectedItemRelObj = DomainRelationship.connect(context, new DomainObject(changeOID), relName_1, new DomainObject(selectedPartId));
                                    // R209 MECO changes - Ends
                                ...
                                if ("false".equals(isPlantChangeConnected)) {
                                    // R209 MECO changes - Starts
                                    doAffectedItemRelObj = DomainRelationship.connect(context, new DomainObject(changePlantID), RELATIONSHIP_AFFECTED_ITEM, new DomainObject(selectedPartId));
                                    // R209 MECO changes - Ends
                                }
                                /* Connect Change-Markup */
                                if (plantChangetype.equals(TYPE_MECO)) {
                            ...
                            if (!"true".equals(hasBOMChanges)) {
                                ...
                                // Multitenant
                                String strRequestedChangeValue = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", new Locale("en"),
    ...
    public void generatePlantBOMMarkupXML(Context context, String[] args) throws Exception {
        ...
        String strPlantBOMColumns = (String) FrameworkProperties.getProperty(context, "emxMBOM.View.PlantBOMChangesAllowed");
                ...
                while (objItr.hasNext()) {
                    ...
                    while (childItr.hasNext()) {
                        ...
                        if (sMarkup != null && sMarkup.equalsIgnoreCase("add")) {
                            ...
                            java.util.List columnElementList = childElement.getChildren("column");
                        ...
                        } else if (sMarkup != null && (sMarkup.equalsIgnoreCase("changed") || sMarkup.equalsIgnoreCase("cut"))) {
    ...
    private String getAsStoredAttributeValue(Context context, String sRelId, String sPlantId, String sAttrName, String sRelName) throws Exception {
        String strAttrValue = "";
        try {
            String relIds[] = new String[1];
            relIds[0] = sRelId;
            ...
            while (checkItr.hasNext()) {
                ...
                } else if ("A".equals(sChangeType)) // if the operation is ADD
                {
                    if ((DomainConstants.RELATIONSHIP_ALTERNATE).equals(sRelName.trim())) {
                        ...
                        String sAlternatePartType = com.matrixone.apps.domain.util.XSSUtil
                                .decodeFromURL(relationshipList.getChild("fromRel").getChild("businessObjectRef").getChild("objectType").getText());
                        if (null != sAlternatePartType && !sAlternatePartType.equals("null") && !sAlternatePartType.equals("")) {
                            strPartType = sAlternatePartType;
                        }
                        String sAlternatePartName = com.matrixone.apps.domain.util.XSSUtil
                                .decodeFromURL(relationshipList.getChild("fromRel").getChild("businessObjectRef").getChild("objectName").getText());
                        String sAlternatePartRev = com.matrixone.apps.domain.util.XSSUtil
                                .decodeFromURL(relationshipList.getChild("fromRel").getChild("businessObjectRef").getChild("objectRevision").getText());
                        ...
                        String altType = sAltObjIdObj.getType(context);
                        ...
                        // 377755 end
                        try {
                            String sRelid = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3", sSelectedId,
                                    "from[" + EngineeringConstants.RELATIONSHIP_ALTERNATE + "|to.id==" + sAlternateObjId + "].id", "|");
                            if ("".equals(sRelid)) {
                                RelToRelUtil relObj = new RelToRelUtil();
                                sRelid = relObj.connect(context, sRelName.trim(), sSelectedId, sAlternateObjId, true, true);
                            }
                    ...
                    // R209 MECO changes - Ends
                    {
                        ...
                        String sSubPartRev = com.matrixone.apps.domain.util.XSSUtil
                                .decodeFromURL(relationshipList.getChild("fromRel").getChild("businessObjectRef").getChild("objectRevision").getText());
                        String strPersonVault = com.matrixone.apps.domain.util.XSSUtil.decodeFromURL(relationshipList.getChild("fromRel").getChild("businessObjectRef").getChild("vaultRef").getText());
     ...
     *            - the current context
     * @param sRelId
     *            - the id of Alternate/EBOM Substitute relationship
     * @param sPlantObjId
     *            - the plant currently selected in the plant filter
     * @param sRelName
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a StingList of object Ids
     * @return merge status - true/false
     * @throws Exception
     *             if the operation fails
     * @since EngineeringCentral X3
     **/
    public String mergePlantBOMMarkups(Context context, String[] args) throws Exception {
        ...
        while (itrMarkups.hasNext()) {
            ...
            while (itrRelationships.hasNext()) {
                ...
                strKey = strRelationshipDefRef.trim() + "~" + strPartType + "~" + strPartName + "~" + strPartRev + "~" + strConnectionId + "~" + strEBOMRelId + "~" + strSelectedPartId + "~"
                        + strPlantId;
                if (strEBOMChgType == null) {
     ...
     * @param context
     *            Context The current user's context
     * @param sPlantId
     *            String Plant id value returns Location Id throws Exception
     */
        ...
        try {
            ...
            while (itrXML.hasNext()) {
                ...
                if ("changed".equals(sMarkup)) {
                    MapList mrList = domPart.getRelatedObjects(context, EngineeringConstants.RELATIONSHIP_MANUFACTURING_RESPONSIBILITY, TYPE_PLANT, null, null, true, false, (short) 1,
                            DomainConstants.SELECT_ID + " == '" + sPlantId + "'", SELECT_STATUS + "==Pending", 0);
                ...
                if ("changed".equals(sMarkup)) {
                    if (leadPlantFound) {
                        ...
                        MapList findObjects = DomainObject.findObjects(context, "*", sDocIn, "*", "*", sVault, null, false, objectSelects);
                    ...
                    if (domRelMR == null) {
                        domRelMR = connectObjectToObject(context, domPart, domPlant, sRelName);
                    }
                    ...
                    if (sFlag.equalsIgnoreCase("false")) {
                        connectObjectToObject(context, domPart, domChange, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY_CHANGE);
                    }
                }
            }
            String strAppliedPartMarkupRel = PropertyUtil.getSchemaProperty(context, "relationship_AppliedPartMarkup");
    ...
    public String getECOIdByName(Context context, String ecoName, String strVault) throws Exception {
        String strECOId = null;
        String strSystem = FrameworkProperties.getProperty(context, "emxMBOM.MBOM.SystemECOName");
        StringList busSelList = new StringList();
    ...
    public Map getPreviousLeadPlantRel(Context context, DomainObject dmPartMaster) throws Exception {
        ...
        try {
            ...
            strRelWhere.append("')");
            MapList mlMRObjs = dmPartMaster.getRelatedObjects(context, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY, TYPE_PLANT, busSelList, relSelList, true, false, (short) 1, "",
                    strRelWhere.toString());
            int iSeq = 0;
            int iHighestSeq = 0;
    ...
    public void generateBOMMarkupXML(Context context, String massUpdateAction, String strMarkupId, String sSelPartId, Hashtable htConflictBOMs) throws Exception {
        ...
        MapList ebomList = dosSelPart.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM, DomainConstants.TYPE_PART, selStmts, selRelStmts, false, true, (short) 1, null, null);
    ...
    public void createMarkupfromAffectedItem(Context context, String[] args) throws Exception {
        ...
        if (!"null".equals(strlMarkupIds)) {
            ...
            if (strlMarkupIds.size() == 1) {
                ...
                else { // When you selct Save As Option in Markup
                       // Bug: 371209 AM Start
                       // generateBOMMarkupXML(context,massUpdateAction,strMarkupIds,sPartObjId,htConflictBOMs);
                       // Bug: 371209 AM End
    ...
    public void savePlantBOMMarkup(Context context, String[] args) throws Exception {
            ...
            if ("Save".equalsIgnoreCase(sCommandType)) {
                ...
                if ("true".equals(hasBOMChanges)) {
                    if (null != ECChange && !"null".equals(ECChange) && !"".equals(ECChange)) {
                        ...
                        /* Connect Change-Part */
                        if ("false".equals(isChangeConnected)) {
                            DomainRelationship.connect(context, chgBOMObj, relName, new DomainObject(sPartObjId));
                        }
                ...
                if ("true".equals(hasPlantChanges)) {
                    ...
                    if (isChangeConnected.equalsIgnoreCase("true") || isPlantChangeConnected.equalsIgnoreCase("true")) {
                        ...
                        isChangeConnected = isConnectedtoChangeProcess(context, sPartObjId, sChangeObjId, null);
                        if ("false".equals(isChangeConnected))
                            DomainRelationship.connect(context, new DomainObject(sChangeObjId), relName_1, new DomainObject(sPartObjId));

                        DomainRelationship.connect(context, new DomainObject(sChangeObjId), relName_2, new DomainObject(snewMarkupId));
    ...
    public void generatePlantBOMMarkupXML(Context context, String[] strArrParams, Hashtable htConflictBOMs) throws Exception {
    ...
    /*
     * This method used to send the strcture browser xml to the Structure Browser to show the Plant Item Markup changes
     * 
     * @param context, args
     * 
     * @return void
    ...
    public static Boolean isNotDevelopmentPart(Context context, String[] args) throws Exception {
     ...
     * @grade 0
     */
    static Map getPlantBasedBOMs(Context context, StringList strlPlantBOMsList) throws Exception {
     ...
     * @grade 0
     */
    public Hashtable getAttributeLevelMerge(Context context, Hashtable htConflictBOMs) throws Exception {
     ...
     * @since EngineeringCentral X3
     **/
    public static Boolean isNameDisplayAllowed(Context context, String args[]) throws Exception {
        Boolean status = Boolean.TRUE;
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        String sType = (String) paramMap.get("commandType");
        if (sType == null || sType.equalsIgnoreCase("Save"))
    ...
    public static Boolean hasBOMChanges(Context context, String args[]) throws Exception {
    ...
    public static Boolean hasPlantChanges(Context context, String args[]) throws Exception {
        boolean status = false;
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        ...
        String sSelectedPart = (String) paramMap.get("sSelPart");
        if (!"DevPart".equalsIgnoreCase(sSelectedPart) && hasPlantChanges.equalsIgnoreCase("true") && ((isPartReleased.equalsIgnoreCase("true") && isFirstRevision.equalsIgnoreCase("true"))
    ...
    public String getECChange(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        ...
        if (showChooser) {
            ...
            builder.append("<input type=\"button\" name=\"btnECChange\" id=\"btnECChange\" value=\"...\" onclick=\"javascript:showChooser('../common/emxFullSearch.jsp?field=TYPES=" + txtType
                    + "&selection=single&excludeOIDprogram=emxPartMarkup:excludeChanges&isPartReleased=" + isPartReleased
                    + "&table=MBOMChangeSearchResults&hideHeader=true&submitURL=../manufacturingchange/emxMBOMSelectChange.jsp&suiteKey=Framework&StringResourceFileId=emxFrameworkStringResource&SuiteDirectory=common&formName=editDataForm&frameName=pageContent&fieldNameActual=ECChange&fieldNameDisplay=ECChangeDisplay&HelpMarker=emxhelpfullsearch','850','630')\">");
    ...
    public String getPlantChange(Context context, String[] args) throws Exception {
        ...
        if (showChooser) {
            ...
            builder.append("<input type=\"button\" name=\"btnPlantChange\" id=\"btnPlantChange\" value=\"...\" onclick=\"javascript:showChooser('../common/emxFullSearch.jsp?field=TYPES=" + txtType
                    + "&selection=single&excludeOIDprogram=emxPartMarkup:excludeChanges&table=MBOMChangeSearchResults&hideHeader=true&submitURL=../manufacturingchange/emxMBOMSelectChange.jsp&suiteKey=Framework&StringResourceFileId=emxFrameworkStringResource&SuiteDirectory=common&formName=editDataForm&frameName=pageContent&fieldNameActual=PlantChange&fieldNameDisplay=PlantChangeDisplay&HelpMarker=emxhelpfullsearch','850','630')\">");
    ...
    public StringList excludeChanges(Context context, String[] args) throws Exception {
        ...
        MapList excludedChangeMapList = DomainObject.findObjects(context, txtType, QUERY_WILDCARD, whereExpression.toString(), objectSelects);
        ...
        if (isPartReleased != null && isPartReleased.equals("false")) {
            ...
            excludedChangeMapList = DomainObject.findObjects(context, txtType, QUERY_WILDCARD, where, objectSelects);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            Context, Relationship type, rel id, Element tag with column names, type of change
     * @return integer value indicating whether or not BOM changes are present
     * @throws Exception
     *             if the operation fails
     * @since EngineeringCentral X3
     **/
    private int checkForBOMChanges(Context context, String sRelType, String strRelId, com.matrixone.jdom.Element element, String changeType) throws Exception {
        int retVal = 0;
        ...
        String strBOMColumns = (String) FrameworkProperties.getProperty(context, "emxEngineeringCentral.View.BOMChangesAllowed");
    ...
    public StringList excludePersonSearch(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            // Get all the Persons
            MapList persons = DomainObject.findObjects(context, TYPE_PERSON, "*", "*", "*", "*", null, null, true, selects, (short) 0);
     ...
     * @param sPartId
     *            String value contains Part Obect Id
     * @param strArrLocIds
     *            String Array value contains Location object ids
     * @param strArrERPStatus
     *            String Array value contains ERP Status values
     * @param strArrLeadPlants
     *            String Array value contains Lead Plant value
     * @param strArrECOs
     *            String Array value contains ECO Name
     * @param strArrECOIds
     *            String Array value contains ECO Object Id
     * @return String
     * @throws FrameworkException
     */

    public void updateManuResp(Context context, String sPartId, String[] strArrLocIds, String[] strArrERPStatus, String[] strArrLeadPlants, String strArrECOs, String strArrECOIds) throws Exception {
    ...
    public void updateManuResp(Context context, String strArrLocIds, String strArrLeadPlants) throws Exception {
        if (EngineeringUtil.isMBOMInstalled(context)) {
            Class clazz = Class.forName("com.matrixone.apps.mbom.PartMaster");
            IPartMaster partMaster = (IPartMaster) clazz.newInstance();
     ...
     *            Context : User's Context.
     * @param sPartId
     *            String value contains ECO Obect Id
     * @param sPartMasterId
     *            String value contains Part Master object id
     * @return boolean
     * @throws Exception
     */
    public boolean isECOConnectedPartMaster(Context context, String sECOId, String sPartMasterId) throws Exception {
        ...
        try {
            ...
            StringList busSelList = new StringList();
            busSelList.add(DomainObject.SELECT_ID);
            mlECOList = doPartMaster.getRelatedObjects(context, RELATIONSHIP_MANUFACTURING_RESPONSIBILITY_CHANGE, DomainConstants.TYPE_ECO, busSelList, null, true, false, (short) 1, "", "", null,
                    null, null);
    ...
    /**
     * Method to check merged BOM Markups Added for Bug 355092
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a Sting of object Ids
     * @return merge status - true/false
     * @throws Exception
     *             if the operation fails
     * @since EngineeringCentral X3
     **/
    public static String checkBOMConflict(Context context, String[] args) throws Exception {
        Hashtable htConflictBOMs = new Hashtable();
        Hashtable htChangedBOMs = new Hashtable();
        Hashtable htNewChangedBOMs = new Hashtable();
        Hashtable htAddedBOMs = new Hashtable();
    ...
    /**
     * Method to check merged Plant BOM Markups Added for Bug 355092
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds a Sting of object Ids
     ...
     * @since EngineeringCentral X3
     **/
    public static String checkPlantBOMConflict(Context context, String[] args) throws Exception {
        Hashtable htChangedBOMs = new Hashtable();
        Hashtable htAddedBOMs = new Hashtable();
        Hashtable htDeletedBOMs = new Hashtable();
        Hashtable htUnChangedBOMs = new Hashtable();
            ...
            if (ecoId != null && !"".equals(ecoId)) {
                ...
                if (slBOMMarkup != null & !slBOMMarkup.isEmpty()) {
                    ...
                    while (itrBOMMarkup.hasNext()) {
                        ...
                        if (strMarkupType.equals(TYPE_BOMMARKUP) && (!"User Agent".equals(markupOriginator) || "User Agent".equals(markupOwner))) { // propagate merged markups to version objects
     ...
     * @since EngineeringCentral X7
     **/
    public void copyMarkupToVersionObject(Context context, DomainObject doMarkup, String partId) throws Exception {
        try {
            String strMarkupOwner = "";
     ...
     * @param args
     * @throws Exception
     *             - if the specified name exists in DB
     */
     ...
     * @since R212
     */
    public String getMarkupXMLForSave(Context context, String[] args) throws Exception {
    ...
    /**
     * Checks the Part current state and accordingly displays ECRs or ECOs for the Change field typechooser for Markup Save and Save As webforms in Markup to select the change.
     * @param context
     *            ematrix context
     * @param args
     *            contains Program map
     ...
     * @since R212
     */
    public String getChangeDynamicSearchQueryForMarkup(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        ...
        if (parentOID != null && !"".equals(parentOID)) {
            ...
            DomainObject doPart = null;
            String strPolicy = "";
            String strCOStateFilter = "policy_FormalChange.state_Propose,policy_FormalChange.state_Prepare";
            ...
            strPolicy = (String) doPart.getInfo(context, DomainConstants.SELECT_POLICY);
            String policyClassification = EngineeringUtil.getPolicyClassification(context, strPolicy);
            ...
            if (blnShowChange) {
                if (blnShowECR && blnShowECO) {
                    strSearchURL = strSearchURL + "type_ChangeOrder,type_ChangeRequest:CURRENT=" + strCOStateFilter + "," + strCRStateFilter;

                } else if (blnShowECO) {
                    strSearchURL = strSearchURL + "type_ChangeOrder:CURRENT=" + strCOStateFilter;

                } else if (blnShowECR) {
                    strSearchURL = strSearchURL + "type_ChangeRequest:CURRENT=" + strCRStateFilter;
    ...
    public int isPromotedFromMarkupActions(Context context, String[] args) throws Exception {
        try {
            ...
            if ("TRUE".equals(fromMarkupActions)) {
                ...
                emxContextUtil_mxJPO.mqlNotice(context, EngineeringUtil.i18nStringNow(context, "emxEngineeringCentral.Alert.UseMarkupActions", context.getSession().getLanguage()));
            }
            return 1;
        } catch (Exception e) {
            throw e;
    ...
    public MapList getAplliedMarkupXMLContent(Context context, String[] args) throws Exception {
        ...
        ContextUtil.pushContext(context); // Added for IR-151138
            ...
            while (itrRelationships.hasNext()) {
                ...
                if (strEBOMChgType != null && "A".equals(strEBOMChgType)) {
                    while (itrAttributes.hasNext()) {
                        ...
                        sbfHTML.append("<B>").append(i18nNow.getAttributeI18NString(strAttrName, languageStr)).append(": </B>").append(strAttrValue);
                ...
                if (strEBOMChgType != null && "C".equals(strEBOMChgType)) {
                    while (itrAttributes.hasNext()) {
                        ...
                        if (attrChgType != null) {
                            ...
                            sbfHTML.append("<B>").append(i18nNow.getAttributeI18NString(strAttrName, languageStr));
    ...
    public Vector getImageType(Context context, String args[]) throws Exception {
        ...
        for (int i = 0; i < paramList.size(); i++) {
            ...
            if ("A".equals(strEBOMChgType)) {
                columnVals.addElement("<img src=\"../common/images/iconStatusAdded.gif\">");

            }
            if ("D".equals(strEBOMChgType)) {

                columnVals.addElement("<img src=\"../common/images/iconStatusRemoved.gif\">");
            }

            if ("C".equals(strEBOMChgType)) {

                columnVals.addElement("<img src=\"../common/images/iconStatusChanged.gif\">");
            }
        }
    ...
    public Vector getChangeDescription(Context context, String args[]) throws Exception {
        Vector columnVals = new Vector();
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
        MapList paramList = (MapList) paramMap.get("objectList");
    ...
    public HashMap commitMRRelationship(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            if (DomainConstants.STATE_PART_RELEASE.equals(strPartState) || EngineeringConstants.STATE_COMPLETE.equals(strPartState) || DomainConstants.STATE_PART_OBSOLETE.equals(strPartState)) {
                ...
                if ("add".equals(markup)) {
                    ...
                    if (EngineeringUtil.isMBOMInstalled(context)) {
                        if (UIUtil.isNullOrEmpty(sChangeId)) {
                            // 076028V6R2012 start
                            throw new Exception(EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", new Locale(strLanguage), "emxEngineeringCentral.Change1"));
                            // 076028V6R2012 end
                        } else {
                        ...
                        if (EngineeringConstants.STR_YES.equalsIgnoreCase(stPlanningReq) && EngineeringConstants.ERP_STATUS_NOTACTIVE.equalsIgnoreCase(stERPStatus)) {
                    ...
                    if (leadPlantFound) {
                        ...
                        MapList findObjects = DomainObject.findObjects(context, "*", stDocIn, "*", "*", sVault, null, false, objectSelects);
     ...
     * @since R216
     */
    public boolean isMarkupCreateAllowed(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        if (getWhereUsed != null && !("".equals(getWhereUsed)) && "true".equalsIgnoreCase(getWhereUsed)) {
            return !(policyClassification != null && !("".equals(policyClassification)) && (("Unresolved".equals(policyClassification)) || ("Development".equals(policyClassification))));
        } else
            return true;
    ...
    /**
     * Gets all the CR Create state Used with the excludeOIDprogram setting of Autonomy Search in case Save SaveAS functionality
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
        ...
        try {
            ...
            if (strPartId != null && strPartId.length() > 0) {
                ...
                MapList mapListMarkups = doPart.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EBOM_MARKUP, PropertyUtil.getSchemaProperty(context, "type_ItemMarkup"), strlObjectSelects,
                        null, true, true, (short) 0, strWhereClause, null);
    ...
    public String getImplementedItem(Context context, String args[]) throws Exception {
        ...
        DomainObject doECO = new DomainObject(strCAId);
        ...
        MapList mapListParts = doECO.getRelatedObjects(context, ChangeConstants.RELATIONSHIP_IMPLEMENTED_ITEM, strPartType, strlObjectSelects, null, false, true, (short) 1, strObjWhereclause, null);
    ...
    public int disconnectMarkups(Context context, String args[]) throws Exception {
        try {
            ...
            if (affectedObject.isKindOf(context, DomainConstants.TYPE_PART)) {
                MapList mapListMarkups = affectedObject.getRelatedObjects(context, RELATIONSHIP_EBOM_MARKUP, "*", new StringList(SELECT_ID), null, false, true, (short) 1, objectWhere, null);
    ...
    public int disconnectAffectedItemCheck(Context context, String args[]) throws Exception {
        try {
            String strCAObjectId = args[0];
            String strAffectedItemId = args[1];
            String strApprovedMarkupItems = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),
                    "emxEngineeringCentral.AffectedItems.HasApprovedMarkups");
            String strMarkupRelName = RELATIONSHIP_APPLIED_MARKUP;
    ...
    public MapList getAllMarkupsConnectedToCA(Context context, String[] args) throws Exception {
        ...
        MarkupIds = domobj.getRelatedObjects(context, RELATIONSHIP_APPLIED_MARKUP, strTypes, selectStmts, selectRelStmts, false, true, (short) 1, null, null);
            ...
            if ((UIUtil.isNotNullAndNotEmpty(id)) && (UIUtil.isNotNullAndNotEmpty(name))) {
                if (id == null) {
                    ...
                    strFullName = "<A HREF=\"JavaScript:emxTableColumnLinkClick('emxTree.jsp?mode=insert&amp;emxSuiteDirectory=" + XSSUtil.encodeForURL(context, strSuiteDir) + "&amp;parentOID="
                            + XSSUtil.encodeForURL(context, strParentObjectId) + "&amp;jsTreeID=" + XSSUtil.encodeForURL(context, strJsTreeID) + "&amp;objectId=" + XSSUtil.encodeForURL(context, id)
                            + "', 'null', 'null', 'false', 'content')\" class=\"object\">" + XSSUtil.encodeForHTML(context, name) + "</A>";
                    strDest += strFullName;
                }
            }
                ...
                while (markupItr.hasNext()) {
                    ...
                    if (map.containsKey("from[" + RELATIONSHIP_EBOM_MARKUP + "].to.to[" + RELATIONSHIP_APPLIED_MARKUP + "].id")
                        ...
                        if (UIUtil.isNotNullAndNotEmpty(strMarkupRelIds) && UIUtil.isNotNullAndNotEmpty(strMarkIds)) {
                            String[] strRelIds = StringUtils.split(strMarkupRelIds, "\\a");
                            String[] strMarkupId = StringUtils.split(strMarkIds, "\\a");

                            for (int i = 0; i < strRelIds.length; i++) {
                                DomainRelationship.setFromObject(context, strRelIds[i], dCAId);
                            }
                        }
                    }
                }
    ...
    /**
     * This API determines whether part is already connected to this change and evaluates if the given part is actually eligible to connect to change order/change request based on its state according
     * to ENG rules. --- Verifies if the given part is already connected to the given change in form of Affected or implemented items. if it is connected then returns map with key ALREADY_CONNECTED
     * and value TRUE along with connected CA info else returns map with key ALREADY_CONNECTED and value FALSE
     * @param context
     * @param partId
     *            -- Part Object Id which is mandatory
     * @param changeId
     *            -- Change Object id which is mandatory
     * @return Map contains ALREADY_CONNECTED TRUE or FALSE info along with CA Object id info
     * @throws Exceptin
     */
    public static Map checkPartAlreadyConnectedToThisChange(Context context, String partId, String changeId) throws Exception {

        if (UIUtil.isNullOrEmpty(partId) || UIUtil.isNullOrEmpty(changeId)) {
            throw new FrameworkException("Invalid arguments passed to checkPartAlreadyConnectedToChange API, this API requires part and changed object ids mandatorily");
        }

        ChangeOrder changeOrder = new ChangeOrder(changeId);
        MapList AffectedItems = changeOrder.getAffectedItems(context);
        MapList implementedItems = getImplementedItems(context, changeId);
        AffectedItems.addAll(implementedItems);

        Map affectedItemMap;
        for (int i = 0; i < AffectedItems.size(); i++) {
            affectedItemMap = (Map) AffectedItems.get(i);
            if (partId.equalsIgnoreCase((String) affectedItemMap.get(DomainConstants.SELECT_ID))) { // Part is already connected to this change
                affectedItemMap.put("ALREADY_CONNECTED", "TRUE");
                return affectedItemMap;
            }
        }
        affectedItemMap = new HashMap();
        affectedItemMap.put("ALREADY_CONNECTED", "FALSE");
        return affectedItemMap;

    }

    /**
     * This api returns Consolidated implemented item list of the change actions from Change Order
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param changeId
     *            the change object id
     * @return MapList of the Consolidated implemented item
     * @throws FrameworkException
     */
    public static MapList getImplementedItems(Context context, String changeId) throws FrameworkException {

        MapList implementedItemList = new MapList();
        StringList objSelects = new StringList();
        ...
        try {
            ...
            String relPattern = new StringBuffer(ChangeConstants.RELATIONSHIP_CHANGE_ACTION).append(",").append(ChangeConstants.RELATIONSHIP_IMPLEMENTED_ITEM).toString();
            DomainObject dO = new DomainObject(changeId);
            MapList dataList = dO.getRelatedObjects(context, relPattern, DomainConstants.QUERY_WILDCARD, objSelects, relSelects, false, true, (short) 2, null, null, (short) 0);
            ...
            while (iterator.hasNext()) {
                ...
                level = Integer.parseInt((String) dataMap.get("level"));

                if (level == 1) {
                    changeActionMap = dataMap;
                }

                if (level == 2) {
                    if (changeActionMap != null) {
                        dataMap.put(ChangeConstants.RELATED_CA_ID, changeActionMap.get(SELECT_ID));
                        dataMap.put(ChangeConstants.RELATED_CA_TYPE, changeActionMap.get(SELECT_TYPE));
                        dataMap.put(ChangeConstants.RELATED_CA_NAME, changeActionMap.get(SELECT_NAME));
                        dataMap.put(ChangeConstants.RELATED_CA_POLICY, changeActionMap.get(SELECT_POLICY));
                        dataMap.put(ChangeConstants.RELATED_CA_REVISION, changeActionMap.get(SELECT_REVISION));
                        dataMap.put(ChangeConstants.RELATED_CA_CURRENT, changeActionMap.get(SELECT_CURRENT));
                    }
                    dataMap.put("level", "1");
                    implementedItemList.add(dataMap);
                }
            }
