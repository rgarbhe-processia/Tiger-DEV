...
public class IEFEBOMSyncNewPartCreation_mxJPO {
    ...
    public BusinessObject createPart(Context context, String[] args) throws Exception {
        ...
        if (MATCHING_PART_RULE.equals(MATCH_CADMODEL_REV)) {
            /*
             * [NDM]if(isMinorType.equalsIgnoreCase("true")) { BusinessObject majorObject = mxUtil.getMajorObject(context, cadObject); majorObject.open(context); cadModelRevision =
             * majorObject.getRevision(); majorObject.close(context); }
             */
    ...
    private Hashtable getAttributeMap(Context context, IEFEBOMConfigObject ebomConfObj, BusinessObject cadObject, String instanceName, String busType, String partType) throws Exception {
        ...
        while (cadAttrEnum.hasMoreElements()) {
            try {
                ...
                if (partAttrName.startsWith("$$"))
                    ...
                    // TIGTK-2466
                    // EbomSync does not work for new parts as interface attributes are not present attributes are not present
                    // Modified by Asha
                    ...
                    if (checkIfAttributeExistsOnPart) {
                        ...
                        if (!attributeList.contains(partAttrName)) {
                            ...
                            // .getString(
                            // "mcadIntegration.Server.Message.AttributeDoesNotExistsOnType",
                            // messageDetails);
                            // MCADServerException.createException(errorMessage,
                            // null);
                        }
                    }
                    if (Attributepresent)
