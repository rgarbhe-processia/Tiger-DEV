...
public class DECNewDataModelMigrationBase_mxJPO {
    ...
    /**
     * Second Step of DEC New Data Model Migration. This method changes the type of object in data base according to types mapped in Page Object.
     * @param context
     *            Matrix context object
     * @param args
     *            Command line arguments provided by user arg[0] is a input directory arg[1] is a Page Object Name
     * @throws Exception
     */
    ...
    /**
     * Third and last step of DEC New Data Model migration This method move file inside Active Versioned object to Revision object and Copies Active Versioned Object's relationship to Revision object
     * @param context
     *            Matrix context object
     * @param args
     *            Command line arguments provided by user arg[0] is a input directory arg[1] is a Page Object Name arg[2] is a GlobalConfigObject Bus Id
     * @throws Exception
     */
                    ...
                    for (int cadMxTypeCnt = 0; cadMxTypeCnt < cadTypeMxType.size(); cadMxTypeCnt++) {
                        ...
                        if (cadMxType.contains("|" + designType)) {
                            ...
                            if (DOLikeCadTypes.contains(doCadType)) {
                                ...
                                if (!MCADStringUtils.isNullOrEmpty(doBusType) && !MCADStringUtils.isNullOrEmpty(doBusName) && !MCADStringUtils.isNullOrEmpty(doBusRevision)) {
                                    try {
        ...
        try {
            ...
            if (relIdChildidsMap != null) {
                ...
                while (relIdsItr.hasNext()) {
                    ...
                    try {
                        if (isVersionObject && relationshipName.equalsIgnoreCase(MCADMxUtil.getActualNameForAEFData(context, "relationship_PartSpecification"))) {
                        ...
                        } else if (!isVersionObject && relationshipName.equalsIgnoreCase(MCADMxUtil.getActualNameForAEFData(context, "relationship_DesignBaseline"))) {
                            ...
                            if (!MCADStringUtils.isNullOrEmpty(newRelId))
                                result = true;
                        } else if ((isActiveVersionedObj || !isVersionObject) && (gcoObject.isRelationshipOfClass(relationshipName, MCADAppletServletProtocol.ASSEMBLY_LIKE)
                            ...
                            /**
                             * Three possible Cases here Case 1. Parent is not finalized and child is not finalized AssemblyLike relationship between Active iteration of Parent and Child Soln : Copy
                             * relationship on Revision of parent and child (Rel exists check not necessary, multiple relationship of same type can be present betn Parent and child) Case 2. Parent is
                             * not finalized and child is finalized AssemblyLike relationship between Active iteration of Parent and Revision obj of Child Soln : Move Relationship's TO end to child
                             * Active Iteration, Copy relationship on Revision of parent and child Case 3. Parent is finalized and child is finalized AssemblyLike relationship between Active iteration
                             * of Parent and Revision obj of Child & AssemblyLike relationship between Revision of Parent and Child Soln : Move Relationship's FROM end to child Active Iteration
                             */
                            ...
                            if (!alreadyProcessedRelIds.contains(relId)) {
                                ...
                                if ((isVersionObject || !isChildMajor) && ((!isParentFinalized && isChildFinalized) || (isParentFinalized && isChildFinalized))) {
     ...
     * @param logFileName
     *            Log file name which will be created inside input directory
     * @throws Exception
     */
