 ...
 */
/*
 ** # @quickreview LX6 XXX 05/12/2015 IR-371391-3DEXPERIENCER2015x Requirement Central Tracebility shows Function's name, but user need the function's title along with Function name.
 */
...
public class emxVPLMTraceabilityReportBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    // START : LX6 IR-371391-3DEXPERIENCER2015x Tracebility shows Function's name, but user need the function's title along with Function name.
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (!context.isConnected()) {
            String strContentLabel = i18nNow.getI18nString("emxRequirements.Alert.FeaturesCheckFailed", "emxRequirementsStringResource", context.getSession().getLanguage());
     ...
     * @since RequirementsManagement V6R2008-2.0
     */
    public MapList getVPLMFunctionalRoots(Context context, String[] rowIds) throws ClassNotFoundException, Exception {
     ...
     * @since RequirementsManagement V6R2008-2.0
     */
    public MapList getVPLMLogicalRoots(Context context, String[] rowIds) throws ClassNotFoundException, Exception {
     ...
     */
    public MapList getVPLMPhysicalRoots(Context context, String[] rowIds) throws ClassNotFoundException, Exception {
        // System.out.println("\n|===>emxVPLMTraceabilityReportBase::getVPLMLogicalRoots");
        if (rowIds != null && rowIds.length == 1 && initVPLMContext(rowIds[0]))
            // START : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type
     ...
     *            the eMatrix <code>Context</code> object
     * @param rootType
     *            indicates Functional or Logical Root type
     * @return MapList List that contains the Function Root Data
     * @throws Exception
     *             if the operation fails
     * @since RequirementsManagement V6R2008-2.0
     */
    // START : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type
    private MapList getVPLMFLRoots(Context context, String rootType) throws ClassNotFoundException, Exception {
        // END : LX6 IR-361052 - Requirement-Physical Traceability not avaible for Product sub type
        ...
        printDebugTrace("RFLPLMFunctional", RFLPLMFUNCTIONAL, debugTrace);
        printDebugTrace("RFLVPMLogical", RFLVPMLOGICAL, debugTrace);
        printDebugTrace("RFLVPMPhysical", RFLVPMPHYSICAL, debugTrace);
        ...
        try {
            ...
            if (rootType.equals("Function"))
                FLRoots = getVPLMRoots(context, RFLPLMFUNCTIONAL);
            else if (rootType.equals("Logical"))
                FLRoots = getVPLMRoots(context, RFLVPMLOGICAL);
            else if (rootType.equals("Physical"))
                FLRoots = getVPLMRoots(context, RFLVPMPHYSICAL);
            ...
            for (int i = 0; i < FLRoots.size(); i++) {
                ...
                // END : LX6 IR-371391-3DEXPERIENCER2015x Requirement Central Tracebility shows Function's name, but user need the function's title along with Function name.
    ...
    public MapList getTraceabilityFLData(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            reqObjects = emxObject.getRelatedObjects(context, strRelType, strObjTypes, objSelects, relSelects, getTo, getFrom, sRecurse, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);
            ...
            vplmTraceabilityData = getVPLMTraceabilityFLData(rootObjId, reportType);
     ...
     * @param iMapList1
     *            m1 traceability data list
     * @param iMapList2
     *            vplm traceability data list
     ...
     * @param rootId
     *            Id of the Functional or Logical Root
     * @param reportType
     *            type of report ( Functional or Logical)
     * @return List object that contains vplm traceability data
     * @throws Exception
     *             if the operation fails
     * @since RequirementsManagement V6R2008-2.0
     */
    private MapList getVPLMTraceabilityFLData(String rootId, String reportType) throws Exception {
        ...
        // Get the implement OO Links
        // printDebugTrace("Before calling getImplementOOLinks", "", debugTrace);
        // ImplementOOLink[] implOOLinks = plmModeler.getImplementOOLinks(rootId, plmId);
        // printDebugTrace("After calling getImplementOOLinks", "", debugTrace);
        // checkForNull("implOOLinks", implOOLinks, debugTrace);

        // if (implOOLinks == null)
        // return(dataList);

        String implementingRootType = "";
        if ("Function".equalsIgnoreCase(reportType)) {
            implementingRootType = RFLPLMFUNCTIONAL;
        } else if ("Logical".equalsIgnoreCase(reportType)) {
            implementingRootType = RFLVPMLOGICAL;
        } else if ("Physical".equalsIgnoreCase(reportType)) {
            implementingRootType = RFLVPMPHYSICAL;
        }
        // Loop thru the implement OO Links to get the implementING and implementED objects
        // for (int jj = 0; jj < implOOLinks.length; jj++)
        for (int i = 0; i < reqObjects.size(); i++) {
            Map reqObj = (Map) reqObjects.get(i);
            String reqObjType = (String) reqObj.get(DomainConstants.SELECT_TYPE);
            ...
            // printDebugTrace("index of implOOLinks", Integer.toString(jj), debugTrace);
            // printDebugTrace("============== BEGIN-OOLinks =========================", "", debugTrace);
            // ImplementOOLink ooLink = implOOLinks[jj];
            // printDebugTrace("Before calling getImplementing", "", debugTrace);

            // ------------------------------------
            // Get the implementING Function/Logical
            // ------------------------------------
            // PLMxEntityDef[] plmPaths = ooLink.getImplementing();
            ...
            for (int j = 0; allPlmPaths != null && j < allPlmPaths.length; j++) {
                ...
                // System.out.println(" => Implementing type: " + plmInst.getPLMType());
                // SpecificationStructure.printIndentedMap(plmInst.getAttributes());

                // First, get the Matrix ID of the projected Requirement entity...
                // StringList refPlmIds = new StringList(refPlmId);
                ...
                if (fnLogM1ID == null || fnLogRefObj == null)
                    continue;

                Hashtable attHash = fnLogRefObj.getAttributes();
                // START : LX6 IR-371391-3DEXPERIENCER2015x Requirement Central Tracebility shows Function's name, but user need the function's title along with Function name.
                ...
                /*
                 * //------------------------------------ //Get the implementED Requirement //------------------------------------ PLMxEntityDef[] reqPaths = path; //ooLink.getImplemented();
                 * PLMxRefInstanceEntity reqInst = (PLMxRefInstanceEntity) reqPaths[reqPaths.length-1]; // BUG 372333/IR A0659289 if (reqInst == null) continue;
                 * 
                 * String plmRefId = reqInst.getRefPLMIdentifier(); printDebugTrace("plmRefId", plmRefId, debugTrace); //System.out.println(" => Implemented type: " + reqInst.getPLMType());
                 * //SpecificationStructure.printIndentedMap(reqInst.getAttributes());
                 * 
                 * // BUG! IR# 642650 - Only implemented Requirements should be returned here, // but it seems that Functions are also returned. So we will ignore them... if
                 * (!PLMDictionaryServices.isSpecializationOf(reqInst, RFLPLMREQUIREMENT)) continue;
                 * 
                 * // First, get the Matrix ID of the published Requirement object... //StringList plmRefIDs = new StringList(plmRefId); String[] prjReqIDs = coreModeler.convertPLMIDinM1ID(new
                 * String[]{plmRefId}); String plmReqID = (prjReqIDs == null || prjReqIDs.length == 0? null: prjReqIDs[0]); printDebugTrace("plmReqID", plmReqID, debugTrace);
                 * 
                 * DomainObject plmReqObj = DomainObject.newInstance(emxContext, plmReqID); checkForNull("plmReqObj", plmReqObj, debugTrace);
                 * 
                 * String objId = plmReqObj.getId(); String objType = plmReqObj.getInfo(emxContext, DomainConstants.SELECT_TYPE); String objName = plmReqObj.getName(); printDebugTrace("objId", objId,
                 * debugTrace); printDebugTrace("objType", objType, debugTrace); printDebugTrace("objName", objName, debugTrace);
                 */
                // Make sure we don't return the same pair more than once...
        ...
        // SpecificationStructure.printIndentedList("dataList", dataList);

        // System.out.println("|===>returning from emxVPLMTraceabilityReportBase::getVPLMTraceabilityFLData\n");
        return dataList;
    }
