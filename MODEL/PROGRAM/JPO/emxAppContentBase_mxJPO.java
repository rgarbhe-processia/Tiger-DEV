...
public class emxAppContentBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    private static String searchTypes = null;
    ...
    public Object getSearchResult(Context context, String[] args) throws Exception {
        ...
        if (sSetName.equals("")) {
            if (savedQueryName.equals("")) {
                if ((scopeId != null) && !(scopeId.equals("")) && !(scopeId.equals("Organization"))) {
                    ...
                    if (strType.equals(DomainObject.TYPE_PROJECT_SPACE) || strType.equals(DomainObject.TYPE_WORKSPACE_VAULT) || strType.equals(DomainObject.TYPE_WORKSPACE)) {
                    ...
                    if (sWorkspaceFolderId != null) {
                        ...
                        while (folderListSt.hasMoreTokens()) {
                            ...
                            MapList docMapList = domainObject.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(), // "*",//patternType.getPattern(),
                                    resultSelects, null, false, true, (short) 0, sObjWhere, null, filTypePattern, null, null);
                            if (docMapList.size() > 0) {
                                docMapList.sort("level", "ascending", "integer");
                            }
                            totalresultList.addAll(docMapList);
                        }
                    } else {
                        MapList docMapList = domainObject.getRelatedObjects(context, "*", // relPattern.getPattern(),
                    ...
                    totalresultList = DomainObject.findObjects(context, selType, txtName, txtRev, txtOwner, Vaults, sWhereExp, null, true, resultSelects, Short.parseShort(queryLimit), txtFormat,
                            txtSearch);
                } else {
                    if (!txtName.equals("*")) {
                    ...
                    totalresultList = DomainObject.findObjects(context, selType, "*", txtRev, txtOwner, txtVault, sWhereExp, null, true, resultSelects, Short.parseShort(queryLimit), txtFormat,
                            txtSearch);
                }
            } else {
    ...
    public String getRouteContentScope(Context context, String[] args) throws Exception {

        String sType = args[1];

        String command = "print Type $1 select $2 dump";
        String RootType = MqlUtil.mqlCommand(context, command, sType, "kindof");

        String relType = DomainObject.RELATIONSHIP_OBJECT_ROUTE;
        String mqlCmd = "print relationship $1 select $2 dump";
        try {
            if (UIUtil.isNullOrEmpty(searchTypes)) {
                searchTypes = MqlUtil.mqlCommand(context, mqlCmd, relType, "fromtype");
            }
        } catch (FrameworkException fe) {
            searchTypes = " ";
            System.out.println("---fe--->" + fe);
        }
        StringList sDefaultSymbolicTypeList = FrameworkUtil.split(searchTypes, ",");
        boolean isValidType = false;

        for (int i = 0; i < sDefaultSymbolicTypeList.size() && UIUtil.isNotNullAndNotEmpty(sType); i++) {
            String symbolicType = (String) sDefaultSymbolicTypeList.get(i);

            if (sType.equals(symbolicType) || RootType.equals(symbolicType)) {
                isValidType = true;
                break;
            }
        }
        ...
        if (isValidType) {
            ...
            if (args.length > 2) {
                isMatrixSearch = "MATRIXSEARCH".equalsIgnoreCase(args[2]);
            }
            ...
            Pattern relPattern = new Pattern(DomainObject.RELATIONSHIP_SUB_VAULTS);
            relPattern.addPattern(DomainObject.RELATIONSHIP_WORKSPACE_VAULTS);
            ...
            MapList docMapList = domainObject.getRelatedObjects(context, relPattern.getPattern(), typepattern.getPattern(), resultSelects, null, true, false, (short) 0, "", null, null, null, null);

            Iterator itr = docMapList.iterator();
            Map docMap = null;

            while (itr.hasNext()) {
                docMap = (Map) itr.next();
                String includeOID = (String) docMap.get("id");
                slReturn.add(includeOID);
            }

            return FrameworkUtil.join(slReturn, delimeter);
        }
        return "";
    }
