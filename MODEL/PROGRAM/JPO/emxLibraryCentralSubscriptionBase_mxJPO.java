...
public class emxLibraryCentralSubscriptionBase_mxJPO extends emxTransactionNotificationUtil_mxJPO {
     ...
     */
    public emxLibraryCentralSubscriptionBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (true) {
            throw new Exception("Must specify method on emxLibraryCentralSubscription invocation");
        }
    ...
    /**
     * Gets all Event object connected to objectId passed thrugh Publish Subscribe object and connected to logged in User
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            the holds following input arguments: 0 - HashMap containing one String entry for key "objectId"
     * @return theTreeMap with all objectEvents
     * @throws Exception
     *             if the operation fails
     */
    public TreeMap getAllObjectEvents(matrix.db.Context context, String[] args) throws Exception {
        ...
        String sRelSubscribedPerson = PropertyUtil.getSchemaProperty(context, "relationship_SubscribedPerson");

        String sRelPublish = PropertyUtil.getSchemaProperty(context, "relationship_Publish");

        String sAttType = PropertyUtil.getSchemaProperty(context, "attribute_EventType");
        ...
        MapList result = domainObj.getRelatedObjects(context, relPattern.getPattern(), LibraryCentralConstants.QUERY_WILDCARD, objSelect, relSelect, false, true, (short) 3, objWhere, null,
                typePattern, null, null);
        ...
        while (iterator.hasNext()) {
            ...
            treeMap.put(temp.get(strBuffer.toString()), temp.get(DomainConstants.SELECT_ID));
        }

        // filter out Events which are not connected to logged in user

        TreeMap filteredMap = filterOutUnWantedEvents(context, treeMap);
     ...
     *            the eMatrix <code>Context</code> object
     * @param treeMap
     *            <code>TreeMap</code> object
     * @return a Java <code>TreeMap</code>
     * @throws FrameworkException
     *             if the operation fails
     */
    protected TreeMap filterOutUnWantedEvents(Context context, TreeMap treeMap) throws FrameworkException {
    ...
    /**
     * This method gets all the Person Objects connected to Event object id passed and return true if logged in user is present
     * @param context
     ...
     *             if the operation fails
     */
    protected boolean checkForConnectedPerson(Context context, String eventId) throws FrameworkException {
        // get the symbolic names
        String sTypeEvent = PropertyUtil.getSchemaProperty(context, "type_Event");
        ...
        MapList mapList = domainObj.getRelatedObjects(context, relPattern.getPattern(), LibraryCentralConstants.QUERY_WILDCARD, objSelect, relSelect, false, true, (short) 1, null, null);
        ...
        while (iterator.hasNext()) {
            temp = (Hashtable) iterator.next();
            treeMap.put(temp.get(DomainConstants.SELECT_ID), temp.get(DomainConstants.SELECT_NAME));
        }
     ...
     *            <code>Context</code> object
     * @param treeMap
     *            TreeMap with person event details
     ...
     *             if the operation fails
     */
    protected void connectEventPerson(Context context, String evtId) throws FrameworkException, MatrixException {
        ...
        evtObj.connect(context, new RelationshipType(sRelSubscribePerson), true, person);
    }

    /**
     * Creates event object, set the event type attribute and connect it to logged in person and Publish Subscribe object
     * @param context
     *            <code>Context</code> object
     * @param psId
     *            person id
     ...
     *             if the operation fails
     */
    protected void createEventObject(Context context, String psId, String attributeValue) throws FrameworkException, MatrixException {
        ...
        String sEventId = FrameworkUtil.autoName(context, "type_Event", "", "policy_Event", context.getVault().getName());
        ...
        Person person = Person.getPerson(context);
        eventObj.connect(context, new RelationshipType(sRelSubscribePerson), true, person);
     ...
     *             if the operation fails
     */
    public static String getPublicSubscribeObject(Context context, String parentId, boolean createNew) throws FrameworkException, MatrixException {
        ...
        while (iterator.hasNext()) {
            temp = (Hashtable) iterator.next();
            treeMap.put(temp.get(DomainConstants.SELECT_ID), temp.get(DomainConstants.SELECT_ID));
        }
        ...
        if (treeMap.size() == 0) {
            if (createNew) {
                psId = FrameworkUtil.autoName(context, "type_PublishSubscribe", "", "policy_PublishSubscribe", context.getVault().getName());
                ...
                workspace.getSubscriptionManager().initiate(context, parentId, psId);
            }
        } else {
            // treeMap has all
     ...
     *            <code>Context</code> object
     * @param args
     *            holds following arguments: 0 - parentId parentId for Subscription 1 - vectorSubEvents the Java <code>Vector</code> containing al Sub events
     * @throws FrameworkException
     *             if the operation fails
     */
    public void processSubscribeEvents(Context context, String[] args) throws Exception {
        if ((args == null) || (args.length < 1)) {
            throw (new IllegalArgumentException());
        }
        Map map = (Map) JPO.unpackArgs(args);
        ...
        for (int i = 0; i < vctSubEvtStr.size(); i++) {
            createEventObject(context, psId, (String) vctSubEvtStr.elementAt(i));
        }
    }
     ...
     *            <code>Context</code> object
     * @param args
     *            hols following arguments 0 - unSubEvents token separated string of event ids to unsubsribe
     * @throws Exception
     *             if the operation fails
     */
    public void processUnSubscribeEvents(Context context, String[] args) throws Exception {
        if ((args == null) || (args.length < 1)) {
            throw (new IllegalArgumentException());
        }
        ...
        while (st.hasMoreTokens()) {
            ...
            person.disconnect(context, new RelationshipType(sRelSubscribePerson), false, domainObj);
        }
    }
     ...
     *             if the operation fails
     */
    public static MapList getSubscriptionDetailsforContextualUser(Context context, String[] args) throws Exception {
        ...
        MapList eventMapList = (MapList) currentUser.getRelatedObjects(context, sRelSubscribedPerson, LibraryCentralConstants.QUERY_WILDCARD, eventObjectSelect, new StringList(), true, false,
                (short) 1, null, null);
        ...
        while (iterator.hasNext()) {
            ...
            MapList dcObjectsMapList = myobj.getRelatedObjects(context, sRelPublish + "," + sRelPublishSubscribe, LibraryCentralConstants.QUERY_WILDCARD, dcObjectSelects, new StringList(), true,
                    false, (short) 2, null, null, null, psRelPattern, null);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments 0 - macro TRANSHISTORY from transaction trigger
    ...
    /**
     * Overridden Method to parse the History Events string to return stringlist of events with | separated This method is overridden to parse the promote/demote events with state (e.g promote to
     * Active_)
     * @param strEventsHistory
     *            is a String containing the list of events occured on a particular object
    ...
    public StringList parseEvents(Context context, String strEventsHistory) throws Exception {
        ...
        while (iEventIndex != -1) {
            ...
            if (itimeIndex != -1) {
                ...
                int iTimeEndIndex = -1;
                if (isAMPM) {
                    iTimeEndIndex = strTime.indexOf(super.TRANS_HISTORY_TIME_DELIMITER1) == -1 ? strTime.indexOf(super.TRANS_HISTORY_TIME_DELIMITER2)
                            : strTime.indexOf(super.TRANS_HISTORY_TIME_DELIMITER1);
                }
    ...
    public StringList loadEvents(Context context, StringList strlNotificationCommandName) {
        ...
        for (int i = 0; i < strlNotificationCommandName.size(); i++) {
            ...
            if (strEventHistory.indexOf("modify_") >= 0 || strEventHistory.indexOf("revisioned_") >= 0 || strEventHistory.indexOf("lock_") >= 0 || strEventHistory.indexOf("promote to") >= 0
     ...
     * @param strEventsHistory
     *            is a String containing the list of events which are to be parsed,Always First Event in the List is checked for modification
