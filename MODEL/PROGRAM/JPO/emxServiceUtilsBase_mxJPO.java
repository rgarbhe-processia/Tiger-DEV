...
public class emxServiceUtilsBase_mxJPO {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds following input arguments 0 - id - id 1 - reqPolicyState - reqPolicyState 2 - searchCriteria - searchCriteria
     * @return int
     * @throws Exception
     *             if the operation fails
     * @since AEF 10-0-1-0.
    ...
    public int checkRevisions(Context context, String[] args) throws Exception {
        ...
        // TIGTK-5782 - 23-03-2017 - VP - START
        // Get Development Part Policy if development Part is Parent and Standard or EC Part is Child of it.
        String strDevelopmentPartPolicyName = PropertyUtil.getSchemaProperty(context, "policy_PSS_Development_Part");
        if (strDevelopmentPartPolicyName.equals(policyName)) {
            reqPolicyState = "state_Complete";
        }
        // TIGTK-5782 - 23-03-2017 - VP - END
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds following input arguments 0 - id - id 1 - type - type 2 - which - which 3 - sECRType - sECRType 4 - sECOType - sECOType 5 - sPartType - sPartType 6 - sDrwPrintType -
     *            sDrwPrintType 7 - sPdtLineType - sPdtLineType 8 - sSketchType - sSketchType 9 - sMarkupType - sMarkupType 10 - sCADMdlType - sCADMdlType 11 - sEBOMMkpType - sEBOMMkpType 12 -
     *            sECRMnPdtAfftdRelName - sECRMnPdtAfftdRelName 13 - sECRSuppDocRelName - sECRSuppDocRelName 14 - sNewRevSpecRelName - sNewRevSpecRelName 15 - sRequestSpecRevRelName -
     *            sRequestSpecRevRelName 16 - sMakeObsRelName - sMakeObsRelName 17 - sNewRevPartRelName - sNewRevPartRelName 18 - sRequestPartObsRelName - sRequestPartObsRelName 19 -
     *            sRequestPartRevRelName - sRequestPartRevRelName 20 - sECOChgRequestRelName - sECOChgRequestRelName
     * @return int
     * @throws Exception
     *             if the operation fails
     * @since AEF 10-0-1-0.
    ...
    public int getBuildStatus(Context context, String[] args) throws Exception {
        ...
        // boolean boolECORevRelFound = false;
        boolean boolECRObsRelFound = false;
        // boolean boolECRRevRelFound = false;
        ...
        if (type.equals(sPartType) || type.equals(sECADMdlType) || type.equals(sMCADMdlType) || type.equals(sMCADAssemblyType) || type.equals(sMCADAssemblyInstanceType)
                || type.equals(sMCADComponentType) || type.equals(sMCADComponentInstanceType) || type.equals(sMCADParameterizedModelType) || type.equals(sMCADParameterizedAssemblyModelType)
        ...
        if (sBusObjType.equals(sPartType)) {
            sRelPattern = sRequestPartObsRelName + "," + sRequestPartRevRelName + "," + sMakeObsRelName + "," + sNewRevPartRelName + "," + sECOChgRequestRelName;
        }

        if (sBusObjType.equals(sDrwPrintType) || sBusObjType.equals(sCADMdlType) || sBusObjType.equals(sCADDrwType)) {
            sRelPattern = sNewRevSpecRelName + "," + sRequestSpecRevRelName + "," + sECOChgRequestRelName;
        ...
        if (checkType.equals(sECRType) || (checkType.equals(sECOType) && !boolNextRevConnected)) {
            ...
            // Added code for the bug 299849 : End
            {
                ...
                if (checkType.equals(sECRType) && (index = relNames.indexOf(sRequestSpecRevRelName)) >= 0) {
                    sIndex = index;
                    cmd.executeCommand(context, "print bus $1 select state dump $2", (String) busIds.elementAt(sIndex), "|");
                    sResult = cmd.getResult().trim();
                    sResult = sResult.substring(sResult.lastIndexOf('|') + 1, sResult.length());
                    if (((String) currentStates.elementAt(sIndex)).equals(sResult)) {
                        sConnectStatus = "Available\t \t";
     ...
     *            the eMatrix <code>Context</code> object.
     * @param id
     *            String representing objectId.
     * @param typeName
     *            String representing typeName.
