...
/*
 * emxMemberListBase Copyright (c) 2002-2015 Dassault Systemes. All Rights Reserved This program contains proprietary and trade secret information of MatrixOne, Inc. Copyright notice is precautionary
...
/**
 * The <code>emxMemberListBase</code> class contains methods to provide functionality related with Member List for Specifications.
...
public class emxMemberListBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public emxMemberListBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList personList
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList groupList
    ...
    public static MapList getCoOwnedDistributionLists(Context context, String args[]) throws Exception {
        try {
            ...
            /*
             * MapList personMap = company.getRelatedObjects(context, RELATIONSHIP_MEMBER_LIST, TYPE_MEMBERLIST, objSelects, relSelects, false, true, level,"","");
             */
    ...
    /**
     * Get the Scopes. Used as a Column method in the Table to get Scopes
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList objectList
    ...
    /**
     * Get the list of Business Units. Used as a Column method in the Table to get Business Units.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public static Vector getBusinessUnits(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            /*
             * while(memberListItr.hasNext()) { //get the memberlist object id String memberListId=(String)((Map)memberListItr.next()).get(DomainConstants.SELECT_ID); DomainObject doObj=new
             * DomainObject(memberListId);
             * 
             * // get the list of regions connected to the memberlist MapList buIdList = doObj.getRelatedObjects(context, RELATIONSHIP_MEMBER_LIST,fromObjectTypePattern, objSelects, relSelects, true,
             * false, level, "", ""); if (buIdList == null || buIdList.size() == 0) { businessUnitList.addElement(""); } Iterator buIdListItr=buIdList.iterator();
             * 
             * while(buIdListItr.hasNext()) { Map bu=(Map)buIdListItr.next(); if(bu != null){ String fromType = (String)bu.get(DomainConstants.SELECT_TYPE); if(fromType.equals(TYPE_BUSINESSUNIT) ||
             * fromType.equals(TYPE_COMPANY) ){ String objId=(String)bu.get(SELECT_ID); if("CSV".equals(exportFormat) || "HTML".equals(reportFormat))
             * retValue=(String)bu.get(DomainConstants.SELECT_NAME); else retValue=
             * "<a href=\"javascript:emxTableColumnLinkClick('../common/emxTree.jsp?emxSuiteDirectory=components&relId=null&parentOID=null&jsTreeID=null&objectId=" +objId+
             * "', '700', '600', 'false', 'popup')\">"+(String)bu.get(DomainConstants.SELECT_NAME)+"</a>"; businessUnitList.addElement(retValue); }else if (fromType.equals(TYPE_PERSON)){
             * businessUnitList.addElement(""); } }else{ businessUnitList.addElement(""); } }
             * 
             * }
             */
        } catch (Exception e) {
    ...
    public static String getRoot(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    /**
     * Get the Scope. Used as a Field method in the Webform to get Scopes
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList objectList
    ...
    public static String getScope(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            boolean isAdmin = (personAssignments.contains(PropertyUtil.getSchemaProperty(context, "role_SpecificationOfficeManager"))
                    || personAssignments.contains(DomainObject.ROLE_COMPANY_REPRESENTATIVE) || personAssignments.contains(DomainObject.ROLE_ORGANIZATION_MANAGER));
                ...
                if (scope != null) {
                    ...
                    if (typeName != null && typeName.equals(TYPE_SPECIFICATIONOFFICE) && isAdmin) {
                        ...
                        if (mode.equalsIgnoreCase("view")) {
                            ...
                            retVal = "<table cellspacing=5 border=0><tr><td><input type=radio name='rb' value='Personal' onClick='javascript:disablePersonalRadio()'>" + personal
                                    + "</td></tr><tr><td><input type=radio name='rb' value='Enterprise' checked >" + enterprise + "</td></tr></table>";

                        }
                    } else {
                        // X3: Added Plant
                        // Added department for bug 362581
                        if (fromType.equals(TYPE_BUSINESSUNIT) || fromType.equals(TYPE_COMPANY) || fromType.equals(TYPE_PLANT) || (fromType.equals(TYPE_PERSON) && isAdmin)
                                || fromType.equals(TYPE_DEPARTMENT)) {
                            // X3: Added Plant
                            ...
                            if (mode == null || mode.equalsIgnoreCase("view")) {
                                ...
                                retVal += "<tr><td><input type=radio name='rb' value='Enterprise' onClick='javascript:enableOrganization(\"" + XSSUtil.encodeForJavaScript(context, organizationName)
                                        + "\",\"" + XSSUtil.encodeForJavaScript(context, organizationId) + "\")'";
                                // X3: Added Plant
                                // Added Department for Bug 362581
                                if (fromType.equals(TYPE_BUSINESSUNIT) || fromType.equals(TYPE_COMPANY) || fromType.equals(TYPE_PLANT) || fromType.equals(TYPE_DEPARTMENT))
    ...
    /**
     * Get the Organization. Used as a Field method in the Webform to get Organization
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList objectList
    ...
    public int checkDemote(Context context, String args[]) throws Exception {
        ...
        if (memberListId == null || memberListId.equals("")) {
            throw (new IllegalArgumentException(
                    StringResource.format(EnoviaResourceBundle.getFrameworkStringResourceProperty(context, FrameworkStringResource.Generic_NullParameterPassed, context.getLocale()), "className",
                            "emxMemberList", "paramName", "memberListId", "methodName", "triggerAction")));
        }
        try {
    ...
    /**
     * Get the Organization of Member. Used as a Field method in the Webform to get Organization
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - objectList MapList
    ...
    public static String getMemberOrganization(Context context, String[] args) throws Exception {
        ...
        if (mode != null && mode.equals("edit") && isAdmin) {
            ...
            retStr += "<input type='button' " + "value='...' onclick=" + "\"javascript:showModalDialog(" + "'../components/emxComponentsSelectOwningOrganizationDialogFS.jsp?"
                    + "fieldName=OrganizationDisplay&" + "fieldNameDisplay=OrganizationDisplay&" + "formPopup=true&" + "fieldId=OrganizationOID&relId=" + XSSUtil.encodeForJavaScript(context, relId)
                    + "','100','100', true, 'MediumTall')\">&nbsp;";
        } else {
            retStr = "";
    ...
    public static String getVault(Context context, String[] args) throws Exception {
        ...
        if (vaultAwareness.equals("false")) {
            retStr = "<input type='text' READONLY='READONLY' name='Vault' id='' value='" + XSSUtil.encodeForHTMLAttribute(context, defaultVault)
                    + "' onFocus='this.blur()' maxlength='' size='20'></input>";
            retStr += "<input type='button' " + "value='...' onclick=" + "\"javascript:showChooser(" + "'../components/emxComponentsSelectSearchVaultsDialogFS.jsp?" + "multiSelect=false&amp;"
     ...
     * @param args
     *            holds the following input arguments: 0 - HashMap requestMap post process for member list creation
    ...
    public Map createMemberListProcess(Context context, String[] args) throws Exception {
        ...
        if (isExists) {
            returnMap.put("ErrorMessage", i18nNow.getTypeI18NString(MemberList.TYPE_MEMBER_LIST, languageStr) + " " + name + " " + revision + " "
                    + EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(languageStr), "emxComponents.Common.AlreadyExists"));
        } else {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - StringList excludeOIDList
    ...
    public static HashMap getScopeForCreation(Context context, String[] args) throws Exception {
        ...
        isCompanyRepresentative = personObj.isRepresentativeFor(context, companyId);

        Vector personAssignments = PersonUtil.getAssignments(context);
        boolean isAdmin = (personAssignments.contains(PropertyUtil.getSchemaProperty(context, "role_SpecificationOfficeManager"))
                || personAssignments.contains(DomainObject.ROLE_COMPANY_REPRESENTATIVE) || personAssignments.contains(DomainObject.ROLE_ORGANIZATION_MANAGER)
                || PersonUtil.hasAssignment(context, sVPLMProjectLeaderRole) || isCompanyRepresentative);
     ...
     *            parameters. reportFormat - a String to identify the Printer Friendly and Export view.
        ...
        for (int k = 0; k < objList.size(); k++) {
            ...
            if (PropertyUtil.getSchemaProperty(context, "type_BusinessGroup").equalsIgnoreCase(objType)) {
                ...
                if (!isprinterFriendly) {
                    strBuffName.append("<a href=\"javascript:showModalDialog('../common/emxTree.jsp?emxSuiteDirectory=components&amp;objectId=" + strMemberId
                            + "', '800', '575')\"><img src=\"../common/images/iconSmallPerson.gif\" alt=\"\" name=\"person\" id=\"PersonId\" border=\"0\" /></a>&#160;");
                    strBuffName.append("<a href=\"javascript:showModalDialog('../common/emxTree.jsp?emxSuiteDirectory=components&amp;objectId=" + strMemberId + "', '800', '575')\">"
                            + XSSUtil.encodeForXML(context, strDisplayName) + "</a>");
                } else {
