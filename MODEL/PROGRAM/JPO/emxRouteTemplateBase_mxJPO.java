...
public class emxRouteTemplateBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @grade 0
     */
    public emxRouteTemplateBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
     ...
     * @grade 0
     */
    public Vector getAvailability(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (objectListItr.hasNext()) {
                ...
                /**
                 * IR-268393V6R2014x - k3d RCA: Latin characters are not getting encoded properly to be parsed by SAXBuilder in UIFormCommon:addFields() FIX: changing encodeForHTML to encodeForXML as
                 * ProgramHTMLOutput field value is being parsed as XML
                 */
                ...
                if ((TYPE_PROJECT.equals(strConnectedType)) || (TYPE_PROJECT_SPACE.equals(strConnectedType)) || mxType.isOfParentType(context, strConnectedType, DomainConstants.TYPE_PROJECT_SPACE)) {
    ...
    public Object getBusinessUnitRouteTemplates(matrix.db.Context context, String[] args) throws Exception {
        ...
        obj.setId(objectId);
        list = obj.getRelatedObjects(context, RouteTemplate.RELATIONSHIP_OWNING_ORGANIZATION, TYPE_ROUTE_TEMPLATE, objectSelects, relSelects, false, true, (short) 1, busWhere, null);
        return list;
    }
    ...
    private Object getRouteTemplates(Context context, String objWhere) throws Exception {
        ...
        try {
            ...
            /*
             * Collection obPersonIds = null; Iterator personItr = null; BusinessObject objPerson = null;
             * 
             * HashMap personHash = new HashMap(); personHash.put("",PersonUtil.getPersonObjectID(context)); obPersonIds = personHash.values(); personItr = obPersonIds.iterator();
             * 
             * while(personItr.hasNext()) { objPerson = new BusinessObject((String)personItr.next()); templateObj.setId(objPerson.getObjectId());
             */// commented for bug 352540

            templateObj.setId(PersonUtil.getPersonObjectID(context));

            ContextUtil.startTransaction(context, false);
            ExpansionIterator expIter = templateObj.getExpansionIterator(context, RELATIONSHIP_ROUTE_TEMPLATES, TYPE_ROUTE_TEMPLATE, selectStmts, new StringList(), false, true, (short) 1, objWhere,
            ...
            if (FrameworkUtil.isSuiteRegistered(context, "featureVersionTeamCentral", false, null, null)) {
                ...
                MapList templateWorkspaceMapList = templateObj.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(), selectStmts, null, true, true, (short) 2, objWhere, "",
                        includeTypePattern, null, null);
            ...
            if (FrameworkUtil.isSuiteRegistered(context, "appVersionProgramCentral", false, null, null)) {
                ...
                MapList templateProjectspaceMapList = templateObj.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(), selectStmts, null, true, true, (short) 2, objWhere, "",
                        includeTypePattern, null, null);
            ...
            if (route.checkIfSubsidiaryOrgMembersEnabled(context)) {
                Company companyObj = (Company) newInstance(context, orgId);
                StringList objectSelects = new StringList();
                objectSelects.addElement(companyObj.SELECT_ID);
                StringList slCompanies = new StringList();
                if ((Person.getPerson(context).getCompanyId(context)).equals(orgId)) {
                    List<Map> mlCompany = companyObj.getAllSubsidiaries(context, objectSelects, 0);
                    for (Map map : mlCompany) {
                        slCompanies.add(map.get(companyObj.SELECT_ID));
                    }
                }
                DomainObject domObjPerson = DomainObject.newInstance(context, PersonUtil.getPersonObjectID(context));
                slCompanies.addAll(domObjPerson.getInfoList(context, "to[" + PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_relationship_Member) + "].from.id"));
                selectStmts.add(buFilter);

                List<Map> mlRT = DomainObject.findObjects(context, DomainObject.TYPE_ROUTE_TEMPLATE, DomainConstants.QUERY_WILDCARD, objWhere, selectStmts);
                StringList slRouteTempId = new StringList();
                for (Map mp : templateMapList) {
                    slRouteTempId.add(mp.get(DomainObject.SELECT_ID));
                }
                for (Map map : mlRT) {
                    String strOwnerOrg = (String) map.get(buFilter);
                    if (slCompanies.contains(strOwnerOrg)) {
                        if (!slRouteTempId.contains(map.get(SELECT_ID))) {
                            templateMapList.add(map);
                        }
                    }
                }
            ...
            while (templateMapListItr.hasNext()) {
                type = ""; // Added by Faurecia for ALM6153. type variable was not reinitialized for each iteration
                ...
                /*
                 * businessUnit = new BusinessUnit(busUnitId); StringList personNameList = new StringList(); personNameList.add(DomainObject.SELECT_NAME); MapList personMapList =
                 * businessUnit.getPersons(context, personNameList,objWhere); Iterator iterator = personMapList.iterator(); while (iterator.hasNext()) { Map buPerson = (Map) iterator.next(); perName =
                 * (String) buPerson.get("name"); if(contextPerson.equalsIgnoreCase(perName)) { finalTemplateMap.add(templateMap); break; } }
                 */
    ...
    public Vector getScope(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (objectListItr.hasNext()) {
                Map objectMap = (Map) objectListItr.next();
                String restrictMembers = (String) objectMap.get(RouteTemplate.SELECT_RESTRICT_MEMBERS);
                if (restrictMembers.equalsIgnoreCase("All")) {
                    restrictMembers = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(langStr), "emxComponents.Common.All");
                } else if (restrictMembers.equalsIgnoreCase("Organization")) {
                    restrictMembers = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(langStr), "emxComponents.Common.Organization");
                }
    ...
    public HashMap getRouteTemplateScopeInfo(Context context, String[] args) throws Exception {
        ...
        if (scopeType.equals(DomainConstants.TYPE_PERSON) || scopeType.equals(DomainConstants.TYPE_DEPARTMENT) || scopeType.equals(DomainConstants.TYPE_ORGANIZATION)
                || scopeType.equals(DomainConstants.TYPE_COMPANY) || scopeType.equals(DomainConstants.TYPE_BUSINESS_UNIT)) {
    ...
    public String showRouteTemplateAvailability(Context context, String[] args) throws Exception {
        ...
        if (person.hasRole(context, DomainObject.ROLE_ORGANIZATION_MANAGER) || person.hasRole(context, DomainObject.ROLE_COMPANY_REPRESENTATIVE)
                || PersonUtil.hasAssignment(context, sVPLMProjectLeaderRole)) {
            hasEnterpriseAccess = true;
        }
        if (connectedType.equals(boProject.TYPE_PERSON)) {
            sChecked = "";
        } else if (connectedType.equals(boProject.TYPE_DEPARTMENT) || connectedType.equals(boProject.TYPE_ORGANIZATION) || connectedType.equals(boProject.TYPE_COMPANY)
                || connectedType.equals(boProject.TYPE_BUSINESS_UNIT)) {
        ...
        if (mode.equals("edit")) {
            ...
            if (hasEnterpriseAccess) {
                ...
                sb.append(connectedType.equals(boProject.TYPE_DEPARTMENT) || connectedType.equals(boProject.TYPE_COMPANY) || connectedType.equals(boProject.TYPE_BUSINESS_UNIT)
                        || connectedType.equals(boProject.TYPE_ORGANIZATION) ? "checked" : "");
            ...
            sb.append("assignValidateMethod(\"txtWSFolder\", \"validateAvailabilityWorkspace\")");
            sb.append("</script>");
        }
        return sb.toString();
    }
    ...
    public String owningOrganizationProgramHTML(Context context, String[] args) throws Exception {
        ...
        sb.append("<input type=\"button\" name=\"selectOrganization\" value=\"...\"  onclick=\"javascript:showRTEditOrganizationChooser()\" ")
                .append(!sOwningOrganization.equals(null) && !sOwningOrganization.equals("null") && !sOwningOrganization.equals("") ? "" : "disabled").append(">");
    ...
    protected boolean routetemplateEditAccessCheck(Context context, String objectId) throws Exception {
        ...
        if (TYPE_PROJECT.equals(scopeType)) {
            ...
            /*
             * Workspace wsObj = new Workspace(scopeId); if(wsObj.isProjectLead(context,personObj)) return true;
             */
        }
    ...
    public Map getSaveAsTemplateAvailabilityRange(Context context, String[] args) throws FrameworkException {
        try {
            ...
            if (contextUser.isRepresentativeFor(context, contextUser.getCompanyId(context)) || PersonUtil.hasAssignment(context, sVPLMProjectLeaderRole)) {
    ...
    public Map getSaveAsTemplateTemplateDataRange(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            return getRangeValuesMap(new StringList("SaveTaskAssignees"), new StringList(ComponentsUtil.i18nStringNow("emxComponents.RouteTemplateSaveDialog.TaskAssignees", sLanguage)),
                    new StringList());
        } catch (Exception e) {
            throw new FrameworkException(e);
        }
    ...
    public Map saveRouteAsTemplate(Context context, String[] args) throws FrameworkException {
        try {
            ...
            DomainObject connectingObj = "User".equals(strAvailability) ? contextUser : "Enterprise".equals(strAvailability) ? company : DomainObject.newInstance(context, workspaceId);
            ...
            MapList routeNodeList = route.getRelatedObjects(context, relPersonPattern.getPattern(), typePersonPattern.getPattern(), objSel, selectPersonRelStmts, false, true, (short) 1, EMPTY_STRING,
                    EMPTY_STRING, 0);
     ...
     * @grade 0
     */
    public Object setRouteTemplateName(Context context, String[] args) throws Exception {
     ...
     * @grade 0
     */
    public Object setRouteTemplateDescrition(Context context, String[] args) throws Exception {
    ...
    /**
     * This method fetch the selected Scope of the Route Template. method returns the scope of the template if the route already has a template else scope of the route. return 'All' if the scope is a
     * Workspace
     * @param objectId
     *            - Object ID of the Route
     * @throws Exception
     */
