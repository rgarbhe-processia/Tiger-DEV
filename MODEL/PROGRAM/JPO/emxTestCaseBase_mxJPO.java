 ...
 * @quickreview KIE1 ZUD 15:09:08 : IR-395903-3DEXPERIENCER2015x If Estimated Completion Date is previous date when edit properties of Test Case, warning message will display and cannot edit
 *              properties
 */
...
public class emxTestCaseBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing the object id.
    ...
    public MapList getTestCasesWhereUsed(Context context, String[] args) throws Exception {
        ...
        String strComma = ",";
        String strRelationshipPattern = strTestCaseSubTestCaseReln + strComma + strTestCaseUseCaseReln + strComma + strTestCaseIncidentReln + strComma + strTestCaseRequirementReln + strComma
                + strTestCaseFeatureReln;
        short sRecursionLevel = 1;
        relBusObjPageList = getRelatedObjects(context, strRelationshipPattern, "*", objectSelects, relSelects, true, false, sRecursionLevel, "", "");
        return relBusObjPageList;
    }

    /**
     * This method gets the object Structure List for the context Test Case object.This method gets invoked by settings in the command which displays the Structure Navigator for Test Case type objects
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * Gets the list of all TestCases on the context Test Execution that have the 'Validation Status' attribute value on the connecting relationship as 'Validation Passed'. This method will be able to
     * show flat view of all such Test Cases connected to Test Execution as during Test Execution create all Test Cases from parent object are collected into a flat structure
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing the object id.
     * @return a <code>MapList</code> object having the list of all Test Cases connected to the context Test Execution with 'Validation Passed' value for 'Validation Status'
     * @throws Exception
     *             if the operation fails
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getPassedTestExecutionTestCases(Context context, String[] args) throws Exception {
        ...
        relBusObjPageList = getTestExecutionTestCases(context, strObjectId, strRelName, EMPTY_STRING, sbRelWhere.toString());
        return relBusObjPageList;
    }

    /**
     * Gets the list of all TestCases on the context Test Execution that have the 'Validation Status' attribute value on the connecting relationship as 'Validation Failed'. This method will be able to
     * show flat view of all such Test Cases connected to Test Execution as during Test Execution create all Test Cases from parent object are collected into a flat structure
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing the object id.
     * @return a <code>MapList</code> object having the list of all Test Cases connected to the context Test Execution with 'Validation Failed' value for 'Validation Status'
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10-6
     */
    ...
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getFailedTestExecutionTestCases(Context context, String[] args) throws Exception {
    ...
    /**
     * Gets the list of all TestCases on the context Test Execution that have the 'Validation Status' attribute value on the connecting relationship as 'Not Validated'. This method will be able to
     * show flat view of all such Test Cases connected to Test Execution as during Test Execution create all Test Cases from parent object are collected into a flat structure
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing the object id.
     * @return a <code>MapList</code> object having the list of all Test Cases connected to the context Test Execution with 'Not Validated' value for 'Validation Status'
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10-6
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getNotValidatedTestExecutionTestCases(Context context, String[] args) throws Exception {
    ...
    /**
     * Gets the list of all TestCases on the context Test Execution based on the 'bus where' and relationship 'where' search clauses passed in as parameters for the object
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param strParentId
     *            id of the object for which Test Case relationship expansion is done
     * @param strRelName
     *            the relationship that connects the parent object and child Test Cases
     * @param strBusWhereCondition
     *            the 'business object where' clause to be applied while expanding for 'Test Case' connections
     * @param strRelWhereCondition
     *            the 'relationship where' clause to be applied while expanding for 'Test Case' connections
    ...
    protected MapList getTestExecutionTestCases(Context context, String strParentId, String strRelName, String strBusWhereCondition, String strRelWhereCondition) throws Exception {
        ...
        relBusObjPageList = getRelatedObjects(context, strRelName, strType, objectSelects, relSelects, false, true, sRecursionLevel, strBusWhereCondition, strRelWhereCondition);
        return relBusObjPageList;
    }

    /**
     * The method gets the Status Column in Test Execution Test Case Summary Table. Returns the Status icon gif depending on the relationship attribute 'Validation Status'.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - MapList containing Relationship id list
    ...
    /**
     * This method returns the value for a perticular range of a attribute reading from emxFrameworkStringResource.propeties file. The key value is dynamically generated using the passed attribute
     * name and range "name.emxFramework.Range. <Attribute Name>. <Range Name>". This value can not be used for display purpose as the language string in GetString method is passed as blank and so
     * always engish string will be returned.
     * @param strAttributeName
     *            String holding the actual name of the attribute.
     * @param strRange
     *            String holding the range of the attribbute.
     * @return String The property file value of the range.
     * @throws Exception
     *             if the operation fails
     * @since Product Central 10-6
     */
    protected String getAttributeRangeValue(Context context, String strAttributeName, String strRange) throws Exception {
        ...
        sbKey.append(strAttributeName.replace(' ', '_'));
        sbKey.append(SYMB_DOT);
        sbKey.append(strRange.replace(' ', '_'));

        // Read the property file value by passing the generated key and
        // language string as blank.
        String strRangeValue = EnoviaResourceBundle.getProperty(context, "Framework", sbKey.toString(), DomainConstants.EMPTY_STRING);
    ...
    public StringList excludeSubTestCases(Context context, String[] args) throws Exception {
        ...
        MapList parentObjects = domObjTestCase.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, strRelationship), "*", new StringList(DomainConstants.SELECT_ID), null, true, false,
                (short) 0, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
        ...
        MapList currentObjectIDs = domObjTestCase.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, strRelationship), "*", new StringList(DomainConstants.SELECT_ID), null, false,
    ...
    /**
     * Estimated Completion Date field make mandatory or not based on a configuration property entry. Called from WebForm type_TestCase
     * @param context
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            - holds the HashMap containing the following arguments
    ...
    public static boolean isEstimatedDateRequired(Context context, String[] args) throws Exception {
        try {
            String strEnforceMandatoryEstimatedCompletionDate = EnoviaResourceBundle.getProperty(context, "emxProduct.TestCase.EnforceEstimatedCompletionDate");
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            hold the hashMap containing the following argument
     * @returns void
     * @throws Exception
     *             if operation fails.
     * @since R212
    ...
    public void validateEstimateDate(Context context, String[] args) throws Exception {
        try {
            ...
            strNewValue = eMatrixDateFormat.getFormattedInputDate(context, strNewValue, iClientTimeOffset, locale);
            // END T25 DJH
            ...
            if (!strOldEstimatedCompletionDate.equals(strEstimatedCompletionDate)) {
                ...
                if (newCompletionDate.before(dtTodaysDate) && day_of_month != tday_of_month) {
                    String language = context.getSession().getLanguage();
                    String strInvalidEstimateDate = EnoviaResourceBundle.getProperty(context, "ProductLine", "emxProduct.TestCase.Alert.InvalidEstimatedDate", language);
    ...
    public MapList getAssociatedParameters(Context context, String args[]) throws Exception {
        try {
            ...
            relBusObjPageList = dom.getRelatedObjects(context, relationships, toTypeName, objSelects, relSelects, false, true, (short) 0, null, null);
