...
public class emxModelBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public MapList getModels(Context context, String[] args) throws Exception {
        ...
        relBusObjPageList = getRelatedObjects(context, strRelName, strType, objectSelects, relSelects, false, true, sRecursionLevel, "", "");
    ...
    public MapList getCommitedFeatures(Context context, String[] args) throws Exception {
        ...
        objectSelects.add("to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from" + ".to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.name");
        objectSelects.add("to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from" + ".to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.revision");
        objectSelects.add("to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from" + ".to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.type");
        ...
        relBusObjPageList = dom.getRelatedObjects(context, sbRelName.toString(), sbTypeName.toString(), objectSelects, relSelects, false, true, sLevel, "", "", 0);
        ...
        if (!relBusObjPageList.isEmpty()) {
            for (int cn = 0; cn < relBusObjPageList.size(); cn++) {
                ...
                if (mxType.isOfParentType(context, sObjectType, ProductLineConstants.TYPE_FEATURES) && ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO.equals(sRelationshipName)) {
    ...
    public MapList getCommitedRequirements(Context context, String[] args) throws Exception {
        ...
        // The getRelatedObjects method is invoked to get the list of Assignees
        relBusObjPageList = dom.getRelatedObjects(context, strRelName, strTypeName, objectSelects, relSelects, false, true, sLevel, "", "");
        return relBusObjPageList;
    }
    ...
    /**
     * This method is used to create a Model. This method is invoked from the Bean.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public String createModel(Context context, String args[]) throws Exception {
        ...
        attributeMap.put(ProductLineConstants.ATTRIBUTE_MARKETING_TEXT, strModelMarketingText);
        attributeMap.put(ProductLineConstants.ATTRIBUTE_MARKETING_NAME, strModelMarketingName);
        attributeMap.put(ProductLineConstants.ATTRIBUTE_PREFIX, strModelPrefix);
        ...
        strNewObjId = pcBean.create(context, strModelType, strModelName, strRevision, strModelDescription, strPolicy, strVaultName, attributeMap, strModelOwner, strProductLineObjId,
                ProductLineConstants.RELATIONSHIP_PRODUCTLINE_MODELS, true);
        ...
        Map mpProd = domModelObj.getRelatedObject(context, ProductLineConstants.RELATIONSHIP_PRODUCTS, true, new StringList(DomainConstants.SELECT_ID), new StringList(DomainRelationship.SELECT_ID));
        ...
        if (mpProd != null) {
            ...
            domProductObj.setOwner(context, strModelOwner);
    ...
    /**
     * This method is used to create a Product on creation of a new Model. This method is invoked on Trigger Action of Model Creation.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds object id of the model as argument
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     */
    public void createProductOnCreate(Context context, String args[]) throws Exception {
        ...
        if (strIPMLCommandName == null || "".equalsIgnoreCase(strIPMLCommandName) || "null".equalsIgnoreCase(strIPMLCommandName)) {
            ...
            if (strCreateProduct != null && !"TRUE".equals(strCreateProduct)) {
                ...
                MapList mpObjList = DomainObject.findObjects(context, PropertyUtil.getSchemaProperty(context, strProductType), strModelName, "", WILD_CHAR, vaultPattern, null, true, objectSelects);
                if (mpObjList.size() == 0) {
                    ...
                    DomainRelationship domRel = domProductObj.createAndConnect(context, PropertyUtil.getSchemaProperty(context, strProductType), strModelName, null, null, null, strReltoConnect,
                            domModelObj, true);
                    ...
                    if (strCompanyId != null) {
                        RelationshipType relType = new RelationshipType(ProductLineConstants.RELATIONSHIP_COMPANY_PRODUCT);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - Holds the following parameters emxTableRowId - table row id parentOID - parent object id
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.0.0.0
     * @grade 0
    ...
    public void addExistingCandidateRequirements(Context context, String[] args) throws Exception {
        try {
            ...
            for (int a = 0; a < strNewObjIdArr.length; a++) {
                ...
                short sh = 1;
                MapList relBusObjPageList = dom.getRelatedObjects(context, strComittedRelationshipName, strType, ObjectSelectsList, null, true, true, sh, strObjectWhere, "");
    ...
    public void addExistingCandidateFeatures(Context context, String[] args) throws Exception {
        try {
            ...
            for (int a = 0; a < strNewObjIdArr.length; a++) {
                ...
                strNewObjId = strNewObjIdArr[a];
                // String is initialized to store the value of relationship name
                String strRelationshipType = ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "," + ProductLineConstants.RELATIONSHIP_COMMITED_ITEM;
                ...
                MapList relBusObjPageList = dom.getRelatedObjects(context, strRelationshipType, "*", objectSelects, null, false, true, (short) 2, "", "");
                int iNoOfObjects = relBusObjPageList.size();
     ...
     * @grade 0
     */
    protected void modelConnect(Context context, String strFromObjectId, String strToObjectId, String strRelationshipName) throws FrameworkException, Exception {
        ...
        DomainRelationship.connect(context, fromObject, strRelationshipName, toObject);
    }
     ...
     *            - Holds parent object id
     * @param strRelationshipName
     *            - Holds the relationship name
     * @param strNewObjectId
     *            - Holds new Productline id
     * @param bIsFrom
     *            - if the Productline is at the to side or the from side of the relationship RELATIONSHIP_PRODUCTLINE_MODELS
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.0.0.0
     * @grade 0
    ...
    /**
     * This method gets the object Structure List for the context Model object.This method gets invoked by settings in the command which displays the Structure Navigator for Model type objects
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing one Map entry for the key "parameterMap" This Map contains the objectId
     * @return MapList
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing one Map entry for the key "parameterMap" This Map contains the object Id.
...

    {
        ...
        relBusObjPageList = modelObj.getRelatedObjects(context, strRelPattern, strTypePattern, false, true, 2, objectSelects, null, null, DomainConstants.EMPTY_STRING, null, strPostTypePattern, null);
     ...
     */
    public int getNextUnitNumber(Context context, String arg) throws Exception {
     ...
     * @since Product Line X+4
     */
    public int getLastUnitNumber(Context context, String arg) throws Exception {
        String strModelId = arg;
        DomainObject objModel = newInstance(context, arg);
        String strCurrentLastUnitNumber = objModel.getAttributeValue(context, ProductLineConstants.ATTRIBUTE_LAST_BUILD_UNIT_NUMBER);
        int iLastUnitNumber = Integer.parseInt(strCurrentLastUnitNumber);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the Model Id, Updated Last Unit Number
     * @return void
     * @throws Exception
     *             if the operation fails
     * @since Product Line X+4
     */
    public void setLastUnitNumber(Context context, String[] args) throws Exception {
        String strModelId = args[0];
    ...
    public MapList getMasterFeatures(Context context, String[] args) throws Exception {
        ...
        String strExpandSelect = "from[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to.from[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "]";
        ...
        selectStmts.addElement("from[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + ProductLineConstants.SELECT_ID);
        selectStmts.addElement("from[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].id");
        selectStmts.addElement(strExpandSelect);
        MapList featureList = domProductPlatformObject.getRelatedObjects(context, strRelType, ProductLineConstants.QUERY_WILDCARD, false, true, 0, selectStmts, null, null, null, null, null, null);
        ...
        while (itrFeatures.hasNext()) {
            ...
            String objFeatureId = (String) hsFeatures.get("from[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].to." + ProductLineConstants.SELECT_ID);
            String strFeatureList = (String) hsFeatures.get(ProductLineConstants.SELECT_ID);
            String strFeatureListToId = (String) hsFeatures.get("from[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].id");
            HashMap hmpFeature = new HashMap();
            hmpFeature.put("level", strLevelKey);

            hmpFeature.put(ProductLineConstants.SELECT_ID, objFeatureId);
            hmpFeature.put(ProductLineConstants.SELECT_RELATIONSHIP_ID, strFeatureListToId);
    ...
    /**
     * Added for Bug 369959 This method is used to control the access levels for Model Creation
     * @param context
     ...
     */
    public static boolean checkProductLineState(Context context, String[] args) throws Exception {
        String strPLState = "";
    ...
    /**
     * This is a trigger method used to update the Revision Count attribute on the Model Object, when the Product Revision object is connected to Model with Products relationship. This method is
     * called on the create trigger on "Products" relationship.
     * @param Context
     *            context - Matrix Context Object
     * @param String
     *            args - holds the FromID on the relationship which is Model.
     * @return int iResult - 0 if operation is successful - 1 if operation fails.
     * @throws Exception
     * @author IVU
     * @since PLC R209
     */
    public int updateRevisionCountOnModel(Context context, String args[]) throws Exception {
        int iResult = 0;
        try {
            String strModelId = args[0];
    ...
    /**
     * This is a trigger method used to update the Revision Count attribute on the Model Object, when the Product object is revised and connected to Model with Products relationship. This method is
     * called on the ModifyTo trigger on "Products" relationship.
     * @param Context
     *            context - Matrix Context Object
     * @param String
     *            args - holds the FromID on the relationship which is Model.
     ...
     * @since PLC R209
     */
    public int updateRevisionCountOnModelModifyTo(Context context, String args[]) throws Exception {
        int iResult = 0;
        try {
            String strModelId = args[0];
            iResult = updateRevisionCount(context, strModelId, ProductLineConstants.RELATIONSHIP_PRODUCTS);
     ...
     *            args - holds the FromID on the relationship which is Model.
     * @param String
     *            strRelName - relationship name to find the number of objects connected through this relationship.
     * @return int iResult - 0 if operation is successful - 1 if operation fails.
     * @throws Exception
     * @author IVU
     * @since PLC R209
     */
    private int updateRevisionCount(Context context, String strObjId, String strRelName) throws Exception {
        int bResult = 0;
        try {
    ...
    public StringList getCommittedProductForFeature(Context context, String[] args) throws Exception {
        ...
        if (!objectList.isEmpty()) {
            for (int j = 0; j < objectList.size(); j++) {
                ...
                objProductName = tempMap.get("to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from" + ".to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.name");
                objProductRevision = tempMap
                        .get("to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from" + ".to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.revision");
                objProductType = tempMap.get("to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_TO + "].from" + ".to[" + ProductLineConstants.RELATIONSHIP_FEATURE_LIST_FROM + "].from.type");
     ...
     * @param args
     *            contains- -- programMap -> requestMap -> String objectId/ String UIContext -- programMap -> fieldMap -> String name
     * @return bResult true: Field will be displayed when the create Model is from Global Actions false: Field will not be displayed when the create Model is from Product Line Context
     * @throws FrameworkException
     */
    public boolean showProductLineChooser(Context context, String[] args) throws FrameworkException {
     ...
     * @param args
     *            contains- -- programMap -> requestMap -> String objectId/ String UIContext -- programMap -> fieldMap -> String name
     * @return bResult true: Field will be displayed when the create Model is from Product Line Context false: Field will not be displayed when the create Model is from Global Actions
     * @throws FrameworkException
     */
    public Object noShowProductLineChooser(Context context, String[] args) throws FrameworkException {
        boolean bResult = false;
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public void copyModelAttributesToProduct(Context context, String[] args) throws Exception {
        ...
        Map mpProd = domModel.getRelatedObject(context, ProductLineConstants.RELATIONSHIP_PRODUCTS, true, new StringList(DomainConstants.SELECT_ID), new StringList(DomainRelationship.SELECT_ID));
        ...
        if (mpProd != null) {
            ...
            MapList mLstModelRelatedData = domModel.getRelatedObjects(context, strRels.toString(), "*", new StringList(DomainObject.SELECT_ID), strRelSelect, true, true, (short) 1,
    ...
    /**
     * This method is used to retrieve all the Products connected to the Model on the Main Derivation Chain. Used to get data for the Summary table Model Context.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * This expand method is used to get the Objects derived from the ObjectId, called when a user expands a node on the Products Derivations SB from the MODEL or PRODUCT.
     * @param context
    ...
    public MapList expandProductDerivationStructure(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            mlDerivationStructure = DerivationUtil.getDerivationStructure(context, objectID, ProductLineConstants.TYPE_PRODUCTS, firstLevel, level, limit);
    ...
    /**
     * Returns product's derivation structure. It is called when a user expands a node on the Products Derivations SB from the MODEL or PRODUCT.
     * @param context
     *            The ematrix context of the request.
     * @param args
     *            string array containing packed arguments.
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getEffProdDerivationChain(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public MapList getAllMilestones(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mlMilestoneTrack = domModel.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_CONFIGURATION_CRITERIA, strTypePattern, objectSelects, relationshipSelects, false, // to
                    // relationship
            ...
            for (int m = 0; m < mlMilestoneTrack.size(); m++) {
                ...
                MapList mlResult = domMilestoneTrack.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_CONFIGURATION_ITEM, strtypePattern, objectSelects1, relationshipSelects1, false, // to
                        // relationship
                        // -
                        // changed
                        true, // from relationship
    ...
    public MapList getAllFeatureOption(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mlProducts = domModel.getRelatedObjects(context, strRel, DomainConstants.QUERY_WILDCARD, objectSelects, relationshipSelects, false, // to relationship
                    true, // from relationship
                    (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);
            ...
            for (int m = 0; m < mlProducts.size(); m++) {
                ...
                String busWhereClause = "";
                // Project Milestones connected with Model Level Milestones Track object
                MapList mlResult = domProduct.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_CONFIGURATION_FEATURES, DomainConstants.QUERY_WILDCARD, objectSelects1, relationshipSelects1,
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: modelId the id of the model milestone tracks attached to disciplinesList the list of milestone tracks' discipline to retrieve(e.g. Engineering,
     *            Manufacturing) If empty string or null is provided, all milestone tracks will be retrieved busSelects the business selects of milestone objects to return If not provided, only
     *            milestones' id will be returned relSelects the relationship selects of milestone objects to return If not provided, no information on relationships returned bInclMilestoneTrack If
     *            "true", milestone tracks will be included in returned MapList
    ...
    /**
     * This method will be executed through PLC only when UNT is not installed. This method creates the build for specified Model and connects the same with model
     * @param context
     *            - Matrix context
     * @param args
     ...
     */
    @com.matrixone.apps.framework.ui.PostProcessCallable
    public void createMultipleBuildsForModel(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            HashMap requestMap = (HashMap) programMap.get("requestMap");
    ...
    /**
     * This method will be executed through PLC only when UNT is not installed. This method connects the Build to a Model under which it is created.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public void connectToModel(Context context, String[] args) throws Exception {
        try {
            ...
            if (strProductOID != null && strProductOID.length() > 0) {
                ...
                strModelID = domPorduct.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_PRODUCTS + "].from.id");
                return;
            } else {
                ...
                if (strParentOID != null && strParentOID.length() > 0) {
                    ...
                    if (mxType.isOfParentType(context, domObj.getInfo(context, ProductLineConstants.SELECT_TYPE), ProductLineConstants.TYPE_MODEL)) {
            ...
            if ((strModelID != null) && !(strModelID.equals("")) || "Unassigned".equalsIgnoreCase(strModelID) || "null".equalsIgnoreCase(strModelID)) {

                setId(strModelID);
                DomainRelationship.connect(context, strModelID, ProductLineConstants.RELATIONSHIP_MODEL_BUILD, strbuildId, true);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * This method will be executed through PLC only when UNT is not installed. This method updates the Build Unit Number based on the Model it is connected to It is called as a create trigger when
     * the Model Build relationship is established
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public void assignBuildUnitNumberForModelContext(Context context, String[] args) throws Exception {
        try {
            ...
            String strModelID = (String) objRelMap.get(DomainRelationship.SELECT_FROM_ID);
            String strBuildID = (String) objRelMap.get(DomainRelationship.SELECT_TO_ID);
            if (strModelID == null || strModelID.length() == 0 || strBuildID == null || strBuildID.length() == 0) {
                return;
            }
            DomainObject objModel = newInstance(context, strModelID);
            if (!objModel.getPolicy(context).getName().equals(ProductLineConstants.POLICY_MODEL)) {
                return;
            }
            ...
            // return.
            if (ProductLineCommon.isNotNull(strUnitNumber) && !strUnitNumber.equalsIgnoreCase("0")) {
                return;
            }
    ...
    /**
     * This is the delete check trigger method on model to check whether context model has any product(s) associated with it.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds model's object ID
     * @return int 0 to proceed, 1 to block the deletion
     * @throws Exception
     *             if the operation fails
     */
    public int checkIfModelHasProducts(Context context, String args[]) throws FrameworkException {
        int result = 0;
        try {
            ...
            MapList mpObjectList = dmObj.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCTS, ProductLineConstants.TYPE_PRODUCTS, objectSelects, relSelects, false, true, (short) 0,
                    DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING, 0);

            for (int i = 0; i < mpObjectList.size(); i++) {
                strObjId = (String) (((Map) mpObjectList.get(i)).get(DomainObject.SELECT_ID));

                if (DomainObject.newInstance(context, strObjId).isKindOf(context, ProductLineConstants.TYPE_PRODUCTS)) {
                    result = 1;
                    break;
                }
            }
