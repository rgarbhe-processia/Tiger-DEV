        ...
        try {
            ...
            while (inputids.hasMoreElements()) {
                ...
                if (!MCADStringUtils.isNullOrEmpty(integrationSource)) {
                    ...
                    /*
                     * if(_util.isSolutionBasedEnvironment(context)){ if(latestMajorStateList.lastIndexOf(latestMajorState) >= latestMajorStateList.lastIndexOf(finalizationState)) { outputId =
                     * busObjectWithSelect.getSelectData("last.id"); // major id if(retrieveHashcode) outputIdHashcode = busObjectWithSelect.getSelectData("last."+SELECT_ATTR_IEFFILEMESSAGEDIGEST); }
                     * else { //outputId = busID; //[NDM] id is coming as major id. No need to find minor id outputId = busObjectWithSelect.getSelectData("last." + SELECT_ON_ACTIVE_MINOR + "id"); //
                     * minor id if(retrieveHashcode) outputIdHashcode = busObjectWithSelect.getSelectData("last." + SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST); } }else{ outputId =
                     * busObjectWithSelect.getSelectData("last.id"); // major id if(retrieveHashcode) outputIdHashcode = busObjectWithSelect.getSelectData("last."+SELECT_ATTR_IEFFILEMESSAGEDIGEST); }
                     */
                ...
                if (!isVersion)// _globalConfig.isMajorType(busType)) // {NDM] OP6
                {
                    ...
                    for (int j = (majorRevisions.size() - 1); j >= 0; j--) {
                        ...
                        if (latestMajorStateList.lastIndexOf(latestMajorState) >= latestMajorStateList.lastIndexOf(finalizationState)) {
                            ...
                            if (retrieveHashcode)
                                outputIdHashcode = busObjectWithSelect
                                        .getSelectData(SELECT_ON_MAJOR + "revisions[" + majorRevisions.elementAt(j) + "]." + SELECT_ON_ACTIVE_MINOR + SELECT_ATTR_IEFFILEMESSAGEDIGEST);
                        }
