...
/*
 * emxModelBasedUEBOMMigrationBase.java program migrates Existing PUEECO and its appicability attribute to NE object and set the expression
 *
...
public class emxModelBasedUEBOMMigrationBase_mxJPO extends emxCommonMigration_mxJPO {
     ...
     * @grade 0
     */
    public emxModelBasedUEBOMMigrationBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }

    /**
     * This method does the migration work. Existing PUEECO and its applicability attribute will migrated to NE object and the expression will be set for 2011x env.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param objectIdList
    ...
    public void migrateObjects(Context context, StringList objectIdList) throws Exception {
        ...
        try {
            ...
            String relationship = UnresolvedEBOMConstants.RELATIONSHIP_EBOM + "," + RELATIONSHIP_EBOMPENDING;
            String objectId;
            ...
            for (int i = 0; i < listSize; i++) {
                ...
                listEBOMAndEBOMPendingData = domObj.getRelatedObjects(context, relationship, DomainConstants.QUERY_WILDCARD, null, relSelect, false, true, (short) 1, null, null, null, null, null);
                ...
                while (iterator.hasNext()) {
                    ...
                    String relInfo = MqlUtil.mqlCommand(context, "print connection $1 select $2 $3 $4 $5 $6 $7 $8 $9 $10", relId, "name", "from", "to", "attribute.value", "interface",
                            "tomid[EBOM Change].id", "tomid[EBOM Change].attribute.value", "frommid.to.id", "frommid.to.attribute.value");

                    isContextChangeCase = MqlUtil.mqlCommand(context, "print connection " + relId + " select frommid[" + RELATIONSHIP_NAMED_EFFECTIVITY_USAGE + "] dump");
            ...
            if (allEBOMRelIds.size() > 0) {
                _effectivityFramework.refreshCurrentRelEffectivity(context, allEBOMRelIds);
            }
            ...
            if (allEBOMOrEBOMPendingRelIds.size() > 0) {
                ...
                for (; idx < allEBOMOrEBOMPendingRelIds.size(); idx++) {
                    EffectivityFramework.setRelEffectivityTypes(context, (String) ((Map) allEBOMOrEBOMPendingRelIds.get(idx)).get(DomainConstants.SELECT_RELATIONSHIP_ID), "Unit");
                }
     ...
     * @param context
     *            eMatrix code context objet
     * @param wipBOMIdList
     *            StringList of rel ids whose undergone wip bom
     * @throws Exception
     *             if the operation fails
     */
    private void doWipBOMMigration(Context context, StringList wipBOMIdList) throws Exception {
        ...
        for (int idx = 0; idx < wipBOMIdList.size(); idx++) {
            ...
            String InvalidExpression = "EFF_INVALID";
            if (!actualExpr.equals(InvalidExpression) && !actualExpr.equals("")) {
                ...
                while (_itr.hasNext()) {
                    ...
                    if (modelAppMap.containsKey(_modelPhyId)) {
                        _applValue = (String) JPO.invoke(context, "emxModelBasedPUEMigrationBase", null, "getUnionOfApplicabilities", new String[] { modelAppMap.get(_modelPhyId), _applValue },
                                String.class);
                    }
                ...
                if (!modelExpr.equals("") && modelExpr.length() > 0) {
    ...
    private String getModelBasedExpression(HashMap modelAppMap) throws Exception {
        ...
        while (_itr.hasNext()) {
            ...
            exprBuffer.append(KEY_PREFIX).append(_modelPhyId).append("[").append(_applValue).append(KEY_SUFFIX);
        }
