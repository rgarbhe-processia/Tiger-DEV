...
/**
 * The <code>ChangeBinaryUpgradeMigrationFindObjectsBase</code> class contains implementation code for ChangeBinaryUpgradeMigrationFindObjectsBase.
...
public class ChangeBinaryUpgradeMigrationFindObjectsBase_mxJPO extends emxCommonFindObjects_mxJPO {
     ...
     * @since CFF V6R2013x
     */
    public ChangeBinaryUpgradeMigrationFindObjectsBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public void getIds(Context context, int chunkSize) throws Exception {
        ...
        try {
            if (!isType) {
                result = MqlUtil.mqlCommand(context, "query connection relationship $1 limit $2 where $3", type, "1",
                        "program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true ");
            } else {
                result = MqlUtil.mqlCommand(context, "temp query bus $1 $2 $3 vault $4 limit $5 where $6", type, name, revision, vaultList, "1",
                        "program[" + migrationProgramName + " -method writeOID ${OBJECTID} \"${TYPE}\"] == true ");
            }
        } catch (Exception me) {
            throw me;
