...
public class emxPLCCommonBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public static final String STR_DEFAULT_INTERMEDIATE_TYPES = ProductLineConstants.TYPE_GBOM + STR_COMMA + ProductLineConstants.TYPE_FEATURE_LIST;
    ...
    public static final String STR_INTERMEDIATE_FILTER_TYPE_NAMES = "IntermediateFilterTypes";
    ...
    public static final String NEW_DERIVATION_ICON = "<img src=\"../common/images/iconSmallHigherRevision.gif\" border=\"0\"  align=\"middle\">";
    ...
    public int mxMain(Context context, String[] args) throws Exception {
        if (!context.isConnected()) {
            String strContentLabel = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Error.UnsupportedClient", context.getSession().getLanguage());
    ...
    /**
     * Get the list of component objects for the object type passed in to be shown in Structured Navigator expansion.The returned MapList will have component objects of the object type corresponding
     * to the node expanded in the Structure Navigator. This utility method will get invoked by getStructureList method in Product Central type specific JPOs when the Structured Navigator is expanding
     * a node of object type other than the type represented by the JPO. Product Central type JPOs should invoke this method only when they have the method 'getStructureList'. Only Product Central
     * types whose tree menus have Structure Menu setting will invoke this method For e.g. emxProductBase JPO's getStructureList method handles type Product Structure Navigator expansion when a
     * Product object is opened from Product Summary page. In the same Product Structure Navigator, if object node of different type, e.g. Product Configuration is expanded, the getStructureList
     * method in emxProductBase JPO redirects the call to this method. The MapList returned by this method is used to display Product Configuration object expansion in the Structure Navigator.
     * @param context
     *            the eMatrix Context object.
     * @param args
     *            contains a Map with the following entries: paramMap - Map having object Id String requestMap - Map having request parameter values
     * @return MapList with component objects of type expanded in Structure Navigator
     * @throws Exception
     *             if the operation fails
     */

    public static MapList getStructureListForType(Context context, String[] args) throws Exception {
    ...
    /**
     * This method returns the Product Central JPO corresponding to the PLC admin type passed in. Every top level Product Central admin type has a JPO Base Wrapper pair like emxProduct, emxProductBase
     * for type Products The Product Central type JPO mapping is retrieved from Properties file e.g. emxProduct is returned for passed in objectType 'Products'
     * @param context
     *            the eMatrix Context object.
     * @param objectType
     *            the Product Central admin type for which JPO program name to be returned
     ...
     *            the eMatrix Context object.
     * @param type
     *            the admin type for which top level parent is being queried
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            is a string array containing a HashMap which in turn holds the following objects: programMap - HashMap containing the Object Id and symbolicname of the command which invoked the
     *            method
    ...
    public static MapList getWhereUsed(Context context, String[] args) throws Exception {
        ...
        if (!((strCommandSymbolicName == null) || (STR_BLANK.equals(strCommandSymbolicName)) || (STR_NULL.equals(strCommandSymbolicName)))) {
        ...
        if (!((strCommandName == null) || (STR_BLANK.equals(strCommandName)) || (STR_NULL.equals(strCommandName)))) {
            ...
            strIntermediateTypeNamesKey = (String) settingsMap.get(STR_INTERMEDIATE_FILTER_TYPE_NAMES);
        }
        /*
         * if strTypeNamesKey is not null get the type pattern otherwise set it to a wildcard value
         */
        if ((strTypeNamesKey != null) && (!STR_BLANK.equals(strTypeNamesKey) && (!STR_NULL.equals(strTypeNamesKey))))
            strTypePattern = getPattern(context, strTypeNamesKey, DomainConstants.QUERY_WILDCARD);
        else
            strTypePattern = DomainConstants.QUERY_WILDCARD;

        /*
         * if strIntermediateTypeNamesKey is not null get the comma separated string containing intermediate type names otherwise set it to a Default value
         */

        if ((strIntermediateTypeNamesKey != null) && (!STR_BLANK.equals(strIntermediateTypeNamesKey) && (!STR_NULL.equals(strIntermediateTypeNamesKey))))
            strIntermediateTypes = getPattern(context, strIntermediateTypeNamesKey, STR_DEFAULT_INTERMEDIATE_TYPES);
        else
            strIntermediateTypes = STR_DEFAULT_INTERMEDIATE_TYPES;
        /*
         * if strRelNamesKey is not null get the relationship pattern otherwise set it to a wildcard value
         */

        if ((strRelNamesKey != null) && (!STR_BLANK.equals(strRelNamesKey) && (!STR_NULL.equals(strRelNamesKey)))) {
            strRelPattern = getPattern(context, strRelNamesKey, DomainConstants.QUERY_WILDCARD);
        ...
        /*
         * Add the names of relationship for Intermediate type to the strRelPattern
         */
        if (!DomainConstants.QUERY_WILDCARD.equalsIgnoreCase(strRelPattern)) {
            ...
            sbRelPattern.append(getIntermediateRelationships(context, strIntermediateTypes));
        ...
        relObjIdList = domainObject.getRelatedObjects(context, strRelPattern, strTypePattern, ObjectSelectsList, RelSelectsList, true, false, sRecurseLevel, DomainConstants.EMPTY_STRING,
        ...
        {
            /*
             * Calling the setLevelsInMapList() method which modifies the maplist (relObjIdList) returned by the method getRelatedObjects().It loops through the maplist and searches for the
             * intermediate objects ,if it finds one it decrement the "level" of the corresponding object(one connected through the intermediate object)by 1.
             * 
             * This manipulation is needed becuase intermediate objects are supposedly transparent to the user.
             */
        ...
        /*
         * The code below fetches the to side objects(till level 1) which are connected to the context object through intermediate objects
         */
        ...
        if (!strRestrictedTypeList.contains(strContextObjType)) {
            /*
             * Calling the getRelatedObjects() method of DomainObject which Returns the Maplist of Object Ids
             */
            ...
            if (!(strInterRel == null || strInterRel.equalsIgnoreCase("null") || strInterRel.equals(""))) {
                mlToSideObjsList = domainObject.getRelatedObjects(context, strInterRel, strTypePattern, (StringList) slObjSelListCopy, RelSelectsList, false, true, (short) 1,
        ...
        for (int i = 0; i < mlToSideObjsList.size(); i++) {
            ...
            for (int j = 0; j < slObjSelList.size(); j++) {
                if (((String) slObjSelList.get(j)).equalsIgnoreCase(sbBCRParentIDFromLE.toString()) || ((String) slObjSelList.get(j)).equalsIgnoreCase(sbBCRParentIDFromRE.toString())) {
                    continue;
                }
    ...
    public static Vector getEditIcon(Context context, String[] args) throws Exception {
        ...
        String strIssueType = PropertyUtil.getSchemaProperty(context, "type_Issue");
        String strTypeBCR = PropertyUtil.getSchemaProperty(context, "type_BooleanCompatibilityRule");
        String strTypePCR = PropertyUtil.getSchemaProperty(context, "type_ProductCompatibilityRule");
        String strTypeInclusionRule = PropertyUtil.getSchemaProperty(context, "type_InclusionRule");
        ...
        while (objectListItr.hasNext()) {
            ...
            strBaseType = FrameworkUtil.getBaseType(context, strType, context.getVault());

            /*
             * checks whether the context user has remove access for each of the selected
             */
            if (FrameworkUtil.hasAccess(context, dObj, "modify")) {
                ...
                if (!(strReportFormat != null && strReportFormat.equals("null") == false && strReportFormat.equals("") == false)) {
                    if (strType.equalsIgnoreCase(strTypeBCR) || strType.equalsIgnoreCase(strTypePCR)) {
                        /* Begin of Modify:Ramandeep,Enovia MatrixOne Bug#300035 3/11/2005 */
                        if (strBCRParentId == null || strBCRParentId.equalsIgnoreCase(STR_BLANK) || strBCRParentId.equalsIgnoreCase(STR_NULL))
                        /* End of Modify:Ramandeep,Enovia MatrixOne Bug#300035 3/11/2005 */
                        {
                        ...
                        if (strFeatureObjId != null && !strFeatureObjId.equals(STR_BLANK) && !strFeatureObjId.equalsIgnoreCase(STR_NULL)) {
                            sbHref.append("&objectId=");
                            sbHref.append(strFeatureObjId);
                        } else if (strGBOMObjId != null && !strGBOMObjId.equals(STR_BLANK) && !strGBOMObjId.equalsIgnoreCase(STR_NULL)) {
                            sbHref.append("&objectId=");
                        ...
                        if (strFeatureIRParentId != null && !strFeatureIRParentId.equals(STR_BLANK) && !strFeatureIRParentId.equalsIgnoreCase(STR_NULL)) {
                            sbHref.append("&parentOID=");
                            sbHref.append(strFeatureIRParentId);
                        } else if (strGBOMIRParentId != null && !strGBOMIRParentId.equals(STR_BLANK) && !strGBOMIRParentId.equalsIgnoreCase(STR_NULL)) {
                            sbHref.append("&parentOID=");
                        ...
                        if (strFeatureRuleTargetId != null && !strFeatureRuleTargetId.equals(STR_BLANK) && !strFeatureRuleTargetId.equalsIgnoreCase(STR_NULL)) {
                            sbHref.append("&relId=");
                            sbHref.append(strFeatureRuleTargetId);
                        } else if (strGBOMRuleTargetId != null && !strGBOMRuleTargetId.equals(STR_BLANK) && !strGBOMRuleTargetId.equalsIgnoreCase(STR_NULL)) {
                            sbHref.append("&relId=");
                        ...
                        strSymbolicTypeName = FrameworkUtil.getAliasForAdmin(context, SELECT_TYPE, strBaseType, true);

                        /*
                         * Here it is assumed that name of the webform for a type is same as type's symbolic name eg type_Products
                         */
                        ...
                        if (strFormName != null && !strFormName.equalsIgnoreCase(STR_BLANK) && !strFormName.equalsIgnoreCase(STR_NULL)) {
                            sbHref.delete(0, sbHref.length());
    ...
    /**
     * This method is used to get the level for all the objects which are associated with context object.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * This method is used to get the string of admin names from a property file corresponding to the key passed.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param String
     *            strKey - The key to be Searched
     * @param String
     *            strDefault - Value to be returned if key is missing
     * @return String
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static String getPattern(Context context, String strKey, String strDefault) throws Exception {
        ...
        if ((strSymbolicNames != null) && (!STR_BLANK.equals(strSymbolicNames) && (!STR_NULL.equals(strSymbolicNames))))
            strPattern = getActualNamesFromSymbolicNames(context, strSymbolicNames);

        if ((strPattern == null) || (STR_BLANK.equals(strPattern) || (STR_NULL.equals(strPattern))))
    ...
    /**
     * This method is used to get a comma separated string of the actual names for admin objects.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param String
     *            strSymbolicNamesList - a comma separated string of symbolic names
     * @return String
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static String getActualNamesFromSymbolicNames(Context context, String strSymbolicNamesList) throws Exception {
        int i = 0;
        String strActualNames = STR_BLANK;
        String strToken = STR_BLANK;
        StringBuffer sbActualNames = new StringBuffer();
        StringTokenizer stActualNames = new StringTokenizer(strSymbolicNamesList, STR_COMMA);
        while (stActualNames.hasMoreTokens()) {

            strToken = PropertyUtil.getSchemaProperty(context, stActualNames.nextToken());
            if (i > 0)
    ...
    /**
     * This method is used to get the string of Relationship for Intermediate types in the database.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param String
     *            strIntermediateTypePattern
     * @return String
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static String getIntermediateRelationships(Context context, String strIntermediateTypePattern) throws Exception {
        int i = 0;
        StringBuffer sbIntermediateRelPattern = new StringBuffer();
        StringTokenizer stIntermediateTypePattern = new StringTokenizer(strIntermediateTypePattern, STR_COMMA);
        ...
        while (stIntermediateTypePattern.hasMoreTokens()) {
            ...
            BusinessType busType = new BusinessType(strCurrentToken, context.getVault());

            /*
             * calling the method getRelationshipTypes(of class BusinessType) to get the list of all Relationship for the type
             */
            lstIntermediateRelationshipList = busType.getRelationshipTypes(context, true, true, false);
            // get the string from the list and append to stringbuffer
            if (i > 0)
                sbIntermediateRelPattern.append(STR_COMMA);
            sbIntermediateRelPattern.append(getStringFromList(lstIntermediateRelationshipList));
            i++;
        } // end while loop
     ...
     * @param List
     *            lstToBeConverted - list to be converted
     * @return String
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static String getStringFromList(List lstToBeConverted) throws Exception {
    ...
    /**
     * This method is used check whether a Particular exists within a string.
     * @param String
     *            strParent - The source string
     * @param String
     *            strPattern -Pattern to be searched
     * @return boolean
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static boolean boolContains(String strParent, String strPattern) throws Exception {
        boolean bContains = false;
        String strCurrentToken = STR_BLANK;
        StringTokenizer stParent = new StringTokenizer(strParent, STR_COMMA);
    ...
    /**
     * This method is used to set the level for all the objects in a MapList as seen by the user by ignoring the Intermediate Objects.
     * @param List
     *            relObjIdList - contains the parent MapList
     * @param String
     *            strIntermediateTypes - comma seperated string of intermediate types
     * @return List- The modified MapList
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static List setLevelsInMapList(List relObjIdList, String strIntermediateTypes) throws Exception {

        /*
         * This processing is dependent upon the structure of the MapList returned by the method getRelatedObjects() which searches for the connected objects in a depth first manner.
         */
        ...
        for (int index = 0; index < relObjIdList.size(); index++) {
            ...
            iLevelCurrent = Integer.parseInt((String) mpMapCurrent.get(DomainConstants.KEY_LEVEL));

            /*
             * if an intermediate object is encountered loop through the maplist and decrement by 1 ,the level in all the Maps having level greater than it
             */
            if (boolContains(strIntermediateTypes, strType)) {
                for (int i = index; i < relObjIdList.size() - 1; i++) {

                    mpMapNext = (Map) relObjIdList.get(i + 1);
                    iLevelNext = Integer.parseInt((String) mpMapNext.get(DomainConstants.KEY_LEVEL));
    ...
    /**
     * This method is used to remove the Intermediate objects from the MapList.
     * @param List
     *            objMapList - contains the MapList
     * @param String
     *            strIntermediateTypes - comma seperated string of intermediate types
     * @return List- The modified MapList
     * @throws Exception
     ...
     * @since ProductCentral 10.6
     **/
    public static List removeIntermediateObjects(List objMapList, String strIntermediateTypes) throws Exception {
        Map mpMapCurrent = null;
        String strType = STR_BLANK;
    ...
    /**
     * This method forms a Map containing stringlist "objectselects" based on Display Relationships and a Map "relName" containing corresponding objectselects as key and Relationship name as value
     * @param String
     *            strDisplayRelPattern - display Relationships
     * @return Map
     * @throws Exception
     *             if the operation fails
     * @since ProductCentral 10.6
     **/
    public static Map getObjectsSelect(String strDisplayRelPattern) throws Exception {

        List slRelSelect = new StringList();
        Map tmpmap = new HashMap();
        StringBuffer sbTmp = new StringBuffer();
        StringTokenizer stRelPattern = new StringTokenizer(strDisplayRelPattern, STR_COMMA);
        String strNextToken = "";
    ...
    /**
     * This method is used to remove the duplicate objects from the MapList.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param List
     *            objMapList - contains the MapList
     ...
     * @since ProductCentral 10.6
     **/
    public static List removeDuplicateObjects(Context context, List objMapList) throws Exception {
        ...
        for (int i = 0; i < objMapList.size(); i++) {
            ...
            for (int j = i + 1; j < objMapList.size(); j++) {
                ...
                /*
                 * Remove duplicate objects if from side object ,to side object is same and one of the following condition is met: 1.relationship is same 2.relationship is LE or RE The second
                 * condition takes care of the scenerio when a Feature is used in rule in left exp as well as Right exp.
                 */

                if (strFromIdNext.equalsIgnoreCase(strFromIdCurrent) && strTypeNext.equalsIgnoreCase(strTypeCurrent) && strNameCurrent.equals(strNameNext)) {
        ...
        for (int jCount = 0; jCount < iNumOfObjects; jCount++) {
            ...
            if (hasHigherRevision) {
                if (reportFormat != null && "CSV".equalsIgnoreCase(reportFormat)) {
                    ...
                    strHigherRevisionIconTag = "<img src=\"../common/images/" + XSSUtil.encodeForHTMLAttribute(context, strIcon) + "\" border=\"0\"  align=\"middle\" " + "TITLE=\"" + " "
    ...
    public String getHigherRevisionIconProperty(Context context, String[] args) throws Exception {
        ...
        if (DerivationUtil.higherRevisionExists(context, strObjectId)) {
            ...
            if (ProductLineCommon.isNotNull(reportFormat)) {
                ...
                strHigherRevisionIconTag = "<a HREF=\"#\" TITLE=\"" + " " + XSSUtil.encodeForHTMLAttribute(context, strTooltipHigherRevExists) + "\">" + HIGHER_REVISION_ICON
                        + XSSUtil.encodeForXML(context, strYes) + "</a>";
            }
    ...
    public List getNewDerivationIcon(Context context, String[] args) throws Exception {
        ...
        // Reading the tooltip from property file.
        String strTooltipNewDerivationExists = EnoviaResourceBundle.getProperty(context, SUITE_KEY, ICON_TOOLTIP_NEW_DERIVATION_EXISTS, context.getSession().getLanguage());
        ...
        for (int jCount = 0; jCount < iNumOfObjects; jCount++) {
            ...
            if (newDerivationExists) {
                if (reportFormat != null && "CSV".equalsIgnoreCase(reportFormat)) {
                    ...
                    strNewDerivationIconTag = "<a HREF=\"#\" TITLE=\"" + " " + strTooltipNewDerivationExists + "\">" + NEW_DERIVATION_ICON + "</a>";
                }
            } else {
    ...
    public String getNewDerivationExistsProperty(Context context, String[] args) throws Exception {
        ...
        if (DerivationUtil.newDerivationExists(context, objectId)) {
            ...
            if (ProductLineCommon.isNotNull(reportFormat)) {
                ...
                strNewDerivationIconTag = "<a HREF=\"#\" TITLE=\"" + " " + XSSUtil.encodeForHTMLAttribute(context, strTooltipNewDerivationExists) + "\">" + NEW_DERIVATION_ICON
                        + XSSUtil.encodeForXML(context, strYes) + "</a>";
            }
    ...
    /**
     * This column JPO method is used to get the RDO for a object. If the context user has read access on the RDO object then it is hyperlinked to the properties page otherwise only name is returned.
     * Also if the context user doesn't have the show access on the RDo object then context is changed to super user to retrieve the RDO name.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public String getDesignResponsibility(Context context, String[] args) throws Exception {
        ...
        String strStateReview = FrameworkUtil.lookupStateName(context, strPolicyProduct, SYMB_state_Review);
        String strStateRelease = FrameworkUtil.lookupStateName(context, strPolicyProduct, SYMB_state_Release);
        String strStateObsolete = FrameworkUtil.lookupStateName(context, strPolicyProduct, SYMB_state_Obsolete);
        ...
        if (strMode != null && !strMode.equals("") && !strMode.equalsIgnoreCase("null") && strMode.equalsIgnoreCase("edit")) {
            ...
            try {
                if ((strRDOId == null || strRDOId.equals("")) && (bIsOwner || hasRoleProductManager || hasRoleSystemEngineer)) {
                    bHasReadAccess = true;
                } else {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            0 - HashMap containing one Map entry for the key PARAM_MAP This Map contains the arguments passed to the jsp which called this method.
    ...
    public int updateDesignResponsibility(Context context, String[] args) throws Exception {
        ...
        if (!strNewOrganizationOID.equals("")) {
            DomainObject domObjOrgnization = new DomainObject(strNewOrganizationOID);
            ...
            domObj.setPrimaryOwnership(context, defaultProj, strNewOrganizationName);
        } else {
    ...
    /**
     * This trigger method is used to check whether a valid Engineering Change object in complete state is connected to the object by Implemented or Affected Item relationship when it is promoted to
     * release or obsolete state. .
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            0 - The id of the object. 1 - The next state of the object (Release/Obsolete).
     * @return int - an integer (0) if the operation is successful
     * @throws Exception
     *             if operation fails
     * @since ProductCentral 10-6
     */
    public int checkConnectedEC(Context context, String[] args) throws Exception {
        ...
        String strPolicyEC = PropertyUtil.getSchemaProperty(context, SYMBOLIC_policy_EngineeringChange);
        String strStateComplete = FrameworkUtil.lookupStateName(context, strPolicyEC, SYMB_state_Complete);
        ...
        if (strNextState.equals(strStateRelease)) {
            lstECList = (MapList) domItem.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EC_IMPLEMENTED_ITEM, DomainConstants.QUERY_WILDCARD, (StringList) lstObjectSelects, null, true, false,
            ...
            if (lstECList != null && !lstECList.isEmpty()) {
                ...
                // End of Add by Vibhu,Enovia MatrixOne for Bug 303269 on 28 April 05
                lstECList = (MapList) domItem.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EC_AFFECTED_ITEM, DomainConstants.QUERY_WILDCARD, (StringList) lstObjectSelects, null, true,
                        false, (short) 1, sbObjWhereExpression.toString(), sbRelWhereExpression.toString());
                if (lstECList != null && !lstECList.isEmpty()) {
                    return 0;
                } else {
        ...
        // Relationship for which Requested Change attribute is For Obsolescence
            ...
            sbRelWhereExpression.append("\"For Obsolescence\"");
            lstECList = (MapList) domItem.getRelatedObjects(context, DomainConstants.RELATIONSHIP_EC_AFFECTED_ITEM, DomainConstants.QUERY_WILDCARD, (StringList) lstObjectSelects, null, true, false,
                    (short) 1, sbObjWhereExpression.toString(), sbRelWhereExpression.toString());
            if (lstECList != null && !lstECList.isEmpty()) {
                return 0;
            } else {
    ...
    /**
     * This method is used to remove a particular branch of maps which have levels higher than specified Index object. The method removed the map until it encounters a map of lower or equal level than
     * the indexed Object.
     * @param MapList
     *            - filteredMapList in which the maps are removed.
     * @param Integer
     *            - Index of the base object which is taken as reference, all objects are compared with the indexed object.
    ...
    public List getStatusIcon(Context context, String[] args) throws Exception {
        ...
        while (objectListItr.hasNext()) {
            ...
            if (flag) {
                ...
                if (strReportFormat == null || (strReportFormat != null && strReportFormat.equalsIgnoreCase(""))) // if(strReportFormat == null || strReportFormat == "") else , Added For Fixing
                                                                                                                  // IR-228607V6R2014x
                {
                    stbNameRev.delete(0, stbNameRev.length());
                    stbNameRev = stbNameRev.append("<img src=\"../common/images/").append(strIcon).append("\" border=\"0\"  align=\"middle\" ").append("TITLE=\"").append(" ").append(strObjState)
                            .append("\"").append("/>");
                    lstNameRev.add(stbNameRev.toString());
                } else {
    ...
    /**
     * This method is used to remove the duplicate values in a String having various elements separated by some delimiting character
     * @param String
     *            to be investigated for duplicate entries
     * @param delimiting
     *            character
    ...
    public StringList excludeConnected(Context context, String[] args) throws Exception {
        ...
        MapList childObjects = domObjFeature.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, strRelationship), toType == null ? fromType : toType,
                new StringList(DomainConstants.SELECT_ID), null, bisTo, bisFrom, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
     ...
     * @param String
     *            [] The args .
     * @return Boolean Object - Returns true if Variant Configuration is Installed. - Returns false if Variant Configuration is not Installed.
    ...
    public String getLifecycleStates(Context context, String[] args) throws Exception {
        ...
        output.append("var iFrameSrc=\' <iframe name=\"Lifecycle\" src=\"emxLifecycleDialog.jsp?objectId=" + objID
                + "\" width=\"100%\" height=\"100\" marginHeight=\"0\" scrolling=\"auto\" frameborder=\"0\"> </iframe>\';");
    ...
    /**
     * To obtain the list of Object IDs to be excluded from the search for Add Existing of Builds under Product Configuration context
     * @param context
     *            - the eMatrix <code>Context</code> object
     * @param args
     *            - holds the HashMap containing the following arguments
    ...
    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
    public StringList filterRelatedBuilds(Context context, String[] args) throws Exception {
        try {
            ...
            String strTypeOfObject = domObject.getInfo(context, DomainConstants.SELECT_TYPE);
            ...
            MapList lstBuildList = DomainObject.findObjects(context, ProductLineConstants.TYPE_BUILDS, DomainConstants.QUERY_WILDCARD, "", objSelect);
            ...
            if (!(mxType.isOfParentType(context, strTypeOfObject, ProductLineConstants.TYPE_PRODUCTS))) {
                // If context is PC
                strParentId = domObject.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_PRODUCT_CONFIGURATION + "].from.id");
                ...
                String strParentModelID = domParentProduct.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_PRODUCTS + "].from.id");

                if (strParentModelID == null || "".equals(strParentModelID)) {
                    strParentModelID = domParentProduct.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT + "].from.id");
                }
                ...
                for (int i = 0; i < tempStrList2.size(); i++) {
                    ...
                    if (!showForProdConfig(context, strBuildId1, strParentModelID, strParentId)) {
                ...
                if (EnoviaResourceBundle.getProperty(context, "eServiceSuiteProductLine.ProductBuildAddExisting.DisplayOrphanBuilds").equals("false")) {
                    return new StringList();
                } else {
                    String strParentModelID = domObject.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_PRODUCTS + "].from.id");

                    if (strParentModelID == null || "".equals(strParentModelID)) {
                        strParentModelID = domObject.getInfo(context, "to[" + ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT + "].from.id");
                    }
    ...
    private static StringList expandAndGetBuilds(Context context, String strBuildId, StringList processedList) throws Exception {
        ...
        MapList list = domBuild.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_INTENDED_PRODUCT_UNIT + "," + ProductLineConstants.RELATIONSHIP_UNITBOM, // relationship pattern
     ...
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param strProdModelInfo
     *            contains model Id of the selected product.
     * @param strBuildId
     *            contains Build Id
     * @param strProductId
     *            contains product id of the product configuration.
     * @return boolean.
     * @since ProductLine X5
     * @throws Exception
     *             if the operation fails.
     */
    private boolean showForProdConfig(Context context, String strBuildId, String strProdModelInfo, String strProductId) {

        try {
            DomainObject objBuild = new DomainObject(strBuildId);
            StringList selectables = new StringList(2);
            String strBuildProdIdSelectable = "to[" + ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD + "].from.id";
            String strBuildModelIdSelectable = "to[" + ProductLineConstants.RELATIONSHIP_MODEL_BUILD + "].from.id";
            ...
            if (strBuildModelInfo == null && strBuildProdId == null) {
                return true;
            } else if (strBuildModelInfo.equals(strProdModelInfo) && (strBuildProdId == null || strBuildProdId.equals(strProductId))) {
                return true;
            }
            return false;
        } catch (Exception e) {
     ...
     *             if the operation fails.
     */
    private boolean showBuildsInProductContext(Context context, String strProdModelInfo, String strBuildId) {
        try {
            DomainObject objBuild = new DomainObject(strBuildId);
            StringList selectables = new StringList(2);
            selectables.add("to[" + ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD + "].from.id");
            selectables.add("to[" + ProductLineConstants.RELATIONSHIP_MODEL_BUILD + "].from.id");
            Map map = objBuild.getInfo(context, selectables);
            String strProdInfo = (String) map.get("to[" + ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD + "].from.id");
            String strBuildModelInfo = (String) map.get("to[" + ProductLineConstants.RELATIONSHIP_MODEL_BUILD + "].from.id");

            if ((strProdInfo == null || strProdInfo.length() == 0) && (strBuildModelInfo == null || strBuildModelInfo.length() == 0)) {
                return true;
            } else if (strProdModelInfo.equals(strBuildModelInfo) && (strProdInfo == null || strProdInfo.length() == 0)) {
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    public StringList excludeProductRevisionConnected(Context context, String[] args) throws Exception {
        ...
        MapList childObjects = domObjModel.getRelatedObjects(context, PropertyUtil.getSchemaProperty(context, strRelationship), toType == null ? fromType : toType,
                new StringList(DomainConstants.SELECT_ID), null, bisTo, bisFrom, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
        // Get all the Products from the database
        MapList lstProductsList = DomainObject.findObjects(context, ProductLineConstants.TYPE_PRODUCTS, DomainConstants.QUERY_WILDCARD, "", objSelect);

        /*
         * for(int cnt=0;cnt<lstProductsList.size();cnt++){
         * 
         * Map prodMap = (Map)lstProductsList.get(cnt); String prodId = (String)prodMap.get(DomainConstants.SELECT_ID);
         * 
         * for(int i=0;i<childObjects.size();i++){
         * 
         * Map tempMap=(Map)childObjects.get(i); String tempID = (String)tempMap.get(DomainConstants.SELECT_ID); if(!tempID.equals(prodId)){ excludeList.add(prodId); }
         * 
         * } }
         */
    ...
    public StringList excludeManagedModels(Context context, String[] args) throws Exception {
        ...
        // Get all the Products from the database
        MapList lstProductsList = DomainObject.findObjects(context, ProductLineConstants.TYPE_MODEL, DomainConstants.QUERY_WILDCARD, "", objSelect);
        for (int cnt = 0; cnt < lstProductsList.size(); cnt++) {
     ...
     * @throws Exception
     */
    public int connectCompanyName(Context context, String[] args) throws Exception {
        Map programMap = (HashMap) JPO.unpackArgs(args);
        Map paramMap = (HashMap) programMap.get("paramMap");
        Map requestMap = (HashMap) programMap.get("requestMap");
        ...
        if (strNewCompanyId.equals("") && count != null && !count.equalsIgnoreCase("-1")) {
            String[] nCompanyOID = (String[]) requestMap.get("Company" + count);
        ...
        Map mDesignResponsibility = domObj.getRelatedObject(context, ProductLineConstants.RELATIONSHIP_COMPANY_PRODUCT_LINES, false, slBusTypes, null);

        if (!((mDesignResponsibility == null) || (mDesignResponsibility.equals(null)))) {
            String strDesRespObj = (mDesignResponsibility.get(DomainConstants.SELECT_ID)).toString();
        ...
        try {
            if (strOldCompanyId == null || strOldCompanyId.equals("null") || strOldCompanyId.equals(""))
                bHasReadAccess = true;
            else
                bHasReadAccess = emxProduct_mxJPO.hasAccessOnProject(context, strOldCompanyId);
        } catch (Exception e) {
            bHasReadAccess = false;
        }
        ...
        if (bHasReadAccess) {
            ...
            // Begin of add by Enovia MatrixOne on 18-Apr-05 for Bug# 300548
            if (!strNewCompanyId.equals("") && !strOldCompanyId.equals("") && strOldCompanyId.equals(strNewCompanyId)) {
                return 0;
            } else {
                // End of add by Enovia MatrixOne on 18-Apr-05 for Bug# 300548
                ...
                if (strOldCompanyName == null || "null".equals(strOldCompanyName))
                    strOldCompanyName = "";
                ...
                if (strOldCompanyId != null && !strOldCompanyId.equals("")) {
                    sbWhereCondition = sbWhereCondition.append(DomainConstants.SELECT_ID);
                    sbWhereCondition = sbWhereCondition.append("==");
                    sbWhereCondition = sbWhereCondition.append("\"");
                    sbWhereCondition = sbWhereCondition.append(strOldCompanyId);
                    sbWhereCondition = sbWhereCondition.append("\"");
                }
                ...
                organizationList = getRelatedObjects(context, strDesignRespRelationship, strOrganizationType, (StringList) lstObjectSelects, (StringList) lstRelSelects, true, true, (short) 1,
                        sbWhereCondition.toString(), DomainConstants.EMPTY_STRING);

                if (organizationList != null && !organizationList.isEmpty()) {
                    String strRelId = (String) ((Map) organizationList.get(0)).get(DomainConstants.SELECT_RELATIONSHIP_ID);
                ...
                if (strNewCompanyId == null || "null".equals(strNewCompanyId))
                    strNewCompanyId = "";

                if (!strNewCompanyId.equals("")) {
                    setId(strNewCompanyId);
                    DomainObject domainObjectToType = newInstance(context, strFeatureId);

                    // Added for RDO Fix
                    // Changing the context to super user
                    ContextUtil.pushContext(context);

                    if (mxType.isOfParentType(context, domainObjectToType.getInfo(context, DomainObject.SELECT_TYPE), ProductLineConstants.TYPE_PRODUCTS)) {
     ...
     */
    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
    public StringList excludeAvailableProductLine(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            relatedFromPlList = domContextObj.getRelatedObjects(context, strRelPattern, strObjectPattern, objectSelects, relSelects, false, true, level, strWhereExp, strWhereExp, limit);
     ...
     * @param context
     * @param args
     * @return true - if ECH is installed false - if ECH is not Installed
     * @exception throws
     *                FrameworkException
     * @since R213
     */
    ...
    /**
     * This column JPO method is used to get the RDO for a object. If the context user has read access on the RDO object then it is hyperlinked on SB page otherwise only name is returned. Also if the
     * context user doesn't have the show access on the RDo object then context is changed to super user to retrieve the RDO name. *
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            - String array containing following packed HashMap with following elements: lstobjectList - list containig the object id.
    ...
    public List getHigherRevisionIconForProductContext(Context context, String[] args) throws Exception {
        ...
        for (iCount = 0; iCount < iNumOfObjects; iCount++) {
            while (objectListItr.hasNext()) {
                ...
                if (nextRevExist != null && !("".equals(nextRevExist))) {
                    if (reportFormat != null && !("null".equalsIgnoreCase(reportFormat)) && reportFormat.length() > 0) {
                        ...
                        strHigherRevisionIconTag = "<img src=\"../common/images/" + strIcon + "\" border=\"0\"  align=\"middle\" " + "TITLE=\"" + " "
                                + XSSUtil.encodeForHTMLAttribute(context, strTooltipHigherRevExists) + "\"" + "/>";
                    }
    ...
    /**
     * The Utility method return true if context user has "ENO_FTR_TP" License, other wise return false
     * @param context
    ...
    /**
     * The Utility method return true if context user has "ENO_FTR_TP,ENO_CFE_TP" License, other wise return false
     * @param context
