...
public class emxCommonPartBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     * @grade 0
     */
    public emxCommonPartBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    public static final String RELATIONSHIP_TESTING_RESPONSIBILITY = PropertyUtil.getSchemaProperty("relationship_TestingResponsibility");
    ...
    public static final String RELATIONSHIP_SUPPLY_RESPONSIBILITY = PropertyUtil.getSchemaProperty("relationship_SupplyResponsibility");

    public static String SUPPLIER_RELATION_PATTERN = RELATIONSHIP_DESIGN_RESPONSIBILITY + "," + RELATIONSHIP_MANUFACTURING_RESPONSIBILITY + "," + RELATIONSHIP_TESTING_RESPONSIBILITY + ","
            + RELATIONSHIP_SUPPLY_RESPONSIBILITY;

    /** attribute "Show Sub Components". */
    public static final String SELECT_ATTRIBUTE_SHOW_SUB_COMPONENTS = "attribute[" + PropertyUtil.getSchemaProperty("attribute_ShowSubComponents") + "]";
    ...
    private void handlePartAddedRemovedSubscriptionEvent(Context context, String parentPartId, String childPartId, String eventType, boolean addPart) throws Exception {
        ...
        subMgr.publishEvent(context, eventType, parentPartId, messageKey + ".Subject", null, null, messageKey + ".MessageBody", messageBodyMacros, messageBodyMacroValues);
    }

    /**
     * Iterates through the objectIds passed as input arguments and checks if object has associated Reference Document and accordingly adds true/false to returning vector.
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap with the following entries: objectList - a MapList of objects. paramList - a HashMap of parameter values.
     * @return Vector of boolean values representing whether the given object has the Reference Document relationship.
     * @throws Exception
     *             if the operation fails.
     * @since VCP 11.0
     */
    public Vector hasReferenceDocumentRelationship(Context context, String[] args) throws Exception {
        // This method is moved from Engineering Central Part bean as Part Reference Documents is moved to commons
        try {
     ...
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds a packed HashMap of the following entries: objectId - the object id of the context part. selType - a String containing the Type to search for. txtName - a String containing the
     *            Name to search for. txtDesc - a String containing the Description to search for. txtOriginator - a String containing the Originator to search for. txtOwner - a String containing the
     *            Owner to search for. txtWhere - a String containing the where clause to use in the search. txtRev - a String containing the Revision to search for. revPattern - a String of either
     *            ALL_REVISIONS, HIGHEST_REVISION or HIGHEST_AND_PRESTATE_REVS Vault - a String containing the Vault name to search in. queryLimit - a String containing the Query Limit value.
    ...
    public MapList getReferenceDocumentsSearchResults(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            refDocList = DomainObject.findObjects(context, sType, sName, sRev, txtOwner, txtVault, sWhereExp, null, true, selectStmts, Short.parseShort(queryLimit));
    ...
    public Boolean isECInstalled(Context context, String[] args) throws Exception {
    ...
    public static com.matrixone.jdom.Document getPartMailXML(Context context, Map info) throws Exception {
        ...
        if ((null != altowner1) && (null != altowner2) && (!"".equals(altowner1)) && (!"".equals(altowner2))) {
        ...
        if (!eventName.equalsIgnoreCase(partDeletedEventName)) {
            if (messageType.equalsIgnoreCase("html")) {
                ...
                String viewLink = MessageUtil.getMessage(context, partId, "emxComponents.Object.Event.Html.Mail.ViewLink", messageValues, null, localeMessage, bundleName);
                ...
                String viewLink = MessageUtil.getMessage(context, partId, "emxComponents.Object.Event.Text.Mail.ViewLink", messageValues, null, localeMessage, bundleName);
    ...
    /**
     * Connection between the cloned Part and source Part is established with Derived Realationship. The following steps are performed: - The Cloned Part id is retrieved - The Source Part id from
     * which the part is cloned is retrieved - The connection is established between the Parts
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the context part The ObjectId of the Source Part
     * @throws Exception
     *             if the operation fails.
     * @since X3.
     */
    public void connectClonedObject(Context context, String[] args) throws Exception {
        ...
        // if (sourceObject.getInfo(context,"type").equals(PropertyUtil.getSchemaProperty(context,"type_Part")) &&
        // clonedObject.getInfo(context,"type").equals(PropertyUtil.getSchemaProperty(context,"type_Part")))
    ...
    /**
     * The Derived Parts in the From side of the Derived relationship of context part is retrieved The following steps are performed: - The Context Part id is retrieved - The Parts connected to the
     * context part with Derived relationship in the from side are retrieved
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the context part
     * @throws Exception
     *             if the operation fails.
     * @since ECX3.
     */
    ...
    /**
     * The Derived Parts in the To side of the Derived relationship of context part is retrieved The following steps are performed: - The Context Part id is retrieved - The Parts connected to the
     * context part with Derived relationship in the To side are retrieved
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectID of the context part
    ...
    /**
     * Added for IR-371814 Returns the expandLevel in integer format. If expandfilter is All then set to "0".
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: -The expandLevel passed by user
     */
    ...
    /**
     * The Level in the Derived relationship of context part is retrieved The following steps are performed: - The Parts connected to the context part with Derived relationship id's are retained and
     * corresponding Level is retrieved
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
     *            holds the following input arguments: - The ObjectList of the parts
     * @throws Exception
     *             if the operation fails.
     * @since ECX3.
     */
     ...
     * @mx.summary this methods takes the values from Application property file And generates the Check boxes depending on the value passed
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     * @since V6R2009-1
     */
    public String showIncludeRelDataField(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if ((objectId != null && !objectId.equals("") && !objectId.equals("null"))) {
                ...
                if (domPartObj.isKindOf(context, TYPE_PART) == true) {
                    ...
                    String attrEnableCompVal = domPartObj.getAttributeValue(context, attrEnableCompliance);
                    if (attrEnableCompVal != null && ("Enabled").equals(attrEnableCompVal)) {
                        isPartEnabled = true;
                    }
                }
            }
                ...
                for (int indAppRel = 0; indAppRel < appSpeIncRel.size(); indAppRel++) {
                    ...
                    if (strListTypeRoles.size() > 0) {
                        ...
                        while (itrRole.hasNext()) {
                            ...
                            if (person.hasRole(context, PropertyUtil.getSchemaProperty(context, (String) itrRole.next()))) {
                                hasRole = true;
                                break;
                            }
                        }
            ...
            sb.append(EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", strLocale, "emxComponents.PartCloneIncludeRelData.AllRel"));
            ...
            for (int allRel = 0; allRel < allAppsI18nIncRelData.size(); allRel++) {
                ...
                sb.append(allRel);
                sb.append("\" value=\"");
                sb.append(PropertyUtil.getSchemaProperty(context, (String) keyLabel.get((String) allAppsI18nIncRelData.get(allRel))));
                sb.append("\"/>");
                sb.append("&#160;");
     ...
     * @mx.whereUsed Invoked from common Clone Part web form's IncludeReleatedData field as an update program.
     * @mx.summary this methods takes the chcek box values which is selected from the Include Releated Field of the clone dialog and creates a comma separated String to pass inside the bean.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     * @throws Exception
    ...
    public void updateIncludeRelDataField(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap requestMap = (HashMap) programMap.get("requestMap");
            ...
            if (relList.size() > 0) {
                ...
                commonPart.cloneCommonPartStructure(context, parentOId, newObjectId, sb.toString());
                ...
                for (int appItr = 0; appItr < applicationList.size(); appItr++) {
                    ...
                    if ((updatePrgm != null && !"null".equals(updatePrgm) && !"".equals(updatePrgm)) && (updateFn != null && !"null".equals(updateFn) && !"".equals(updateFn))) {
     ...
     * @mx.whereUsed Invoked from common Clone Part web form's policy field used to Clone object
     * @mx.summary this methods takes the passed copyObjectId and displays the policy in a combo box. If copyObjectId is null, then value of the type passed from the HREF is taken in to consideration
     *             to populate the policies.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     * @return the <code>String</code> value
