...
/**
 * The <code>emxLibraryCentralCommonBase.java</code> class contains utility methods for getting data using configurable tables in Library Central.
 * @version Common 10-5 - Copyright (c) 2002, MatrixOne, Inc.
 */
public class emxLibraryCentralCommonBase_mxJPO extends emxDomainObject_mxJPO implements LibraryCentralConstants {
    /**
    ...
    public emxLibraryCentralCommonBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    }
    ...
    /**
     * Entry point for business object creation. The required arguments are defined in <code>getRequiredCreateArgKeys ()</code> Warning: do to the static nature of this system if sub-classes need to
     * modified implementation details, this function must be overridden.
     * @param context
     *            the eMatrix <code>Context</code> user's context
     * @param args
     *            contains a Map with the following entries: type - a String of the Object Type name - a String of the Object Name policy - a String of the Object Policy
     * @return string with created object ID
     * @throws Exception
     *             if a major failure occurs
     */
    public String createObject(Context context, String[] args) throws IllegalArgumentException, FrameworkException {
        String type = null;
        String name = null;
    ...
    /**
     * This method performs the overall create operation using "native" arguments.
     * @param context
     *            - user's context
     * @param type
     ...
     * @param revision
     *            - revision of object to create
     * @param policy
     *            - policy of object to create
     * @param vault
     *            - vault of object to create
     * @param attributeMap
     *            - custom attributes, description, & owner
     * @param specificationMap
     *            - other required information for create
     * @return object id created
     * @throws com.matrixone.framework.util.FrameworkException
     *             if a major failure occurs
     */
    protected String createObject(Context context, String type, String name, String revision, String policy, String vault, Map attributeMap, Map specificationMap) throws FrameworkException {
        ...
        try {
            ...
            busObj = createBusObject(context, type, name, revision, policy, vault, owner, description);
            ...
            finishBusObject(context, busObj, owner, attributeMap, specificationMap);

            ContextUtil.commitTransaction(context);
            ContextUtil.startTransaction(context, true);

            handleNotifyUsers(context, busObj, specificationMap);
     ...
     *             if the operation fails
     */
    protected BusinessObject createBusObject(Context context, String type, String name, String revision, String policy, String vault, String owner, String description) throws Exception {

        BusinessObject busObj = null;

        // Create using auto name if name is null
        if (name == null || name.length() < 1) {
            String aliasType = FrameworkUtil.getAliasForAdmin(context, "type", type, true);

            String aliasPolicy = FrameworkUtil.getAliasForAdmin(context, "policy", policy, true);

            busObj = new BusinessObject(FrameworkUtil.autoName(context, aliasType, "", aliasPolicy, vault, revision));
        } else {
            busObj = new BusinessObject(type, name, revision, vault);
     ...
     *            - owner of object to create
     * @param attributeMap
     *            - map containing custom attributes, description, & owner
     * @param specificationMap
     *            - map containing other required information for clone
     * @throws Exception
     *             if a major failure occurs
     */
    protected void finishBusObject(Context context, BusinessObject busObj, String owner, Map attributeMap, Map specificationMap) throws Exception {

        setCustomAttributes(context, busObj, attributeMap);
        ...
        if (obj != null) {
            ...
            String containingRel = (String) specificationMap.get(LibraryCentralConstants.JPO_SPEC_CONTAINING_RELATIONSHIP);

            addToContainingObject(context, domainObj, containingObjId, containingRel);
        }
     ...
     *             if the operation fails
     */
    protected void grantOwnerAccess(Context context, BusinessObject busObj, String owner) throws Exception {
     ...
     *             if the operation fails
     */
    protected void setCustomAttributes(Context context, BusinessObject busObj, Map attrMap) throws Exception {
        ...
        while (itr.hasNext()) {
            key = (String) itr.next();
            if (key.equals("attribute_Description") || key.equals("attribute_Owner"))
            ...
            if (symbolicAttrName != null && symbolicAttrName.length() > 0) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: objectid - objectid id list - list of ids to connect to
     * @throws Exception
     *             if a major failure occurs
     */
    public void addToContainingObject(Context context, String[] args) throws Exception {
        try {
     ...
     *             if the operation fails
     */
    protected void addToContainingObject(Context context, DomainObject domainObj, String[] containingObjIds, String containingRel) throws Exception {

        if (containingRel != null && !containingRel.equals("null") && containingObjIds != null && containingObjIds.length > 0) {
            for (int i = 0; i < containingObjIds.length; i++) {
                if ((containingObjIds[i] != null) && (containingObjIds[i].length() > 0)) {
                    DomainObject newDO = new DomainObject(containingObjIds[i]);
                    DomainRelationship.connect(context, newDO, containingRel, domainObj);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: objectId - objectId folderId - folder to connect to user list - users to send messages to
     * @throws Exception
     *             if a major failure occurs
     */
    public void addToFolder(Context context, String[] args) throws Exception {
        try {
            Map specificationMap = (Map) JPO.unpackArgs(args);
            String objectId = (String) specificationMap.get(LibraryCentralConstants.OBJECT_ID);
     ...
     *             if the operation fails
     */
    protected void addToFolder(Context context, DomainObject domainObj, String folderId) throws Exception {
        ...
        if (folderId != null && folderId.length() > 0 && !folderId.equals("null")) {
            ...
            while (st.hasMoreTokens()) {
                ...
                DomainRelationship.connect(context, new DomainObject(temp), folderRelationship, domainObj);
    ...
    /*
     * Pulls from the specification map the required values, and then invokes necessary method for notifying the users.
     * 
     * @param context A Matrix DB context
     * 
     * @param objectId The Matrix ObjectId of the object for which notification should be sent.
     * 
     * @param specificationMap A specification map, containing the keys for the message (JPO_SPEC_NOTIFY_MESSAGE) and the users to whom the notification is to be sent (JPO_SPEC_NOTIFY_TO_USERS).
     * 
     * @throws Exception if the operation fails
     */
    public void handleNotifyUsers(Context context, BusinessObject busObject, Map specificationMap) throws Exception {
        ...
        notifyUsers(context, busObject, subject, message, toUsersList);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: subject - subject message - message to send user list - users to send messages to
     * @throws Exception
     *             if a major failure occurs
     */
    public void notifyUsers(Context context, String[] args) throws Exception {
        try {
            Map specificationMap = (Map) JPO.unpackArgs(args);
     ...
     *             if the operation fails
     */
    protected void notifyUsers(Context context, BusinessObject busObj, String subject, String message, String toUsersList) throws Exception {
        ...
        if (toUsersList != null && toUsersList.length() > 0 && !toUsersList.equals("null")) {
            ...
            if (subject == null || subject.length() == 0) {
                String strNotificationSub = "emxDocumentCentral.DocumentCentralCommon.NotificationSubject";
                subject = emxMailUtil_mxJPO.getString("emxDocumentCentralStringResource", strNotificationSub, null, locale);
            }

            if (message == null || message.length() == 0) {
                String[] msgKeys = new String[] { "type", "name", "revision", "user", "type" };

                type = i18nNow.getTypeI18NString(type, context.getSession().getLanguage());
                String[] msgValues = new String[] { type, name, revision, context.getUser(), type };

                String strNotificationMessage = "emxDocumentCentral.DocumentCentralCommon.NotificationMessage";
                String msg = emxMailUtil_mxJPO.getString("emxDocumentCentralStringResource", strNotificationMessage, null, locale);

                message = StringResource.format(msg, msgKeys, msgValues);
            } // if
            else {
                String strAutoName = "emxDocumentCentral.DocumentCentralCommonWizard.AutoName";

                String autoName = emxMailUtil_mxJPO.getString("emxDocumentCentralStringResource", strAutoName, null, locale);

                int autoNameIndex = message.indexOf(autoName);

                if (autoNameIndex != -1) {
                    message = message.substring(0, autoNameIndex) + name + message.substring(autoNameIndex + autoName.length());
                }
            } // else

            MailUtil.sendNotification(context, toList, null, null, subject, null, null, message, null, null, objList, companyName);
        }
    }
     ...
     *             if the operation fails
     */
    protected void sendEventIconMail(Context context, String event, Subscribable subscribedObj, String objAtt) {

        try {
            SubscriptionManager subMgr = new SubscriptionManager(subscribedObj);
    ...
    /**
     * Helper function which provides all required argument keys for the create operation. Specifically, the following Strings are required: <code>LibraryCentralConstants.JPO_ARGS_TYPE,
    ...
    static public String[] getRequiredCreateArgKeys() {
        String[] reqdArgKeys = new String[] { LibraryCentralConstants.JPO_ARGS_TYPE, LibraryCentralConstants.JPO_ARGS_REVISION, LibraryCentralConstants.JPO_ARGS_POLICY,
                LibraryCentralConstants.JPO_ARGS_VAULT, LibraryCentralConstants.JPO_ARGS_ATTR_MAP, LibraryCentralConstants.JPO_ARGS_SPEC_MAP };

        return reqdArgKeys;
    }

    /**
     * This method validates that all required argument keys are contained in the argMap.
     * @param argMap
     *            - argument passed from bean to JPO.
     * @return true if bare-minimum for a valid argument map has been met.
     */
    protected boolean isArgMapValid(Context context, String[] argKeys, Map mapToValidate) throws Exception {
    ...
    public void addChildren(Context context, String[] args) throws Exception {
            ...
            if (mapResultList.size() == 0) {
                ...
                if (access.hasDeleteAccess()) {
                    ...
                    if (strTypeName.equalsIgnoreCase(TYPE_GENERAL_LIBRARY)) {
                        MapList referenceFeatureList = doObj.getRelatedObjects(context, LibraryCentralConstants.RELATIONSHIP_LIBRARY_REFERENCE_FEATURE,
                                LibraryCentralConstants.TYPE_LIBRARY_FEATURE_REFERENCE, selSelect, null, false, // boolean getTo,
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following arguments: type - the object type
     ...
     *            the eMatrix <code>Context</code> object
     * @param holds
     *            the following input arguments: 1 - object ID 2 - event the event name
     * @throws Exception
     *             if the operation fails
     */
    public void classifiedItemContentModify(matrix.db.Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (objectId != null && !"".equals(objectId) && !"null".equals(objectId)) {
                ...
                mListParent = (MapList) domainObj.getRelatedObjects(context, DomainConstants.RELATIONSHIP_CLASSIFIED_ITEM, LibraryCentralConstants.QUERY_WILDCARD, slSelAttrs, null, true, false,
                        (short) 0, null, null);
    ...
    /**
     * To restrict connecting any from item on Subclass relation to have only limited to types e.g Part Library can have Part Family(and its derivatives) as subclass Any classification type (General
     * Class, Part Family and Manfacturing Partfamily) can have a same type or its derivatives as Subclass i.e. General Class can have an other General Class as subclass Part Family can have Part
     * Family and Manfacturing Partfamily as subclass Manfacturing Partfamily can have an other Manfacturing Partfamily as subclass Libraries (General Library and Part Library) can have any
     * classification type as subclass So no need to put 'Libraries' in return Map, i.e. if any from item is not included in return Map all items on to side can connect as Subclass to that from side
     * item.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * To restrict connecting any from item on Classified Item relation to have only limited to types e.g Part Family (and its derivates) can have only Part (and its derivatives) as its end items
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds no argument
    ...
    public Map getConditionalAllowedEndItemsInClassifiedItemRelation(Context context, String[] args) throws Exception {
        ...
        for (Iterator iter = relFromTypes.keySet().iterator(); iter.hasNext();) {
            ...
            BusinessType bu = (BusinessType) relFromTypes.get(from);
            String buName = bu.getName();
            if (buName.equals(LibraryCentralConstants.TYPE_PART_FAMILY) || mxType.isOfParentType(context, buName, LibraryCentralConstants.TYPE_PART_FAMILY)) {
            ...
            } else if (buName.equals(LibraryCentralConstants.TYPE_DOCUMENT_FAMILY) || mxType.isOfParentType(context, buName, LibraryCentralConstants.TYPE_DOCUMENT_FAMILY)) {
                ...
                allowedTypeMap.put(from, derivativeSymbolicNames);
            } else if (buName.equals(LibraryCentralConstants.TYPE_GENERAL_CLASS) || mxType.isOfParentType(context, buName, LibraryCentralConstants.TYPE_GENERAL_CLASS)) {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - policy
     * @return String - the default revision
     * @throws Exception
     *             if the operation fails
     */
    public String getRevisionForPolicy(Context context, String[] args) throws Exception {
    ...
    /**
     * This method checks whether a given type is Document Central Type i.e. Document library/Book shelf/ Book
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - type
     * @return boolean true - if the given type is of Library Central Type
     * @throws Exception
     *             if the operation fails
     */
    public boolean isDocumentCentalType(Context context, String args[]) throws Exception {
        String selectedType = "";
     ...
     *             if the operation fails
     */
    public boolean isDocumentCentalType(Context context, String type) throws Exception {
        return (mxType.isOfParentType(context, type, TYPE_DOCUMENT_FAMILY) || mxType.isOfParentType(context, type, TYPE_DOCUMENT_LIBRARY));
    }
     ...
     *             if the operation fails
     */
    public boolean isOrphanAllowed(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails
     */
    public boolean isOrpahanNotAllowed(Context context, String[] args) throws Exception {
        return !isOrphanAllowed(context, args);
    }

    /**
     * This method returns true if orphan object can be created and the given type is DocumentCentralType
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
    ...
    public boolean isOrphanAllowedAndIsDCType(Context context, String[] args) throws Exception {
        return (isOrphanAllowed(context, args) && isDocumentCentalType(context, args));
    }

    /**
     * This method returns true if orphan object can be created and the given type is not a DocumentCentralType
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
    ...
    public boolean isOrphanAllowedAndIsNotDCType(Context context, String[] args) throws Exception {
        return (isOrphanAllowed(context, args) && !isDocumentCentalType(context, args));
    }

    /**
     * This method returns true if orphan object can not be created and the given type is DocumentCentralType
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
    ...
    public boolean isOrphanNotAllowedAndIsDCType(Context context, String[] args) throws Exception {
        return (!isOrphanAllowed(context, args) && isDocumentCentalType(context, args));
    }

    /**
     * This method returns true if orphan object can not be created and the given type is not a DocumentCentralType
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
    ...
    public boolean isOrphanNotAllowedAndIsNotDCType(Context context, String[] args) throws Exception {
        return (!isOrphanAllowed(context, args) && !isDocumentCentalType(context, args));
    }
    ...
    public String getCreateInTypes(Context context, String[] args) throws Exception {
        String type = "";
        String baseType = "";
        try {
    ...
    /**
     * This method returns exclude OIDs (for which user does not have form connect access)
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - field - create In type and it should be like TYPE=Document Library
     * @return StringList - exclude oids.
     * @throws Exception
     *             if the operation fails
     */
    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
    public StringList getCreateInExcludeOIDs(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mapList = DomainObject.findObjects(context, createInType, null, null, null, "*", whereClause, true, new StringList(DomainObject.SELECT_ID));
     ...
     *             if the operation fails
     */
    public HashMap getVaultList(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - type 1 - languagestr
     * @return String - notification Message
     * @throws Exception
     *             if the operation fails
     */
    public String getNotificationMessage(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - TypeActual 1 - Name 2 - Owner 3 - Policy
     * @return Map - notification Message
     * @throws Exception
     *             if the operation fails
     */
    public Map reloadNotificationMessage(Context context, String[] args) throws Exception {
        Map relaodMap = new HashMap();
        HashMap paramMap = (HashMap) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object Id 1 - launguagestr
     * @return html of lock status
     * @throws Exception
     *             if the operation fails
     */
    public String getLockStatus(Context context, String[] args) throws Exception {
    ...
    /**
     * This method sets the Create In Default value when the form is launched from any parent
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     ...
     *             if the operation fails
     */
    public String setCreateInDefaultValue(Context context, String[] args) throws Exception {
        StringBuffer sbReturnString = new StringBuffer();
    ...
    /**
     * This method updates Create In Field connects the created object with seleceted container Id
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object Id 1 - New OID (i.e value of Create In Field)
     * @throws Exception
     *             if the operation fails
     */
    public void updateCreateInField(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            String objectId = (String) paramMap.get(OBJECT_ID);
     ...
     *             if the operation fails
     */
    public void updateCreateInFieldForGenericDocument(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            String objectId = (String) paramMap.get(OBJECT_ID);
    ...
    /**
     * This method updates Folder field connects the created object with selected folder
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object Id 1 - New OID (i.e value of fodler Id)
     * @throws Exception
     *             if the operation fails
     */
    public void updateFolderField(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            String objectId = (String) paramMap.get(OBJECT_ID);
    ...
    /**
     * This method updates notify field Sends the message to selected notify users
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object Id 1 - New Value (i.e notify users) 2 - launguagestr 3 - Message
     * @throws Exception
     *             if the operation fails
     */
    public void updateNotifyField(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            HashMap requestMap = (HashMap) programMap.get("requestMap");
    ...
    /**
     * This method updates Title field If it is Empty with Object Name The updation of Title field is done in updateNotifyField
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
     ...
     *             if the operation fails
     */
    public void updateTitleField(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            HashMap requestMap = (HashMap) programMap.get("requestMap");
    ...
    /**
     * This method updates Message field The updation of Message field is done in updateNotifyField
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments
     ...
     *             if the operation fails
     */
    public void updateMessageField(Context context, String[] args) throws Exception {
        // do nothing
    }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object Id
     ...
     *             if the operation fails
     */
    public boolean hasLockAccess(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails
     */
    public boolean hasUnLockAccess(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails
     */
    public boolean hasReadWriteAccess(Context context, String[] args) throws Exception {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object ID 1 - vault 2 - name 3 - type
     * @return Map - object ID
     * @throws Exception
     *             if the operation fails
     */
    @com.matrixone.apps.framework.ui.CreateProcessCallable
    public Map createClone(Context context, String[] args) throws Exception {
     ...
     */
    @com.matrixone.apps.framework.ui.CreateProcessCallable
    public Map createRevision(Context context, String[] args) throws Exception {
        HashMap requestMap = (HashMap) JPO.unpackArgs(args);
        Map returnMap = new HashMap();
        try {
            Libraries lib = new Libraries();
    ...
    public StringList getFolders(Context context, String[] args) throws Exception {
    ...
    public boolean isDCFolder(Context context, String id) throws Exception {
    ...
    public HashMap getFolderSecurityOptions(Context context, String[] args) throws Exception {
        HashMap rangeMap = new HashMap();
        try {
            StringList fieldChoices = new StringList();
            StringList fieldDisplayChoices = new StringList();
            fieldChoices.add("True");
            fieldDisplayChoices.add(
                    EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()), "emxDocumentCentral.CreateFolder.GlobalReadAccess"));
            fieldChoices.add("False");
            fieldDisplayChoices
                    .add(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()), "emxDocumentCentral.CreateFolder.Private"));
            rangeMap.put("field_choices", fieldChoices);
            rangeMap.put("field_display_choices", fieldDisplayChoices);
        } catch (Exception ex) {
            throw new FrameworkException(ex.toString());
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - object ID 1 - languageStr
     * @return String - Global Read Value
     * @throws Exception
     *             if the operation fails
     */

    public static String getFolderSecurityValue(Context context, String[] args) throws Exception {
        ...
        if (attrGlobalReadValue != null && !"null".equalsIgnoreCase(attrGlobalReadValue) && attrGlobalReadValue.equalsIgnoreCase("True")) {
            attrGlobalReadValue = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - mode
     * @return boolean - Edit Mode
     * @throws Exception
     *             if the operation fails
     */
    public static boolean isEditMode(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public static boolean isViewMode(Context context, String[] args) throws Exception {
     ...
     *             if the operation fails
     */
    public static String getVCSelector(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        String objectId = (String) paramMap.get(OBJECT_ID);
     ...
     *             if the operation fails
     */
    public static String getVCServer(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        String objectId = (String) paramMap.get(OBJECT_ID);
        DomainObject dObject = new DomainObject(objectId);
     ...
     *             if the operation fails
     */
    public static String getVCPath(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap paramMap = (HashMap) programMap.get("paramMap");
        String objectId = (String) paramMap.get(OBJECT_ID);
        DomainObject dObject = new DomainObject(objectId);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 0 - ObjectId 1 - newObjectId
     * @return boolean - isVersionable
     * @throws Exception
     *             if the operation fails
     */
    @com.matrixone.apps.framework.ui.PostProcessCallable
    public static void connectDocumentSheetToGenericDocument(Context context, String[] args) throws Exception {
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            HashMap paramMap = (HashMap) programMap.get("paramMap");
            HashMap requestMap = (HashMap) programMap.get("requestMap");
     ...
     *             if the operation fails
     */
    public String getObjectRevision(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
     ...
     *             if the operation fails
     */
    public boolean isReviseMode(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String mode = (String) programMap.get("mode");
    ...
    public boolean isNotReviseMode(Context context, String[] args) throws Exception {
    ...
    public static StringList getAllowedClasificationTypes(Context context, String[] args) throws FrameworkException {
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments 1 - object Is
     * @throws Exception
     *             if the operation fails
     */
    @com.matrixone.apps.framework.ui.PostProcessCallable
    public void sendObjectModifyNotification(Context context, String args[]) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        HashMap requestMap = (HashMap) programMap.get("requestMap");
        ...
        if (domObj != null && domObj.getInfo(context, DomainConstants.SELECT_TYPE).equals(TYPE_WORKSPACE_VAULT)) {
            sendEventIconMail(context, EventConstants.EVENT_FOLDER_MODIFIED, (Subscribable) domObj, objectId);
        }
    }
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: contentIDs - contentIds folderId - folder to connect to
     * @throws Exception
     *             if a major failure occurs
     */
    public void updateAddContentField(Context context, String[] args) throws Exception {
        try {
            Map specificationMap = (Map) JPO.unpackArgs(args);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: type - type
     * @throws Exception
     *             if a major failure occurs
     */
    ...
    /***
     * Trigger method, which would be called for every create of Subclass relationship. During the connection, child Objects revision would be compared with the parent objects physicalid, if its same,
     * then the revision would not be changed to physicalid of the parent. Otherwise it would be changed. During creation of object, if there exists a parent, the object revision would be changed to
     * physicalid of the parent,if no parent exist, the revision would not be changed.
     * @param context
     * @param args
     * @return
     * @throws FrameworkException
    ...
    /***
     * Changes the revision of the object to the PhysicalId of the parent object. Since changing the revision alone is not permitted, we are changing the name of the object also with a "~" and
     * reverting back to the original name.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param childId
     *            , the objectId whose revision needs to be changed to PhysicalId
     * @param objectName
     *            , the name of the object
     * @param parentPhysicalId
     *            , physicalId of the parent Object
     * @throws FrameworkException
     *             ,if the operation fails
     */
    public void changeRevisiontoPhysicalId(Context context, String childId, String objectName, String parentPhysicalId) throws FrameworkException, Exception {
        String errorMessage = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
                "emxMultipleClassification.AddExisting.Error");
        try {
    ...
    /**
     * This method returns the vault associated with the context's Person this is temporarily added to replace ${CLASS:emxDocumentCentralRootBase}:systemVaults()
     * @param context
     * @param args
     * @return
     * @throws Exception
    ...
    /***
     * This method return all the Subclasses for a Library. Used during classify or Reclassify on End Items
     * @param context
     * @param args
     * @return
     * @throws FrameworkException
    ...
    public String getAllSubClassforLibrary(Context context, String args[]) throws FrameworkException {
        ...
        try {
            ...
            DomainObject domObj = new DomainObject(objectId);
            MapList mList = null;
            mList = domObj.getRelatedObjects(context, LibraryCentralConstants.RELATIONSHIP_SUBCLASS, "*", objectSelects, relselects, true, // boolean getTo,
    ...
    /**
     * This method checks if the Document Object is locked Used as check trigger in Generic Document lifecycle changes
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            an array of String arguments for this method 0 - objectId, the id of this Generic Document
    ...
    public int isLocked(Context context, String[] args) throws Exception {
        try {
            ...
            MapList activeVersionsList = domainObj.getRelatedObjects(context, rel_ActiveVersion, "*", objectSelects, null, false, true, new Short("1"), "locked == true", "", 0);
     ...
     * @throws Exception
     */
    public void dummyUpdateFunction(Context context, String[] args) throws Exception {
        // do nothing
    }
     ...
     */
    private void invalidateVPLMCatalogueCache(Context context) throws FrameworkException {
        try {
     ...
     */
    public Boolean hasLibraryViewerRole(Context context, String[] args) throws Exception {
     ...
     * @since CommonComponents 11.0
     */
    public Boolean hasRetentionRecordRole(Context context, String[] args) throws Exception {
        matrix.db.Person personObj = new matrix.db.Person(context.getUser());
     ...
     *            :
     * @param args
     *            : contains HashMap Object which in turn has either a Library or Class Info
        ...
        try {
            ...
            if (null != libMap.get(LibraryCentralConstants.SELECT_ID) && !libMap.get(LibraryCentralConstants.SELECT_ID).isEmpty()) {
                ...
                if (null != libMap.get(LibraryCentralConstants.SELECT_OWNER)) {
                    /*
                     * Person person = new Person(); boolean exists = person.doesPersonExists(context, libMap.get("owner")); if(exists) {
                     */
