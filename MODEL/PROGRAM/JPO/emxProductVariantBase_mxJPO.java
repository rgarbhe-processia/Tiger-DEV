...
/*
 ** emxProductVariantBase Copyright (c) 1993-2015 Dassault Systemes. All Rights Reserved. This program contains proprietary and trade secret information of Dassault Systemes. Copyright notice is
...
/**
 * The <code>emxProductVariantBase</code> class contains methods related to the admin type Product Variant. This includes methods for the Create
...
public class emxProductVariantBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    /**
     * This method accepts Relationship name and WHERE condition and expands the context object based on the speicified relationship, to retrievel all the Product Variants matching the where
     * condition.
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * This method is used to get all the Product Variant in the Product Context This will use the Dynamic Column Behivour to show the Product Variants
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public MapList getProductVariantsSelectedFeatures(Context context, String args[]) throws Exception {
        ...
        try {
            ...
            mapChildParts = dom.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCT_VERSION, ConfigurationConstants.TYPE_PRODUCT_VARIANT, strListSelectstmts, new StringList(), false,
                    true, (short) 0, strSelIdsWhrClause, "", 0);
            if (mapChildParts.isEmpty()) {
                String language = context.getSession().getLanguage();
                String strAlertMessage = EnoviaResourceBundle.getProperty(context, SUITE_KEY, "emxProduct.Error.ProductRevision.NoProductVariants", language);
     ...
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the following input arguments: 0 - HashMap containing one MapList entry for the key "objectList" This MapList is a list of HashMaps containing one String entry for the key
     *            "objectId".
     * @param strAttributeName
     *            the attribute for which drop down HTML has to be build
     * @param strSelectAttribute
     *            select expression for the attribute
     * @param iLength
     ...
     */
    protected Vector getHTMLTagsForTextBox(Context context, String[] args, String strAttributeName, String strSelectAttribute, int iLength) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        ...
        for (int i = 0; i < iNumOfObjects; i++) {
            ...
            if (object instanceof HashMap) {
                arrObjId[i] = (String) (((HashMap) object).get(DomainConstants.SELECT_ID));
                arrRelId[i] = (String) (((HashMap) object).get(DomainConstants.SELECT_RELATIONSHIP_ID));
            } else {
                arrObjId[i] = (String) (((Hashtable) object).get(DomainConstants.SELECT_ID));
                arrRelId[i] = (String) (((Hashtable) object).get(DomainConstants.SELECT_RELATIONSHIP_ID));
            }
        }
        // getting the default values for object ids
        StringList strSelectAttrib = new StringList(strSelectAttribute);
        matrix.db.BusinessObjectWithSelectList businessObjectWithSelectList = matrix.db.BusinessObject.getSelectBusinessObjectData(context, arrObjId, strSelectAttrib);
        ...
        for (int i = 0; i < iNumOfObjects; i++) {
            ...
            strSelectedAttribute = (businessObjectWithSelectList.getElement(i)).getSelectData((String) strSelectAttrib.get(0));
            // forming the HTML tag for one row
            strTextBoxHTML = new StringBuffer("<input type=\"text\" size=\"20\"");
            strTextBoxHTML.append(" maxlength=\"").append(strLength).append("\" name=\"");
            strTextBoxHTML.append(arrObjId[i]).append(":").append(strAttributeName);
            strTextBoxHTML.append("\" value=\"").append(strSelectedAttribute).append("\" />");
            textBoxVector.add(strTextBoxHTML.toString());
        }
        return textBoxVector;
    }

    public Vector editVariantName(Context context, String[] args) throws Exception {
        String strAttribute = ConfigurationConstants.VARIANT_NAME;
        String strSelectAttribute = "attribute[" + strAttribute + "]";
        return getHTMLTagsForTextBox(context, args, strAttribute, strSelectAttribute, (short) 80);
    }

    public Vector editMarketingName(Context context, String[] args) throws Exception {
    ...
    /**
     * This method is used as a access function to show/not to show Create,Clone,Edit, and Delete Commands of Product Variants
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds arguments
     * @return Object - true if the Product is not in Release state false if the Product is in Release state
     * @throws Exception
     *             if the operation fails
     * @since Feature Configuration Module X3
     */
    ...
    /**
     * This method is used to Clone Product Variants when the Context Product is Revised or Copied to Other Product
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param isRevise
     *            if the Product is Revised - true otherwise false
     * @param srcProductId
     *            the Context Source Product Id
     * @param dstProductId
     *            the Context Destination Product Id
     * @throws Exception
     *             if the operation fails
     * @return void
     * @since Feature Configuration Module X3
    ...
    public void cloneProductVariantOnProductReviseAndCopy(Context context, String args[]) throws Exception {
        try {
            ...
            if (productVariants.size() > 0) {
                ...
                for (int i = 0; i < productVariants.size(); i++) {
                    ...
                    if (isRevise.equalsIgnoreCase("true")) {
                        ...
                        if (strInheriteManBG.equals("Yes")) {
                            ...
                            JPO.invoke(context, "emxRule", null, "inheritMandatoryRulesToProductVariant", argsIM);
                        }
                    }
                ...
                MapList mLstLogicalStructure = (MapList) logFeature.getLogicalFeatureStructure(context, null, null, sLstObjeSelects, sLstRelSelects, false, true, 0, 0, DomainObject.EMPTY_STRING, null,
                ...
                if (mLstLogicalStructure.size() > 0) {
                    ...
                    for (int i = 0; i < productVariants.size(); i++) {
                        ...
                        Object objOldLogFeatureRelID = (Object) tempMap.get("from[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].torel.to.id");
                        ...
                        for (int k = 0; k < connectedLogFeatureIDs.size(); k++) {
                            ...
                            for (int j = 0; j < mLstLogicalStructure.size(); j++) {
                                ...
                                if (logFeatureID.equalsIgnoreCase(connectedLogFeatureID)) {
                                    ...
                                    // plCommon.connectObject(context,relation,newPVID,true);
    ...
    /**
     * checks if the product is in release state . called as part of check trigger program, when promoting ProductVariant to release state
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    /**
     * Used As Wrapper method for expandLogicalStructureForProductVariant method It is used as an Expand Program Function for the structure browser expand of Logical Features
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds arguments
     ...
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList expandLogicalStructureForProductVariant(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            slRelSelects.addElement("tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].attribute[" + ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE + "]");
            ...
            slRelSelects.add("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION + "]");
            slRelSelects.add("tomid[" + ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION + "].from.attribute[" + ConfigurationConstants.ATTRIBUTE_RIGHT_EXPRESSION + "]");
            ...
            if (strIsStructureCompare != null && strIsStructureCompare.equalsIgnoreCase("true") && (mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_LOGICAL_FEATURE))
                    || (mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_PRODUCTS) && !mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_PRODUCT_VARIANT))) {
                mapList = new MapList();
                if (!mxType.isOfParentType(context, strType, ConfigurationConstants.TYPE_PRODUCTS)) {
                    mapList = utilObj.getObjectStructure(context, strTypePattern, strRelPattern, slObjSelects, slRelSelects, false, true, level, limit, null, null, (short) 0, filterExpression);
                }
            } else {
                mapList = utilObj.getObjectStructure(context, strTypePattern, strRelPattern, slObjSelects, slRelSelects, false, true, level, limit, strObjWhere, strRelWhere, (short) 0,
                        filterExpression);
            }
    ...
    /**
     * Used As Wrapper method for getPFLUsageRange method It is used to get the Usage attribute value ranges on PFL connetion *
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds arguments
    ...
    public HashMap getPFLUsageRange(Context context, String[] args) throws Exception {
        ...
        String strFeatureUsageRequiredDisplay = EnoviaResourceBundle.getProperty(context, SUITE_KEY, strFeatureUsageRequired, languageStr);
    ...
    /**
     * Used As Wrapper method for getPFLUsage method It is used to get the Usage attribute value on PFL connetion *
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds arguments
    ...
    public List getPFLUsage(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String strFeatureUsageStandardDisplay = EnoviaResourceBundle.getProperty(context, SUITE_KEY, strUsageStandard, languageStr);
            String strFeatureUsageOptionalDisplay = EnoviaResourceBundle.getProperty(context, SUITE_KEY, strUsageOptional, languageStr);
            String strFeatureUsageRequiredDisplay = EnoviaResourceBundle.getProperty(context, SUITE_KEY, strUsageRequired, languageStr);

            ConfigurationUtil util = new ConfigurationUtil();
            String strPRDSelect = "tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].from.id";
            String strFATSelect = "tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].attribute[" + ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE + "]";
            ...
            for (int i = 0; i < ObjectList.size(); i++) {
                ...
                } else if (mpLF.containsKey(DomainRelationship.SELECT_ID) && ProductLineCommon.isNotNull((String) mpLF.get(DomainRelationship.SELECT_ID))) {
                    DomainConstants.MULTI_VALUE_LIST.add(strPRDSelect);
                    DomainConstants.MULTI_VALUE_LIST.add(strFATSelect);

                    MapList mlUsedResult = DomainRelationship.getInfo(context, new String[] { (String) mpLF.get(DomainRelationship.SELECT_ID) }, selectables);
    ...
    /**
     * Used As Wrapper method for updatePFLUsage method It is used to update the Usage attribute value on PFL connetion *
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds arguments
    ...
    public void updatePFLUsage(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            MapList mLstPFLDetails = ProductLineCommon.queryConnection(context, strPFLRelPattern, strSelect, sBufWhereCond.toString());
             ...
             * 
             * for (int i = 0; i < ObjectList.size(); i++) { Map mp = (Map) ObjectList.get(i); usage = (Object) mp.get("to["+ConfigurationConstants.RELATIONSHIP_LOGICAL_FEATURES +"]"+
             * ".tomid["+ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].attribute["+ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE+"]"); if(usage instanceof String){
             * usageList.add(usage.toString());
             * 
             * }else if(usage instanceof StringList){ usageList= (StringList)usage; } String[] UsageArry = (String[]) usageList.toArray(new String[0]);
             * 
             * //PFL ids PFLids = (Object)mp.get("to["+ConfigurationConstants.RELATIONSHIP_LOGICAL_FEATURES +"]"+ ".tomid["+ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST +
             * "]."+DomainConstants.SELECT_ID); if(usage instanceof String){ strPLFId.add(PFLids.toString());
             * 
             * }else if(usage instanceof StringList){ strPLFId= (StringList)(PFLids); }
             * 
             * String[] PLFidArry = (String[]) strPLFId.toArray(new String[0]); StringList Selectables = new StringList(); Selectables.add(DomainConstants.SELECT_FROM_ID);
             * 
             * DomainRelationship domainrelation = new DomainRelationship(); MapList ml = domainrelation.getInfo(context, PLFidArry, Selectables);
             * 
             * for (int count=0; count<ml.size(); count++) { Map nmp = (Map)ml.get(count); String connectedid = (String)nmp.get(DomainConstants.SELECT_FROM_ID);
             * 
             * if (connectedid.equalsIgnoreCase(variantId)) { String PFLid = (String)PLFidArry[count]; domainrelation.setAttributeValue(context, PFLid, strFeatureAllocationType, value); } }
             * 
             * 
             * }
             */} catch (Exception e) {
    ...
    /**
     * This method is used to show all the Product Variant This is called from getProductVariantsSelectedFeatures method
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds the FormBean contents
     ...
     * @since R211
     */
    public List getDynamicColumnPFLUsageForView(Context context, String[] args) throws Exception {
        matrix.util.List PFLusageList = new StringList();
        Object PFLids;
        ...
        try {
            ...
            for (int i = 0; i < ObjectList.size(); i++) {
                ...
                // PFL ids
                PFLids = (Object) mp.get(
                        "to[" + ConfigurationConstants.RELATIONSHIP_LOGICAL_FEATURES + "]" + ".tomid[" + ConfigurationConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "]." + DomainConstants.SELECT_ID);
    ...
    /**
     * This method is used to make the cell value editable/non editable depending upon connections. If PFL connection is present then its editable else its non editable.
     * @param context
    ...
    public List getPFLUsageofProductVariant(Context context, String[] args) throws Exception {
        matrix.util.List PFLusageList = new StringList();
        try {
            Map programMap = (Map) JPO.unpackArgs(args);
            MapList ObjectList = (MapList) programMap.get("objectList");
            ...
            for (int i = 0; i < ObjectList.size(); i++) {
                Map mpLF = (Map) ObjectList.get(i);
                StringList slProductIds = util.convertObjToStringList(context, mpLF.get("tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].from.id"));
                int columIndex = slProductIds.indexOf(columnID);
                StringList slDesignUsage = util.convertObjToStringList(context,
                        mpLF.get("tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].attribute[" + ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE + "]"));
    ...
    /**
     * gets rows to view productVariant grid and pass it on to GridComponent View product Variants
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
    ...
    public MapList getLogicalStrutcureForProductVariantGrid(Context context, String[] args) throws Exception {

        MapList mapLogicalStructure = null;
        try {
            String cellIdSelect = "tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].from.id";
            String cellvalueSelect = "tomid[" + ProductLineConstants.RELATIONSHIP_PRODUCT_FEATURE_LIST + "].attribute[" + ConfigurationConstants.ATTRIBUTE_FEATURE_ALLOCATION_TYPE + "]";

            mapLogicalStructure = (MapList) JPO.invoke(context, "LogicalFeatureBase", args, "getLogicalFeatureStructure", args, MapList.class);
    ...
    /**
     * gets Column to view product Variant grid and pass it on to GridComponent Viewe Product Variants
     * @param context
     *            the eMatrix <code>Context</code> object
     * @param args
     *            holds input arguments.
    ...
    public MapList getProductVariantForGrid(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            colList = new DomainObject(stprObjectId).getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCT_VERSION, ConfigurationConstants.TYPE_PRODUCT_VARIANT, strListSelectstmts,
                    new StringList(), false, true, (short) 0, strSelIdsWhrClause, "", 0);
