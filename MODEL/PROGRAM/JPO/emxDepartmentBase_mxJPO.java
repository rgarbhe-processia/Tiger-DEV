...
/**
 * @author SG2 The <code>${CLASSNAME}</code> class/interface contains ...
...
public class emxDepartmentBase_mxJPO extends emxOrganization_mxJPO {
    ...
    /**
     * Post process JPO for create department. Here it do some basic checks like Dept Name, Dept ID, and Cage Code are unique for the org where this dept is created Also connects the dept to the
     * organization
     */
    ...
    /**
     * Post process JPO for edit department. Here it do some basic checks like Dept Name, Dept ID, and Cage Code are unique for the org where this dept is created
     */
    ...
    public Map editDepartmentPostProcess(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            String messageKey = validateUniqueness(context, (String) deptDetails.get(PARANT_OBJECT_SELECT), oldName, oldDeptId, oldCageCode, false, !deptId.equals(oldDeptId),
                    cageCodeRequired && !cageCode.equals(oldCageCode));
    ...
    /**
     * Helper method for checking whether dept name, id and cage code are unique with in the organization or not. If any of them are not unique retrun the error string resoruce key to displayed back
     * to the user.
     * @param context
     * @param parentOID
     * @param deptName
    ...
    protected String validateUniqueness(Context context, String parentOID, String deptName, String departmentId, String cageCode, boolean checkName, boolean checkDeptId, boolean checkCageCode)
            throws FrameworkException {
        ...
        try {
            ...
            query.append("print bus ").append(parentOID).append(" select from[").append(relDepartment).append("].to.id dump ,");
            String existingIDs = MqlUtil.mqlCommand(context, query.toString());
            ...
            if (existingDeptIds.length != 0) {
                ...
                messageKey = nameExists ? "emxComponents.Department.DepartmentAlreadyExists"
                        : deptIdExists ? "emxComponents.CreateOrEditOrganization.OrganizationIdAlreadyExists" : cageCodeExists ? "emxComponents.CreateOrEditCompany.CageCodeAlreadyExists" : null;
    ...
    /**
     * Expand program for select Department Displays All the Business Units and Departments in the selected organization. It will disable Business Units from selection Also If isFrom and
     * relationshipName names are passed and if the context object is already connected to a Department it will be disabled for selection.
     */
    ...
    public MapList selectionDepartmentExpandProgram(Context context, String[] args) throws FrameworkException {
        try {
            ...
            if (!UIUtil.isNullOrEmpty(isFrom) && !UIUtil.isNullOrEmpty(relationshipName)) {
                ...
                SELECT_CURRENT_OBJECT_ID = "true".equalsIgnoreCase(isFrom) ? "from[" + relationshipName + "].to.id" : "to[" + relationshipName + "].from.id";
                isAddExisting = true;
            }
            ...
            MapList mapList = object.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(), false, true, (short) 1, objSel, null, "current=='Active'", null, 0, null, null,
                    null);
