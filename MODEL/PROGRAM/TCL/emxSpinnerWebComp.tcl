         ...
         if {$bAdd} {
			...
			if {$bDeleteFlag} {
				...
				if {$bDelColumn} {
					...
					puts " \n To delete table column all excel columns except column name and table name must be blank and column name must be enclosed within delete tag << >>"
				}
			} else {
					pPlanActualAddDel $lsColumnNameActual "" $lsColumnNamePlan "table" "" $aCol(0) "column delete name" "column name" ""
			}
				...
				if {$sPrevForm != $aCol(0) || $sLastElement == "TRUE"} {
					...
					set sQuery ""
					 foreach sColumnNameP $lsFinalPlanOrder {
						...
						# PSS : INTEGRATION : 21/03/2017 : START
						...
						#}
						# PSS : INTEGRATION : 21/03/2017 : END
					  }
			...
			if {$lsColumnNamePlan != $lsColumnNameActual} {
				if {$bDeleteFlag} {
					...
					if {$bDelField} {
						...
						puts " \n To delete webform field all excel columns except field name and webform name must be blank and field name must be enclosed within delete tag << >>"
					}
				} else {
					pPlanActualAddDel $lsColumnNameActual "" $lsColumnNamePlan form "" $aCol(0) "field delete name" "field name" ""
				}
